// Polls the backend for received messages and lets the user publish new
...
function setBroker(state) {
  ...
  if (state === 'connected') {
    ...
    pill.textContent = 'NATS 已连接';
  } else if (state === 'down') {
    pill.classList.add('bad');
    pill.textContent = '后端不可用';
  } else {
    pill.textContent = 'NATS 连接中';
  }
}
...
async function poll() {
  try {
    ...
    if (items.length === 0) {
      el('msg-list').innerHTML = '<li class="empty">还没有消息，先发布一条试试</li>';
    } else {
      el('msg-list').innerHTML = items
        .map(
          (m) => `<li>
...
el('send-form').addEventListener('submit', async (event) => {
  ...
  try {
    ...
    if (res.ok) {
      input.value = '';
      hint.className = 'hint';
      hint.textContent = '已发布，订阅者会即时接收并回显。';
      setTimeout(poll, 300);
    } else {
      const data = await res.json().catch(() => ({}));
      hint.className = 'hint bad';
      hint.textContent = `发布失败: ${data.error || res.status}`;
    }
  } catch (error) {
    hint.className = 'hint bad';
    hint.textContent = `发布失败: ${error.message || error}`;
  }
});
