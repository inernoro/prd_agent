...
const CDS_AGENT_CAPABILITY_SEEDS = [
  ...
  ['cds-system-sealed-storage', 'CDS 密封存储初始化', 'cds-system-sealed-storage.ts', 'system', 'human-only', 'destructive', 'guided', 'cds', '', '只允许全局人工管理员同源初始化；服务端生成密钥且永不接受或回显客户端密钥材料'],
  ...
  ['identity', '身份、用户级凭证与项目授权', 'identity.ts', 'system', 'system', 'write', 'guided', 'cds', 'connect', '用户级凭证只能建项目/列项目/为已授权项目签钥，可达路由一处收口；签发、撤销与授权变更留痕，撤源头级联撤下游'],
  ...
  ['infra-credential-rotation', '基础设施凭据轮换', 'infra-credential-rotation.ts', 'project', 'project', 'destructive', 'guided', 'cds', '', '仅在密封存储就绪、维护作业排空并完成可恢复备份后轮换；必须验证旧凭据撤销和业务就绪'],
  ...
  ['replica-sets', '复制集成员、隔离库与执行计划', 'replica-sets.ts', 'project', 'project', 'destructive', 'direct', 'cds-deploy-pipeline', '', '加副本/调权重可逆；解散、隔离库、回切涉及数据库克隆与容器拆除，执行前必须锁定分支与服务'],
...
export const AGENT_MISSION_CAPABILITY_BINDINGS: Record<AgentPageContextId, readonly string[]> = {
  ...
  settings: ['comment-template', 'cds-system-connections', 'cds-system-offsite-backup', 'cds-system-sealed-storage', 'peer-sync', 'remote-hosts'],
  'project-onboarding': ['projects', 'pending-import', 'project-compose'],
  'code-review': ['project-compose', 'deployment-runs'],
  branches: ['branches', 'deployment-runs', 'deployment-versions', 'replica-sets', 'infra-credential-rotation'],
  ...
  'project-settings': ['projects', 'project-compose', 'project-storage', 'project-db-isolation', 'db-probe', 'db-ledger', 'comment-template'],
  'service-integration': ['branches', 'infra-data', 'infra-backup', 'project-infra-resync', 'remote-hosts'],
  'env-diagnostics': ['projects', 'branches', 'db-probe'],
