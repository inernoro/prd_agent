...
NGINX_BASE
  ...
  site_content=$({
    ...
    echo ""
    # B'.5.1 蓝绿改造:cds_master upstream 不再 inline,改用 include 引用
    # cds-active-upstream.conf。daemon 蓝绿切换时改这个文件 + nginx -s reload
    # 让流量切到新端口,而不需要重新生成整个 nginx.conf。
    # 文件不存在时(运维首次启用前)用 fallback 兜底,nginx -t 不会报错。
...
EOF
  ...
  # B'.5.1 蓝绿改造:确保 cds-active-upstream.conf 存在(初值指向当前 master 端口)。
  # 必须在 docker compose up 之前存在 — bind mount 一个不存在的文件 docker 会
  # 创建一个目录,后续 nginx 启动时挂载错乱。daemon bootstrap 同样会兜底创建
  # 这个文件,但 init / first start 流程从这里就保证存在。
  local active_upstream_file="$NGINX_DIR/cds-active-upstream.conf"
  if [ ! -f "$active_upstream_file" ]; then
    cat > "$active_upstream_file" <<EOF
# Generated by exec_cds.sh — auto-managed by CDS daemon (blue-green supervisor)
