...
case "$CMD" in
  ...
  master-run)
    ...
    info "[master-run] CI=true pnpm install --frozen-lockfile --prefer-offline --config.confirmModulesPurge=false"
    ...
    if ! CI=true pnpm install --frozen-lockfile --prefer-offline --config.confirmModulesPurge=false; then
