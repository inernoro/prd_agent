...
forwarder_runtime_signature() {
  ...
  local repo_root="${CDS_REPO_ROOT:-$(cd "$root/.." 2>/dev/null && pwd)}"
  local git_sig=""
  if command -v git >/dev/null 2>&1 && git -C "$repo_root" rev-parse --is-inside-work-tree >/dev/null 2>&1; then
    git_sig="$(
      git -C "$repo_root" ls-tree -r HEAD -- \
        cds/src/forwarder-main.ts \
        cds/src/widget-script.ts \
        cds/src/forwarder 2>/dev/null | hash_stream
    )"
    if [ -n "$git_sig" ]; then
      printf '%s\n' "$git_sig"
      return 0
...
NGINX_BASE
  ...
  site_content=$({
    ...
    echo ""
    # cds_master upstream 通过 include cds-active-upstream.conf 引用 ——
    # 这样改 master 端口时只改这一个文件 + nginx -s reload 即可,不需要重新
    # 生成整个 nginx.conf。文件不存在时(首次启用前)用 fallback 兜底。
    if [ -f "$NGINX_DIR/cds-active-upstream.conf" ]; then
      echo "include /etc/nginx/cds-active-upstream.conf;"
    else
...
services:
  nginx:
    ...
    volumes:
      ...
      - ./cds-active-upstream.conf:/etc/nginx/cds-active-upstream.conf:rw
...
EOF
  ...
  # 确保 cds-active-upstream.conf 存在(初值指向当前 master 端口)。必须在
  # docker compose up 之前存在 —— bind mount 一个不存在的文件 docker 会
  # 创建一个目录,后续 nginx 启动时挂载错乱。
  local active_upstream_file="$NGINX_DIR/cds-active-upstream.conf"
  if [ ! -f "$active_upstream_file" ]; then
    cat > "$active_upstream_file" <<EOF
# Generated by exec_cds.sh
upstream cds_master { server 127.0.0.1:${master}; keepalive 8; }
EOF
    info "已创建初始 cds-active-upstream.conf (port=${master})"
  fi
...
case "$CMD" in
  ...
  install-systemd)
    ...
    ;;
  nginx-render)
    # 重新渲染 cds/nginx/{nginx.conf,cds-site.conf}。daemon self-update 后
    # 如果 nginx 配置模板变了(例如新增 forwarder 端口、新增域名),
    # 需要 sudo nginx -s reload 才能让 nginx 看到新模板。
    load_env
    render_nginx
    ;;
