...
| 唯一术语 | 含义 | 实体位置 |
|---|---|---|
| **CDS 系统设置** | 整个 CDS 实例的设置入口 | `cds/web-legacy/cds-settings.html`（迁移后将到 `cds/web/src/pages/CdsSettingsPage.tsx`） |
| **CDS 全局变量** | 所有项目共享的 `_global.customEnv`（系统设置的一部分） | 系统设置页里的一个 tab |
| **项目设置** | 单个项目的设置入口 | `cds/web-legacy/settings.html?project=<id>` |
