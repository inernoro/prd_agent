...
<main class="shell"><section class="content">
  ...
  <div class="status"><span class="chip">后台同步 · 自动重试</span><span class="chip">持续检测服务恢复</span></div>
...
(function(){
  ...
  function probe(){
    var probeUrl=new URL(originalUrl);
    probeUrl.searchParams.set('__cds_wait_probe',String(Date.now()));
    fetch(probeUrl.toString(),{
      cache:'no-store',
      credentials:'same-origin',
      headers:{Accept:'text/html','X-CDS-Waiting-Probe':'1'}
      ...
      scheduleProbe(2000);
    }).catch(function(){scheduleProbe(2000);});
  }
  tick();
