 ...
 *     只认一种写法的判据会静默永不命中。
 *   - 形状 3（判据分裂）：严重度归一化**不在本文件定义**，从 acceptance-severity 复用
 *     同一份（那边是阻断告警判据的所在）。两份各自演化的后果是「简报统计到 2 个 P0，
 *     而阻断告警从未响过」——两边都不报错，谁也看不出自己错了。
 *   - 形状 6（读到的不是生效的值）：一篇报告同时有 defectRows（逐行证据）与
 *     defectCounts（聚合数字）时，两者**不相加**，以 defectRows 为准；否则同一个缺陷
 *     会被数两遍，而两个来源都真实存在、日志里看不出异常。
 */

import { normalizeSeverity } from './acceptance-severity.js';
...
/**
 * 严重度归一化：**从 acceptance-severity 复用同一份判定**，本模块不再自持。
 *
 * 这里曾经有一份独立实现。它与告警侧那份逻辑一致，但一致是当时的巧合而非约束——
 * 任何一侧日后收紧/放宽键名匹配，就会出现「简报统计到 2 个 P0，而阻断告警从未响过」
 * 的分歧：两边都不报错，也没有任何一侧看得出自己错了（形状 3）。
 * 保留同名再导出，让本模块既有调用方与测试无需改动。
 */
