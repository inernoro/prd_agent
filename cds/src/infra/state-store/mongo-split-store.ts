...
export class MongoSplitStateBackingStore implements StateBackingStore {
  ...
  async init(): Promise<void> {
    ...
    const [globalDoc, projectDocs, branchDocs, deploymentRunDocs, deploymentVersionDocs, selfUpdateDocs] = await Promise.all([
    ...
    if (!globalDoc && projectDocs.length === 0 && branchDocs.length === 0 && deploymentRunDocs.length === 0 && deploymentVersionDocs.length === 0 && selfUpdateDocs.length === 0) {
  ...
  async seedIfEmpty(state: CdsState): Promise<boolean> {
    const [globalCount, projectsCount, branchesCount, deploymentRunsCount, deploymentVersionsCount, historyCount] = await Promise.all([
    ...
    if (globalCount > 0 || projectsCount > 0 || branchesCount > 0 || deploymentRunsCount > 0 || deploymentVersionsCount > 0 || historyCount > 0) return false;
