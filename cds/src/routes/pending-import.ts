...
export function createPendingImportRouter(deps: PendingImportRouterDeps): Router {
    ...
    try {
      ...
      if (check) {
        const bad = check.buildProfiles
          // 预构建镜像站点（cds.prebuilt-image，如 llmgw/web 纯 nginx 前端）没有 command 是合法的：
  ...
  router.post('/pending-imports/:id/approve', (req, res) => {
    ...
    const scopedProfiles: BuildProfile[] = normalizeProjectProfileDependencies(
      (parsed.buildProfiles as BuildProfile[]).map((profile) => ({ ...profile, id: `${profile.id}${idSuffix}`, projectId: project.id })),
      idSuffix,
    );
