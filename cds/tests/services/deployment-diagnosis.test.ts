...
describe('DeploymentDiagnosisService', () => {
  ...
  afterEach(async () => {
    await Promise.all(states.splice(0).map((state) => state.flush()));
    for (const dir of dirs.splice(0)) fs.rmSync(dir, { recursive: true, force: true });
  });

  async function setup(provider?: { explain(facts: DeploymentExplanationFacts): Promise<{ summary: string; actions: string[] }> }) {
    ...
    await runs.begin({ projectId: 'p1', branchId: 'b1', trigger: 'manual' });
    runs.append('dr_1', {
  ...
  it('builds a deterministic explanation from terminal facts and evidence references', async () => {
    const diagnosis = (await setup()).deterministic('dr_1');
  ...
  it('passes only redacted structured facts to the AI provider', async () => {
    ...
    const diagnosis = await (await setup({
      async explain(facts) {
        received = facts;
