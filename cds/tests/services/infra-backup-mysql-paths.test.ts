...
describe('mysql 下载走真正的导出', () => {
  ...
  it('下载用与周期备份同一段 mysqldump 脚本，不是 tar /data', () => {
    ...
    const mysqlBranch = SRC.slice(SRC.indexOf("} else if (kind === 'mysql' || kind === 'postgres') {"));
  ...
  it('文件名按真实格式命名 .sql.gz', () => {
    expect(SRC).toContain("kind === 'mysql' || kind === 'postgres' ? 'sql.gz'");
  });
});
...
describe('mysql 恢复入口', () => {
  it('恢复有 mysql 分支，不再一律 400', () => {
    ...
    expect(restorePart).toContain("} else if (kind === 'mysql' || kind === 'postgres') {");
  });
  ...
  it('大 dump 走文件 + docker cp，不走 stdin 字符串', () => {
    ...
    const restorePart = SRC.slice(SRC.indexOf("} else if (kind === 'mysql' || kind === 'postgres') {"));
