...
describe('reconcileStuckDeployStates — TYPE 1 极速版镜像落后 HEAD 告警', () => {
  ...
  it('already-alarmed branch skips diffRuntimePaths entirely (perf: no repeat execSync)', () => {
    // 2026-07-21 性能事故回归：diffRuntimePaths 在 master 是 execSync(git diff)，
    // 同步阻塞事件循环。幂等判断必须在 diff **之前**——同一 target..head 的告警
    // 已落过时，每个 5 分钟 tick 都不许再跑 diff（旧写法反复白跑，多条 stale
    // 分支叠加即一次冻结服务数十秒）。
    const now = new Date('2026-06-27T10:00:00.000Z');
    const branch = makeBranch({
      ciImageStatus: 'ready',
    ...
    const opts = {
      now,
      getBuildProfiles: () => PREBUILT_PROFILES,
      diffRuntimePaths: () => { diffCalls += 1; return true; },
    };

    // 第一轮：告警落盘，diff 跑 1 次。
    const first = reconcileStuckDeployStates([branch], opts);
    expect(first).toHaveLength(1);
    expect(diffCalls).toBe(1);

    // 后续轮次：ciImageError 已等于 reason → 直接跳过，diff 不再被调。
    const second = reconcileStuckDeployStates([branch], opts);
    const third = reconcileStuckDeployStates([branch], opts);
