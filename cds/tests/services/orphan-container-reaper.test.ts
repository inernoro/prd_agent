...
describe('sweepOrphanCdsContainers', () => {
  it('stops running orphan infra + app containers, spares known owners', async () => {
    ...
    mockPs(
      shell,
      [
        'cds-infra-known-mysql|running|cds.managed=true,cds.type=infra,cds.service.id=mysql',
        'cds-infra-ghost-redis|running|cds.managed=true,cds.type=infra,cds.service.id=redis',
      ],
      [
        'cds-live-api|running|cds.managed=true,cds.type=app,cds.branch.id=live-branch,cds.profile.id=api',
        'cds-ghost-api|running|cds.managed=true,cds.type=app,cds.branch.id=deleted-branch,cds.profile.id=api',
      ],
    );
  ...
  it('records already-stopped orphans without issuing docker commands', async () => {
    const shell = new MockShellExecutor();
    mockPs(shell, ['cds-infra-ghost|exited|cds.managed=true,cds.type=infra,cds.service.id=x'], []);
  ...
  it('never touches the protected system state-mongo container', async () => {
    const shell = new MockShellExecutor();
    mockPs(shell, ['cds-infra-cds-state-mongo|running|cds.managed=true,cds.type=infra,cds.service.id=cds-state-mongo'], []);
    shell.addResponsePattern(/^docker stop /, () => ({ stdout: 'ok', stderr: '', exitCode: 0 }));
  ...
  it('skips entirely when state looks empty (load failure / fresh install guard)', async () => {
    const shell = new MockShellExecutor();
    mockPs(shell, ['cds-infra-ghost|running|cds.managed=true,cds.type=infra,cds.service.id=x'], []);
    const result = await sweepOrphanCdsContainers({
      shell,
