...
3. 新增备用 Offering，目标选择 OpenRouter 或另一条兼容 Endpoint，优先级填 20。若使用 OpenRouter 专用图片 API，协议填 `openrouter-image`、Endpoint path 填 `images`；若真实上游模型名不同，在“上游模型覆盖”填写供应商精确 id。
...
- OpenRouter 返回“没有支持图片输出的端点”或“输入至少需要 1 个 token”：先核对该 Offering 是否误用了旧 `openrouter` 协议。专用图片模型应使用 `openrouter-image` + `images`，不能继续发 `chat/completions + modalities`。
