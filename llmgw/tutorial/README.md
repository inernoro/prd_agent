...
检查会确认 0 至 32 章连续存在、每章具备完整操作与排错结构、下一章衔接正确，并拒绝图片占位符、疑似明文密钥和请求自报 `tenantId`。本轮视觉增密已经形成 132 个不同图片地址、222 个功能证据映射；修订后的 253 个编号步骤均在步骤下方直接跟图，共 292 次内联图片。全部截图逐张读回，不用“图片总数”代替功能覆盖。
