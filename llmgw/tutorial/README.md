...
检查会确认 0 至 32 章连续存在、每章具备完整操作与排错结构、下一章衔接正确，并拒绝图片占位符、疑似明文密钥和请求自报 `tenantId`。主线之外的“实战教程”目录按真实问题组织，当前覆盖逻辑模型目录、多上游 Offering、异构协议故障切换、限流、日志验证和三分钟 Quickstart。模型池只作为未显式选择时的默认与兼容路径。本轮视觉增密已经形成 136 个不同图片地址、222 个功能证据映射；修订后的 255 个编号步骤均在步骤下方直接跟图，共 294 次内联图片。全部截图逐张读回，不用“图片总数”代替功能覆盖。
