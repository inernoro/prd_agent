...
key、appCaller、逻辑模型和默认模型池都能影响请求，但生命周期不同。key 泄露时只撤销调用身份；业务用途变化时改 appCaller 治理；应用明确选择逻辑模型时由 Offering 路由；没有明确选择时才调整默认池。若把这些对象绑成一个，轮换密钥会意外改变路由，替换上游又可能要求所有应用发版。
...
- 想限制整个租户却只找到 appCaller：到“预算与用量”设置独立的租户总预算和总 RPM；不要把各 appCaller 预算相加后冒充租户总上限。
