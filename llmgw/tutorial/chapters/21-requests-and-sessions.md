...
4. 切到“请求与响应”，核对 Request ID、Generation ID、Key 前缀、流式状态和请求内容。图片生成记录的“响应内容”应先显示可打开的安全缩略图、图片数量、媒体类型和文件大小；Base64 只属于客户端原始协议响应，日志正文必须脱敏。缩略图由后台异步保存，状态从 pending 变成 stored，不得延迟用户收到上游响应。需要交给同事排查时只复制 requestId；不要复制整页原始数据，也不要把可能包含业务内容的请求正文贴到公开群。
