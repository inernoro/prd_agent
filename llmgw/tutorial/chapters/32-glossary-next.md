...
3. 记住“逻辑模型、Offering、默认池”：逻辑模型是应用选择，Offering 是 Gateway 的多上游供应线路，默认池只处理没有显式选择的请求；Provider、Endpoint 和密钥都留在 Gateway。
...
你能不用猜测回答五个问题：当前属于哪个租户，谁和哪把 key 发起请求，哪个 appCaller 与逻辑模型或默认池负责路由，实际命中了哪个 Offering，费用证据处于哪一阶段以及谁修改过配置。遇到故障时知道回到哪一章。
...
- 准备接入新平台：从[[第 9 章：创建第一个 appCaller|第 9 章]]重新创建独立 appCaller，再按第 10、11、12、31 章生成 key、安全测试和小流量切换。
...
全书到这里完成。下一步回到模型网关首页，按“健康状态、Quickstart、最近请求、费用可信度”的顺序完成一次周检；新增接入方时，从[[第 9 章：创建第一个 appCaller|第 9 章]]重新走独立身份链路。
