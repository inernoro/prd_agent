...
2. 在顶部唯一的 `requestId` 搜索框粘贴标记为 chat 的完整编号，按 Enter 或点击“查找”。输入过程中不会反复请求；提交后如果当前租户只匹配一条记录，浏览器会进入带独立路径的“请求详情”页，不需要再点表格行。也可以直接点击任意日志行进入同一个详情页，浏览器后退仍回到原筛选结果。
...
3. 返回列表先横向核对时间、请求 ID、模型、Provider、App、输入、输出、费用、用途、速度、结束原因、客户端用户和状态。列表正文使用与控制台一致的无衬线字体和可读字号，不应在模型与 Provider 之间出现大片无信息空白。点击 App 会进入该 appCaller 的独立配置与治理页面；返回后再打开同一条请求，在“概览”页签核对实际模型、Provider、token、费用、速度、总耗时和每次上游响应。
...
7. 点击“返回请求记录”，在同一个搜索框换成标记为 vision 的 requestId 并再次提交。新的精确匹配会进入另一条独立详情路径；核对 vision appCaller、另一条 Key 前缀、内容类型和路由。返回列表后不会因为同一个筛选值反复跳转，只有提交新的 requestId 才会再次进入详情。
...
requestId 提交后精确命中一条当前租户日志并进入独立详情路径；点击日志行和点击 App 分别进入请求详情与 appCaller 页面。列表列宽连续、字号可读，OpenRouter 常用的输入、输出、费用、用途、速度、结束原因和客户端用户均直接可见。请求、上游调用、会话三个列表视图都有真实数据来源，四个详情页签分别回答结果、内容、路由和审计问题。可见身份字段与 Quickstart 配置一致，日志不含 key 明文；安全直测明确显示未调用上游，unknown 费用保持未知。
