...
<!-- tutorial-step: governance-budget-usage -->
...
4. 读 unknown：它表示当前证据不足。记下缺价格请求数量，返回模型页补齐价格来源后，只影响有明确规则的后续或重算流程。
   ...
   对图片生成尤其要区分三种证据：上游响应中的 `usage.cost` 是逐请求实际值；模型配置中的每次调用价或 token 单价用于 Gateway 估算；两者都缺失时只能标为未计价。图片成功数和耗时可以证明请求完成，但不能单独推导金额。
