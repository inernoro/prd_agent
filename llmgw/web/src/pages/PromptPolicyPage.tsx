    ...
    <div style={{ maxWidth: 1040, margin: '0 auto', display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <section style={card}>
        <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap' }}>
          <div><strong>当前版本</strong><div style={hint}>{data?.current ? `v${data.current.version} · ${data.current.policyHash.slice(0, 12)} · ${data.current.policyChars} 字符` : '尚未创建'}</div></div>
        ...
        <div className="lg-prompt-policy-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12, marginTop: 12 }}>
          <TextArea label="System Prompt 前缀" value={draft.systemPromptPrefix} onChange={(value) => setDraft({ ...draft, systemPromptPrefix: value })} />
          <TextArea label="System Prompt 后缀" value={draft.systemPromptSuffix} onChange={(value) => setDraft({ ...draft, systemPromptSuffix: value })} />
        </div>
        <div style={{ marginTop: 12 }}><div style={label}>允许变量</div><div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 6 }}>{VARIABLES.map((name) => <label key={name} style={checkLabel}><input type="checkbox" checked={draft.allowedVariables.includes(name)} onChange={(e) => setDraft({ ...draft, allowedVariables: e.target.checked ? [...draft.allowedVariables, name] : draft.allowedVariables.filter((x) => x !== name) })} />{`{{${name}}}`}</label>)}</div></div>
        ...
        <p style={hint}>合并顺序固定为策略前缀、请求自身 system prompt、策略后缀。首版只应用 chat/vision；raw、图片、视频和 ASR 不注入。日志仅保存策略 id、版本、hash 和字符数。</p>
      ...
      <section style={card}><h2 style={heading}>预览</h2><TextArea label="示例请求 system prompt" value={sample} onChange={setSample} />
        {preview ? <div style={{ marginTop: 10 }}><div style={{ display: 'flex', gap: 7, flexWrap: 'wrap' }}><Chip label={`${preview.policyChars} 策略字符`} color="#58a6ff" bg="rgba(88,166,255,.14)" /><Chip label={`${preview.mergedChars} 合并字符`} color="#58a6ff" bg="rgba(88,166,255,.14)" /><Chip label={preview.policyHash.slice(0, 12)} color="var(--text-primary)" bg="var(--bg-elevated)" /></div><pre style={pre}>{preview.mergedSystemPrompt || '空 system prompt'}</pre></div> : <p style={hint}>修改草稿后点击预览；预览不保存、不调用付费模型。</p>}
      </section>
      <section style={card}><h2 style={heading}>版本历史</h2>{data?.versions.length ? <div style={{ overflowX: 'auto' }}><table style={{ width: '100%', borderCollapse: 'collapse' }}><thead><tr>{['版本','状态','hash','字符','更新时间','操作'].map((x) => <th key={x} style={th}>{x}</th>)}</tr></thead><tbody>{data.versions.map((item) => <tr key={item.id}><td style={td}>v{item.version}</td><td style={td}>{item.enabled ? '启用' : '禁用'}</td><td style={td}><code>{item.policyHash.slice(0, 12)}</code></td><td style={td}>{item.policyChars}</td><td style={td}>{item.updatedAt ? new Date(item.updatedAt).toLocaleString() : '—'}</td><td style={td}><Button size="sm" variant="ghost" disabled={busy || item.version === data.current?.version} onClick={() => void rollback(item.version)}>回滚</Button></td></tr>)}</tbody></table></div> : <p style={hint}>暂无版本。</p>}</section>
    </div>
  </div>;
}
