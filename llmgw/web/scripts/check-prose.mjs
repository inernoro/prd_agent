...
const BUDGETS = [
  ...
  { file: 'pages/QuickstartPage.tsx', maxParagraphs: 2, maxCjk: 628, reason: '四协议接入片段常量属产品内容，且部分渲染在 HelpPopover 内。2026-08-25 主线改版后 421→628：新增的 207 字全部来自 FAILURE_RULES —— 15 条失败码各自的一句归因与下一步文案（`error.code` → 哪一环坏了）。它们是**条件渲染**：一次失败只会显示其中一条，成功路径一条都不出现，页面常驻可见的解释反而比改版前少（三处「工作原理」已合并成一处折叠块，四步说明压到标签级）。守卫按常量字面量计数、看不出常量最终落到哪个出口（本文件顶部「已知边界」写明的那一类），把这些归因收进折叠块等于让用户失败时读不到下一步，因此按实测值封顶登记，只减不增' },
];
