...
const BUDGETS = [
  ...
  { file: 'pages/ModelPoolsPage.tsx', maxParagraphs: 4, maxCjk: 558, reason: '2026-08-18 信息架构改版后重新登记。四段里只有副标题对所有人常驻，其余三段都有条件：补齐面板的写操作警告只在展开确认面板时出现、ReadOnlyNotice 只对只读角色出现、六种策略说明是常量（渲染在 HelpPopover 与新建向导的策略卡里）。汉字数上升的主体是**诊断信息本身**——池状态结论句、第1顺位连续失败与最近失败/成功时间、指标的窗口标签，它们是后端早就返回、此前一个都没显示的字段，正是这次改版要露出来的东西，收进折叠块就等于没改。守卫按 `>文本<` 计数，分不出「常驻说明」与「按数据渲染的结论」，这里如实登记而不是把结论藏起来凑数' },
