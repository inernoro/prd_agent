...
const BUDGETS = [
  ...
  { file: 'pages/QuickstartPage.tsx', maxParagraphs: 1, maxCjk: 356, reason: '四协议接入片段常量属产品内容，且部分渲染在 HelpPopover 内。2026-08-27 三步向导改版后 386→356 / 2 段→1 段（棘轮继续下调）：用途预设与它那段格式说明整块删掉，改成第一步的一句「我想要做什么」——判定用的关键词表与场景名搬去 lib/appCallerIntent.ts，不再占页面文字预算；调用类型的两句解释随之删除（类型现在由这句话推出来，不需要再讲）。剩下的字里 FAILURE_RULES 的 15 条失败归因仍是条件渲染（一次失败只显示一条，成功路径一条不出），常驻可见的那一段是主按钮上方的密钥默认值说明。守卫按常量字面量计数、看不出常量最终落到哪个出口，因此按实测值封顶登记，只减不增' },
];
