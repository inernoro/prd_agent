...
const BUDGETS = [
  ...
  { file: 'pages/QuickstartPage.tsx', maxParagraphs: 2, maxCjk: 386, reason: '四协议接入片段常量属产品内容，且部分渲染在 HelpPopover 内。2026-08-26 两屏改版后 628→386（棘轮下调）：创建屏只剩三个决定，四步说明、三处解释块与右栏空态清单全部删除或收进折叠块；剩下的字里 FAILURE_RULES 的 15 条失败归因仍是条件渲染（一次失败只显示一条，成功路径一条不出），常驻可见的两段分别是主按钮下方的密钥默认值说明与密钥卡的保管提醒。守卫按常量字面量计数、看不出常量最终落到哪个出口，因此按实测值封顶登记，只减不增'  },
];
