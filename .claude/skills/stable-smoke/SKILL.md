...
25. 矩阵里每一条「一次请求、只读、无副作用」的轻用例，必须**同时**登记进仓库根 `cds-monitors.yml`，作为 6 小时常设监控（`probe: light`、`intervalSeconds: 21600`）。本技能是 48 小时全量复测，常设探针是它的高频补集：**两者共用同一份判据，只是跑的频率不同**，禁止各写一份判据后各自漂移。新增永久回归用例（规则 7）时同步判断它是不是轻用例，是就一并登记。这条直接治「一测试就发现问题、系统却迟迟不反馈」——最坏情况下故障存活 6 小时，而不是等到下一轮人工复测。契约由 `scripts/tests/test_cds_monitors_contract.py` 每次 CI 校验（挂在无 path filter 的 job 上，改声明文件必跑）；规则 SSOT 是 `.claude/rules/degradation-must-alarm.md`。
...
| 标题 | 导航链接 | 验收图片 | 验收状态 |
|---|---|---|---|
| {人类可读验收项} | [打开已验证入口]({verified-navigation-url}) | [查看本轮线上证据]({verified-evidence-url}) | pass \| fail |
