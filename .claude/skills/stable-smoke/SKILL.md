...
1. 环境地址只能来自权威配置。测试环境调用 `preview-url`；正式环境读取已配置的生产目标。缺失时报告阻塞，禁止拼接或猜域名。
2. 正式环境使用专用合成账号、专用知识库、专用工作区和 `stsmk-` 数据前缀，禁止读取、修改或删除真实用户数据。
3. 测试环境跑完整正向、负向、故障恢复和清理；正式环境跑健康检查、最小真实产物和清理，不做破坏性故障注入。
...
| 模块 | 测试环境 | 正式环境 | 用例数 | 失败 | flaky | 产物 | 耗时 |
|---|---|---|---:|---:|---:|---|---:|
...
执行：解析测试与正式地址；分别跑录音上传、转录、保存、关闭后恢复，以及两图和三图生成、进度、结果和错误恢复；清理所有 `stsmk-` 资源。
...
- 环境：`preview-url` 提供测试环境真实入口，生产入口必须来自已配置发布目标。
