...
17. 视觉验收必须运行 `scripts/stable-smoke-visual-gate.mjs`。采集文件和合格证据分开统计；截图总量、模块下限、关键状态、产品实际支持的主题、桌面或真实触控移动端、证据元数据和页面判定必须同时通过。当前 prd-admin 业务页只支持暗色，LLMGW 控制台覆盖明暗主题；不得通过强制浏览器配色伪造产品不支持的主题。缺少逐张绑定的历史截图只算采集文件，不得显示为有效证据。
...
19. 视觉报告必须生成逐张证据账本。每张图都要显示所属模块、唯一主验收状态、冒烟/功能/视觉/回归类型、通过状态、主题、设备、真实面包屑、证明内容、截图跳转和测试方法跳转；一张图不能替多个主状态核销。任一字段缺失时该图不计入合格证据，模块视觉状态为未执行或需干预。
20. 全面视觉复测的合格证据硬下限为 148 张，按十个模块分别核算；单图视觉创作和多图视觉创作各不少于 18 张。主管报告必须给出每个模块的采集文件数、合格证据数、计划数和“查看全部截图”链接，禁止只展示每模块首图让审核者误以为只有十几张。
