...
def format_master_md(design_system: dict) -> str:
    ...
    if anti_patterns:
        ...
        for anti in anti_list:
            if anti:
                lines.append(f"- [NO] {anti}")
    lines.append("")
    lines.append("### Additional Forbidden Patterns")
    lines.append("")
    lines.append("- [NO] **Emojis as icons** — Use SVG icons (Heroicons, Lucide, Simple Icons)")
    lines.append("- [NO] **Missing cursor:pointer** — All clickable elements must have cursor:pointer")
    lines.append("- [NO] **Layout-shifting hovers** — Avoid scale transforms that shift layout")
    lines.append("- [NO] **Low contrast text** — Maintain 4.5:1 minimum contrast ratio")
    lines.append("- [NO] **Instant state changes** — Always use transitions (150-300ms)")
    lines.append("- [NO] **Invisible focus states** — Focus states must be visible for a11y")
    lines.append("")
...
def format_page_override_md(design_system: dict, page_name: str, page_query: str = None) -> str:
    ...
    lines.append("")
    lines.append("> [CAUTION]️ **IMPORTANT:** Rules in this file **override** the Master file (`design-system/MASTER.md`).")
