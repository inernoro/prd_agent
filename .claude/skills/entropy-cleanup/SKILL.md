...
- 若已有标题含「熵减计划」的 open PR：
  ...
  - **积压必须当轮清零，不是当轮可见**：本节跑完之后，标题含「熵减计划」的 open PR
    应当只剩两类——`[需人工] ` 前缀的，和 6.4 核查不通过的。除此之外还剩下任何一个，
    都说明本节没执行到位。**禁止**在不处置历史 PR 的前提下创建第 N+1 个新 PR。
...
- **评审熔断**：第 3 轮评审起 / 累计 5 个 review 修复提交 / 同一处措辞被反复要求改第 2 次 → 立即停止修复并合并，剩余意见记 `debt.*`（见 6.5.1）
