...
- 若 `git log origin/main..HEAD --oneline` 输出为空（当前分支没有超过 main 的 commit），**跳过 PR 创建**。
- 若当前分支是 `main`，**跳过 PR 创建**（不能从 main 向 main 发 PR）。

> **「跳过创建 PR」≠「结束任务」——6.2 与 6.2.1 无论如何都要跑。**
>
> 这两条清算是**无条件的**，不挂在「本轮有没有新提交」上。理由很直白：D1-D7 越干净、
> 本轮越可能没有净变更，而历史积压恰恰是在这种「干净日」里无人清理、一天天堆起来的。
> 把清算放在 no-change 提前返回之后，等于**积压越接近稳定态越没人管**，正好和本节目标相反
> （2026-08-14 PR #1369 第三轮评审提出，判断成立——这是标准的「建了一半」接线漏洞：
> 清算逻辑写好了，但常见路径根本走不到它）。
>
> 正确顺序：**先跑 6.2 + 6.2.1（无条件）→ 再按本节判断要不要建新 PR**。本轮无净变更时，
> 只跳过 6.3 建 PR，不跳过清算。
>
> **但这个顺序有个反作用，必须一并处理**：6.2 可能刚合并掉一个历史熵减 PR，而它修的
> 正是本轮又修了一遍的欠债（同一个 D2/D3/D6 缺口，两轮都会扫出来）。此时本轮分支的提交
> 已在 Step 5 推出去了，而 `git log origin/main..HEAD` **既不会自动刷新 origin/main，
> 也认不出 squash 后 commit id 不同但内容等价**，于是照样判「有净变更」，建出一个内容
> 已经进 main 的重复 PR。所以 6.2 有任何合并动作后，**必须重新 `git fetch origin main`
> 并按内容重判净值**（口径同 6.2.1：取本分支动过的文件比 main 端点，全一致就是没有净新增）：
> 没有净新增就跳过建 PR，并把本分支登记进 6.2.1 的待删清单
> （2026-08-14 第七轮评审提出，判断成立——这是本轮调整执行顺序引入的新耦合）。

#### 6.2 清算历史未合并的熵减 PR（在其 head 上审计，审计通过就合并掉）

> **本节的目标是让历史熵减 PR 数量归零，不是让它变得可见。** 只上报不处置等于每轮 +1。

使用 `mcp__github__list_pull_requests`（state=open, base=main）查询 `inernoro/prd_agent`。

> **先过身份核验，再谈任何处置。** 标题含「熵减计划」只是**初筛**，不是身份。对每个候选，
> 在做**合并 / 关闭 / 更新分支**中任何一个写操作之前，先跑下面「身份必须逐项核验」那四项；
> **任一项不符即整个退出本节**，只当作普通 open PR 记录一句，不合并、不关闭、不更新、
> 也不把它的分支列进任何待删清单。
>
> 把核验只放在合并那一支是不够的（2026-08-14 第九轮评审提出，判断成立）：**关闭路径同样
> 是破坏性写操作**——一个别人写的、恰好标题含「熵减计划」且内容已在 main 的 PR，会被关掉
> 并把源分支标成「已废弃」待删。授权只覆盖本技能自产物，那就必须在**所有**写路径上一致生效。

- 若已有标题含「熵减计划」、**且通过身份核验**的 open PR：
  ...
  - **其余的每一个都要在它自己的 head 上跑审计**，不是在当前分支上跑——当前分支干净
    ...
    **用 worktree，禁止直接 `git checkout <sha>`**——那会 detach 当前工作区的 HEAD，
    ...
    - 结构审计（D1-D4+棘轮）与 6.4 内容核查**都**通过，且 `mcp__github__pull_request_read`
      （method=`get`）返回的 `mergeable_state` 为 `clean`（无冲突、必需检查全绿）→ **直接
      squash 合并它**，不要留给下一轮。

      **合并前必须先读它自己的评审意见**（`pull_request_read` method=`get_review_comments`），
      按 6.5.1 分类：**有未解决的 A 类就先修掉或关闭它，不能直接合**。`mergeable_state=clean`
      只说明无冲突且必需检查绿，**不代表没有语义缺陷**——自动评审多以普通 comment 形式留存，
      不会阻塞 mergeable 状态，D1-D4/棘轮/6.4 也查不出语义问题（2026-08-14 第五轮评审提出，
      判断成立）。这与合并第 4 条闸是同一条要求，对历史 PR 一视同仁。

      > **授权来源（必须写明，别当成默认权力）**：共用规则 §5.5 的通则是「PR 默认保持
      > Ready、关闭自动合并，未经用户明确指示不得合并」。本节是那条通则下的**一处具名例外**，
      > 由用户 2026-08-14 的明确指示确立：「最近出现了无数 PR 创建分支创建没有合并的每日
      > 熵减问题…1、合并或删除分支或 pr 2、修改技能提交或删除 pr, 避免无限扩展」。
      >
      > **例外的适用边界，不得外扩**：只适用于**本技能自己产出**的熵减 PR。对任何其它 PR——
      > 别的 Agent 的功能分支、含产品代码的 PR、非本技能产出的文档 PR——§5.5 通则原样成立，
      > 一律不得自动合并。
      >
      > **身份必须逐项核验，标题不算身份**（2026-08-14 第八轮评审提出，判断成立）。只凭
      > 「标题含『熵减计划』」就套用这条例外，等于**任何人把这四个字写进标题就能让 Agent 替他
      > 自动合并** ——那是把用户窄授的例外偷偷放宽，属授权越界。核验四项，缺一即不适用例外、
      > 退回 §5.5 通则（只报告不合并）：
      >
      > 1. **作者**是本自动化身份（与本会话推送所用身份一致），不是任意用户；
      > 2. **head 分支**符合本技能的会话分支形态（`claude/<会话名>`），不是长期分支或功能分支；
      > 3. **改动范围**只含 doc / index.yml / guide.list / changelog / 技能元数据（6.4 已查）；
      > 4. **PR 正文**含 §6.3 模板的 `## 熵清理摘要` 小节。
      >
      > 第 4 项的标记**必须与 §6.3 模板实际输出的字样逐字一致**。写成模板并不输出的字样
      > （例如「D1-D7 摘要」，而模板实际只输出 `## 熵清理摘要`），会让**每一个合法熵减 PR
      > 都核验失败**：永远合不掉、又触发下面的积压阻塞，等于本技能自我关停——正好制造它
      > 要消灭的堆积（2026-08-14 第九轮评审提出，判断成立；这是上一轮加身份核验时引入的
      > 回归）。**改这一项与改 §6.3 模板必须同时进行**，任何一处单独改都会重现该故障。
      >
      > 前两项查的是**来源**，后两项查的是**内容**；6.4 只覆盖内容，所以来源必须单独查——
      > 这正是原写法的缺口。
      >
      > 为什么这处例外成立：熵减 PR 与本轮 PR 是同一类产物、走同一套 D1-D4 + 棘轮 + 6.4 核查，
      > 而 6.5 早已授权本技能自动合并当轮 PR；把同类的历史 PR 排除在外没有安全收益，只有堆积
      > 代价。实测「只上报、请用户确认」的结果是它们**永远没人确认**——用户盘点时 7 个熵减
      > 分支躺在远端，最老的 11 天，每个只是几行文档改动。
      > 「审计通过却不合并」在这一类产物上不是谨慎，是把清理欠债的技能变成产生欠债的技能。
      >
      > **历史 PR 与当轮 PR 走同一套闸，不另设更松的一套**：合并前必须满足 §6.5 那**四条**
      > 充分条件（CI 绿 + 6.4 通过 + 未命中 D4 硬闸 + **无未解决 A 类**），外加上面的身份核验。
      > 早先这里写成「只剩 D4 前缀与 6.4 两道闸」，那是漏掉第 4 条的旧说法，会让历史 PR 带着
      > 已知 A 类缺陷被合并——已删（2026-08-14 第八轮评审提出）。**判定源只有 §6.5 一处**，
      > 本节只引用不复述。若用户日后收回这条指示，本节即刻回到 §5.5 通则。
    - `mergeable_state` 为 `unstable`（无冲突，但有必需检查未过——GitHub `MergeStateStatus`
      ...
      不要混进「可合并」那一类。
    - `mergeable_state` 为 `behind`（分支落后于 main，无实质冲突只是没更新）→ **调
      `mcp__github__update_pull_request_branch` 把 main 合进来**，然后重新读一次状态：
      转 `clean` 就按上面合并，转 `dirty` 就走下面 `dirty` 那条。

      > 这里与「不擅自动别的会话的分支」不冲突：本节处理的对象**只有本技能自产的熵减 PR**
      > （6.2 开头已限定），更新它自己的分支属于本技能的自有产物维护。
      >
      > 2026-08-14 第三轮评审指出原写法造成死锁：`behind` 只给了「报告、不处理」，而
      > 下面又规定「历史 PR 没清零就禁止建新 PR」——`behind` 不会自己消失，于是本轮推完
      > 分支却永远建不出 PR，**正好制造出本 PR 要消灭的孤儿分支**。判断成立，已给终态动作。
    ...
    - 6.4 内容核查不通过（发现幽灵删除、代码文件混入、越界改动）→ 明确标注「不建议合并，
      原因：xxx」，同样只报告不处理。
    - 审计发现该 PR **每一处改动**都已被更晚的 commit 覆盖 → **调
      `mcp__github__update_pull_request` 置 `state: closed`**，关闭评论写明
      「内容已被 <sha/PR> 覆盖，无净新增价值」。不强行合并空转 PR，也不继续挂着。

      **判据是「整份无净新增」，不是「抽查到一处已覆盖」。** 一个熵减 PR 常同时含
      D2/D3/D6 多处改动，只核对 manifest 有没有那一行、或某个设计章节在不在，
      **会把仍有独有内容的 PR 当成空转关掉，还顺手把它的分支标成已废弃**。
      正确做法是走端点比对（同 6.2.1 判据）：取该 PR 动过的全部文件，逐个确认 main
      侧内容已一致；**只要还剩任何一处有差异，就不走关闭路径**，按其它分支处置
      （2026-08-14 第六轮评审提出，判断成立）。
  - **哪些状态允许留下、且不阻塞建新 PR**（必须显式列清，否则「清零」会变成死锁）：
    `[需人工] ` 前缀、6.4 核查不通过、`unstable`（CI 未全绿）、`dirty`（真冲突）——这四类
    都是**需要人判断**的终态，留在 open 是正确结果，**不构成 Step 6.3 的阻塞条件**。
    把它们连同结论写进本轮新 PR 的「已知（非本轮阻塞）」小节即可。

  - **积压必须当轮清零，不是当轮可见**：本节跑完之后，标题含「熵减计划」的 open PR
    应当只剩上面那四类「需人工判断的终态」。**除此之外**还剩下任何一个（即：审计干净、
    `clean`、却既没合并也没关闭的），都说明本节没执行到位——**这一类才禁止带着它去创建
    第 N+1 个新 PR**。四类终态不阻塞建 PR，否则清零规则本身会变成死锁。

#### 6.2.1 顺手清理熵减自己留下的分支

熵减 PR 合并后 GitHub 会自动删源分支，但**被关闭的 PR、以及历史上跑完没开 PR 的分支不会**。
每轮跑完 6.2 之后做一次：

**判据必须 fail-closed，且三点 diff 在这里是错的。** 两个坑都实测踩过（2026-08-14，PR #1369 第一轮评审抓出）：

- **仓库常是浅克隆**，`git merge-base` 找不到公共祖先时 `git diff A...B` 会 `fatal: no merge base` 并**只往 stderr 写**，stdout 是空的。于是 `[ -z "$(...)" ]` 把**命令失败读成了「无差异」**——一次实跑里 50 个分支被判「可删」，实际其中 44 个含独有工作。这就是 `predicate-and-wiring-discipline.md` 形状 4b（静默空跑）+ fail-open。
- **三点 `A...B` 语义是 `diff(merge-base, B)`**，答的是「这个分支自己改了什么」，不是「这些改动在不在 main 里」。要回答后者必须拿 main 端点比。

正确写法（先补全历史、判 merge-base 成功、再按分支碰过的文件比 main 端点）：
...
# 候选必须限定在「本技能自己造的分支」——用白名单，不是黑名单。
# 黑名单（只排 main/master/release）挡不住 develop / staging / 别的 Agent 的长期分支：
# 它们只要当前碰过的文件恰好与 main 一致就会被列进「可安全删除」，而所有权和后续复用
# 意图从没被检查过（2026-08-14 第八轮评审提出，判断成立）。
# 本技能历来用会话分配的分支名 claude/<会话名>；其余一律不进候选。
for b in $(git ls-remote --heads origin | awk '{print $2}' | sed 's|refs/heads/||' \
           | grep -E '^claude/' \
           | grep -vE '^(main|master|release/.*|develop|staging)$'); do   # 双保险：白名单后再排一次主干名
  ...
  files=$(git diff "$mb" "origin/$b" --name-only)                # 该分支自己动过哪些文件
  [ -z "$files" ] && { echo "REDUNDANT: $b"; continue; }
  # 这些文件在 main 里内容是否已经一致——一致说明分支没有 main 缺的东西
  [ -z "$(git diff origin/main "origin/$b" -- $files --name-only)" ] && echo "REDUNDANT: $b"
done
```

`REDUNDANT` 才可删：`git push origin --delete <branch>`。

**被主动关闭的熵减 PR，其源分支要单列一类「已废弃」。** 机械判据对它无能为力——分支里有
未合并内容，冗余扫描必然判「保留」，于是**每一次关闭都会永久留下一个分支**，正是本技能要
消灭的东西（2026-08-14 第四轮评审提出，判断成立）。这类分支的可删依据不是「内容冗余」而是
**「已由本技能显式判定废弃」**，所以走单独名单：关闭 PR 时把 `<branch> —— 因 <原因> 关闭
PR #N，内容已废弃` 追加进交给用户的待删清单，与 `REDUNDANT` 分开列，让用户看得出两者的
删除理由不同。

**这个判据只保证「不误删」，不保证「删干净」**：分支分叉后 main 又改过同一批文件时，它会判为保留（残差里混着 main 自己的演进）。那是有意的保守方向——漏删只是留个分支，误删是毁掉别人没合并的工作。想进一步确认某个保留项是否真冗余，用内容级证据（grep main 找该分支声称新增的关键内容）人工复核，别放宽机械判据。

> **已知限制（2026-08-14 实测）**：Claude Code 的 GitHub App 凭据**只能推自己的指定分支，
> 删除任意分支会返回 403**；GitHub MCP 也没有 delete-ref 工具。所以本步**做得到审计、
> 做不到删除**。正确做法是把 `REDUNDANT` 清单和现成命令输出给用户，由用户执行；
> **不要假装删掉了**，也不要因为删不掉就跳过审计——清单本身就是价值。
...
**合并的四条充分条件（唯一定义处，别处只许引用不许复述）**：

1. **CI 出终态且为绿**（`mergeable_state=clean`；pending 不算绿也不算红，见下）
2. **6.4 内容核查通过**
3. **Step 4.5 未命中 D4 硬闸**
4. **没有未解决的 A 类缺陷**

第 4 条不可省：6.4 只查 diff 范围与幽灵删除，**查不出语义错误**（判据 fail-open、描述与实现相反、未核实覆盖就登记 manifest 都能通过 6.4）。判定第 4 条时**必须先读本 PR 自己的评审意见**（`pull_request_read` method=`get_review_comments`）再分类——检查跑绿的同时评审可能已经贴出 A 类缺陷，不读就等于这条闸没接线。历史 PR 同样要读（6.2）。

**读一次还不够：CI 绿不代表评审跑完。** 自动评审与 CI 相互独立，PR 可以在评审还没贴出第一条意见时就 `clean`；此刻读到空、立刻合并，等于第 4 条被时序绕过（2026-08-14 第八轮评审提出）。所以推完新提交后要**有上限地等评审出结果**（等到评审对**当前 head sha** 有产出，或到自设上限）。**等到上限仍没有评审产出就走 6.6 兜底：保持 open、写明「CI 绿但评审未出结果」**，交下一轮清算——不要把「还没评」当成「没问题」。

**「机器评审意见清空」不是条件。** §5.5：不得以「清空所有机器评论」为完成标准。闸门是**没有未解决的 A 类**，不是「没有意见」。

#### 6.5.1 评审意见分类（A 必修，B/C 到点停手）

| 类别 | 判定 | 动作 |
|---|---|---|
| **A** | 可证明违反本 PR 目标 / 真实契约 / 既有测试：判据写错或 fail-open、描述与实现不符、条款自相矛盾、写了却走不到的死路径 | **任何轮次都必须修**，修完才满足第 4 条 |
| **B** | 建议合理但扩范围：引入新语义类别、兼容层、产品行为改动 | ≤2 轮正常回应，且**只记 `debt.*`、本轮不实现**；第 3 轮起连记账也停 |
| **C** | 推测性变体：只加措辞 / 同义词 / 假设输入，**找不到真实生产路径** | **从第 1 轮起就不改、也不记账**，按 §5.5 明确回「证据不足，不实现」。把 C 类塞进 `debt.*` 等于把熔断本该丢弃的东西换个地方留下来，仍然是范围扩张（2026-08-14 第九轮评审提出，判断成立） |

分类**看内容不看形式**：表面是「措辞问题」但指出上次改措辞时把事实改错了，那是 A 类。

**连续两轮仍冒新 A 类** → 这个 PR 没做对。**优先删条款而不是加条款**——规格越细，「你没规定 X」的边界情况越多，补一个冒两个；改不动就关闭、缩小范围重开。

> **两次实录**：① 2026-08-13 PR #1367，2 文件 +12 行的纯文档 PR 被拖 8 轮 15 条，后期全是措辞/版本号/文件名——B/C 陪跑的典型。② 2026-08-14 PR #1369（本节自身）走了 6 轮 18 条，13+ 条是真 A 类：先是摘要与详述漂移三次，删掉副本后又在相邻两节间复发，简化一轮后仍被指出三处「未规定的状态」。**根因是把治理规则写成了带条件分支的散文规格，其边界情况无穷**。最终靠「唯一定义 + 全函数兜底」收敛。教训：**治理规则自己更要守奥卡姆，且必须是全函数——每多一条分支就多一处可被问「那 X 呢」的接缝。**

#### 6.6 收尾与当轮落地

- 合并成功 → 记录 SHA，结束。
- `dirty` 需本地解冲突 → 解完 force-push，回 6.4 重新核查。
- **禁止**用 `sleep` 空转轮询。

**本轮 PR 必须落地**，落地状态是下表的全集——**表中未列出的一切情况，一律走最后一行的兜底**，不允许出现「没规定所以卡住」：

| 状态 | 动作 |
|---|---|
| 四条条件全满足 | **合并** |
| CI 出终态为红，且当轮修不好 | **关闭**，结论写进关闭说明，源分支登记进 6.2.1「已废弃」待删清单 |
| 命中 D4 硬闸 | 保持 `[需人工] ` 前缀**留 open**，交人工 |
| 仍有未解决 A 类 | 修完再合；连续两轮仍冒 → 关闭并缩小范围 |
| 只剩 B/C 类 | 不构成阻塞，合并 |
| **等到自设上限 CI 仍 pending，或任何上面未列出的状态** | **保持 open，不关闭也不合并**，在交付消息与 PR 里写明「卡在什么状态、下一轮从哪继续」。**pending 不是红，把它当红关掉会毁掉健康 PR 并留下孤儿分支。** 这条兜底存在的意义就是让本表成为全函数：宁可如实留一个说明清楚的 open PR，也不许因「规格没覆盖」而胡乱处置 |

「禁止过夜」约束的是**无人认领的沉默 PR**，不是这条已写明状态与续接点的兜底——后者下一轮 6.2 会接着清算它。
---
...
> **本节只做目录，不复述判据。** 2026-08-14 PR #1369 的第 2/3/4 轮评审连续三次抓到同一形状：
> 详述改对了、这里的摘要没跟上，于是两份说法互相矛盾，而只读摘要的 Agent 会照错的那份执行
> （典型如摘要写「熔断即合并」而详述要求「A 类必修」、摘要漏列 `unstable`/`dirty` 两类终态）。
> 那正是 `predicate-and-wiring-discipline.md` **形状 3「判据分裂成多份，然后各自漂移」**，
> 而它给的解法是**抽成唯一判定源**，不是每次再补一处。所以这里的规则副本已全部删除，
> 只留指针。**新增或修改任何判据只改 6.x 正文，不要在本节复述**——复述一次，就是给下一次漂移埋种。

| 关注点 | 判定源 |
|---|---|
| 标题约定 | `每日熵减计划 YYYY-WXX — <本次主要修复内容>`（这是命名格式，非判据） |
| 何时建 PR / 无净变更怎么办 | §6.1（注意：跳过建 PR ≠ 跳过 6.2/6.2.1 清算） |
| 历史熵减 PR 怎么处置、哪些允许留 open | §6.2 |
| 冗余分支怎么审计、为什么删不掉 | §6.2.1 |
| 合并前内容核查 | §6.4 |
| 合并动作与**合并的四条充分条件** | §6.5 |
| 评审熔断、A/B/C 分类、连续冒 A 类怎么办 | §6.5.1 |
| 当轮落地：合并还是关闭 | §6.6 |
