...
| 同一份文档被反复要求调措辞第 2 次 | 同上（措辞类天然是 B/C，直接停手） |
...
- **合并条件只有三条**：CI 绿 + 6.4 核查通过 + 未命中 D4 硬闸。**「机器评审意见清空」不是条件**（共用规则 §5.5）
- **评审熔断**：第 3 轮评审起 / 累计 5 个 review 修复提交 / 同一处措辞被反复要求改第 2 次 → **停改 B/C 类**（记 `debt.*`），**A 类仍必须修完**才合并；连续两轮还在冒 A 类 → 关闭 PR 缩小范围重开（见 6.5.1）
- **当轮落地**：本轮 PR 必须在本轮合并或关闭，**禁止过夜**——下一轮会另开分支另开 PR，两个都会烂掉
- **历史 PR 清零**：跑完 6.2 后，open 的熵减 PR 只允许剩 `[需人工] ` 与核查不通过两类；其余审计通过就合并、已被覆盖就关闭
- **分支审计**：每轮输出 `REDUNDANT` 分支清单（内容已全在 main）交用户删除——本 Agent 凭据删不了分支（403），见 6.2.1
