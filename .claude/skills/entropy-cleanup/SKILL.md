  ...
  - **其余的每一个都要在它自己的 head 上跑审计**，不是在当前分支上跑——当前分支干净
    ...
    **用 worktree，禁止直接 `git checkout <sha>`**——那会 detach 当前工作区的 HEAD，
    ...
    - 审计发现该 PR 内容已被更晚合并的其它 commit 完全覆盖（manifest 条目已在 main、
      或对应设计文档章节已存在）→ **直接调 `mcp__github__update_pull_request` 把它
      `state: closed`**，并在关闭评论里写明「内容已被 <sha/PR> 覆盖，无净新增价值」。
      不强行合并一个空转的 PR，也不把它继续挂着。
...
**合并的充分条件（唯一一处定义，别处不得复述）**——四条全满足才合并：

1. **CI 绿**：`mergeable_state` 为 `clean`。
2. **6.4 内容核查通过**。
3. **Step 4.5 未命中 D4 硬闸**。
4. **没有未解决的 A 类缺陷**（判定见 6.5.1）。

第 4 条不能省：6.4 只查 diff 范围与幽灵删除，**查不出语义错误**——判据 fail-open、把功能描述写反、未核实覆盖就登记 manifest，这些都能通过 6.4。前三条全绿仍可能带病。

**但「机器评审意见清空」不是条件，永远不是。** 共用规则 §5.5：「不得以『清空所有机器评论』为完成标准」。合并要求的是**没有未解决的 A 类**，不是「没有意见」。

#### 6.5.1 评审意见怎么处理（A 类必修，B/C 类到点停手）

先按 §5.5 分类，再按轮次决定还要不要陪着改：

| 类别 | 判定 | 动作 |
|---|---|---|
| **A** | 可证明违反本 PR 目标 / 真实契约 / 既有测试。含：判据写错或 fail-open、描述与实现不符、条款自相矛盾、写了却走不到的死路径 | **任何轮次都必须修**，修完才满足合并第 4 条 |
| **B/C** | 措辞、同义词、补充限定、格式统一、版本号刷新、找不到真实触发路径的假设 | ≤2 轮正常回应；**第 3 轮起一律不再改**，明确回「按熵减评审熔断记账」并记 `debt.*` |

**分类看内容不看形式**：一条报告表面是「措辞问题」，但指出的是上次改措辞时把事实改错了，那它是 A 类。

**熔断只降 B/C 类的陪跑成本，不放行 A 类。** 本技能产出的是文档级 PR，正确耗时是十分钟不是一小时；但「快」不能靠带病合并换。

**连续两轮仍在冒新的 A 类** → 说明这个 PR 本身没做对：**关闭它、缩小范围重开**，或就地把规则改简单——**条件越多越容易自相矛盾，删条款往往比加条款更接近正解**。不要靠「再修一轮」硬推。

> **两次实录**：① 2026-08-13 PR #1367，一个 2 文件 +12 行的纯文档 PR 被拖 8 轮 15 条、9 个修复提交，后期全是措辞/版本号/文件名——B/C 类陪跑的典型，熔断为此而设。② 2026-08-14 PR #1369（本节自身），5 轮 15 条里 13 条是真 A 类，根因是初版规则条件过多、交叉引用过密，**产生自相矛盾的速度快过修补速度**（先是底部摘要与详述漂移三次，再是 6.5 与 6.5.1 相邻打架），最终靠删掉重复副本、把三节合并成一段收敛。教训：**治理规则自己也要守奥卡姆，多写一个条件就多一处可漂移的接缝。**
...
**本轮 PR 必须在本轮落地，落地只有「合并」与「关闭」两种结果**，禁止留「还在等评审」的 PR 过夜——下一轮会另开分支另开 PR，两个都会烂掉。

**先分清「还没跑完」和「跑挂了」**：刚建完 PR 时检查普遍是 pending，**pending 不是 CI 红**。必须**有上限地等它出终态**再判定；把 pending 当红直接关闭，会关掉一个本来健康的 PR，并正好留下本节要消灭的孤儿分支。
...
|---|---|
| 四条合并条件全满足 | **合并** |
| CI 真红且当轮修不好 | **关闭**，结论写进关闭说明，源分支登记进 6.2.1 的「已废弃」待删清单 |
| 命中 D4 硬闸 | 保持 `[需人工] ` 前缀留人工——**唯一允许过夜的例外** |
| 仍有未解决 A 类 | 修完再合；连续两轮还在冒 → 关闭并缩小范围 |
| 只剩 B/C 类没回完 | 不构成阻塞，直接合并 |
