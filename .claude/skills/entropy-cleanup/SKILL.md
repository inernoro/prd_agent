    ...
    **用 worktree，禁止直接 `git checkout <sha>`**——那会 detach 当前工作区的 HEAD，
    ...
    - 结构审计（D1-D4+棘轮）与 6.4 内容核查**都**通过，且 `mcp__github__pull_request_read`
      ...
      **合并前必须先读它自己的评审意见**（`pull_request_read` method=`get_review_comments`），
      ...
      >
      > **例外的适用边界，不得外扩**：只适用于**本技能自己产出**的熵减 PR（标题含「熵减计划」、
      > 内容只含 doc/index/guide.list/changelog/技能元数据）。对任何其它 PR——别的 Agent 的
      > 功能分支、含产品代码的 PR、非本技能产出的文档 PR——§5.5 通则原样成立，一律不得自动合并。
      >
      ...
      >
      > 仍然保留的闸有两个：`[需人工] ` 前缀（D4 硬闸）与 6.4 内容核查不通过。这两个之外，
      > 审计通过即合并。若用户日后收回这条指示，本节即刻回到 §5.5 通则。
