...
**合并的充分条件就是上面这三条，不多一条**：CI 绿（`mergeable_state` 为 `clean`）+ 6.4 内容核查通过 + Step 4.5 未命中硬闸。

**「机器评审意见全部清空」不是合并条件，永远不是。** 共用规则 §5.5 白纸黑字：「自动 Reviewer 的评论不能单独授权需求扩张；**不得以「清空所有机器评论」为完成标准**」。把它当条件的后果已经实测过，见下面 6.5.1。

#### 6.5.1 评审轮次熔断（硬上限，先于一切「意见还没回完」的冲动）

**本技能产出的 PR 一律是文档/索引/changelog 级改动，风险下限极低，不值得多轮评审。** 因此设死上限：

| 判据 | 动作 |
|---|---|
| 评审轮次 ≤ 2 轮 | 正常回应：A 类真缺陷就修，B/C 类按 §5.5 明确回「记账 / 证据不足，不改」 |
| **第 3 轮起** | **只修 A 类，B/C 类一律不再动**。处理完当轮 A 类后 CI 绿 + 6.4 通过 → **直接合并**，PR 里留一条「已按熵减评审熔断，B/C 类意见记入 `debt.*`」 |
| 累计 review 修复提交 ≥ 5 个 | 同上 |
| 同一份文档被反复要求调措辞第 2 次 | 同上——但**先按 A/B/C 判一次再套这条**：若上一次改措辞时把事实改错了、这次报的正是那个错（描述反了 / 与实现不符），那是 A 类必须修，不能拿「又是措辞」挡掉 |

**熔断降的是「B/C 类还要不要陪着改」，不是「A 类可不可以带病合并」。** 两者必须分清（2026-08-14 PR #1369 第一轮评审提出，判断成立并已采纳）：

- **A 类仍然必须修**：可证明违反本 PR 目标、真实调用契约或既有测试的缺陷。典型如判据写错、脚本 fail-open、把功能描述写反、未核实覆盖就登记 manifest——这些 6.4 的结构核查**查不出来**（它只管 diff 范围与幽灵删除，不管语义对不对），所以不能靠「6.4 过了」推定安全。
- **B/C 类不再陪跑**：措辞、同义词、补充限定、格式统一、版本号刷新、以及找不到真实触发路径的假设输入。这些正是无限扩张的燃料。

**若熔断后仍持续冒出 A 类**（连续两轮还在发现真缺陷），说明这个 PR 本身没做对，**关闭它、缩小范围重开**，不要靠「再修一轮」硬推——也不要为了收工把已知 A 类合并进去。

熔断后**必须真的合并或真的关闭**，不允许「熔断了但 PR 还挂着」——那正是本节要消灭的状态。

> **历史实录（2026-08-13，PR #1367）**：一个 **2 文件、+12 行**的纯文档 PR，被自动评审拖了 **8 轮 15 条**，产生 **9 个修复提交**，从 D6 补一段设计小节一路扩展到改债务台账行格式、刷版本号日期、改文件名、拆分/合并债务册。全程每条意见单独看都成立，但**整体严重失衡**：投入远超这个 PR 的风险与价值。更糟的是它把 PR 挂了一个多小时没合并——而下一次定时熵减跑起来时会**再开一个分支、再开一个 PR**，于是分支与 PR 双双堆积。用户当天盘点发现：**76 个远端分支、36 个 `claude/*`、其中 7 个是熵减自己留下的 `claude/zen-allen-*`**，直接要求「修改技能提交或删除 pr, 避免无限扩展」。本节由此诞生。
>
> 判据口诀：**这是一个改文档的 PR。它的正确耗时是十分钟，不是一小时。第 3 轮评审还没合并，说明错的是流程不是文档。**
...
- **本轮 PR 必须在本轮内落地**——落地有且只有两种结果：**合并**，或**关闭并说明原因**。**禁止留一个「还在等评审」的 PR 过夜**，下一次定时任务会另开分支另开 PR，两个都会烂在那里。
  ...
  **「当轮落地」不是合并的第四条通道。** 三道闸（CI 绿 / 6.4 核查 / 未命中 D4 硬闸）任一不满足时，落地方式**只能是关闭**，不是熔断合并：
  ...
  | 落不了地的原因 | 正确动作 |
  |---|---|
  | CI 红 | 当轮修得好就修；修不好 → **关闭 PR**，把结论写进关闭说明，下轮重开。关闭后**必须把它的源分支登记进 6.2.1 的待删清单**（见下） |
