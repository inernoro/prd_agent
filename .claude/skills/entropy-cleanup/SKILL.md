  ...
  - **其余的每一个都要在它自己的 head 上跑审计**，不是在当前分支上跑——当前分支干净
    ...
    **用 worktree，禁止直接 `git checkout <sha>`**——那会 detach 当前工作区的 HEAD，
    ...
    - 结构审计（D1-D4+棘轮）与 6.4 内容核查**都**通过，且 `mcp__github__pull_request_read`
      ...
      > 2026-08-13 修正（此前这里写的是「不要直接合并、请用户确认」，实测该写法是堆积的
      > 直接成因）：熵减 PR 全部由本技能自己产出、内容只限文档/索引/changelog、且已经过
      > 与本轮同一套 D1-D4 + 棘轮 + 6.4 内容核查。让每一轮都「只上报、请用户确认」的结果
      > 是它们**永远没人确认**，一轮堆一个——用户 2026-08-14 盘点时发现 7 个熵减分支
      > 躺在远端，其中最老的已经躺了 11 天，而每一个都只是几行文档改动。
      > 「审计通过却不合并」不是谨慎，是把清理欠债的技能变成了产生欠债的技能。
      >
      > 仍然保留的人工闸只有两个，且都不靠「等用户点头」实现：`[需人工] ` 前缀（D4 硬闸）
      > 和 6.4 内容核查不通过。这两个之外，审计通过即合并。
...
# 列出所有熵减分支（本技能历来用会话分配的分支名，形如 claude/<随机词>-<id>）
# 判据只认「内容差异」，不认祖先关系——仓库用 squash 合并，已合并分支按祖先关系仍显示未合并
for b in $(git ls-remote --heads origin | awk '{print $2}' | sed 's|refs/heads/||'); do
  git rev-parse --verify -q "origin/$b" >/dev/null 2>&1 || continue
  [ -z "$(git diff origin/main...origin/$b --name-only)" ] && echo "REDUNDANT: $b"
done
```

`REDUNDANT` 的分支内容已全在 main，可安全删除：`git push origin --delete <branch>`。
...
| 判据 | 动作 |
|---|---|
| 评审轮次 ≤ 2 轮 | 正常回应：真缺陷就修，推测性意见按 §5.5 C 类明确回「证据不足，不改」 |
| **第 3 轮起** | **停止修复**。CI 绿 + 6.4 通过 → **直接合并**；在 PR 里留一条说明「本轮起按熵减 PR 评审熔断，剩余意见（如成立）记入 `debt.*` 下轮处理」 |
| 累计 review 修复提交 ≥ 5 个 | 同上，立即熔断合并 |
| 同一份文档被反复要求调措辞第 2 次 | 同上，立即熔断合并 |
