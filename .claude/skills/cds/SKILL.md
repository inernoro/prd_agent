...
1. 直接 `python3 .claude/skills/cds/cli/cdscli.py scan <repo-root>` 跑（或当前目录），看 `data.signals.source`：
...
2. 跑出 yaml 后**直接展示前 30 行 + signals**给用户审视，不要追问"要不要改"。如用户表态 OK 才走 `--apply-to-cds <projectId>` 提交 CDS。
...
> commit+push 后别人通过 `cdscli update` 或 (zip) 拿到新版。**没有发布流程**。
...
- `/cds-sync` [OK]
- `/cds-sync-skill` [OK]
- `帮我同步 cds 技能` [OK]（必须含"cds"关键字）
- `cdscli sync-from-cds` [OK]（用户直接给 CLI 命令）
...
Step 1/3: CDS 地址
  ...
  > cds.miduo.org
  [OK] 已写入 ~/.cdsrc: CDS_HOST=cds.miduo.org
