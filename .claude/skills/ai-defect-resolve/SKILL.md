...
自动化任务只是调度壳，不承载缺陷处理细则。每日任务提示词只需要提供：

1. 工作目录。
2. 正式缺陷系统 `domain`。
3. K 的本机文件路径或当次输入的 K。
4. 可选过滤条件：`projectId`、`teamId`、`status`。
5. 明确调用本技能执行闭环。
...
```text
执行 PRD Agent 缺陷自动修复每日闭环。

工作目录：/Users/inernoro/.codex/automation-worktrees/prd_agent_defect_daily
正式缺陷系统 domain：https://map.ebcone.net
授权 K 文件：/Users/inernoro/.codex/secrets/map-defect-agent-key
状态过滤：submitted,assigned,processing

请读取 AGENTS.md 和项目内置 .claude/skills/ai-defect-resolve/SKILL.md，使用 ai-defect-resolve 技能完成本轮闭环。
启动前保持工作区干净并切到 origin/main；运行 scripts/defect-automation-probe.mjs --safe 通过后再领取缺陷。
不要输出或写入 K 明文。输出本轮 runId、处理结果、commit/PR、验收报告、通知结果和阻塞项。
```
...
3. 每条缺陷先做 triage：判断是否过期、重复、已修复、无法复现、不是缺陷、缺少关键复现信息或不属于本仓库自动化范围。
4. 过期或不合适缺陷必须先评论说明证据和结论，再调用 `workflow/block`，`failurePhase=triage`，`failureReason` 使用 `expired_or_stale` / `not_actionable` / `duplicate` / `already_fixed` / `not_reproducible_with_evidence` 前缀，`stopRun=false`，然后继续下一条。
5. 合适且轻量的缺陷才进入修复：评论计划、改代码、验证、commit、创建 PR，然后调用 `workflow/complete` 一次性回写 PR、commit、写入 `defect_resolution_traces`、标记缺陷已修复。
6. 重量级、无法自测、高风险或需要产品确认时调用 `workflow/block`，写入阻塞原因并默认 `stopRun=true` 停止本轮运行。
7. `workflow/complete` 或 `workflow/block(stopRun=false)` 返回下一次 `workflow/start-next` 入参后，继续下一条；直到 `start-next` 返回 `hasNext=false` 才结束本轮缺陷领取。
...
8. `workflow/complete` 或 `workflow/block(stopRun=false)` 返回下一次 `workflow/start-next` 入参，证明流程能继续下一条。
9. 过期、重复、已修复、无法复现或不合适缺陷必须先评论，再调用 `POST /agent/workflow/block`，`failurePhase=triage` 且 `stopRun=false`，证明不会因为单条不可处理缺陷停掉整轮任务。
10. 阻塞或重量级缺陷必须调用 `POST /agent/workflow/block` 并写入失败原因；高风险缺陷默认 `stopRun=true`。
11. 更新中心的 commit 记录必须在 UI 上出现可点击的“关联缺陷 N”或“我的缺陷 N”标志，点击后能看到缺陷编号、标题、PR、commit、发布状态、验收报告或知识库链接；只验证接口 `linkedDefects` 不算完成。普通 changelog 文案行没有 commit id，不允许按日期批量贴缺陷标志。如果 commit 未正式发布，只能记录“需要真人审核发布”，不能冒充已发布。
12. 正式发布后才能跑 `create-visual-test-to-kb`，报告必须进入“缺陷修复验收报告”知识库。
13. 只有正式发布且验收通过后，才能调用 `validation-report` 通知提交人。
...
领取每条缺陷后，必须先分类再决定是否修复。分类结论要写入正式缺陷评论，不能只写在本地日志。

以下类别不进入代码修复，但不能停掉整轮每日任务：
...

{
  ...
  "defectId": "{defectId}",
  "failurePhase": "triage",
  "failureReason": "expired_or_stale: 证据和原因",
  "comment": "写给提交者的结论、证据和后续建议",
  "stopRun": false
}
```
...
重量级缺陷默认停止，让用户确认。过期、重复、已修复、无法复现或不合适缺陷属于 triage 跳过，必须 `stopRun=false` 并继续下一条；这类跳过不是高风险阻塞。
...
2. 一次只修一个缺陷：提交并回写 commit 后再继续下一条；triage 跳过并回写 `block stopRun=false` 后也继续下一条。
3. 不确定就停：破坏性、重量级、跨系统变更必须评论并等待确认；过期、重复、已修复、无法复现或不合适缺陷不属于“不确定就停”，必须回复、标记并继续。
