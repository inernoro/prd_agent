...
10. 更新中心能通过 commit id 自动看到关联缺陷；如果 commit 未正式发布，只能记录“待发布”，不能冒充已发布。
11. 正式发布后才能跑 `create-visual-test-to-kb`，报告必须进入“缺陷修复验收报告”知识库。
12. 只有正式发布且验收通过后，才能调用 `validation-report` 通知提交人。

执行者必须维护一份本轮证据摘要，至少记录 `domain`、`runId`、`defectNo`、`defectId`、`messageId`、`commitSha`、`commitInfoResult`、`fixStatusResult`、`nextResult`、`previewUrl`、`visualReportUrl`。摘要不能包含 K 明文。
