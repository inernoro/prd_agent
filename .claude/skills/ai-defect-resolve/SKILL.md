...
- 修复完成后：先引用缺陷内容，再说明 PR、commit、测试或预览验收地址、验收步骤；发布后验收报告生成后必须再评论报告地址和知识库地址。

缺陷引用是硬要求，不能只写缺陷编号或修复标题。引用格式必须包含：

| 字段 | 要求 |
|------|------|
| 缺陷编号 | `DEF-...` 或系统缺陷编号 |
| 缺陷标题 | 用户提交的标题或系统归一化标题 |
| 用户原始问题 | 从 `RawContent` 或结构化字段摘出 1-3 句，不超过 240 字 |
| 影响位置 | 页面、接口、模块或用户路径；未知时写“待补充” |
| 修复目标 | 本次 commit/PR 要证明的问题已被解决到什么程度 |
...
1. 正式缺陷系统只负责读取待验收 trace、回写报告和通知提交人；使用 `create-visual-test-to-kb` 在测试或预览环境跑视觉验收。目标优先取 `item.acceptance.target`，commit 取 `item.acceptance.commitSha`，PR 取 `item.pullRequestUrl` 或 trace 字段，验收地址取 `item.acceptance.previewUrl`。
...
5. 缺陷专用验收报告必须在首屏包含“缺陷引用”“提交与发布证据”“验收事项清单”三段：
   - 缺陷引用：缺陷编号、标题、用户原始问题摘录、影响位置、修复目标。
   - 提交与发布证据：PR 地址、commit 地址、commit SHA、发布状态、验收地址。
   - 验收事项清单：从缺陷内容和 commit/PR 总结出 3-8 条用户可理解的验收事项，每条写预期结果。
6. 归档后必须用验收技能的 `verify-open.mjs` 打开报告地址，确认标题、正文、提交链接、验收事项和截图可见。
7. 回写验收报告并通知提交人。验收失败时不要发送“已修复”，要发送“需要继续改进”。如果正式验收报告证明用户描述不成立，`verdict` 使用 `invalid`，回复必须引用验收报告证明该结论。
...
每日任务或手动运行结束时，最终输出必须让用户直接看懂“修的是哪个缺陷”，不能只列 `DEF-...` 和修复标题。

每个处理项使用以下结构：

```markdown
- DEF-2026-0001
  缺陷引用：{缺陷标题}；用户原始问题：{RawContent 或结构化字段摘录，1-3 句}
  修复结论：{已修复/已阻塞/已跳过/陈述不成立}
  提交证据：PR {PR URL 或编号}；commit {commit URL 或完整 SHA}
  验收预期：{从缺陷和 commit 总结出的 1-3 条验收事项}
  发布状态：{需要真人审核发布/已正式发布/待发布后验收}
```

每日更新要求：列出正式缺陷系统 runId、处理缺陷引用、处理结果、修复 commit、PR、测试/预览验收地址、正式发布待验收项、验收报告链接、已通知用户和阻塞项。不得省略缺陷内容摘录。
