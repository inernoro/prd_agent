...
- 如果 `agentLaunch.skill.minVersion` 高于头部声明的当前版本，停止执行并提示升级技能。
...
> SSOT：`doc/design.defect-agent.automation-autonomy.md`。本技能不是线性脚本，是一个有边界、能担责的自治体系。下面的判断先于、并贯穿上面的流程步骤。判定口诀：动手前先问「这条该不该我全权改，还是该停下问人」。
...
| 禁止自动做 | `forbiddenScopes`：破坏性删除 / DB 迁移 / 权限模型重写 / 跨服务协议改造 / 无法自测的关键路径 | 一律 `block` 升级，绝不自动执行 |

### 五层自治回路（每条缺陷按序自检）

1. **认知（指令≠目标）**：领单后先还原「这条缺陷要解决的真实问题」，并**实际复现/定位**，确认描述仍成立。描述与代码现状不符 → `block`（`failurePhase=analysis`），不照着过时描述改。
2. **规划（值不值，不只看行数）**：即使 ≤200 行，也要心算成功率 / 影响面 / 回滚代价；高风险或没把握 → 主动降档「请示后做」。行数小 ≠ 该自动修。
3. **执行（事实核查）**：执行中工具/代码实况与预判**严重不符**，停下重规划或 `block`（`failurePhase=fix`），不硬按原计划走；推送前跑通至少一条真实自测（CLAUDE.md §8.1）。
4. **记忆（情景召回）**：动手前先查同模块 / 同症状的历史处置——`git log` 相关改动、已有修复 commit、相似缺陷评论——复用上次策略与踩过的坑。（后端 `start-next.similarRecall` 钩子落地前，用 git/代码检索兜底；不假装有语义检索。）
5. **监督（三道事前阀，按序）**：① 合规红线：触 `forbiddenScopes` / protected paths / 跨环境 → 拒绝执行；② 不确定性预警：没把握 → `block` 升级而非硬猜着改；③ 紧急熔断：同一 `defectId` 最近几轮反复领取/反复 `failed`（fix-break-fix），或长任务兜圈（≥8 提交 / ≥2h 净零进展，见 `blocked-state-circuit-breaker.md`）→ 停 + 在 `failureReason` 标 `loopGuard`，不再自动领取。

### complete / block 前的自治自查（必过）

- [ ] 我定了档，且这条确属「全权自主」才走 `complete`？
- [ ] 认知：还原了真实目标 + 复现确认了描述成立？
- [ ] 规划：算过值不值，不是只看行数就修？
- [ ] 执行：跑通真实自测，没把验证甩给用户？
- [ ] 记忆：查过相似历史处置？
- [ ] 监督：三道阀都过了（没踩红线、没硬猜、没陷入 fix-break-fix）？

任一为否，转 `block` 或降档，**不许 `complete`**。
...
发布后验收必须先形成两个互不覆盖的结论，并在报告与缺陷评论中分别写明：

1. `functionalVerdict`：`pass`、`conditional`、`fail` 或 `invalid`，只根据缺陷所述用户场景、真实行为、自测与回归风险判定。
2. `evidenceStatus`：`complete`、`partial` 或 `blocked`，只描述更新中心 commit 行、关联缺陷弹窗、报告可访问性和截图等闭环证据是否齐全。

严禁用证据缺口推翻功能事实。更新中心只展示最近一周导致目标 commit 不可见、目标分支预览下线、截图失败、关联弹窗暂时不可用、报告链接尚未出现在弹窗中，均属于 `evidenceStatus` 问题，不等于功能未修复。应优先用更新中心完整历史、正式 commit、PR、工作流 trace 或接口结果兜底，并如实记录缺少哪一项 UI 证据。

`validation-report.verdict` 映射规则：

- 功能通过且证据完整：`pass`。
- 功能通过但证据为 `partial` 或 `blocked`：`conditional`，消息以“功能验收通过；闭环证据不完整”开头，并说明缺口与补证路径。
- 功能本身有已确认限制但核心修复成立：`conditional`，明确写功能限制；若同时有证据缺口，两类限制分别陈述。
- 用户可见场景仍失败、出现回归，或缺少关键功能验证而无法确认修复：`fail`。只有此时才能使用“验收未通过，需要继续改进”。
- 正式证据证明缺陷陈述不成立：`invalid`。

知识库归档使用独立的 `reportVerdict`，只能取 `pass`、`conditional` 或 `fail`，不得把 `invalid` 传给 `create-visual-test-to-kb` 或 `archive_report.py`。当 `functionalVerdict=invalid` 时，归档报告必须使用 `reportVerdict=conditional`，正文明确写“缺陷陈述不成立”并引用核验证据；报告成功归档后，正式系统 `validation-report.verdict` 再使用 `invalid`。

报告正文必须分别展示 `functionalVerdict`、`evidenceStatus` 和 `reportVerdict`。不得把自动化取证能力不足描述成代码仍未修复，也不得让提交人为取证基础设施问题承担复测责任。
...
- `pass`：功能验收通过、闭环证据完整，且报告已回写正式缺陷系统。
- `conditional`：功能验收通过但闭环证据不完整，或功能本身存在已确认限制；必须在报告和消息中分别说明功能结论、证据状态和限制。
- `fail`：用户可见功能仍失败、出现回归，或缺少关键功能验证而无法确认修复，需要继续改进。
- `invalid`：验收报告证明用户提交的缺陷陈述不成立，用于正式回调和有证据地回复用户；知识库归档 verdict 必须使用 `conditional`，不能传 `invalid`。

`conditional` 回调必须填写 `message`，分别说明功能结论、闭环证据状态以及限制或缺口；缺少时后端返回 400，不再发送泛化通知。
