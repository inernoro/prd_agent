...
> **版本**：v1.9.0 | **状态**：已落地 | **触发**：「修复缺陷」、「缺陷自动修复」、`ai-defect-resolve` | **SSOT**：`doc/design.defect-agent.automation-autonomy.md`
...
报告顶层结论跟随 `functionalVerdict`，证据状态作为独立限定项展示。不得把自动化取证能力不足描述成代码仍未修复，也不得让提交人为取证基础设施问题承担复测责任。
...
6. 回写验收报告并通知提交人。只有功能验收为 `fail` 时才发送“需要继续改进”；功能通过但闭环证据不完整时回写 `conditional`，明确说明功能已通过以及待补证据。如果正式验收报告证明用户描述不成立，`verdict` 使用 `invalid`，回复必须引用验收报告证明该结论。
