...
3. 发表评论说明计划。
4. 判断是否轻量修复。
5. 轻量修复则改代码、验证、commit，然后调用 `workflow/complete` 一次性回写 commit、写入 `defect_resolution_traces`、标记缺陷已修复。
...
7. `POST /agent/workflow/complete` 返回成功，证明 commit 已写回缺陷系统，且单缺陷已标记修复。
