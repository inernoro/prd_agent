---
name: createzzdemo
description: 批量创建「教程小书」的路径式引导演示(DailyTip + 多步 Tour)。用户说"创建 XX 演示" / "做一个 XX 教程" / "增加教程" / "增加引导" / "/createzzdemo 缺陷管理" 时触发。**工作流分两阶段**:(1) 先枚举可用步骤让用户选,(2) 按选中组合输出 DailyTip JSON + 打断风险分析 + 角色推荐(PM/DEV/QA)。内置模板:缺陷管理、更新中心周报、知识库发布。键盘快捷键(Ctrl+B / Ctrl+K 等)**不适合**此技能,应走静态 key-hint。
---
