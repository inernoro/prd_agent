...
2. **可打勾可回溯**：每个 checkpoint 有 `[ ]` 符号，跑完一项打一个 `[x]` 通过 / `[!]` 失败
3. **失败不留悬案**：任何失败都必须有对应的排查路径（failurePlaybook），不能光说"失败了"
...
- **预览地址自动化**：Web 场景跑 `python3 .claude/skills/cds/cli/cdscli.py --human preview-url` 拿 URL，**禁止任何形式的手拼**。SSOT: `cds/src/services/preview-slug.ts:computePreviewSlug`
