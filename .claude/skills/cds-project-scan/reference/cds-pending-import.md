...
审批流程：AI 提交后，CDS 实时向 dashboard 推送 `pending-import.created` 事件，
任何已登录页面右下角立即弹出"Agent 导入 N"徽章。用户点徽章 → 弹抽屉看 YAML +
摘要 → 批准/拒绝。**不需要 AI 主动告诉用户去哪个 URL**——徽章是全局的。

老的 `/project-list?pendingImport=<id>` 直达链接仍然可用作 fallback。
...
case "$HTTP_CODE" in
  201)
    ...
    echo ""
    # 2026-05-28 起 CDS 任何已登录页面都会自动弹起右下角"Agent 导入 N"徽章。
    # 不再需要 AI 主动给地址（备用直达链接保留作 fallback）。
    echo "  CDS Dashboard 任意已登录页面右下角会自动弹出'Agent 导入 N'徽章，点击批准/拒绝即可"
    echo "  （备用直达链接：https://$CDS_HOST/project-list?pendingImport=$IMPORT_ID）"
    ;;
...
[Phase 8]   检测到 --apply-to-cds 参数：
  ...
  → CDS Dashboard 右下角已自动弹出'Agent 导入 1'徽章,请点击批准
    (备用直达链接：https://cds.miduo.org/project-list?pendingImport=imp_xyz789)
```
