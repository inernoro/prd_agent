...
## 2.0-pre 浅克隆闸（必须先跑，在下面任何 git 命令之前）

浅克隆会让 2.0 写出的 `COMMITS_FILE` 少算，且不报错。**这道闸必须排在 2.0 之前**——
2.0 那条 `git log` 就是提交集本身的来源，等它跑完再拦已经晚了。判据与用法见 SKILL.md
Phase 2 开头那段（`--check-shallow-only`，退出码 `0` 放行 / `2` 先补历史 / `3` 没查成 /
其余一律拒绝）。

**闸红过、补完历史之后，必须从 2.0 重跑，不是从 2.1。** `COMMITS_FILE` 是 2.0 写的，
2.1 只是 `wc -l` 读它；只重跑 2.1 读到的还是那份截断的旧文件，数字一个都不会变。
