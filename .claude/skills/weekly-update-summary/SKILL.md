...
### Phase 8: 生成主页弹窗海报草稿（可选但推荐）

周报生成完成后，**询问用户是否希望同时产出一张主页弹窗海报**（登录用户首次访问主页时弹出的轮播图文卡，末页 CTA 跳回完整周报）。

> 「这张周报要不要顺手做成主页弹窗海报？我可以把 4-6 页脚本和每页配图提示词一口气写好，你在后台点一下生图就行。」

若用户确认：

1. 读取刚写好的 `$REPORT_FILE` 全文
2. 按照 [reference/poster-pages.md](reference/poster-pages.md) 的规则拆成 4-6 页（开篇 / 1-3 个亮点 / 展望 / CTA）
3. 为每页生成：`title`、`body`（80-120 字）、`imagePrompt`（80-160 字英文，适合文生图模型）
4. 通过 `POST /api/weekly-posters` 创建草稿（状态 = draft），字段如下：
   ```json
   {
     "weekKey": "2026-W17",
     "title": "2026-W17 平台更新速览",
     "subtitle": "6 个新功能 + 11 个修复 + 3 条实验性特性",
     "pages": [
       { "order": 0, "title": "本周亮点", "body": "...", "imagePrompt": "...", "accentColor": "#7c3aed" },
       ...
     ],
     "ctaText": "阅读完整周报",
     "ctaUrl": "/changelog"
...
5. 创建成功后告知用户：
   ```
   海报草稿已创建。
   【位置】百宝箱 → 周报海报编辑器（wip 施工中徽章）
   【路径】登录首页 → 左侧底部「百宝箱」 → 搜索「周报海报」 → 打开编辑器 → 选中最新草稿
                → 每一页点右上角紫色「生成图片」按钮(调平台默认文生图,约 10-30 秒)
                → 4 页都生成完,点右上角「保存草稿」→「发布到主页」
   发布后，登录用户第一次访问主页即可看到轮播弹窗;末页 CTA 会跳到 {ctaUrl}。
   ```

> **为什么要走草稿 + 管理员点生图的两步走**：AI 自动生图的质量不稳定,由人工点按钮生成、不满意可原地改 prompt 重生,比一次性直出更可控。技能负责「写好 prompt」,人负责「点一下确认配图」。
