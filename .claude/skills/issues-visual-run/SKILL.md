...
3. 按 §3 执行矩阵跑测试
4. 处理结束：
   - 加对应终态 label（`visual-test:reviewing` / `visual-test:passed` / `visual-test:blocked`）
   - 删 `visual-test:pending` label（issue 离开待接单队列）
   - 发终态评论（详见 §5 失败回报或 §8 通过模板）
   - 评论末尾追加幂等指纹 `<!-- visual-run:{run_id}:{今日日期 YYYY-MM-DD} -->`（同日重跑时跳过，跨日不算）
