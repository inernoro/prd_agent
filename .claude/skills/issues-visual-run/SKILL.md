...
§8 单 run 内 `max_minutes_per_issue` timeout 救不了"worker 崩溃 / 容器重启"导致的死锁——崩溃后没人执行清理。§1 接单条件又只看 `visual-test:pending` 且拒绝 `agent-processing`，issue 会永久卡死。所以每轮 sweep 必须先跑 reaper，**两路分别处理崩溃位置**：

1. `GET /repos/.../issues?labels=agent-processing&state=open` 后**必须再过滤**为视觉测试领地：标题以 `[visual-test]` 开头 **或** 含任一 `visual-test:*` label（`agent-processing` 与 `/issues-autofix` 共用，不过滤会侵犯 autofix 领地 —— autofix issue 被本 reaper 强加 `visual-test:pending` 后，autofix 跳过、visual-run 也跳过（标题不是 `[visual-test]`），永久卡死）
2. 对每条命中：扫描所有 `visual-run:*:claim:*` / `*:terminal:*` / `*:withdrawn:*` 指纹，**锁定当前活跃 run_id**：
   a. 过滤掉所有 `*:withdrawn:*` 对应的 run_id（这些是已撤回的 loser）
   b. 在剩余 claim 中按时间戳取**最新**那条，记为 `currentRunId`（即"本周期的实际接单者"——可能完成 terminal、也可能还在跑）
   c. 后续 A/B 判定**只看 `currentRunId`** 的状态，不看其他历史 run_id。重测场景下,上一轮已完成的 W1 terminal 留在历史里,但本轮 W2 才是 currentRunId,A/B 由 W2 状态决定,不会被 W1 的旧 terminal 误判
3. **路径 B**（当前周期已 terminal + lock 未释放）：`currentRunId` 已有对应 `:terminal:` 指纹 **且** `agent-processing` 仍在 → 当前周期终态后崩溃：
   - 删 `agent-processing` label（**不要**加回 pending——本周期终态已落定，加回会破坏已宣告的结果）
   - 评论：`reaper 清理孤儿锁（B 路径，终态后崩溃）：currentRunId={X} 已发布 :terminal: 但 lock 未释放，已清除`
4. **路径 A**（当前周期终态前崩溃）：B 不命中 **且** `currentRunId` 既无 `:terminal:` 也无 `:withdrawn:` 距今 > **reaper 阈值**（默认 30 分钟，必须 > `max_minutes_per_issue`）→ 认定业务进行中崩溃：
   - 删 `agent-processing` label
   - 加回 `visual-test:pending` label（重新入队）
   - 评论：`reaper 重置（A 路径，终态前崩溃）：活跃 claim run_id={X} 在 {ts} 接单后未完成（>30 分钟无 :terminal:/:withdrawn: 指纹），可能为 worker 崩溃。已恢复为待接单状态`
5. **路径 C**（claim 写入前崩溃，无指纹孤儿锁）：B/A 都不命中 **且** issue 有 `agent-processing` 但**完全没有** `visual-run:*:claim:*` 指纹（claim 评论根本没发出）**且** lock 添加时间（issue updated_at 或 label timeline）> 30 分钟 → 认定崩溃在"加 lock"与"发 claim 评论"之间：
   - 删 `agent-processing` label
   - 加回 `visual-test:pending` label（无终态被宣告，可安全重新入队）
   - 评论：`reaper 重置（C 路径，claim 写入前崩溃）：lock 存在但无任何 :claim: 指纹，已恢复为待接单状态`
6. 三路都不命中的（还在合理处理时间内 或 只有 withdrawn 历史 + 无活跃 claim）→ 保留不动

### 接单阶段

1. **预检**（**必须重新对照 §1 全套接单条件**，不只是检查 lock）：
   - 重新拉取 issue 当前 labels（list issue 之后 issue 状态可能已变）
   - 当前不含 `visual-test:pending` → 跳过（已被另一 worker 接走或转入终态）
   - 当前含 `agent-processing` / `visual-test:reviewing` / `visual-test:passed` / `visual-test:blocked` 任一 → 跳过
   - 标题不再以 `[visual-test]` 开头 → 跳过
