...
2. **删 label `visual-test:pending`**（与 SSOT `doc/rule.issues-system.md` §2.2 一致，否则 issue 留在 pending 队列被反复 fetch + skip）
3. 评论："已接单 · 预计 N 分钟 · run_id={uuid}"

处理结束（任一终态）：
1. 删 `agent-processing`
2. 加对应终态 label（`visual-test:reviewing` / `visual-test:passed` / `visual-test:blocked`）
3. 兜底再次确认 `visual-test:pending` 已不在 issue 上（防御 step 2 在 race 下未生效）
4. 终态评论末尾追加 `<!-- visual-run:{run_id}:{ts} -->` 指纹
