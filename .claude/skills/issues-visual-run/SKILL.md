...
2. 对每条命中：找该 issue 评论里**最新**的 `visual-run:*:claim:*` 指纹和对应 run_id 的 `:terminal:` / `:withdrawn:` 指纹
3. **路径 A**（终态前崩溃）：claim 距今 > **reaper 阈值**（默认 30 分钟，必须 > `max_minutes_per_issue`）**且无对应 `:terminal:` 指纹** → 认定崩溃残留：
   - 删 `agent-processing` label
   - 加回 `visual-test:pending` label（重新入队）
   - 评论：`reaper 重置（A 路径，终态前崩溃）：上一次 run_id={X} 在 {ts} 接单后未完成（>30 分钟无终态指纹），可能为 worker 崩溃。已恢复为待接单状态`
4. **路径 B**（终态后崩溃，孤儿锁）：claim 距今 > 阈值 **且已有对应 `:terminal:` 指纹** **且** `agent-processing` 仍在 → 认定崩溃在"发终态评论"与"删 lock"之间：
   - 删 `agent-processing` label（**不要**加回 pending——终态已落定，加回会破坏已宣告的结果）
   - 评论：`reaper 清理孤儿锁（B 路径，终态后崩溃）：run_id={X} 已发布 :terminal: 指纹但未释放 lock，已清除`
5. **路径 C**（claim 写入前崩溃，无指纹孤儿锁）：issue 有 `agent-processing` 但**完全没有** `visual-run:*:claim:*` 指纹（claim 评论根本没发出）→ 认定崩溃在"加 lock"与"发 claim 评论"之间。此时没有 claim 时间戳可对比，**只要 lock 已存在 > 30 分钟**（GitHub 评论可读 issue 的 updated_at 或者 label 添加事件 timeline 都可作为参考时间）即认定崩溃：
   - 删 `agent-processing` label
   - 加回 `visual-test:pending` label（无终态被宣告，可安全重新入队）
   - 评论：`reaper 重置（C 路径，claim 写入前崩溃）：lock 存在但无任何 :claim: 指纹，已恢复为待接单状态`
6. 不命中阈值的（还在合理处理时间内）→ 保留不动
