...
2. 对每条命中：扫描所有 `visual-run:*:claim:*` / `*:terminal:*` / `*:withdrawn:*` 指纹，按**状态优先级**判定（不按 run_id 也不按"最新 claim"，否则 withdrawn loser 的晚 claim 会把已 terminal 的 issue 误判为 A）
3. **路径 B 优先**（任何 run_id 写过 terminal → 终态已宣告）：若评论里**存在任一** `visual-run:*:terminal:*` 指纹 **且** `agent-processing` 仍在 → 终态后崩溃（lock 未释放）：
   - 删 `agent-processing` label（**不要**加回 pending——终态已落定，加回会破坏已宣告的结果）
   - 评论：`reaper 清理孤儿锁（B 路径，终态后崩溃）：已存在 :terminal: 指纹，仅清除残留 lock`
4. **路径 A**（终态前崩溃）：B 不命中 **且** 存在"活跃 claim"（同 run_id 既无 `:terminal:` 也无 `:withdrawn:` 指纹）距今 > **reaper 阈值**（默认 30 分钟，必须 > `max_minutes_per_issue`）→ 认定业务进行中崩溃：
