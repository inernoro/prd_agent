...
3. **verify**（等 3-5 秒让其他 worker 落地后）：
   ...
   b. **过滤掉已完成的 claim**：若评论里同时存在 `visual-run:{X}:claim:*` 和 `visual-run:{X}:terminal:*`（同 run_id），则 X 是历史已完成 round，从竞争池里剔除——只看"活跃 claim"（有 claim 无 terminal）。否则重测场景下,旧 round 的最早 claim 会永远战胜新 round
   c. 在活跃 claim 池里按 ISO 时间戳升序排序，**最早**的为 winner（同毫秒则比 run_id 字典序）
   c. 本 run_id 是 winner → 继续 §3 执行矩阵
   d. 本 run_id 是 loser → **静默 back-out**：
      - 发评论：`撤回 run_id={uuid}，已被 run_id={winnerId} 抢先`
...
3. 兜底再次确认 `visual-test:pending` 已不在 issue 上（防御接单阶段在 race 下未删干净）
4. 终态评论末尾追加 `<!-- visual-run:{run_id}:terminal:{iso8601-ts} -->` 指纹
