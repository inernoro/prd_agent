...
| 维度 | 状态 | 详情 |
|------|------|------|
| 管理后台路由 | ✅ 新增 / 🔄 修改 / ➖ 无变化 | {路由路径} |
| 管理后台菜单 | ✅ 新增 / 🔄 修改 / ➖ 无变化 | {菜单位置} |
...
2. **测试顺序明确**：给出先后顺序和具体步骤
3. **页面测试可操作**：打开哪个页面→点什么→期望看到什么
4. **文档欠债列出**：应更新但未更新的明确标注待办
5. **风险诚实**：已知限制必须如实说明

### 禁止

1. **空话**：不能写"测试一下就行"，要写"用 `/smoke defect-agent` 测试"
2. **遗漏 Breaking Change**：API 变更/字段重命名/行为改变必须标出
3. **跳过质量检查**：编译和类型检查必须实际运行
4. **模糊后续事项**：每项必须有优先级和可操作描述
