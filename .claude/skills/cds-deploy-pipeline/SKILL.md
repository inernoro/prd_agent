...
> ⚠️ **已合并入 `cds` 技能 (2026-04-18)**。本技能进入维护模式。
>
> **新用法**：
> ```bash
> cdscli deploy                         # 完整流水线 push+pull+deploy+ready+smoke
> cdscli help-me-check <branchId>       # 自动诊断 + 根因分析
> cdscli self update --branch <b>       # CDS 自更新
> ```
> 完整文档：`.claude/skills/cds/SKILL.md` + `.claude/skills/cds/reference/`
>
