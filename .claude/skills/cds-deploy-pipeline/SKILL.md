...
等价于：`git push → cdscli branch pull → cdscli branch deploy → 轮询 → smoke`。

智能判断：
- 仅改 `prd-api/` → 单 profile deploy（`--profile api`）
- 仅改 `prd-admin/` → 单 profile deploy（`--profile admin`）
- 混合改动 → 全量 deploy
...
| "环境变量是不是少配了" | `$CLI env get --scope <projectId>` + `$CLI branch exec <id> 'printenv'` |
| "API 返回 401" | 参考 [../cds/reference/auth.md](../cds/reference/auth.md) 双层认证决策树 |
| "灰度什么状态" | `$CLI project list --human` |
| "重启 API" | `$CLI branch deploy <id> --profile api` |
| "停了不用了" | `$CLI branch stop <id>`，确认不再要再 `$CLI branch delete <id>` |
...
| L2 代码验证 | 改动确实部署上去了 | `$CLI branch exec <id> 'grep -c NewFunc /app/...'` |
