...
```bash
CDS="https://$CDS_HOST"
# CDS canonical branch id 是 ${projectSlug}-${slugify(branch)}，不是裸 `tr '/' '-'`。
# 多项目 CDS 下若直接用 `claude-foo` 会 404；正确做法是查 /api/branches 找到
# git branch 字段匹配当前分支的那条记录的 `id`。
GIT_BRANCH=$(git branch --show-current)
BRANCH_ID=$(curl -sf -H "X-AI-Access-Key: $AI_ACCESS_KEY" "$CDS/api/branches" \
  | python3 -c "import json,sys,os
branch=os.environ['GIT_BRANCH']
for b in json.load(sys.stdin).get('branches',[]):
    if b.get('branch')==branch: print(b['id']); break")
if [ -z "$BRANCH_ID" ]; then
  echo "✗ 找不到分支 $GIT_BRANCH 在 CDS 的 canonical id；先跑 /cds-deploy 部署" >&2
  exit 1
fi
