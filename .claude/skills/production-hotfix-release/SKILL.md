...
> **版本**：v1.0.2 | **状态**：已落地 | **触发**：`/hotfix-prod`、"热发布线上版本"、"线上热修"、"把特定分支发到正式环境"、"production hotfix"
...
- `doc/rule.platform.production-release-safety.md`：生产发布安全唯一规则 SSOT。
- `doc/debt.platform.production-release.md`：当前尚未偿还的发布机制缺口。

如果规则要求的原子切换或公网表面 smoke 仍为 open，本技能必须明确风险并使用现有最安全路径，不得把容器/API 健康包装为产品发布完成。
...
至少验证以下四类，不输出敏感信息。页面验证必须使用 GET 取得正文，解析实际入口 JS/CSS 并验证资源 200、MIME 合理、大小大于 0；只执行 HEAD 或只检查 `/health` 不算页面通过：
...
若主页面、入口资源或关键交互失败，即使 API 和容器健康也必须判定发布失败，并按规则 SSOT 恢复 previous 或执行用户授权的最小回滚。
...
| 冒烟接口生成 | `smoke-test` |
| 公网最终入口真人验收 | `acceptance-checklist` |
| 发布前运维风险评估 | `risk-matrix` |
| 对抗式代码与边界复审 | `human-verify` |
