...
当目标是缺陷复测、缺陷修复、发布后缺陷验收，或报告库为“缺陷修复验收报告”时，报告首屏必须额外包含三段，且这些段落必须出现在步骤截图之前：

1. `缺陷引用`
   - 缺陷编号。
   - 缺陷标题。
   - 用户原始问题摘录，来自缺陷 `RawContent` 或结构化字段，1-3 句，不超过 240 字。
   - 影响位置：页面、接口、模块或用户路径。
   - 修复目标：本轮 commit/PR 预计解决什么。
2. `提交与发布证据`
   - PR 地址，必须是可点击链接；没有 PR 时写明原因。
   - commit 地址，必须是可点击链接；没有 commit URL 时至少写完整 commit SHA。
   - commit SHA、分支、发布状态、验收地址。
   - 更新中心关联缺陷入口的验证结论。
3. `验收事项清单`
   - 从缺陷内容、PR 标题、commit message 和改动断言总结 3-8 条验收事项。
   - 每条必须包含“验收事项”“预期结果”“证据”。
   - 用户看完这一段，应先知道本报告准备证明什么，再看后续截图。

缺陷报告不得只写“页面可达”或“截图如下”。缺陷引用、提交证据、验收事项三者任一缺失，归档脚本应拒收。
