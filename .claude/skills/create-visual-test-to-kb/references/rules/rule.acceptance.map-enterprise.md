...
sourceCommit: 44a28a6d86c5ca51354d72821fb950eed50a3ab4+worktree
contentHash: sha256:cf3365adbbe6c803287f6ed03dca63e1e58a1ad40c91395abc31408689ed4aec
Update with: python3 scripts/sync-acceptance-rule-snapshots.py
-->

# MAP 企业级自动化验收规范 · 规则

> **版本**：v1.6 | **日期**：2026-08-14 | **状态**：已落地
...
任何带验收结论、证据或 Verdict 的执行报告都必须归档到 CDS `/reports`，并以线上直达深链通过 `verify-open` 后才算完成。`/tmp`、`file://`、本机 HTML 和 Markdown 只允许作为生成过程中的临时写作源或诊断草稿，不是验收报告，不得作为通知入口、分享链接或交付结果。

CDS 归档失败、页面打不开、正文或图片缺失、内部链接不可点击时，必须判定验收链路失败并保持未完成。不得因为本地文件能打开就降级为本地交付，也不得用本地通过冒充线上验收完成。
