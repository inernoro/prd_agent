...
def build_interactive_html(title, verdict, markdown_content, manifest):
    ...
    for shot in manifest:
        ...
        if verdict == "fail" and status_class == "risk":
            status_class = "fail"
...
<style>
:root{{color-scheme:light;--text:#111827;--muted:#4b5563;--subtle:#6b7280;--line:#d0d7de;--soft:#f6f8fa;--soft-2:#eef2f7;--panel:#fff;--pass:#00703c;--warn:#b35c00;--fail:#b42318;--fail-ink:#7f1d1d;--link:#005ea8;--ink:#0b0f19;--focus:#1d70b8;--shadow:0 1px 2px rgba(16,24,40,.06)}}
*{{box-sizing:border-box}}html{{scroll-behavior:smooth}}body{{margin:0;background:#f3f4f6;color:var(--text);font:15px/1.58 -apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,"Helvetica Neue",Arial,sans-serif}}
.layout{{display:grid;grid-template-columns:minmax(260px,320px) minmax(0,1fr);min-height:100vh}}
aside{{position:sticky;top:0;height:100vh;overflow:auto;border-right:1px solid var(--line);background:#fff;color:var(--text);padding:18px 16px}}
main{{min-width:0;width:100%;max-width:none;padding:0 36px 72px}}
.hero{{margin:0 -36px 24px;padding:28px 36px 24px;background:#fff;border-bottom:1px solid var(--line);box-shadow:0 1px 0 rgba(16,24,40,.02)}}
.title{{max-width:1120px;margin:0 0 14px;font-size:30px;line-height:1.22;letter-spacing:0;font-weight:800;color:var(--ink)}}.badge{{display:inline-flex;align-items:center;min-height:28px;padding:3px 11px;border-radius:3px;color:#fff;font-size:14px;font-weight:900;letter-spacing:0;background:var(--subtle);vertical-align:middle;text-transform:none}}
.badge.pass{{background:var(--pass)}}.badge.conditional{{background:var(--warn)}}.badge.fail{{background:var(--fail)}}
.failure-focus{{margin:0 0 20px;border:2px solid var(--fail);border-left-width:8px;border-radius:2px;background:#fff;padding:16px 18px;box-shadow:var(--shadow)}}.failure-focus h2{{margin:2px 0 12px;padding:0;color:var(--fail-ink);font-size:22px;line-height:1.3}}.focus-kicker{{font-size:13px;font-weight:900;color:var(--fail);letter-spacing:0}}.problem-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(280px,1fr));gap:12px}}.problem-card{{position:relative;border:1px solid var(--line);border-left:5px solid var(--subtle);border-radius:3px;background:#fff;padding:12px 13px}}.problem-card.is-fail{{border-left-color:var(--fail);background:#fff7f7}}.problem-card.is-risk{{border-left-color:var(--warn);background:#fff8eb}}.problem-card.is-gap{{border-left-color:#59636e;background:#f6f8fa}}.problem-card strong{{display:block;font-size:14px;line-height:1.45}}.problem-card strong span{{display:inline-block;margin-right:8px;padding:1px 7px;border-radius:3px;background:var(--fail);color:#fff;font-size:12px;font-weight:900}}.problem-card.is-risk strong span{{background:var(--warn)}}.problem-card.is-gap strong span{{background:#59636e}}.problem-card p{{border:0;box-shadow:none;background:transparent;margin:7px 0 9px;padding:0;color:var(--muted);font-size:13px;line-height:1.45}}.problem-card a{{font-weight:800;text-decoration:underline;text-underline-offset:2px}}
.method-note{{margin:-2px 0 14px;border:1px solid #9cc3e5;border-left:5px solid var(--link);border-radius:2px;background:#eff6ff;color:#1f2937;padding:10px 12px;font-size:13px;line-height:1.55;box-shadow:var(--shadow)}}.method-note strong{{color:#0f172a}}.method-note a{{font-weight:800;text-decoration:underline;text-underline-offset:2px}}.method-note span{{color:#475569}}
.metric-grid{{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-top:18px;max-width:1180px}}.metric{{background:#fff;border:1px solid var(--line);border-top:4px solid var(--link);border-radius:3px;padding:11px 12px;box-shadow:var(--shadow)}}.metric span,.metric small{{display:block;color:var(--subtle);font-size:12px}}.metric strong{{display:block;font-size:28px;line-height:1.05;margin:5px 0 4px;color:var(--ink);font-weight:850}}
.toolbar{{position:sticky;top:0;z-index:5;background:#fff;border:1px solid var(--line);border-radius:3px;padding:10px;margin:0 0 18px;display:flex;gap:8px;flex-wrap:wrap;align-items:center;box-shadow:0 2px 8px rgba(16,24,40,.08)}}
.toolbar input{{min-width:280px;flex:1;border:2px solid #6b7280;border-radius:2px;padding:8px 10px;font:inherit;background:#fff}}button{{border:1px solid #9ca3af;background:#fff;border-radius:3px;padding:8px 11px;font:inherit;font-weight:700;cursor:pointer;transition:background .14s ease,border-color .14s ease,color .14s ease}}button:hover{{border-color:#4b5563;background:#f3f4f6}}button:focus-visible,input:focus-visible,a:focus-visible{{outline:3px solid var(--focus);outline-offset:2px}}button.active{{border-color:var(--link);color:#fff;font-weight:800;background:var(--link)}}
.nav-title{{font-size:15px;font-weight:900;margin:0 0 10px;color:var(--ink)}}.evidence-nav{{display:flex;flex-direction:column;gap:8px;margin-bottom:18px}}.evidence-nav a{{display:grid;grid-template-columns:78px minmax(0,1fr);gap:10px;align-items:start;text-decoration:none;color:var(--text);border:1px solid var(--line);border-left:4px solid transparent;background:#fff;border-radius:3px;padding:7px;min-height:66px;box-shadow:var(--shadow)}}.evidence-nav a.is-fail{{border-color:#f0b4ae;border-left-color:var(--fail);background:#fff7f7}}.evidence-nav a.is-risk{{border-color:#f1c27a;border-left-color:var(--warn);background:#fff8eb}}.evidence-nav a:hover{{border-color:var(--link);box-shadow:0 0 0 2px rgba(0,94,168,.14)}}.evidence-nav .nav-thumb{{width:78px;aspect-ratio:16/9;object-fit:cover;border:1px solid var(--line);border-radius:2px;background:#f3f4f6}}.nav-copy{{min-width:0}}.evidence-nav span{{display:block;font-weight:900;margin:0 0 2px;color:var(--ink)}}.evidence-nav small{{display:block;color:var(--muted);font-size:12px;line-height:1.35;margin-top:0;overflow:hidden;display:-webkit-box;-webkit-line-clamp:3;-webkit-box-orient:vertical}}
.gallery-title{{font-size:18px;font-weight:900;margin:8px 0 10px;color:var(--ink)}}.evidence-gallery{{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px;margin:0 0 24px}}.evidence-card{{position:relative;display:block;text-decoration:none;color:var(--text);background:#fff;border:1px solid var(--line);border-left:4px solid transparent;border-radius:3px;overflow:hidden;box-shadow:var(--shadow)}}.evidence-card:hover{{border-color:var(--link);box-shadow:0 0 0 2px rgba(0,94,168,.14)}}.evidence-card.is-fail{{border-color:#f0b4ae;border-left-color:var(--fail);box-shadow:0 6px 16px rgba(180,35,24,.12)}}.evidence-card.is-risk{{border-color:#f1c27a;border-left-color:var(--warn);box-shadow:0 6px 14px rgba(179,92,0,.1)}}.evidence-card img,.thumb-placeholder{{width:100%;aspect-ratio:16/9;object-fit:cover;border:0;border-radius:0;border-bottom:1px solid var(--line);background:#f6f8fa}}.thumb-placeholder{{display:grid;place-items:center;color:var(--muted);font-size:13px}}.card-badge{{position:absolute;top:8px;left:8px;z-index:2;padding:2px 8px;border-radius:3px;background:var(--fail);color:#fff;font-style:normal;font-size:12px;font-weight:900;box-shadow:0 1px 4px rgba(0,0,0,.22)}}.card-badge.risk{{background:var(--warn)}}.card-badge.gap{{background:#59636e}}.evidence-card strong,.evidence-card span{{display:block;padding:0 10px}}.evidence-card strong{{padding-top:9px;color:var(--ink)}}.evidence-card span{{font-size:12px;color:var(--muted);line-height:1.35;padding-bottom:10px}}
#reportBody{{display:block;max-width:1280px}}#reportBody>h1{{display:none}}h1,h2,h3{{scroll-margin-top:92px}}h2{{font-size:21px;line-height:1.35;margin:30px 0 10px;padding:0 0 7px;border-bottom:2px solid var(--line);color:var(--ink);font-weight:850}}h3{{font-size:16px;margin:18px 0 8px;color:var(--ink)}}p,ul,ol,blockquote,pre{{background:#fff;border:1px solid var(--line);border-radius:3px;margin:0 0 12px;padding:12px 14px;box-shadow:var(--shadow)}}a{{color:var(--link)}}code{{background:var(--soft);border:1px solid var(--line);border-radius:3px;padding:1px 4px}}pre{{background:#111827;color:#f9fafb;overflow:auto}}
.table-wrap{{overflow-x:auto;margin:0 0 18px;border:1px solid var(--line);border-radius:3px;background:#fff;box-shadow:var(--shadow)}}table{{border-collapse:separate;border-spacing:0;width:100%;font-size:14px;background:#fff}}th,td{{border-bottom:1px solid var(--line);border-right:1px solid var(--line);padding:10px 12px;text-align:left;vertical-align:top}}th{{background:#eef2f7;color:#111827;font-weight:900}}tr:last-child td{{border-bottom:0}}th:last-child,td:last-child{{border-right:0}}tbody tr:hover td{{background:#f5fbff}}tr.row-fail td{{background:#fff5f5;border-top:1px solid #f0b4ae;border-bottom:1px solid #f0b4ae}}tr.row-fail td:first-child{{box-shadow:inset 5px 0 0 var(--fail)}}tr.row-risk td{{background:#fff8eb}}tr.row-risk td:first-child{{box-shadow:inset 5px 0 0 var(--warn)}}tr.row-gap td{{background:#f6f8fa;color:#59636e}}tr.row-gap td:first-child{{box-shadow:inset 5px 0 0 #59636e}}tr.is-hidden{{display:none}}
figure{{margin:18px 0 30px}}img{{max-width:100%;height:auto;border:1px solid var(--line);border-radius:3px;display:block;background:#fff}}figcaption{{color:var(--muted);font-size:13px;margin-top:6px}}blockquote{{border-left:5px solid #9ca3af;color:#374151}}.section-toggle{{float:right;font-size:12px;padding:4px 8px}}.figure-anchor{{display:block;scroll-margin-top:112px;height:1px}}:target{{outline:4px solid #facc15;outline-offset:4px;border-radius:3px}}
@media(prefers-reduced-motion:reduce){{*{{scroll-behavior:auto!important;transition:none!important}}}}
@media(max-width:980px){{.layout{{display:block}}aside{{position:relative;height:auto;border-right:0;border-bottom:1px solid var(--line)}}main{{padding:0 16px 60px}}.hero{{margin:0 -16px 18px;padding:22px 16px}}.metric-grid{{grid-template-columns:repeat(2,minmax(0,1fr))}}.toolbar{{top:0}}}}
</style>
</head>
<body>
<div class="layout">
