...
**一句话**：改任何被多方共享的全局值之前先列出消费方清单——共享密钥、共享库、共享队列已经出过四次静默事故。
**什么时候撞上**：动全局密钥、全局环境变量、共享 Mongo / Redis，或任何跨项目共用的状态。
...
| 9 | CDS **项目级** env（`customEnv[projectId]`，2026-09-01） | 该项目**所有分支预览**容器 | 项目 env 是项目范围的单份值，任一分支的 Agent 改它，其余分支在**下一次部署**时静默拿到新值——改的人看到自己那条分支好了，别人的分支在几十分钟后才发现登不进去，两件事之间没有任何提示 | **未隔离**（现状）。cds-self 一个项目下同时挂着三条 Agent 分支预览，`CDS_PREVIEW_USERNAME` / `CDS_PREVIEW_PASSWORD` 只有一份。改项目 env 前先 `GET /api/branches` 看这个项目还有谁在跑；门禁类的键（`CDS_PREVIEW_*`）尤其不要「为了自己能登进去」就地轮换。真要按分支隔离，得让预览实例支持分支级 env 覆盖——那是新契约，现在没有 |

| 10 | 共享 Mongo 的**外站快照集合**（`model_leaderboard_snapshots`，2026-09-15） | 所有分支预览 + 权威部署的榜单页与首页挂件 | 一个榜在库里只有一条文档。周期同步已按 `DeploymentAuthority.CanRunSharedScheduledWork` 只在权威部署跑，但**手动同步入口没有这道判断**——预览上点一次「立即同步」就把兄弟分支正在读的那条换成本分支未合并解析器的产物。后果不是打坏数据，是**把别人的验收变成谎话**：兄弟分支以为看到的是自己代码的产出（通道 8 那次「修了像没修」的形状） | **已隔离**（2026-09-15，PR #1538）：`ModelLeaderboardSnapshot.DeploymentSlug` 盖 `DeploymentScope.CurrentDurable`（取**分支级**、不含 revision——快照怕跨分支互覆，不怕滚动发布抢单；带 revision 会让每次推送都丢掉自己刚同步的那份，而预览上没有周期 worker 来补）。读取是「自己的优先、权威的兜底」，预览没自己同步过时照旧显示权威那份，否则每条新预览都是空页、验收无从下手。判据（看得见 / 是自己的 / 挑哪一份 / 写哪条文档的 Id）全部收在 `ModelLeaderboardScope` 一处。权威部署（作用域 null）行为逐字不变，存量文档缺该字段读作 null，**不需要迁移**。新增其它「存外站数据的共享集合」照此加作用域 |
...
| 2026-09-01 | CDS 项目级 env 跨分支 | cds-self 的 `CDS_PREVIEW_USERNAME` / `CDS_PREVIEW_PASSWORD` 在 10:52 被同项目另一条分支（`claude/cds-button-skill-source-design-mpq1bh`）的 Agent 用 `PUT /api/env/<KEY>?scope=f9e8b956d3dd` 轮换，它随即部署了自己的分支；`claude/task-schedule-redesign-lunkri` 在 11:11 的下一次部署把新口令一起烤进容器，原本 07:00 还能登进去的那对口令 11:18 起 401，验收链路断了一整轮。证据：父实例 http-logs 两条 PUT（同一秒、同一出口段）+ 两次登录响应头（200/401，同分支同上游）。修复：本表新增通道 9。**曾试图**给子实例补一条 `CDS_PREVIEW_AI_ACCESS_KEY` 专用钥匙通道让 Agent 不必为自测去动共享门禁口令——**该方案已在合并前撤除，不要照做**：它本身就落在通道 9 上（项目级 env 同项目所有分支共用一份），而进程级静态钥匙在鉴权里不带项目作用域、等价管理员，结果是任一条未合并分支能以管理员身份打到兄弟分支的预览。正解是让子实例**在自己库里**建一把只对自己有效的凭据——注意不是把父实例签的项目级 Agent Key 转交过去，那把钥匙是拿收方实例自己库里的哈希比对的，而子实例独立存储、首启 seed，父签的它不认——判据是「一条预览的凭据拿去打同项目另一条预览必须被拒」；在它落地之前，Agent 自测只能由人给口令，台账见 `doc/debt.cds.md` |
