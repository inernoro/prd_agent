...
### 3.4 Key：两个方向都必须永久，签一次用到撤销

用户明确要求不重复配对。两个方向分别是：

| 方向 | 用哪把 | 永久性 |
|---|---|---|
| MAP / llmgw → CDS（提交声明、上报） | CDS **项目级 Agent Key** `cdsp_<slug12>_<suffix>` | `AgentKey` 类型**没有 `expiresAt` 字段**，只有显式 DELETE 才写 `revokedAt`——天生永久 |
| CDS → 被监控方自检端点（探测） | 声明里携带的 `probeToken` | 被监控方**在自己库里** seed，随声明交付，CDS 加密存 |
...
`probeToken` 有一条硬禁令：**不许放在 CDS 项目级 env**。
`cross-project-isolation` 通道 9 记着 2026-09-01 的事故——项目级 env 是项目范围的单份值，
同项目所有分支共用，任一分支的 Agent 轮换它，别人的分支在下一次部署时静默 401。
那次事故的复盘结论就是「让子实例在自己库里建一把只对自己有效的凭据」，这里照办。

判据：**一条预览的 probeToken 拿去打同项目另一条预览，必须被拒。**
