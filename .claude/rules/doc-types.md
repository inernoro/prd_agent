...
**一句话**：doc/ 下每篇文档必须属于七种前缀之一、按应用名点分层级命名、带导读三行；design 类不写实现代码。
**什么时候撞上**：新建或重命名 doc/ 下的文档。
...
H1 与版本行之后、正文之前，必须写三行，让读者十秒决定读不读（SSOT：`doc/rule.doc.readability.md`）：

```markdown
**一句话**：说清这篇的内核（关键机制 / 对象 / 约束），20-100 字；术语允许但要紧跟中文括号解释，禁代码、路径、空话。
**谁该读**：什么角色、在什么情况下会来读。
**读完能做什么**：读完能采取的具体动作，或能回答的具体问题。
```

例外：`report.` 周报读者固定，沿用既有的 `**本周一句话**` 即可，不强加另外两行。

自查与闸门：`python3 scripts/doc-readability-check.py --ratchet`（CI `docs-readability` job）。
存量欠账走棘轮，只许降不许升——**新文档不带导读三行会直接把 CI 弄红**。

## design.* 特别要求

- **管理摘要必填**：第一节必须是管理摘要，30 秒让非技术读者看懂方案全貌
- **受众分层**：前四节（摘要 → 背景 → 目标 → 决策）面向所有读者，禁止代码；技术章节允许代码但需有上下文
- **不写实现**：禁止实现语言代码块（C#/TS/Rust/Python 等）、接口签名、目录树、逐文件改法；保留数据流与关键表 / 字段设计，源码路径集中到文末「实现来源」一节。判据「实现无关性测试」：换个语言重写这块，这段话还成立吗（`doc/rule.doc.readability.md` 条款 4）
- **故事靠前，设计靠后**：章节顺序为 管理摘要 → 产品定位 → 用户场景 → 核心能力 → 架构 → 数据设计 → 接口设计 → 关联文档 → 风险。人能看懂的在前，接口字段在后
