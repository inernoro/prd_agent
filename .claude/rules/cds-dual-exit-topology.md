...
**一句话**：一个分支跑多个对外容器时必须给两个独立 HTTPS 出口，并逐个标明容器职责与共享基础设施，不许留匿名容器。
**什么时候撞上**：一个分支里同时部署主应用与独立服务（网关剥离、微服务拆分、独立观测前端）。
...
网关剥离后，一个分支里同时跑着 MAP 后端（prd-api）、MAP 前端（prd-admin）、网关 serving（llmgw-serve，8091）、网关观测前端（llmgw-web），外加共享的 Mongo / Redis。用户面对 CDS 面板上的一排容器，分不清哪个是干嘛的、网关到底从哪个 URL 进、是不是每个出口都是 HTTPS。「双出口是不是两个都走 HTTPS」「redis/mongo 是不是共享的」这些本该一目了然的事，用户得反复问。剥离的价值之一就是「网关有独立入口、可被别人调用」——入口不透明，这个价值就打了折。本规则把「双出口 + 拓扑透明」固化为多容器部署的交付契约。
...
- 入口名称与页面路径必须由 `cds.web-entry-name` / `cds.web-entry-path` 声明（compose 是 Agent 侧的 SSOT；用户侧等价入口是分支面板「应用已上线 → 配置入口」，写的是同一份 `BuildProfile.subdomain` / `webEntry`，可选存项目档或分支档）。`cds.readiness-path` 是机器探针，禁止展示成用户入口；API-only 服务可以有 `cds.subdomain`，但不应声明 Web 入口。
...
| 角色 | 说明 | 本次实例 |
|---|---|---|
| 前端（web） | 用户直接访问的 UI | prd-admin（MAP 前端）、llmgw-web（网关观测前端，自带登录） |
| 后端（api） | 主应用业务 API | prd-api（MAP 后端） |
| serving | 独立可被调用的服务引擎 | llmgw-serve（网关 serving，8091，`/gw/v1/*`，X-Gateway-Key 门） |
...
- **命名契约**：serving 容器名以 `llmgw-serve` 为前缀、观测前端以 `llmgw-web` 为前缀；主应用维持 `prd-api` / `prd-admin`。命名即角色，禁止随机名。
...
- [ ] 网关容器命名合契约（llmgw-serve / llmgw-web）、数量确定？
...
- 本次实例：`doc/debt.platform.llm-gateway.isolation.md`（波2.5 CDS 命名子域实施证据）、`doc/design.platform.llm-gateway.physical-isolation.md`（当前部署拓扑）。
