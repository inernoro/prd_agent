...
| `prd-admin` 的 tsc / vitest / vite build | CI 的 `Admin Dashboard Build` job 绿（本地 `pnpm` 跑一遍更快） | 本地 或 Actions | 本地即时；那个 job 在 `ci.yml` 里，**普通 feature 分支 push 不会跑**——要么开 PR 到 main/develop，要么手动 dispatch |
