...
| `prd-admin` 的 tsc / lint / vitest | 本地 `pnpm` 跑一遍即可（快），CI 的 `Admin Dashboard Build` 是复核 | 本地 或 Actions | 本地即时；CI 同上 |
...
- 要说测试→ 同上，但**那一档不是全量**：`ci.yml` 跑的是
  ...
  改动碰到那两类用例时，没有任何 CI 会验它——要么本地去掉 filter 自己跑，要么在交付里
  明说这部分没验过。写成「xUnit 全量通过」就是在夸大。
