...
| `llmgw/**` | 编译有远端判据：三家的镜像都在 `branch-image.yml` 里构建（`console-api` 跑 `dotnet publish`、`web` 跑 `pnpm build`），push 即触发；`serving` 另在 `PrdAgent.sln` 里，`Server Build & Test` 也覆盖它，**`console-api` 不在 sln 里**。**此外还有开 PR 才触发的检查**，用下面那条命令列全。其余校验见 `llmgw/AGENTS.md` | 本地 或 Actions | 见命令列出的各条 |
...
- **有远端权威判据的**（sln 编译、非集成非手工 xUnit、镜像构建）：本地只是加速器，它让你更快
  发现错误，但「验证过了」这句话要引那条流水线的结论。
