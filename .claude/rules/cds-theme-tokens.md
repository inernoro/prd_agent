...
**什么时候撞上**：写 CDS 前端的弹窗、代码块、日志面板等任何带背景色或文字色的组件。
...
## 尺寸单位：rem 唯一，px 只留给 1–3px 细线（2026-09-08）

用户实测浏览器缩到 80–85% 时 CDS 最好看，于是整站改为**根字号 85%**（`html { font-size: 85% }`）呈现。
这条路成立的前提是尺寸全部用 rem——任何 >3px 的 px 字面量都不会跟着缩，会在页面里「独自变大」。
