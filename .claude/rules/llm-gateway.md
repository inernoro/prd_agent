...
**一句话**：所有大模型调用走网关；MAP 选择 AppCaller 获准的模型池，LLMGW 默认只在池内调度，跨池代选必须显式开启。
...
1. 用户可选模型必须来自 AppCallerCode 获准的模型池，提交稳定池标识；禁止用 Provider 模型名或 Offering 代替业务选择。
2. LLMGW 可以在所选池内更换成员。跨模型池代选默认禁止，只有 AppCaller 明确配置允许时才能发生。
3. 显式池未知、越权、类型不符或候选耗尽时必须结构化失败，禁止静默落到全局默认或 legacy 路由。
4. 单成员失败只更新该成员健康，不得清空模型目录、禁用 AppCaller 或关闭 MAP 功能。
5. MAP 维护推荐、风险建议与用户偏好；LLMGW 只返回池目录、健康和真实执行指标。
6. Open 成员通过受限真实业务请求进入 HalfOpen；禁止默认创建消耗额度的合成探测请求。
...
**根因**：`ILlmGateway` 在执行前会读 `ILLMRequestContextAccessor.Current`；若 `UserId == null`，走授权/限流逻辑就会炸。HTTP 入站请求默认**不会**自动填入 UserId —— 必须由调用方显式 `BeginScope` 一次。

### 硬规则

任何 LLM 调用前（Controller / Service / Worker 均适用），必须在调用栈上打开 `LlmRequestContext` 作用域：
...
using var _ = _llmRequestContext.BeginScope(new LlmRequestContext(
    ...
    RequestType:   "chat",                  // chat / intent / vision / generation ...
    AppCallerCode: "your-agent.feature::chat"
));
...
Worker / BackgroundService 场景：UserId 从触发任务的实体读出（如 `run.OwnerUserId`），同样必须设置，否则 LLM 日志里 `UserId` 会是空，告警/配额/账单都对不上人。

参考实现：`ChatService.cs:278`、`AIGapDetector.cs:53`、`ImageGenController.cs:405`。

### 判定清单

写任何新 Controller / Service 调 LLM 时逐条核对：

- [ ] 调用方注入了 `ILLMRequestContextAccessor`？（字段常见名 `_llmRequestContext` / `_ctxAccessor`）
- [ ] LLM 调用前用 `using var _ = ctx.BeginScope(new LlmRequestContext(...))` 打开作用域？
- [ ] `UserId` 从 `this.GetRequiredUserId()`（Controller）/ 实体字段（Worker）填入，不是 `null`？
- [ ] `AppCallerCode` 格式为 `{app-key}.{feature}::{model-type}`？
- [ ] 跑一次真实流程，`llmrequestlogs` 集合里对应记录的 `userId` 非空？
...
4. 重试 3 次都是同一个错，用户看到超时

修复：Controller 在 `_gateway.StreamAsync` 之前 `using var _ = _llmRequestContext.BeginScope(...)`，UserId 走 `this.GetRequiredUserId()`。
