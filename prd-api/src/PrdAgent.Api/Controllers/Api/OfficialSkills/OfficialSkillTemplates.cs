...
1. `GET {domain}/api/defect-agent/agent/connector` 确认连接器协议和长期授权。
...
2. 使用视觉验收技能跑正式环境验收，并把报告归档到“缺陷修复验收报告”知识库。
3. `POST {domain}/api/defect-agent/agent/resolution-traces/{traceId}/validation-report` 回写报告地址并通知提交人。
