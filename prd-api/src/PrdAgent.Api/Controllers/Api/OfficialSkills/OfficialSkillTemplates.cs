...
如果用户没设 `PRD_AGENT_API_KEY`，引导去 `{{BASE_URL}}/marketplace` 的「接入 AI」按钮新建。明文 Key 只允许保存到本机 secrets/Keychain/CI Secret 或当前 shell 临时变量，禁止写入仓库、`.claude/settings.local.json`、PR、验收报告或公开日志。
