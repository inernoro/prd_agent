...
public static class OfficialSkillTemplates
{
    ...
    public const string AiDefectResolveVersion = "1.2.0";
    public const string AiDefectResolveReleaseDate = "2026-06-17";
...
兼容旧分享包：如果只有 `scope.shareUrl`，仍可按分享端点处理，但不要把分享链接当成日常任务主路径。

## 自动化流程

1. `GET {domain}/api/defect-agent/agent/next` 拉取下一条缺陷。
2. `POST {domain}/api/defect-agent/agent/defects/{defectId}/comments` 评论修复计划。
3. 按轻量标准判断能否自动修复；重量级问题评论原因后停止。
4. 轻量修复后执行代码校验并提交中文 commit。
5. `POST {domain}/api/defect-agent/agent/defects/{defectId}/commit-info` 回写 `commitSha`、分支、预览和验收报告地址。
6. 评论验收方式。
7. `POST {domain}/api/defect-agent/agent/defects/{defectId}/fix-status` 标记修复。
8. 再拉下一条，重复以上步骤。
