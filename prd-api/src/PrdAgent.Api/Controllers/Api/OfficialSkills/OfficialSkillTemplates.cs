...
public static class OfficialSkillTemplates
{
    ...
    public const string AiDefectResolveVersion = "1.3.0";
    public const string AiDefectResolveReleaseDate = "2026-06-18";
...
首次 setup 可以只提供 domain，但必须由登录用户发起：
...
已有可用 Key 时复用并返回元信息；没有时新建并仅本次返回明文 `apiKey`。后端不保存明文 K，日常任务必须保存这次返回的 K。
