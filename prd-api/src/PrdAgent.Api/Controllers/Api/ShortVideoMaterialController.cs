...
public class ShortVideoMaterialController : ControllerBase
{
    ...
    public async Task<IActionResult> CreateRun([FromBody] CreateShortVideoMaterialRequest req)
    {
        ...
        try
        {
            var parsed = await ResolveParsedSourceAsync(videoUrl, platform, req.SourceText, req.Title);
            title = CleanTitle(req.Title) ?? CleanTitle(parsed.Title) ?? title;
            var sourceText = NormalizeSourceText(parsed.SourceText, title, videoUrl, platform);
            ...
            var store = await ResolveStoreAsync(req.StoreId, userId);
            ...
            var sourceEntry = await CreateMarkdownEntryAsync(
                ...
                userName,
                $"{title} · 原始视频素材.md",
                BuildSourceMarkdown(title, videoUrl, platform, parsed),
                $"{title}：原始短视频素材",
                new List<string> { "短视频", "素材", platform, "原始视频" },
            ...
            var transcriptEntry = await CreateMarkdownEntryAsync(
                ...
                userName,
                $"{title} · 字幕文稿.md",
                BuildTranscriptMarkdown(title, videoUrl, platform, sourceText, parsed.SourceMode),
                $"{title}：短视频字幕与文案素材",
                new List<string> { "短视频", "字幕", "文案", platform },
                new Dictionary<string, string>
                {
            ...
            var timelineEntry = await CreateMarkdownEntryAsync(
                ...
                userName,
                $"{title} · 时间轴片段.md",
                BuildTimelineMarkdown(title, videoUrl, platform, timeline),
                $"{title}：短视频时间轴片段",
                new List<string> { "短视频", "时间轴", "片段", platform },
                new Dictionary<string, string>
                {
            ...
            var response = new ShortVideoMaterialRunResponse
            {
                ...
                EntryIds = new List<string> { sourceEntry.Id, transcriptEntry.Id, timelineEntry.Id },
                StoreUrl = $"{baseUrl}/document-store?storeId={store.Id}",
                SourceUrl = $"{baseUrl}/document-store?storeId={store.Id}&entryId={sourceEntry.Id}",
                TranscriptUrl = $"{baseUrl}/document-store?storeId={store.Id}&entryId={transcriptEntry.Id}",
                TimelineUrl = $"{baseUrl}/document-store?storeId={store.Id}&entryId={timelineEntry.Id}",
            };
            return Ok(ApiResponse<ShortVideoMaterialRunResponse>.Ok(response));
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "短视频素材解析失败 run={RunId}", run.Id);
            run.Status = "failed";
            run.ErrorMessage = ex.Message;
            MarkFirstRunningStageFailed(run, ex.Message);
            run.UpdatedAt = DateTime.UtcNow;
            await SaveRunAsync(run);
            return StatusCode(500, ApiResponse<ShortVideoMaterialRunResponse>.Fail(ErrorCodes.INTERNAL_ERROR, $"短视频素材解析失败：{ex.Message}"));
        }
    }
