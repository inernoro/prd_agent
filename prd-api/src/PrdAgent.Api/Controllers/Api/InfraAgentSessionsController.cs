...
public class InfraAgentSessionsController : ControllerBase
{
        ...
        string blockingCode)
    {
        ...
        return new[]
        {
            ...
            new SidecarExecutionTask(
                7,
                "R0.5",
                "CDS-managed runtime capacity contract",
                r0Done ? "done" : r0Active ? "done_minimal" : "waiting",
                r0Done ? "CDS-managed runtime capacity is available." : "CDS capacity contract is in place; remote host/env/image remain operator fallback.",
                r0Done ? "done" : "done",
    ...
    [HttpGet("schedule-dashboard")]
    public async Task<IActionResult> ScheduleDashboard([FromQuery] int days = 14, CancellationToken ct = default)
    {
        var userId = this.GetRequiredUserId();
        var dashboard = await _service.GetScheduleDashboardAsync(userId, days, ct);
        return Ok(ApiResponse<object>.Ok(new { dashboard }));
    }
