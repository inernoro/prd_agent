...
public class AiToolboxController : ControllerBase
{
    ...
    private static string EnrichSystemPromptWithTools(string basePrompt, List<string>? enabledTools)
        // 实现已抽到 ToolboxPromptEnricher（SSOT），与 AgentUniverseController 的 custom:{id} 路径共用，避免漂移。
        => ToolboxPromptEnricher.EnrichSystemPromptWithTools(basePrompt, enabledTools);
