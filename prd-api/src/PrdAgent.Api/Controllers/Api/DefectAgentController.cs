...
public class DefectAgentController : ControllerBase
{
    ...
    public async Task<IActionResult> ResolveDefect(string id, [FromBody] ResolveDefectRequest request, CancellationToken ct)
    {
        ...
        // 报告人自己标记完成时无需再通知自己。
        if (!string.IsNullOrEmpty(defect.ReporterId) && defect.ReporterId != userId)
        {
            ...
            var reporterNotification = new AdminNotification
            {
                ...
                Message = resolutionText == null
                    ? $"你提交的缺陷「{defectLabel}」已由 {resolverName} 处理完成，请前往验收。"
                    : $"你提交的缺陷「{defectLabel}」已由 {resolverName} 处理完成：{resolutionText}",
                Level = "success",
                ActionLabel = "查看并验收",
                ActionUrl = $"/defect-agent?id={defect.Id}",
                ActionKind = "navigate",
