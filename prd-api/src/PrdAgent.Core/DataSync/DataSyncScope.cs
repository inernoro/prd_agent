...
public static class DataSyncScope
{
    ...
    public static readonly IReadOnlyList<DataSyncGroup> Groups = new[]
    {
        new DataSyncGroup("users", "账号与组织", new[]
        {
            ...
            new DataSyncCollection("groups", new[] { "InviteCode" }),
            ...
            new DataSyncCollection("teams", new[] { "InviteCode" }),
    ...
    public static readonly IReadOnlyDictionary<string, string> Excluded = new Dictionary<string, string>
    {
        ...
        ["direct_video_job_ownerships"] = "凭据/票据/分享令牌：跨实例复制等于复制访问权——这张表存的是「谁有权看这个视频任务」（OwnerAdminId + JobId + OfferingId，留 7 天），而查状态/取内容的接口只要匹配到一行就放行。和 users 一起搬过去之后，接同一个视频上游的目标站就能拿这些 JobId 读到源站的视频",
        ...
        ["deployment_markers"] = "本站自己的运行状态（哪些一次性动作已经做过）。搬过去会把目标站已消费的一次性动作重新武装、或反过来把该做的那次吞掉",
