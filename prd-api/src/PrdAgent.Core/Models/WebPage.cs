...
public class HostedSite
{
    ...
    /// <summary>
    /// 这条站点记录还在「占坑」状态：数据库行已经建好、入口对象还没传完。null = 已发布。
    ///
    /// 为什么要有它：带幂等键发布时站点 id 是确定性的，两个并发请求算出同一个对象 key。
    /// 先传后插会让输掉数据库竞争的那一方覆盖赢家的页面，所以顺序是**先插后传**；
    /// 代价是中间存在一小段「行在、对象还没在」的窗口。这个字段就是那段窗口的标记 ——
    /// 幂等查询看见它就不能回「去重成功」（那会把一个还打不开的地址当成功交出去），
    /// 而是接手把这次的内容传完。进程在窗口里挂掉时，它也是那条残留记录的身份证。
    /// </summary>
