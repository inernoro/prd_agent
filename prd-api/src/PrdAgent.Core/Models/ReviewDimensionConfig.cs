...
public static class DefaultReviewDimensions
{
    public static readonly List<ReviewDimensionConfig> All = new()
    {
        ...
        new ReviewDimensionConfig
        {
            ...
            Name = "文档规范完整性",
            MaxScore = 8,
            Description = "检查方案是否包含所有必要章节：文档头（文档名称、产品线/模块、版本、迭代类型、作者、审批人）、项目目的（3个核心要点）、现状背景与问题陈述（现状、问题、需求）、用户范围（核心用户角色及诉求）、实现思路（结构归母+具体设计思路）。每缺少一个主要章节扣 1.5 分，子项缺失酌情扣分。注意：完整≠优秀，仅给「章节齐全」打满分，不替代凝练度/数据密度的评估。",
            OrderIndex = 2,
        },
        new ReviewDimensionConfig
        {
        ...
        new ReviewDimensionConfig
        {
            ...
            Name = "表达质量与凝练度",
            MaxScore = 10,
            Description = "评估文档的凝练度、信息密度与表达规范：① 凝练度：是否一句话能说清的事被拆成多段、是否存在同一观点跨章节反复展开、是否有「我们认为/建议加强/需要重视」等空话占字数。② 信息密度：相同信息量下篇幅是否克制（1000 字优于 3000 字），关键论点是否有具体数据/链接/案例支撑（而非泛泛而谈）。③ 表达规范：术语一致、章节结构正确、语言简明。【重要扣分原则】如方案明显存在可删减 30% 不损失信息的冗余/重复/堆砌，本维度按 50% 得分率封顶；如通篇均为空话套话无具体抓手，本维度按 30% 得分率封顶。质量看密度不看长度，凝练扎实但篇幅短不扣分。",
            OrderIndex = 8,
        },
    };
}
