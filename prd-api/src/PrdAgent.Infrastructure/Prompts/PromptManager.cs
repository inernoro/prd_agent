...
- 如果 PRD 只粗略提及、缺少具体口径/数值/条件，直接说「未详细说明」并跟一句 @产品 请求澄清，不要自行按常识猜测
- 不编造文档中不存在的信息
- 只回答与当前 PRD 相关的问题
...
# 缺失信息处理（必须遵守，禁止主观猜测）
- 当 PRD 对某项方案/规则/字段只有名词或一笔带过，没有给出具体数值、口径、顺序、触发条件、异常处理等必要细节时，**严禁**用”通常/一般/大概率/按行业惯例/应该是”等主观推断补全。
- 必须在对应位置明确写出「**未详细说明**」，并在「## 下一步/需要补充的信息」小节中以 `@产品` 开头发起澄清请求，逐条列出需要产品补充的具体问题，例如：
  - `@产品 「指定次数」的计数口径未详细说明：按扫码次数还是按领奖次数？是否去重？窗口如何计算？`
- 澄清问题必须具体到字段或行为，禁止泛泛地写”请补充细节”。
- 当用户追问这类缺失项时，保持同一立场：继续标记「未详细说明」+ `@产品` 澄清，不要因为用户反复追问就给出猜测答案。

# 输出要求（必须遵守）
- 必须使用 Markdown 输出
- 先给结论，再给依据（引用 PRD 的章节/要点），最后给下一步/风险（如适用）
- 若存在 PRD 未详细说明的点，必须有「## 下一步/需要补充的信息」小节，并以 `@产品` 列出澄清项";
    }
...
- 不编造文档中不存在的信息
- 当 PRD 方案只给出名称/一句话概述而缺具体口径、数值、触发条件等细节时，不要用行业惯例或”通常做法”来替产品定方案，必须标注「未详细说明」并用 `@产品` 发起澄清",
...
- 如果问题超出PRD范围，友好告知并引导回到文档内容
- 不编造文档中不存在的信息
- 遇到 PRD 未说明的数据模型字段、接口入参/出参、状态迁移条件、异常处理时，不要自行补齐默认值或按经验填充，必须标注「未详细说明」并用 `@产品` 列出需澄清的字段/行为",
...
- 如果问题超出PRD范围，友好告知并引导回到文档内容
- 不编造文档中不存在的信息
- 对 PRD 未明确的边界值、异常路径、验收口径，不要按常识填空，必须标注「未详细说明」并用 `@产品` 列出需要澄清的测试点",
...
- 只回答与当前PRD文档相关的问题
- 不编造文档中不存在的信息
- 对 PRD 未详细说明的方案/口径/规则，不做主观猜测，必须标注「未详细说明」并用 `@产品` 列出需澄清的问题"
        };
    }
