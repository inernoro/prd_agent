 ...
 * 离线 HTML 打包的输出上限。
 *
 * SSOT 在后端 `HostedSiteHtmlInliner.DefaultMaxOutputBytes`（守卫见 sourceDownload.test.ts）。
 * 前端抄这个数只为一件事：入口文件**自己**就已经超过它时，按下去必然失败——那一档在按之前
 * 就说清楚，不让用户点一次换一个报错。其余情况（图片多、内嵌后膨胀）只有服务端量得准，交给它判。
 *
 * 此前这里是「取正文代理 2MB 上限 + CDN 注入余量」两个数：下载走的是托管域名经 CDN 服务出来的
 * 那一份，于是要给 CDN 塞进去的遥测留余量。离线打包直接按文件清单从对象存储读原字节，
 * 不经过 CDN，那段余量与那个 2MB 一起退场。
 */
export const OFFLINE_EXPORT_MAX_BYTES = 20 * 1024 * 1024;
...
export type SourceDownloadPlan =
  ...
  /**
   * 下载离线版网页：服务端把站内的样式、脚本、图片、字体都内嵌进入口 HTML，打成一个文件。
   * fileName 只是兜底名，真正的文件名以服务端 Content-Disposition 为准。
   */
  | { kind: 'html'; fileName: string; fileCount: number }
 ...
 *
 * SSOT 在后端 `HostedSiteService.IsRevisionReadableWrapper`——离线打包端点用它判包装站能不能打，
...
export function planSourceDownload(site: SourceDownloadSite): SourceDownloadPlan {
  ...
  /*
   * 只拿**入口文件自己的**大小判：入口一份就超过打包上限，内嵌之后只会更大，必然失败。
   * 其余文件的体积要看入口到底引用了哪些，只有服务端量得准——不在这里凭 totalSize 猜，
   * 否则会把「目录里躺着一个没被引用的大视频」的站点误拦掉。
   * 后端取不到入口条目时给 0：那就不拦，失败时仍有受控文案兜底。
   */
  const entrySize = site.entrySize ?? 0;
  if (entrySize > OFFLINE_EXPORT_MAX_BYTES) {
    return {
      kind: 'unavailable',
      reason: `这份网页的入口文件就有 ${formatBytes(entrySize)}，超过离线打包 20MB 的上限，装不进一个文件。`
...
/**
 * 把一份 Blob 存成本地文件——本仓库网页托管相关页面落盘的唯一出口。
 *
 * 为什么不直接给托管直链挂 a[download]：托管内容在独立域名，跨域的 a[download]
 * 会被浏览器忽略掉 download 属性、退化成导航打开——而「导航打开一份 text/html」
 * 正是某些 App 内置浏览器会弹「Download：(null)」的那条路径。内容先从同源 API 拿到，
 * 再从 Blob 落盘，浏览器不必再为这份内容发一次跨域请求。
 */
