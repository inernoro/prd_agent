...
describe('阶段列表', () => {
  ...
  it('用时时钟与剩余时间估算', () => {
    ...
    expect(remainingEstimateText('open-design', 5 * 60)).toBe('按经验值估算（耗时数据还在积累），预计还需 4–7 分钟');
    expect(remainingEstimateText('open-design', 11 * 60)).toBe('按经验值估算（耗时数据还在积累），预计还需不到 1 分钟');
