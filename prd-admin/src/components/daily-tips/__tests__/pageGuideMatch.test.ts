...
describe('matchPageGuide —— 仅本页未走完的 *-page-guide 才算「有教程」', () => {
  ...
  it('actionUrl 带 hash 时仍按页面匹配,用于同路由内重置页面状态', () => {
    const docGuide = tip({
      id: 'document-store-page-guide',
      sourceId: 'document-store-page-guide',
      actionUrl: '/document-store#guide-list',
      autoAction: { steps: [{ selector: '#a', title: 's' }] },
    });
    expect(filterPageTips([docGuide], NONE, '/document-store', '').map((t) => t.id)).toEqual([
      'document-store-page-guide',
    ]);
    expect(matchPageGuide([docGuide], NONE, '/document-store', '')?.id).toBe('document-store-page-guide');
  });
});
