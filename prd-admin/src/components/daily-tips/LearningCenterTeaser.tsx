...
/**
 * 账号菜单「教程中心」行右侧的等级小字：`Lv.4 · 14/15`。
 * 2026-09-24 从「30px 进度环 + 加粗大标题 + 第二行小字」收成普通菜单行的右侧提示——
 * 原来那一行和别的行外形都不一样，在菜单里很突兀（用户原话）。等级进度由头像外圈承担。
 */
export function LearningCenterMenuHint() {
  const progress = useDailyTipsStore((s) => s.progress);
  const loadProgress = useDailyTipsStore((s) => s.loadProgress);
  ...
  const total = progress.total ?? 0;
  const learned = progress.learned ?? 0;
  return <>{total > 0 ? `Lv.${level} · ${learned}/${total}` : `Lv.${level}`}</>;
}
