...
import { getLlmLogDetail, restoreLlmLogText } from '@/services';
...
// COS 占位符标记：[TEXT_COS:sha256:charcount]，命中即提供「还原完整内容」入口。
const TEXT_COS_MARKER = '[TEXT_COS:';
function hasCosPlaceholder(...texts: (string | null | undefined)[]): boolean {
  return texts.some((t) => typeof t === 'string' && t.includes(TEXT_COS_MARKER));
}
...
function Collapsible({ title, body, canRestore, restoring, restored, onRestore }: {
  title: string;
  body?: string | null;
  /** body 含 COS 占位符且尚未还原 → 显示「还原完整内容」按钮 */
  canRestore?: boolean;
  restoring?: boolean;
  restored?: boolean;
  onRestore?: () => void;
    ...
    <div className="rounded-[10px]" style={{ border: '1px solid var(--border-subtle)' }}>
      ...
      >
        <span className="flex items-center gap-1.5 min-w-0">
          ...
          {restored ? (
            <span className="inline-flex items-center rounded-full px-1.5 h-[16px] text-[9px] font-semibold shrink-0" style={{ color: '#34d399', background: 'rgba(52,211,153,0.15)' }}>已还原</span>
          ) : null}
        </span>
        <span className="flex items-center gap-2 shrink-0">
          {canRestore && !restored ? (
            <span
              ...
              className="inline-flex items-center gap-1 rounded-md px-1.5 h-[20px] text-[10px] font-medium cursor-pointer"
              style={{ color: '#38bdf8', background: 'rgba(56,189,248,0.12)', border: '1px solid rgba(56,189,248,0.28)' }}
              onClick={(e) => { e.stopPropagation(); if (!restoring) onRestore?.(); }}
              onKeyDown={(e) => { if ((e.key === 'Enter' || e.key === ' ') && !restoring) { e.preventDefault(); e.stopPropagation(); onRestore?.(); } }}
              title="正文含 COS 占位符，点击从对象存储还原完整原文"
            >
              {restoring ? <MapSpinner size={11} /> : null}
              {restoring ? '还原中…' : '还原完整内容'}
            </span>
          ) : null}
...
// 图片段：缩略图网格，点开走新标签大图（COS URL，禁 data:image 内联）。
function ImageSection({ title, images, note }: { title: string; images: { url: string; label?: string | null }[]; note?: string }) {
  if (!images.length) return null;
  return (
    <div>
      <div className="text-[12px] font-semibold mb-1.5 flex items-center gap-1.5" style={{ color: 'var(--text-primary)' }}>
        <span>{title}</span>
        <span className="text-[10px] font-normal" style={{ color: 'var(--text-muted)' }}>×{images.length}</span>
      </div>
      <div className="flex flex-wrap gap-2">
        {images.map((img, i) => (
          <a
            ...
            target="_blank"
            rel="noreferrer"
            className="block rounded-[8px] overflow-hidden shrink-0"
      ...
      {note ? <div className="text-[10px] mt-1" style={{ color: 'var(--text-muted)' }}>{note}</div> : null}
    </div>
  );
}
...
export function GenerationDetailsDrawer({ logId, onClose }: { logId: string; onClose: () => void }) {
  ...
  const [restored, setRestored] = useState<RestoredText | null>(null);
  ...
  const doRestore = async () => {
    ...
    setRestoring(true);
    setRestoreErr(null);
    const res = await restoreLlmLogText(logId);
    if (res.success && res.data) {
      setRestored({
        answerText: res.data.answerText,
        systemPromptText: res.data.systemPromptText,
        questionText: res.data.questionText,
        thinkingText: res.data.thinkingText,
      });
      if (res.data.restoreErrors?.length) setRestoreErr(res.data.restoreErrors.join('；'));
    } else {
      setRestoreErr(res.error?.message || '还原失败');
    }
    setRestoring(false);
  };

  // 还原优先：已还原字段取还原值，否则取原 detail 值。
  const questionText = restored?.questionText ?? detail?.questionText ?? null;
  const systemPromptText = restored?.systemPromptText ?? detail?.systemPromptText ?? null;
  const answerText = restored?.answerText ?? detail?.answerText ?? null;
  const thinkingText = restored?.thinkingText ?? detail?.thinkingText ?? null;

  const promptBody = useMemo(
    () => [questionText, systemPromptText].filter(Boolean).join('\n\n---\n\n') || null,
    [questionText, systemPromptText],
  );

  const hasRestored = restored != null;
  const promptCanRestore = !hasRestored && hasCosPlaceholder(detail?.questionText, detail?.systemPromptText);
  const completionCanRestore = !hasRestored && hasCosPlaceholder(detail?.answerText);
  const thinkingCanRestore = !hasRestored && hasCosPlaceholder(detail?.thinkingText);

  const inputImages = useMemo(() => {
    const refs = (detail?.imageReferences ?? []).filter((r) => r?.cosUrl).map((r) => ({ url: r.cosUrl as string, label: r.label }));
    const ins = (detail?.inputImages ?? []).filter((i) => i?.url).map((i) => ({ url: i.url, label: i.label }));
    return [...ins, ...refs];
  }, [detail?.imageReferences, detail?.inputImages]);

  const outputImages = useMemo(
    () => (detail?.outputImages ?? []).filter((i) => i?.url).map((i) => ({ url: i.url, label: i.label })),
    [detail?.outputImages],
  );
        ...
        <div className="flex-1 px-4 py-3 space-y-4" style={{ minHeight: 0, overflowY: 'auto', overscrollBehavior: 'contain' }}>
          {loading || !detail ? (
            ...
            <>
              ...
              {/* 图片（参考图 / 输入图 / 输出生成图）—— COS URL 缩略图，治「返回数据图片无法显示」 */}
              {(inputImages.length > 0 || outputImages.length > 0) ? (
                <div className="space-y-3">
                  <ImageSection title="输入 / 参考图片" images={inputImages} />
                  <ImageSection title="输出生成图片" images={outputImages} />
                </div>
              ) : null}
              ...
              <div className="space-y-2">
                ...
                <Collapsible
                  ...
                  body={promptBody}
                  canRestore={promptCanRestore}
                  restoring={restoring}
                  restored={hasRestored && hasCosPlaceholder(detail.questionText, detail.systemPromptText)}
                  onRestore={doRestore}
                />
                <Collapsible
                  title="Completion（模型输出）"
                  body={answerText}
                  canRestore={completionCanRestore}
                  restoring={restoring}
                  restored={hasRestored && hasCosPlaceholder(detail.answerText)}
                  onRestore={doRestore}
                />
                <Collapsible
                  title="Thinking（思考过程）"
                  body={thinkingText}
                  canRestore={thinkingCanRestore}
                  restoring={restoring}
                  restored={hasRestored && hasCosPlaceholder(detail.thinkingText)}
                  onRestore={doRestore}
                />
              </div>
            </>
