...
export interface BookEntry {
  ...
  takeaway: string;
  /**
   * 这本书对应我们自己的哪几条规则（`.claude/rules/` 里的文件名，不带 .md）。
   *
   * 精读稿的全部价值就在这里：书摘网上到处都是，「这本书讲的事我们在哪条规则上踩过」
   * 别处抄不来。没填的书照样能生成稿子，只是少了最后那一段。
   *
   * 填之前先读那条规则的导读两行，确认讲的真是同一件事 —— 硬凑一条不相干的，
   * 比留空更糟：读者会照着去翻，然后发现对不上。
   */
  relatedRules?: string[];
}
