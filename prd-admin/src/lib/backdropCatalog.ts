 ...
 * 指标用来发现问题，不用来代替看。
 *
 * 素材走本产品自己的生图链路（`visual-agent.image.text2img::generation`），
 * **按出图的原生 1536x1024 存 WebP，质量 0.88**。
 *
 * 这里也是一次实测纠错：上一版把 1536 的原图降到 1280 再存，白扔 20% 的像素。
 * 用户说「是不高清还是什么我琢磨不透」——算一下就清楚了：1520 CSS px 宽的视口、
 * DPR 2，横向需要 3040 个物理像素，而图只有 1280，等于**放大 2.375 倍**；
 * 再叠上 q0.72-0.80 在大面积渐变上的色带，就是那种说不上来的糊。
 * 改成原生 1536 + q0.88 之后放大倍数降到 1.98，且没有二次压缩。
 *
 * 还剩的软是模型天花板，不是参数能调的：image1 与 image2 对 2752x1536 都直接
 * IMAGE_GEN_REQUEST_REJECTED，最大只到 1536 宽。想再清晰只能换非生成的素材，
 * 或者引入放大模型——那是另一件事，不在这批里假装解决。
 *
