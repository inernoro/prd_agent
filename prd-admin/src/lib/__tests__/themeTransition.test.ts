...
describe('主题水波切换契约', () => {
  ...
  it('保留纸面身份页面对 data-theme 的所有权', () => {
    expect(isSelfManagedThemePath('/daily-post')).toBe(true);
    expect(isSelfManagedThemePath('/report-agent/detail/1')).toBe(true);
  ...
  it('桌面侧栏、皮肤设置与移动首页共用同一切换入口', () => {
    const consumers = [APP_SHELL_PATH, THEME_EDITOR_PATH, MOBILE_HOME_PATH]
