import {
  ...
  Video, PenTool, Terminal, Image, CloudUpload, Bot,
} from 'lucide-react';
...
export const CAPSULE_CATEGORIES: CapsuleCategoryMeta[] = [
  { key: 'trigger', label: '触发', description: '流水线的起点，负责产生触发信号', emoji: 'TR' },
  { key: 'processor', label: '处理', description: '数据采集、分析、转换', emoji: 'PR' },
  { key: 'control', label: '流程控制', description: '延时、条件分支等流程控制', emoji: 'CT' },
  { key: 'output', label: '输出', description: '结果输出、通知、导出', emoji: 'OUT' },
];
...
const EMOJI_MAP: Record<string, string> = {
  ...
  'data-aggregator': 'STAT',
  'delay': 'WAIT',
  'condition': 'IF',
  'report-generator': 'RPT',
  'webpage-generator': 'WEB',
  'file-exporter': 'EXP',
  'webhook-sender': 'SEND',
  'notification-sender': 'NOTICE',
  'event-trigger': 'EVT',
  'site-publisher': 'SITE',
  'email-sender': 'MAIL',
  'douyin-parser': 'VIDEO',
  'video-downloader': 'DL',
  'video-to-text': 'TEXT',
  'text-to-copywriting': 'COPY',
  'cli-agent-executor': 'CLI',
  'cds-agent': 'CDS',
};
...
export function getEmojiForCapsule(typeKey: string): string {
  return EMOJI_MAP[typeKey] || 'BOX';
}

/** 根据 category 获取 emoji */
export function getCategoryEmoji(category: string): string {
  return CATEGORY_EMOJI[category] || 'BOX';
}
