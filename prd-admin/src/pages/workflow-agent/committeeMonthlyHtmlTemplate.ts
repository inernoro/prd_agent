...
// Rate bar chart
H.push('(function(){var dom=document.getElementById("ch-ins-rate");if(!dom)return;var ch=echarts.init(dom);ch.setOption({tooltip:Object.assign({trigger:"axis",formatter:function(p){return p[0].name+"\\n及时率: "+p[0].value+"%";}},tt),grid:{top:20,bottom:30,left:10,right:30,containLabel:true},xAxis:{type:"category",data:'+safeJson(insNames)+',axisLabel:{fontSize:10,color:"#7D8590",rotate:15}},yAxis:{type:"value",max:100,axisLabel:{color:"#7D8590",formatter:"{value}%"},splitLine:{lineStyle:{color:"rgba(48,54,61,0.5)"}}},series:[{type:"bar",data:'+safeJson(insRates)+',itemStyle:{color:function(p){return p.value>=90?"#27C97F":p.value>=80?"#F5A623":"#E84040";},borderRadius:[4,4,0,0]},barMaxWidth:36,label:{show:true,position:"top",formatter:"{c}%",fontSize:10,color:"#7D8590"}}]});window.addEventListener("resize",function(){ch.resize()});})();');
