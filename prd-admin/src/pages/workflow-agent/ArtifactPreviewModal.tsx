...
export function ArtifactPreviewModal({ artifact, onClose }: ArtifactPreviewModalProps) {
  ...
  return (
    ...
    >
      <div
        className="surface-raised relative w-[90vw] max-w-5xl max-h-[85vh] rounded-2xl flex flex-col overflow-hidden"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="surface-reading-header flex items-center gap-3 px-5 py-4">
          <FileText className="text-token-accent w-4 h-4 flex-shrink-0" />
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              <h2 className="text-token-primary text-[14px] font-semibold truncate">
                {displayName}
              </h2>
              {isAutoGenerated && (
                <span className="surface-inset text-token-accent inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full flex-shrink-0">
                  <Bot className="w-3 h-3" />
                  自动透传
                </span>
              )}
            </div>
            <p className="text-token-muted text-[11px] flex items-center gap-1.5">
              <span className="bg-token-nested text-token-secondary px-1.5 py-0.5 rounded text-[10px] font-medium">
              ...
              {mimeOverridden && (
                <span className="text-token-warning text-[9px]">
              ...
              <div className="relative">
                ...
                {downloadMenuOpen && (
                  <>
                    ...
                    <div className="surface-popover absolute right-0 top-full mt-1 z-20 rounded-lg py-1 min-w-[140px]">
                      {[
        ...
        <div className="flex-1 overflow-auto p-5">
          {!content && !artifact.cosUrl && (
            <div className="text-token-muted text-center py-12 text-[13px]">
              产物内容为空
            </div>
          )}
          ...
          {!content && artifact.cosUrl && (
            <div className="text-center py-12">
              <a
                ...
                className="surface-action-primary inline-flex items-center gap-2 px-4 py-2 rounded-lg transition-colors"
              >
                <Download className="w-4 h-4" />
                下载文件
              </a>
          ...
          {content && viewMode === 'preview' && (
            <>
              ...
              {isMarkdown && (
                <div
                  className="text-token-secondary prose-sm text-[13px] leading-relaxed"
              ...
              {isHtml && !isFullHtmlPage && (
                <div
                  className="text-token-secondary text-[13px] leading-relaxed"
...
function CodeBlock({ content }: { content: string }) {
  return (
    <pre
      className="surface-code text-token-secondary text-[12px] font-mono leading-relaxed p-4 rounded-xl overflow-auto whitespace-pre-wrap"
    >
      {content}
    </pre>
  );
...
function DataTable({ columns, rows }: { columns: string[]; rows: string[][] }) {
  return (
    <div className="surface-inset overflow-auto rounded-xl">
      <table className="w-full text-[11px]" style={{ borderCollapse: 'collapse' }}>
        <thead>
          <tr className="bg-token-nested">
            <th
              className="text-token-muted bg-token-nested border-b border-token-subtle text-left px-3 py-2 font-semibold sticky top-0 min-w-8"
            >
              #
            </th>
            {columns.map((col) => (
              <th
                key={col}
                className="text-token-muted bg-token-nested border-b border-token-subtle text-left px-3 py-2 font-semibold sticky top-0 whitespace-nowrap"
              >
                {col}
              </th>
            ))}
        ...
        <tbody>
          {rows.map((row, rowIdx) => (
            <tr
              key={rowIdx}
              className="border-b border-token-subtle transition-colors hover:bg-token-nested"
            >
              <td className="text-token-muted px-3 py-1.5">{rowIdx + 1}</td>
              {row.map((cell, colIdx) => (
                <td
                  key={colIdx}
                  className="text-token-secondary px-3 py-1.5 max-w-[300px] truncate"
                  title={cell}
                >
                  {cell}
                </td>
