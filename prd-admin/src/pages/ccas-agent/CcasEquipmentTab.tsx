    ...
    <div className="flex-1 min-h-0 grid grid-cols-1 lg:grid-cols-[360px_1fr] gap-4 overflow-hidden">
      ...
      <div className="flex flex-col gap-3 min-h-0 overflow-y-auto pr-1" style={{ overscrollBehavior: 'contain' }}>
        <section className="rounded-lg border border-token-subtle bg-token-nested p-4">
          <h2 className="text-sm font-medium text-token-primary mb-3 flex items-center gap-1.5">
            <Plus className="w-4 h-4" /> 生成新素材
          </h2>

          <label className="block text-xs text-token-secondary mb-1">设备类型 <span className="text-red-400/80">*</span></label>
          <input
          ...
          <label className="block text-xs text-token-secondary mb-1">风格预设</label>
          ...
          <label className="block text-xs text-token-secondary mb-1">额外提示词（可选）</label>
          <textarea
            value={extraPrompt}
            onChange={(e) => setExtraPrompt(e.target.value)}
            rows={3}
            placeholder="如：白色机身、银色金属支架、配 4 个相机头"
            className="w-full mb-3 rounded-md bg-token-nested border border-token-subtle px-3 py-1.5 text-sm text-token-primary placeholder-token-muted focus:outline-none focus:border-amber-400/60"
          />
          ...
          <div className="mt-3 text-[11px] text-token-muted leading-relaxed">
        ...
        <section className="rounded-lg border border-token-subtle bg-token-nested p-4">
          ...
          <p className="text-[11px] text-token-muted mb-2 leading-relaxed">
          ...
          <label className="block text-xs text-token-secondary mb-1">备注（可选）</label>
          <textarea
            value={uploadNote}
      ...
      <div className="flex flex-col min-h-0">
        <div className="shrink-0 flex items-center justify-between gap-2 mb-2">
          <div className="flex items-center gap-1.5 text-xs text-token-secondary">
            <span>共 {items.length} 张</span>
            <select
              value={filterStyle}
              onChange={(e) => setFilterStyle(e.target.value)}
              className="rounded bg-token-nested border border-token-subtle px-1.5 py-0.5 text-[11px] text-token-primary"
            >
        ...
        >
          {loading && items.length === 0 ? (
            <div className="col-span-full text-center text-sm text-token-muted py-12">加载中…</div>
          ) : items.length === 0 ? (
            <div className="col-span-full text-center text-sm text-token-muted py-12">
              暂无素材，去左侧生成或上传第一张
            </div>
          ) : (
            items.map((it) => (
              <div
                key={it.id}
                className="rounded-lg border border-token-subtle bg-token-nested overflow-hidden flex flex-col"
              >
                <div className="aspect-square bg-token-nested relative">
                  ...
                  <div className="absolute top-1.5 right-1.5 flex gap-1">
                    <button
                      ...
                      className="p-1 rounded bg-token-nested hover-bg-soft"
                      title={it.isFavorite ? '取消收藏' : '收藏'}
                    >
                      <Star className={`w-3.5 h-3.5 ${it.isFavorite ? 'fill-amber-400 text-amber-400' : 'text-token-secondary'}`} />
                    </button>
                    <button
                      type="button"
                      onClick={() => onDelete(it.id)}
                      className="p-1 rounded bg-token-nested hover:bg-red-500/40"
                      title="删除"
                    >
                      <Trash2 className="w-3.5 h-3.5 text-token-secondary" />
                    </button>
                  </div>
                </div>
                <div className="px-2 py-1.5 text-[11px] text-token-secondary flex items-center justify-between">
                  <span className="truncate" title={it.equipmentType}>{it.equipmentType}</span>
                  <span className="text-token-muted ml-1 shrink-0">
