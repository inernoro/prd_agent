...
describe('接入台第一屏那句判断', () => {
  ...
  it('一台用过、一台从来没用过：新的那台照样拿到引导，不被老的那台盖掉', () => {
    // 判据是「有没有一台从来没用过」。按「有没有一台用过」写的话，老钥匙一存在，
    // 新发的那把就永远等不到那句「重启客户端」（review 抓出来的混合场景）。
    const mixed = buildHeadline({
      clients: [
        client({ keyId: 'old', lastUsedAt: '2026-09-05T08:00:00.000Z' }),
        client({ keyId: 'new', lastUsedAt: null }),
      ],
      today: today(), recentCalls: [], hasHistory: true,
    });
    expect(mixed.detail).toContain('重启客户端');
    expect(mixed.detail).toContain('1 台');
  });
