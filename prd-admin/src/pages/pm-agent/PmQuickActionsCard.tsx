  ...
  return (
    ...
    <div className="min-h-0 flex flex-col rounded-xl border border-token-subtle bg-token-nested p-4" style={{ flexGrow: 3, flexBasis: 0 }}>
      <div className="shrink-0 flex items-center gap-2 mb-3">
        ...
        <span className="text-sm font-semibold text-token-primary">便捷操作</span>
        <button
      ...
      {loading ? (
        ...
        <div className="flex-1 flex items-center justify-center text-[12px] text-token-muted text-center px-4">还没有便捷操作，点右上角「配置」添加。</div>
      ) : (
          ...
          {actions.map((a) => {
            ...
            return (
              <button
                key={a.id}
                onClick={() => a.run(ctx)}
                className="pa-row flex items-center gap-2 px-2.5 py-2 rounded-lg border border-token-subtle bg-token-nested hover:bg-blue-500/10 hover:border-blue-500/30 text-left"
              >
                <Icon size={14} className={a.group === 'create' ? 'text-blue-300 shrink-0' : 'text-token-secondary shrink-0'} />
    ...
    <div className="fixed inset-0 z-[10000] flex items-center justify-center bg-black/50" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between px-4 py-3 border-b border-token-subtle shrink-0">
          <div>
            <h2 className="text-sm font-semibold text-token-primary">配置便捷操作</h2>
            <p className="text-[11px] text-token-muted mt-0.5">勾选要展示在首页的操作并排序（个人配置，云端同步）</p>
          </div>
        ...
        <div className="flex-1 p-4 flex flex-col gap-4" style={{ minHeight: 0, overflowY: 'auto', overscrollBehavior: 'contain' }}>
          ...
          {groups.map((g) => {
            ...
            return (
              <div key={g}>
                <div className="text-xs font-medium text-token-secondary mb-2">{PM_QUICK_ACTION_GROUP_LABEL[g]}</div>
                  ...
                  {rest.map((a) => {
                    ...
                    return (
                      <button
                        ...
                        className="flex items-center gap-2 px-2.5 py-1.5 rounded-lg border border-token-subtle bg-token-nested hover:bg-blue-500/10 hover:border-blue-500/30 text-left"
                      >
