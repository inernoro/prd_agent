...
import { X, Stethoscope, Square, Save, Eye, Pencil } from 'lucide-react';
...
/**
 * AI 项目健康诊断 —— SSE 流式生成 Markdown 诊断（基于项目实时执行信号：进度/逾期/里程碑健康/
 * 风险分布/未决决策/预算/周报趋势）。区别于组织级 portfolio health（规则红黄绿）：这是单项目的 LLM 深度诊断。
 * 用户审核/编辑后可保存到项目知识库（DocumentEntry，作为某一时点的健康快照）。
 */
export function HealthDiagnosisPanel({ projectId, projectNo, onClose }: Props) {
  ...
  const start = async () => {
    ...
    try {
      await connectSse({
        url: api.pm.projects.healthDiagnosis(encodeURIComponent(projectId)),
        method: 'POST',
        ...
        onEvent: (evt) => {
          ...
          try {
            ...
            else if (evt.event === 'error') toast.error('诊断失败', (data as { message?: string }).message || '');
          } catch { /* ignore */ }
        },
      });
  ...
  const save = async () => {
    ...
    const er = await addDocumentEntry(sr.data.storeId, { title: `健康诊断 · ${projectNo} · ${stamp}`, sourceType: 'import', contentType: 'text/markdown', summary: content.slice(0, 200) });
    ...
    <div className="surface-backdrop fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={phase === 'streaming' ? undefined : onClose}>
      <div className="rounded-xl border flex flex-col w-full" style={{ maxWidth: 820, height: '88vh', maxHeight: '88vh', background: 'var(--bg-elevated)', borderColor: 'var(--border-subtle)' }} onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center gap-2 px-5 py-4 shrink-0 border-b" style={{ borderColor: 'var(--border-subtle)' }}>
          <Stethoscope size={17} style={{ color: ACCENT }} />
          <div className="text-[15px] font-semibold" style={{ color: 'var(--text-primary)' }}>AI 项目健康诊断</div>
          {phase === 'review' && (
        ...
        <div className="flex-1 px-5 py-4 flex flex-col gap-3" style={{ minHeight: 0, overflowY: 'auto', overscrollBehavior: 'contain' }}>
          {phase === 'idle' && (
            <div className="text-[12.5px]" style={{ color: 'var(--text-muted)' }}>
              将扫描本项目的实时信号——任务进度与逾期、里程碑健康、风险概率×影响分布、长期未决的待决策、预算使用率与周报同步情况，给出健康评级、关键风险信号与可立即执行的纠偏建议。生成后可编辑，再保存到「资料 → 知识库」作为健康快照。
            </div>
          )}

          {phase === 'streaming' && (
            <div className="flex flex-col gap-2">
              <div className="flex items-center gap-2 text-[12px]" style={{ color: ACCENT }}><MapSpinner size={14} /> {stageMsg}</div>
        ...
        <div className="flex justify-end gap-2 px-5 py-3.5 shrink-0 border-t" style={{ borderColor: 'var(--border-subtle)' }}>
          {phase === 'idle' && (<><Button variant="ghost" onClick={onClose}>取消</Button><Button variant="primary" onClick={start}><Stethoscope size={14} />开始诊断</Button></>)}
          {phase === 'streaming' && <Button variant="secondary" onClick={stop}><Square size={13} />停止</Button>}
          {phase === 'review' && (<><Button variant="ghost" onClick={start}>重新诊断</Button><Button variant="primary" onClick={save} disabled={saving || !content.trim()}>{saving ? <MapSpinner size={14} /> : <Save size={14} />}保存到知识库</Button></>)}
        </div>
      </div>
    </div>
  );
