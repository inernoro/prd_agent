  ...
  return (
    <div className="min-h-0 flex flex-col rounded-xl border border-token-subtle bg-token-nested p-4" style={{ flexGrow: 7, flexBasis: 0 }}>
      ...
      {loading ? (
        ...
        <div className="text-[12px] text-token-muted py-6 text-center">暂无待办，保持清爽。</div>
      ) : (
          ...
          {items.map((it) => {
            ...
            return (
              <button
                key={`${it.kind}-${it.id}`}
                onClick={() => open(it)}
                className="pa-row shrink-0 text-left flex items-center gap-2 px-2.5 py-2 rounded-lg border border-token-subtle"
              >
                {it.kind === 'evaluation' ? (
                  ...
                  <span className="text-[11px] text-token-muted shrink-0 truncate" style={{ maxWidth: 88 }} title={it.projectTitle}>{it.projectTitle}</span>
                )}
                <span className="text-sm text-token-primary truncate flex-1">{it.kind === 'evaluation' ? `「${it.projectTitle}」${it.title}` : it.title}</span>
                {it.overdue && (
                ...
                {statusMeta && (
                  <span className="text-[11px] px-1.5 py-0.5 rounded bg-token-nested text-token-secondary border border-token-subtle shrink-0">{statusMeta.label}</span>
                )}
              </button>
            );
          })}
