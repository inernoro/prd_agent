...
export function SyncManagerPanel() {
  ...
  const handleRun = async (link: DocumentSyncLink) => {
    ...
    if (res.success) {
      // 后端按结果返回 error/pending/synced，前端据此给对的提示，别一律报"同步完成"
      if (res.data.status === 'error')
        toast.error('同步未完成，部分条目失败', res.data.lastResult ?? undefined);
  ...
  return (
    <div className="flex flex-col">
      ...
      >
        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="shrink-0 mt-0.5" style={{ color: 'var(--accent-fg-warning)' }}>
