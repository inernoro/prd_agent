...
export function RecordAudioSheet({ onClose, onComplete, onPickFile }: RecordAudioSheetProps) {
  ...
  // 完成/取消的意图标记：onstop 回调按它决定产出 File 还是丢弃
  const finishModeRef = useRef<'complete' | 'discard'>('discard');
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (analyser && stateRef.current === 'recording') {
        ...
        levelsRef.current.push(Math.min(1, Math.sqrt(sum / data.length) * 3));
