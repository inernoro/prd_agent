    ...
    <MobileDrawer open={open} onOpenChange={(v) => { if (!v) onClose(); }} side="right" width={520}>
      <div className="h-full flex flex-col bg-[#15171b]">
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-token-subtle">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-[11px] text-token-muted mt-0.5">{appeals.length} 条记录</p>
            </div>
          </div>
          <button onClick={onClose} className="text-token-secondary hover-text-primary transition-colors">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-4" style={{ overscrollBehavior: 'contain' }}>
          {loading ? (
            <div className="text-center text-sm text-token-muted py-10">加载中...</div>
          ) : appeals.length === 0 ? (
            <div className="text-center text-sm text-token-muted py-10">暂无申诉记录</div>
          ) : (
        ...
        {/* 受理表单 */}
        {confirmAction && (
          <div className="border-t border-token-subtle p-4 bg-token-nested">
            <div className="text-sm text-token-primary mb-2">
              {confirmAction.action === 'approve' ? '通过申诉' : '驳回申诉'}
              <span className="text-xs text-token-muted ml-2">请填写受理意见（≥5 字）</span>
            </div>
            <textarea
              value={comment}
              onChange={e => setComment(e.target.value)}
              placeholder={confirmAction.action === 'approve' ? '说明通过的理由，便于提交人理解...' : '说明驳回的理由，帮助提交人改进...'}
              className="w-full min-h-[80px] p-2.5 rounded-lg border border-token-subtle bg-token-nested text-sm text-token-primary placeholder-token-muted focus:outline-none focus:border-indigo-500/50"
              disabled={resolving}
            />
            ...
            <div className="flex justify-end gap-2 mt-3">
              <button
                ...
                disabled={resolving}
                className="text-xs px-3 py-1.5 rounded-lg bg-token-nested border border-token-subtle text-token-secondary hover-bg-soft transition-colors"
              >取消</button>
              <button
...
function AppealCard({
  ...
  return (
    <div className="bg-token-nested border border-token-subtle rounded-lg p-3.5">
      <div className="flex items-center justify-between mb-2.5">
        <div className={`flex items-center gap-1 text-[11px] px-2 py-0.5 rounded-full border ${statusInfo.cls}`}>
          {statusInfo.icon}
          {statusInfo.label}
        </div>
        <div className="text-[11px] text-token-muted">{new Date(appeal.createdAt).toLocaleString('zh-CN')}</div>
      </div>

      <div className="text-[11px] text-token-muted mb-1">{appeal.submitterName} 的申诉理由：</div>
      <div
        className="appeal-content text-sm text-token-primary bg-token-nested border border-token-subtle rounded-md p-2.5 mb-2 leading-relaxed"
        dangerouslySetInnerHTML={{ __html: appeal.reasonHtml }}
      />

      {appeal.status !== 'Pending' && (
        <>
          <div className="text-[11px] text-token-muted mb-1 mt-2">{appeal.resolverName ?? '管理员'} 的受理意见：</div>
          <div className="text-sm text-token-primary bg-token-nested border border-token-subtle rounded-md p-2.5 whitespace-pre-wrap">
            {appeal.resolverComment ?? '—'}
          </div>
          {appeal.resolvedAt && (
            <div className="text-[11px] text-token-muted mt-1">{new Date(appeal.resolvedAt).toLocaleString('zh-CN')}</div>
          )}
        </>
      )}
