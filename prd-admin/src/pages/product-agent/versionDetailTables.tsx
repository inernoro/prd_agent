...
export function requirementDetailColumns(reqWorkflow?: WorkflowDefinition | null) {
  return [
    ...
    { header: '标题', width: '30%', render: (row: { id: string }) => <span className="text-token-primary truncate block" title={(row as Requirement).title}>{(row as Requirement).title}</span> },
...
export function featureDetailColumns() {
  return [
    ...
    { header: '标题', width: '32%', render: (row: { id: string }) => <span className="text-token-primary truncate block" title={(row as Feature).title}>{(row as Feature).title}</span> },
...
export function defectDetailColumns() {
  return [
    ...
    { header: '标题', width: '30%', render: (row: { id: string }) => <span className="text-token-primary truncate block" title={(row as TracedDefect).title ?? ''}>{(row as TracedDefect).title || '—'}</span> },
