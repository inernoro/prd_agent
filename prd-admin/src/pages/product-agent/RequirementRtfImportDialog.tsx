...
interface UnmatchedProductSummary {
  label: string;
  count: number;
  examples: string[];
}
...
function summarizeUnmatchedProducts(unmatched?: UnmatchedProductRow[]): UnmatchedProductSummary[] {
  const map = new Map<string, UnmatchedProductSummary>();
  for (const item of unmatched ?? []) {
    const label = item.label?.trim();
    if (!label) continue;
    const current = map.get(label) ?? { label, count: 0, examples: [] };
    current.count += 1;
    const example = (item.title ?? item.planName)?.trim();
    if (example && current.examples.length < 2) current.examples.push(example);
...
export function RequirementRtfImportDialog({
  ...
  const runImport = async () => {
    ...
    const unmatchedProducts = summarizeUnmatchedProducts(imported.data.unmatched);
    setResult({ created, updated, skipped, skippedImages: skippedImageCount, unmatchedProducts });
    setImageWarnings(warningMessages.slice(0, 8));
    setProgress(skippedImageCount > 0 ? `导入完成，${skippedImageCount} 张图片未上传（需求正文已去除对应占位）` : '');
    await onImported();
    // 有跳过/图片失败时保留弹窗，让用户看到明细反馈；全量成功才自动关闭。
    if (created + updated > 0 && skipped === 0 && skippedImageCount === 0) onClose();
  };
          ...
          {result && (
            result.created + result.updated === 0 ? (
              <div className="mb-3 rounded-lg border border-amber-500/30 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                ...
                {result.unmatchedProducts.length > 0 && (
                  <ul className="mt-2 space-y-1 text-amber-100/80">
                    {result.unmatchedProducts.slice(0, 8).map((item) => (
                      <li key={item.label}>
                        {item.label}：{item.count} 条
                        {item.examples[0] ? `，例如「${item.examples[0]}」` : ''}
                      </li>
                    ))}
                  </ul>
                )}
              ...
              >
                ...
                {result.unmatchedProducts.length > 0 && (
                  <div className="mt-2">
                    <div className="font-medium">未识别到系统产品的分类：</div>
