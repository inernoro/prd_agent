        ...
        <div className="ml-auto flex items-center gap-1.5 shrink-0">
          ...
          {editing ? (
            <>
              ...
              <button onClick={() => void handleSave()} disabled={saving} className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-lg bg-cyan-500/20 text-[color:var(--accent-fg-blue)] border border-cyan-500/40 text-sm hover:bg-cyan-500/30 disabled:opacity-50">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {CREATIVE_PUBLISH_OPTIONS.map(({ kind: optionKind, label, desc, Icon }) => {
                      ...
                      return (
                        ...
                        >
                          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-cyan-400/10 text-[color:var(--accent-fg-blue)]">
                ...
                {publishedCreative && (
                  <div className="mt-3 flex flex-col gap-2 rounded-lg border border-emerald-400/20 bg-emerald-400/8 px-3 py-2 text-xs text-[color:var(--accent-fg-success)]/85 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <a
                      ...
                      target="_blank"
                      rel="noreferrer"
                      className="inline-flex shrink-0 items-center gap-1.5 rounded-md border border-emerald-300/25 px-2.5 py-1 text-[color:var(--accent-fg-success)] hover:bg-emerald-300/10"
                    >
                      <Globe size={12} />
                      打开网页
                    </a>
...
function DocBody({ entry, content, fileUrl, html, markdown, codeMode, editable, onStartEdit, kindColor, KindIcon }: {
  ...
  if (fileUrl) {
    return (
      <div className="flex flex-col items-center gap-3 py-16 text-center">
        ...
        <a href={fileUrl} target="_blank" rel="noreferrer" className="px-3 py-1.5 rounded-lg bg-cyan-500/15 text-[color:var(--accent-fg-blue)] border border-cyan-500/30 hover:bg-cyan-500/25 text-sm">
          下载 / 新窗口打开
        </a>
      </div>
    );
  ...
  return (
    <div className="flex flex-col items-center gap-3 py-16 text-center">
      ...
      {editable && (
        <button onClick={onStartEdit} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-cyan-500/15 text-[color:var(--accent-fg-blue)] border border-cyan-500/30 hover:bg-cyan-500/25 text-sm">
          <Pencil size={13} /> 开始编写
        </button>
      )}
    </div>
