      ...
      <div className="flex items-center gap-2 flex-wrap">
        <div className={FOCUS_BOX}>
          ...
          <input
            ...
            className="no-focus-ring bg-transparent text-sm text-token-primary outline-none w-64"
          />
          {keyword && <button onClick={clearAll} className="text-token-muted hover-text-primary"><X size={13} /></button>}
        </div>
        <select
          value={productFilter}
        ...
        {hasFilter && <button onClick={clearAll} className="text-[11px] text-token-muted hover-text-primary underline underline-offset-2">清除筛选</button>}
        <span className="ml-auto text-[11px] text-token-muted">新建 / 上传 / 治理请进入具体产品的知识库</span>
      </div>
      ...
      <div className="flex flex-col gap-1.5">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center text-center py-16 gap-2">
            <BookOpen size={32} className="text-token-muted" />
            <div className="text-sm text-token-secondary">{hasFilter ? '没有匹配的知识' : '各产品还没有知识文档'}</div>
            <div className="text-xs text-token-muted">{hasFilter ? '换个关键词或产品试试' : '进入任意产品的「知识库」tab 上传或新建'}</div>
          </div>
        ) : (
          rows.map((r) => {
            ...
            return (
              <SelectableRow
                ...
                trailing={
                  <>
                    <span className="shrink-0 text-[11px] px-2 py-0.5 rounded-full bg-token-nested border border-token-subtle text-token-secondary">
                      {r.productName ?? '未知产品'}
                    </span>
                    <button
                      onClick={(ev) => { ev.stopPropagation(); if (r.productId) navigate(`/product-agent/p/${r.productId}?tab=knowledge`); }}
                      className="shrink-0 flex items-center gap-1 text-[11px] text-token-muted hover:text-cyan-300 opacity-0 group-hover:opacity-100 transition-opacity"
                      title="进入产品知识库"
                    >
