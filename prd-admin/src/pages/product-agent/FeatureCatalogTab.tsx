...
import { listFeatures } from '@/services/real/productAgent';
...
export function FeatureCatalogTab({ productId }: { productId: string }) {
    ...
    <div className="flex h-full min-h-0 flex-col">
      <div className="flex shrink-0 flex-wrap items-center gap-2 border-b border-white/10 px-4 py-3">
        ...
        <button
          ...
          className="inline-flex items-center gap-1.5 rounded-lg bg-cyan-500/15 px-3 py-1.5 text-xs text-cyan-200 border border-cyan-500/30 hover:bg-cyan-500/25"
        >
          <Plus size={14} /> 新建功能
        </button>
        <button
          ...
          className="ml-auto rounded-lg border border-white/10 px-3 py-1.5 text-xs text-white/45 hover:bg-white/5"
        >
          申领正式版本号
        </button>
      </div>
      ...
      <div className="flex min-h-0 flex-1">
        ...
        <div className="flex min-h-0 min-w-0 flex-1 flex-col">
          <div className="shrink-0 flex flex-wrap items-center gap-3 border-b border-white/10 px-4 py-3">
                ...
                <tbody>
                  {tableRows.map((f) => (
                    ...
                    >
                      ...
                      <td className="px-3 py-2.5 text-white/55">{ITEM_GRADE_LABEL[f.grade]}</td>
