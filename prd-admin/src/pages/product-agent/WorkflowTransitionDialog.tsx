...
export function WorkflowTransitionDialog({
  ...
  return createPortal(
    <div className="fixed inset-0 z-[120] flex items-center justify-center bg-black/65 p-4">
      <div
        className="w-full max-w-lg rounded-xl border border-token-subtle bg-[#111319] shadow-2xl flex flex-col"
      ...
      >
        <div className="shrink-0 flex items-center justify-between gap-3 px-5 py-4 border-b border-token-subtle">
          <div>
            <div className="text-base font-semibold text-token-primary">确认流转</div>
            <div className="text-xs text-token-muted mt-1">动作：{label}</div>
          </div>
          <button type="button" onClick={onClose} disabled={busy} className="p-1.5 rounded-lg text-token-muted hover-text-primary hover-bg-soft disabled:opacity-40" title="关闭">
            <X size={17} />
          </button>
        </div>
        ...
        >
          {fields.includes('title') && (
            <label className="flex flex-col gap-1.5">
              <span className="text-xs text-token-secondary">{WORKFLOW_TRANSITION_FIELD_LABELS.title} *</span>
              <input
                value={title}
          ...
          {fields.includes('grade') && (
            <label className="flex flex-col gap-1.5">
              <span className="text-xs text-token-secondary">{WORKFLOW_TRANSITION_FIELD_LABELS.grade} *</span>
              <select
                value={grade}
          ...
          {fields.includes('assigneeId') && (
            <label className="flex flex-col gap-1.5">
              <span className="text-xs text-token-secondary">{WORKFLOW_TRANSITION_FIELD_LABELS.assigneeId} *</span>
          ...
          {(fields.includes('versionIds') || gateFields.includes('versionIds')) && (
            <div className="flex flex-col gap-1.5">
              <span className="text-xs text-token-secondary">{REQUIREMENT_GATE_FIELD_LABELS.versionIds} *</span>
              {versions.length === 0 ? (
                <div className="text-xs text-amber-200/80">暂无产品版本，请先在「版本」tab 创建。</div>
              ) : (
                <div className="flex flex-col gap-1.5 max-h-36 overflow-y-auto rounded-lg border border-token-subtle p-2" style={{ minHeight: 0 }}>
                  {versions.map((v) => (
                    <label key={v.id} className="flex items-center gap-2 text-sm text-token-secondary">
          ...
          {gateFields.includes('initiationId') && (
            <label className="flex flex-col gap-1.5">
              <span className="text-xs text-token-secondary">{REQUIREMENT_GATE_FIELD_LABELS.initiationId} *</span>
              <select
                value={initiationId}
          ...
          {gateFields.includes('releaseId') && (
            <label className="flex flex-col gap-1.5">
              <span className="text-xs text-token-secondary">{REQUIREMENT_GATE_FIELD_LABELS.releaseId} *</span>
              <select
                value={releaseId}
          ...
          {fields.includes('comment') && (
            <label className="flex flex-col gap-1.5">
              <span className="text-xs text-token-secondary">{WORKFLOW_TRANSITION_FIELD_LABELS.comment} *</span>
              <textarea
                value={comment}
        ...
        <div className="shrink-0 px-5 py-4 border-t border-token-subtle flex items-center justify-end gap-2">
          <button type="button" onClick={onClose} disabled={busy} className="px-3.5 py-2 rounded-lg border border-token-subtle text-sm text-token-secondary hover-text-primary hover-bg-soft disabled:opacity-40">
            取消
          </button>
          <button
            type="button"
