  ...
  return (
    <div className="flex flex-col gap-3">
      {!creating ? (
        ...
        <div className="rounded-xl border border-token-subtle bg-token-nested p-3 flex flex-col gap-3">
          <input
            autoFocus
            value={title}
          ...
          <div className="flex justify-end gap-2">
            <button onClick={() => setCreating(false)} className="px-3 py-1.5 rounded-lg text-sm text-token-secondary hover-bg-soft">
              取消
            </button>
            <button
              onClick={create}
      ...
      {items.length === 0 ? (
        <div className="text-center text-token-muted text-xs py-10 px-6">
          还没有升级申请。大版本升级走可配置的申请表单，关联本次要交付的需求与功能，提交后走审批流转。
        </div>
      ) : (
        <div className="flex flex-col gap-2">
          {items.map((u) => (
            <div key={u.id} className="pa-row rounded-lg border border-token-subtle bg-token-nested px-3 py-2">
              <div className="flex items-center justify-between gap-2">
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    <span className="text-sm text-token-primary truncate">{u.title}</span>
                    <span
                      className="text-[10px] px-1.5 py-0.5 rounded shrink-0"
                      style={{ color: STATUS_COLOR[u.status] ?? '#fff', background: 'var(--nested-block-bg)' }}
                    >
                ...
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <button
                    ...
                    className="text-token-muted hover:text-red-300"
                  >
                    <Trash2 size={14} />
                  </button>
                </div>
              </div>
              {u.reason && <div className="text-[11px] text-token-secondary mt-1.5 line-clamp-2">{u.reason}</div>}
            </div>
          ))}
        </div>
      )}
...
function PickList({
  ...
  return (
    <div>
      <div className="text-xs font-medium text-token-secondary mb-1">{title}</div>
      {items.length === 0 ? (
        <div className="text-[11px] text-token-muted py-1">{empty}</div>
      ) : (
        <div className="flex flex-col gap-0.5 max-h-40 overflow-y-auto pr-1" style={{ overscrollBehavior: 'contain' }}>
          {items.map((it) => (
            <label key={it.id} className="flex items-center gap-2 px-2 py-1 rounded hover-bg-soft cursor-pointer">
              <input type="checkbox" checked={selected.has(it.id)} onChange={() => toggle(it.id)} className="accent-cyan-500" />
              <span className="text-sm text-token-primary truncate">{it.label}</span>
            </label>
          ))}
        </div>
      )}
