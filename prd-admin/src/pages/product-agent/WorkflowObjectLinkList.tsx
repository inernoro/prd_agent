...
export function RequirementLinkList({
  ...
  if (requirements.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-token-subtle px-4 py-8 text-center text-sm text-token-muted">
        {emptyText}
      </div>
    );
  }
  return (
    <div className="flex flex-col gap-2">
      {sectionTitle && <div className="text-xs text-token-muted">{sectionTitle}</div>}
      {requirements.map((r) => (
        <button
          key={r.id}
          type="button"
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-sm text-token-primary">{r.title}</span>
            {r.grade && (
              <span className="mt-0.5 inline-block text-[10px] text-token-muted">{GRADE_LABEL[r.grade] ?? r.grade}</span>
            )}
          </span>
        </button>
      ))}
    ...
    <div className="flex flex-col gap-2">
      ...
      {features.map((f) => (
        ...
        >
          ...
          <span className="min-w-0 flex-1">
            <span className="block text-sm text-token-primary">{f.title}</span>
            {f.requirementIds.length > 0 && (
              <span className="mt-0.5 inline-block text-[10px] text-token-muted">关联需求 {f.requirementIds.length} 条</span>
            )}
          </span>
        </button>
      ))}
