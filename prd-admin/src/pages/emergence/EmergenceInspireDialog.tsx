  ...
  const modal = (
    ...
    >
      <div
        ...
        className="surface-popover flex max-h-[80vh] w-[min(520px,calc(100vw-32px))] flex-col rounded-[16px]"
        style={{
          backdropFilter: 'blur(40px) saturate(180%)',
        }}
      >
        {/* 头部 */}
        <div className="surface-panel-header flex shrink-0 items-center gap-3 px-5 pb-3 pt-4">
          <div className="surface-state-warning flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-[10px]">
            <Lightbulb size={15} />
          </div>
          <div className="flex-1 min-w-0">
            <h3 className="text-[14px] font-semibold text-token-primary">增加灵感</h3>
            <p className="mt-0.5 text-[11px] text-token-muted">
              {parentTitle
                ? <>给 <span className="text-token-secondary">{parentTitle}</span> 指定一个探索方向</>
        ...
        >
          <textarea
            ...
            rows={4}
            className="prd-field w-full resize-none rounded-[10px] p-3 text-[12px] leading-[1.6] outline-none"
          />

          <div>
            <p className="mb-2 text-[11px] text-token-muted">快速灵感(点一下自动填入):</p>
            <div className="flex gap-1.5 flex-wrap">
              {PRESETS.map(preset => (
                <button
                  key={preset}
                  onClick={() => setPrompt(prev => prev ? `${prev}\n${preset}` : preset)}
                  className="surface-state-warning cursor-pointer rounded-[8px] px-2.5 py-1 text-[11px] transition-colors duration-150 hover:brightness-125"
                >
                  {preset}
                </button>
              ))}
        ...
        {/* 底部操作 */}
        <div className="surface-panel-footer flex shrink-0 items-center justify-between gap-2 px-5 py-3">
