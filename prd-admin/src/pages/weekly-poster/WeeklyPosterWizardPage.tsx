...
function saveCanvasOrientation(id: string, orientation: CanvasOrientation) {
  try { localStorage.setItem(canvasOrientationStorageKey(id), orientation); } catch { /* ignore */ }
}
import {
  SlidersHorizontal,
  Play,
...
export default function WeeklyPosterWizardPage() {
  ...
  const isNewCanvasRoute = location.pathname.endsWith('/new');
  const initialPrefs = useMemo(() => loadPrefs(), []);
  const initialTemplateKey = useMemo(() => {
    const value = searchParams.get('template');
    return POSTER_TEMPLATES_SEED.some((item) => item.key === value)
      ? value as WeeklyPosterTemplateKey
      : initialPrefs.templateKey ?? 'release';
  }, [initialPrefs.templateKey, searchParams]);
  const initialSourceType = useMemo(() => {
    ...
    return SOURCE_TYPES.some((item) => item.key === value)
      ...
      : initialPrefs.sourceType ?? 'changelog-current-week';
  }, [initialPrefs.sourceType, searchParams]);
  const [templates, setTemplates] = useState<WeeklyPosterTemplateMeta[]>(POSTER_TEMPLATES_SEED);
  const [templateKey, setTemplateKey] = useState<WeeklyPosterTemplateKey>(initialTemplateKey);
  const [sourceType, setSourceType] = useState<WeeklyPosterSourceType>(initialSourceType);
  ...
  const [canvasMode, setCanvasMode] = useState<CanvasMode>(searchParams.get('mode') === 'import' ? 'import' : 'blank');
  const [canvasOrientation, setCanvasOrientation] = useState<CanvasOrientation>(searchParams.get('orientation') === 'portrait' ? 'portrait' : 'landscape');
  const [pageCount, setPageCount] = useState(clampPageCount(Number(searchParams.get('pageCount')) || 5));
  ...
  const openWorkbench = useCallback((id: string) => {
    navigate(`/weekly-poster?id=${encodeURIComponent(id)}`);
  }, [navigate]);
  ...
  // 刷新恢复:URL ?id 优先;旧 /wizard 兼容最近草稿,/new 始终保持新建心智。
  useEffect(() => {
    const urlId = searchParams.get('id');
    const draftId = urlId || (isNewCanvasRoute ? null : loadDraftId());
    if (!draftId) return;
  ...
  const sse = useSseStream<unknown>({
    ...
    onDone: (raw) => {
      ...
      saveCanvasOrientation(data.poster.id, canvasOrientation);
  ...
  const handleCreateCanvas = useCallback(async () => {
    ...
    if (canvasMode === 'blank') {
      ...
      saveCanvasOrientation(res.data.id, canvasOrientation);
      const pg: Record<number, PageProgress> = {};
      res.data.pages.forEach((p) => { pg[p.order] = 'pending'; });
      setPageProgress(pg);
      toast.success('空白画布已创建,可以进入微调编辑');
      openWorkbench(res.data.id);
      return;
    }
          ...
          <div className="mt-6">
            <div className="flex items-center justify-between mb-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <Link
                  to={poster ? `/weekly-poster?id=${encodeURIComponent(poster.id)}` : '/weekly-poster'}
