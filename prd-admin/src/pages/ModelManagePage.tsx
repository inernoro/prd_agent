...
import { Check, Link2, Pencil, Plus, Star, Trash2 } from 'lucide-react';
...
export default function ModelManagePage() {
  ...
  const filteredModels = useMemo(() => {
    const list = selectedPlatformId === '__all__' ? models : models.filter((m) => m.platformId === selectedPlatformId);
  ...
  const selectedPlatform = useMemo(() => {
    if (selectedPlatformId === '__all__') return null;
  ...
  const onDeletePlatform = async (p: Platform) => {
    ...
    if (!res.success) return;
    if (selectedPlatformId === p.id) setSelectedPlatformId('__all__');
    await load();
  };
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-end justify-between gap-4 flex-wrap">
        <div>
          ...
          <div className="mt-1 text-sm" style={{ color: 'var(--text-muted)' }}>
            平台 {platforms.length} 个 / 模型 {models.length} 个（mock）
          </div>
        </div>

        <div className="flex items-center gap-2">
          <Button variant="secondary" size="sm" onClick={openCreatePlatform}>
            <Plus size={16} />
            添加平台
          </Button>
          <Button variant="primary" size="sm" onClick={openCreateModel}>
            <Plus size={16} />
            添加模型
          </Button>
        </div>
      ...
      <div className="grid gap-4 lg:grid-cols-[320px_1fr]" style={{ minHeight: 560 }}>
        <Card>
          <div className="flex items-center justify-between">
            <div className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>平台</div>
            <Badge variant="subtle">{platforms.filter((p) => p.enabled).length} 启用</Badge>
          </div>
          ...
          <div className="mt-4 grid gap-2">
            <button
              ...
              className="rounded-[14px] px-4 py-3 text-left transition-colors hover:bg-white/2"
              style={{
            ...
            >
              <div className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>全部模型</div>
              <div className="text-xs" style={{ color: 'var(--text-muted)' }}>共 {models.length} 个</div>
            </button>
            ...
            {filteredPlatforms.map((p) => {
              ...
              return (
                <div
                  key={p.id}
                  className="rounded-[14px] px-4 py-3 transition-colors hover:bg-white/2"
                  style={{
                    background: isSelected ? 'rgba(255,255,255,0.04)' : 'transparent',
                    border: isSelected ? '1px solid var(--border-default)' : '1px solid transparent',
                  }}
                >
                  <button type="button" className="w-full text-left" onClick={() => setSelectedPlatformId(p.id)}>
                    <div className="flex items-center justify-between gap-3">
                      <div className="min-w-0">
                        <div className="text-sm font-semibold truncate" style={{ color: 'var(--text-primary)' }}>{p.name}</div>
                        <div className="text-xs" style={{ color: 'var(--text-muted)' }}>{count} 个模型</div>
                      </div>
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <label className="flex items-center gap-2 text-xs" style={{ color: 'var(--text-muted)' }}>
                      <input
                        ...
                        checked={p.enabled}
                        onChange={async (e) => {
                          await updatePlatform(p.id, { enabled: e.target.checked });
                          await load();
                        }}
                      />
                      启用
                    ...
                    <div className="flex items-center gap-2">
                      <Button variant="ghost" size="sm" onClick={() => openEditPlatform(p)}>
                        <Pencil size={16} />
                      </Button>
                      <Button variant="danger" size="sm" onClick={() => onDeletePlatform(p)}>
                        <Trash2 size={16} />
                      </Button>
                    </div>
                  </div>
        ...
        <Card variant={selectedPlatform ? 'gold' : 'default'}>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="text-sm font-semibold" style={{ color: 'var(--text-primary)' }}>
                {selectedPlatformId === '__all__' ? '全部模型' : (selectedPlatform?.name || '模型')}
              </div>
              {selectedPlatform && (
                <div className="mt-1 text-xs" style={{ color: 'var(--text-muted)' }}>
                  API: {selectedPlatform.apiUrl} / Key: {selectedPlatform.apiKeyMasked}
                </div>
              )}
            </div>
            <div className="flex items-center gap-3">
              <input
                ...
                className="h-10 rounded-[14px] px-4 text-sm outline-none"
                style={{ ...inputStyle, width: 260 }}
                placeholder="搜索模型..."
              />
              <Badge variant="subtle">{filteredModels.length} 条</Badge>
            </div>
          </div>
