...
export interface ReportTemplateSection {
  ...
  tableColumns?: string[];
  /** Table 专用: 列宽（px，与 tableColumns 按下标对齐，0 = 自动；拖拽调宽后随周报快照保存） */
...
export interface WeeklyReport {
  ...
  autoGeneratedModelId?: string;
  autoGeneratedPlatformId?: string;
  autoGeneratedBy?: string;
  /** 规则兜底原因代码；不包含上游错误原文 */
  autoGenerationFallbackReason?: string;
  /** v2.0 关联的采集工作流执行 ID */
...
/** Table 列宽下限 px（与后端 ReportInputType.MinTableColumnWidth 对齐；0 = 自动不受此限） */
export const MIN_TABLE_COLUMN_WIDTH = 60;

/** Table 列宽上限 px（与后端 ReportInputType.MaxTableColumnWidth 对齐） */
export const MAX_TABLE_COLUMN_WIDTH = 800;
...
export interface ReportAiPromptSettings {
...
export type GetMyAiReportPromptContract = () => Promise<ApiResponse<ReportAiPromptSettings>>;

export type UpdateMyAiReportPromptContract = (input: {
  prompt: string;
