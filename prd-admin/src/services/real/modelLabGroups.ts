...
export const listModelLabGroupsReal: ListModelLabGroupsContract = async (args) => {
  ...
  return await apiRequest<{ items: ModelLabGroup[] }>(`${api.lab.model.labGroups.list()}${qs ? `?${qs}` : ''}`);
};

export const upsertModelLabGroupReal: UpsertModelLabGroupContract = async (input) => {
  return await apiRequest<ModelLabGroup>(api.lab.model.labGroups.list(), {
    method: 'POST',
...
export const deleteModelLabGroupReal: DeleteModelLabGroupContract = async (id) => {
  return await apiRequest<true>(api.lab.model.labGroups.byId(encodeURIComponent(id)), { method: 'DELETE', emptyResponseData: true });
};
