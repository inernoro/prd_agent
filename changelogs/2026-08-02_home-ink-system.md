...
| fix | prd-admin | 首页点击补记打开次数（此前只有命令面板记账，「你常用的」对只用首页的人永远不出现） |
| fix | prd-admin | 主按钮对比度：暗色改「亮一档陶土底 + 深墨字」5.5:1（原白字 3.74:1 不达标），浅色 hover 由压暗改提亮 5.98:1（原 4.18:1）；themeSystem 契约扩到暗浅双向 + hover 态 |
| fix | prd-admin | inkPalette 守卫补 `hsl()/hsla()` 字面量扫描——注释声称覆盖但扫描器实际漏掉，改用 hsl 写紫色可绕过 |
