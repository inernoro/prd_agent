...
| fix | llmgw | 合并的幸存者映射表补上模型名两种写法：池成员的 ModelId 存的是调用方原样给的值（可以是名字），只记 _id 会让存名字的成员在去重后断在原地，且运行时匹配大小写敏感 |
| fix | llmgw | 会话日志端点补收 platformId：与请求页共用同一份筛选参数，只有一边收会让平台筛选亮着却列出所有平台的会话 |
| test | prd-api | 补两条守卫：合并要认三种模型别名；吃这份筛选的每个日志端点都要把 platformId 传进同一个 BuildFilter |
| fix | llmgw | 合并的模型名别名匹配收回到源平台范围内：上一版把 _id 与模型名合成一张表做全局匹配，会把无关平台里同名的池成员也改掉（只改 ModelId 不改 PlatformId），亲手造出解析不到的组合 |
...
| fix | llmgw | 合并去重按「最终指向哪个模型」算键：目标平台上 (幸存者_id) 与 (同一模型的名字) 字面不同却指向同一模型，留两条会让加权选择把该模型权重算两遍 |
...
| fix | llmgw | 合并去重改为「目标优先」而非数组先到先得：源成员排在前面时会用源的配置顶掉目标那条现成的 Priority/Protocol/定价/能力/健康历史，静默换掉整条成员比留两条重复更糟；去重计数同时不再漏算 |
...
| test | prd-api | 补合并去重的目标优先守卫 |
| fix | llmgw | 合并写模型池补上版本闸与递增：全仓其它八个池写点都带，只有合并没带——会盖掉并发改动，或让读到旧版本的操作在合并后把陈旧成员写回来，让已删引用复活 |
| fix | llmgw | 归一去重的别名表收全目标平台的 ModelName 与 Name 两种写法：只索引 ModelName 时，用 Name 别名建的目标成员算出的键仍与幸存者 _id 不同，重复成员照样并存 |
| test | prd-api | 补合并的版本闸守卫与别名表守卫 |
