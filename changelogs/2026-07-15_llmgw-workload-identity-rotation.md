...
| security | llmgw | 建立可连续执行的密钥轮换阶段与并发状态约束及拒绝请求归因，历史通配来源 key 可在轮换时一次性升级工作负载身份，连续轮换中止时恢复前驱原阶段，创建返回前验证新钥未被并发撤销并在冲突时回滚，失败日志按服务端 requestId 和生命周期标记避免重复且不保存密钥明文或哈希 |
