...
| fix | prd-admin | 分数榜模型列上限提到 560px（长模型名不再截断）、对战分条上限 460px（不再拉成 960px 长横线），富余用表格 maxWidth + 居中收口 |
| fix | prd-admin | 指标条加满宽轨道：右边缘齐平，长列表不再是一条锯齿线（条在轨道里填充，长短差异与误差须都保留） |
| fix | prd-admin | 撤掉表格居中：轨道已经保证右边缘齐平，富余全给对战分那列，表格重新靠边填满画布 |
