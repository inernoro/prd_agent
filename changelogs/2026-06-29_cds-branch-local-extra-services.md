...
| fix | cds | 远端 complete 后重置分支态（Bugbot Medium）：远端部署 complete + 按 executor 权威 svcMap 剪枝 entry.services 后，entry.status 仍停留在派发时钉的 building（空清单清空/全 running 的成功部署要等下次心跳才转态）；现 complete 时按 svcMap 同步存活服务 status + 重算分支态（无服务=idle、有 running=running、否则 error） |
