...
| fix | prd-api | 运行档、MD 转 PPT、密钥完整性告警里「模型设置 / 模型平台」的指路文案统一改为 LLM Gateway 控制台 |
| fix | prd-admin | 「清理已下线菜单」只在服务端全量目录加载成功后露出，目录缺失时不再用权限视图当判据 |
| fix | prd-api | 单用户导航重置的写入不跟随请求取消 |
