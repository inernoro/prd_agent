...
| security | prd-api | 体积上限提到站点形态分支之前，修复 PDF 包装站绕过下载闸门 |
| fix | prd-api | Stub 平台模型列表补登 stub-embedding，无 key 自测通道才真正可达 |
| test | prd-api | 新增 PDF 站体积上限守卫，断言超大 PDF 根本不发起下载 |
| fix | prd-api | 重传成视频等不支持形态时自动关闭提问，已发分享不再挂必失败的入口 |
| fix | prd-admin | 提问开关只挡「开启」不挡「关闭」，避免站点变形态后 owner 无法关掉 |
| security | prd-api | 提问失败不再把网关原始错误回给访客，改为稳定文案 + 错误码，详情只进日志 |
| fix | prd-admin | srcDoc 判据改为逐属性解析，识别不带引号的 type=module，修复此类页面预览白屏 |
| test | prd-admin | 新增属性等价形式守卫，覆盖无引号/单引号/大小写/属性顺序等 14 种写法 |
