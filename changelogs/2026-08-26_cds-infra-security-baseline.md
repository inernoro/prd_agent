...
| security | cds | redis 的「有没有认证」收敛成一份判据：此前三处各写一份、两个口径——一台「env 里有口令、启动却没开认证」的 redis，创建门禁拒绝创建、凭据派生不发凭据，而每日体检判它「已认证」并压掉裸奔告警，同一台库一边说裸奔一边说安全，对外发声的偏偏是判错的那一边。判据只认启动参数（安全结论上假阴性比假阳性贵），门禁与派生都改为委派 |
| test | cds | 补「三处判据对同一台库结论一致」守卫：创建门禁 / 运行态体检 / 凭据派生四种输入逐个过，任一处再分家就红。同批修正一条与门禁结论相反的旧断言（原来断言 env-only 判已认证），改的原因写在断言旁边 |
| fix | cds | 暴露面自检把容器的 Entrypoint 和 Cmd 一起采下来：memcached / nats 这类预设把 entrypoint 覆盖成 sh、真正的认证参数在 -c 那段里，只读 Cmd 等于只看到半条命令。判据收敛成只认启动参数之后，缺的这半条会直接变成天天响的假警报 |
| fix | cds | 口令只写在启动命令里的服务也派生得出凭据：`redis-server --requirepass <口令>`、`nats-server --pass <口令>` 这类形态门禁一直接受，而凭据派生只从 env 取值——门禁放行、服务真开着认证、消费方一个键都收不到。取值改走与认证判据同一套分词，env 优先、命令行兜底 |
| fix | cds | 命令行上取到的口令逐字保留大小写：认证判据为匹配开关名把 token 全转成了小写，取值若复用那份，派生出的口令大小写被抹平——它长得像凭据、也真会发出去，消费方认证必然失败而错误只说「认证失败」。CDS 自己生成的十六进制口令正好不会暴露这个 bug |
| feat | cds | 凭据表补上 NATS：两条真实形态都覆盖，直接建的把账号口令写在 --user/--pass 上取得到；预设那条路在容器内写配置文件、明文不进 argv，命令行上证不出认证开着就一个键都不发 |
| fix | cds | cds-compose 导入基础设施不再丢启动字段：`command` / `entrypoint` / `restartPolicy` 三个全丢，于是从 compose 导入的 redis 连 --requirepass 都没有，门禁判它没配认证、凭据派生也拿不到证明。解析器读得出、序列化器写得回，唯独建服务时丢掉——一条只断在中间的往返链 |
| test | cds | 上一条的守卫不写死字段名：真跑一次 parseCdsCompose，拿解析器的实际输出去比导入侧读了哪些 def.*，解析器哪天多读一个字段而导入侧没跟上就会红，不需要谁记得回来改测试 |
| fix | cds | 每日体检把展开后的值喂给认证判据：台账里存的是未展开的 `${...}`（线上四个项目就这么存着），拿生值去判，判据看到一串占位符当然「有值」，于是每一台模板式配置的库都被判成已配认证，豁免倒计时一次都不提示，到期那天直接起不来 |
| test | prd-api | 限流用例的「等到新窗口」挪到建库建索引之后、紧挨着发请求那一批之前：判据取的是等待那一刻的时钟，保护的却是几秒之后的请求，CI 上 Mongo 慢时刚买到的余量在发请求前就被花掉。顺带跳过用例时不再白等一次 |
| docs | cds | debt.cds.md 的 E69 / E75 / E78 / E79 / E80 / E81 / E82 七条结项，各记落地方式；E81 换来的新边界（读不到启动命令时从严多报一次）写进台账 |
