...
| fix | cds | Codex Review 第四轮：一次性钥匙判据补「来源」维度（凭据文件记 bootstrapSource=page-approval），`init --yes` 迁进来的静态/全权 Key 不再被当成一次性授权而在项目已建好后报死局；认证探测失败的原始报错收进折叠诊断区并截断，第一眼看到的是固定结论文案 |
