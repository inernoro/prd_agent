...
| feat | prd-admin | 团队周报列表新增轻量搜索入口：默认仅显示搜索图标，按当前团队/当前周展开关键词搜索并支持加载更多结果 |
| feat | prd-api | 团队周报列表视图接口新增 keyword/page/pageSize 搜索与分页返回能力，严格按当前可见权限过滤后再搜索 |
| fix | prd-admin | 统一弱化周报搜索框展开态描边，补齐“我的周报/团队周报”列表关键词高亮与命中摘要展示 |
| fix | prd-admin | 无团队管理权限用户不再显示“团队周报AI分析Prompt”入口，面板内部也不再回退展示全部团队 |
| fix | prd-api | 团队周报AI分析 Prompt 接口权限收紧为团队管理权限或负责人/副负责人，不再因仅有查看全部权限而可见/可改 |
