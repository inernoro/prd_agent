...
| fix | prd-api | 管理员替换头像前校验新对象存在，避免错误文件名删除仍可用的旧头像 |
| fix | prd-api | 头像生成幂等键改用稳定项目与分支作用域，滚动发布后重试继续复用原任务 |
| fix | llmgw | Offering 版本感知唯一索引复用任意等价旧索引，仅在缺少等价约束时执行并发幂等迁移 |
| fix | create-visual-test-to-kb | 验收报告渲染保留关联测试方法锚点，确保审核人员可从结论表点击跳转 |
...
| fix | prd-api | 损坏的 Open XML 文件在对象写入前返回可恢复提示，合法无文本附件仍可保存 |
| fix | prd-api | 文档空间解除附件引用后立即唤醒对象回收任务，避免受租约延迟影响留下测试文件 |
| fix | e2e | 稳定冒烟按当前录音阶段键和知识库深链验收，并自动关闭教程遮罩避免后续旅程被阻断 |
| fix | prd-api | 图片分层逻辑模型强制限定专用调用链，阻止普通文生图和多图请求误入分层协议 |
| test | e2e | 生图主备故障测试按真实上游过滤并临时切换确定性路由，结束后恢复全部网关配置 |
...
| fix | prd-api | 录音与字幕转写失败只返回结果和恢复动作，禁止向普通用户暴露模型、协议和上游诊断 |
| test | prd-api | 增加语音转写错误隐藏内部细节并保留恢复动作的永久回归测试 |
...
| test | prd-api | 修复网关字段转发守卫的注释解析误报，并补齐历史归档接管用例的时间授权边界 |
| test | prd-api | 源码守卫兼容普通仓库与 Git worktree 两种元数据形态，恢复本地完整回归能力 |
...
| fix | stable-smoke | 跨主应用与模型网关的视觉计划可在同一运行内安全复用，避免重新计时误判全部截图失效 |
| fix | stable-smoke | 验收归档显式传入标准日期，避免运行标识中的紧凑日期导致报告归档失败 |
| fix | prd-api | 视频创作错误统一脱敏为用户可读恢复动作，原始供应商额度、链接和协议诊断仅进入服务端日志 |
| fix | prd-admin | 视频页面兼容清理历史任务中的上游错误，移除供应商任务和环境变量等内部诊断展示 |
| test | stable-smoke/prd-api | 增加跨服务视觉计划复用、归档日期和视频错误泄漏永久回归覆盖 |
...
| fix | prd-api | 旧版视频任务下载改为全链路流式响应，并在直出视频授权查询时主动拒绝过期归属记录 |
| fix | prd-api | 合成文档在对象上传前登记持久化回滚意图，覆盖附件与条目写入失败产生的孤儿对象 |
| fix | cds | 预览组件默认定位到桌面右侧安全区并随窗口自适应，避免遮挡展开后的导航与账号菜单 |
| fix | prd-api | 已受理视频任务按持久化 Offering 完成生命周期查询，不受后续控制面禁用影响 |
| security | prd-api | 视频供应商结果地址与每次重定向均执行公网地址校验并禁用未校验跳转 |
| fix | prd-api | 分别持久化水印展示图与原图摘要，下载时严格返回用户当前看到的图片对象 |
| fix | prd-admin | 通用格式错误在通过安全约束后保留具体字段校验与修正提示，避免退化为无法操作的泛化文案 |
| fix | prd-api | 为录音转写任务补齐统一 BSON 主键映射，避免测试顺序和历史 ObjectId 数据触发不一致行为 |
| fix | prd-api | 由正式环境和 CDS main 按固定截止线接管升级前转写队列，避免滚动发布遗留任务永久挂起 |
| fix | cds | 视口缩放或旋转后保留浮动预览控件的自定义位置，并自动钳制到当前可见边界 |
| security | prd-api | 视频 Agent 任务查询、操作与下载票据统一绑定应用边界，阻止同用户跨应用访问视觉创作视频 |
| security | prd-api | 新增正式环境 RSA-PSS 签名式稳定冒烟破窗，仅允许一次性登录票据和定向通知并防重放 |
| test | scripts | 稳定冒烟运行器支持钥匙串私钥签名，避免复用或轮换正式环境全局超级密钥 |
| test | e2e | 故障切换验收只注入真实存在且未被隔离的双上游，避免污染失效路由配置 |
| security | prd-api | 破窗签名认证对分块请求体实施读取前硬上限，避免验签前缓存大请求 |
| fix | prd-api | 工作区删除覆盖成员任务并在数据库引用解除后回收原图与展示图对象 |
| fix | prd-admin | 删除多图参考后同步持久化同一画布快照，避免旧自动保存使图片刷新后复活 |
| security | prd-api | 合成登录必须由显式开关启用，已登记签名公钥不再绕过紧急熔断 |
| fix | llmgw | Offering 路由字段变更创建不可变新版本，已受理视频任务继续按原路由轮询与下载 |
| ops | deployment | 正式部署默认按固定截止线接管升级前旧 main 任务，CDS 功能分支仍禁止接管 |
| test | stable-smoke | 多图删除持久化验收与上游生图完成解耦，避免额度故障伪装为删除超时 |
| fix | prd-api | 幂等重试自动恢复头像临时任务，缺少源图时安全重建而不再永久排队 |
| security | prd-api | 稳定冒烟签名通知只能投递到配置的固定用户，拒绝直接用户标识和全局通知 |
| fix | prd-api | 跨版本头像 Worker 仅接管未开始任务，不再按运行时长猜测并重复执行付费生成 |
| fix | scripts | 视觉验收计划按状态绑定主应用或 LLM Gateway 的真实入口与页面路径，拒绝跨应用截图错占模型路由验收位 |
| security | prd-api | 视频生成错误在任务写库、事件推送和 API 返回三处统一净化，历史任务也不再泄漏上游诊断与链接 |
| test | stable-smoke | 移动端视觉验收位固化独立路径和入口结果阶段，归档报告自动生成主管可读的验收用例与证据链接 |
