...
| fix | prd-api | 图片逻辑模型按文生图、图生图和多图场景能力过滤，避免不兼容模型进入业务目录或被直接调用 |
| fix | prd-api | 生成图片清理仅探测图片扩展名，避免安全策略拒绝无关字体和文本对象探测 |
| test | e2e | 多图网关验收改为核对脱敏哈希、引用数量和顺序，不再要求日志保留原始图片数据 |
| ops | prd-api | 视频任务删除记录空项目清理保护条件，便于定位遗留项目原因 |
| fix | prd-api | 视频删除保留任务墓碑至项目清理完成，失败后后台任务可继续恢复 |
| fix | prd-api | 生成图片写入登记与引用清理共用内容哈希租约，避免并发清理产生悬空记录 |
| fix | llmgw | 平台、模型和 Exchange 密钥变更后自动恢复受影响 Offering 的健康探测状态 |
| ops | deployment | 正式环境首次升级默认接管旧版 main 后台任务，避免发布切换后任务滞留 |
| test | e2e | 生图 SSE 恢复强制验证心跳，多图结果按参考图未明示主色验证逐图影响 |
...
| test | stable-smoke | 录音恢复按真实列表入口关闭教程引导并点击进入知识库，避免深链状态替代用户路径 |
| fix | prd-api | 视频任务删除先持久化清理清单，并以跨实例租约串行化共享视频写入与删除 |
| fix | prd-admin | 头像描述超限和重复订阅分别提供可执行的用户恢复提示 |
| test | prd-admin | 补充头像描述超限与重复订阅错误分类回归测试 |
| fix | prd-api | 控制台票据与视频归属索引移回 DBA 清单，避免最小权限数据库阻断 API 启动 |
| fix | visual-acceptance | 末次截图后到达的阻断项同步把自动验收状态改为不通过 |
| fix | prd-api | 资源级视频 404 不再隔离健康模型供给，仅模型或端点缺失触发隔离 |
| fix | stable-smoke | 视觉证据绑定浏览器真实页面路径并与模块入口逐项核对 |
| fix | prd-api | 工作区删除先引用检查并清理生成产物与底层对象，再移除生图运行归属 |
| test | stable-smoke | 生图验收真实中断并续传 SSE、点击下载并校验文件、删除后复查产物清理 |
| fix | prd-api/prd-admin | 视频成片改由鉴权下载端点返回稳定 MP4 文件名并提供用户可读失败提示 |
| test | stable-smoke | 多图验收核对网关引用数量顺序与标识、运行中刷新恢复，视频验收点击真实下载并校验文件 |
| fix | prd-api | 生图模型目录仅展示当前租户与应用能够完整解析上游的逻辑模型，并严格允许回收无共享引用的内容寻址生成图片 |
| fix | prd-api | 逻辑模型选择目录与实际生成统一使用独立 LLM Gateway 权威配置，禁止回退进程内过期逻辑模型 |
| test | stable-smoke | 视频设置操作限定在创作工作区内，避免与全局设置入口重名造成验收误判 |
| fix | prd-admin/prd-api | 生成图片改走当前用户受权的同源下载链路，避免跨域链接无法落盘 |
| test | stable-smoke | 隔离视频项目测试数据并消除多图刷新完成竞态 |
| fix | prd-api/llmgw | 持久化视频提交实际命中的 Offering，并在轮询、下载及跨网关恢复时锁定同一路由 |
| fix | prd-api | 头像文件名切换在写库前校验本人对象真实存在，兼容端点不再误删原有效头像 |
| fix | prd-api | 生成图下载按本人产物哈希读取对象，避免 CDN 基础路径导致对象键推断错误 |
| test | stable-smoke | 生图下载显式核对同源响应，多用户共享视频清理按引用保留语义验收 |
| fix | prd-admin | 生成图贴近画布顶部时将快捷操作栏收回选区内，确保下载等操作始终可点击 |
| test | stable-smoke | 单图验收记录真实下载请求并消除快速完成时的轮询采样竞态 |
| fix | prd-api/prd-admin | 视频下载改用短时资源级凭据直连浏览器响应，避免大文件 Blob 下载被浏览器拦截 |
| test | prd-api | 视频归属索引回归断言对齐 DBA 索引清单，移除已下线直出验收脚本的陈旧源码断言 |
| fix | prd-admin | 原生视频下载表单延迟回收，避免浏览器在请求发出前取消提交 |
| fix | prd-api | 已完成旧格式视频从资产登记簿安全回填摘要，兼容共享数据库中的旧 Worker 产物 |
| test | stable-smoke | 多图删除验收等待后端与画布持久化，刷新并再次提交确认旧引用不回流 |
| fix | prd-api | 视频任务被某一 Offering 接单后，轮询与下载固定原路由且不再切换备用候选 |
| fix | prd-admin | 生成图下载复用统一鉴权下载链路，访问令牌过期时自动刷新后重试 |
| fix | prd-admin | 群组未绑定 PRD 时明确引导进入群组设置完成绑定，不再退化为通用输入提示 |
| fix | prd-admin | 报告评论引用失效图片时明确引导移除并重新上传 |
| fix | prd-api | 为头像生成预览增加应用后即时回收与终态任务限时清理，删除对象前校验跨任务引用 |
| fix | prd-api/prd-admin | 视频下载改为顺序流式传输，并补齐头像存储错误的用户恢复指引 |
| fix | prd-api | 头像终态任务每轮持续分批回收至无候选，避免高负载下过期产物形成积压 |
| fix | llmgw/prd-api | 迁移并补齐图片逻辑模型的场景能力，同时兼容尚未完成迁移的通用能力记录 |
| fix | prd-api | 将替换头像后的对象删除持久化为可退避重试任务，避免瞬时存储故障留下永久孤儿文件 |
| fix | prd-api | 头像切换与旧对象清理共用按用户跨实例租约，避免切回头像时被并发删除 |
...
| fix | prd-api | 回收旧版本遗留的头像生成任务，并在对象写入前持久化头像清理意图 |
| fix | prd-admin/prd-api | 头像上传统一支持登录刷新重试，并消除幂等唯一索引收紧时的无保护窗口 |
| fix | prd-api | 直出视频任务归属按 Offering 命名空间隔离，并回收兼容端点清空的版本化头像 |
| fix | prd-api | 补全图片与视频持久化引用保护，并支持下载本地相对路径文档附件 |
| fix | prd-admin | 头像状态流在登录过期时统一刷新令牌后重连，并保留上传失败的稳定错误码 |
| fix | prd-api | 直出视频下载改为响应头优先的顺序流式传输，避免完整成片进入托管内存 |
| fix | prd-api | 图片 Worker 兼容持久化与头像回收共用内容摘要租约，关闭保存到引用登记之间的并发删除窗口 |
| fix | prd-admin | 原生视频下载表单复用配置后的 API 基址，兼容独立域名与路径前缀部署 |
| fix | prd-api/prd-admin | 同步单图下载使用本人产物登记授权，并优先下载已登记的原图地址 |
| fix | prd-api | 带水印运行项按本人确定性请求标识关联原图摘要，恢复非工作区批量图片下载 |
| fix | prd-admin | 未登记业务错误码在文案满足安全与恢复动作契约时保留，并补齐既有模型配置、文件类型和生成失败分类 |
| fix | prd-api | 文档验收对象改为先登记持久化清理任务，待数据库引用解除后由后台安全回收 |
