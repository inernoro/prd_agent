...
| fix | prd-admin | 上传完成提示改说「提问默认已开着、想关去哪关」，原文案还在说默认关闭——默认全开之后它成了会让人白花钱的假话；视频站另给不支持提问的说明 |
| fix | prd-admin | 分享档搜索纳入行里显示的站点名，输入框承诺「搜索链接或站点」却搜不到站点名 |
| fix | prd-admin | 提问设置抽屉的说明文案也改说「默认开着、不想开在这里关」；askEnabled 类型改三态并注明判断要用 !== false |
| fix | scripts | 每日验收：三条路由补真锚点并改为等锚点出现（原判据靠外壳文案恒绿）；直连 iframe 不再免检，跨源真取正文、取不到则用真实像素证据 |
| fix | prd-admin | 提问配额窗口到点自动重拉快照（接住 Retry-After），不再需要折叠重开或刷新页面才恢复 |
...
| fix | prd-api | 开场问题生成加跨进程原子认领（带租约），防两个部署为同一版正文各调一次模型重复计费；盖戳补「这一版未盖过」条件防覆盖 |
| fix | prd-api | 失败冷却表读时清过期项，模型长时间不可用时不再随时间单调增长 |
...
| fix | prd-api | 释放开场问题生成的认领改为只清自己盖的那一笔——原先无条件清空会抹掉接手者的认领，放第三个进程再调一次模型 |
| fix | prd-api | 站点日配额 TTL 对齐 UTC 零点（键按日期轮转），Retry-After 不再比真实重置晚数小时 |
| fix | prd-api | 开场问题的正文版本判据补 $exists:false 分支——存量站点文档里没有 ContentVersion 字段，只认零值会让它们认领必然落空、自动与手动生成永不触发 |
| fix | prd-api | 撤销分享改为只记第一笔审计：重发 DELETE 不再改写撤销时刻、不再把已写下的撤销理由抹成空，仍保持幂等成功 |
| fix | prd-admin | 分享是否过期改按时钟判而不是读服务端那次快照，并把散在八处的「有效」判据收敛进 shareStatus |
| fix | prd-api | 提问退款守卫扩到罩住会话建立——数据库抖动时配额已扣、一个字没生成，原先退不回来 |
| perf | prd-api | 数据抽屉的日趋势与时段分布合成一次聚合分桶，去掉最多 365 次串行 CountDocuments |
| fix | prd-api | 时段分布改按全窗口聚合，不再拿封顶 5000 条的样本冒充「近 N 天」 |
