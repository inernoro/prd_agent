...
require_contains "$PROGRESS_OUTPUT" '| R0.6 CDS-managed runtime capacity reconciler | in_progress |' 'progress task board has R0.6 reconciler'
...
if [[ "$audit_rc" -eq 0 ]]; then
  fail "goal audit unexpectedly completed; R0.6 should remain blocked until runtime capacity exists"
fi

require_contains "$AUDIT_LOG" 'CDS-managed runtime capacity reconciler has not produced running official SDK runtime' 'audit blocks on reconciler'
require_contains "$AUDIT_LOG" 'Next cycle plan: r0-cds-managed-runtime-reconciler state=cds-managed-runtime-reconciler-missing items=D1,R0.3,R0.4,R0V,R0.5,R0.6' 'audit next cycle is R0.6 reconciler'
