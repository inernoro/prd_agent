...
#   4. 如果提供 CDS_SHARED_POOL_LONG_TOKEN，则直接调用 /api/projects/:id/instances
...
printf '\n>>> [3/4] 可选远程 shared-service pool 审计\n'
if [[ "$REMOTE" != "1" ]]; then
...
printf '\n>>> [4/4] 可选 long-token instance discovery 验证\n'
if [[ -z "${CDS_SHARED_POOL_LONG_TOKEN:-}" ]]; then
  ...
  instance_json=$(curl --max-time 20 --show-error --silent --fail-with-body \
    ...
    "$cds_base/api/projects/$SHARED_POOL_ID/instances")
