...
CASES = [
    ...
    ("R24-后代行内隐藏", "SVG 自己的行内 style=\"display:none\"", DAILY,
     lambda t: t.replace('<svg viewBox="0 0 120 120" fill="none" data-emblem="moon"',
                         '<svg style="display:none" viewBox="0 0 120 120" fill="none"'
                         ' data-emblem="moon"', 1)),
    ("R24-表现属性零透明", "SVG 表现属性 opacity=\"0\"（与 CSS 声明等价）", DAILY,
     lambda t: t.replace('<svg viewBox="0 0 120 120" fill="none" data-emblem="moon"',
    ...
    ("R24-变量透明-档案", "验收生成器把 --accent 定义成 transparent（走渲染产物才看得到）",
     ARCHIVE,
     lambda t: t.replace('"accent": "#0f766e"', '"accent": "transparent"', 1)),
    ("R23-后代隐藏", ".emblem svg{display:none} 把水印整个抹掉", DAILY,
     lambda t: t.replace("  .emblem svg { display: block;",
                         "  .emblem svg { display: none;", 1)),
    ("R23-后代零透明", ".emblem svg 这一层 opacity:0", DAILY,
     lambda t: t.replace("  .emblem svg { display: block;",
                         "  .emblem svg { opacity: 0; display: block;", 1)),
    ("R22-伪类主语", "基础规则改成 .emblem:focus，声明永不生效", DAILY,
     lambda t: t.replace("  .emblem { position: absolute;",
                         "  .emblem:focus { position: absolute;", 1)),
    ("R22-hover违规", ":hover 状态下推翻契约（有状态规则仍须合契约）", DAILY,
     lambda t: sub_after(t, "  .emblem svg { display: block; width: 100%; height: 100%; }",
                         "\n  .emblem:hover { position: static; }")),
