...
test('执行汇总缺少环境数组时从当前执行账本回填首屏且不生成矛盾文案', () => {
  const functional = `# 核心业务稳定验收主管报告

> 主管结论：不通过。共 3 项，1 项通过、1 项不通过、1 项未执行。

## 执行覆盖账本

| 环境 | 计划 | 已执行 | 通过 | 失败 | 未执行 | 阻塞类别 |
|---|---:|---:|---:|---:|---:|---|
| CDS | 2 | 2 | 1 | 1 | 0 | 无 |
| 正式环境 | 1 | 0 | 0 | 0 | 1 | 安全门槛 |`;
  const gate = `# 视觉门禁
...
|---|---|---|
| 能否发布 | 不可以 | 尚有异常 |
| 状态结果 | 通过 1，不通过 0，需补证 0，需干预 1 | 严格结论 |

## 模块覆盖

| 模块 | 视觉结论 | 真实面包屑 | 采集文件 | 可审核证据 | 状态结果 | 缺口 |
|---|---|---|---:|---:|---|---|
| 订单确认 | 需干预 | 首页 → 订单 → 确认页 | 2 | 2/2 | 通过 1，需干预 1 | 无 |`;
  const report = composeSupervisorReport(functional, gate, gate, '', '', {
    coverage: { total: 3, passed: 1, failed: 1, notRun: 1, verdict: 'fail' },
  });

  assert.match(report, /# 核心业务稳定验收报告/);
  assert.doesNotMatch(report, /验收验收报告/);
  assert.match(report, /CDS完成 2\/2，通过 1，失败 1，未执行 0/);
  assert.match(report, /正式环境完成 0\/1，通过 0，失败 0，未执行 1/);
  assert.doesNotMatch(report, /本轮未提供可计算的双环境执行汇总/);
  assert.match(report, /需干预；状态分布：通过 1，需干预 1/);
  assert.doesNotMatch(report, /需干预[：；]无/);
});
