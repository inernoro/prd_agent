...
# Prefer the maintained main script when available in-repo.
if [[ -f "${ROOT_DIR}/scripts/init-pr-prism-basis.sh" ]]; then
  exec bash "${ROOT_DIR}/scripts/init-pr-prism-basis.sh" "$@"
fi
...
usage() {
  cat <<'EOF'
Usage:
  scripts/bootstrap-pr-prism.sh [--repo owner/repo] [--architect architect] [--context engineering-governance]

Description:
  Standalone bootstrap package for new repositories.
  Initializes the minimum PR Review Prism top-design basis.

Notes:
  - Zero-argument mode is preferred.
  - Auto-detect order for repo:
      1) git remote origin
      2) gh repo view --json nameWithOwner
      3) git config github.repo
  - Auto-detect order for owner/architect:
