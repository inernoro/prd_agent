...
is_non_runtime_cycle_drift_path() {
  ...
  case "$path" in
    *.md|*.MD|*.txt|*.TXT) return 0 ;;
    ...
    scripts/smoke-*|scripts/audit-*|scripts/check-*|scripts/doctor-*|scripts/preflight-*|scripts/verify-*|scripts/index-*) return 0 ;;
...
check_docs_calibration() {
  ...
  require_doc_text "$migration_plan" '当前剩余主矛盾是有效 Claude/Anthropic-compatible runtime profile 与真实 provider 证据' \
    "migration plan names current provider closure blocker"
