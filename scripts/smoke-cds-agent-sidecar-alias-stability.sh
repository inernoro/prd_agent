...
#   SMOKE_CDS_AGENT_ALLOW_BRANCH_LOCAL_ALIAS_PROBE default: 0
...
if [[ "$SMOKE_CDS_AGENT_SIDECAR_ALIAS" =~ (^|[-_])claude-agent-sdk-runtime ]] \
  && [[ "$SMOKE_CDS_AGENT_ALLOW_BRANCH_LOCAL_ALIAS_PROBE" != "1" ]]; then
  smoke_fail "refusing to probe branch-local sidecar alias '$SMOKE_CDS_AGENT_SIDECAR_ALIAS'; set SMOKE_CDS_AGENT_ALLOW_BRANCH_LOCAL_ALIAS_PROBE=1 only for legacy contamination diagnosis"
fi
