...
for file in "$TOOL_FILE" "$REGISTRY_FILE" "$CONTROLLER_FILE" "$SESSION_SERVICE_FILE" "$ADMIN_PAGE_FILE" "$DOC_FILE"; do
  test -f "$file"
done
...
if awk '/policy is "readonly-auto" or "auto-allow-readonly"/,/return true/' "$SESSION_SERVICE_FILE" | grep -Eq 'kb_apply|kb_reject'; then
  echo "readonly-auto runtime must not expose kb_apply/kb_reject" >&2
  exit 1
fi

if ! awk '/policy is "readonly-auto" or "auto-allow-readonly"/,/return true/' "$SESSION_SERVICE_FILE" | grep -q 'kb_diff'; then
