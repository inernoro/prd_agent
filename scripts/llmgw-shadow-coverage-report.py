...
def _cell(base: str, key: str, app: str | None, kind: str | None, min_total: int, since_hours: float) -> dict:
    query: dict[str, str] = {}
    label = "global"
    if app:
...
def main() -> int:
    ...
    if not report["failures"]:
        report["cells"].append(_cell(base, key, None, None, args.min_per_cell, max(0, args.since_hours)))
        for kind in kinds:
            report["cells"].append(_cell(base, key, None, kind, args.min_per_cell, max(0, args.since_hours)))
        for app in app_callers:
            for kind in kinds:
                report["cells"].append(_cell(base, key, app, kind, args.min_per_cell, max(0, args.since_hours)))
        for item in report["cells"]:
