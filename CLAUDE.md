...
- 不适用：仅文档（`doc/`）/ 仅 `.claude/` 元数据 / 仅 `changelogs/` 的 push 可省略
...
| `autonomous-agent-charter.md` | 无人值守 / 定时 / 多轮自动执行 Agent（缺陷自动化、issues-autofix、CDS agent 等） | 自主智能体宪章（总纲）：无人值守 Agent 是「能担责的数字员工」不是「会办事的脚本」，第一步是定义自主边界（全权自主/请示后做/禁止自动做三档）。五层大脑自检：认知（指令≠目标）/规划（值不值）/执行（事实核查）/记忆（情景召回）/监督（合规红线+不确定性预警+紧急熔断三道事前护栏）。收编熔断/越界/验收闭环/自测优先等散落规则为统一框架 |
