...
- **规则按需加载**：`.claude/rules/` 的 frontmatter 用 `paths:`（不是 `globs:`——那个键不被识别，会退化成每 session 全量加载）。39 条按 `paths` 命中当前文件时才载入，19 条跨切面行为规则常驻。新增规则时 glob 必须命中真实文件，死 glob 会让规则永不加载。
