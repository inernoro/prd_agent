...
禁止只给路由、位置模糊、未注册百宝箱就声称完成。详见 `.claude/rules/navigation-registry.md`（有 CI 守卫 `navCoverage`）。
...
只要项目接了 CDS，任何代码改动 push 后或需人工验收时，最终回复必须先跑这条命令再列地址：
...
**必须给最终深链，不给根地址**：cdscli 输出是根域名 SSOT（域名部分不许自己造），功能页路由 + query/tab/锚点由你按本次改动的真实路由追加，让用户点一下就落到验收那一屏。多个功能页改动就给多条。

```
反例：【预览】https://xxx.miduo.org/
正例：【预览】https://xxx.miduo.org/open-platform?tab=open-api
```

不适用：仅 `doc/` / 仅 `.claude/` 元数据 / 仅 `changelogs/` 的 push。
...
- **架构规则** `.claude/rules/`：52 条。带 `paths` frontmatter 的在你读到匹配文件时自动加载；无 frontmatter 的（验收纪律、熔断、预期管理等跨切面行为规则）每次 session 常驻。每条开头两行导读（`**一句话**` / `**什么时候撞上**`）。守卫：`python3 scripts/tests/test_claude_memory_contract.py`。
- **技能** `.claude/skills/`：57 个，名称与描述由 harness 自动注入，此处不再重复维护清单。

生命周期主线（按顺序取用）：

```
需求 /validate → 方案 /plan-first → 风险 /risk → 链路 /trace
→ 实现 → 交叉验证 /verify → 边界 /scope-check
→ 部署 /cds-deploy → 冒烟 /smoke → 预览 /preview → 验收 /uat（复杂场景先 /验收场景 再 /验收）
→ 交接 /handoff → 周报 /weekly
```
