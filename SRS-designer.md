...
6. **合规与审计**：PRD内容不落盘；传输仅限HTTPS；日志需脱敏；如涉及跨境传输需满足数据出境合规，具备审计留痕能力
...
| 数据库可用 | 系统依赖PostgreSQL数据库存储用户、群组、日志等持久化数据 |
...
- 数据库：PostgreSQL 15+，存储用户、群组、消息历史、缺失记录、审计日志
...
- 合规约束：PRD内容不落盘（仅缓存），敏感信息通过环境变量注入，日志脱敏处理
...
| **数据库** | PostgreSQL | 15.0+ | 用户、群组、日志持久化存储 |
...
| `start_guide` | `session_id: string, role: string` | `void` | 启动引导讲解 |
| `control_guide` | `session_id: string, action: string, step?: int` | `GuideStatus` | 控制引导进度 |

**IPC事件清单**：

| 事件名 | 数据结构 | 说明 |
|--------|----------|------|
| `message-chunk` | `{ type: string, content?: string }` | 流式消息响应 |
| `guide-chunk` | `{ type: string, step?: int, content?: string }` | 引导讲解响应 |
...
- 审计与日志：记录关键操作（上传文档、切换角色、启动/停止引导），日志脱敏且不含用户原文
...
**功能详述**：
1. 用户选择角色后，可启动"引导讲解"模式
2. 系统按预设的讲解大纲，分步骤讲解PRD
3. 每个步骤讲解后，用户可提问或继续下一步
4. 支持中途退出引导模式
...
|------|------|
| 启动引导 | 根据角色启动引导讲解会话 |
| 获取步骤内容 | 流式返回当前步骤的讲解内容 |
| 导航控制 | 支持上一步、下一步、跳转、停止 |

**导航操作**：`Next`（下一步）、`Previous`（上一步）、`GoTo`（跳转）、`Stop`（停止）
...
| 需求 | 描述 |
|------|------|
| 数据不持久化 | PRD内容仅存于会话Cache，会话结束后清除 |
...
| 审计日志 | 关键操作需审计，日志脱敏，不含用户原文 |
| 数据保留 | 会话缓存30分钟过期；日志保留期可配置（默认30天），可按需清理 |
| 脱敏 | 日志与错误上报不得包含密钥、个人信息、PRD正文 |
...
| 字段 | 类型 | 必填 | 默认值 | 说明 | 约束 |
|------|------|------|--------|------|------|
| sessionId | string | 是 | UUID | 会话唯一标识 | UUID格式 |
| groupId | string | 是 | - | 关联的群组ID | 外键关联Group |
| currentRole | UserRole | 是 | PM | 当前角色视角 | PM/DEV/QA |
...
- 保留与清理：会话缓存默认30分钟未活跃即清理；不持久化PRD正文；日志需脱敏（不含用户原文）
...
| 配置路径 | 类型 | 默认值 | 说明 |
|----------|------|--------|------|
| `LLM:Provider` | string | `Claude` | LLM服务商（Claude/OpenAI） |
| `LLM:Claude:ApiKey` | string | - | Claude API密钥 |
| `LLM:Claude:Model` | string | `claude-3-5-sonnet-*` | Claude模型版本 |
| `LLM:Claude:MaxTokens` | int | `4096` | 最大输出Token数 |
| `LLM:OpenAI:ApiKey` | string | - | OpenAI API密钥 |
| `LLM:OpenAI:Model` | string | `gpt-4-turbo` | OpenAI模型版本 |
