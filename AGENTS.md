...
Review 的职责是验证当前目标，不是无限扩写需求。修复第一条 Review 评论前，必须在 PR 描述或任务记录中写清：

- 本 PR 的单一目标和必须保持的不变量
- 明确不在本 PR 处理的事项
- 首轮提交的文件数和 diff 行数，作为范围基线

每条 Review 评论必须先分类，再决定是否修改：

| 分类 | 判定 | 当前 PR 动作 |
|------|------|-------------|
| A 直接缺陷 | 可证明违反本 PR 目标、真实调用契约或既有测试 | 修复并补代表性测试 |
| B 有价值但扩范围 | 建议本身合理，但引入新的语义类别、兼容层或产品行为 | 记录后续 issue/debt，不在当前 PR 展开 |
| C 推测性变体 | 只增加新的措辞、同义词或假设输入，找不到真实生产路径 | 不实现，并在 Review 中说明证据不足 |

命中任一条件必须立即停止追加修复并执行 `/scope-check`：

- 连续 3 轮 Review 仍在产生新的语义类别
- Review 修复提交达到 8 个
- diff 行数超过首轮基线的 2 倍，或文件范围扩出首轮清单
- 同一个自由文本解析器第二次被要求增加同义词、语序或嵌套格式

熔断后必须回到原始目标，选择缩小 PR、改为结构化字段/有限枚举，或由用户明确批准扩范围。自动 Reviewer 的评论不能单独授权需求扩张；不得以“清空所有机器评论”为完成标准。PR 默认保持 Ready、关闭自动合并，未经用户明确指示不得合并。
