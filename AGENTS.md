...
**文档的主语是我们自己**：`design.` `rule.` `spec.` `guide.` `plan.` 里的判断、需求与取舍一律以我方口吻写，禁止把提需求的人写成「用户」再转述其言论（「用户当时的原话是……」「用户反馈说……」）。产品的终端使用者作为角色出现是合法的；判定口诀与改写范例见 `.claude/rules/doc-authorial-voice.md`。
