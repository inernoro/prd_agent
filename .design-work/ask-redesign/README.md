...
`prototype.html` —— 主线三态真能点：胶囊 → 长条（三条提示 + 自己输入）→ 右侧对话，
答案假流式吐字、引用点回正文高亮、历史面板、三档折叠、⌘K / Esc、额度真的在减。
图标用的是 MAP 品牌标（`prd-admin/src/components/ui/MapBrandMark.tsx` 那条带三节点的 M 路径）。

驱动测过的判据（真实指针序列，不是 `.click()`）：点开 132×40 → 660×54、提示 3+1 → 展开 5、
问完 400×757 且额度 18/4 → 17/3、引用点了正文真高亮、折竖条 44px、收回回到 132×40、
⌘K 唤起、自己打字提问命中对应答案，JS 零报错。
