...
- **解决什么问题**：业务流程散落在多个系统中，手动操作重复耗时——数据采集、LLM 分析、报告生成、通知推送每次都要人工串联
- **方案概述**：通用 DAG 工作流引擎，25+ 种舱类型（触发器/处理器/控制流/输出器），支持可视化编排、定时/事件触发、异步执行，是整个系统的**编排中枢**
- **业务价值**：任何你能画出流程图的业务都可以自动化；Agent 通过工作流获得外部能力（TAPD、Git、邮件等），工作流通过 Agent 获得 AI 能力，两者组合产生涌现
- **影响范围**：所有 Agent（Report/Defect/Visual/Literary/Video）、Channel Adapter、AutomationHub、Open Platform
- **当前状态**：已实现，含 DAG 执行、25+ 舱类型、定时调度、事件触发、SSE 推送、制品管道

## 二、用户场景与协同涌现

### 场景 1：周报数据自动采集（Report Agent 调用工作流）

> 管理者希望周报中自动包含 TAPD 任务完成情况和 Git 代码统计。

1. 管理员在工作流编辑器中拖入：TAPD 采集舱 → 数据聚合舱 → Git 数据提取舱
2. 将工作流绑定到 Report Agent 团队的 DataCollectionWorkflowId
3. 每周一 9:00 自动触发 → 采集结果注入周报素材
4. AI 汇总时自动引用："本周关闭 23 个 TAPD 工单，提交 127 次 commit"

**协同涌现**：Report Agent 自己不懂 TAPD API，工作流替它完成了数据采集。

### 场景 2：缺陷提报自动通知（事件驱动工作流）

> 测试员提交缺陷后，相关开发者应自动收到邮件通知。

1. Defect Agent 发布事件 `defect-agent.report.created`
2. AutomationHub 匹配规则 → 触发通知工作流
3. 工作流：LLM 分析舱提取缺陷摘要 → 邮件发送舱推送给开发者
4. 零人工介入，缺陷 → 通知 → 开发者，全自动

### 场景 3：抖音内容工厂（外部数据 + AI + 输出）

> 运营分享抖音链接，系统自动解析、转文字、改写、生成报告。

1. 手动触发或 Webhook 接收链接
2. 抖音解析舱 → 视频下载舱 → 语音转文字舱 → LLM 改写舱 → 报告生成舱
3. 最终产出：原始文案 + 改写文案 + 分析报告

### 场景 4：网页一键发布（工作流调用基础服务）

> 工作流生成的 HTML 报告需要变成可访问的网页。

1. 报告生成舱产出 HTML
2. 站点发布舱调用 IHostedSiteService → 上传到 COS → 生成访问链接
3. 通知推送舱将链接发送给相关人
