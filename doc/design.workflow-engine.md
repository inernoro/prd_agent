...
> **版本**：v1.0 | **日期**：2026-02-12 | **状态**：已实现
>
> **appKey**：`workflow-agent`

## 一、管理摘要

- **解决什么问题**：团队每月从 TAPD 等平台手动拉取数据、清洗、统计、生成报告，流程重复耗时
- **方案概述**：通用工作流引擎，支持可视化编排多种节点（数据采集/LLM 分析/代码统计/渲染），定时或手动触发端到端自动执行
- **业务价值**：从数据采集到报告生成全自动化，每次执行保留完整历史可追溯，产出物支持云链接分享
- **影响范围**：prd-api Workflow 模块、prd-admin 工作流编排页面、定时任务系统
- **预计风险**：低 — 功能已实现并上线
