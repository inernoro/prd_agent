...
### 4.3.1 手势与入口动效补充（2026-06-29）

- 触摸板双指滑动语义由「拖动平移」调整为「围绕中心旋转视角」（呼应 `.claude/rules/gesture-unification.md` 的手势统一原则，宇宙图作为 3D 力导向场景采用轨道旋转而非平移）
- 单击聚焦节点时相机切换改用自然速度缓动（非线性匀速位移），提升聚焦观感
- 文档星系入口图标新增简化星空进入动效，随后迭代为「原始 icon + 胶囊光扫」组合动效

### 4.3.2 2D/3D 折叠切换 + 公开分享页图视图（2026-07-07）

- 知识星球 3D 页顶栏新增「折叠 2D / 展开 3D」切换：星点/光路/流光整组沿 Z 轴动画压平成平面图，相机同步飞到正视机位；折叠态锁旋转、两指滑动改平移（呼应 `.claude/rules/gesture-unification.md`：2D 态走平移手势，3D 态走 4.3.1 的轨道旋转手势，二者不混用）。
- 知识库**公开分享页**（非登录用户可访问）新增图视图支持，与登录态编辑器内的宇宙图/Obsidian 双链图共享同一份渲染实现，游客可查看知识星球（3D，可折叠为 2D）与 Obsidian 风格双链图两种视图。
- 分享链接生成改为幂等：同一文档/库已有有效分享记录时直接复用并登记统一短链，不再每次生成新短链（避免同一篇文档在不同分享入口下产生多个短链，用户在图视图中追溯"这篇是不是分享过"时口径不一致）。
...
- ~~服务端自动补链~~ 已实现「一键生成双链」（2026-07-07，见 4.3.3）：**手动触发**的正则精确标题匹配补链，不是"AI 生成文本时自动包 `[[]]`"——原条目描述的"AI 生成时自动补链"仍未实现，v2 候选保留
- 跨实体引用（缺陷  PR  文档）
- 别名（aka）配置
- 改名时自动更新所有引用
- 宇宙图：按团队分色、时间轴回放、AI 推荐虚线连接

### 4.3.3 一键生成双链（autolink，2026-07-07）

- **触发方式**：Obsidian 双链图页右上角「生成双链」按钮，手动触发，非保存时自动执行。
- **匹配规则**：标题精确匹配——正文中其他文档标题的**首次出现**改写为 `[[标题]]`，跳过代码块/已有链接/frontmatter 区间，幂等可重跑（重复点击不会重复包裹）。
- **实现路径**：Run/Worker 模式（呼应 `server-authority.md`），SSE 推送逐篇扫描进度；后端从 `DocumentStoreController` 抽取 `EntryContentWriteService` 作为共享正文写入路径（内容寻址 + 摘要索引 + 重锚评论 + 重算双链 + 双版本快照），在线编辑 / 版本恢复 / 自动补链三条路径共用同一写入逻辑，避免三处各自维护双链重算而漂移。
- **测试**：`WikiLinkAutoLinker` 单元测试覆盖长标题优先匹配、保护区间（代码块/已有链接/frontmatter）、幂等、ASCII 边界、候选过滤等 17 例。
