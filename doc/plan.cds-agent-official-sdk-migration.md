...
- P1.6 sidecar 自检也已可操作：`/readyz` 现在返回 `blockers/nextActions`，直接说明缺 `SIDECAR_TOKEN`、`ANTHROPIC_API_KEY`、`claude_agent_sdk`、Claude CLI 或 workspaceRoot，并在默认 `runtime-profile-or-env` provider key 模式下提示 provider key 可由 MAP runtime profile/per-request 下发。
