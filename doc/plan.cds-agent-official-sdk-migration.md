...
- P1.6 S3/S4 smoke 证据包补强：`/cds-agent` Runtime 调试会从事件流提炼 approval request/decision 数量、pending 数量、最新 approvalId/decision，以及 cancel request/cancelled 事件证据，并写入复制诊断包，减少靠人工翻 timeline 判断审批和取消是否闭环。
