...
R0 runtime pool 仍未恢复：`prd-agent` 业务项目还有 branch-local
`claude-agent-sdk-runtime-v2-prd-agent` 残留，CDS remote host 为空，
`shared-sidecar-pool-mp4anabh` 没有 running official SDK runtime。
A0 official SDK adapter boundary、V1 页面发布/可观察口径、N6 非代码兼容已有证据，
但都不能替代 R0。R0 通过后下一层仍是 R1：默认 runtime profile 仍为
...
preview alias 绕行的 `R0=pass` 或 `blocked_r1` 记录当作当前 shared runtime pool 已恢复。
...
- 2026-05-18 `scripts/doctor-cds-agent-runtime.sh` 已支持 `SMOKE_CDS_AGENT_DOCTOR_REPORT` 机器可读诊断包；后续校准后，该脚本默认不再探测 branch-local `claude-agent-sdk-runtime*` alias，`aliasCheck.status=blocked_branch_local_alias` 才是当前安全默认。历史 `aliasCheck.status=stable` 只能作为污染清理前的 preview alias 绕行证据，不能证明 shared-service runtime pool 已恢复。
...
- 2026-05-18 `scripts/smoke-lib.sh` 与 `scripts/smoke-cds-agent-one-cycle.sh` 已补远程 preview host 推断：设置 `CDS_HOST=https://cds.miduo.org` 且不显式设置 `SMOKE_TEST_HOST` 时，脚本会优先通过 `cdscli branch status` 读取 `previewSlug`，取不到时按 branch id 确定性推导 preview slug，避免本地网络瞬断时误打 `localhost:5000`。历史 one-cycle 证据目录 `/tmp/cds-agent-cycle-20260518131650` 总耗时 83s，最慢步骤为 V1 authenticated workbench visual 30s、R0 sidecar alias stability 15s、runtime doctor 10s；当时结果为 `status=blocked_r1`、`commercialComplete=false`、failed=0、skipped=0，商业 gate 为 `R0=pass`、`A0=pass`、`V1=pass`、`N6=pass`、`R1/S1/S2S3=pending`。这只能说明当时 preview alias 绕行链路可用，不能替代后续 runtime pool 污染复核。
- 2026-05-18 最新 runtime pool 只读复核目录为 `/tmp/cds-agent-runtime-pool-evidence-20260518165720`，总耗时 12s：`prd-agent` 远程 state 当前仍有 4 个分支带 branch-local `claude-agent-sdk-runtime-v2-prd-agent`，`shared-sidecar-pool-mp4anabh` 当前 `sharedRunning=0`，remote host `0/0`。阻塞项为 `BRANCH_LOCAL_SIDECAR_CLEAN=contaminated:4`、`REMOTE_HOST_AVAILABLE=missing`、`SHARED_POOL_RUNNING=missing`。因此当前有效顺序仍是：先清理业务项目 branch-local sidecar residual，再登记 remote host 并部署 shared official SDK runtime，最后重跑 MAP R0/S1/S2/S3/one-cycle；旧 `/tmp/cds-agent-cycle-*` 只能作为历史 preview alias 绕行证据，不能证明当前 shared runtime pool 已恢复。
...
- 2026-05-18 `diagnostics.executionPanel.deploymentAdvice` 已下沉到 MAP runtime-status 并显示在 `/cds-agent` 当前执行结论中：R1/profile-blocked 不靠 redeploy 解决，S1/S2/S3 pending 不靠 redeploy 解决，只有代码变更、远程容器网络/鉴权、视觉证据或 promotion 才需要新的 deploy/build。页面进一步把执行结论折成 `部署判定`、`命令性质`、`Provider 调用` 三个短判定：R1 dry-run 显示不需要重新部署且不会触发真实 provider；provider one-cycle 必须显式 `SMOKE_CDS_AGENT_ALLOW_PROVIDER_CALL=1`。`scripts/smoke-cds-agent-one-cycle.sh` 同步输出 phase、step index、slowest steps 和 deploy/build advice，用于解释时间花在本地、远程 API、容器 exec、provider 调用还是视觉截图。
...
- 2026-05-18 `cdscli deploy` 热路径已校准：顶层 deploy 会先按 CDS `/api/branches` 的真实 `branch` 字段解析 branch id，不再把本地 `codex/cds-agent-workbench-ui` 误猜成 `codex-cds-agent-workbench-ui`；多项目同名时用 `CDS_PROJECT_ID` 消歧。deploy 触发也改为 stream-safe：5s 触发请求超时时不立即判失败，而是继续轮询 branch status。实测 `CDS_HOST=https://cds.miduo.org python3 .claude/skills/cds/cli/cdscli.py deploy --timeout 900 --no-smoke` 已能对 `prd-agent-codex-cds-agent-workbench-ui` 全绿收敛到 `running`，避免把慢响应误判成需要重复部署。
