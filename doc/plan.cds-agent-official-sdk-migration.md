...
- 2026-05-18 A0 边界证据已从纯静态 grep 升级为静态检查 + sidecar 路由单测 + adapter 行数预算：`scripts/smoke-cds-agent-official-sdk-boundary.sh` 会运行 `claude-sdk-sidecar/tests/test_sidecar_readiness.py`，并在 JSON 报告中写入 `executableEvidence.routingTestStatus=pass` 与 `officialLoopOwnerEvidence.officialAdapterWithinBudget=true`。这证明 `_adapter_for` 默认路由到 `claude-agent-sdk`，`legacy-sidecar` 只能显式 fallback，且官方 SDK adapter 继续保持薄接入层，不能继续膨胀成第二套自研 loop。
