...
- 2026-05-18 A0 边界证据已从纯静态 grep 升级为静态检查 + sidecar 路由单测：`scripts/smoke-cds-agent-official-sdk-boundary.sh` 会运行 `claude-sdk-sidecar/tests/test_sidecar_readiness.py`，并在 JSON 报告中写入 `executableEvidence.routingTestStatus=pass`。这证明 `_adapter_for` 默认路由到 `claude-agent-sdk`，`legacy-sidecar` 只能显式 fallback。
