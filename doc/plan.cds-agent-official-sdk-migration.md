...
- 2026-05-18 后续 runtime pool 复核推翻了“只剩 R1”的执行顺序：`prd-agent` 远程 state 仍有 7 个分支带 branch-local `claude-agent-sdk-runtime-v2-prd-agent`，`shared-sidecar-pool-mp4anabh` 当前 `runningServiceCount=0`，且 `/api/cds-system/remote-hosts` 返回空。最新恢复证据目录为 `/tmp/cds-agent-runtime-pool-evidence-20260518150655`，总耗时约 13s，阻塞项为 `BRANCH_LOCAL_SIDECAR_CLEAN=contaminated:7`、`REMOTE_HOST_AVAILABLE=missing`、`SHARED_POOL_RUNNING=missing`。因此当前有效顺序改为：先清理业务项目 branch-local sidecar residual，再登记 remote host 并部署 shared official SDK runtime，最后重跑 MAP R0/S1/S2/S3/one-cycle；旧 `/tmp/cds-agent-cycle-20260518131650` 只能作为历史 preview alias 绕行证据，不能证明当前 shared runtime pool 已恢复。
