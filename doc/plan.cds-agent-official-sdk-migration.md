...
- P1.6 workspace 准备补强：同一 repo/ref 的 clone/fetch 已加 sidecar 进程内异步锁；`readyz.adapterDiagnostics.workspacePreparation` 会暴露 workspace root、git 是否安装、支持的仓库格式和锁策略。仍未完成的是跨进程/多副本分布式锁、私有仓库 token、workspace GC 和真实远程 run 验证。
