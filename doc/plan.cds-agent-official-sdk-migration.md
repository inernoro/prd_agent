...
- P1.3/P1.6 runtime request 契约继续收缩为 adapter 边界：MAP 现在会把 `mapSessionId/traceId` 透传到 sidecar；协议也支持 per-run `workspaceRoot/gitRepository/gitRef`。官方 SDK adapter 会优先使用 request `workspaceRoot` 作为 `ClaudeAgentOptions.cwd`，并在 `runtime_init` 中回报 `workspaceSource/gitRepository/gitRef`，为后续“审核当前仓库/其他仓库”的 workspace 选择和证据包关联打基础；当前产品入口还没有完整仓库选择器，因此真实其他仓库审核仍需下一轮 UI/API 接入。
