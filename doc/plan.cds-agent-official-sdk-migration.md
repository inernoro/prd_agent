...
- 2026-05-18 `scripts/smoke-lib.sh` 与 `scripts/smoke-cds-agent-one-cycle.sh` 已补远程 preview host 推断：设置 `CDS_HOST=https://cds.miduo.org` 且不显式设置 `SMOKE_TEST_HOST` 时，脚本会通过 `cdscli branch status` 读取 `previewSlug`，自动把目标切到 `https://cds-agent-workbench-ui-codex-prd-agent.miduo.org`。最新 one-cycle 证据目录为 `/tmp/cds-agent-cycle-20260518031829`，总耗时 40s，最慢步骤为 R0 sidecar alias stability 10s、doctor 9s、readiness ledger 5s；结果 `status=blocked_r1`、failed=0、skipped=1，商业 gate 为 `R0=pass`、`N6=pass`、`R1/S1/S2S3=pending`、`V1=skipped`。这证明当前不需要继续重复部署来修 runtime pool，下一步只应修默认 Anthropic/Claude-compatible profile。
