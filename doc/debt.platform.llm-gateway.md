...
当前 open: 53 / in-progress: 8 / paid: 32 / 总计: 93
...
| 2026-08-29-catalog-normalize-drops-any-vendor-prefix | medium | 2026-08-29 | 名录归一化把斜杠前的**任意**前缀都丢掉（`openai/gpt-4o` 与 `private-provider/gpt-4o` 归一后是同一条），于是一个没登记过的上游别名会被判成名录内的那个模型，连带继承它登记的用途与能力；两侧镜像同样如此，所以导入确认与数据面名录门都拦不住它 | 接了一个把自家模型命名成 `{自定义前缀}/{知名模型名}` 的上游时 | open | 去前缀是有意的：聚合型上游（OpenRouter 一类）普遍用 `厂商/模型` 命名，不去前缀同一个模型会分裂成许多条登记。要收紧只有一条正路——名录改登记**完整别名**、前缀参与匹配。不在本 PR 做的理由是它会立刻把大量存量模型判成名录外：存量放行迁移已经是一次性的（只在名录门上线那一次跑），此后新判成名录外的模型没有补戳的时机，enforce 档下会当场开始被拒，等于用一次归一化口径调整制造一场线上拒绝。正确做法是单独一轮：先在 observe 档统计「按新口径会被判成名录外」的实际条数与调用方，补齐完整别名登记，再切判据。风险边界写在这里，别当它已经解决 |
