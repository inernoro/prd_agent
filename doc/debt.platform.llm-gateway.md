...
模型名录也已经可以在控制台补登（`llmgw_model_catalog_entries` + 控制台「模型名录」一段），
剩下这些还写死在代码里：
...
| `ModelCatalog.All` 内置 38 条 | 260 行 | **已还一半（2026-09-16）**：补登机制已落地，同一个标识补登的赢、补登里没有的回落到这 38 条，改完立刻生效（端点每次现查，不经任何缓存）。内置那份保留为兜底与「照这条建一份」的模板 | 不再是债。真实的缺口在**存量**：实测线上两个上游共 573 个模型，落在名录里的只有 27 个（4.7%），其余走关键词猜测，其中 116 个连一条用途都猜不出来（导进来是「哑」模型，不参与任何用途匹配）。补这批存量是运营动作不是代码动作 |
...
## 对外模型删除没有保留窗口（2026-09-16）

**状态**：未还，且不是本轮引入的

删一个对外模型会级联删掉它名下的全部线路。对同步请求没问题——下一次解析拿不到它，
当场结构化失败，说得清。**异步任务不一样**：视频生成把 Offering ID 持久化在任务归属记录里，
之后的状态查询与下载都拿那个 ID 走 `ResolveOfferingAsync` 精确解析，worker 重启后也一样。
线路被删掉的那一刻，一笔已经付过钱、正在跑的视频任务就永远轮询不到、也下载不了。

判据是「删之前有没有人还指着它」，而现在一个都没查。

**为什么不在这一刀里做**：还它要引入「软删除 / 墓碑 / 保留窗口」这一整套新语义——
谁来判过期、过期之后谁来真删、界面上一个「已删但还留着」的模型长什么样。
这是新的产品行为，不是修一个判据（AGENTS.md §5.5 的 B 类）。本轮 PR 的目标是把
模型概念从五个入口收敛成两个，塞进这套东西会让它的边界失控。

**还债的样子**（两条路，二选一）：
- 删之前查在途引用（`video_gen_runs` 等异步任务的归属记录），有就拒绝并说清是哪几笔；
- 或者标记删除、保留到异步任务的留存窗口过期，由清理任务真删。

第一条更小、更快、也够用：删模型本来就是低频动作，让它在有人用的时候拒绝一次，
比造一套墓碑生命周期划算得多。
...
当前 open: 53 / in-progress: 8 / paid: 34 / 总计: 95
...
|----|--------|---------|------|---------|------|------|
| 2026-09-16-offering-has-no-price-override | medium | 2026-09-16 | 线路（`llmgw_model_offerings`）没有价格字段，计价只看物理模型文档。而退场的模型池是**按池成员**计价的，成员身上可以配自己的价格覆盖——搬迁因此会改变这类池的计费：成员价与模型文档不一致时按模型文档算，兑换所成员更彻底（没有物理模型可回落，直接判成未计价、不进用量与限额） | 搬迁一个成员上配过价格覆盖的池时 | open | 本轮（PR #1546）没有让它静默发生：搬迁逐条列出会丢的那几项、值是多少、该填到哪个模型上，线路照常建好。**没有**给线路加价格覆盖层——那是新增字段 + 解析优先级的新语义，属 AGENTS.md §5.5 的 B 类，要用户明确批准。还债的样子是给 Offering 加`InputPricePerMillion` 等五个可空字段，解析时线路价优先于模型价，并在模型页的线路编辑里露出来。判据：给一条线路配上覆盖价后，走它的调用在用量里按覆盖价记账，而同一个物理模型的另一条线路不受影响 |
| 2026-09-16-readiness-scenario-offering-shallow | medium | 2026-09-16 | 就绪探针的 `scenario-capability` 组件判「有没有可用线路」时只看线路自己的 `Enabled` 与健康状态，不看线路指向的物理模型/平台/兑换所还在不在，也不过名录门——而同一个文件里的 `router` 组件两样都判。于是一条指向已停用平台、或会被名录门拦死的线路，在 router 判红、在 scenario 判绿 | 两个就绪组件对同一份配置给出相反结论时 | paid | 同一轮内还清（PR #1546，第 31 轮 review 点名）：两个组件现在都从 `BuildTenantRoutingViewAsync` 取数，目标可用性、名录门、兑换所别名、租户四层判据只有一份；纯函数入口改成接收「已经筛过的可用模型 id 集合」，收线路的旧签名连编译都过不去。顺带把 router 里的池那一半删掉了——运行时早已没有池分支，留着它是替一条死路作保 |
| 2026-09-16-quickstart-catalog-not-caller-specific | medium | 2026-09-16 | Quickstart 的模型选择器列的是「这个用途下、启用着、且至少有一条线路参与排队」的对外模型，**没有**按当前调用方筛：运行时还会过授权名单与场景能力契约（`SupportsAppCallerScenario`），所以一个只授权给别的调用方、或能力对不上这次场景的模型仍会被摆进选择器，用户选中它、点「真实路由测试」当场失败 | 一个租户里存在按调用方限定授权、或按场景细分能力的对外模型时 | open | 本轮（PR #1546）把线路那一半修了（改看服务端算好的排队名次，熔断中与指向已停用目标的不再出现）。**刻意没有**在前端补授权名单与能力判断：能力契约的注释写明「只此一份实现，禁止任何调用方再写 Contains / EndsWith / 字符串数组」，前端补一份就是第三份判据。正解是让服务端按调用方算：`GET /gw/logical-models` 加一个可选的 appCallerCode，由控制台后端用契约筛。代价是 console-api 目前不引用 PrdAgent.\*，要照名录那样再立一份受守卫钉住的镜像。判据：把一个模型的授权名单改成只含别的调用方，Quickstart 的选择器里它必须消失 |
| 2026-09-16-tutorial-still-pool-shaped | medium | 2026-09-16 | 内置教程（`llmgw/tutorial/chapters/`，33 章）整体按「模型池」的概念组织：20 章提到它，其中 3 章（08 准备默认模型池 / 17 池成员 / 18 健康与优先级）的第一步就是「点左侧『模型池』」——那一项已经没有了，`/pools` 跳到模型页，跟着走的人卡在第一步 | 有人照着内置教程从头走一遍时 | open | 本轮（PR #1546）在那 3 章顶部加了「本章待重拍」提示，说清新位置与概念对应（池的默认 → 模型上的默认标记，池成员 → 模型下的线路），不让人卡住也不假装章节已经改好。**没有重写正文**：这三章的主题本身就是池，而且每一步都配着旧 UI 的截图，改文字不换图等于换一种方式误导。还债的样子是按新概念重拍这三章 + 扫一遍其余 17 章的术语，需要部署好的新 UI 与截图流水线。判据：照着教程从第 1 章走到第 20 章，每一步的入口都点得到 |
| 2026-09-16-serving-hosts-catalog-logic | medium | 2026-09-16 | 对外模型清单那个端点里不只有端点绑定：授权、取数、线路可用性、名录门、报价聚合都写在 serving 这一侧。而 llmgw 的模块不变量写着「serving 只负责 host 装配与 HTTP 端点，核心实现仍由明确的 ProjectReference 复用」 | 在 serving 里再加一处与运行时同源的判断时 | open | 本轮（PR #1546）把**判据**逐条搬进了共享实现（名录门、兑换所别名、目标可用性都走同一份），留在那个文件里的是取数与 JSON 组装。剩下那一步（整个端点实现搬进被引用的 core 工程、这里只留绑定与序列化）是结构性改动，在这条已经连开 36 轮 review 的 PR 里展开属 AGENTS.md §5.5 的 B 类，要用户明确批准。判据：那个文件里不再出现取集合的调用与任何可用性判断，只剩把入参交给 core、把结果写成 JSON |
| 2026-09-16-imagegen-contract-no-tenant-dimension | medium | 2026-09-16 | 生图契约的覆盖表（`ImageGenModelAdapterRegistry`）是**进程全局**的一张表，键只有模型名、没有租户维度，而全链路二十来个调用点（`TryMatch` / `BuildRequestParams` / `SupportsResponseFormat` / `GetAdapterInfo`）全是静态方法、拿不到请求的租户。因此按请求密钥判定租户的宿主（`llmgw-serving`）只能装平台级契约，带租户的一条都不装——控制台里配的生图契约在网关那一侧不生效，走网关的生图请求用代码内置那 26 条 | 想让控制台配的生图契约对走网关的请求也生效时 | open | 本轮（PR #1546）先把**跨租户改写**这个洞堵上：多租户宿主跳过带租户的契约，跳过条数与原因回写同步状态、在控制台那一屏说出口（不说的话界面只会显示「生效 0 条」而无从解释）。还债的样子是给注册表补租户维度：`ReplaceOverrides(tenantId, …)` + `TryMatch(tenantId, modelName)`，并把请求租户从 `GatewayRequestContext` 一路传到 `OpenAIImageClient` / `ImageGenRequestBuilder` 这些静态调用点。那是二十来处签名变更、跨 MAP 与 serving 两个宿主，属 AGENTS.md §5.5 的 B 类扩范围，没有在本 PR 展开。判据：A 租户配 `nano-banana*` 后，B 租户在 serving 上发同名模型的生图请求，尺寸与参数必须仍按 B 自己的契约（或代码内置）走 |
| 2026-09-11-scheduler-config-is-dead | medium | 2026-09-11 | 后台「调度器配置」页可以填降级阈值、不可用阈值、健康检查间隔、自动恢复开关、恢复成功次数，保存成功、界面回显正常，但**没有任何运行时代码读取它**。运维把不可用阈值从 5 调到 2 以为收紧了熔断，实际毫无变化 | 恒定成立，只要有人相信那张表单 | open | 证据：`ModelSchedulerConfig.cs` 的字段全仓消费方只有 `SchedulerConfigController.cs`（纯 CRUD）、`InitController.cs` 与前端表单；真实判据是 `GatewayCircuitBreakerPolicy` 的常量。二选一：把这些字段接进 `GatewayCircuitBreakerPolicy`（须先想清楚租户级覆盖的语义），或者把表单删掉。**填了不生效比不能填更糟**——它让运维以为自己调过参 |
| 2026-09-11-health-probe-never-ran | medium | 2026-09-11 | `ModelPoolHealthProbeService` 三重失效：默认关闭且全仓无任何配置打开（代码路径从未执行过）；即使打开也只扫 `model_groups`，网关权威池 `llmgw_model_pools` 与 `llmgw_model_offerings` 一个都覆盖不到；多实例共享同一 Mongo 却只靠进程内字典去重，会同时探同一端点并各自无条件覆写健康与成功计数。同时 `PoolFailoverNotifier` 给运维的告警文案写着「系统已启动自动探活，恢复后将自动通知」，这句话在当前配置下是假的 | 想靠主动探活而不是被动半开来恢复时 | open | 本轮已用「冷却 + 半开租约」把恢复做成被动路径（不依赖探针，也不额外发付费探测），所以探针不是必需品。但那句虚假告警文案必须先改掉——它比没有文案更误导。要真启用探针，得先解决集合错位与分布式租约（可照抄 `TryClaimHalfOpenOfferingAsync` 的条件写 + ModifiedCount 校验） |
| 2026-09-17-bind-active-app-callers-still-writes-pools | medium | 2026-09-17 | 配置权威那条「给 active 调用方绑池」的写端点还活着，它按池的可用性把 `ModelPoolId` / `AllowedModelPoolIds` / `DefaultModelPoolId` 写回调用方文档。而解析器已经不读这几个字段了——跑一次它，结果是给一批调用方盖上「残留池绑定」的戳，解析器每次请求都会为此打一条告警，路由一点没变 | 有人为了修「调用方没人接得住」去跑这条端点时 | open | 本轮（PR #1546）只把**读**这一侧扳正了：配置权威报告与发布闸都改判「有没有对外模型接得住」，不再看池绑定。写这一侧没动——删端点要确认没有运维脚本在调它，改语义（改成给调用方找一个对外模型来认领）是新行为，两者都越过本 PR 的范围基线（AGENTS.md §5.5 的 B 类）。判据：这条端点要么下线并在响应里指路到模型页，要么改成写认领关系而不是写池绑定 |
| 2026-09-17-offering-delete-has-no-tombstone | medium | 2026-09-17 | 删对外模型会把它名下的线路真删掉，而已提交的视频任务把线路 id 写在自己文档里、后续轮询与下载都靠它回到同一个上游。本轮已在删除前加了一道「还有没有在途任务在用」的查询闸，但那是一次读，挡不住「读完到删之间又提交了一个任务」的竞态 | 在删对外模型的同一瞬间恰好有新任务提交时 | open | 不漏的做法是线路软删除：盖墓碑、等引用它的任务全部走到终态再回收。那要改动每一处列线路的查询（列表、解析、发布闸、清单端点都得学会跳过墓碑），是一个新的语义类别，越过本 PR 的范围基线（AGENTS.md §5.5 的 B 类）。判据：删除之后，一个在那一瞬提交的任务仍能取回结果 |
| 2026-09-17-gateway-startup-still-creates-indexes | medium | 2026-09-17 | 网关两侧的启动路径上都还有存量的自动建索引：数据面初始化器十五批（走 `CreateBsonIndexesAsync`）、控制台二十四处。与 `no-auto-index` 相悖，副本集滚动启动时各建各的、建失败会让进程起不来，而这两种后果都发生在没人盯着的启动路径上 | 下一次有人给网关集合加索引，或排查一次启动失败时 | open | 本 PR 自己引入的五条已改成只查不建（线路身份索引、同用途唯一默认、同用途唯一认领、补登键空间唯一、同租户模式唯一），DBA 指南里逐条可查，并有守卫按棘轮盯住控制台的建索引处数只降不升。存量那两批没动——一次性搬迁要同时给指南补齐近四十条的建法，并确认各部署都已手动建过，否则拔掉自动建等于让新库缺索引且没人知道。判据：两侧启动路径都不再出现建索引调用，且指南里逐条可查 |
| 2026-09-17-console-unique-indexes-need-dba | medium | 2026-09-17 | 控制台的四条唯一索引（同用途唯一默认、同用途唯一认领、补登键空间唯一、同租户模式唯一）改成启动只查不建之后，尚未手动建过索引的部署会失去这四条库级不变量：端点里的「先查有没有别人」在并发下挡不住，两个管理员同时改会各写各的，两人界面都显示已生效 | 每一个尚未跑过这四条建索引命令的网关部署 | open | 定义与后果逐条写进了 DBA 指南 5.1.2；缺哪一条，控制台启动日志里就有对应的一行，写着缺席期间会退化成什么样。这不是代码待办，是一次运维动作：判据是四条索引在每个部署的网关库里都查得到 |
| 2026-09-17-system-pool-picker-lists-unmigrated | low | 2026-09-17 | 服务网关设置里「系统级模型来源」选模型池时，下拉仍会列出**还没搬成对外模型**的池。选中保存会被明确拒绝并给出下一步（去跑一次搬迁，或改选指定模型），所以不会出现静默坏配置；但把一个注定失败的选项摆在那里，仍然是让用户替系统做一次无效尝试 | 打开服务网关设置、系统级模型来源选模型池时 | open | 本轮（PR #1546）只做了保存端点与取用路径两道拒绝——那两道是缺陷闭合所必需的。列表侧要标出「未搬迁」需要改控制台前端的接口形状与渲染，属 AGENTS.md §5.5 的 B 类扩范围，没有在这条已经连开多轮 review 的 PR 里展开。还债的样子：读端点给每个池带上「能不能被解析到」的标记，界面把不能选的置灰并写明原因与下一步（minimal-user-input：不该把注定失败的选项摆给用户）。判据：一个没搬迁过的池在下拉里不可选，且旁边写得出为什么 |
| 2026-09-16-catalog-backlog | medium | 2026-09-16 | 名录补登的**机制**已落地（控制台可补、立刻生效、可回退），但**存量没补**：实测线上两个上游共 573 个模型，落在名录里的只有 27 个（4.7%）。OpenAI 那 130 个里 116 个在名录外（89.2%），其中 `babbage-002` / `davinci-002` 这类连一条用途都猜不出来，导进来就是「哑」模型——入库了，但模型池选型时不参与任何用途匹配 | 有人导入一批名录外模型、以为它们能被选中时 | open | 这是运营动作不是代码动作：得有人逐批核对上游文档再补。优先级排序建议按「已导入且在用」的那批先来（OpenAI 已导入 110 个）。不做也不会坏，只是那些模型选不中——但**选不中这件事今天在界面上是看得见的**（「名录外」chip + 「用途待定」chip），不是静默的 |
| 2026-09-16-capability-vocab-has-synonyms | low | 2026-09-16 | 运行时的用途词表（`LogicalModelCapabilityPolicy.CanonicalCapabilities`）有 35 项，其中多组互为同义：`tools` / `tool_calling` / `function_calling` / `parallel_tools` / `parallel_tool_calls` / `parallel_function_calling`；`image` / `image_generation` / `text2img`；`vision` / `multimodal` / `image_input` / `vision_generation`；`json_mode` / `json_schema` / `response_format` / `structured_output`；`logprobs` / `top_logprobs` / `token_logprobs`。补登表单直接把这 35 个摆成复选框，人从里面挑很容易挑错或漏挑，而挑错的后果是「填了没用」 | 补登模型时 / 给对外模型配能力时 | open | 本轮刻意没动：收敛词表会改变线上路由行为（哪些模型算命中哪种用途），属产品行为变更，须用户明确批准（AGENTS.md §5.5 范围熔断，B 类）。还债的样子是先在词表里标出「规范项 / 同义项」，界面只让选规范项、同义项做输入时的自动归一，且归一表要有覆盖守卫 |
| 2026-09-11-sse-midstream-no-failover | low | 2026-09-11 | 流式请求一旦吐出第一个 chunk，之后上游断连就不能再切线路：只记一次失败 + 推 Fail chunk，已产出内容原样留给调用方。切换窗口严格在首个 chunk 之前 | 长回答中途连接重置 / 读超时 / 上游 5xx trailer | open | 中途重放会重复计费且可能重复内容，不切是正确取舍。但**连 0 token 的情况都没有重试**：`firstByteAt` 为 null（一个 SSE 事件都没到）时本来可以安全重试，现在也直接失败。补这一档是低风险改动 |
| 2026-09-11-model-pool-dispatcher-is-a-shell | low | 2026-09-11 | `PoolHealthTracker` / `ModelPoolFactory` / `ModelPoolDispatcher` / `Strategies/*` 这一整套看起来在管熔断调度，实际**在生产请求链路上是死代码**：`IModelPool.DispatchAsync` 全仓无调用方，`ModelPoolFactory` 唯一使用点是池连通性测试而该测试还绕过了策略。其中 `PoolHealthTracker.IsAvailable()` 带副作用置半开标志且无租约，置位后只能靠后续 Record 复位，而调用方是对全部端点做过滤、只对第一个发请求，其余端点从此在本进程内永久不可用；`GetSnapshot` 读 `LatencyWindow` 还没加锁，与 `AddLatency` 并发会抛异常，而类注释写着「线程安全」 | 有人以为改它就改了线上行为时 | open | 真正跑在请求链路上的熔断全部在 `ModelResolver.cs`。这套空壳的危险在于误导后来者。建议随「模型池冻结新建」一起删除或明确标注为仅测试用 |
| 2026-09-11-unpriced-calls-not-blockable | medium | 2026-09-11 | 缺价模型的调用现在「看得见、不计入限额」，但还**拦不住**：成本状态记为 `unpriced`、不进美金汇总，调用本身照常放行。对「避免滥用」来说这只做到一半——一个没配价的模型可以无限量调用而不触发任何预算闸 | 有人拿没配价的模型跑量时 | open | 本轮（模型可编辑 + 缓存计价 + 美金口径）刻意没做拦截：拒绝调用会改变线上可用性，属于产品行为变更，须用户明确批准（AGENTS.md §5.5 范围熔断）。补法是租户级开关 `BlockUnpricedModels`，默认关闭；开启后 `CostStatus=unpriced` 的模型在解析阶段直接拒绝并给出「去补价格」的下一步。判据：开关打开时，对一个没配价的模型发请求必须被拒且错误里点名该模型 |
| 2026-09-08-default-generation-member-flapping | medium | 2026-09-08 | 图片生成默认池（`pool-default:tenant_map_internal:generation`）有两个成员，`chatgpt-image-latest` 健康态在抖：连打 14 次真实网关解析，12 次落到健康的 `gpt-image-1`，2 次落到它且 `healthStatus=Unavailable`。命中时出图请求会失败，而对话出图工具（`chat-agent.image-gen::generation`）走的正是「不指定模型、由默认池兜底」。**影响面只有这一个调用方**：视觉创作不落到这个池——它建 run 时先经视觉模型开放策略把「未指定」选成一个显式逻辑模型 id（`platformId=logical-model`），worker 沿用该身份，与未指定模型的默认池无关 | 对话出图工具的池内调度恰好选中该成员时（约 1/7） | open | 来源 #1338 关闭时的运维观察。不是池缺失，是成员健康。补法二选一：在网关控制台把该成员从默认池停用；或查它对应 Offering 的上游状态（401 / 限流）修好上游。修完用 `GET /api/open-platform/app-callers/resolve-model?appCallerCode=chat-agent.image-gen::generation&modelType=generation` 连打 20 次，`healthStatus` 全 Healthy 即可结清 |
| 2026-08-27-appcaller-intent-finite-keywords | low | 2026-08-27 | Quickstart 颁发 appCallerCode 前要求先写清「我想要做什么」，判定靠控制台前端的一张有限关键词表（7 个调用方 + 13 个场景，最长命中优先）。表外的说法认不出来，用户只能从清单里手动选一段 | 用户用清单外的说法描述用途、又不愿手动选时 | open | 这是刻意的取舍不是遗漏：AGENTS.md §5.5 的熔断条款点名「同一个自由文本解析器第二次被要求加同义词或嵌套格式」就该停手，所以这里只做有限枚举 + 别名，认不出来就明说认不出来并给出清单（`no-rootless-tree.md`：不编造），不往里加正则或嵌套语法。要扩就加词条，不要把它变成解析器；真要理解自然语言，正确做法是换成后端一次结构化抽取，而不是在前端堆同义词。闸门本身已在预览环境过真视觉验收（13 张证据、双主题、真实签发闭环、手机端独立路径），报告见 CDS 验收中心 [功能验收 · LLM Gateway Quickstart / 颁码闸门与三步向导 · 2026-08-27](https://cds.miduo.org/reports?project=prd-agent&folder=fb52b905cbee46838078a4c6f458eb01&report=ae0200e96de24580beb961450a6e32a6) |
| 2026-08-28-llmgw-subdomain-console-api-prefix | medium | 2026-08-28 | 网关观测前端子域（`*-llmgw`）上，控制台 SPA 请求的是 `/llmgw/gw/*`，但该出口的 nginx 只把 `/gw/*` 转给 console-api——POST `/llmgw/gw/auth/login` 直接被静态 location 判 405，从这个出口用账号密码登不进控制台（经 MAP 用户菜单 SSO 进来的那条路不受影响，本轮验收走的就是它） | 有人直接打开 `*-llmgw` 子域用账号密码登录时 | paid | **2026-08-28 已修复**（commit 5086ffd）：给 llmgw-web 的 nginx 补了 `/llmgw/gw/` 反代（rewrite 掉前缀后转同一个 console-api 上游），并加了一条构建期的挂载点覆盖守卫（前端能算出的每个 API base 都必须有对应 location，删掉那条反代立刻红）。线上已验：该路径的登录端点返回 API 自己的 JSON 而不是 405，且经 SSO 进入后在子域 `/llmgw/quickstart` 上接入向导正常渲染、没有任何 console API 响应是 HTML。原始记录：本轮做「上输入下输出」时顺带发现。当时列的两条修法：给该出口的 nginx 补 `/llmgw/gw/` → console-api 的转发，或让子域构建把前端 API base 设成 `/gw`。与 `cds-dual-exit-topology.md`「双出口都要能用」直接相关——出口在，但从它进不去登录 |
| 2026-08-28-model-catalog-coverage-and-enforcement-point | medium | 2026-08-28 | 内置模型名录只覆盖常见的那批（OpenAI / Anthropic / Google / DeepSeek / 通义 / 智谱 / Kimi / 豆包，约 38 条），其余一律算名录外、要管理员显式放行。两道门都已就位（导入入库 + 数据面解析），仍欠的是覆盖面本身，以及主站进程内旧路径解析出的模型这道门管不到 | 接了名录未覆盖的上游，或有人绕过控制台直接往库里写模型文档时 | open | 第一道门在导入入库：用户在那一刻就知道被拦了、为什么、怎么放行。2026-08-29 补上第二道门（数据面）：拦截点罩在模型解析器的出口上，一个函数罩住全部分支而不是逐个 return 前判一次，名录按仓库既有的镜像 + 守卫模式下沉到运行时侧（同能力契约那对镜像表）。配套的三件事都做了：三条写入路径都盖放行标记、名录门上线时把存量一次性补成已放行（如实记成未经人工审阅）、`LlmGateway:ModelCatalogGate=observe` 作为降档逃生阀。**仍然管不到的一处**：主站进程内旧路径（回滚保险）解析出的模型来自主站自己的模型集合、不在网关模型库里，名录外的会被这道门拦下且没有地方盖放行标记——真要走那条路径得先把门降到 observe。覆盖面是有意的起点而不是终点：名录只登记确实知道的事实，宁可判成名录外让人显式放行，也不猜一把当事实（猜正是这次要修掉的病根）。扩充方式是往名录加条目，不是往关键词表加词。2026-08-29 线上核过一轮：预览环境 319 条存量模型在名录门上线时已被一次性补上放行标记，名录内模型真实调用通（3 秒拿到回复），名录外但已盖戳的模型也照常打到上游（回的是上游 404，不是名录门的拒绝），即门没有误伤。「拦得住」那一半线上验不了——每条写入路径都会盖戳，正常操作造不出没戳的行——改由真 Mongo 的行为用例覆盖，三条断言互为对照（名录内放行 / 名录外无戳拦下并点名错误码 / 同一条补戳后放行）。**滚动升级窗口的已知边界（2026-08-29）**：存量放行迁移改成一次性之后，若多副本滚动升级期间旧副本又导入了一个名录外模型（旧代码不盖放行标记），而新副本这边迁移已标记完成，那条模型此后不会再被自动补戳，数据面按 enforce 档会拒它。这是刻意选的方向：宁可拒（管理员在 Provider 页重新导入并放行即可），也不要为了兼容让迁移每次启动都跑一遍——后者正是它被判 P1 的原因（绕过控制台直接写库的模型会被自动放行，门每重启开一道缝）。要消除这个窗口，正解是升级期间把名录门降到 observe、等副本全换新再切回，而不是放宽迁移条件 |
| 2026-08-28-intent-draft-env-now-inert | low | 2026-08-28 | CDS 项目 env 里的 `LLMGW_INTENT_DRAFT_KEY` 与 `LLMGW_INTENT_DRAFT_APP_CALLER` 已无人读取（凭据改为网关自签、appCaller 在代码里固定），但 cdscli 没有删除子命令，两个键仍留在项目配置里 | 下次有人翻项目 env 时会以为它们还生效 | open | 值本身已无害：那把 key 对应的 service key 已撤销，留下的是一串失效字符串。不置空是因为 cdscli 只能 set 不能 delete，置空会新造一个「空值键」状态而不是移除。补法：cdscli 支持 env delete 后一并删掉；`LLMGW_INTENT_DRAFT_BASE_URL` 要留着——它仍是 serving 地址的探测来源之一 |
| 2026-08-28-system-gateway-settings-single-consumer | low | 2026-08-28 | 「服务网关设置」的系统级模型目前只有一个消费方（Quickstart 的一句话推导）。页面上「谁在用它」那张卡是写死的一条，不是从后端扫出来的注册表 | 第二个系统级功能要用同一份配置时 | open | 只有一个消费方时做注册表是过度设计（`predicate-and-wiring-discipline` 形状 2 的反面：建一半不如先不建）。补法：新增第二个系统级功能时，把 consumers 改成后端按 appCaller 前缀 `llmgw-console.` 扫出来的列表，并加一条守卫断言「每个系统级 appCaller 都出现在 consumers 里」，否则加了功能而设置页不提它，用户改配置时不知道影响面。设置页整体已在预览环境过真视觉验收（14 张证据、双主题 + 手机端真实触控、真实部署上触发并跑通一次凭据自愈），报告见 CDS 验收中心 [功能验收 · LLM Gateway 服务网关设置与 401 治理 · 2026-08-28](https://cds.miduo.org/reports?project=prd-agent&folder=fb52b905cbee46838078a4c6f458eb01&report=17fa0c65c331491796cb7af284d1a0f5) |
| 2026-08-28-intent-draft-reason-language-drift | low | 2026-08-28 | 推导端点的 `reason` 字段要求写中文（提示词明写「这是唯一允许出现中文的字段」），实测同一个模型有时仍回英文句子，中文界面里会混出一句英文解释 | 用户看推导结论那一行时 | open | 只影响解释句的语种，`app`/`feature`/`requestType` 三段已被「禁止中文 + 两个示例」钉稳（真实网关连打 5 次同句 5/5 一致，4 句未见过的输入全部产出合法 kebab 码）。不在本次继续加提示词措辞：按 AGENTS.md §5.5，同一个自由文本判定第二次被要求加同义词就该停手。补法是让推导端点在解析后判一次 `reason` 是否含中日韩字符，不含就换成本地关键词表给出的中文解释，而不是继续往提示词里堆句子。改版整体已在预览环境过真视觉验收（16 张证据、桌面双主题 + 手机端真实触控、真实模型推导闭环），报告见 CDS 验收中心 [功能验收 · LLM Gateway Quickstart / 三屏创建线与三页签产物屏 · 2026-08-28](https://cds.miduo.org/reports?project=prd-agent&folder=fb52b905cbee46838078a4c6f458eb01&report=cbead65099ba4ac58b8ffe9b81c27ce1)。2026-08-29 产物屏又补了设计稿里一直没落地的两条结构改动（一次性密钥常驻到页签之上、页签降级成分段控件），单独过了一轮真视觉验收（16 张证据、双主题 + 手机端真实触控、真实签发闭环），报告见 CDS 验收中心 [功能验收 · LLM网关 / LLM Gateway Quickstart 产物屏结构改稿 · 2026-08-29](https://cds.miduo.org/reports?project=prd-agent&folder=824dc5add30e41b38b586040d7a9c3c5&report=af94f55f566140c5a1fc9779041bfb54) |
| 2026-08-28-system-team-budget-not-enforced | medium | 2026-08-28 | 系统内部消耗已经归到专属的「系统内部」团队（`EnsureSystemTeamAsync`），但这个团队本身没有月度预算、没有速率上限——归属分开了，**限额还没分开** | 系统级功能开始规模化调模型时 | open | 本次要解决的是「记在谁头上」，那一步已闭环；「记多少就停」是另一件事：`LlmGwTeam` 有 `MonthlyBudgetUsd` / `RateLimitPerMinute` 字段，但系统团队建出来时两项都留空（不猜一个数字比猜错好，`no-rootless-tree`）。补法：在「服务网关设置」加一项系统团队月预算，落到同一个团队文档上，复用既有的团队预算判定，不另起一套系统级限额 |
| 2026-08-28-system-team-not-visible-in-team-page | low | 2026-08-28 | 「系统内部」团队会出现在「团队与成员」列表里，但看不出它是系统自建的——名字之外没有任何标记，也没有拦住「改名 / 停用」这类操作 | 有人在团队页把它改名或停用时 | open | 停用不会造成静默故障：`EnsureSystemTeamAsync` 每次都会把它掰回 active，`GATEWAY_KEY_TEAM_INACTIVE` 也已进可自愈码。改名会让设置页显示的名字变了但归属不变（id 是 `{tenantId}_system`，认 id 不认名字）。补法：团队列表给系统团队一个「系统」标记并禁用删除，同时把它从「可选归属团队」的下拉里排除 |
| 2026-08-29-catalog-normalize-drops-any-vendor-prefix | medium | 2026-08-29 | 名录归一化把斜杠前的**任意**前缀都丢掉（`openai/gpt-4o` 与 `private-provider/gpt-4o` 归一后是同一条），于是一个没登记过的上游别名会被判成名录内的那个模型，连带继承它登记的用途与能力；两侧镜像同样如此，所以导入确认与数据面名录门都拦不住它 | 接了一个把自家模型命名成 `{自定义前缀}/{知名模型名}` 的上游时 | paid | **2026-08-30 已修**：归一化不再剥前缀（前缀是标识的一部分），查名录先按完整标识查；查不到且带前缀时，只在那个前缀是**名录自己登记过的厂商段**（各条登记的出品方 + 带前缀别名里 `/` 左边那一段）时才剥一层重查。于是 `openai/o1-preview` 照旧命中，`private-provider/gpt-4o` 落空。两侧镜像同改，镜像守卫的一致性探针加了这三种写法。**配套的一次性窗口**：口径收紧会把一批「旧口径判成名录内、因而导入时没盖过放行标记」的存量模型变成名录外，而上一条存量迁移早已跑完——所以新增迁移 `model-catalog-grandfather-v2-strict-vendor-prefix`，同一套认领机制、另一个 id，跑完即关；放行人如实写成「口径收紧前已入库」。原始记录：去前缀是有意的：聚合型上游（OpenRouter 一类）普遍用 `厂商/模型` 命名，不去前缀同一个模型会分裂成许多条登记。要收紧只有一条正路——名录改登记**完整别名**、前缀参与匹配。不在本 PR 做的理由是它会立刻把大量存量模型判成名录外：存量放行迁移已经是一次性的（只在名录门上线那一次跑），此后新判成名录外的模型没有补戳的时机，enforce 档下会当场开始被拒，等于用一次归一化口径调整制造一场线上拒绝。正确做法是单独一轮：先在 observe 档统计「按新口径会被判成名录外」的实际条数与调用方，补齐完整别名登记，再切判据。风险边界写在这里，别当它已经解决 |
| 2026-08-30-intent-draft-ignores-inband-error | medium | 2026-08-30 | 一句话推导的服务端读流只挑 `delta.content`，不认 serving 在响应头之后回的失败帧（`finishReason=error` + 顶层 error）。模型若在失败前已经吐出一段可解析的 JSON，端点会对一次**失败且计费**的调用回 `ok: true` | 上游在响应头发出之后才失败，且此前已吐出可解析片段时 | paid | **2026-08-30 已修**：服务端读流认失败帧（`finishReason=error` 或顶层 error，两种命名都认），命中即中止并降级到本地关键词判定。 原始记录：与 2026-08-30 已修的 Quickstart 前端那条同形，判据可直接照搬（认失败帧就中止并降级到本地关键词判定）。这条在服务端，前端那条修完并不覆盖它。因 §5.5 熔断线未在本轮展开 |
| 2026-08-30-intent-draft-no-completion-cap | medium | 2026-08-30 | 一句话推导把用户那句原样发给模型，没有设 `max_tokens`，也没有对累积的响应体积设上限；系统 appCaller 没有预算闸，其密钥每分钟允许 120 次。模型若没有自带小上限，单次可以一直生成到 40 秒 deadline | 有人拿注入式的长 intent 反复打这个端点时 | paid | **2026-08-30 已修**：请求带 `max_tokens=256`（产物只是四个短字段），另加累积体积上限 4000 字符兜住无视上限的上游。 原始记录：期望产物只是四个字段的 JSON，给一个紧的 completion 上限 + 累积体积上限即可，两处都很小。这条是**成本面**而不是正确性面，但系统调用花的是平台的钱，值得单独一条。因 §5.5 熔断线未在本轮展开 |
| 2026-08-30-settings-model-options-bounded-page | low | 2026-08-30 | 服务网关设置的逻辑模型下拉取前 200 条且没有排序，租户启用的对话类模型超过 200 个时，已保存的那个可能不在这一页里——设置接口仍然回那个值，但下拉里没有对应选项，页面看起来像「没选模型」，而系统调用仍在用那个看不见的模型 | 单租户启用的对话类逻辑模型超过 200 个时 | paid | **2026-08-30 已修**：下拉查询加固定排序，并把已保存的那条显式并进结果——排在 200 条之外也能选中。 原始记录：修法是把已保存的那条显式并进结果，或给下拉做确定性分页/搜索并保住当前选中项。触发门槛较高（200 个启用的对话模型），故记 low。因 §5.5 熔断线未在本轮展开 |
| 2026-08-30-system-appcaller-takes-over-colliding-code | high | 2026-08-30 | 系统级凭据自愈在登记自己的 appCaller 时，认的是「同租户下这个码的文档存在吗」，存在就把该码的**全部**文档搬到系统团队。搬迁本意是迁自己上一版留下的文档（那时挂在租户第一个业务团队上），但判据分不出「我自己的旧文档」和「恰好同码的业务文档」——租户若真有一条自助登记的同码 appCaller，只要有人打开这一页，它就会被搬走，其团队级密钥随后在 serving 的归属比对上被判 403 | 租户下已存在自助登记的 `llmgw-console.intent-draft::chat` 时 | paid | **2026-08-30 已修**：系统自建的 appCaller 盖 `SystemManaged` 标记，只迁带标记的；存量按当时写死的 `SourceSystem` 认亲并顺手补标记；遇到同码的业务文档不再接管，返回说得出原因的失败（提示改码或协调）。 原始记录：**合并前应先修**：这是唯一一条会动到用户既有数据、且只要打开设置页就会触发的。修法与系统团队那条同形——给系统自建的 appCaller 文档盖 `SystemManaged` 标记，只迁带标记的；发现同码但不带标记的业务文档时，按名字占用那套返回说得出原因的失败（提示改码或改名），而不是默默接管。不在本轮做的原因是 PR 已过 §5.5 熔断线（Review 修复提交 11 个），停不停待仓库负责人定 |
| 2026-08-30-quickstart-member-pick-sent-as-pool | medium | 2026-08-30 | Quickstart 测试栏选中池内某个成员时，请求体带的是 `model_policy: pool` 但既没有池 id 也没有钉住成员的字段。serving 于是把模型名留在 `ExpectedModel` 里，而 appCaller 一旦绑了严格池契约，解析器会把它当成**池标识**去找，找不到就回 `ROUTE_CONFIG_INCOMPATIBLE`——选了成员反而跑不通 | 用绑了严格池契约的 appCaller 在测试栏选具体成员时 | paid | **2026-08-30 已修**：选中成员改按「在已绑池内钉这个成员」的契约发（`model_policy: pinned` + `pinned_model_id` 与 model 同值），解析器不再把模型名当池标识。 原始记录：修法是发真正的池身份 + 钉成员的字段（`pinned_model_id` 一类），或按 serving 既有契约表达「在已绑池内选成员」。同样因 §5.5 熔断线未在本轮展开。注意与「auto」那条路径无关：不选成员时策略是 auto，不受影响 |
| 2026-08-30-quickstart-stream-eof-without-done | medium | 2026-08-30 | 真实调用的流在没有 `[DONE]` 帧的情况下结束时，页面仍按「收完了」处理并报成功。serving 在响应头已发出后遇到取消会直接收尾——不发错误帧也不发 `[DONE]`，于是一次被截断的付费调用在页面上看起来是成功的 | 上游或代理在响应头之后中断这次流式调用时 | paid | **2026-08-30 已修**：客户端跟踪是否收到 `[DONE]`，EOF 时没收到就判「未完成」并说明这次仍会计入用量，已收到的半截输出保留当证据。验收脚本让桩在第 3 帧直接断连，红绿闭环已跑。 原始记录：与 2026-08-30 已修的「流里夹着的失败帧」同一族，但判据不同：那条认的是**出现了失败帧**，这条要认的是**没有出现完成帧**。修法是客户端跟踪有没有收到 `[DONE]`，EOF 时没收到就判「未完成」而不是成功。不在本轮做的原因：PR 已触发 §5.5 熔断线（Review 修复提交超 8 个），停不停由仓库负责人定。触发概率有限——最常见的取消源是客户端自己断开，那种情况下没有人在看这一屏 |
| 2026-08-30-quickstart-vision-fallback-ignores-prompt | low | 2026-08-30 | 视觉类测试在**没有上传图片**时走内嵌的 1x1 测试图，这条分支直接返回写死了 `Describe this test image` 的内容，把用户在输入框里写的那句丢掉了。输入框显示的是用户那句，真正发出去的是写死那句 | 选视觉类、不传图、又在输入框里写了自己的提示词时 | paid | **2026-08-30 已修**：三个内嵌图助手改成接收 prompt，无附件分支把算出来的那句传下去；守卫钉住这条接线（不传即红）。 原始记录：注意与 review 的描述有出入：cURL 片段与真实请求走同一个 `userContentFor`，所以**两者一致**，不一致的只是输入框那一处显示。修法是让无附件分支也用算出来的 prompt 拼内容（有附件那条分支已经这么做了）。同样因 §5.5 熔断线未在本轮展开 |
| 2026-08-30-gemini-ingress-model-only-from-route | medium | 2026-08-30 | Gemini 入口的模型只从路由段 `/v1beta/models/{model}:generateContent` 读，body 里的 `model` serving 不看（它还会被当成 dropped parameter）。于是名字带斜杠的模型（聚合型上游的 `厂商/模型`，本仓库池成员的常见形态）在 Gemini 入口上**钉不住**——路由段装不下一个带斜杠的名字 | 用 Gemini 入口测试一个 `厂商/模型` 形态的池成员时 | paid | **2026-08-30 已修**：serving 的 Gemini 入口在路由段为 auto/空时回退读 body 的 `model`，并把 `model` 从 dropped parameters 里摘掉（它现在是被消费的路由字段）。前端相应简化：钉得住就写进路由段，带斜杠的名字走 body，两条路落到同一个模型。 原始记录：Quickstart 侧已按事实说话（2026-08-30）：钉得住就把模型写进路由段，钉不住就保持 auto 并明说「本次按池调度执行」，不再出现「路径打 auto、屏幕写实际执行 X」这种编造。**真正的修法在 serving**：要么路由段按 URL 解码后再取模型（客户端发 `%2F`），要么让 gemini 端点在路由段为 auto 时回退读 body 的 `model` 并把它从 dropped parameters 里摘掉。两条都要改网关 serving 侧的 Gemini 入口处理，而那个文件不在本 PR 首轮文件清单内，按 §5.5 范围熔断条款（文件范围扩出首轮清单即熔断）不在本轮展开。**未验证**：ASP.NET Core 对路由段里 `%2F` 的处理我没有实测，所以上面第一条修法要先验再做，别照着写 |
| 2026-08-30-catalog-gate-exchange-models-unstamped | medium | 2026-08-30 | 走兑换所（exchange）解析出来的模型不在 `llmgw_models` 里，名录门查不到它们的放行标记，所以只能按「管理员在控制台建兑换所时当场列出了这些别名」这一层人工声明放行——兑换所文档本身没有 per-model 的放行标记，也没有导入时的确认闸门。于是绕过控制台直接往兑换所的 `Models` 数组里加一个别名，两道门都拦不住 | 有人绕过控制台直接改兑换所文档，往里塞新的模型别名时 | paid | **2026-08-30 已修**：兑换所改成**逐条**盖放行标记——控制台写入路径（`BuildExchangeModels` 单一出口）对名录外的别名逐条记下谁放行、什么时候；数据面不再认「这个 PlatformId 是一条兑换所记录」（那是认容器：进了这个门的都算放行），改成核对兑换所确实声明过这条别名、且该条目带着放行标记，与手工新增模型同一套依据。存量兑换所建于盖戳之前，配套一次性窗口 `exchange-model-allowance-v1` 补戳（只补当时确实被列出过的别名，放行人如实写成迁移），否则判据一收紧它们会集体开始被拒。行为用例走真 Mongo：声明了别名但没盖标记必须被拦并点名 `MODEL_NOT_IN_CATALOG`，补上标记后不再被这道门拦。仍然成立的边界：能直接改库的人绕得过所有应用层的门——这条门防的是「通过控制台之外的写入路径悄悄扩大放行面」，不是防有库权限的人。 |
| 2026-08-29-dotnet-tests-not-run-on-feature-branches | medium | 2026-08-29 | 功能分支上没有任何流水线跑 .NET 用例：分支镜像那条只构建五个镜像（连测试工程都不编译），跑 `dotnet test` 的那条只在主干与 develop 的 push/PR 上自动触发。于是分支上新增的守卫要等开 PR 才第一次真跑 | 在功能分支上加了 C# 守卫、或改动会让既有用例失效时 | open | 代价这次就撞上了：本分支早先把网关会话默认时长从 7 天改成一个月，两条逐字钉着 7 天的用例从那时起就是红的，没人知道——直到手动 dispatch 了一次完整 CI 才暴露。手动 dispatch（workflow_dispatch）是可用的补救，本次即用它跑完全量并修掉那两条，但它靠人记得跑，不是守卫。补法：给分支镜像那条流水线加一个只跑非集成用例的 job，或让跑测试的那条也在功能分支 push 时触发；两者都要把测试工程纳入编译，否则守卫连编译错误都发现不了（这次名录镜像的两处编译错误正是镜像构建先红才暴露的） |
| 2026-08-25-llmgw-web-acceptance-not-in-ci | medium | 2026-08-25 | 两条 llmgw/web 验收脚本（`e2e/llmgw-service-key-appcaller.mjs` 接入密钥生成链路、`e2e/llmgw-quickstart-states.mjs` Quickstart 五状态与失败归因）只能手动跑：它们要先 `pnpm --dir llmgw/web build` 出 dist 再起 Playwright，而 CI 里根本没有构建 llmgw/web 的 job（只有 branch-image 的 Docker 构建），同目录的 `llmgw-layout-drift.mjs` 也是同样处境 | 下次有人改 ServiceKeysPage 的生成逻辑、QuickstartPage 的主线结构或失败归因映射时 | open | 两条落地时都跑过红绿闭环（接入密钥：改坏段内归一化 → 2 条红、去掉登记接线 → 4 条红；Quickstart：结果块挪回地址之后 → 3 条红、三环判定改成全绿 → 2 条红；2026-08-26 两屏改版后重跑红绿闭环 → 预算改成单边提交 1 条红、成功态去掉 requestId 深链 1 条红、负责人默认取成员列表第一行 3 条红），但没接进 CI 就等于没有回归守卫（`predicate-and-wiring-discipline.md` 形状 7）。不在本次展开的理由：要新开一个 llmgw/web 构建 + Playwright 的 CI job，属基础设施改动，按 §5.5 判 B 类。补法：加一个 `llmgw-web` job（build → 跑 layout-drift 与这两条），并把 `llmgw/web/**`、`e2e/llmgw-*.mjs` 一起登记进它的 path filter |
| 2026-08-25-mds-exchange-write-still-open-for-ops | medium | 2026-08-25 | MAP 的 `api/mds` 写接口已整体退场（`MdsWriteRetiredFilter` 一律 410），唯独 `PUT api/mds/exchanges/{id}` 放行——线上豆包 ASR 中继密钥的轮换脚本靠它工作，没有 UI 入口也没有网关侧等价路径 | 中继（Exchange）配置迁到网关控制台、该脚本改打网关端点之后 | open | 这是退场留的唯一写口子，不是设计。守卫 `MdsWriteRetiredGuardTests.EveryMdsWriteEndpointIsCoveredByTheGate` 钉住豁免表长度必须恒为 1，多一条就红。退场顺序：① 网关补中继密钥轮换端点 ② 脚本改打网关 ③ 删豁免并同步守卫 |
| 2026-08-25-map-legacy-model-collections-still-fallback | medium | 2026-08-25 | `ModelResolver` 仍保留「网关自有集合优先、MAP 集合兜底」两条路径（`llmgw_platforms`/`llmgw_models`/`llmgw_model_pools` → `llmplatforms`/`llmmodels`/`model_groups`）。管理面已经只剩网关一处，但**数据面**还是两套 | MAP 侧三个旧集合确认无存量被引用之后 | open | 本次只封了写入口，没动读路径——读路径是 LlmGateway 运行时解析的一部分，误删会让所有模型调用失去兜底。补法：先在网关侧把存量池与成员补齐（控制台「补齐缺失的池与成员」），确认一个发布周期内 MAP 兜底分支零命中，再删兜底与旧集合 |
| 2026-08-25-managed-default-pool-derived-members | low | 2026-08-25 | 平台托管默认池（ManagedByRegistry + AppendOnly）的成员是按用途派生的，删源模型时由 `PruneManagedPoolMembersAsync` 同步摘除；注册表下次收敛会重新收进现存模型，但「摘除」与「收敛」之间没有事务 | 某个用途的默认池在退役上游期间被并发调用 | open | 触发面窄（退役上游是低频运维动作，且被摘的成员本就解析不到）。补法：把 prune 与 delete 放进同一个 Mongo 事务，或让注册表收敛改为幂等重算而不是增量追加 |
| 2026-08-26-map-legacy-default-pool-was-deletable | medium | 2026-08-26 | 已修，留档：本轮清理里「对话主池」被删时，删除阻挡清单报的是「不是当前默认」，而那个池自身标着默认。判据缺 TenantId 就早退，MAP 遗留池永远命不中默认保护 | 回顾这次为什么会误删、或再有人改动默认池判据时 | paid | 该池确实冗余（真正生效的是 262 成员的托管池），删掉没造成损失，但守卫当时对操作者撒了谎。已改为无租户时落到自身的 IsDefaultForType，并加守卫钉住早退里不许再出现 tenantId 判空 |
| 2026-08-27-set-default-retires-map-legacy-default | low | 2026-08-27 | 把某个类型的默认位交给网关池时，会顺手清掉 MAP 遗留集合 `model_groups` 里同类型的 `IsDefaultForType`。这是那条池此前唯一的退役路径（遗留侧没有任何写入口，标记一旦为真就再也清不掉，池永远删不掉）。连带后果：`ModelResolver` 第三步「回退默认池」只读 `model_groups`，清零后既未进网关 appCaller 注册表、又无专属绑定的调用方不再命中这个遗留池，按既定优先级降到第四步的传统配置（`IsMain` / `IsIntent` / ...） | 有人换了某类型的默认池，之后发现某个老调用方的兜底换了一档时 | open | 这是有意的语义：在此之前控制台说默认是 A、解析器却仍在给未注册调用方发 B，那才是真正的不一致。真正的收尾是把兜底本身从 `model_groups` 迁走（见 2026-08-25-map-legacy-model-collections-still-fallback），迁完这条自动消失。守卫 `接管默认位必须同时退役MAP遗留默认位` 钉住这段迁移路径与判据两头 |
| 2026-08-26-resolvability-ignores-disabled | low | 2026-08-26 | 判「这个池成员还指得到东西吗」有两套口径：池列表的可解析索引把**停用**的上游和模型也算作指不到；而悬空成员判定只看存在不存在、不看停用 | 有人把某个上游临时停用，而它的模型在托管默认池里时 | open | 后果是那种成员会被标成不可用，但托管池又不许手工摘——和本分支要解的死锁同形，只剩「停用」这一档没解。要先定产品语义：停用到底算不算不可用（停用通常是临时的，不该等同于删除）。定完两处共用同一口径，并加守卫钉住。2026-08-26 降级为 low：口径分歧还在，但它最伤人的那个出口已经堵上——不可用归因拆成 missing / disabled 四值，摘除入口只对两个 missing 开放，被停用的成员不会再长出一个点了必然 409 的按钮，文案也不再把「停用」说成「已不存在」。2026-08-26 二次收口后影响进一步缩小：能不能摘除已改由后端下发的 removable 决定（与删除端点同一判据），归因枚举退化成纯文案，即使口径分歧仍在，最坏也只是措辞不准，不会再产生点不动的按钮 |
| 2026-08-26-gateway-console-has-no-behavior-tests | medium | 2026-08-26 | 网关控制台的模型池删除、遗留清扫、成员归一这些判定全部只有源码字面量守卫，没有一条真正调用它们的行为测试；console-api 也没有被任何测试工程编译 | 下次要改这几个 handler 的逻辑时 | open | 字面量守卫的已知短板：改个局部变量名就红（假阳），保留字面量而改坏逻辑则不红（假阴）。它目前唯一坐实的价值是「整段分支被删会红」。补法：把这些判定抽成不依赖 HttpContext 的纯函数，link 进 PrdAgent.Tests（该工程已有 link 现成源码的先例），至少给「先算引用再删」「只删同源」「归一只改展示不回写」各补一条真调用的用例 |
| 2026-08-26-pool-delete-needs-two-passes-when-shadowed | medium | 2026-08-26 | 同一个池 id 在网关自有集合与 MAP 遗留集合里各有一份时，列表只显示网关那份；删掉之后 MAP 那份浮上来，行「删了又回来」，要再点一次才真没了 | 清理任何一个从 MAP 认领过来的池时 | open | 本次清三个空壳专用池时实测：第一遍 200、列表仍在，第二遍才消失（第二遍走的是 MAP 分支）。两次删除都成功、都有审计，不是失败重试，但对人来说就是「点了没反应」。补法：删除时把两侧同 id 的文档一起清掉，或在响应里告诉调用方「还有一份来自兼容层，需要再确认一次」 |
| 2026-08-30-workflow-curl-body-not-shell-quoted | low | 2026-08-30 | 工作流编排页复制出去的 cURL 把请求体与 Cookie 直接塞进 `-d '…'` 与 `-H '…'` 的单引号里，两者都来自用户粘贴。带一个撇号就会让参数提前收尾，后半句被 shell 当命令跑 | 有人把那一页复制出来的命令粘进终端时 | open | 与网关 Quickstart 那处同形，本轮已在网关侧修好（关引号-给字面撇号-开引号）。这条在 prd-admin，不属于本 PR 目标，按 AGENTS.md §5.5 记账不在当前 PR 展开。补法：把同一个转义函数下沉成共享工具，两页都走它 |
...
| 2026-08-13-provider-onboarding-no-design-section | low | 2026-08-13 | 上游 Provider 接入体验（18 个内置预设、测试连接、查看模型/批量导入、自动带入价格、探测安全边界）没有对应的 `design.*` 章节；仅 [plan.platform.llm-gateway.external-platform.md](./plan.platform.llm-gateway.external-platform.md) §9.6.7 提到"新增 endpoint 只要供应方、地址/协议预设和密钥"这一句最小化表述，测试连接、模型发现/导入、价格获取、探测安全的具体设计都没有写进 design 文档 | entropy-cleanup PR #1367 review（Codex）指出：D6 若把交付该功能的 changelog 标记为已处理，会让后续扫描永久跳过这个真实的设计文档缺口 | paid | 2026-08-14 entropy-cleanup PR #1372：已在 [design.platform.llm-gateway.md](./design.platform.llm-gateway.md) 新增「十四、Provider 接入体验」章节，覆盖预设注册表、测试连接协议（含耗时口径）、模型发现与导入、探测安全边界（含内部租户豁免）；对应 changelog `2026-08-11_网关上游接入最小输入.md` 已登记进 `changelogs/.entropy-manifest.yml` |
...
## 上游/模型/团队/成员/租户删除治理（2026-08-11，changelog 碎片 `2026-08-11_recording-quality-and-llmgw-deletion.md` 已归档进 `CHANGELOG.md`）

上游平台、模型、模型池、逻辑模型、AppCaller、交换所、团队、成员、租户全部补齐删除端点，删除前统一查引用并分类报阻挡原因（拒绝留下解析不到的池成员）；控制台补齐对应删除入口，团队 chip 补上重命名入口（后端端点此前一直在，前端此前无入口）。平台列表展示头尾打码的密钥指纹，同名同 URL 的两条上游终于分得清是哪一把；密文解不开时状态如实显示「解不开」，不再伪装成已配置。日志支持按上游平台 ID 精确过滤，平台行新增「查看日志」深链。

破坏性操作统一改走站内确认弹窗（`createPortal` + ESC + 点蒙版 + 主题 token + 要求逐字输入才放行），替换全部 28 处原生 `window.confirm`/`prompt`——原生弹窗不受主题控制、移动端会截断、自动化测试也无法覆盖它。

已知边界（本轮遗留，未在本 PR 修）：删除的查-删时间窗（并发新建引用会留下残留）；改类型/改名等归一判据多轮修过跨模型池、跨协议、offering 孤儿等具体坑，详见该 changelog 逐条记录。
