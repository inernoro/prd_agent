...
|----|--------|---------|------|---------|------|------|
| 2026-08-25-llmgw-web-acceptance-not-in-ci | medium | 2026-08-25 | 两条 llmgw/web 验收脚本（`e2e/llmgw-service-key-appcaller.mjs` 接入密钥生成链路、`e2e/llmgw-quickstart-states.mjs` Quickstart 五状态与失败归因）只能手动跑：它们要先 `pnpm --dir llmgw/web build` 出 dist 再起 Playwright，而 CI 里根本没有构建 llmgw/web 的 job（只有 branch-image 的 Docker 构建），同目录的 `llmgw-layout-drift.mjs` 也是同样处境 | 下次有人改 ServiceKeysPage 的生成逻辑、QuickstartPage 的主线结构或失败归因映射时 | open | 两条落地时都跑过红绿闭环（接入密钥：改坏段内归一化 → 2 条红、去掉登记接线 → 4 条红；Quickstart：结果块挪回地址之后 → 3 条红、三环判定改成全绿 → 2 条红），但没接进 CI 就等于没有回归守卫（`predicate-and-wiring-discipline.md` 形状 7）。不在本次展开的理由：要新开一个 llmgw/web 构建 + Playwright 的 CI job，属基础设施改动，按 §5.5 判 B 类。补法：加一个 `llmgw-web` job（build → 跑 layout-drift 与这两条），并把 `llmgw/web/**`、`e2e/llmgw-*.mjs` 一起登记进它的 path filter |
