...
|----|--------|---------|------|---------|------|------|
| 2026-08-25-mds-exchange-write-still-open-for-ops | medium | 2026-08-25 | MAP 的 `api/mds` 写接口已整体退场（`MdsWriteRetiredFilter` 一律 410），唯独 `PUT api/mds/exchanges/{id}` 放行——`scripts/llmgw-prod-asr-credential-rotate.py` 靠它轮换线上豆包 ASR 中继密钥，没有 UI 入口也没有网关侧等价路径 | 中继（Exchange）配置迁到网关控制台、该脚本改打网关端点之后 | open | 这是退场留的唯一写口子，不是设计。守卫 `MdsWriteRetiredGuardTests.EveryMdsWriteEndpointIsCoveredByTheGate` 钉住豁免表长度必须恒为 1，多一条就红。退场顺序：① 网关补中继密钥轮换端点 ② 脚本改打网关 ③ 删豁免并同步守卫 |
