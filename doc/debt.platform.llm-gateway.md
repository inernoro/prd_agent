...
|----|--------|---------|------|---------|------|------|
| 2026-08-02-run-timeline-anonymous-share | medium | 2026-08-02 | 任务诊断时间线只做了登录态页面：按运行 ID 把一次异步任务的全部上游调用（提交/轮询/下载）排成一条链，能看出卡在哪一步、重试几次、总共等多久。原设想的「可直接发给用户或贴进工单的免登录链接」未做——网关服务内不存在任何分享 token 或匿名只读路由，匿名放行只用在健康检查、登录、SSO 与改密四处 | 需要把诊断结果发给不持有网关账号的人（客服转交、工单附证）时 | open | 这是新建不是复用：要在一个把租户隔离焊在团队过滤与必需上下文上的服务里凿出第一条匿名读取通道，写错的后果是跨租户日志越权且页面照常渲染、功能层面完全看不出来。且即便不含 prompt，列表映射仍会外泄模型、供应商、调用方标识、服务 key、客户端、环境、估算成本与上游错误原文，哪些字段允许出现在免登录链接里需产品与安全先拍板 |
| 2026-08-02-run-timeline-query-cost | medium | 2026-08-02 | 时间线按运行 ID 查询，但日志集合上没有该字段的索引：网关侧索引前缀是租户+团队+服务key+客户端+环境+开始时间，另一侧虽定义了若干请求标识索引却不含运行 ID 且建索引已停用。查询会退化成租户前缀 + 90 天时间范围扫描 | 日志量增长后时间线页明显变慢，或该端点被高频调用时 | open | 刻意没有顺手加索引：仓库规则规定启动时禁止自动建索引、索引需求交 DBA 手动执行，而网关控制台自身在启动时建索引与该规则存在口径冲突。该冲突怎么收口需先拍板，不应由一次功能改动扩大。另：步骤上限 500 是拍的不是实测，超限时前端没有「已截断」提示，真实分位耗时与步数积累后应一并修正 |
