...
| 2026-09-17-bind-active-app-callers-still-writes-pools | medium | 2026-09-17 | 配置权威那条「给 active 调用方绑池」的写端点还活着，它按池的可用性把 `ModelPoolId` / `AllowedModelPoolIds` / `DefaultModelPoolId` 写回调用方文档。而解析器已经不读这几个字段了——跑一次它，结果是给一批调用方盖上「残留池绑定」的戳，解析器每次请求都会为此打一条告警，路由一点没变 | 有人为了修「调用方没人接得住」去跑这条端点时 | open | 第 71 轮又把**界面**这一侧扳正了：总览页那个「把 active 调用方绑到池上」的按钮已删（点了写池绑定、解析器一个字段不读，点完显示成功而闸门照红——一个假装能修的按钮），换成指向对外模型页；端点保留给存量脚本，但它的回执现在明说「这不会改变路由」并给出真正的修法。剩下没做的只有「删端点」或「改成写认领」。此前（PR #1546）只把**读**这一侧扳正了：配置权威报告与发布闸都改判「有没有对外模型接得住」，不再看池绑定。写这一侧没动——删端点要确认没有运维脚本在调它，改语义（改成给调用方找一个对外模型来认领）是新行为，两者都越过本 PR 的范围基线（AGENTS.md §5.5 的 B 类）。判据：这条端点要么下线并在响应里指路到模型页，要么改成写认领关系而不是写池绑定 |
...
| 2026-09-17-models-list-uses-one-caller | medium | 2026-09-17 | 对外模型清单按**一个**调用方出清单：请求没带调用方头时按协议推一个默认（OpenAI 那条推的是 chat），而一把授权了多个调用方的 key 拿到的就只是那一个调用方的清单——它能调的生图、视觉等模型不在里面；那把 key 若压根不含 chat 这个调用方，这个清单请求会被整个拒掉 | 给一把 key 授权多个调用方并让对方 `client.models.list()` 时 | open | 修法是按这把 key 授权的**全部**调用方出清单，而不是一个协议默认。它要动三处：鉴权结果带上 key 的调用方全集、清单构造函数从单个调用方改成接收一个集合、以及这个发现型端点的鉴权不再要求某一个具体调用方。跨鉴权中间件与清单构造两层、且引入「一个清单面向多个调用方」这个新语义，越过本 PR 的范围基线（AGENTS.md §5.5 的 B 类）。判据：一把授权了 chat 与 generation 的 key 调 `GET /v1/models`，两类模型都在返回里 |
| 2026-09-17-pool-migration-has-no-console-entry | medium | 2026-09-17 | 把旧模型池搬成对外模型只有接口 `POST /gw/pools/migrate-to-models`，控制台没有任何入口：模型池页已经下线并重定向到对外模型页，前端也没有任何地方调这个接口。于是「这个池还没搬过」的几处拒绝只能让人去改选具体模型，真要留着那个池就得有人手动打接口 | 有人想继续用某个尚未搬迁的旧模型池时 | open | 本轮先把文案改对：主路径给「改选指定模型」（当场能走通），并如实说明搬迁目前只有接口、没有控制台入口——原先那两句指向的是一个已经下线的页面。补入口要做的是一屏能看懂的搬迁向导（先试运行看报告、再确认执行、逐条展示跳过原因），那是新界面，越过本 PR 的范围基线。判据：控制台上点得到一次搬迁，并能看到它跳过了哪几个成员、为什么 |
...
| 2026-09-17-offering-identity-index-blind-to-fallback | medium | 2026-09-17 | 线路身份唯一索引 `uniq_llmgw_offering_tenant_logical_target_v3` 键的是 `UpstreamModelId` 的**原值**，而运行时在它为空时会回落到目标的名字（兑换所取主别名、物理模型取模型名）。于是「没写」的那条与「显式写了同名」的那条，运行时是同一个上游、索引却是两个身份——数据库这一层看不穿回落 | 两个管理员同时给同一个对外模型挂同一个上游、一个写了名字一个没写时 | open | 本轮（PR #1546 第 66 轮 review）把三处**控制面**判重收敛成 `OfferingIdentityPolicy.SameUpstreamFilter`，按运行时实际打出去的那个名字比，顺序操作已经拦得住；并发那一格仍由索引兜底，而索引看不穿回落。要让索引也看得穿，只能把回落后的名字固化进 `UpstreamModelId`——那会把「跟着目标走」变成「建那一刻钉死」，目标改名后线路指着旧名字，是另一种语义，越过本 PR 的范围基线（AGENTS.md §5.5 的 B 类）。判据：两个并发请求，一个不写 UpstreamModelId、一个写上目标的当前名字，只有一条能建成 |
| 2026-09-17-layering-provisioning-probe-compares-raw | low | 2026-09-17 | 图片分层能力的安装状态探针按 `UpstreamModelId` 的原值查那条线路。若有人手工建了一条不写 UpstreamModelId、而兑换所主别名正好就是分层模型名的线路，探针查不到它，状态会报成未安装 | 手工建分层线路且没填上游模型标识时 | open | 这一处是**状态探针**不是判重闸，放宽它会改变「已安装」的判定口径（可能把一条并非为分层建的线路认成分层线路），风险大于收益，所以第 66 轮收敛三处判重时刻意没动它。判据：探针与 `ImageLayeringCapabilityRules.IsInstalled` 用同一份身份判据，且改完之后「手工建的等价线路」也报已安装 |
| 2026-09-17-imagegen-contract-untranslatable-not-surfaced | low | 2026-09-17 | 一条翻不过去的生图契约（例如参数改名的键只差大小写，而运行时那张表不分大小写）现在会被同步器逐条跳过并写进日志，其余契约照常生效；但控制台那一屏看不到它——只看得到装上的条数比配置的少，看不到少了哪一条、为什么 | 库里躺着一条在写入闸上线之前写进去的坏契约时 | open | 写入侧已经当场拒（控制面比运行时严一档），所以新的造不出来；这条债只对存量成立。补法是给同步状态加一个与「按租户跳过」分开的计数与原因清单（成因要分得开，external-cause-first），那是新的状态字段 + 新的一屏展示，越过本 PR 的范围基线。判据：控制台能指着那条契约说「它没装上，因为改名键重复」 |
| 2026-09-17-unregistered-vendor-prefix-no-longer-merges | low | 2026-09-17 | 公开模型名改成「只在名录认得出来时才剥厂商前缀」之后，名录**外**的模型不再自动合并：同一个模型从官网导进来是 `foo-9`、从中转导进来是 `relay/foo-9`，白名单里是两条 | 导入一批名录外模型、又想让多来源合成一条时 | open | 这是刻意换来的：上一版无条件剥前缀，于是管理员显式导入的 `private-provider/gpt-4o` 会被算成公开名 `gpt-4o`，直接挂到已存在的那条 `gpt-4o` 底下，普通流量可能落到一个毫不相干的上游（第 70 轮 review）。名录内的写法照常合并（`openai/gpt-4o` → `gpt-4o`），名录外要合并的补法**已经就位**：去模型名录把那个模型补登一条、把带前缀的写法登记成它的别名，之后两种写法立刻收敛。判据：补登 `foo-9` 并登记别名 `relay/foo-9` 之后，两种写法导进来落到同一个公开模型 |
| 2026-09-17-existing-logical-model-capabilities-never-revised | medium | 2026-09-17 | 补登名录之后重新导入，物理模型那份猜错的用途现在会被纠正（第 72 轮），但**已经存在的对外模型**的能力集不会跟着改：发布那一步只在对外模型不存在时按物理能力建它，已存在就只多挂一条线路。而路由判据（`SupportsAppCallerScenario`）读的是对外模型那一份，所以对早就发布过的模型，名录纠正仍然改不动路由 | 给一个早就导入并发布过的模型补登名录、想纠正它的用途时 | open | 没有顺手改，是因为对外模型的能力集**可能来自多个物理来源**（一个对外模型允许挂多条线路、多个 Provider）。一个来源的纠正该覆盖整份、还是只并进去、还是只覆盖它自己贡献的那几条——这是一条新语义，没有现成答案，越过本 PR 的范围基线（AGENTS.md §5.5 的 B 类）。能确定的边界：只有一条线路且指向这个物理模型时，覆盖是无歧义的，可以先做这一档。判据：给一个已发布模型补登名录并重新导入，它的对外模型能力集跟着变，且多来源的那种不会丢掉别的来源贡献的能力 |
| 2026-09-17-call-trace-halfopen-cooldown-mirrored | low | 2026-09-17 | 调用全貌那句「有 N 条已摘掉的线路过了冷却期，下一条请求可能先拿其中一条做半开试探」，冷却秒数用的是 console-api 里镜像的默认值 120 秒（`CallTracePlanner.HalfOpenAfterSecondsDefault`）；运行时那边可由 `LlmGateway:CircuitBreaker:HalfOpenAfterSeconds` 改，而 console-api 是独立进程、读不到 serving 那份配置。改过这个配置的部署，面板报出的**条数**会偏多或偏少 | 有人把熔断冷却时长改离 120 秒之后看调用全貌时 | open | 刻意只镜像默认值：这一句本来就只说「可能」、不指名道姓说一定是谁，条数这一档的误差不会把结论说反（说反的是原先那个「不提半开、指着队首说下一跳是它」的版本）。真要对齐，正解是让 serving 的自检端点把生效中的冷却秒数暴露出来、console-api 去读，而不是在 console-api 这边再加一份同名配置——后者是判据分裂成两份各自漂移（`predicate-and-wiring-discipline` 形状 3）。判据：把 serving 的冷却改成 600 秒之后，面板那句里的条数与运行时真正会试探的那几条一致 |
| 2026-09-17-claim-unique-index-still-case-sensitive | medium | 2026-09-17 | 认领的比较已经在三处收齐成 appCaller 身份规则（不分大小写）：运行时那一句、控制面四处查询、面板的归属显示。**只剩库级那道唯一索引还是按字节的**——索引清单脚本里那条 `uniq_llmgw_logical_claim_per_type` 没有 collation，所以两个模型分别认领 `Foo` 与 `foo` 时，索引不认为它们撞车 | 两个管理员并发保存、双双绕过控制面那道「已经被谁认领了」的检查时 | open | 控制面现在会拒（查询带 collation），所以这条只剩并发竞态那一档；真撞上了运行时也不会乱指——认领查询按 DisplayOrder/PublicId 排序取第一个，取值确定。不顺手改是因为**给已存在的索引换 collation 不能原地改**：必须新名字建好、核对通过、再丢旧的，也就是第 73 轮给线路身份索引做的那套 v3 迁移。而 `ensureTightenedUniqueIndex` 的 `sameIndexDefinition` 目前不比 collation，直接往 options 里加一行是**静默空转**——已存在同名索引的部署一行都不会变，而脚本会报成功（形状 8：拿一份不成立的证据当证明）。还债的样子：照 v3 那套加 `uniq_llmgw_logical_claim_per_type_ci`（带 collation），核对建成且唯一之后再丢旧的；同时给 `sameIndexDefinition` 补上 collation 比较，否则下一个想换 collation 的人还会踩同一个空转。判据：一个模型认领 `Foo`、另一个认领 `foo` 时，第二次写入被库拒掉 |
| 2026-09-17-default-switch-not-atomic | medium | 2026-09-17 | 管理员把某个用途的默认从 A 改成 B 时，控制台先把 A 的 `IsDefaultForType` 清掉、再把 B 置上，两次写之间有一个**没有任何默认**的窗口（一次库往返）。落在这个窗口里的「不点名、也没人认领」的请求会解析失败（`no-logical-model`）。现有的回滚账本只管最终写入失败那一档，护不住这个「两步都成功」的过渡态 | 线上有流量时改某个用途的默认指向 | open | **换顺序解决不了**：库里那条 `uniq_llmgw_logical_default_per_type`（同租户同用途唯一、只覆盖 `IsDefaultForType:true` 的部分索引）明确禁止「同时有两个默认」这个重叠态，所以先置新再清旧会直接撞唯一键。也就是说，只要那条不变量还在，切换就必然经过零默认的一瞬。真正的两条出路：① 把这两次写放进一个 Mongo 事务（要求网关库是副本集，本仓库至今没有对部署形态做过这个要求，属于新前提）；② 改成单文档指针——另建一行「这个用途的默认是谁」，切换就是那一行的一次原子更新，而 `IsDefaultForType` 降级成它的派生展示字段。后者更稳但要动读侧（运行时第二层查询、面板、搬迁、控制台四处）与一次数据迁移，属 AGENTS.md §5.5 的 B 类，越过本 PR 的范围基线。判据：持续打不点名请求的同时切换默认，一条都不失败 |
...
| `2026-09-24-federation-deadline-propagation` | P1 | open | LLMGW 把另一台 LLMGW 当上游时，外层请求在 120 秒超时后会正确返回 504，且 `ProviderId=llmgw` 已禁用外层重试；但内层 serving 遵循 server-authority 规则，不因客户端断开取消任务，也没有收到外层剩余截止时间。实测 requestId `federation-keepalive-formal-08` 已到达 CDS Local 并调用硅基流动，正式端 120 秒记 504 后，本地记录仍保持 `running`，直到操作者重启该预览 serving 并按 499 审计收口。连续超时会在内层自身超时前占用并发槽。<br>**收口条件**：联邦协议增加可验证的绝对 deadline 或剩余预算，内层把自身上游超时限制在该预算内；或者由可信网关身份调用 request cancel，且断连、超时、重复取消都有集成测试。不能把普通浏览器断开直接等价为取消，也不能恢复联邦外层重试。 |
