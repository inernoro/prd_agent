...
| 2026-08-25-orphan-specialty-pools-pending-cleanup | low | 2026-08-26 | 三个专用池（ASR 豆包 BigModel、ASR 豆包 Stream、视频 Seedance 2.0 Fast）各自只有一个成员，而它们挂的上游都已随模型管理退场被删掉；三者都没有任何应用绑定，一次请求都发不出去 | 下次有人在网关控制台看到这三个空壳池时 | open | 已核实：三者绑定数为 0，成员模型在网关侧与 MAP 侧都查不到。删除动作在本会话被安全策略拦下，尚未执行，需人工在控制台点删除。健康统计已修成「不可用」，不会再显示「健康」 |
| 2026-08-25-duplicate-chat-default-pool | low | 2026-08-26 | 手工建的「对话主池（OpenAI + OpenRouter）」（MAP 权威、5 个成员）与平台托管的「对话默认池」（262 个成员，全部落在 OpenAI / OpenRouter）同时被标成默认，控制台上出现两个「默认」 | 有人按控制台的「默认」标记去判断到底哪个池在服务时 | open | 运行时真正生效的是池类型表里登记的那个，也就是 262 成员的托管池——它自动跟随两个上游的模型目录，不需要人手维护。手工池无任何绑定，且含一个上游并不存在的成员，已属冗余。删除动作同样被安全策略拦下未执行 |
