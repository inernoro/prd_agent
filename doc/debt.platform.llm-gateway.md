...
|----|--------|---------|------|---------|------|------|
| 2026-08-12-delete-import-race | medium | 2026-08-12 | 删除 Provider 与批量导入并发时存在 TOCTOU 窗口：导入端点先读到 Provider 还在，删除端点随后拍下模型与池引用快照并执行级联，导入再把模型插进去——结果 Provider 没了、留下一个启用态的孤儿模型，而池同步会静默排除它，导入那侧还报成功 | 同一个 Provider 上删除与导入真正并发时 | open | PR #1362 review 第四轮提出，判断成立。修它要引入 fencing（把 Provider 原子标记为 deleting 并让导入端拒绝）或多文档事务，而本部署是 standalone Mongo 无多文档事务（同库已有 `2026-07-14-tenant-provision-crash-consistency` 记着同一类约束）——属新语义类别，按范围熔断规则记账不在当前 PR 展开。当前风险边界：两个操作都需要 ConfigWrite 权限且由同一批管理员手动触发，不是自动化高频路径；真撞上时的残留是一个孤儿模型文档，可由删除端点再跑一次清掉。补法：给 Provider 加 `DeletingAt` 租约字段，导入端点开头校验，删除端点先原子置位再拍快照 |
