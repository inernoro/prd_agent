...
| 2026-08-28-model-catalog-coverage-and-enforcement-point | medium | 2026-08-28 | 内置模型名录只覆盖常见的那批（OpenAI / Anthropic / Google / DeepSeek / 通义 / 智谱 / Kimi / 豆包，约 38 条），其余一律算名录外、要管理员显式放行。两道门都已就位（导入入库 + 数据面解析），仍欠的是覆盖面本身，以及主站进程内旧路径解析出的模型这道门管不到 | 接了名录未覆盖的上游，或有人绕过控制台直接往库里写模型文档时 | open | 第一道门在导入入库：用户在那一刻就知道被拦了、为什么、怎么放行。2026-08-29 补上第二道门（数据面）：拦截点罩在模型解析器的出口上，一个函数罩住全部分支而不是逐个 return 前判一次，名录按仓库既有的镜像 + 守卫模式下沉到运行时侧（同能力契约那对镜像表）。配套的三件事都做了：三条写入路径都盖放行标记、名录门上线时把存量一次性补成已放行（如实记成未经人工审阅）、`LlmGateway:ModelCatalogGate=observe` 作为降档逃生阀。**仍然管不到的一处**：主站进程内旧路径（回滚保险）解析出的模型来自主站自己的模型集合、不在网关模型库里，名录外的会被这道门拦下且没有地方盖放行标记——真要走那条路径得先把门降到 observe。覆盖面是有意的起点而不是终点：名录只登记确实知道的事实，宁可判成名录外让人显式放行，也不猜一把当事实（猜正是这次要修掉的病根）。扩充方式是往名录加条目，不是往关键词表加词。2026-08-29 线上核过一轮：预览环境 319 条存量模型在名录门上线时已被一次性补上放行标记，名录内模型真实调用通（3 秒拿到回复），名录外但已盖戳的模型也照常打到上游（回的是上游 404，不是名录门的拒绝），即门没有误伤。「拦得住」那一半线上验不了——每条写入路径都会盖戳，正常操作造不出没戳的行——改由真 Mongo 的行为用例覆盖，三条断言互为对照（名录内放行 / 名录外无戳拦下并点名错误码 / 同一条补戳后放行） |
