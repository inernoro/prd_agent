...
|----|--------|---------|------|---------|------|------|
| 2026-09-08-default-generation-member-flapping | medium | 2026-09-08 | 图片生成默认池（`pool-default:tenant_map_internal:generation`）有两个成员，`chatgpt-image-latest` 健康态在抖：连打 14 次真实网关解析，12 次落到健康的 `gpt-image-1`，2 次落到它且 `healthStatus=Unavailable`。命中时出图请求会失败，而调用方（对话出图工具、视觉创作默认路径）走的正是「不指定模型、由默认池兜底」 | 池内调度恰好选中该成员时（约 1/7） | open | 来源 #1338 关闭时的运维观察。不是池缺失，是成员健康。补法二选一：在网关控制台把该成员从默认池停用；或查它对应 Offering 的上游状态（401 / 限流）修好上游。修完用 `GET /api/open-platform/app-callers/resolve-model?appCallerCode=chat-agent.image-gen::generation&modelType=generation` 连打 20 次，`healthStatus` 全 Healthy 即可结清 |
