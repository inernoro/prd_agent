...
| 2026-08-28-model-catalog-coverage-and-enforcement-point | medium | 2026-08-28 | 内置模型名录只覆盖常见的那批（OpenAI / Anthropic / Google / DeepSeek / 通义 / 智谱 / Kimi / 豆包，约 38 条），其余一律算名录外、要管理员显式放行；且白名单目前只拦在**导入入库**这一步，serving 侧没有第二道判据 | 接了名录未覆盖的上游，或有人绕过控制台直接往库里写模型文档时 | open | 覆盖面是有意的起点而不是终点——名录只登记确实知道的事实，宁可判成名录外让人显式放行，也不猜一把当事实（猜正是这次要修掉的病根）。扩充方式是往名录加条目，不是往关键词表加词。拦截点选在入库是因为请求只会打到池成员，进不了库就进不了池，且用户在导入那一刻就知道被拦了、为什么、怎么放行；代价是绕过控制台直接写库的路径不受这道门管。要补的话，serving 侧解析模型时再查一次名录即可，但那需要 serving 能读到同一份名录（现在名录在 console-api 项目里），跨项目共享要么走既有的镜像 + 守卫模式（同 capability 契约那对镜像表），要么把名录下沉到共享库 |
