...
当前 open: 52 / in-progress: 8 / paid: 33 / 总计: 93
...
| 2026-08-30-catalog-gate-exchange-models-unstamped | medium | 2026-08-30 | 走兑换所（exchange）解析出来的模型不在 `llmgw_models` 里，名录门查不到它们的放行标记，所以只能按「管理员在控制台建兑换所时当场列出了这些别名」这一层人工声明放行——兑换所文档本身没有 per-model 的放行标记，也没有导入时的确认闸门。于是绕过控制台直接往兑换所的 `Models` 数组里加一个别名，两道门都拦不住 | 有人绕过控制台直接改兑换所文档，往里塞新的模型别名时 | open | 2026-08-30 名录门已把这条路径**显式**认出来（之前它静默落进「管不着」，结果一样是放行，但读代码的人会以为兑换所本来就不在门的射程内，于是永远排不上队）。要真正覆盖，得先给兑换所的写入路径盖戳：控制台新增/更新兑换所时给每个别名写放行标记与放行人，数据面再改成认那个标记而不是认「这是一条兑换所记录」。不在本 PR 做的理由同上一条——那是给兑换所补一套导入确认与迁移，属新语义类别（AGENTS.md §5.5 B 类）。当前风险有限：建兑换所需要 ConfigWrite 权限，而能直接改库的人本来就绕得过所有应用层的门 |
| 2026-08-30-tutorial-evidence-commit-dangling | medium | 2026-08-30 | 教程证据图谱里的验收提交戳指向一个仓库历史中不存在的提交，于是「截图与正文已按该提交逐项读回」这句话没有任何东西撑得住。教程漂移那道闸因此长红——它先报这一条，再顺带让五条依赖「无漂移」的用例一起变红，遮住后续真正的漂移信号 | 任何改动 llmgw 前端、控制台或教程的 PR 都会看到它 | open | 这条在主干上就是红的，不是某个 PR 引入的。**不要靠改戳修它**：把它换成一个当前存在的提交，等于替一次没做过的读回签字（本仓库 `predicate-and-wiring-discipline` 形状 8：不成立的证据不能当证据）。正解是重跑一次教程证据读回——按当前部署逐项核对图片与正文，通过后把戳写成那一次读回所在的提交，属独立一轮验收工作。在那之前这道闸的结论只有一个含义：证据戳失效，别把它读成「有漂移」 |
