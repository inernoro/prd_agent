...
| 2026-09-16-readiness-scenario-offering-shallow | medium | 2026-09-16 | 就绪探针的 `scenario-capability` 组件判「有没有可用线路」时只看线路自己的 `Enabled` 与健康状态，不看线路指向的物理模型/平台/兑换所还在不在，也不过名录门——而同一个文件里的 `router` 组件两样都判。于是一条指向已停用平台、或会被名录门拦死的线路，在 router 判红、在 scenario 判绿 | 两个就绪组件对同一份配置给出相反结论时 | open | 本轮（PR #1546）把两个组件的**租户**判据对齐了（跨租户的模型不再算数），但没有把 router 那套目标可用性 + 名录门搬进 scenario。搬的做法是把 router 里的 `OfferingTargetUsable` 与名录门过滤抽成两边共用的一份，scenario 的纯函数入口 `EvaluateScenarioCapability` 相应改成接收「已经筛过的可用线路」。判据：把一条线路指向的平台停用，两个组件必须同时判红 |
