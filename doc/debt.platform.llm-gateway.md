...
当前 open: 60 / in-progress: 8 / paid: 24 / 总计: 92
...
| 2026-08-30-intent-draft-ignores-inband-error | medium | 2026-08-30 | 一句话推导的服务端读流只挑 `delta.content`，不认 serving 在响应头之后回的失败帧（`finishReason=error` + 顶层 error）。模型若在失败前已经吐出一段可解析的 JSON，端点会对一次**失败且计费**的调用回 `ok: true` | 上游在响应头发出之后才失败，且此前已吐出可解析片段时 | open | 与 2026-08-30 已修的 Quickstart 前端那条同形，判据可直接照搬（认失败帧就中止并降级到本地关键词判定）。这条在服务端，前端那条修完并不覆盖它。因 §5.5 熔断线未在本轮展开 |
| 2026-08-30-intent-draft-no-completion-cap | medium | 2026-08-30 | 一句话推导把用户那句原样发给模型，没有设 `max_tokens`，也没有对累积的响应体积设上限；系统 appCaller 没有预算闸，其密钥每分钟允许 120 次。模型若没有自带小上限，单次可以一直生成到 40 秒 deadline | 有人拿注入式的长 intent 反复打这个端点时 | open | 期望产物只是四个字段的 JSON，给一个紧的 completion 上限 + 累积体积上限即可，两处都很小。这条是**成本面**而不是正确性面，但系统调用花的是平台的钱，值得单独一条。因 §5.5 熔断线未在本轮展开 |
| 2026-08-30-settings-model-options-bounded-page | low | 2026-08-30 | 服务网关设置的逻辑模型下拉取前 200 条且没有排序，租户启用的对话类模型超过 200 个时，已保存的那个可能不在这一页里——设置接口仍然回那个值，但下拉里没有对应选项，页面看起来像「没选模型」，而系统调用仍在用那个看不见的模型 | 单租户启用的对话类逻辑模型超过 200 个时 | open | 修法是把已保存的那条显式并进结果，或给下拉做确定性分页/搜索并保住当前选中项。触发门槛较高（200 个启用的对话模型），故记 low。因 §5.5 熔断线未在本轮展开 |
| 2026-08-30-system-appcaller-takes-over-colliding-code | high | 2026-08-30 | 系统级凭据自愈在登记自己的 appCaller 时，认的是「同租户下这个码的文档存在吗」，存在就把该码的**全部**文档搬到系统团队。搬迁本意是迁自己上一版留下的文档（那时挂在租户第一个业务团队上），但判据分不出「我自己的旧文档」和「恰好同码的业务文档」——租户若真有一条自助登记的同码 appCaller，只要有人打开这一页，它就会被搬走，其团队级密钥随后在 serving 的归属比对上被判 403 | 租户下已存在自助登记的 `llmgw-console.intent-draft::chat` 时 | open | **合并前应先修**：这是唯一一条会动到用户既有数据、且只要打开设置页就会触发的。修法与系统团队那条同形——给系统自建的 appCaller 文档盖 `SystemManaged` 标记，只迁带标记的；发现同码但不带标记的业务文档时，按名字占用那套返回说得出原因的失败（提示改码或改名），而不是默默接管。不在本轮做的原因是 PR 已过 §5.5 熔断线（Review 修复提交 11 个），停不停待仓库负责人定 |
| 2026-08-30-quickstart-member-pick-sent-as-pool | medium | 2026-08-30 | Quickstart 测试栏选中池内某个成员时，请求体带的是 `model_policy: pool` 但既没有池 id 也没有钉住成员的字段。serving 于是把模型名留在 `ExpectedModel` 里，而 appCaller 一旦绑了严格池契约，解析器会把它当成**池标识**去找，找不到就回 `ROUTE_CONFIG_INCOMPATIBLE`——选了成员反而跑不通 | 用绑了严格池契约的 appCaller 在测试栏选具体成员时 | open | 修法是发真正的池身份 + 钉成员的字段（`pinned_model_id` 一类），或按 serving 既有契约表达「在已绑池内选成员」。同样因 §5.5 熔断线未在本轮展开。注意与「auto」那条路径无关：不选成员时策略是 auto，不受影响 |
| 2026-08-30-quickstart-stream-eof-without-done | medium | 2026-08-30 | 真实调用的流在没有 `[DONE]` 帧的情况下结束时，页面仍按「收完了」处理并报成功。serving 在响应头已发出后遇到取消会直接收尾——不发错误帧也不发 `[DONE]`，于是一次被截断的付费调用在页面上看起来是成功的 | 上游或代理在响应头之后中断这次流式调用时 | open | 与 2026-08-30 已修的「流里夹着的失败帧」同一族，但判据不同：那条认的是**出现了失败帧**，这条要认的是**没有出现完成帧**。修法是客户端跟踪有没有收到 `[DONE]`，EOF 时没收到就判「未完成」而不是成功。不在本轮做的原因：PR 已触发 §5.5 熔断线（Review 修复提交超 8 个），停不停由仓库负责人定。触发概率有限——最常见的取消源是客户端自己断开，那种情况下没有人在看这一屏 |
| 2026-08-30-quickstart-vision-fallback-ignores-prompt | low | 2026-08-30 | 视觉类测试在**没有上传图片**时走内嵌的 1x1 测试图，这条分支直接返回写死了 `Describe this test image` 的内容，把用户在输入框里写的那句丢掉了。输入框显示的是用户那句，真正发出去的是写死那句 | 选视觉类、不传图、又在输入框里写了自己的提示词时 | open | 注意与 review 的描述有出入：cURL 片段与真实请求走同一个 `userContentFor`，所以**两者一致**，不一致的只是输入框那一处显示。修法是让无附件分支也用算出来的 prompt 拼内容（有附件那条分支已经这么做了）。同样因 §5.5 熔断线未在本轮展开 |
| 2026-08-30-gemini-ingress-model-only-from-route | medium | 2026-08-30 | Gemini 入口的模型只从路由段 `/v1beta/models/{model}:generateContent` 读，body 里的 `model` serving 不看（它还会被当成 dropped parameter）。于是名字带斜杠的模型（聚合型上游的 `厂商/模型`，本仓库池成员的常见形态）在 Gemini 入口上**钉不住**——路由段装不下一个带斜杠的名字 | 用 Gemini 入口测试一个 `厂商/模型` 形态的池成员时 | open | Quickstart 侧已按事实说话（2026-08-30）：钉得住就把模型写进路由段，钉不住就保持 auto 并明说「本次按池调度执行」，不再出现「路径打 auto、屏幕写实际执行 X」这种编造。**真正的修法在 serving**：要么路由段按 URL 解码后再取模型（客户端发 `%2F`），要么让 gemini 端点在路由段为 auto 时回退读 body 的 `model` 并把它从 dropped parameters 里摘掉。两条都要改网关 serving 侧的 Gemini 入口处理，而那个文件不在本 PR 首轮文件清单内，按 §5.5 范围熔断条款（文件范围扩出首轮清单即熔断）不在本轮展开。**未验证**：ASP.NET Core 对路由段里 `%2F` 的处理我没有实测，所以上面第一条修法要先验再做，别照着写 |
