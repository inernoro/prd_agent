...
| 2026-08-26-resolvability-ignores-disabled | medium | 2026-08-26 | 判「这个池成员还指得到东西吗」有两套口径：池列表的可解析索引把**停用**的上游和模型也算作指不到；而悬空成员判定只看存在不存在、不看停用 | 有人把某个上游临时停用，而它的模型在托管默认池里时 | open | 后果是那种成员会被标成不可用，但托管池又不许手工摘——和本分支要解的死锁同形，只剩「停用」这一档没解。要先定产品语义：停用到底算不算不可用（停用通常是临时的，不该等同于删除）。定完两处共用同一口径，并加守卫钉住 |
