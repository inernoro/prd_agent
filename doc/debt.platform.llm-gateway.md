...
| 2026-09-17-gateway-initializer-still-creates-indexes | medium | 2026-09-17 | 网关的数据库初始化器在启动时还会建十几批索引（走 `CreateBsonIndexesAsync` 的那些集合），与 `no-auto-index` 规则相悖：副本集滚动启动时各建各的、建失败会让进程起不来，而这两种后果都发生在没人盯着的启动路径上 | 下一次有人给网关集合加索引，或排查一次启动失败时 | open | 本轮只把**本 PR 自己引入**的那一条（线路身份唯一索引）改成了只报不建，存量的十几批没动——一次性搬迁要同时给 DBA 指南补齐这十几条的建法，并确认各部署都已手动建过，否则拔掉自动建等于让新库缺索引且没人知道。判据：初始化器里不再出现任何建索引调用，且指南里逐条可查 |
