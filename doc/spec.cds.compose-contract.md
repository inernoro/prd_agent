...
**一句话**：项目编排文件该怎么写的唯一权威：字段契约、优先级、最容易踩的七个坑与校验规则。
**谁该读**：要给项目写或改编排文件的人和 AI；排查扫描结果不对的人。
**读完能做什么**：写出一份能通过校验的编排文件，并看懂校验器为什么报错。
...
### 坑 8:`${VAR}` 只认 `:-default`,POSIX 其余三种操作符被静默漏识别

```yaml
# 旧正则只匹配这两种:
DB_HOST: ${DB_HOST}              # 裸引用
DB_PORT: ${DB_PORT:-5432}        # :- 默认值

# 这三种曾经完全不匹配(既不展开也不计入缺失,必填变量被静默遗漏):
API_KEY: ${API_KEY:?missing}     # :? 必填无默认,未设置应报错
LOG_LEVEL: ${LOG_LEVEL:=info}    # := 提供默认(同时写回)
EXTRA_FLAG: ${EXTRA_FLAG:+on}    # :+ 有值则替换为固定串,否则空串
```

**根因**:env 模板正则只覆盖 `${VAR}` 与 `${VAR:-default}`,`:?` / `:=` / `:+` 不在匹配范围内,导致这三种写法既不会被展开、也不会被计入"必填但缺失"的扫描结果。

**修复**:统一为 `group1=变量名 / group2=操作符 / group3=操作数` 的通用解析,`envTemplateDefault` 作为默认值口径 SSOT(`compose-parser.ts` 与 `env-provenance.ts` 共用):`:-` 与 `:=` 提供默认值、`:+` 无默认时兜底空串、`:?` 与裸 `${VAR}` 无默认值(未设置即视为缺失)。`:-` 对外可见行为保持逐字不变。

**怎么写不出错**:
- 四种操作符(`:-` `:?` `:=` `:+`)在 CDS env 模板里都是合法语法,不要因为不确定支持与否就退化成裸 `${VAR}` + 业务代码里再兜底
- 必填变量想要"未设置就报错"的语义,用 `${VAR:?msg}`,不要用裸 `${VAR}` 假装等价(裸引用不带错误信息)
...
- [doc/plan.cds.status.md](./plan.cds.status.md) — CDS 当前状态看板(本文是 mysql 接入 Phase 2.5 产出,完整里程碑见 §二)
