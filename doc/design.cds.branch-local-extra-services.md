...
**一句话**：项目级配置是稳定底座，分支可临时加挂额外服务做验证，改底座要走审批。
**谁该读**：要给分支临时加服务的人。
**读完能做什么**：说清临时服务的生效范围与回收方式。

---

> 进度：代码 + 单测就绪，待 CDS 灰度真机演示后转「已落地」。
...
- 额外容器由现有分支 teardown(按 `branch.services` 全部 rm)清掉;**移除单个额外服务**(extraProfiles 删掉它/置空)时,下一次部署会把它从期望清单识别为孤儿并拆容器+删条目(2026-06-29 补的对称收尾)。该孤儿拆除循环在 `containerService.remove` 前后各 `assertBranchOperationCurrent`,租约被取代时中止,不在已取消的 deploy 下改 state(Bugbot Medium)。**远端执行器**owned 分支走 `/exec/deploy`,该 handler 同样对 payload profiles 里没有的 service 主动下掉容器+删条目(Codex P2,否则 worker 上旧容器残留);
