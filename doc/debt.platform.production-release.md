...
> **版本**：v1.4 | **日期**：2026-09-08 | **状态**：部分落地

**一句话**：既有生产发布安全债务已经还清，CDS 的入口身份、双入口探针与重启完成判断仍待闭环。
**谁该读**：做发布评审的人；想追溯当初怎么修的人。
**读完能做什么**：区分已偿还的发布债务与仍会影响 CDS 可用性判断的缺口。

---

## 总览

当前 open: 3 / paid: 8 / 总计: 11

## 债务列表

| ID | 严重度 | 创建日期 | 描述 | 触发条件 | 状态 | 备注 |
|---|---|---|---|---|---|---|
| 2026-09-08-cds-primary-entry-identity | P1 | 2026-09-08 | 主入口服务不可用时，路由从健康服务重新挑选默认入口，可能将独立网关界面送到 MAP 主域；页面返回 200 仍可能是错误产品 | 主前端构建、故障或部署切换，而兄弟网页服务已运行 | open | 路由发布器和 master 的跨服务兜底已修复并部署；类型检查、全量 CDS 回归和限定独立复审通过，正常公网产品身份及入口资源已验真。仍需实际主服务故障与恢复时的公网证据，故不关闭 |
| 2026-09-08-cds-restart-confirmation | P2 | 2026-09-08 | 更新成功记录早于部署排空和进程切换，客户端又可能在事件流超时后仅凭旧进程健康响应报告已重启 | 在途部署使重启排空超过客户端事件流等待时间 | open | 区分构建完成、等待重启与新进程就绪；客户端须确认操作终态和进程代际，旧进程健康不得证明重启完成。补排空超时、旧进程仍健康和新进程确认测试 |
