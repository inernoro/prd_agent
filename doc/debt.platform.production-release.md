...
|---|---|---|---|---|---|---|
| 2026-09-08-cds-primary-entry-identity | P1 | 2026-09-08 | 主入口服务不可用时，路由从健康服务重新挑选默认入口，可能将独立网关界面送到 MAP 主域；页面返回 200 仍可能是错误产品 | 主前端构建、故障或部署切换，而兄弟网页服务已运行 | open | 主入口身份须独立于就绪状态；不可用时展示该入口的等待或错误页，不切换兄弟产品。补主服务不可用、恢复及命名网关隔离测试；公网验收同时核对产品身份和实际入口资源 |
