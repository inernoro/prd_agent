...
**一句话**：生产发布安全相关债务已全部还清，本文留作追溯与关闭条件。
...
| 2026-08-04-gateway-bind-mount-drift | 待本次发布归档 | 2026-08-04 | 生产仓库目录被替换后，长时间运行的 gateway 仍绑定旧目录 inode，宿主机 `current` 已切换但容器继续读取旧页面。发布脚本现在同时对账宿主机与容器内的静态指针、首页 hash 和 Nginx 配置 hash；发生漂移时仅从显式稳定 Compose 目录重建 gateway，重建后再次对账。公网表面探针同时要求首页入口资源包含目标 commit，杜绝 API 新而页面旧仍被判成功。 |
