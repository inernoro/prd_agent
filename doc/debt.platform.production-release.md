...
> **版本**：v1.5 | **日期**：2026-09-24 | **状态**：部分落地

**一句话**：MAP 发布的跨服务回滚、验收凭据预检和旧产物可恢复性，以及 CDS 的入口与重启判断仍有待关闭的缺口。
**谁该读**：做发布评审的人；想追溯当初怎么修的人。
**读完能做什么**：区分已偿还的发布债务与仍会影响版本一致性、恢复能力和可用性判断的缺口，按关闭条件安排后续修复。

---

## 总览

当前 open: 6 / paid: 8 / 总计: 14

## 债务列表

| ID | 严重度 | 创建日期 | 描述 | 触发条件 | 状态 | 备注 |
|---|---|---|---|---|---|---|
| 2026-09-24-map-coordinated-rollback | P1 | 2026-09-24 | 新服务镜像已启用后，强制业务验收失败仅恢复静态入口，可能留下前后端混合版本；页面恢复不等于发布验收通过 | 服务与静态资源均切换后，业务验收拒绝新版本 | open | 责任归属：发布链路维护者。关闭条件：记录并协同恢复各服务与静态资源的准确版本，增加失败阶段行为测试及真实恢复证据；独立确认公网入口、资源、API 与网关版本一致，并分别记录恢复结果与原发布失败，禁止用恢复成功覆盖门禁失败 |
| 2026-09-24-map-release-probe-credential-readiness | P1 | 2026-09-24 | 内部健康检查所用旧密钥不能代表外部发布验收调用权限，缺少专用凭据会在切换后才触发业务验收拒绝 | 发布验收以外部调用来源执行，但可用凭据仅允许内部来源 | open | 责任归属：发布链路与网关鉴权维护者。关闭条件：切换前以无副作用方式核对验收所需授权与凭据有效性，缺失时明确阻止切换；补旧密钥拒绝及合规短期凭据成功用例。不得伪装内部来源、弱化鉴权或跳过门禁；新建凭据须单独获得授权，限定范围并在用后撤销 |
| 2026-09-24-map-rollback-artifact-retention | P2 | 2026-09-24 | 已知可用版本的不可变静态包远端不可取回，恢复依赖现场仍保留且经过核对的旧静态资源，削弱灾后恢复能力 | 需要回滚的旧版本静态包已缺失，或现场缓存同时不可用 | open | 责任归属：发布产物与保留策略维护者。关闭条件：在回滚窗口内可按精确提交获取服务镜像、静态包及校验值，并完成不依赖现场唯一副本的恢复演练；已有资源复用必须验证版本与完整性，不能据此宣称远端产物可恢复 |
...
以下为历史已落地的基础条件，不替代上表各项 open 债务的关闭条件；后续若任一条件退化，必须重新登记 open：
