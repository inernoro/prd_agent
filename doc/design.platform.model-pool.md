...
## 十、目录标识一致性（2026-09-21 补充）

MAP 提交给 LLMGW 的必须始终是逻辑模型的稳定标识（PublicId），而不是给人看的展示名。旧的业务模型目录曾经把展示名当作可提交的标识下发，展示名与调度用的运行时标识本来就允许不一致（同一个池换皮改名不影响调度），于是一旦运营改了展示名，前端提交回去的值在运行时侧就找不到对应模型，整条创作入口在用户看来是"有模型可选、一提交就失败"。

修复原则：业务目录下发的每一项都必须带上运行时用来调度的稳定标识，且这个标识与默认选中项在目录读取路径和运行时路由路径上必须来自同一个权威来源，不允许两条路径各自维护一份、事后再对齐。目录的默认项与运行时的默认解析结果不一致，属于目录本身的缺陷，而不是"用户没选对"。

配套的深度自检新增了两类探针：一类验证业务模型目录返回的默认项与运行时实际解析结果一致（防止再次出现目录标识漂移）；一类对已开放的生图模型做场景路由预检（同一个模型在文生图 / 图生图 / 多图三种场景下的身份映射是否与白名单一致），让路由配置错误在用户提交前被 CDS 探针发现，而不是等用户提交后才收到"模型不受支持"的拒绝。健康探针本身改为无副作用的可执行目录检查，避免探测行为本身占用故障恢复所需的半开验证配额。

线路故障场景下，目录不因为运行时暂时不可用就整体消失：保留逻辑模型条目并下发"不可用"状态，让用户仍能看到完整目录、只是当前这一项不能用；空 AppCaller（未配置任何获准池）时目录退回到类型默认模型这一条兼容契约本身不变，只是现在这条默认值也必须经过同一套一致性校验。
