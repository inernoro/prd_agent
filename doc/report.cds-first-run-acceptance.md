...
| B01 | preview hostname 的 CDS bypass 可用 | 从预览域名访问 `/_cds/api/projects` 或同类接口 | 能拿到当前 source-project 范围内的数据，不串项目 | 失败 | 当前生产 preview 下 `/_cds/api/projects` 三个 host 都返回 `prd-agent/mdimp/mytapd` 全量；手动补 `x-cds-source-host` 后过滤正常，已定位为 forwarder 未透传 source host |
| B02 | 分支列表可发现 | 新用户进入 CDS 后找到项目分支列表 | 能看到 main 分支和状态，不出现跨项目脏数据 | 失败 | 当前生产 preview 下 `/_cds/api/branches` 返回全量分支；本地已补 forwarder header 并加回归测试 |
...
| B04 | 容器日志可用 | 查看某个 service 的日志 | 日志可读，密钥被 mask，不泄漏明文 | 未跑 | 待测 |
| B05 | 部署入口可用 | 从 UI 触发或查看 deploy | 状态能从排队 / 构建 / 运行中流转，有失败原因 | 未跑 | 待测 |
...
| E01 | 新项目 onboard 可用 | 用仓库从零导入项目 | scan 识别服务、端口、env、init SQL；不生成 ghost service | 未跑 | 可能需要下载 mdimp / mytapd 仓库 |
