...
**一句话**：新增功能时权限该归哪一类的判定规则，以及内置角色怎么分配，避免权限越加越乱。
**谁该读**：要加新权限点的工程师；调整角色的管理员。
**读完能做什么**：按决策树给新权限定类，并改全该改的几处。
...
| `llm-gateway.access` | 进入独立模型网关控制台；管理员默认拥有，也可按用户单独授予 |
...
独立控制台例外：当外部控制台有独立入口、独立会话或需要按用户授权时，必须注册
`{module}.access`，不得复用业务角色或借用不相干的管理权限。模型网关使用
`llm-gateway.access`。
