...
| H | ~~存量密码主机 `sshPrivateKeyFingerprint` 的轮换~~ **已核实无存量（2026-09-08）** | 来源 #1287 跟进项。PR #1292 把新写入改成随机 `opaqueCredentialRef()`，担心此前创建的密码主机库里仍是由明文口令派生的截断 sha256。**判据只能看认证方式，不能看指纹形状**：旧派生值与新随机值都是 16 hex（`fingerprintPrivateKey` 取哈希前 16 位，`opaqueCredentialRef` 取 8 字节随机），按长度分不出来。2026-09-08 用 `GET /api/cds-system/remote-hosts` 按 `authMethod` 盘点：共 4 台（06-21 至 08-01 创建），**全部是 `private-key` 认证，没有一台密码认证主机**，因此不存在由口令派生的指纹。若日后有人把某台改成密码认证，走的是 PR #1292 之后的 `update` 路径，写入的就是随机值。三选一决策不需要做，无迁移脚本需求 |
