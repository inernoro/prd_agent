...
### 阻断缺陷播报 —— 只播报到实例级站内信，没有「按人认领」（2026-08-02）
- **已落地的那一半**：验收报告归档时若判定为阻断级（判定不通过 / 存在 P0 / 自称通过却带未决 P0-P1），
  CDS 会发出事件、记进站内信账本、按既有外发通道推给 MAP，并给出一步落到该报告的深链。
  归档脚本同时把 P0-P3 计数随元数据上传，此前该字段无人填写。
- **没做的那一半**：通知**没有责任人**。CDS 的站内信账本是实例级全局的（一条通知全员共见，
  只有已读/不再提醒两种状态），既没有 per-user 字段，也没有可用于「按模块路由到负责人」的角色矩阵。
- **为什么不硬做**：要支持认领，得先给 CDS 引入通知的用户维度（每人一份已读态 + 指派态），
  这是账本的数据模型变更，不是在播报侧加个字段能解决的；在没有用户维度的前提下做「认领」，
  只能做成一个全员共享的假状态——谁都能点、点完别人也看不出是谁认的。
- **当前替代**：通知正文直接带上报告标题、阻断原因与各档位计数，读者不点进去也能判断急不急；
  项目名随通知展示，人工分派时至少知道该找哪个项目的人。
- **关闭动作**：CDS 站内信账本引入用户维度（每用户已读态 + 可指派人）后，再在验收播报上叠加认领动作；
  在那之前不要在播报侧单独造一套只服务于验收的指派状态。
