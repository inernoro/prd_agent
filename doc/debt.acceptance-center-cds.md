...
- **根因**：**验收中心侧根本没有「同一件事只留一条」的概念** —— 归档走的是一条
  **无条件新建**的路径：客户端 `POST /api/reports` 的请求体只带 标题 / 正文 / 项目 / 结论 /
  档位 / 分支 / commit / PR 号，**不带任何幂等键**，CDS 每次收到都自铸一个新的规范 id 并落一条新报告。
  API 侧也没有「更新既有报告」或「给同一目标出新版本」的操作可用。于是重跑一次 = 多一条报告，
  没有任何一层会拦。属「判据与接线纪律」形状 2 的一种：去重这条链**根本没建**，
  不是建了但判据太窄 —— 所以「把它删掉会不会红」这个问法在这里天然为否，也就没人发现它不存在。
- **一处容易走错的岔路（2026-08-03 更正）**：归档脚本内部确实生成过一个带**分钟级时间戳**的
  report id，且元数据注释里写着「跨环境同步幂等（reportId 去重）」。但那条只作用在
  **写 MAP 知识库**的分支上；本条债务针对的**默认 CDS 路径**并不把它发给 CDS，它只被嵌进报告正文。
  因此**改那个 id 的拼法不会消除任何重复**。本条最初就是这么归因的，已按实际调用链更正
  （Codex #1319 P2 指出）—— 记在这里，免得下一个人再顺着 id 公式去改一遍。
...
- **关闭动作**（落在 **CDS API 侧**，不是改客户端的 id 拼法）：
  1. 给 `POST /api/reports` 增加**幂等键**入参（建议 `project + target + 冻结 SHA`，由归档脚本传），
     并在 CDS 侧对同键请求做 upsert 或返回既有报告，而不是无条件新建；
  2. 补一个「同一目标出新版本」的操作（`version+1` 或 `supersededBy`），让重验改结论有正规出口，
     不必靠再发一条并列报告；
  3. 通过率统计（tally）改为**每个身份只计最新版**，否则历史版本仍然撑大分母；
  4. 补守卫测试：同一目标连跑两次归档，验收中心必须只增加一条（或只有最新版计入通过率）。
     没有这条守卫，去重这条链「删掉也不会红」—— 它本来就不存在，正是这样才悄悄拖了两周。
...
| 重复归档（默认 CDS 路径） | `.claude/skills/create-visual-test-to-kb/scripts/archive_report.py` 的 `run_cds`：向 `POST /api/reports` 发的请求体不含幂等键，CDS 返回自铸 id；CDS 侧入口见 `cds/src/routes/reports.ts` |
