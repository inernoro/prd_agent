...
### 验收主页「结论优先」重做 —— 四条已知边界（2026-09-08）
- **「合并的分支，验没验」只认 GitHub 合并事件留下的分支墓碑**（`removedBranches`，reason=merged），没接 GitHub 的项目这一栏永远为空；墓碑表有容量上限，被淘汰的老合并不再出现在任何时间窗里。
- **报告之间的身份键是标题合同**（项目 · 前缀 · 对象 · 目标日）。不按 `{前缀} · {对象} · {日期}` 命名的历史报告会归到「其他」，也不会被折叠去重；上游归档脚本没带身份键之前，折叠只能靠标题。
- **「负责人」没有落地**：报告模型里没有该字段，设计稿里的这一列在实现中省略，等缺陷指派打通后再补。
- **产品失败与验收失败的判定只看 `defectCounts`**：未通过且 P0/P1 计数为零判为验收链路失败；未记录缺陷计数的未通过一律按产品失败保守处理。归档脚本不写缺陷计数的报告会把头条推向「有功能坏了」。

### 验收报告重复归档 —— 归档是无条件新建，缺的是身份查找而非更新能力（2026-08-03 定位）
...
- **根因**：归档只走**无条件新建**一条路。客户端 `POST /api/reports` 的请求体只带
  标题 / 正文 / 项目 / 结论 / 档位 / 分支 / commit / PR 号，**不带任何身份或幂等键**；
  CDS 收到就自铸一个新 id 落一条新报告，既不查「这个目标是不是已经有报告了」，
  归档脚本也从不先查再决定新建还是更新。于是重跑一次 = 多一条，没有任何一层会拦。
  属「判据与接线纪律」形状 2：去重这条链**根本没建** —— 「把它删掉会不会红」在这里天然为否，
  所以两周里没人发现它不存在。
- **注意：缺的不是「更新能力」，是「身份查找」**。CDS 侧**已经有** `PATCH /api/reports/:id`
  （可替换标题正文并更新结论、档位、commit、分支、PR 等元数据）。也就是说「原地更新」这件事
  今天就能做，卡住的是上游那一步：没有任何约定能把「这次归档」认回「上次那条报告」。
  所以关闭工作应当是**复用既有 PATCH**，而不是再造一个更新接口。
- **两条不要走的岔路**（都源自本条最初的错误归因，2026-08-03 更正，Codex #1319/#1320 指出）：
  1. 归档脚本内部确实生成过一个带**分钟级时间戳**的 report id，元数据注释也写着
     「跨环境同步幂等（reportId 去重）」。但它只作用在**写 MAP 知识库**的分支上，
     默认 CDS 路径压根不把它发给 CDS，只把它嵌进报告正文。**改它的拼法不会消除任何重复。**
  2. 它也**不是**深链的一部分：CDS 深链用的是服务端返回的规范 id。
     因此不存在「为兼容既有链接所以不能动它」这回事 —— 别为这个理由绕路。
- **为什么不在本次直接改**：身份键怎么定需要先把三种语义分开，而它们在今天的数据里长得一样
  （同 target、同日、同 project）：
  1. **纯重复**（内容与结论都相同，如 07-27 / 07-31 那两组）—— 应当原地更新，不该新增；
  2. **重验且结论变了**（07-29 的 fail → 修订版 → 重算）—— 新结论应当**取代**旧结论进入统计，
     旧报告若要留作审计痕迹，就得引入「supersededBy / 只有最新版计入 tally」；
  3. **修完缺陷后的合法复测** —— 必须是一条**新**报告，绝不能被去重吃掉。
  分不开这三种就贸然去重，会把第 3 种误吃掉，比现在的重复更糟。故本次只定位不动刀。
- **关闭动作**：
  1. 定义报告**身份键**（建议 `project + target + 冻结 SHA`）并让归档脚本随请求带上；
  2. 归档前按身份键查既有报告：命中且属「纯重复 / 重验改结论」→ 走**已有的**
     `PATCH /api/reports/:id` 原地更新（必要时记 `supersededBy` 或版本号），未命中才新建；
  3. 通过率统计（tally）改为**每个身份只计最新版**，否则历史版本仍然撑大分母；
  4. 补守卫测试：同一目标连跑两次归档，验收中心必须只增加一条（或只有最新版计入通过率）。
     没有这条守卫，这条链「删掉也不会红」—— 它本来就不存在，正是这样才悄悄拖了两周。
...
| 重复归档（默认 CDS 路径） | `.claude/skills/create-visual-test-to-kb/scripts/archive_report.py` 的 `run_cds`：向 `POST /api/reports` 发的请求体不含身份键，深链用服务端返回的 id；CDS 侧入口见 `cds/src/routes/reports.ts` |
| 已可复用的更新能力 | `cds/src/routes/reports.ts` 的 `PATCH /reports/:id`（替换标题正文 + 更新结论/档位/commit/分支/PR），关闭动作应复用它而非新造 |
| 易走错的岔路 | 同文件 `run_doc_store` 的 `entry_meta.reportId`（带分钟级时间戳）只作用于写 MAP 知识库，**不参与** CDS 路径去重 |
