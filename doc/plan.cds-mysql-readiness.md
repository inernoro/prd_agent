...
| 2026-05-01 | Phase 6 准备阶段(契约测试 + bug 修复 + runbook) | ✅ done | `<本次>` | tests/integration/phase6-yaml-contract.smoke.test.ts(2 case)把 cdscli scan 输出 ↔ CDS parseCdsCompose 串起来做契约验证;**发现并修复真 bug** — mysql `./init.sql:/docker-entrypoint-initdb.d/` 单文件挂载被 hasRelativeVolumeMount 误判为 app(compose-parser.ts isAppSourceMount 排除 INIT_SCRIPT_TARGET_PREFIXES + CONFIG_FILE_EXT_RE);新增 doc/guide.cds-mysql-validation-runbook.md(5 候选项目 + Step 1-7 + 完成判定 + 6 风险表);cds 后端 728 全绿。**真实开源项目实战待用户挑选**(Prisma examples / databases/mysql 推荐) |
