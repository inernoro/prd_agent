...
**一句话**：从缺陷管理沉淀出软删除、外部分享、回调通知、文件夹管理四个通用模式，供其他模块复用。
**谁该读**：要实现同类能力的工程师。
**读完能做什么**：判断自己的需求能否直接复用已有模式。

---

## 一、管理摘要

- **解决什么问题**：各业务模块重复实现软删除、分享链接、Webhook 等通用功能，代码冗余且体验不一致
- **方案概述**：从缺陷管理 Agent 沉淀 4 个通用组件模式（软删除/回收站、外部分享、Webhook 通知、文件夹管理），供其他模块复用
- **业务价值**：新模块接入通用能力从数天缩短到数小时，用户体验跨模块一致
- **影响范围**：所有需要删除恢复、外部分享、事件通知、层级管理的业务模块
- **预计风险**：低 — 模式已在缺陷管理中验证
