...
```mermaid
flowchart TD
    REQ["调用方发请求<br/>appCallerCode + 用途"] --> REACH{"这个调用方<br/>放行吗"}
    REACH -->|"状态未放行"| X1["拒绝 · 调用方未放行"]
    REACH -->|"放行"| NAMED{"点名了吗"}
    NAMED -->|"点名了"| GATE["过目录闸<br/>在不在表里 · 授权了吗 · 启用了吗"]
    NAMED -->|"没点名"| CLAIM{"有模型认领了它吗<br/>第一层默认"}
    CLAIM -->|"有"| GATE
    CLAIM -->|"没有"| DEF{"这个用途有启用的默认吗<br/>第二层默认"}
    DEF -->|"有"| GATE
    DEF -->|"没有"| X0["拒绝 · 没有人接得住这个调用方"]
...
**图与现状的一处差距**（不写出来就是拿设计图冒充实现）：「按能力筛出候选」这一步今天是按
名字上的能力并集算的，还没有真的落到每条线路上（见第 8 节第 1 条）。

第一个菱形此前还有第三条岔路——「配了专属池 → 落到它自己的池」。模型池 2026-09-15 退场后
运行时不再看 `AllowedModelPoolIds`，**只要放行就认这张目录**，那条岔路与它背后的例外名单
一起消失（见第 5 节末）。
...
当时运行时并不是这么走的：调用方一旦配了专属池，对外模型这一档**整个被跳过**，不点名的请求
落在它自己的池上。冒烟之所以全绿，是因为只跑了一个调用方，而它恰好不是这种——**一个样本判绿
了一句全称命题**。所以「这个调用方认不认对外模型目录」也收进了共享判据，面板改成逐个调用方
给结论，冒烟改成逐个调用方去对。

**2026-09-16 这个判据从三档收到两档**，而且是被同一类错误第二次咬到才发现的。模型池退场之后
运行时不再读 `AllowedModelPoolIds`，判据却还在读它——**判据读的值不是真正生效的那个值**
（形状 6）。结论因此可以是反的：`document-store.transcribe-summary::chat` 名下还留着那个历史
字段，面板据此说它「点名与不点名都走不到 default-chat」，真打一次点名，它落到了 default-chat
的队首。而当时那条守卫用例**正在保护这个错误**——它逐字要求面板继续按池来判（形状 4a）。

收敛之后：走不到这张目录的只剩一种人——**状态未放行的**。不点名落到谁，由「有没有模型认领了
它 → 这个用途的默认是谁」两层回答，与「认不认这张目录」是两个问题。

顺带少了一份判据：「即便配了专属池也仍然认对外模型目录」的那三个视觉调用方名单，此前权威侧
与控制台镜像各存一份、靠行为对照钉住。那个职责随池消失，名单剩下的唯一用途是**模型选择器的
目录展示**，于是搬回它唯一的消费方（解析器），份数 2 → 1。它仍然是一份写在代码里的调用方
特例，正解是让它变成调用方记录上的一个字段——那属于第 8 节。
...
「不点名那条路」那一格**逐个调用方**给结论，不给一句笼统的总结：谁会落到它、谁被别的模型
认领了、谁状态未放行。理由见第 5 节末——那句话必须有主语。
...
**后两条的结局（2026-09-16）**：「池身份」那一种语义确实随断流自己没了。那份名单**没有整个
消失，但份数减半**——它还兼着「视觉创作模型选择器的目录展示」这个职责，所以搬回唯一的消费方
（解析器），控制台那份镜像与对应的行为对照测试一并删掉。预判对了一半：该消失的那半消失了，
剩下那半暴露出它本来就是两件事挤在一份名单里。
