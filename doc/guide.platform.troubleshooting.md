...
1. 在「案例索引」中搜索关键词，看是否有已知案例
2. 如无匹配，使用「分层日志法」逐层确认断点位置
3. 定位后，按案例模板记录排查过程，补充到本文档
...
当怀疑"调用了但没反应"时，按以下分层逐层打日志：

```
用户操作 → 事件回调 → 框架 API → 浏览器 API → DOM/URL 变化 → 框架状态更新 → UI 渲染
```

哪一层日志断了，问题就在那一层和上一层之间。不要猜，逐层确认。
...
关键代码模式（unmount-before-navigate）：
...
| 问题 | 解答 |
|------|------|
| 为什么不用 `window.location.href` 强制跳转？ | 会导致整页刷新，丢失所有 React 状态（登录态、store 缓存等），体验差 |
| unmount-before-navigate 会有闪烁吗？ | 用户看到极短的 loading spinner（约 1 帧 ≈ 16ms），实际感知不到 |
| 浏览器后退键也会受影响吗？ | 后退键触发 popstate 事件，React Router 通过不同路径监听，通常不受影响；但如果也卡住，同样需要先卸载重组件 |

## 相关文档

- `design.workflow-agent.engine.md` — 工作流引擎设计
