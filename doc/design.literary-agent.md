...
> **版本**：v1.0 | **日期**：2026-01-08 | **状态**：已落地

## 一、管理摘要

- **解决什么问题**：内容创作者为文章配图时需逐张手动操作，效率低且难以保证风格一致
- **方案概述**：五阶段工作流（上传 → 预览 → AI 标记配图位置 → 批量生图 → 导出），支持状态恢复和修改即清后续
- **业务价值**：一键生成全文配图，大幅降低创作者配图成本，提升内容产出效率
- **影响范围**：prd-api 文学创作 Agent、prd-admin 文学创作页面、VisualAgent 生图模块
- **预计风险**：低 — 已落地运行，核心流程已验证
...
> 注：代码层 `ImageMaster` 已更名为 `VisualAgent`，数据库集合名保留兼容。下文代码引用保留历史命名。
