...
| 波5 无 Agent 接入(事后栈检测 race-free 后端 + UI 入口) | 80 | 进行中 | 灰度真视觉验收(空项目态点「检测技术栈」→ 生成配置 → 建分支,双主题截图) | 灰度真机验收该 UI 流程后收波5 | 后端:detect-preview(只读扫 worktree)+ detect-apply(用户确认后建 profile,空项目守门,race-free)+ 6 例端点测试绿。UI:BranchListPage 空项目态(buildProfiles=0)引导「检测技术栈」→ DetectStackDialog(shadcn Dialog,主题 token 双色,列表可勾选)调 preview→apply→刷新;contract 测试断言 wiring。全量 vitest 2605 passed(server-integration 10 例存量环境失败无关)+ backend/web tsc + web build 绿(2026-07-06 本地);关键发现见下(建项目对话框已有 URL 式 3 步接入) |
