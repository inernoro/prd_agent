...
| 波5 无 Agent 接入(事后栈检测 race-free 后端 + UI 入口) | 55 | 进行中 | 无(后端可用) | UI:空项目态加「检测技术栈」按钮(调 detect-preview → 确认 → detect-apply),走双主题 + 移动兜底 + 真视觉验收 | 关键发现:建项目对话框(ProjectListPage)**已**有 detect-runtime + onboardingServices + autoDetectOnClone 的 URL 式 3 步接入,PM 建项目即可用;本波补齐的是「已 clone 空项目」事后检测的断头路——detect-preview(只读扫 worktree)+ detect-apply(用户确认后建 profile,空项目守门,race-free)+ 6 例端点测试绿;全量 vitest 2604 passed(server-integration 10 例存量环境失败无关)+ backend tsc 绿(2026-07-06 本地) |
