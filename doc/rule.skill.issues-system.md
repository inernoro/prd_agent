...
| `visual-test:reviewing` | `3B82F6` 蓝 | 已接单，等待开发者回应 | `/issues-visual-run` 报失败时 | 开发者修复 push 后**同时加回 `visual-test:pending`**（否则 visual-run 接单条件只看 pending，永远不会重测） |
| `visual-test:passed` | `10B981` 绿 | 全部 P0/P1 通过，可关闭 | `/issues-visual-run` 通过时 | — |
| `visual-test:blocked` | `EF4444` 红 | 环境不通 / 模板填不全 | `/issues-visual-run` 阻塞时 | 开发者澄清后**同时加回 `visual-test:pending`**（同上，没有 pending 永远不被接单） |
...
# 豁免（§2.3 全集 — 不创建则 label 无法应用，跳过规则形同虚设；--force 兼容 GitHub 默认就有的 wontfix/invalid）
gh label create "human-only"   --color 1F2937 --description "显式禁止 Agent 介入"        --repo $REPO --force
gh label create "discussion"   --color 6B7280 --description "长期讨论，不是请求"         --repo $REPO --force
gh label create "protocol"     --color 8B5CF6 --description "协议演化讨论（不是请求）"    --repo $REPO --force
gh label create "meta"         --color 6B7280 --description "元 issue（关于流程本身）"   --repo $REPO --force
gh label create "rfc"          --color 6B7280 --description "RFC 讨论"                    --repo $REPO --force
gh label create "tracking"     --color 0E8A16 --description "追踪型 issue，无单点修复"    --repo $REPO --force
gh label create "epic"         --color 0E8A16 --description "Epic 级别追踪"               --repo $REPO --force
gh label create "umbrella"     --color 0E8A16 --description "伞型 issue，聚合子任务"      --repo $REPO --force
gh label create "wip"          --color FBCA04 --description "Work in progress"            --repo $REPO --force
gh label create "wontfix"      --color FFFFFF --description "不会修复"                    --repo $REPO --force
gh label create "invalid"      --color E4E669 --description "无效 issue"                  --repo $REPO --force
gh label create "on-hold"      --color FBCA04 --description "暂缓处理"                    --repo $REPO --force
```
...
- §3 测试矩阵（视口 × 主题 × 状态 × 交互态 × **路径**；**点击导航进入、禁地址栏直达**；**页面支持 light 才双主题强制，dark-only 单图 + 注明不计 fail**）
- §4 项目专属硬约束（10 条，对应 CLAUDE.md / .claude/rules/）
- §5 业务用例（至少 3 条角色+任务+预期）
- §6 已知不覆盖（防止执行者越界）
- §7 失败回报格式（表格列：# / 检查点 / 视口 / 主题 / 截图 / 问题描述 / **主题归因** / **规则文件** / **修复指向** / 严重级）
- §8 通过标志（P0/P1 清零 → `/visual-pass`；归档到验收知识库并回贴分享链）

### 5.0 v1.0 增量（固化自 #605 三位执行 Agent 反馈，2026-06-04）

v0.1 → v1.0 收敛的执行侧共识（实现见 `create-visual-test-to-kb/scripts/harness.mjs` + `reference/standard-v2.md` §5.3-5.5，自测 10/10 通过）：

1. **运行时错误自动捕获**（机器最该补、人最易漏）：取证全程自动收 `console.error`(P1) / 同源 4xx-5xx(5xx=P0,4xx=P1) / 未捕获异常(P0)；P0 自动成为归档硬门禁。
2. **机读 `result.json`**：每轮产出 `{verdict,autoFindings,themeSupport,timing,shots}`，下游 Agent 直接消费不解析 markdown。
3. **dark-only 双主题伪命令消除**：机器探测页面是否真支持 light，dark-only 页单图 + 注明，不逼交两张一样的暗图。
4. **失败回报加列**：`主题归因`（系统级 token / 页面级）+ `规则文件`（命中的 `.claude/rules/*`）+ `修复指向`（文件/组件），把"发现问题"升级成"可回交开发的任务"。
5. **点击导航取证升为硬约束**：禁地址栏直达，模拟真实用户路径（Bridge 2026-05-28 起默认关闭，执行者默认走 Playwright 无头直连）。
6. **过程视频（可选）**：本地附件，不进知识库正文（沿用 2026-05-27 用户决定）。

派单/闭环侧建议（从 diff 自动起草子 issue、P0/P1 转独立 bug 回链、分支级 rollup）记为 **v1.1 候选**，未纳入 v1.0 硬要求。
