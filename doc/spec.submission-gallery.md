...
> **版本**：v1.0 | **日期**：2026-03-23 | **状态**：开发中

**一句话**：把视觉与文学作品发布到公共画廊：不同创作类型投稿粒度不同，展示策略也不同。
**谁该读**：投稿与画廊的产品与工程师；关心什么能公开的运营。
**读完能做什么**：说清两类作品的投稿粒度差异与展示规则。
...
- 排序：长期新鲜度 + 短期互动热度；新鲜度按 7 天为一个刻度平滑衰减，发布 7 天后仍保留约 54%，30 天后约保留 18%
- 低流量保护：零互动新作也能持续获得发现机会；浏览与点赞仍按小时衰减，避免历史爆款长期占据首页
- 时间口径：视觉单图按投稿时间；文学作品按投稿记录与 Workspace 的最近有效内容更新时间
- 分页：每页 20 条，点击加载更多
- 卡片：封面图 + 作者头像 + 用户名 + 最近有效内容日期 + 浏览数 + 点赞数
