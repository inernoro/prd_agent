...
### 管理后台增强 - P1/P2
- [x] **AdminLlmLogsController + LlmLogsPage** - LLM 请求日志查看
  - GET /api/v1/admin/llm-logs（列表/筛选/分页）
  - GET /api/v1/admin/llm-logs/{id}（详情）
  - GET /api/v1/admin/llm-logs/meta（下拉枚举）
  - 说明：日志用于排查问题，内容尽量保留；仅对 API Key/Token/密码等**密钥类字段**脱敏；展示用户问题与模型最终回答（≤200k字符）；不保存 rawSse；`running` 超时由 watchdog 自动纠错为 TIMEOUT
- [ ] **系统告警配置**
  - 错误率告警
  - 响应时间告警
  - LLM 失败率告警
...
| 密钥名称 | 说明 | 值 |
|---------|------|-----|
| `APPLE_ID` | Apple 开发者账号 | `your-apple-id@example.com` |
| `APPLE_PASSWORD` | App 专用密码 | `your-app-specific-password` |
| `APPLE_TEAM_ID` | Apple Team ID | `YOUR_TEAM_ID` |
| `APPLE_CERTIFICATE` | P12 证书 Base64 编码 | *需导出并编码* |
| `APPLE_CERTIFICATE_PASSWORD` | 证书密码 | `your-certificate-password` |
...
```bash
# 1. 更新版本号
# 编辑 prd-desktop/src-tauri/tauri.conf.json 中的 version
# 编辑 prd-desktop/src-tauri/Cargo.toml 中的 version
# 编辑 prd-desktop/package.json 中的 version
