...
3. 页面提供前端项目表，用户可按项目名、仓库地址、技术栈、构建地址、备注搜索项目位置，并复制项目地址摘要。
4. 页面提供 PDA 项目手册，用户可查询 uniapp-pda 项目说明、开发调试、现场操作、构建发布、常见排障和 AI 提示词模板。
5. 用户输入需求描述、目标技术栈、样式约束，并可上传文本类接口/代码/日志材料。
6. 后端通过 `ILlmGateway` 流式生成前端交付方案，前端展示阶段状态、模型信息、思考过程和逐步输出。
7. 输出必须包含结论摘要、交付文件、可复制代码、接入步骤、自测清单、风险与假设。
