...
授权健康中心上线后，首页与移动端一直没有可点的入口，只能靠地址栏直达。补齐两处：首页静态实用工具清单新增该条目，按 `logs.read` 权限门控显示；移动端底部抽屉的实用工具改为从统一的导航注册表派生目录，不再是与桌面端分离维护的另一份清单。两处入口与导航注册表之间新增覆盖测试，防止后续新增导航项时再次漏掉某一端。
