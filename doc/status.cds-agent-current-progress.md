...
> **更新时间**：2026-05-18 23:35 Asia/Shanghai
...
| 3 | D1 架构纠偏计划 | done | 0 |
| 4 | R0 CDS-managed runtime 恢复设计 | in_progress | R0.7 liveApply 代码路径已接入；下一步是真实 live evidence |
...
| `CDS_MANAGED_RUNTIME_CAPACITY` | missing_live_evidence_ready | 当前缺的是运行中的 CDS 产品级 runtime capacity；CDS `/runtime-capacity` 与 `/runtime-capacity/reconcile liveApply`、runtime-status/progress/audit/smoke 已锁定这个 blocker，remote host/env/image 只能作为 operator fallback 证据 |
...
| R0 CDS-managed runtime pool | [~] | `doc/design.cds-agent-managed-runtime-fact-source.md`、runtime-status task board、CDS route test、MAP session transport smoke、R0V live evidence、R0.5 capacity smoke、CDS runtime-capacity/reconcile/liveApply route test | blocked_capacity | 跑 R0.7 live evidence，让真实 shared-service official SDK runtime running > 0 |
...
下一轮只做 R0.7 CDS-managed runtime live evidence，不做普通 preview redeploy，不把 SSH/env/image 重新提升为产品路径。
...
| 3.1 | 在真实 CDS shared-service runtime 执行 live evidence | 不需要普通用户补 SSH/env/image | shared-service official SDK runtime running > 0，R0 pass |
