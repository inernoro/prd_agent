...
> **更新时间**：2026-05-18 19:15 Asia/Shanghai
...
| `SIDECAR_LOCAL_BUILD` | blocked | Colima 标记 running，但 Docker CLI 当前无法连接 daemon，build smoke 输出 `docker_unavailable` |
...
| sidecar local docker build | `docker_unavailable` | `/tmp/cds-agent-sidecar-image-build-current.json` |
...
| sidecar image build smoke | `/tmp/cds-agent-sidecar-image-build-current.json` | Docker daemon 当前不可达；未 push、未 deploy | <1s |
...
| sidecar image build smoke | <1s 当前失败 | 把 Docker daemon 不可达写成机器可读证据，不再把它混进 R0 远程 deploy 失败 |
