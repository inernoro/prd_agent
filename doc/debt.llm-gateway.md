...
| 2026-07-09-gw-config-authority-not-migrated | high | 2026-07-09 | serving 已补 GW Native/OpenAI/Claude/Gemini 入口、IR 上下文字段、`llm_gateway.llmgw_app_callers` 被动登记与控制台配置；`Status=active + ModelPoolId` 已进入 resolver 优先级；控制台可将 MAP 池、平台、模型、exchange 认领到 `llm_gateway` 自有集合，resolver 优先读取 GW 副本，已有模型与 Exchange 观测页、GW-owned 对象基础密钥轮换、批量密钥轮换和密钥删除、key 解密健康自检、GW 模型池新建/属性编辑/批量认领、GW 权威池成员基础增删改和批量导入、GW-owned 模型通用能力矩阵批量维护、OpenAI/Claude/Gemini/OpenRouter 字段级参数模板、池成员能力快照、控制台能力过滤、appCaller owner/月预算/每分钟限流治理元数据维护、按筛选批量治理 owner/USD 月预算/RPM/状态/策略、操作审计筛选页、serving 每分钟限流 enforcement、月预算基于已有日志成本证据 enforcement、GW 日志成本归因写入、控制台成本展示、日志详情 routerTrace 派生、ProviderAttempts 快照与详情抽屉路由/attempts 解释区块、Exchange async raw submit/poll/poll-timeout attempts、非流式 auto provider retry attempts、流式输出前 auto provider retry attempts、raw JSON/multipart submit 阶段 auto provider retry attempts、成功完成日志根字段更新为最终 provider/model、模型池成员价格币种 CNY/USD 配置、历史价格币种批量校准入口、结构化输出与 logprobs 模型筛选、并行工具调用模型筛选、字段级参数能力轻量维护与 `/gw/parameter-capabilities/meta` 元数据提示、运行时 function_calling/parallel_tool_calls/vision/image_generation/thinking/structured_output/logprobs 能力软门、扩展后的 `parameter:<name>` 字段级参数能力门、strict-require 能力门和 dropped 参数拒绝门、配置权威迁移报告、MAP-only 统一批量认领入口、active appCaller 自动绑定 GW 默认池入口，以及 `LlmGateway:DisableMapConfigFallbackForActiveAppCallers` active 调用方 MAP fallback 退场门禁；控制台报告已暴露 `mapFallbackObjectsRemaining` 和 `activeAppCallerMapFallbackReady`。但 provider 参数模板覆盖面还需按真实文档持续扩展，流式 retry 仅覆盖输出前失败且不支持输出后断流换 provider，raw retry 仅覆盖 submit 阶段且不跨 Exchange async poll/二进制下载/WebSocket ASR 重放，provider retry 仍需生产灰度证据，生产 MAP 配置读取退场开关仍未开启 | 宣称“GW 已成为统一 AI 请求治理层”或准备让外部系统长期只接 GW 时 | in-progress | 下一步按 `plan.platform.llm-gateway-protocol-router.md` Phase 3/5 迁移：在生产执行 MAP-only 认领与 active 绑池后复核 `activeAppCallerMapFallbackReady=true`，再灰度开启 active appCaller MAP fallback 退场门 |
...
- `http-full` 发布证据门已补配置权威检查：`scripts/llmgw-release-gate.py --require-config-authority` 会读取控制台 `/gw/config-authority/report`，rollout ledger 会拒绝缺少 `configAuthority.ok=true` 的 `http-full` 成功记录。该 gate 只是防误切，不代表生产已执行 MAP-only 认领、active 绑池或开启 `DisableMapConfigFallbackForActiveAppCallers`。
- 配置权威退场已有可执行脚本并纳入生产阶段：`scripts/llmgw-config-authority-apply.py` 默认只读；显式 `--execute` 才会调用 `bulk-claim` 与 `bind-active-app-callers`，`--require-ready` 会把最终 readiness 作为非零退出门。`scripts/llmgw-prod-stage.sh --stage config-authority` 已把该动作写入 rollout ledger，且不运行 `fast.sh` / `exec_dep.sh`。该阶段执行链已改为备份先行：先运行 `scripts/llmgw-config-authority-backup.sh` 备份 `llm_gateway` 全库与 MAP 模型配置关键集合，再执行配置权威迁移；ledger 会拒绝 dry-run、空归档或无 SHA256 的备份证据。该阶段尚未在生产执行。
