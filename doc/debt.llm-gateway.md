...
- CDS 当前只把主管理后台 profile 暴露到分支预览根域名；独立运行的 `llmgw-web` profile 没有可从公网预览域名到达的路由。PR-2 先以本地真实登录浏览器完成桌面/移动页面验收，并以 CDS 容器内 `/quickstart`、静态产物和同 commit serving health 取证。PR-4 控制台信息架构验收前，CDS 需要为 `llmgw-web` 提供独立外部地址或稳定路径转发，届时才能用最终深链完成外部视觉验收。
