...
| 12 | 网页发布的幂等靠「确定性 id + 占坑租约」跨两套存储凑出来 | 站点要同时写对象存储与 Mongo，而幂等键把 id 压成确定性的，于是「谁先谁后、谁能接手、谁能收尾」全靠占坑标记加租约在应用层拼。已经能自洽（抢不到租约就回 409 让重试，撤回与收尾都按持有者过滤），但它是三轮 review 迭代出来的形状，租约靠时间阈值判过期——这类判据只要窗口取错就会退化成「随时可抢」。本轮不再继续加固：真要彻底简化，得让「发布」变成单一存储的原子动作（先把 HTML 写进 Mongo，再由后台任务同步到对象存储），那是新的语义类别 | 两条选一：① 发布落库为唯一真相源，对象存储由 worker 异步同步，站点未同步完就不给外链；② 保留现状但把租约换成存储层原生的条件写入（如对象存储的 If-None-Match），让「谁赢」由存储自己裁决，不再由应用层的时间阈值裁决 |
