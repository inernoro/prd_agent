...
**一句话**：平台层十一个模块的欠账合成一册：资产存储、更新中心、历史表情符号清理、首页、登录会话、连接器、跨节点互传、预览入口下发、分享链接安全、监控自发现、后端单元测试项目的接线方式。
...
**限定范围**：只说 `PrdAgent.Tests`（那个逐文件登记被测源码的单元测试项目）这一个。
同目录下的 `PrdAgent.Api.Tests`（集成测试项目）用的是整项目引用、没有这个欠账——
偿还方案不要误用到它身上。

**欠什么**：`PrdAgent.Tests` 与被测项目之间不是整项目引用，而是**逐个文件登记**的关系。
新增被测文件时漏登记，测试项目就编译不过——而这个错误落在两条流水线的缝里：
一条在功能分支上不触发，另一条不编译测试项目。于是它可以一路绿到合并前。

**为什么要紧**：它坏的不是某个功能，是**守卫本身**。漏登记的那一刻，新加的测试
从未被编译、更谈不上跑过，而看板上它与真正跑绿的测试长得一模一样。

**怎么暴露的**：2026-09-15，模型排行榜的一批解析守卫从落地起一次都没编译过。
手动把 CI 触发到那条分支才炸出编译错误，连带又暴露一条早就欠着的枚举涟漪
（新集合没分类）——两个都是「加了守卫但守卫从没跑过」放出来的。

**现状**：靠人记得登记、记得手动触发一次 CI，规则写在 `cds-first-verification.md`。
没有任何机械判据。集成测试项目不受影响。
...
| 方向 | 取舍 |
|---|---|
| 把它也改成整项目引用（与集成测试项目一致） | 「忘了登记」这个错误类别直接消失；代价是它会拖进被测项目的全部依赖，编译变慢 |
| 保留逐文件登记，加一条守卫断言「测试引用到的被测源文件都已登记」 | 判据精确；但守卫自己也要有人跑，除非同时放宽 CI 的触发分支 |
| 把后端构建与测试那个 job 放宽到所有分支 push 触发 | 最直接；代价是每条分支每次 push 多跑一轮数据库容器 + 那一档 xUnit（非集成、非手工） |

**验收标准**：新增一个被测文件却漏登记时，有东西在合并前变红，而不是靠人想起来。

**触发条件**：再出现一次「新加的测试没被编译 / 没被跑过」就必须选一个落地，
不能第三次靠人记。
...
| ID | 说明 | 优先级 | 触发条件 | 状态 |
|---|---|---|---|---|
| CI-1 | `ci-status` 的 `needs` 与结果计算补上 `release-script-test`，让这批守卫真的把门 | **P2** | 下一次有人依赖这批守卫拦回归时；或任何一次「守卫红了却合进去了」 | **触发条件已命中**（2026-09-07 PR #1507）——见下 |

**CI-1 的触发条件已经命中（2026-09-07，PR #1507）。** 该 PR 往这批守卫里新增了一条
周报浅克隆闸的行为守卫（16 项断言），并且**正是在依赖它拦回归**：那道闸在七轮 review
里被找出六种 fail-open，守卫存在的全部意义就是让第七种变红。但只要
`release-script-test` 不在 `ci-status` 的 `needs` 里，它红了也照样能合——守卫接了 CI，
没接到把门的那一处。

没有在 #1507 里顺手改掉，理由是**爆炸半径超出那个 PR 的目标**：把
`release-script-test` 接进聚合闸，会让它里面另外八九批守卫（静态发布布局、网关挂载、
公网表面冒烟、发布证据、gw-smoke、rollout 账本、ASR 转发、cdscli 冒烟、changelog 合并）
**同时**变成阻断合并的条件。这是仓库级的合并策略变更，该由人明确拍板，而不是由一个
改周报技能的 PR 顺带改掉；何况主干 CI 当时本身是红的（`CDS Build & Test` 自 09-04 起
红到 09-07），贸然接上可能当场卡住所有人的 PR。

要动的时候，动之前先确认：`release-script-test` 在主干当前是绿的；`skipped` 与
`success` 两种状态都要按其余六个 job 的写法处理（否则未触发该 filter 的 PR 会被误判为
失败）。
...
| 35 | 接入台的能力用色点表示，颜色这一路对色觉障碍不友好 | 客户端卡上的五块能力压成了五个点：形状分档（实心=能写、圆环=只能看、虚线=未开），颜色分能力（视觉/文学/知识库/网页/海鲜市场各一色）。**形状这一路是无障碍的**，把屏幕调成灰度也读得出档位；**颜色这一路不是** —— 红绿色觉障碍的用户分不出「哪个点是哪块能力」。现有的冗余是三道：每个点自带可读名字（读屏与长按都拿得到）、点一下展开逐块的文字清单、旁边一句带数字的摘要。所以信息一条不少，但「一眼扫出是哪块」这个便利对他们不成立，得多点一下 | 关键取舍：真要让颜色这一路也无障碍，得给每块能力配一个形状或图标（五个不同形状的小标记），而那会把这一行的视觉重量加回去，等于退回改版前的密度 —— 这次改版的目标恰恰是把它压下来。另一条路是跟随系统的高对比/色觉辅助偏好切换成图标版。验收标准：把屏幕设成色觉模拟（红绿色盲）后，用户仍能在不展开的情况下说出「哪块能力给到了哪一档」 |
| 36 | 接入台色点「折叠态带标签、展开区做装饰」这条守卫仍是源码扫描 | 色点组件自身的四档形状、标签、装饰模式已经是渲染后断言；但「页面里两处色点恰好一处是装饰点」这一条仍靠扫页面源码判定。Review 指出两个盲点：把某处色点列表抽成 helper 会误红；一个没挂到页面上的装饰点也能骗过计数 | 改成渲染断言要能把整张客户端卡连同「展开」这个交互一起跑起来，当前前端测试环境做不到，补齐它超出「视觉编码」这次改版的范围，作为后续单独一笔。验收标准：删掉展开区那个装饰标记，或给折叠态的点错加上装饰标记，都有一条渲染后的断言变红 |
| 37 | 接入台色点的档位注册表只登记了形状名，具体画法与图例仍各写一份 | 四档的说法与形状名已经在同一张表里，新增一档时类型检查会逼着补登记；但「圆环怎么画、虚线圈怎么画」在色点组件与图例里各有一份，改画法时两处要同步改。渲染后的契约守卫会在色点这一侧变红，图例那一侧没有守卫 | 这是同一处第二次被 review 要求再收敛，而本 PR 的修复提交已到范围熔断线，故记为后续单独一笔：把画法也收进注册表、图例与色点共用一个渲染器。验收标准：只改注册表里某一档的画法，色点与图例同时跟着变，不需要改第二处 |
...
## 文学 MCP 图文流程边界（2026-09-23）

| ID | 边界 | 当前影响 | 关闭条件 |
|---|---|---|---|
| MCP-LIT-01 | 改稿后自动重新规划配图尚未开放 | 新建时可提交带标记文章并完成配图归档；改稿会使旧标记失效，需在页面重新规划，或新建一版图文稿。不能声称已有文章任意改稿也已全自动 | 增加带版本冲突保护的重新规划命令，并验证旧任务无法回填新标记 |
| MCP-LIT-02 | 文件夹仍是文章分组，不是独立空目录 | 页面明确提示「新建文件夹和文章」；MCP 可直接指定文件夹或移动已有文章，不制造占位文章。删除最后一篇后空组不保留 | 用户确实需要独立空文件夹时，统一网页与开放接口的数据契约并覆盖空文件夹生命周期 |

## 通用对话运行时已下线（2026-09-08）

通用对话（左侧导航「对话」）的执行环境是部署在 CDS 上的共享服务项目「Claude SDK Sidecar Pool」。
该项目 2026-07-02 被手动暂停、此后未再部署（最后一次部署 2026-05-18），2026-09-08 维护者决定**不恢复**，
#1337 据此关闭。后果：入口仍然暴露给全员，但每一轮对话都以「运行时还没配置好」收尾；
「模型自主调度工具」这一环（见 [design.platform.chat-agent.md](./design.platform.chat-agent.md)「尚未验证」第 1 条）在运行时重新部署前无法验证。
...
协议与落地见 [spec.platform.monitor-discovery.md](./spec.platform.monitor-discovery.md)。服务在自检响应的
`cds:monitor` 段里申报「该怎么监控我」，部署平台对账后建监控项。下面两条是交付时声明的已知边界。

| ID | 说明 | 优先级 | 触发条件 | 状态 | 责任 |
|---|---|---|---|---|---|
| MD-1 | **自描述里的 `failuresToAlarm` 与 `severity` 没被消费，声明了等于没声明**。对账只把 `name` / `field` / `op` / `value` / `intervalSeconds` / `observeMode` / `sampleComponentId` 七项写进监控定义；去抖一律套全局阈值（`CDS_UPTIME_FAILURE_THRESHOLD`，默认连续 3 次失败才算真故障），严重级无处落地。后果是网关那条 P0「未处理异常数 == 0」**拿不到它声明的「出现一次就报」**：该监控 6 小时探一次、异常计数窗口也是 6 小时，一次性异常会在攒够 3 次之前滑出窗口，**铃可能一次都不响**——而它正是为了治「降级吃掉后端崩溃」才建的。验收判据：把 `failuresToAlarm: 1` 的监控接上真实一次性异常，确认站内通知真的到达；同时补一条守卫，声明里有而对账没读的字段必须变红（否则下一个字段还会这样静默失效） | **P1**（承诺的告警可能不触发） | 任何带 `failuresToAlarm` ≠ 全局阈值 或带 `severity` 的自描述 | open（2026-09-13，由 PR #1526 的自动 review 查出） | 部署平台侧：对账与运行态去抖 |
| MD-2 | **「每条 check 都自带 `cds:monitor`」只由源码文本守卫保证，且只认当前那两条 check**。`ServingFaultTrackerTests.CheckBlock` 按两个写死的起始标记切块：插在两标记之间的新 check 会被并进前一块，追加在最后的会让末块延到文件尾；此时从目标 check 上摘掉 `cds:monitor` 可能仍判绿。根治是起一次端点、按真实 JSON 逐条 check 断言（形状 6：扫源码字面量 ≠ 拿求值结果），属新的测试基础设施，2026-09-13 那批修复未展开。验收判据：新增第三条 check 后，从任意一条上摘掉 `cds:monitor` 都能让守卫变红 | P2（当前两条 check 下守卫成立，红绿闭环实测过） | 往 `/gw/v1/healthz/deep` 或 `/api/healthz/deep` 加第三条 check | open（2026-09-13） | 加 check 的人必须同时改守卫 |
...
**影响**：只影响读数与判断，不影响数据本身。但「积压」这个数会被系统性高估，任何拿它做人力或流程决策的地方都偏保守。

**高估量不是「全部未关的软删缺陷」**，而是其中**同时满足积压那三个谓词**的那些：有指派人、指派人在本次统计的成员集合里、
创建于 7 天以前。软删的未关缺陷若较新、没指派人、或指派给了成员集合之外的人，它根本进不了积压，也就不构成高估。
把范围说大会把修复后的核对引向错误的人群——量它的时候要按这三个谓词一起筛。
...
**完成判据**：**不能只用「积压 ≤ 未关」这个不等式**——它能在缺陷仍在时照样成立。
两个端点数的本来就不是同一批：积压那侧只算**有指派人、指派人在统计的成员集合里、且停留超 7 天**的，未关那侧算**全部未删的未关**。
只要较新的或没指派的未关缺陷够多，即使软删记录仍被算进积压，不等式依然为真。这种判据是「不会红的绿灯」。

要红得起来，判据得直接打在那个被漏掉的谓词上：

1. 造一条**同时满足四个条件**的缺陷：状态为未关、有指派人（且该指派人在统计的成员集合里）、创建于 7 天以前、`IsDeleted = true`；
2. 断言**两个端点都不把它算进去**（积压数与未关总数都不含它）；
3. 跑一次红绿闭环：把那侧的删除过滤去掉，这条守卫必须变红，再恢复。

判据来源：`predicate-and-wiring-discipline.md`——「如果这处改动删掉之后测试仍然全绿，那它就需要一条守卫」。
