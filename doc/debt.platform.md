...
| 36 | 接入台色点「折叠态带标签、展开区做装饰」这条守卫仍是源码扫描 | 色点组件本身四档的形状、标签、装饰模式已经是渲染后断言 DOM；但「页面里两处色点恰好一处是装饰点」这一条仍靠扫页面源码里的 JSX 写法判定。Review 指出：把某处色点列表抽成 helper 会误红，而一个没挂到 DOM 上的装饰点也能骗过计数 | 要改成渲染断言，得渲染整张客户端卡并模拟展开：卡片的展开态是组件内部状态，前端测试环境是 node（没有 jsdom / testing-library），静态渲染做不了点击。可选路径：给测试引入 jsdom 并用 react-dom/client + act 点开；或把客户端卡拆成受控组件（展开态由外部传入）后静态渲染两次。两条都超出「视觉编码」这次改版的范围，作为后续单独一笔。验收标准：删掉展开区那个 `decorative`，或给折叠态的点加上 `decorative`，都有一条渲染后断言变红 |
