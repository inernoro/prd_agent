...
**CI-1 的触发条件已经命中（2026-09-07，PR #1507）。** 该 PR 往这批守卫里新增了
`scripts/tests/test_weekly_shallow_gate.py`（周报浅克隆闸的行为守卫，16 项断言），
并且**正是在依赖它拦回归**：那道闸在七轮 review 里被找出六种 fail-open，守卫存在的
全部意义就是让第七种变红。但只要 `release-script-test` 不在 `ci-status` 的 `needs` 里，
它红了也照样能合——守卫接了 CI，没接到把门的那一处。
