...
| 20 | 调用记录的折叠只在已加载的那一页里 | 「一件事一行」是前端按产物身份（`kind:id` + keyId）把流水折起来的，而列表一次只取最近 50 条。一件跨过这 50 条边界的事（长时间轮询的生图）会被切成两段，显示成两件事，且靠前那段的「发起参数」其实是一次轮询而不是入队。第一期不做：要根治得让服务端按事件分页，也就是先有事件的概念再有分页，属新增语义类别 | 关键取舍：折叠放前端简单且不引入派生字段（关联本来就在产物 id 上），代价就是它只看得见一页；放后端准确但要引入「事件」这个新实体并重做分页。验收标准：一件跨过单页上限的事仍显示成一行，且发起参数取的是真正的入队那次 |
