...
| 3 | ~~智能体接入台没进百宝箱~~（已修复 2026-09-03） | 原因写的是「没有可用插画素材」——那半句对，但结论错了：`hasAgentCardArtwork` 只查 presentation 表，真正缺的是两张 960x600 webp。这台机器没有任何图片编码器（无 PIL / cwebp / imagemagick），但有 Chromium ——用画布画完 `toDataURL('image/webp')` 导出即可，脚本留在 `prd-admin/scripts/gen-mcp-console-art.mjs`。已按规则带 `wip: true` 注册，四条前端守卫（插画覆盖、任务文案、token 唯一、双主题 token 计数）本地全绿 | 已闭环；插画是程序化的，换成手绘版只需覆盖同名文件，先读那个脚本看画的是什么 |
