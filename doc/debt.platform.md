...
**一句话**：平台层十一个模块的欠账合成一册：资产存储、更新中心、历史表情符号清理、首页、登录会话、连接器、跨节点互传、预览入口下发、分享链接安全、监控自发现、后端测试项目的接线方式。
...
**欠什么**：`PrdAgent.Tests` 不 `ProjectReference` `PrdAgent.Api`，被测的 Api 源文件靠 csproj 里
一条条 `<Compile Include ... Link="..."/>` 链进去。新增被测文件忘了链，测试项目就编译不过——
而 `ci.yml` 在 feature 分支不触发、`branch-image.yml` 又只 publish API 项目不编译测试，
两条流水线恰好把这个错误漏在中间，可以一路绿到合并前。

**怎么暴露的**：2026-09-15，模型排行榜的 14 条解析守卫从落地起一次都没编译过。
手动把 `ci.yml` dispatch 到那条分支才炸出 `error CS0234`，连带又暴露一条早就欠着的
`DataSyncScope` 涟漪（新集合没分类）。两个都是「加了守卫但守卫从没跑过」放出来的。

**现状**：靠人记得改 csproj + 记得手动 dispatch 一次 CI，规则写在 `cds-first-verification.md`。
没有任何机械判据。
...
| 方案 | 代价 |
|---|---|
| 让 `PrdAgent.Tests` 直接 `ProjectReference` `PrdAgent.Api` | 测试项目会拖进 Api 的全部依赖（Mongo、Hosting、MVC），编译变慢；好处是「忘了链」这个错误类别直接消失 |
| 保留 Compile Include，加一条守卫：扫 `tests/**/*.cs` 里 `using PrdAgent.Api.*` 的每个命名空间，断言对应源文件已在 csproj 里登记 | 判据精确，但守卫自己也得有人跑——除非同时把 `ci.yml` 的触发条件放宽到所有分支 |
| 把 `ci.yml` 的 `Server Build & Test` job 拆出来，对所有分支 push 触发 | 最直接；代价是每条分支每次 push 多跑一轮 mongo 容器 + 全量 xUnit |

**触发条件**：再出现一次「新加的测试没被编译/没被跑过」就必须选一个落地，不能第三次靠人记。
