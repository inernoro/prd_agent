...
| 7 | 分享链没有幂等键 | `map_web_create_share` 现在走服务端复用路径（同用户 + 同站点 + 同访问级别 + 未吊销即复用），重试拿到的是同一条链接，所以「攒出一堆链接」这半边已经堵住。但用户如果真想为同一个站建**第二条**不同有效期的链接，现在这条路走不通 —— 复用会把有效期刷新到本次所选，而不是新开一条 | 给这个工具加 `clientRequestId`：带了就按幂等键判（同键复用、不同键新建），没带维持复用。要跟 §5.5 一起看：这是新增语义，属后续 PR |
