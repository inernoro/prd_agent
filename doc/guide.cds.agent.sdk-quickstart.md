# guide.cds.agent.sdk-quickstart
...
新工具的注册位置在 `prd-api/src/PrdAgent.Infrastructure/Services/AgentTools/Tools/`。一个工具一个文件，按 `IAgentTool` 接口实现，构造函数里 `Register` 一下即可。详见 `doc/design.cds.agent.sdk-executor.md` §6.2。
...
如果 sidecar 由 CDS 作为 shared-service 部署，MAP 不需要手工维护 `Sidecars[]`。流程：

1. CDS 生成 `cds-connect:v1:` 连接密钥。
2. MAP `/infra-services` 粘贴密钥，落库 `infra_connections`。
3. `DynamicSidecarRegistry` 周期读取 active CDS 连接，用 longToken 拉 `{PartnerBaseUrl}{InstanceDiscoveryUrl}`。
4. 返回的实例会以 `Source="cds-pairing"` 合并进 `ClaudeSidecarRouter`。

这条路径不依赖 MAP 本地 `ANTHROPIC_API_KEY`；外部 sidecar 自己持有 Anthropic key。即使 `ClaudeSdkExecutor.Enabled=false`，只要配对发现到 `cds-pairing` 实例，`claude-sdk` 执行器也会可用。

MVP 冒烟与接力启动项见 `doc/guide.cds.agent.sdk-cds-map-mvp.md`。
...
A: 不是替代，是并行。LLM Gateway 处理"业务代码自己编排"的场景；Claude SDK 处理"让 Claude 自主决定调用哪些工具、调几次"的场景（多轮 tool_use 循环）。详见 `doc/design.cds.agent.sdk-executor.md` §2。
...
- `doc/design.cds.agent.sdk-executor.md` —— 完整架构设计
- `doc/guide.cds.agent.sdk-cds-map-mvp.md` —— CDS → MAP 外部 sidecar 调度 MVP 冒烟
- `doc/debt.cds.agent.sdk-executor.md` —— 已知边界 / 工程债务
