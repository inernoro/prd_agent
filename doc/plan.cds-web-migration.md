...
> **作者**：Claude (Opus 4.7) · **更新**：2026-05-01（Week 4.9 多项目网络隔离）
...
6. [x] **ProjectSettingsPage + CdsSettingsPage 套壳 + 三大类扁平化**：使用 AppShell + TopBar；TabsList 在 trigger 之间渲染分组标题，把 7-8 个并列 tab 重组成 3 个语义大类（接入 / 运行时 / 危险区或维护）。
7. [x] **BranchTopologyPage 套壳**：使用 AppShell + TopBar + surface tokens；统计内联到 TopBar。React Flow 升级仍待用户确认。
8. [x] **全局视觉残留清理**：所有页面里 `rounded-md border border-border bg-card` / `bg-muted/{20,30,40}` 堆叠通过批量 sed 替换为 `cds-surface-raised cds-hairline` / `cds-surface-sunken cds-hairline`，与新视觉语言保持一致。按钮颜色权重审计留作下一轮。
...
### Week 4.7：部署阶段树 + Active/History 分流

**为什么**：Week 4.6 第九刀完成后，分支详情抽屉的部署 tab 仍是 9 条平铺 + inline 红色错误块的 Railway 反例。用户对比 Railway 截图明确想要"阶段树 + 当前部署吃满主区 + 历史折叠成一行 + 失败诊断 CTA"。

**目标**：
- [x] 抽 `cds/web/src/lib/deploymentPhases.ts` 纯函数：log + finalStatus → 4 阶段（拉取代码 / 构建镜像 / 启动服务 / 健康检查）状态树；保守降级 + 失败传播 + errorMessage 注入
- [x] 新组件 `cds/web/src/components/deployment/PhaseTree.tsx`：Railway 风格的阶段树，左 16px lucide icon + label + duration + 失败诊断 CTA slot
- [x] 新组件 `cds/web/src/components/deployment/ActiveDeployment.tsx`：顶部状态徽章 + commit + kind + duration；中间 PhaseTree（minHeight 168px 防 running→success 抖动）；失败时按阶段挂诊断 CTA（build 缺 BuildProfile → 「修复构建配置」跳设置；build 通用 / deploy / verify → 「查看完整日志」「重置异常」「重新诊断」）；底部固定行「复制排错摘要」「查看完整日志」
- [x] 新组件 `cds/web/src/components/deployment/HistoryRow.tsx`：单行折叠（dot + kind + sha + duration + 相对时间 + 展开 + 日志按钮），展开后渲染同款 PhaseTree
- [x] 改 `cds/web/src/components/BranchDetailDrawer.tsx` deployments tab：`combinedDeployments = visible + legacyLogToDeploymentItem(logs).slice(0,6)` 走单一通道；`pickActiveDeployment(items, now)` 选 running 优先 / 60s 内结束 / 否则最新；active 渲染 ActiveDeployment 大卡，剩余渲染 HistoryRow，默认 5 行 + 「显示全部」展开
- [x] 保留 `DeploymentCard / LegacyDeploymentCard` 顶层函数（导出避免 noUnusedLocals 警告，供后续可能引用）
- [x] BranchDetailPage 部署日志渲染评估后不动（重复程度低，OperationLog 直接用，迁过来反而牵涉过多）

**完成判定**：
- 切到「部署」tab，顶部一张大卡承载 ActiveDeployment（4 阶段树 + 状态徽章 + 操作行）
- 失败状态下，红行下方有诊断 CTA（构建配置缺失走「修复构建配置」跳 `/settings/<projectId>`）
- 历史段折叠到 5 行 HistoryRow，可点「显示全部」展开
- `pnpm tsc --noEmit` + `pnpm build` 在 cds/web 与 cds 两侧全绿

**改动文件**：
- 新增 `cds/web/src/lib/deploymentPhases.ts`
- 新增 `cds/web/src/lib/__tests__/deploymentPhases.spec.ts`（vitest 待接入，债务记录）
- 新增 `cds/web/src/components/deployment/PhaseTree.tsx`
- 新增 `cds/web/src/components/deployment/ActiveDeployment.tsx`
- 新增 `cds/web/src/components/deployment/HistoryRow.tsx`
- 改 `cds/web/src/components/BranchDetailDrawer.tsx`
- 改 `doc/plan.cds-web-migration.md` / `doc/guide.cds-web-migration-runbook.md`
- 新增 `changelogs/2026-04-30_cds-week-4-7-phase-tree.md`
...
### Week 4.9：多项目 docker network 隔离（CDS 重构动机之一）

**为什么**：CDS 全部项目的容器一直跑在同一个 docker network（`cds-network`），跨项目容器互通——这是用户在 CDS 重构中明确想修的根本痛点。`Project.dockerNetwork` 字段 P4 Part 18 在 createProject 时已自动赋值为 `cds-proj-<id>`，但 ContainerService 没用它（注释里坦白的"pre-P4 shared network until P4 Part 3 threads projectId through"），所以新建项目实际上还是共享网络。本节是 P4 Part 3 的"穿透 projectId" 收口。

**Round 1 — ContainerService 接入 ProjectNetworkResolver（一次到位）**:

- [x] `cds/src/services/container.ts` 新增 `ProjectNetworkResolver` 接口 + 构造函数可选参数注入；新增私有 helper `getNetworkForProject(projectId)`（三个兜底点合一：未传 projectId / resolver 缺失 / project.dockerNetwork 字段为空 → 全部回退 `config.dockerNetwork`）
- [x] `runService` 用 `entry.projectId` 取网络；`startInfraService` 用 `service.projectId` 取网络；`appLabels` / `infraLabels` 接收 network 参数让 `cds.network=` 跟随实际网络名
- [x] `ensureNetwork` 升级为 `ensureNetwork(network?: string)`，按需 inspect/create 项目专网
- [x] `discoverInfraContainers` / `discoverAppContainers` 移除 `cds.network=` filter（多项目隔离后 filter 一个 network 漏掉跨项目容器），只保留 `cds.managed=true` + `cds.type=infra/app`
- [x] `cds/src/index.ts` `new ContainerService(...)` 注入轻量 resolver 适配器（直接读 `stateService.getProject(id)?.dockerNetwork`，避免循环依赖）

**Round 2 — StateService 启动 backfill**:

- [x] `cds/src/services/state.ts` `load()` 加一个新 migration `migrateProjectDockerNetworks()`：扫所有非 legacy 项目，缺 `dockerNetwork` 字段的统一回填 `cds-proj-<id>`；legacy default 项目跳过（保护其下 pre-P4 容器跟现有共享网络的连接，否则会让 mongodb / redis 失联）
- [x] Idempotent：每次启动都跑，已有 dockerNetwork 字段的项目跳过

**Round 3 — 测试**:

- [x] `cds/tests/services/container-network-isolation.test.ts` 6 个 case：项目 A 用 cds-proj-A、项目 B 用 cds-proj-B、老项目（resolver 返回 undefined）走 config 兜底、无 resolver（向后兼容）走 config、ensureNetwork 创建项目专网、infra 容器同样按 service.projectId 选 network
- [x] 现有 `cds/tests/services/container.test.ts` 不破坏（旧 fixture 不传 resolver → 走 fallback 分支，断言 `--network cds-network` 仍然成立）
- [x] `pnpm tsc --noEmit` 通过；vitest 在沙箱权限受限暂未本地跑，依赖 CDS 远端编译验证

**完成判定**:
- 用户在 CDS 创建一个新项目 X，部署一个分支 → docker inspect 容器看到 `--network cds-proj-X`
- 项目 X 的容器 ping 不到项目 Y 的容器（隔离生效）
- legacy default 项目原有分支重启 → 仍走 `cds-network`（向后兼容）
- 启动日志看到 `migrateProjectDockerNetworks` 给老项目 backfill 了 `cds-proj-<id>`

**改动文件**:
- `cds/src/services/container.ts` — 8 处使用 dockerNetwork 改用项目级 network；新增 resolver 接口
- `cds/src/services/state.ts` — 新增 migrateProjectDockerNetworks
- `cds/src/index.ts` — 注入 networkResolver 适配器
- `cds/tests/services/container-network-isolation.test.ts` — 新增
- `changelogs/2026-05-01_cds-network-isolation.md` — 新增
...
| 2026-04-30 | Week 4.7 — 部署阶段树 + Active/History 分流 | Claude (Opus 4.7) | 待填 | BranchDetailDrawer 部署 tab 升级到 Railway 心智：抽 `deploymentPhases.ts` 纯函数（log + finalStatus → 4 阶段状态树，保守降级 + 失败传播 + errorMessage 注入）；新增 `PhaseTree / ActiveDeployment / HistoryRow` 组件；deployments tab 改为顶部 active 大卡（状态徽章 + 阶段树 + 失败诊断 CTA）+ 历史折叠 5 行 + 「显示全部」；保留旧 `DeploymentCard / LegacyDeploymentCard` 函数为 export 避免 noUnusedLocals。Vitest 在 cds/web 未配置，单测以 .spec.ts 形式留档作为债务 |
| 2026-05-01 | Week 4.9 — 多项目 docker network 隔离 | Claude (Opus 4.7) | 待填 | ContainerService 接入项目级 dockerNetwork：注入轻量 ProjectNetworkResolver 适配器,8 处 docker 命令改用 `entry/service.projectId` 查 `project.dockerNetwork`(默认 `cds-proj-<id>`),老项目缺字段时 fallback 到 `config.dockerNetwork` 共享网络保持兼容；ensureNetwork 升级为按需创建项目专网；discover* 不再 filter `cds.network=` 以发现跨项目容器；StateService 新增 `migrateProjectDockerNetworks` 启动 backfill 非 legacy 项目；新增 6 个测试 case |
