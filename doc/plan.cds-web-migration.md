...
4. [ ] **BranchListPage 切片**：保留 AppShell；首屏改为「左侧分支资源列表 + 右侧选中分支主工作区」的 Railway service-canvas 形态；现有"运维栏（容量/主机/执行器/活动流/批量）"全部进入"运维抽屉"二级入口，默认不展开。
5. [ ] **BranchDetailPage 切片**：首屏只保留状态卡 + 服务卡 + 主操作（部署 / 预览 / 打开仓库）；7 个 DisclosurePanel 折叠成 4 个二级 tab（诊断 / 配置 / 历史 / Bridge）。
6. [ ] **ProjectSettingsPage + CdsSettingsPage 切片**：把 7+ tab 重组成 3 个语义组（接入 / 运行时 / 危险区），每组里再分子项。验证用户语义"3 秒内能找到要改的东西"。
7. [ ] **BranchTopologyPage 切片**：套用 AppShell + surface tokens；视觉层面与分支页一致；React Flow 升级与否由用户确认后再做。
...
| 2026-04-29 | Week 4.6 — ProjectListPage 切片 | Claude (Opus 4.7) | 待填 | 顶部 hero「粘贴 Git URL → 创建」收敛为唯一主操作；项目卡改 Railway-style 极简卡（状态点 + 标题 + 仓库 + 内联指标 + 进入按钮 + ghost 工具栏）；自动化工具（技能包 / 全局 Key / Agent 申请）下沉到底部折叠面板 |
