...
### 2026-06-14 校验+自动接线+自愈闭环落地

- **后端闭环**：AI 生成的工作流自动规范化插槽、按 dataType 自动接线、结构错误回喂 LLM 自愈最多 2 轮
- **自动校验卡**：对话气泡新增校验状态、自动接线说明、待补齐配置/密钥项展示
- **SSE 事件修复**：`from-chat` SSE 事件用 `event: message` 包裹导致前端 `data.type` 分发失效问题修复
- **系统提示注入**：注入「暂未开放能力」清单（定时/Webhook 触发），引导 AI 改用手动触发
- **就地补齐表单**：待补项改为就地可填表单，填完一键「补齐并应用到编辑器」
- **一句话生成工作流**：列表页新增入口，描述需求 → 自动建流进画布并由 AI 生成
- **落库条件**：仅在结构校验通过时才落库，自愈仍失败的退回草稿不持久化残缺工作流
- **应用门禁**：结构无效时禁用「应用到编辑器」，有缺项时隐藏绿钮
- **自动接线改为补缺连线**：漏接一跳的处理节点从前序节点补上游，按插槽粒度处理多输入节点
- **历史加载竞态修复**：`auto-send` 与历史加载竞态不再相互覆盖
- **变量持久化**：画布保存补回 AI 缺项补齐填入的变量默认值
