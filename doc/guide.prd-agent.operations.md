...
**一句话**：桌面端与管理后台逐页面的功能、操作路径、界面布局和对应接口清单，供验收、写用例、前后端对齐。
**谁该读**：做验收的产品、写测试用例的测试、要对齐前后端行为的开发。
**读完能做什么**：按模块找到操作路径、界面布局和对应接口，逐条核对实现是否一致。
...
| 功能 | Desktop | Web | 备注 |
|------|---------|-----|------|
| **群组管理** | 是 完整群组功能 | 否 会话模式 | Web 使用个人会话替代群组概念，无多人协作 |
| **实时 SSE（群组）** | 是 afterSeq 断线重连 | 否 非群组模式 | Web 使用请求-响应式 SSE，无需群组级实时同步 |
| **技能系统** | 是 完整技能选择与执行 | 否 无 | Desktop 支持丰富的技能扩展 |
| **AI Anyway** | 是 支持 | 否 不支持 | Desktop 专属的 AI 辅助功能 |
| **自动更新** | 是 Tauri 自动更新 | N/A 不适用 | Web 端无需客户端更新机制 |
| **划词提问** | 是 支持 | 否 不支持 | Desktop 支持选中文本后直接向 AI 提问 |
| **深度链接** | 是 `prdagent://` 协议 | 否 不支持 | Desktop 注册自定义协议，支持从外部跳转打开指定会话 |
| **字体缩放** | 是 支持 | 否 不支持 | Desktop 支持用户自定义字体大小 |
| **调试模式** | 否 不支持 | 是 支持 | Web 管理后台提供调试面板，查看 API 调用日志等 |
| **测试模式** | 否 不支持 | 是 支持 | Web 管理后台支持测试模式，使用 Mock 数据或测试环境 |
| **主题切换** | 是 水波纹过渡动效 | 是 全局切换 | 两端均支持明暗主题切换，Desktop 使用 View Transition API 实现圆形水波纹扩散动画，Web 使用全局 CSS 切换 |
