...
### 2026-06-14 剪贴板健壮性与安装体验

- **健壮剪贴板工具**：新增 `lib/clipboard.ts`（async API + execCommand 兜底 + 真实成功反馈），统一快捷指令各处复制
- **非安全上下文修复**：复制到剪贴板在非安全上下文/内嵌浏览器下静默失败假报「已复制」问题修复
- **连接自检**：安装页新增「验证密钥」功能，探活密钥是否被服务器接受
- **错误提示优化**：扫码安装页按 error.code 区分过期/失效/已删除/网络错误，给出具体下一步与重试指引
- **FAQ 补充**：安装页新增「遇到问题」常见卡点 FAQ（iOS 15+/不受信任/分享表单/网络权限）
- **iCloud 流程修复**：配置复制失败时不再跳转 iCloud；自动复制被拦截时新增「已手动复制？点此打开 iCloud 模板」直达链接
- **手动兜底完善**：暴露「完整配置」JSON 可手动复制，自动复制被拦截时不再死胡同
- **换指令重置**：路由 `:id/t` 变化复用组件时重置 step/copyTried 并 remount 连接自检
- **verify 端点专用**：连接自检改走 `GET /api/shortcuts/verify`，校验口径与 Collect 一致（密钥有效+未禁用+未过期）
