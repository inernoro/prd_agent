...
## 7. P4：多项目真落地（**Part 1 + 2 已落地 ✓ 2026-04-13**, Part 3 待办）
...
- **Part 3（下个 session）**：给 `BranchEntry` / `BuildProfile` / `InfraService` / `RoutingRule` 加 `projectId` 字段 + 迁移；所有 API 加 project scope filter；`/api/projects/:pid/branches` 路由落地；前端按 projectId 隔离显示
