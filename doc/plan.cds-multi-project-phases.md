...
## 7. P4：多项目真落地（**Part 1 + 2 + 3a 已落地 ✓ 2026-04-13**, Part 3b 待办）
...
- **Part 3b（下个 session）**：routing + frontend scoping。新增 `/api/projects/:pid/branches` (+profiles/infra/rules) 路由作为旧路径的项目感知 alias；Dashboard 读 URL `?project=<id>` 参数并在所有 API 调用里带上；顶栏显示当前项目名
...
- [ ] 新增 `cds/src/middleware/project-scope.ts`：从 URL `:pid` 读 projectId，校验存在后注入 `req.project`
- [ ] 新增 `/api/projects/:pid/branches` (+profiles/infra/rules) 路由作为旧路径的 project-aware alias
- [ ] 前端 `index.html` 读 URL `?project=<id>` 参数并在所有 API 调用里带上，Dashboard 顶栏显示当前项目名
- [ ] Dashboard 里"切换项目"入口（返回 `/projects.html` 或下拉选择）
