...
## 8. P5:团队 workspace + 成员同步 · **未启动**(建议 2026-Q2 启动)

**2026-04-16 状态注记**:

- P4 已稳定 + FU-02 MapAuthStore 设计已出(`doc/design.cds-fu-02-auth-store-mongo.md`),具备启动条件
- 明确 **P5 前置依赖 = FU-02 落地** —— 团队 workspace 需要持久化的 User model,MemoryAuthStore 重启就丢,不能承载团队协作
- 推荐启动顺序:**FU-02 → P5** 一条直线推进,不要并行
