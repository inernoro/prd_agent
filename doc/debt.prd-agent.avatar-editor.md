...
- 新建 `doc/design.prd-agent.avatar-editor.md`（appname 用 `prd-agent`，与本台账一致；或并入某个既有的账户/个人设置 design 文档），覆盖：为什么需要沉浸式编辑器（同步生成体验差）、AI 生成预览确认前不落地的产品决策、后台任务化的原因（避免同步请求超时）、内容哈希对象键的取舍（防 CDN 脏缓存）。
