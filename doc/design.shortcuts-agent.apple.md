...
> **版本**：v2.0 | **日期**：2026-03-10 | **状态**：已落地

**一句话**：用手机分享菜单把各处收藏的链接一键收进平台，走独立轻量令牌认证，并预留后续触发能力。
**谁该读**：做手机端接入的工程师。
**读完能做什么**：说清一条分享链接怎么进入平台。

---

## 一、管理摘要

- **解决什么问题**：用户在各 App 中收藏的内容（抖音视频、公众号文章等）散落各处，无法统一管理和触发后续工作流
- **方案概述**：通过苹果快捷指令 + 独立轻量 scs- token 认证，用户从任意 App 分享菜单一键收藏链接到 PrdAgent，预留工作流触发能力
- **业务价值**：打通手机端内容采集入口，实现跨 App 统一收藏管理，为后续自动化工作流奠定基础
- **影响范围**：新增 ShortcutsController、user_shortcuts / user_collections / shortcut_templates 三个集合、prd-admin 管理页面
- **预计风险**：低 — 独立模块，不影响现有系统；token 泄露风险通过支持随时删除/重建缓解
...
| 方案 | 优势 | 劣势 | 判定 |
|------|------|------|------|
| A. 独立 scs- token | 轻量、不污染 OpenPlatform 应用列表、可独立管理生命周期 | 需自行实现 token 校验 | 是 采用 |
| B. 复用 OpenPlatform sk- | 认证链路已有 | 每创建一个快捷指令就多一个 OpenPlatformApp，概念混淆 | 否 否决 |
...
## 十一、剪贴板健壮性与连接自检（2026-06-14）

> 对应 changelog：`2026-06-14_shortcuts-clipboard.md`

### 11.1 剪贴板工具（lib/clipboard.ts）

提供健壮的跨环境复制能力，统一快捷指令安装页各处复制入口：

| 路径 | 场景 |
|------|------|
| Async Clipboard API（主路径） | 现代浏览器安全上下文 |
| `document.execCommand('copy')`（兜底） | 非安全上下文 / WebView 内嵌浏览器 |

只在真实成功时返回 true，不再假报「已复制」。iCloud 流程下自动复制失败时提供明确说明与手动复制入口（「已手动复制？点此打开 iCloud 模板」直达链接）。

### 11.2 连接自检（GET /api/shortcuts/verify）

安装完成后新增「验证密钥」一键自检：
- 专用端点 `GET /api/shortcuts/verify`，校验口径与 Collect 一致（密钥有效 + 未禁用 + 未过期）
- 非破坏性探活，不展示收藏内容
- 前端按 `error.code` 区分：已禁用 / 已失效 / 已过期 / 网络错误，给出各自提示与下一步

### 11.3 安装页增强

| 改动 | 说明 |
|------|------|
| 卡点 FAQ | 「遇到问题」常见卡点（iOS 15+/不受信任/分享表单/网络权限） |
| 错误提示修复 | 按 `error.code` 区分过期/失效/已删除/网络；不再永远显示「加载失败」 |
| 手动兜底诚实化 | 说明为何多走几步 + 补全手动添加步骤（请求体/头部/自检） |
| iCloud 影响说明 | 配置面板增加「不配=全员手动，配好=全站一键」说明 |
| 组件重置 | 切换快捷指令（路由 `:id/t` 变化复用组件）时重置 step/copyTried 并 remount 自检 |
