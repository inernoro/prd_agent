...
| 字段 | 内容 |
|---|---|
| 模块 | CDS 验收报告镜像库自动刷新（`CdsReportImportWorker` + `CdsReportSyncTargets`） |
| 状态 | open（首版已交付，2026-08-26；以下为已知边界） |
| 关联 | `prd-api/.../Services/CdsReportImportWorker.cs`、`.../CdsReportSyncTargets.cs`、`.../CdsReportImportService.cs`、`DocumentStore.CdsReportScope*` |
...
1. **只刷「库里已经存过的」那些报告，不主动去 CDS 拉新的。** 要刷什么不另记一套账——
   ...
   >（Codex review P1）。那三个范围字段已经删掉，少一处会漂移的状态。

1b. **一个库一轮最多刷 200 份**，超出的这轮不刷，但会在日志里说清截了多少
   （no-silent-caps）。上限是为了别让一个异常大的库把整轮拖死、顺带打爆 CDS。
2. **不替任何人新建知识库。** `ImportAsync` 会 find-or-create 一个属于某个用户的镜像库；
   后台任务如果替所有用户建库，等于替人做主。所以只枚举**已经存在**的 `cds-reports` 库并
   刷新——第一次同步仍由人手动触发。
3. **只在权威部署上跑。** 同一个 CDS 项目下所有分支预览共用一个 Mongo，每个分支预览也都
   跑着一份 MAP。不加闸的话 N 个分支会同时对 CDS 发起导入、往同一个共享库写同一批文档
   （既是对 CDS 的自我 DDoS，也让「谁写的」不可追）。判据走
   `DeploymentAuthority.CanRunSharedScheduledWork`——**不是** `IsAuthoritativeDeployment`：
   后者按契约只管通知，而 `ManageGlobalNotification=true` 是留给分支「临时接管全局告警行」
   的逃生阀；一个为了看告警而打开它的分支，不该顺带获得对共享库和 CDS 跑周期拉取的权限。
   软开关只能收紧、不能放宽，这是 `DeploymentAuthority` 自己已经立过的纪律。
...
- [ ] 给老库一次性回填范围记录（按 `PeerSyncLastAt` 判全量），免去「手动导入一次」这一步
- [ ] 自动与手动导入互斥（按库粒度的同步锁）
