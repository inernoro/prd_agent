...
**是什么**：条目能读（`LoadReadableEntryAsync`：公开库、团队可见库、观察者）不等于能写
（`LoadWritableEntryAsync`）。录音结果页现在只要有笔记就把保存回调传下去，于是只读访客
看得到逐句校对与改说话人，点了保存必然被后端拒。

**为什么不在这一轮修**：这不是录音页独有的——知识库主页的 `DocBrowser` 同样无条件传写回调，
两处一起才算修完；而正确的修法要后端**明确下发一个可写能力**（现在 `GET entries/{id}` 只回
条目本身，前端拿不到「我能不能写」）。拿「是不是库主」去猜会错杀团队协作者：他们不是库主，
但确实可写。加一个能力字段属于新的接口契约，按 `CLAUDE.md` §5.5 记成 B 类，不在本 PR 展开。

**修法**：`GET entries/{entryId}` 一并返回 `canWrite`（由 `CanWriteStoreAsync` 算），
录音结果页与 `DocBrowser` 都据此决定传不传写回调——不传即只读，这是 `DocBrowser` 已有的约定。
...
| 没有原文时「导出」仍可点 | 点了得到一份空文件。属于边界体验，不影响主路径 | 顺手做体验打磨那一轮 |
