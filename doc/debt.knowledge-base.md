...
机器上渲染生产组件本体，接线照抄唯一的生产调用方，只有台词与在途录音是 mock，双主题各截一版。
对照台复刻出的「转录前」形态与用户当天的线上截图逐像素一致，所以判断可信。

**第一轮只出了体检报告没动手，是误读了要求**；第二轮按设计稿把界面改到位，下表是逐条结果。

| # | 设计稿要求 | 结果 |
|---|---|---|
| 1 | 结束录音后自动推进三阶段（硬约束「绝不出现空白等待页」） | **已对齐**。录音链路本来就是自动的，问题出在界面：转录正在跑的时候，同屏既挂「正在后台处理」的横幅，又摆一个「转成文字」按钮——两句话互相打脸，而且那个按钮点下去会再发起一次转录。现在状态卡按 run 状态三选一：处理中给三阶段进度，失败给失败卡，没跑过才给手动入口 |
| 2 | 状态永远回答四句：音频是否安全 / 在做什么 / 还要多久 / 你能做什么 | **已对齐**。三阶段各自带阶段内百分比与进度条；「还要多久」按本次运行自己的耗时线性外推，算不出来就明说在积累数据 |
| 3 | 失败必须给 code / 时间 / 仍可用能力 / 重试方式并逐条渲染 | **已对齐**。四项分行渲染；自动重试期间显示倒计时并明说无需操作，不给一个点了没用的按钮 |
| 4 | 标题是业务标题 +「已保存到「某库」· 时长」 | **未做**。标题仍是录音文件名。业务标题要由整理环节命名，属产品行为改动，未擅自做 |
| 5 | 全稿无紫色，强调色只用于播放进度 / 可点击文本 / 当前句底色 | **已对齐**。播放器、词云、当前句、来源文件 chip 的紫色全部换成主题强调色 token（双主题）；未按设计稿写死 `#1F5EFF`——硬编码颜色没有浅色档，也会被双皮肤棘轮拦下 |
| 6 | 播放器吸顶 + 当前句大字 + 句序 +「下一句」预告 | **已对齐**（迷你条折叠未做）。原文往下滚时播放器与当前句一直在视野里 |
| 7 | 理解 / 纪要 / 待办 / 提问四分区 | **已对齐**。摘要一直存在笔记的「## 摘要」段里，只是此前没有任何界面读它；待办按 Markdown 任务列表这个**结构**提取，不靠标题措辞去猜 |
| 8 | 说话人显示句数与占比 | **已对齐**。占比按有说话人标签的句子算，无标签句子不进分母 |
| 9 | 点词后出现命中面板 | **已有，我第一轮说重了**。点词本来就会把词填进搜索框并列出命中行，只是没有设计稿那种「命中 / 全部」切换。这条按 B 类（有价值但扩范围）留着，不在本轮展开 |
| 10 | 转录前这一屏应被处理进度填满 | **部分**。三阶段卡撑起了上半屏；设计稿那块「原文逐句生成中」的实时预览需要把实时转写流接进这一屏，未做 |

**还欠什么**（都属于「需要新链路或产品决策」，不是调样式能补的）：

- 业务标题命名（第 4 条）与实时原文预览（第 10 条），后者要接实时转写流。
- 命中面板的「命中 / 全部」切换（第 9 条）。
- 挂载时把当前句滚到中心会连带把整页滚下去，用户一进来看到的不是这一屏的顶部。
- 知识库文件列表空状态、以及一个**没有任何调用方**的交互式转录弹窗组件里仍有紫色；
  前者不在这条交付链路上，后者是死代码，本轮都没动。
