...
用户选了另一条：单独立一个 `report:read`，只发给之后重新授权的连接。代价是得去重新授权
一次，存量凭据在那之前读报告会被拒——这是有意的。已落地：

- `connection-token-routes.ts` 里报告那两条改要 `report:read`（不再共用 `instance:read`；
  Bridge 仍留在 `instance:read`）。
- `DEFAULT_SCOPES` 加上 `report:read`，并且**授权页展示的、授权跳转签发的、token 端点回报的，
