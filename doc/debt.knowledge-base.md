...
1. **本改动之前建的老库，只有当年按全量导入过的才会被自动刷新。** 自动刷新照着每个库
   自己记下的导入范围重放（单条 / 单项目 / 全量都行），而范围是这次才开始记的。老库没有
   这个记录，只能靠间接证据判断——`PeerSyncLastAt` 只有「默认全量且零失败」的导入才回写，
   带过滤的从不写它，所以「有水位」约等于「被当作全量镜像用过」。既没范围也没水位的老库
   证明不了自己是什么，宁可不碰；**手动导入一次即登记范围，此后自动生效**。日志会说清
   本轮为什么没有可刷新的库，不留静默。
