...
MAP 去 CDS 拉报告时，用的是「系统互联」授权时拿到的那把长效凭据。核对 CDS 的鉴权入口后
确认：**那把凭据只在 Page Agent Bridge 那条路上被接受，报告接口不认它**。报告接口认的是
CDS 自己的静态访问密钥、项目级或全局的 Agent Key。

线上那台 CDS 已开启登录（不带凭据访问会被拒），所以：

- 每小时的自动刷新会**每一份都被拒**，一份也刷不进来；
- 手动点「保存到 MAP 知识库」走的是同一把凭据、同一个接口，**同样被拒**——这一条早于本次
  改动就存在，不是自动刷新引入的，但自动刷新把它从「偶尔点一次没反应」变成「每小时报一次错」。

两条出路，都涉及安全面上的取舍，需要人来定，不属于一次 review 修复能闭环的范围：

1. 让 CDS 把这把凭据在**只读取报告**这条路上也认下来。它本来就是「用户已批准、给 MAP 长期用」
   的授权，扩到读报告与它的用途一致；代价是动了 CDS 的鉴权面，而 CDS 是多方共用的。
2. 让 MAP 改用 CDS 的 Agent Key 去拉报告。不动 CDS，但要多存一把密钥，且推翻了当初
   「一次授权、一把凭据走到底」的设计前提。

**用户当日选定第 1 条并已实现**：CDS 侧把「这把凭据能到达哪些路由」收敛成一张显式的表，
在原有的页面代理之外加了两条**只读**的报告路由（列表 + 正文）。建报告、删报告、传附件以及
整个 CDS 管理面一律不开——外部系统是读者，不是作者。报告正文里的截图本来就是匿名可读的
（内容寻址、不可枚举），不必开。

**还有一步部署依赖**：这段改动在 CDS 自己的代码里，需要那台 CDS 更新到含此改动的版本才生效；
只把 MAP 这侧合进主干不够。在 CDS 更新之前，自动刷新仍会被拒。
