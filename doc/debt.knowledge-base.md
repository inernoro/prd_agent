...
| K-10 | **GitHub 目录同步的已知边界**：① 单次批量上限 50 个目录，超大仓库的目录清单在服务端截断（默认 1500 条，优先保留推荐目录及其祖先）；② 同步是**单层**目录、只认 Markdown，要覆盖子目录必须把子目录一起勾上；③ **每篇文档要打两次 GitHub 接口**（取内容、取最近修改时间），没有节流与退避：实测私有仓 53 篇耗时 379 秒，验收期间真实撞到过频率超限（能正确显示原因并可重试，但不会自动退避重排）；④ 同步失败也会刷新「上次同步时间」，按日调度要等次日才自动重试，中间只能靠界面上的「重试同步」按钮；⑤ 另外三个应用（PR 审查、项目路由、技术文档格式化）各自有一套 GitHub 授权入口，尚未迁到共用的连接中心，四份入口读写同一张表，改判据时要一起改。 | P3（③ 在大仓库上会升到 P2） | 用户反馈「同步很慢 / 经常超限」「子目录没同步」「失败后要等一天」，或再有第五个应用要接 GitHub 登录时 | open |
| K-11 | **L2 复测未覆盖的四项**（2026-09-10）：次日重复同步不 churn（去重键换成仓库内路径这件事只有代码与单测，没有跨日实证）、重复走向导的幂等、断开 GitHub 后已有订阅的长期行为、超大仓库截断提示（两个实测仓库都没触发截断）。前三项有单测或首轮大验收覆盖过，但都不是在最新构建上跨日实证的。 | P3 | 下次做这块的回归时优先补这四项 | open |
| K-12 | **GitHub 同步流程尚未编进知识库页面教程**（2026-09-14 Codex review 提出）：知识库页已有 `document-store-page-guide`，新增的「添加订阅 → GitHub 目录 → 连接并勾选目录」入口没有 `data-tour-id`，`DailyTipsController.BuildDefaultTips` 里的步骤也没覆盖它。既有教程锚点未被破坏（不构成 `onboarding-tips.md` 违规），但新用户跟着教程走不会遇到这条主流程。补它要决定教程步数与讲述顺序，属产品决策，故未在原 PR 展开。 | P3 | 下次动知识库页面教程时一并补，或用户反馈「不知道能从 GitHub 同步」时 | open |
| K-13 | **GitHub 目录同步期间页面不自动刷新**（2026-09-14 从 PR #1528 撤出）：一次目录同步可能跑几分钟（私有仓 53 篇实测 379 秒），期间子文档在后台一篇篇建出来，但知识库页面只在开启同步那一刻刷一次，用户要手动刷新才知道跑完没有。PR 里做过一版轮询（按来源类型批量取父条目 + 并回当前页 + 防重入），但它在连续四轮 review 里持续产出新缺陷（父条目被子文档挤出分页、乐观状态锁死重试按钮、请求量随目录数放大、批量请求失败即永久停轮询），按 CLAUDE.md §5.5 熔断条款整体撤出，另开 PR 重做。重做时该先定形态：轮询 vs 服务端推送（本仓库已有 SSE 与 Run/Worker 两套现成机制），而不是在页面里堆状态。 | P2 | 用户反馈「同步完了页面没动静 / 要手动刷新」，或做下一轮知识库同步体验时 | open |
| K-14 | **批量订阅的幂等靠「先查后插」，并发下不成立**（2026-09-14 Codex review 提出）：`subscribe-github/batch` 先查该空间已有的 GitHub 目录条目、再逐条插入，两个并发请求可以读到同一份快照并各插一条，产出重复的目录父条目（随后会导入重复子文档）。要真正做成原子的，需要 `document_entries` 上一条唯一索引（StoreId + github_owner + github_repo + github_branch + github_path，仅对 `SourceType=github_directory` 生效的 partial index），而本仓库禁止应用自动建索引（`no-auto-index.md`），须由 DBA 执行，故不在原 PR 内解决。当前暴露面：向导在提交期间禁用按钮，实际触发要靠两个并发客户端或手工调接口。 | P3 | DBA 执行索引变更时一并落地，或用户报告出现重复目录条目 | open |
| K-15 | **GitHub 二级限额未识别 + 计数回写失败无法自愈**（2026-09-14 Codex review 提出，范围熔断后记账）：~~① GitHub 的**二级限额**（secondary rate limit）会在 `X-RateLimit-Remaining` 仍非零时返回 403 并给 `Retry-After`，共用判据只看前者，会被说成「拒绝访问，请确认权限」而不是「等一会儿」~~ —— **已偿还（2026-09-14）**：`GitHubRateLimit` 把「403/429 且带 `Retry-After`」一并判为限额，等待时长优先取 `Retry-After`（主配额的 `X-RateLimit-Reset` 往往远晚于真正该等的时间），并加了「503 带 Retry-After 不算限额」的对照守卫。剩下的一半仍欠：② 批量订阅的空间计数回写若自身失败（条目已落库），重试时全部条目被判成「已订阅」、`created` 为空，计数回写被 `created.Count > 0` 跳过，`DocumentCount` 就一直偏小，除非之后某次目录同步恰好成功重算。第②条真实但触发面窄（要求条目全部插入成功、而计数那一次 UpdateOne 恰好失败），仍记账不在本 PR 展开。 | P3 | 用户反馈「明明有权限却说没权限」、或空间文档计数与实际条目数对不上时 | open |
| K-16 | **文档树只取首页 200 条，条目多了父条目会从树上消失**（2026-09-14 Codex review 提出，判为扩范围）：`DocumentStorePage.loadEntries` 固定 `listDocumentEntries(storeId, 1, 200)`（main 上已是如此，非本 PR 引入），树没有加载更多；后端排序是「文件夹优先 → CreatedAt 倒序」，而 GitHub 目录父条目至今 `IsFolder=false`（见下文结构修正待办），所以同步补进来的子文档比父条目新，条目总数一过 200，父条目就被挤出首页——用户从树上看不到它，也就点不到立即同步 / 暂停 / 删除。现有缓解：空间内的关键词搜索走 `all=true` 的独立查询（同样封顶 200 条），仍能按目录名找回父条目。正解是给文档树加分页或增量加载，属新语义类别、动的是知识库主页面，不在本 PR 展开；把父条目改成 `IsFolder=true` 会顺带缓解（排序上文件夹优先），但那是下文那条结构修正待办的一部分，需要数据迁移与同步 worker 回归验证。 | P2 | 单个空间条目数超过 200（一个 GitHub 目录同步 53 篇，四个目录就够）时，父条目从树上消失 | open |
| K-17 | **GitHub 令牌若会过期，没有续期路径**（2026-09-15 对抗审查提出）：连接建立时拿到的访问令牌只保存、不续期。GitHub 有两种应用形态：一种签发的用户令牌不过期（那这条就不存在），另一种可以开启「用户令牌会过期」，开了之后令牌几小时就失效，用户每天要重连数次。真过期时不会静默错乱——同步会失败并提示重连，连接状态也会标成已失效——所以是体验问题不是正确性问题。**先要核实线上用的是哪种形态**，再决定要不要做续期。 | P3（若线上是会过期的那种则升 P2） | 核实应用形态时；或用户反馈「刚连过又要连」 | open |
| K-18 | **仓库搜索只在当前这一页里过滤**（2026-09-15 对抗审查提出，原 PR 已作为已知边界写明）：仓库列表是一页页取回来的，关键词过滤发生在取回来的这一页之内（翻页可以穷尽，不会漏掉仓库）。后果是：名下仓库很多时，想找的那个可能在第三页，得一页页翻过去才搜得到。正解是改用 GitHub 的仓库搜索能力，或在有关键词时把可访问仓库全量拉回再过滤；两者都要重新定并发与调用频率策略，故未在修复批里展开。 | P3 | 用户反馈「搜不到我的仓库」，或名下仓库超过两三页时 | open |
| K-19 | **申请的授权范围比「读文档目录」需要的大得多**（2026-09-15 对抗审查提出）：现在要的是「仓库」这一整档权限，按 GitHub 的定义它包含私有仓代码的**读写**，而本功能只需要读文件内容。传统 OAuth 应用无法把私有仓权限再细分（只读公开仓的那一档读不了私有仓，换它等于砍掉核心能力）。要做到最小权限只有一条路：改用 GitHub App 形态，按仓库授权、只给「内容：只读」。那是独立工程——要新建应用、改授权流程里的安装态处理、已连账号全部需要重连一次，且会连带解决 K-17（GitHub App 的令牌过期语义是显式的）。 | P2 | 做权限收敛、或安全评审要求最小权限时 | open |
| K-20 | **同步删除子文档时的级联清理不完整**（2026-09-15 对抗审查第四轮提出，判为扩范围）：远端删掉文档后，同步只删条目本身与它的历史版本快照；而知识库里正式的删除入口还会连带清掉同步日志、浏览记录、行内评论、Agent 运行记录、双链账本，以及带引用计数守卫的正文与带清理意图的附件。**这是既有缺口**（main 上远端删掉任何一篇文档时就已如此），本次未改变「每条删什么」，但让整目录删除也走上这条路，放大了影响面：一次删掉几十篇就会留下几十份孤儿记录。正解是把正式删除入口那套级联抽成两边共用的一份，但它落在知识库删除契约那一侧、不在同步这一侧，且要先回答一个设计问题——后台同步撞上「附件清理意图失败」时该怎么办（正式入口是回 503 让用户重试，后台没有用户可回）。属新语义类别，按 §5.5 熔断记账，不在本 PR 展开。 | P3 | 做知识库删除契约统一时；或发现孤儿记录堆积、被删文档的评论/双链仍然可见时 | open |

| K-21 | **存量子条目的路径推断假定 ref 只占一段**（2026-09-15 对抗审查第五轮提出，判为证据不足）：早期子条目没单独存路径、只存了地址，认亲要从地址里抠出仓库内路径，而抠法是固定跳过一段 ref；ref 若含斜杠（`feature/foo`）就会抠偏一段，认不上亲→原件被当成远端已删除、连历史版本一起没。**但这条路走不通**：能带斜杠的 ref 是从「勾选目录一键同步」那条入口进来的，而在它上线之前，子条目就已经开始随身携带自己的路径了——两个时间窗不重叠，于是「没带路径 + ref 含斜杠」的条目造不出来。真要加固，正解不是把抠法修得更聪明（下一种形状照样抠偏），而是把「认不出路径的存量条目一律不删」坐实成硬约束，现在那句承诺只覆盖了「一个键都没有」的条目。 | P3 | 改动认亲键的算法时；或发现子条目开始不再随身携带路径时 | open |
| K-22 | **「本来就没有可撤销的东西」被当成「已收回授权」**（2026-09-15 对抗审查第五轮提出，判为扩范围）：断开连接时若本地压根没有记录，接口报的是「无需撤销」，而契约字段把它和「确认已撤销」归成同一档。真正会出问题的序列是：上一次断开撤销没成功但本地记录已经删了，用户再点一次断开——这一次我们手里没有任何凭据，既撤不了也查不了。**当前界面不因此说谎**（那一次只说「这里已经没有连接了」，不声称 GitHub 那边已收回；而且撤销失败的那一次已经把「去 GitHub 手动移除」的提示给过用户了）。要做到第二次也如实提示，必须分得清「从没连过」与「上次删了但没撤成」——那需要新存一份撤销状态，属新语义类别。 | P3 | 做授权生命周期（含令牌刷新 K-17）时一并处理 | open |
| K-23 | **断开与重新连接之间仍有一个无法用重读消除的窗口**（2026-09-15 对抗审查第六轮提出，判为扩范围）：断开会钉住刚读到的那一条去删，删成功就报「已断开」；若用户在另一个标签页重新授权，而那次写入恰好落在「删除返回」与「方法返回」之间，这一侧不会察觉，界面会说已断开，而新连接其实已经建好。已处理的是另一半（删除条件没命中时回读一次，报「被替换」），这一半靠回读消不掉——回读之后写入照样能落进来。真正的解法是把同一用户的「写入连接」与「断开连接」串起来，或者存一份世代号/墓碑让重连不可能挤在两步之间，都要新增持久化语义。**后果是界面标签短暂不准**（下一次进页读一次真实状态就对了），不丢数据、不影响新连接可用。 | P3 | 做授权生命周期（K-17 令牌刷新、K-22 撤销状态）时一并处理 | open |
