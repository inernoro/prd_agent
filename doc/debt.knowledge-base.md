...
| K-15 | **GitHub 二级限额未识别 + 计数回写失败无法自愈**（2026-09-14 Codex review 提出，范围熔断后记账）：① GitHub 的**二级限额**（secondary rate limit）会在 `X-RateLimit-Remaining` 仍非零时返回 403 并给 `Retry-After`，共用判据 `GitHubRateLimit.IsExhausted` 只看前者，这种情况会被说成「拒绝访问，请确认权限」而不是「等一会儿」；② 批量订阅的空间计数回写若自身失败（条目已落库），重试时全部条目被判成「已订阅」、`created` 为空，计数回写被 `created.Count > 0` 跳过，`DocumentCount` 就一直偏小，除非之后某次目录同步恰好成功重算。两条都真实但触发面窄，且本 PR 的 review 修复提交已达 §5.5 熔断线（8 个），故记账不在本 PR 展开。 | P3 | 用户反馈「明明有权限却说没权限」、或空间文档计数与实际条目数对不上时 | open |
