...
### 一·补三：错误码在转异常时被丢掉（七处待迁）

**是什么**：网关失败时会给一个结构化原因（是「配置没配好」还是「上游暂时抖动」），
业务层据它决定跟用户怎么说、要不要自动重试。把网关的错误块转成异常这件事，
各处 Worker 各写各的——录音、再加工、Vision 这三条已经改成统一带码，
另有七处（生图、竞技场、模型实验室、平台连通性测试、多图合成、图片描述、模型域服务）
仍然只搬文案、丢掉码。

**为什么先不动**：它们的失败文案链路目前不读这个码，改造前要逐个确认下游怎么用这句话，
贸然改会动到用户可见文案。已用守卫钉住：新写的调用点一律会红，这七处显式列在缓迁清单里，
且清单不许含已经改好的文件（改好了不删就退不回去）。

**验收标准**：这七条链路遇到「模型池没配」时，界面说的是配置问题、并且不做无意义的自动重试；
缓迁清单清空，守卫回到零例外。
