...
| K-10 | **GitHub 目录同步的已知边界**（2026-09-09 交付，2026-09-10 L2 复测 pass 后更新）：① 单次批量上限 50 个目录，超大仓库的目录清单在服务端截断（默认 1500，优先保留推荐目录及其祖先）；② 同步是**单层**目录、只认 `.md`，要覆盖子目录必须把子目录一起勾上；③ **每篇文档要打 2 次 GitHub API**（内容 + 最近提交时间），没有节流与退避：实测私有仓 53 篇耗时 379 秒，复测期间真实撞到过频率超限（已能正确显示原因并可重试，但没有自动退避重排）；④ 同步失败会刷新 `LastSyncAt`，按日调度要等次日才自动重试，中间只能靠「重试同步」按钮；⑤ 老的三处 Device Flow 端点（pr-review / project-route-agent / tech-doc-format-agent）尚未迁到共用连接中心 `/api/github/*`，四份端点读写同一张表，改判据时要一起改；⑥ ~~远端目录被清空（一篇 Markdown 都不剩）时旧子文档不会被删~~ —— **已偿还（2026-09-15）**：列目录拿到 0 篇时曾直接 return、跳过整个删除环节，现在「空」和「少了几篇」走同一条路径：列目录成功而一篇不剩，就按「远端删光了」处理。 | P3（③ 在大仓库上会升到 P2） | 用户反馈「同步很慢 / 经常超限」「子目录没同步」「失败后要等一天」，或再有第五个应用要接 GitHub 登录时 | open |
