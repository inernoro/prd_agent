...
## 录音词云的分词质量 — 部分偿还（2026-08-10）

**现状**：中文没有空格，词云按 2 字滑窗切词。零依赖、打开页面即用，代价是滑窗必然
产出「交付 + 质量 → 付质」这类骑在两个真词中间的碎片。

**这轮做了什么**：先把展示层的真问题治了——权重从「按排名」改成「按频次」，
并在词云开头给一句结论。分词侧换了思路：不再按词形猜哪个是碎片（试过两版，
第一版把真词「质量」顶掉了，第二版对「考图 / 如说 / 后第」这类无右侧锚点的碎片无效），
改成**按位置抢座**：滑窗时记下每个组合出现在第几个字上，频次高的先占字、
同频次的位置靠前的先占，两个字里有一个被占过就判定是碎片。停用词也参与抢座，
否则「一个东西」里的「个东」没人挡。真实转写实测：18 个词里的半截词从 9 个降到 0。

**还欠什么**：抢座只解决「同一串字里谁是词」，解决不了「这两个字合起来到底是不是一个词」。
频次相同且互不重叠的两个碎片仍可能同时留下（真实语料里很少见，本轮未观测到）。
另外虚词字表是手工维护的，新语料出现表外的虚词字仍会漏。

**根治方向**：换真分词器（jieba 类）。届时抢座、虚词字表、停用词表三者一并删除。
---
