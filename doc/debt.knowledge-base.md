...
### 待你定：这次放开报告只读，用的是老授权，没让你重新点头

**这条不影响合并，但影响你什么时候更新那台 CDS——更新之后才真正生效。**

现在的做法是：已经授权过的外部系统本来就带着一个「读这台实例」的范围，这次直接把
「读验收报告」算进那个范围里。好处是你什么都不用做；代价是**当初点授权时，页面上并没有
说这把钥匙以后能读所有项目的验收报告正文**，等于事后扩大了那次同意的含义。而且连接凭据
不带项目限制，一旦放开就是**跨所有项目**都能读。

另一种做法是单独立一个「读报告」的范围，只发给之后重新授权的连接。更干净，代价是你得去
「系统互联」把 CDS 重新授权一次，否则这个功能仍然不通。

当前这套环境里，那个「外部系统」就是你自己的 MAP，实际风险很低；但这是你的同意，不该由
我替你扩大。要改成第二种做法，代价只是你多点一次重新授权。
