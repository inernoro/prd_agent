...
- **已关闭（2026-08-03，见 `changelogs/2026-08-03_r2-recording-archive.md`）**：修复真实录音大小上传 Cloudflare R2 时的签名不一致，并将代表性音频写读删纳入就绪门禁；同步阻止用小文本探针或仅检查请求参数替代真实录音对象存储验收，避免同类回归被弱验收放过。
