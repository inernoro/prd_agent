...
| K-17 | **GitHub 令牌若会过期，没有刷新路径**（2026-09-15 对抗审查提出）：Device Flow 拿到 token 后只存不续——`GitHubAccessTokenResponse` 里的 `expires_in` 读的是 **device_code** 的有效期，不是 token 的；`refresh_token` 字段根本没解析。传统 OAuth App 发的 user token 不过期，这条就不存在；但如果线上配的是 **GitHub App 且开启了「过期用户令牌」**，token 8 小时就失效，用户每天要重连数次。真过期时不会静默错乱（同步会失败并提示重连，状态接口现在也会把它标成 revoked），所以是体验问题不是正确性问题。**先要核实线上 `GitHubOAuth:ClientId` 对应的是哪种应用**，再决定要不要做刷新。 | P3（若线上是会过期的 GitHub App 则升 P2） | 核实应用类型时；或用户反馈「刚连过又要连」 | open |
| K-18 | **仓库搜索只在当前这一页里过滤**（2026-09-15 对抗审查提出，原 PR 已作为已知边界写明）：`ListRepositoriesAsync` 按 `user/repos` 分页取回后在本页内按关键词过滤，`HasMore` 按过滤前条数算（翻页可穷尽），但用户输入关键词时看到的只是当前页的命中——名下仓库很多时，想找的那个可能在第三页，得一页页翻过去。正解是改用 GitHub 的仓库搜索接口（`GET /search/repositories` 带 `user:` 限定），或在有关键词时把可访问仓库全量拉回再过滤；两者都要重新定并发与限额策略，故未在修复批里展开。 | P3 | 用户反馈「搜不到我的仓库」，或名下仓库超过两三页时 | open |
| K-19 | **申请的 OAuth scope 是 `repo`，比「读文档目录」需要的权限大得多**（2026-09-15 对抗审查提出）：GitHub 对 `repo` 的定义包含私有仓代码的**读写**，而本功能只需要读 Contents。传统 OAuth App 无法把私有仓权限再细分（`public_repo` 只覆盖公开仓，读不了私有仓，换它等于砍掉核心能力）。要做到最小权限只有一条路：改用 **GitHub App**，按仓库授权、只勾 Contents: Read。那是独立工程——要新建应用、改 Device Flow 的安装态处理、已连账号全部需要重连一次，且会连带解决 K-17（GitHub App 的令牌过期语义是显式的）。 | P2 | 做权限收敛、或安全评审要求最小权限时 | open |
