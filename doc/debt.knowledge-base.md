...
兜住了静态阅读场景，但流式改写时 `content` 每个 token 都在变 —— 实测一次 60 帧的改写里，
正文产生 700 次 DOM 变更、`<ins>` 元素被重建 278 次（文档里最多只有 9 个）。

**已经付过的代价**：进场动画被无限重启，早已写完的新增文字在整个流式过程中不停闪烁，
用户的原话是「一闪一闪，就像是老电脑一样，一点都不丝滑」。已用「流式期间不播逐元素动画」堵住可见症状
（`doc-diff.css` + `src/styles/__tests__/docDiffCascade.test.ts` 层叠守卫），
重挂本身还在，只是现在肉眼看不见了。

**还剩什么代价**：① 流式期间用户没法选中正文（原生选区每次重挂就被清空）；
② 长文档 + 每秒十几次全量重挂是实打实的 CPU 开销；
③ 文档里若有 Mermaid 图，每个 token 都会重新初始化一次，长文里会明显卡。

**为什么现在不修**：`MarkdownViewer` 是三处阅读页共用的组件，把 `components` 稳成模块级常量
需要把 slugger / 图片索引 / 主题这些实例态改走 context 或 ref，是一次有回归风险的重构；
而它当前的写法正是当初为修「划词选区自动撤销」而留的（见该文件 memo 处的注释）。
本轮用户问的是「为什么闪」，按范围熔断只修到症状消失为止。

**什么时候必须还**：出现下面任一条就不能再拖 ——
知识库正文开始大量用 Mermaid / KaTeX 等重初始化组件；或用户反馈「AI 写的时候选不中字」；
或流式改写要支持更长的文档（当前只改选区，重挂范围有限）。

**怎么验它还没修好**：`document.querySelectorAll('.doc-inline-diff ins')` 在流式期间用
MutationObserver 数 addedNodes，重建次数远大于元素总数就说明还在重挂。
