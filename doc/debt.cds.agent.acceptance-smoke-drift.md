# CDS Agent 验收 smoke 断言漂移 · 债务台账

> **版本**：v1.1 | **日期**：2026-06-26 | **状态**：开发中

**一句话**：一组冒烟脚本的断言已与实际行为漂移，整组退役而不是逐条修，本文记原因与后续处置。
**谁该读**：接手这批脚本的人；想知道为什么它们被停用的人。
**读完能做什么**：知道这组脚本为何退役，以及重建时该怎么设计断言。
...
按 §三「修复或退役」二选一，**整组退役**：
- 38 个 `scripts/smoke-cds-agent-*.sh` + `audit-cds-agent-goal.sh` 头部统一加 `# RETIRED 2026-07-09` 标注（不移动/删除文件，防历史引用路径断裂；`.github/` 与 package.json 均无 CI 引用，零风险）。
- 退役依据：这批是 2026-05 workbench 轮的一次性手工取证产物，断言链式漂移，逐条打补丁属 blocked-state-circuit-breaker 反模式；且已无人跑。
- 如未来需要同类校验：按 §三方案 2 用结构化锚点重写并接最小 CI，**不要**复活这批脚本。
