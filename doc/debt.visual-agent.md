...
| 边界 | 影响与完成条件 | 状态 |
|---|---|---|
| 环境目录配置 | 上线前必须至少有一个启用、场景匹配且上游处于授权范围内的业务模型；没有配置时显示明确空态，不回退到默认池 | 待环境验证 |
| 默认模型规则 | 复用管理端显示顺序，首个可解析且授权通过的业务模型作为默认；本轮不新增独立默认模型配置字段 | 已明确 |
| 真实生图恢复 | 已确认截图中的失败是无参考图的请求，不能归因于参考图格式；修正参数契约后，仍需以真实出图及产物解码确认，不能据此断言已经解释该次全部失败原因 | 待直连预览复验 |
| 定期监控 | 既有 48 小时任务已增加业务模型、实际调用一致性、真实出图及尺寸检查；任务已配置不代表首轮通过，受阻必须报告而非按接口健康判绿 | 待首轮有效执行 |
| 多尺寸进度验收 | 本轮不改前序进度条补丁；仍需在实际发布版本检查方形、横版、竖版与画布缩放后的可读性 | 待真实页面复验 |
