...
| 投稿与删除并发 | 投稿已统一执行私有策略，工作区删除也会清理存量投稿；但两条写链路尚未共享工作区级租约，极端并发下仍需事务化防止删除后出现迟到投稿 | 待补工作区发布/删除原子边界与并发回归 |
| 账户迁移与源删除并发 | 参考图迁移已使用按 SHA 租约，整工作区克隆尚未与源工作区删除共享资产租约；极端并发下需防止克隆引用已回收对象 | 待补工作区克隆/删除原子边界与并发回归 |
