...
## 业务模型目录与生图恢复验收（2026-08-31）

本轮将创作页选择对象改为网关业务模型，模型池仅作后台授权边界；同时修正 GPT Image 请求不支持的输出格式参数。代码验证不代表实际环境已经恢复。

| 边界 | 影响与完成条件 | 状态 |
|---|---|---|
| 环境目录配置 | 灰度已登记 GPT Image 1 与 GPT Image 2，只允许视觉创作三个场景，复用既有已授权上游；两种上游均真实出图并解码。正式环境须独立核对配置 | 灰度已验证，正式待验证 |
| 默认模型规则 | MAP 显式配置开放列表与默认模型，顺序只影响展示；网关只发布授权模型和技术能力。灰度开放顺序为 Image 2、Image 1，但默认明确为 Image 1；保存刷新、撤回已选型号后不替换、恢复配置均已验证。旧配置不会按模型池顺序自动迁移 | 灰度已验证，正式待验证 |
| 升级顺序与回滚 | 先部署支持模型目录和标准图片请求的网关，再部署 MAP 并保存业务策略；未配置时停止生成并给出配置提示，不静默回退。灰度实际运行 dbfeff9f 并完成真实出图；回滚应成对恢复 MAP 与网关兼容版本，保留策略数据 | 灰度已验证；正式部署不在本轮授权范围 |
| 场景边界 | 本轮收拢普通视觉创作的文生图和参考图链路；分层 PSD、视频、分镜等既有消费者不迁移到此业务策略。供应商不可用时不替换用户选择的业务型号 | 已明确 |
| 新模型策略监控 | 既有 48 小时任务增加显式默认、真实触控手机能力一致性与可解码产物断言。专用身份在当前分支合成登录返回 401，巡检预检也未通过 MAP 与网关身份检查；未轮换凭据或扩大白名单。实际验收使用已授权环境账号，不能替代专用身份巡检 | candidate；需修复目标环境专用身份并完成正式安全变体验证 |
| 真实生图恢复 | 已从网关原始日志确认截图请求被不支持的 response_format 拒绝；修复灰度经环境账号登录后，创作页使用 GPT Image 1 成功生成白桃并解码为 1024×1024 图片 | 灰度方图已验证，正式待验证 |
| 定期监控 | 既有 48 小时任务已增加业务模型、实际调用一致性、真实出图及尺寸检查；本次参数/业务模型和桌面视口逃逸已登记为 active 回归，计划测试断言双环境必跑，并补齐真实画布源码路径映射；任务已配置不代表首轮通过，受阻必须报告而非按接口健康判绿 | 待专用身份首轮有效执行，旧 JPEG/移动端 candidate 不冒充关闭 |
| 多尺寸进度验收 | 修复灰度真实生成 1536×1024、1024×1536、1024×1024；横版 210%、竖版 25% 与 46%、方形 100% 的运行中进度条均可见且可读，超出视口时正确贴合可见边缘 | 灰度已复验，正式待验证 |
| 默认模型尺寸能力 | 984b731 灰度已补齐 GPT Image 1 官方三个尺寸；真实菜单显示方形、横版、竖版，选择横版后生成 1536×1024 图片，26% 和 210% 下进度可读；控制器测试覆盖业务标识到尺寸响应 | 灰度已复验，正式待验证 |
| 画布工具栏遮挡 | 新链路复验发现缩放末次状态未同步、图片标题阻挡连续缩小；已补齐节流尾次同步并抬高固定缩放控件。dbfeff9f 灰度横图 100%/210%、竖图 100%/26% 连续点击和生成中刷新均通过，不再绕开遮挡区域验收 | 灰度已复验 |
| 实际执行型号 | 任务保留用户选择的逻辑型号，完成事件和消息另存网关回传的实际型号。灰度 image2 对应 gpt-image-2、image1 对应 gpt-image-1 均通过真实产物和日志核对 | 灰度已复验 |
| 参考图与手动缩放 | f22facbbc 灰度将 JPEG 字节以 PNG 声明上传，实际输入 MIME 为 image/jpeg，输出资产关联输入并真实生成 1024×1024 图片；生成开始立即点击 100% 不再被自动适配覆盖，完成与刷新保留均通过 | 灰度已复验 |
| 手机业务能力 | 真实 iPhone 13 触控上下文，390×664；模型设置保存按钮可滚动到达，第二项 Image 1 保持业务默认，尺寸来自目录。实际生成 1024×1536 竖图，重新进入工作区后解码和视觉回读均通过 | 灰度已复验；不代表物理键盘与安全区全机型覆盖 |
| 验收报告归档 | 七条业务路径和 21 张截图（含 6 张手机截图）完成本地检查与逐张回读。向 CDS 项目验收中心上传截图及报告的权限被拒绝，未尝试其他通道；线上归档与 verify-open 尚未执行 | 等待用户确认具体上传范围；不声称正式验收归档完成 |
| 自动投稿会话偏好 | 新建或重新进入会话后投稿开关可能重新开启，测试每次生成前都须显式关闭，不能假设偏好持久化 | 已知边界，测试产物未投稿 |
| 同步诊断接口超时 | 单次同步生图超过入口约 30 秒上限时可能返回 504，但上游随后完成；创作页异步任务已证明能正常回填，其他同步消费方需单独治理 | 已知边界，不影响本次异步创作路径 |
