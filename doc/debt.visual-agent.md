...
| 定期监控 | 既有 48 小时任务已增加业务模型、实际调用一致性、真实出图及尺寸检查；任务已配置不代表首轮通过，受阻必须报告而非按接口健康判绿 | 待首轮有效执行 |
| 多尺寸进度验收 | 真实页面确认 30% 与 100% 下进度可读，但 131% 时移出视口；本轮追加可见区域定位修复及多尺寸几何回归，发布后仍须复验方形、横版、竖版及平移 | 待修复版本真实页面复验 |
