...
| 默认模型规则 | 本次收拢为 MAP 显式配置开放列表与默认模型，顺序只影响展示；网关只发布授权模型和技术能力。旧配置不会按模型池顺序自动迁移，管理员需在视觉创作的模型设置中确认并保存 | 已实现，待新链路灰度验收 |
| 升级顺序与回滚 | 先部署支持模型目录和标准图片请求的网关，再部署 MAP 并保存业务策略；未配置时停止生成并给出配置提示，不静默回退。回滚应成对恢复 MAP 与网关兼容版本，保留策略数据 | 灰度待验证；正式部署不在本轮授权范围 |
| 场景边界 | 本轮收拢普通视觉创作的文生图和参考图链路；分层 PSD、视频、分镜等既有消费者不迁移到此业务策略。供应商不可用时不替换用户选择的业务型号 | 已明确 |
| 新模型策略监控 | 既有 48 小时任务增加显式默认与可解码产物断言；新策略回归暂记 candidate，只有真实灰度前后对照和正式安全变体验证后才能闭环 | 待验收与首轮有效执行 |
...
| 画布工具栏遮挡 | 图片标题或快捷操作在特定位置会覆盖缩放工具栏中部；验收通过未遮挡边缘完成操作，需后续统一层级治理 | 既有布局边界，不属于进度条定位修复 |
