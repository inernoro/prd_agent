...
- 已完成：R1/S1/S2/S3/V1 最小 provider-backed 闭环。默认 OpenRouter DeepSeek Claude Code profile 可通过 CDS-managed official SDK runtime 执行只读仓库巡检；危险内置工具没有审批、没有尝试、没有执行。
- 当前剩余：写文件、执行命令、跑测试、创建 PR 需要单独的 writable runtime profile 和人工审批策略；默认 profile 只声明为只读巡检可用，不能夸大成全自动改代码/PR。
...
| 02:03 | CDS 选到旧 `main` sidecar，并把 MAP runtimeProfileId 当成 sidecar profile，导致 `profile not found` | CDS 优先选择 `cds-managed-runtime` 上的 `claude-agent-sdk-runtime`，并把 baseUrl/apiKey/protocol 作为 request override 传给 sidecar，不泄露到 view | `cds/src/routes/remote-hosts.ts`、`cds/tests/routes/remote-hosts-instances.test.ts` | CDS route 8/8；CDS build | R1 profile 透传修复 |
| 02:09 | S1 能调到 provider，但 workspace 没准备，且模型尝试 Bash | MAP `StartAsync` 补传 `workspaceRoot/gitRepository/gitRef`；sidecar 默认 `disallowed_tools=Bash,Edit,Write`；S1 smoke 增加危险工具断言 | `1cf62ab02`、sidecar tests、MAP tests | sidecar 46/46；MAP 12/12；CDS 8/8 | workspacePrepared 修复；首次危险工具防线建立 |
| 02:15 | managed runtime 容器缺 `git`，workspace prepare 失败 | CDS runtime command 安装 `git ca-certificates`；workspace 错误结构化为 `git_not_installed` | `2b7209173`、CDS runtime branch exec `git --version` | runtime 冷启动约 46-48s | 真实 git workspace 可准备 |
| 02:20 | MAP `/messages` 同步等待 CDS/provider，真实模型超过 100s 会 500；MAP 事件也不会持续镜像 CDS stream | CDS `/messages` 改为 202 后台派发；MAP `ListEvents/ListMessages` 先同步 CDS stream 再返回 | `373c9d287`、CDS route 8/8、MAP 12/12、CDS build | self-update 32s，`tsc_cds=28440ms` | 控制面不再被模型耗时拖死；事件/消息可后续拉取 |
| 02:23 | S1 消息链路通过，但 SDK 仍尝试 2 次 Bash，虽被拒绝但不符合商业级安全边界 | sidecar 改为 CDS 显式工具白名单 `tools=Read,Grep,Glob`，默认 `--bare`，`setting_sources=[]`，并追加只读工具契约，避免目标仓库 `.claude/settings.json` 影响 runtime 权限 | `e0af6d2e8`、`pytest claude-sdk-sidecar/tests` | sidecar 47/47；self-update 37s，`tsc_cds=31204ms` | 默认只读 runtime 不再尝试 Bash/Edit/Write |
| 02:28 | S1 hardened provider run | 运行真实 OpenRouter/DeepSeek provider S1 | `/tmp/cds-agent-s1-provider-hardened.json` | 51s | pass：assistantMessages=1，workspacePrepared=true，dangerousApprovals=0，dangerousBuiltinTools=0 |
| 02:30 | 旧 S2 smoke 仍期待危险工具审批，但默认 hardened profile 已不暴露危险工具 | 修正 S2 语义：默认只读 profile 验证危险工具不会进入审批/执行；只有显式 writable profile 才跑人工审批拒绝 | `scripts/smoke-cds-agent-official-sdk-controls.sh`、`bash -n` | S2/S3 34s | pass：mode=`hardened-readonly`，S3 stop=`stopped` |
| 02:31 | 需要页面可见证据 | 运行 workbench visual smoke | `/tmp/cds-agent-workbench-visual.png`、`/tmp/cds-agent-workbench-visual.coverage.json` | 35s | pass：远端页面 HTTP 200，文本断言和截图通过 |
| 02:39 | one-cycle 首轮失败在 T1 静态模板断言：旧逻辑仍认为 DeepSeek 必须 incompatible | 修正 T1：DeepSeek/cc-switch Anthropic-compatible upstream 是允许路径；只把 raw Chat Completions endpoint 判为 incompatible | `scripts/smoke-cds-agent-profile-templates.sh` | T1 4s | pass |
| 02:41 | 进度面板脚本仍输出 `blocked_r1`，会再次误导用户 | 让 `print-cds-agent-current-progress.sh` 优先读取最新 one-cycle summary；commercialComplete=true 时覆盖 gate/task board/next step | `scripts/print-cds-agent-current-progress.sh` | <1s | 面板显示 `provider_smokes_passed`、`complete`、10/1/0、总 161s |
| 02:42 | 需要统一汇总证据 | 跑 provider one-cycle | `/tmp/cds-agent-cycle-hardened-current/cycle-summary.json`、`/tmp/cds-agent-cycle-hardened-current/evidence-index.md` | 161s | commercialComplete=true；R0/A0/R1/S1/S2S3/V1/N6 全部 pass；最慢 S1 58s、S2/S3 37s、V1 32s |
