...
> **版本**：v1.2 | **日期**：2026-09-09 | **状态**：严格设计请求策略合流回归通过，真实配对待验
...
### Codex 原生 Responses 接入（2026-09-09）

| 边界 | 当前行为 | 关闭条件 |
|------|----------|----------|
| 真实部署 | 新增原生流式路径已本地编译并通过针对回归；固定 Codex 运行镜像及 CDS 接入尚未部署验真 | CDS 真实任务完成工具写文件，模型与任务审计对应，独立验收通过 |
| 会话历史 | 仅接受完整输入历史和不在上游保存会话的请求；拒绝引用上游已存会话，防止越过 MAP 的任务隔离 | 当前合同保留；未来若支持上游会话引用，先补明确的所有权校验，不能直接放开 |
| 协议与既有策略 | 仅接原生 OpenAI Responses 上游；不转换为 Chat，不支持的协议或现有提示词策略明确失败 | 以实际部署模型和策略完成验收，不通过关掉保护换取通过 |

### 设计执行器的严格同条件比较

新网页任务可冻结 MAP 指定的模型、平台及采样/推理策略，供本地网关与 OpenDesign 执行器共同消费。限定独立复审及主树组合回归通过，仍需 CDS 真实验证，不能把旧作品追认为严格对照。

| 边界 | 当前行为 | 关闭条件 |
|------|----------|----------|
| 历史及未声明参数 | 历史任务不回填；新任务未声明的参数保持原执行器行为，不能证明相同默认值 | 严格配对明确冻结同一模型与平台、采样值及推理策略，并核对实际物理请求 |
| 模型能力证据 | 不支持或未知能力继续明确拒绝；既有成功请求和通用模板不是该平台能力证明 | 取得精确模型的可信能力记录及真实参数审计，不伪造能力或绕过 Key 门禁 |
| 省略推理参数 | 两路显式不请求推理输出，不能据此声称模型内部完全没有推理 | 真实请求证明两路没有悄悄重新加入相关参数 |
| 执行器输出上限 | 新任务由 MAP 显式省略执行器默认上限；旧任务不改写，不等同删除模型技术限制。当前精确模型及实际默认池成员均未登记输出上限 | 逐实际请求核对来源，不能将其他协议的默认值或未来模型配置外推为无限输出 |
| Claude 协议的推理参数 | 原生严格路径对尚未实现的转换提前拒绝，不静默删参；本批未实现新的 Claude 推理适配 | 实现与验证实际协议映射后，再开放对应严格参数；原始透传路径按既有合同处理 |

调用次数、Token 与费用上限由 Key 门禁治理，本批没有另建消费预算。超时、并发、隔离及审计仍是必须保留的运行保障。
