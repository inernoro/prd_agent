...
>
> **审计基线**：本文中的生产数字均为 `2026-07-11` 快照，不是实时状态。最新复核确认生产四类镜像、health、runtime gate 和 rollout ledger 均为 `d37c31f504d071896c3f260873d8b6c47a0b2ae4`。Phase 4 治理代码已进入生产；本分支正在关闭 GW registry caller 仍被 MAP 静态注册表拒绝的最后一处权威倒置。
...
| 当前边界 | 同提交受控证据覆盖四协议、3 个真实 appCaller 和 1 条 shadow，但尚无 vision、图片、ASR 的同提交真实样本；Phase 4 治理仅在本分支通过本地验证 | 不得用短文本成功推导全系统长期稳定，也不得把独立数据库解释成完全物理隔离 |
...
| 层次 | 当前事实 | 结论 |
|---|---|---|
| 代码与制品 | `origin/main=d37c31f504d071896c3f260873d8b6c47a0b2ae4`，同提交 CI 成功，四类不可变镜像均已产出 | Phase 4 代码与制品完成 |
| 生产发布 | 生产 `api / llmgw / llmgw-serve / llmgw-web` 均运行 `d37c31f50...`；runtime gate `passed=13 retained=2 blocked=0 waiting=0` | full-http、治理代码和维护发布证据继承门已生效 |
| 本分支 registry 修正 | 动态 appCaller 改按规范格式和 GW registry 路由，不再要求先写 MAP 静态常量；生产无费用治理验收脚本已加入 | 尚未合并和部署；部署前不得宣称外部新 caller 已可用 |
...
|---:|---|---|---|
| 1 | 合并并发布 registry 权威修正 | 当前新 caller 虽能写入 GW registry，执行层仍会因未在 MAP 静态常量中登记而返回 `APP_CALLER_INVALID` | 才能验证外部系统被动注册和独立配置 |
| 2 | 一次运行无费用生产治理验收 | scoped key、预算、并发、生命周期和主备已部署，但要用同一脚本给出生产行为证据 | 关闭 Phase 4 的“代码已部署但行为未验收”边界 |
| 3 | 一次执行 vision、图片、ASR 各一条真实请求 | 使用 `--skip-text-seeds`，不附带默认 chat/preview，不运行视频 | 才能宣称关键非视频能力具备同提交生产证据 |
...
| 是否已经达到“零惊吓” | 已消除直连、配置回退、数据串库、发布单点和主备切换风险 | Phase 4 尚未生产生效，保留期未启用，多模态同提交证据仍不足 |
...
4. “预算已配置”不等于生产已经原子预占。本分支已实现并发 reserve/settle/release，但上线并完成生产竞争测试前仍按旧能力处理。
...
|---|---|---|
| 生产发布提交 | `d37c31f504d071896c3f260873d8b6c47a0b2ae4`；`api / llmgw / llmgw-serve / llmgw-web` 镜像、health commit 和 rollout ledger 一致 | 当前版本身份可信；后续发布仍必须重新做三方一致性检查，不能把服务器源码目录当版本权威 |
| 下一候选提交 | `codex/llmgw-gw-registry-authority` 修正动态 appCaller 的运行时权威并增加批次验收工具 | 合并部署前，新外部 caller 仍会被静态注册门拒绝；现有 19 个生产 caller 不受影响 |
...
| 同提交受控请求 | `llmrequestlogs` 总计 1939 条，当前提交 15 条：13 条 `transport=http`，另 2 条为发布探针产生、缺少 source/ingress 元数据的内部记录；当前无媒体和视频调用 | 证明生产业务与协议 canary 经过 HTTP；2 条内部记录不能解释成 MAP 业务回退，也暴露探针 transport 标注仍需单列 |
...
| 生产治理 dry-run | 两台 serving 均报告 `mode=dry-run sensitiveLogs=0 expiredMultipart=0`；删除开关未设置，默认 false | 当前没有待清理命中，不能据此自动开启长期删除策略 |
| 临时验收清理 | `llmgw-acceptance*` caller/platform/model/pool 和启用 key 计数均为 0 | 前两次被静态门拒绝的无费用测试没有残留生产配置或日志 |
...
| 付费验证边界 | 最新同提交受控请求 8 条，均为低 token 文本；未触发图片、视频、ASR 付费验证 | 成本受控，但多模态真实证据明确缺失；后续每种昂贵能力必须有调用上限和停止条件 |

### 2.2 事实边界

- 上表是 `2026-07-10`、提交 `92fac961...` 的审计快照。实施阶段必须重新执行只读数据库、磁盘、容器、health 和日志查询，并把新时间写入证据。
...
| R06 | P1 | 代码已关闭，待生产 | 安全合规 | 月预算依赖日志估算、无原子预占；成本证据缺失时放行 | 并发越过预算或账单与网关估算不一致 | 已实现 Decimal128 原子 reserve/settle/release；8 路竞争只放行 1 路。生产部署与结算观测后关闭 |
...
| R18 | P1 | 未关闭 | 正确性 | 同提交受控证据以短文本为主，只有 1 条 shadow，缺少多模态真测 | 新 caller、多模态或低频路径可能在用户首次使用时才暴露问题 | 低成本文本用契约测试覆盖；vision、图片、ASR 各一次受控真测；视频单列预算并由用户启用后再测 |
...
| R21 | P1 | 代码已关闭，待生产 | 安全合规 | 月预算非原子，双 serving 同时承载流量会扩大并发超额窗口 | 两实例各自通过检查后同时消费，实际费用突破 appCaller 预算 | Mongo 原子预算预占已通过 8 路竞争测试；部署前继续主备，生产验收后才评估双活 |
...
| R23 | P2 | 代码已关闭，待生产 | 性能容量 | 平台 `MaxConcurrency` 目前没有形成跨实例原子并发门 | 上游并发限制可能被多个 serving 同时突破，触发限流、熔断和费用抖动 | platform/model 双层 Mongo 租约已实现；8 路双实例竞争只放行 1 路，释放后可复用 |
| R24 | P1 | 已缓解 | 运维可用性 | 不可变大镜像拉取仍使用固定 30 秒超时 | 网络稍慢即在真正部署前反复失败，形成“运行很久但生产没有变化”的假进度 | 本次通过长时限完成不可变镜像发布；通用脚本仍应把下载阶段与超时做成显式参数 |
| R25 | P2 | 代码已关闭，待生产 | 运维可用性 | 深度 `/readyz` 受服务 key 保护，公网无 key 返回 401 | 操作者可能把安全拒绝误判为实例不健康，或为了方便而错误开放 readiness | CDS/Compose 无凭据探针改用 `/healthz`；stage 带 key 调 `/readyz`；部署验证后关闭 |
| R26 | P1 | 修复中 | 架构边界 | serving 已把新 appCaller 写入 GW registry，但 `LlmGateway.TryValidateAppCaller` 仍要求命中 MAP 静态注册表 | 外部系统或新 MAP 功能完成被动注册后仍返回 `APP_CALLER_INVALID`，形成“数据归 GW、准入仍归 MAP”的双权威 | 运行时只校验 canonical code 和 modelType 后缀；GW registry 负责状态、池、预算和 scope；MAP 静态表只保留编译期守卫和展示元数据；生产动态 caller 验收通过 |
| R27 | P2 | 未关闭 | 可观测性 | 当前提交有 2 条发布探针日志标为 `inproc` 且 source/ingress 为空 | 操作者可能把内部探针误判为 MAP 业务回退，造成错误回滚判断 | 发布探针统一写 `sourceSystem=release-probe`、`ingressProtocol=gw-native`、`transport=http`，控制台可单独过滤 |
...
- 本分支包含 Phase 4 代码、Compose/CDS 探针和控制台改动；生产数字仍只记录已完成事实，不把本地测试写成生产能力。
