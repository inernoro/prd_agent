...
>
> **审计基线**：本文中的生产数字均为 `2026-07-10` 快照，不是实时状态。最新复核确认生产四类镜像、health、runtime gate 和 rollout ledger 均为 `92fac961c87b007849400b326b628cea77940632`。本分支新增的 Phase 4 治理代码尚未进入生产，实施下一阶段前必须重新取证。
...
| 层次 | 当前事实 | 结论 |
|---|---|---|
| 代码与制品 | `origin/main=92fac961c87b007849400b326b628cea77940632`，同提交 CI 成功，四类不可变镜像均已产出 | Phase 3 代码与制品完成 |
| 生产发布 | 生产 `api / llmgw / llmgw-serve / llmgw-web` 均运行 `92fac961...`；runtime gate `passed=14 blocked=0 waiting=0 retained=1` | Phase 3 生产切换与 gate 完成 |
| 本分支 Phase 4 | scoped key、原子预算、显式取消、raw 幂等、生命周期与分布式并发门已实现并通过本地测试 | 尚未合并、构建不可变镜像和部署生产，不得写成生产已生效 |
...
|---:|---|---|---|
| 1 | 合并并发布 Phase 4 治理提交 | scoped key、预算、取消、幂等、保留期和并发门只有进入同一生产提交才具备运行意义 | 执行生命周期 dry-run 与生产治理验收 |
| 2 | 先运行生命周期 dry-run，再开启敏感正文清理和 TTL | 直接开启删除可能误清理历史证据；dry-run 先给出命中数量 | 按保留策略启用 apply 并核对空间变化 |
| 3 | 为 MAP 和外部接入方逐步换发 scoped key | 共享 key 仍作为迁移兼容；外部系统不得继续使用它 | 关闭外部冒用 appCaller 的权限窗口 |
| 4 | 执行 vision、图片、ASR 的同提交单次受控验证 | 文本与四协议已经有证据，剩余不确定性集中在真实多模态和对象存储 | 才能宣称关键非视频能力具备同提交生产证据 |
...
|---|---|---|
| 生产发布提交 | `92fac961c87b007849400b326b628cea77940632`；`api / llmgw / llmgw-serve / llmgw-web` 镜像、health commit 和 rollout ledger 一致 | 当前版本身份可信；后续发布仍必须重新做三方一致性检查，不能把服务器源码目录当版本权威 |
| 下一候选提交 | 当前 Phase 4 位于 `codex/llmgw-risk-remediation-doc` 工作分支，尚未合并、制品化或部署 | 只能宣称本地实现和测试完成，不能宣称生产具备 scoped key、原子预算和生命周期清理 |
...
| 同提交受控请求 | 共 8 个受控请求：四协议 canary 4 条、active appCaller 3 条、强制 shadow stream 1 条；provider legs 共 9 条；无媒体和视频调用 | 证明短文本、四入口、auto/pinned 和 shadow 可运行；样本面不足以证明 vision、图片、视频、ASR 和长流式无问题 |
...
| 发布编排 | `http-full` stage 成功，runtime gate `passed=14 blocked=0 waiting=0 retained=1`；额外 ledger 条目关联同提交 4/4 协议证据 | 发布链已能输出机器可读证据；后续不得跳过 stage 或手工拼接完成结论 |
