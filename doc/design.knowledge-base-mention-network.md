---
type: design
title: 知识库引用网络（双链 + 反向链接 + 宇宙图）
status: in-progress
owner: 知识库小组
last-updated: 2026-06-11
related:
  - design.document-store.md
  - rule.frontend-architecture.md
  - rule.data-audit.md
---
...
详细 UAT 清单在 `doc/report.knowledge-base-mention-network.uat.md`（由 `/create-visual-test-to-kb` 技能产出，本期归档到「日报知识库」）。核心断言：

- [ ] 在文档 A 写 `[[B]]`，保存后 B 文档底部「被引用」面板出现 A
- [ ] B 文档底部「本文档引用了」面板隐藏（B 引用了 A 才显示）
- [ ] 删除 A 文档，B 文档底部「被引用」面板自动不再列出 A（级联清理生效）
- [ ] 打开宇宙图，看到 A 和 B 之间一条边，hover 显示预览卡，双击进入文档
- [ ] 拖动 Forces 滑块，整张图可见实时重排
- [ ] 移动端 `prd-desktop` 不受影响（未集成不报错）

## 十二、关联文档

- `doc/design.document-store.md` — 知识库整体架构（mention 是其扩展层）
- `doc/debt.knowledge-base-mention-network.md` — MVP 已知边界与遗留事项
- `wikilink-prototype.html` / `graph-view-prototype.html` — 设计阶段交互原型（仓库根）
- `changelogs/2026-06-11_wikilinks-mvp.md` — 本期碎片
