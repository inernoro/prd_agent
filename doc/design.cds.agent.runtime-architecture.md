# CDS Agent 运行时架构 · 设计说明 · 设计

> **版本**：v1.3 | **日期**：2026-09-07 | **状态**：会话容器已落地，第二轮盲验后等待证据矩阵复核
...
| OpenDesign 会话级容器 | 已完成代码与定向测试 | 独立命名卷、资源限额、限域出口代理、产物清单提交和结束清理已经落地；当前提交仍需在真实 CDS 节点复验 |
...
## 11. 通用 Runtime Provider 合同

CDS Agent 不再只用 `claude-sdk`、`codex` 这些名称判断能力。运行时目录现在同时声明：适配器类型、工作负载、执行归属、产品准入、所需隔离、当前支持的隔离以及事件协议。

首批目录包括测试运行时、Claude Agent SDK、Codex、OpenDesign 和自定义适配器。OpenDesign 被限定为设计产物工作负载，并要求会话级容器。会话分配器现已实现独立工作卷、资源限制、限域出口代理、运行事件、结果校验和清理；只有节点真实探针同时证明 Docker、固定镜像、OpenCode CLI 与限额策略可用时，目录才允许它进入可选择状态。

共享运行时继续执行已接通的 Claude Agent SDK 传输；要求强隔离的设计工作负载不复用共享 sidecar，而进入独立 Workspace Session。因此：

1. 普通共享运行时任务可以继续使用现有链路。
2. 明确要求会话容器的 Provider 只有通过节点能力探针后才能启动，否则失败关闭。
3. 会话容器分配、资源强制和回收留在 CDS Remote Agent 基础设施边界，不进入 MAP 业务代码。
4. MAP 只读取目录、创建会话、订阅事件和接收产物，不持有容器实现细节。
