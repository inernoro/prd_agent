...
首批目录包括测试运行时、Claude Agent SDK、Codex、OpenDesign 和自定义适配器。OpenDesign 被限定为设计产物工作负载，并要求会话级容器；由于分配器尚未实现，其目录状态必须是 planned、不可选择。这样 MAP 可以提前完成调用合同，但不会把共享容器误称为安全隔离。

共享运行时当前能执行已接通的 Claude Agent SDK 传输；资源策略仍是会话元数据，尚未按会话强制 CPU、内存、网络和清理。因此：

1. 普通共享运行时任务可以继续使用现有链路。
2. 明确要求会话容器的 Provider 必须拒绝启动。
3. 会话容器分配、资源强制和回收应在独立 CDS Remote Agent 基础设施项目实现。
