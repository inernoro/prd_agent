...
当前远程 preview 的真实阻塞是 `cds-agent` 专属：R0 runtime pool 和 A0 official SDK
boundary 已有通过证据，剩余 blocker 是 R1 默认 runtime profile 仍为
OpenAI-compatible DeepSeek/OpenRouter profile，不能作为 `claude-agent-sdk` 的默认
Anthropic/Claude-compatible profile。非代码智能体的兼容性 smoke 仍要跑，但它们不应该
因为 CDS Agent 的 R1/provider gate 暂未关闭而失败。若它们失败，应按各自服务链路
排查，而不是把问题归因到官方 SDK adapter。
