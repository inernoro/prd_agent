...
| --- | --- | --- | --- | --- |
| `cds-agent` | MAP/CDS session + sidecar runtime | 是，目标主路径 | sidecar pool 发现、官方 SDK/CLI、MAP approval、cancel | 创建句柄、事件流、审批、停止、PR |
...
当前远程 preview 的真实阻塞是 `cds-agent` 专属：`runtime-status` 显示 sidecar
pool discovery 为 0。非代码智能体的兼容性 smoke 仍要跑，但它们不应该因为
CDS sidecar pool 暂不可用而失败。若它们失败，应按各自服务链路排查，而不是把
问题归因到官方 SDK adapter。
