...
| **无法强制 Agent 只走极速版部署**（2026-09-08，2026-09-16 优化） | 项目设置提供三档 Agent 部署策略：`prebuilt-only` 对所有源码构建保持硬门禁；`prefer-prebuilt` 只在服务已有 CI 预构建能力时强制极速版，尚未接入的服务可继续源码构建；`unrestricted` 不额外限制。策略只能由真人修改，旧 `agentPrebuiltOnly` 状态继续兼容。受约束的部署、分支模式覆盖、项目默认模式写入只要违反当前策略，就返回 409 `agent_prebuilt_only`；响应包含项目策略、受影响服务、精确 `cdscli branch set-mode` 命令、项目设置深链与是否需要真人介入。Agent 接入口令先读取项目策略，不再把项目级选择误报成“CDS 新版本全局强制”。镜像拉取失败时，严格模式全部禁止源码回退；优先模式只对已经具备预构建能力的服务禁止回退。判据仍按实际生效模式和 `prebuilt` 标志计算，覆盖单服务部署、通用构建配置 PUT、项目默认模式、bulk 批量改模式与远端执行器派发。**已知边界**：`import-and-init`、`copy-config-from` 两个入口以及「机器自建 prebuilt 配置」路径尚未纳入门禁覆盖核实，见「已知边界 / 待补」新增行 |
...
| 位置 | 文件 |
|------|------|
| 版本双值对账 | `prd-api/Dockerfile`（build 阶段 `ARG GIT_COMMIT` + `-p:SourceRevisionId`）、`prd-api/src/PrdAgent.Core/Diagnostics/BuildIdentity.cs`、`prd-api/src/PrdAgent.Api/Program.cs` 的 `VersionInfo`、`prd-api/tests/PrdAgent.Tests/BuildIdentityTests.cs`、`.claude/rules/config-runtime-drift.md` 第三点五节；未覆盖项：`llmgw/console-api/Dockerfile`、`llmgw/serving/Dockerfile` |
