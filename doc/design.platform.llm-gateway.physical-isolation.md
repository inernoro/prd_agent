...
### 6.1 子域反代挂载点一致性（2026-08-28）

console Web 既可能挂在根路径，也可能挂在独立子域的子路径下，两种场景下前端按当前访问路径计算出的 API 请求前缀不同。反向代理必须对每一种前端可能算出的前缀都配等价路由；漏配一种时，该前缀的请求会落进 SPA 静态资源兜底而不是转发到 console API——GET 请求还能拿到页面 HTML 不易察觉，POST 请求（如登录）会直接收到 405。构建阶段新增一道机械校验，枚举前端运行时实际可能产生的全部 API 前缀，逐一核对反代配置是否都能命中，缺一个就阻断构建。
