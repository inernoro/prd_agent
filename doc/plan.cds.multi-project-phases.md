...
## 8. P5:团队 workspace + 成员同步 · **Phase 1 已完成**(2026-04-16)

**2026-04-16 状态注记**:

- **FU-02 落地** ✅ — MongoAuthStore 已上线，P5 前置依赖已满足
- **P5 Phase 1 已完成** ✅ — 域类型 + AuthStore 扩展 + MemoryAuthStore + MongoAuthStore + WorkspaceService + `/api/workspaces` 路由 + 前端工作区 pill 动态化
- **Phase 2 待做**：GitHub Org 成员自动同步（`OrgSyncService`）、RBAC 中间件、项目列表按工作区过滤 UI
...
- P4 完成,多项目创建已稳定 ✓
- **FU-02 MapAuthStore mongo 后端落地**(users/sessions 持久化)
...
## 9. P6:手动项目 + webhook + 自动部署 · **未启动**(2026-Q3+)

**2026-04-16 状态注记**:

- 明确 **deferred** —— 前置依赖 P5 未启动
- 路线图 Phase 3 "Release Agent + 环境管理" 和本 P6 重叠度高,启动时需先评审定位边界:
  - P6 = 在当前 CDS 所在机器上自动部署到本地容器(webhook 驱动)
  - Phase 3 = 部署到 **远端** 服务器(Release Agent 作为远端守护进程)
  - 两者可独立,也可合并作一期 `P6 → Release Agent` 渐进推进
