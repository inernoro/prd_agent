...
| [x] | P4-11 专业模式一屏收敛 | 专业模式不再把 SLA/治理/调度/诊断全部铺在首屏，用户能先完成输入、启动和观察 | 首屏顺序调整为状态条 -> 工作区；SLA/调度/治理/执行链路下沉；Runtime 诊断、事件时间线、上下文默认折叠；保留所有高级能力入口 | 修复文件：`prd-admin/src/pages/cds-agent/CdsAgentPage.tsx`；本地 `pnpm --prefix prd-admin exec tsc --noEmit` pass；`pnpm --prefix prd-admin exec vitest run src/pages/cds-agent/__tests__/cdsAgentReadiness.test.ts`：9/9 pass；`pnpm --prefix prd-admin build` pass；远端视觉截图计划路径 `artifacts/cds-agent/2026-05-19/remote-pro-workbench-collapsed.png` |
