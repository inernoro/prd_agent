...
Phase 0/1/2/3 已完成本地验收。下一轮只做 `P4-1 远端发布前验收与试用入口`：基于当前本地证据决定是否执行一次关键远端部署，部署后只验证用户三步巡检、治理面板、工作流调度和知识库只读入口，不新增 agent loop。

| 顺序 | 任务 | 预计 | 是否需要用户 |
| --- | --- | --- | --- |
| 1 | P4-1 远端发布前验收范围核对 | 0.5d | 可能需要确认是否部署 |
| 2 | 复跑关键本地 smoke 和聚焦测试，确认可发布基线 | 0.5d | 不需要 |
| 3 | 执行一次关键远端部署并截图 CDS Agent 工作台 | 0.5-1d | 可能需要云端权限 |
| 4 | 远端验证三步巡检、治理面板、工作流调度、知识库只读入口 | 0.5-1d | 不需要 |
| 5 | 回写唯一看板和发布验收报告 | 0.5d | 不需要 |
...
Phase 0/1/2/3 已完成本地验收。下一次开发入口是 `P4-1 远端发布前验收与试用入口`：先复跑关键本地 smoke 和聚焦测试，再决定是否执行一次关键远端部署；远端只验证用户三步巡检、治理面板、工作流调度和知识库只读入口，不在这一轮新增 agent loop 或跨团队写入。
