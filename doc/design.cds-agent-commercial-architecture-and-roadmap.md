...
当前不再回到 Phase 1/Phase 2。`P3-1 trace/artifact bundle`、`P3-2 多运行时 adapter matrix`、`P3-3 成本、token、SLA 面板`、`P3-4 定时巡检和批量知识治理最小入口` 已完成本地验收。下一轮进入 `P3-5 权限和组织治理`，只做团队隔离、权限边界和策略可见性，不扩大到自动写入。

| 顺序 | 任务 | 预计 | 是否需要用户 |
| --- | --- | --- | --- |
| 1 | P3-5 权限和组织治理边界校准 | 0.5-1h | 不需要 |
| 2 | 梳理仓库、知识库、runtime profile、审批策略的团队隔离模型 | 0.5d | 不需要 |
| 3 | 后端补最小权限/组织治理只读视图和必要 gate | 0.5-1d | 不需要 |
| 4 | 前端把治理状态放入 CDS Agent 商业面板，不引入自动写入 | 0.5d | 不需要 |
...
| [ ] Phase 3 规模化商业能力 | 进行中，P3-5 入口 | 团队级巡检、知识治理、多运行时和成本治理 | trace bundle 可导出；adapter matrix 可见；SLA/成本面板可见；定时巡检/知识治理只读视图可见；后续进入权限和组织治理 |
...
Phase 2 已完成 P2-1/P2-2/P2-3/P2-4/P2-5/P2-6。当前进入 Phase 3：规模化商业能力。`P3-1 trace/artifact bundle 导出与回放边界`、`P3-2 多运行时 adapter matrix`、`P3-3 成本、token、SLA 面板`、`P3-4 定时巡检和批量知识治理最小入口` 已完成本地验收；下一次开发入口是 `P3-5 权限和组织治理`，先做团队隔离、策略可见和治理 gate，不做自动写入。
