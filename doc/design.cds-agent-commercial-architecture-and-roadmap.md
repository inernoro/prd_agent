...
当前不再回到 Phase 1/Phase 2。`P3-1 trace/artifact bundle`、`P3-2 多运行时 adapter matrix`、`P3-3 成本、token、SLA 面板` 已完成本地验收。下一轮进入 `P3-4 定时巡检和批量知识治理最小入口`，只做可配置调度和只读治理视图，不扩大到自动写入。

| 顺序 | 任务 | 预计 | 是否需要用户 |
| --- | --- | --- | --- |
| 1 | P3-4 定时巡检和批量知识治理边界校准 | 0.5-1h | 不需要 |
| 2 | 设计最小只读调度入口：任务模板、cron/手动触发、运行历史 | 0.5-1d | 不需要 |
| 3 | 前端增加批量治理只读入口，不引入自动 apply/write | 0.5d | 不需要 |
...
| [ ] Phase 3 规模化商业能力 | 进行中，P3-4 入口 | 团队级巡检、知识治理、多运行时和成本治理 | trace bundle 可导出；adapter matrix 可见；SLA/成本面板可见；后续进入定时巡检和批量知识治理 |
...
Phase 2 已完成 P2-1/P2-2/P2-3/P2-4/P2-5/P2-6。当前进入 Phase 3：规模化商业能力。`P3-1 trace/artifact bundle 导出与回放边界`、`P3-2 多运行时 adapter matrix`、`P3-3 成本、token、SLA 面板` 已完成本地验收；下一次开发入口是 `P3-4 定时巡检和批量知识治理最小入口`，先做只读调度和治理视图，不做自动写入。
