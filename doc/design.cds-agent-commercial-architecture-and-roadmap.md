...
Phase 0/1/2/3 已完成本地验收，`P4-1 远端发布前验收与试用入口` 已完成。P4-1 的结论是：远端 preview 当前运行时代码已覆盖 P3-5d，后续提交未改 `prd-api/prd-admin/cds`，因此本轮不重复部署；下一轮只做 `P4-2 远端 R1 provider-switch profile 闭环与试用入口`，目标是让用户能在远端真实发起一次 provider-backed 只读代码巡检。

| 顺序 | 任务 | 预计 | 是否需要用户 |
| --- | --- | --- | --- |
| 1 | P4-2 校准远端默认 provider-switch runtime profile | 0.5d | 仅当远端没有可用 provider key/baseUrl/profile 时需要 |
| 2 | 跑一次 provider-backed one-cycle，只验证只读巡检 | 0.5d | 不需要 |
| 3 | 复抓远端简洁面板视觉证据和 runtime-status 证据 | 0.5d | 不需要 |
| 4 | 生成 P4-2 验收报告，回写唯一看板 | 0.5d | 不需要 |
| 5 | 只有运行时代码变化或远端试用需要时，才做关键 self-update/branch deploy | 0.5-1d | 可能需要云端权限 |
...
| [ ] Phase 4 远端试用与发布验收 | P4-1 完成，P4-2 待做 | 远端 preview 可被真实用户路径试用，并证明 provider-backed 只读巡检能闭环 | P4-1 已确认不重复部署、远端 API/视觉通过；P4-2 需要闭环 R1 provider-switch profile 并跑一次 one-cycle |
...
Phase 0/1/2/3 已完成本地验收，P4-1 已完成远端发布前验收。下一次开发入口是 `P4-2 远端 R1 provider-switch profile 闭环与试用入口`：不新增 agent loop，不把 remote host/env/image 变成用户路径；只校准远端默认 profile/provider 配置，跑一次 provider-backed one-cycle，证明用户可以开始真实只读代码巡检。
...
| [ ] | P4-2 远端 R1 provider-switch profile 闭环 | 用户可以真实发起一次 provider-backed 只读代码巡检 | 远端 `runtime-status` 不再卡在 `R1/N1`；默认 profile 使用可用 provider-switch 配置；provider-backed one-cycle pass；简洁面板显示真实 session/result/artifacts | 待做 |
