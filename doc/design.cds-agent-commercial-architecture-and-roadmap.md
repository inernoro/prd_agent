...
| [ ] | P4-24 远端 preview 真实验收修复 | 不能把 GitHub CI 通过、本地 mock 视觉通过等同于用户可用；真实 preview 必须能打开、登录态可访问、进入 CDS Agent 页面并完成视觉/交互 smoke | 修复 CDS 分支服务缺失或重新部署；确认 `admin-prd-agent` preview HTTP 200；用真实登录态打开 `/cds-agent`；检查无权限页、无 503、无旧 bundle；再执行简洁模式视觉断言和一次只读发送 smoke | 当前阻塞证据：GitHub checks 全绿，`Cursor Bugbot=NEUTRAL`，但 `curl -I -L https://cds-agent-workbench-ui-codex-prd-agent.miduo.org/cds-agent` 返回 `HTTP/2 503`；Chrome CDS 控制台显示 `codex/cds-agent-workbench-ui` commit `2d749c0`，同时服务日志提示 `api-prd-agent :10655` 容器不存在；下一步需先修复 CDS preview 运行态，再重新验收 |
