...
当前不再回到 Phase 1/Phase 2。`P3-1 trace/artifact bundle`、`P3-2 多运行时 adapter matrix`、`P3-3 成本、token、SLA 面板`、`P3-4 定时巡检和批量知识治理最小入口` 已完成本地验收。`P3-5a 权限和组织治理只读视图` 已完成本地验收，但 P3-5 不能视为完成：runtime profile 仍存在 global default/resolve 语义，需要下一轮做 team/user-scoped profile resolve。

| 顺序 | 任务 | 预计 | 是否需要用户 |
| --- | --- | --- | --- |
| 1 | P3-5b runtime profile team/user-scoped resolve 设计 | 0.5d | 不需要 |
| 2 | 收敛 `List/Resolve/Update/Delete` 的 profile 作用域，保留管理员 fallback | 0.5-1d | 不需要 |
| 3 | 工作流和会话创建时校验 profile owner/team scope | 0.5d | 不需要 |
| 4 | 前端治理面板把 `GOV-PROFILE-SCOPE` 从 warn 推到 pass | 0.5d | 不需要 |
...
| [ ] Phase 3 规模化商业能力 | 进行中，P3-5a 完成 | 团队级巡检、知识治理、多运行时和成本治理 | trace bundle 可导出；adapter matrix 可见；SLA/成本面板可见；定时巡检/知识治理只读视图可见；权限/组织治理只读视图可见；后续补 runtime profile scoped resolve |
...
Phase 2 已完成 P2-1/P2-2/P2-3/P2-4/P2-5/P2-6。当前进入 Phase 3：规模化商业能力。`P3-1 trace/artifact bundle 导出与回放边界`、`P3-2 多运行时 adapter matrix`、`P3-3 成本、token、SLA 面板`、`P3-4 定时巡检和批量知识治理最小入口`、`P3-5a 权限和组织治理只读视图` 已完成本地验收；下一次开发入口是 `P3-5b runtime profile scoped resolve`，把当前治理面板中的 `GOV-PROFILE-SCOPE` 从 warn 推到 pass。
