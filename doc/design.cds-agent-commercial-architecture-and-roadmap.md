...
当前不再回到 Phase 1/Phase 2。`P3-1 trace/artifact bundle`、`P3-2 多运行时 adapter matrix`、`P3-3 成本、token、SLA 面板`、`P3-4 定时巡检和批量知识治理最小入口`、`P3-5a 权限和组织治理只读视图`、`P3-5b runtime profile scoped resolve` 已完成本地验收。下一轮只做 `P3-5c team-shared runtime profile policy`，不要再回到全局默认 profile 语义。

| 顺序 | 任务 | 预计 | 是否需要用户 |
| --- | --- | --- | --- |
| 1 | P3-5c team-shared runtime profile policy 设计 | 0.5d | 不需要 |
| 2 | 明确 user-owned 与 team-shared profile 的数据边界和 UI 文案 | 0.5d | 不需要 |
| 3 | 工作流/session 使用 team-shared profile 时校验 team membership | 0.5-1d | 不需要 |
| 4 | 治理面板展示 repository/profile/approval owner 策略缺口 | 0.5d | 不需要 |
| 5 | 补 smoke、单测、必要视觉证据并回写 checklist | 0.5d | 不需要 |
...
| [ ] Phase 3 规模化商业能力 | 进行中，P3-5b 完成 | 团队级巡检、知识治理、多运行时和成本治理 | trace bundle 可导出；adapter matrix 可见；SLA/成本面板可见；定时巡检/知识治理只读视图可见；权限/组织治理只读视图可见；runtime profile 已按主体 scoped resolve；后续补 team-shared profile policy |
