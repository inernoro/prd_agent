...
| 步骤 | 状态 | 说明 |
| --- | --- | --- |
| 契约归位到 Core | 已完成（2026-08-05） | 13 个纯接口与数据类型迁到 Core；命名空间刻意不动，因此 164 个引用点零改动 |
| 合并两份网关接口 | 未开始 | Core 与基础设施层各有一份，靠一处强制类型转换桥接；这是「没有统一来源」的直接病灶 |
| 实现整体迁进 Gateway 侧项目 | 未开始 | 迁完后主系统只引用契约与 HTTP 客户端，不再持有任何上游协议实现 |
| 命名空间改名 | 未开始 | 纯机械操作，波及 164 个文件，独立提交，不与搬迁混在一起 |
