...
- **现状**：知识来源以冻结快照和哈希进入任务；产物中的受限相对链接被降为不可导航标记，事实保真主要依赖提示词与人工验收。
- **影响**：读者不能从每条结论直接打开知识来源，模型也可能在视觉润色时改写事实。
- **偿还条件**：增加 claim 级引用映射、发布侧安全引用查看器和自动事实一致性检查，失败时阻止发布而不是只给提示。
