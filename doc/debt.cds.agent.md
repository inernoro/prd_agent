...
### D14 · 清理账本连续不可写的即时回收兜底（open）

- **现状**：设计产物只会在带会话代际与消息身份的持久清理账本写入成功后交付；后台回收器支持多副本租约、指数退避和重启续扫。若 MongoDB 权威写连续两次失败，产物不会交付，远端资源由 CDS 会话的强制自动清理时限最终回收。
- **影响**：不会错发产物、误停新轮或永久泄漏，但数据库完全不可写期间的资源释放可能延后到 CDS 自动清理时限。
- **偿还条件**：增加携带预期会话代际与完成消息身份的即时停止兜底，或建立可从 CDS 会话标签重建的独立清理收件箱，并加入 MongoDB 连续故障注入测试。
