...
网页生成「我的风格」新增一项人工维护索引：`personal_design_styles` 的
`idx_personal_design_styles_owner_updated`。风格画廊按归属人列出自己的风格、新建前按归属人计数，
两条读法都以 `OwnerUserId` 为前导；集合很小（每人最多 20 套），缺索引只会让打开风格画廊变慢，
不影响正确性。同样只通过 DBA 清单创建。
