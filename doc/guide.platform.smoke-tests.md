...
**一句话**：部署后用真实请求打预览域名的分层冒烟脚本，几十秒暴露「容器起来了但接口 401、数据库连不上」。
**谁该读**：每次部署到灰度环境后要确认没坏的人；给流水线配自动检查的工程师。
**读完能做什么**：跑通一次冒烟测试、看懂结果，并把它接进自己的部署流程。
...
[doc/guide.cds.agent.code-review-quickstart.md](./guide.cds.agent.code-review-quickstart.md)。本文件只解释 smoke 怎么跑、
...
## 合成测试登录（自动化验收专用，2026-08-11）

稳定冒烟需要以真实用户身份走完整登录态的页面（不只是打 API），但账号密码不适合下放给自动化脚本长期持有。为此新增了一条独立于正常登录的合成登录通道：`POST /api/v1/auth/synthetic/ticket` 用 AI Access Key 或专用的稳定冒烟凭据签发一张一次性票据（默认 180 秒，60-300 秒可调），前端消费页拿票据换取一个不可续期的短会话（默认 30 分钟）。三层限制保证它不会变成后门：

- 只有白名单账号（配置项显式列出）能领票据，非白名单账号一律 403；
- 票据一次性消费，purpose 字段绑定 `stable-smoke-login`，不能挪作他用；
- 换出的会话不可续期，到点必须重新走票据流程，不会像正常登录一样静默续签。

对应的落地面：CDS 与正式环境各自独立配置该通道的开关与白名单（不共用凭据，呼应 `cross-project-isolation.md`），永久回归套件里新增了登录页冒烟与首页深链打开的证据归档，稳定冒烟报告新增「执行覆盖账本」——每个未执行项必须给出阻塞原因、代码入口、补跑命令和关闭条件，不允许验收报告里出现「跳过了但没写为什么」。
...
- [doc/plan.cds.status.md](./plan.cds.status.md) —— CDS 当前状态看板(Phase 2 已并入主进度)
