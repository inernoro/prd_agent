...
**一句话**：对外兼容网关的关键设计不变量（勿回退）、已偿还阶段与仍未做的排期。
**谁该读**：接手开放接口的工程师。
**读完能做什么**：避免回退已定的设计不变量，并挑下一项来做。

---

> 模块：开放接口（OpenAI 兼容）对外网关
...
5. **接入指南**：[doc/guide.open-platform.open-api.md](./guide.open-platform.open-api.md)（quickstart + 契约 + 白名单语义）。
...
- **日志索引/TTL 待 DBA 建**：`open_api_request_logs` 含 IP/UA，按 no-auto-index 规则禁止应用自建索引。
  ...
  [doc/guide.platform.mongodb-indexes.md](./guide.platform.mongodb-indexes.md)，由 DBA 手动建（Codex PR#732 P2）。
