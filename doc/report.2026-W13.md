...
- **设计文档**：完整技术设计文档 [design.review-agent.md](./design.review-agent.md) (#352)
...
| W12 建议方向 | W13 实际进展 |
|--------------|-------------|
| P0 知识库 RAG 集成 | 警告 多格式文件上传已完成（PDF/Word/Excel/PPT），但 RAG 向量索引仍未开始 |
| P0 产品质量收敛 | 是 fix 占比 43% 说明持续修复中，28 个 Controller 安全修复 + localStorage 禁用 + 主题切换黑屏修复 |
| P1 CDS 生产环境稳定性 | 是 资源优化（去内存限制）、部署模式切换、启动加载页、state.json 安全修复，稳定性持续提升 |
| P1 作品广场社交功能 | 警告 瀑布流布局 + 无限滚动上线，但评论/收藏/推荐算法未推进 |
| P2 缺陷管理 Webhook | 否 连续四周未推进 |
| P2 移动端系统性 QA | 否 未推进 |
...
| PR | 标题 | 分类 |
|----|------|------|
| #304 | 周报阅读追踪 + Todo 计划周 + 使用指引 + TTL 自愈 | 更新 更新 |
| #315 | 桌面端服务器选择重设计 + 版本号 + 引导提示词种子 | 界面 UI/UX |
| #316 | PRD 知识库多文件上传 + 三阶段格式检测 | 新功能 新功能 |
| #317 | 投稿修复集：Space 粒度投稿 + 并发 marker 修复 + 撤稿 | 修复 修复 |
| #318 | UTF-16 BOM 检测 + 20MB 上传限制 | 修复 修复 |
| #320 | 文学创作展示重设计 + 全站 localStorage 禁用 + 安全修复 | 新功能 新功能 |
| #321 | AppCallerCode 别名归一化，修复排行榜维度泄漏 | 修复 修复 |
| #322 | 文学创作 NotebookLM 风格卡片 + 封面图逻辑 | 界面 UI/UX |
| #323 | CDS 构建动画统一 + 标签页标签 + pinned commit | 新功能 新功能 |
| #324 | CDS state.json 敏感信息泄露修复 | 安全 安全 |
| #325 | 模型池管理 master-detail 布局 + 增量同步 | 界面 UI/UX |
| #326 | 聊天图片和头像显示修复 | 修复 修复 |
| #327 | 文学创作 sidebar 沉浸式卡片布局 | 界面 UI/UX |
| #328 | CDS 标签页标题 + 分支悬浮操作 + 自动更新修复 | 新功能 新功能 |
| #329 | VisualAgent 生图提示词智能澄清 + watchdog | 新功能 新功能 |
| #330 | 视频加载动画 + 权限加载屏 | 界面 UI/UX |
| #331 | 对话推荐追问功能 (Suggested Questions) | 新功能 新功能 |
| #332 | TypeScript 编译错误修复 | 修复 修复 |
| #333 | CDS Activity 面板缩放 + 预览眨眼动效 | 界面 UI/UX |
| #334 | CDS Activity 面板可拖拽边框调整 | 界面 UI/UX |
| #335 | VisualAgent 直连模式 + 生图意图前缀 | 更新 更新 |
| #336 | 文学创作配图按时间顺序匹配 + 认证修复 | 修复 修复 |
| #337 | CDS env 编辑器移除变量遮蔽 | 修复 修复 |
| #338 | MAP 品牌过渡动画替换全站 spinner | 界面 UI/UX |
| #339 | 作品广场瀑布流布局 + 无限滚动 | 新功能 新功能 |
| #340 | VisualAgent 智能模式英文意图前缀 | 修复 修复 |
| #341 | 移除未使用 Upload 导入修复 TS6133 | 修复 修复 |
| #342 | CDS 启动加载页 + CSS MIME 类型修复 | 新功能 新功能 |
| #343 | VisualAgent 智能模式默认关闭 + 模型选择竞态修复 | 修复 修复 |
| #344 | 作品广场心形图标放大 | 界面 UI/UX |
| #345 | 规则：禁止自动创建 PR | 文档 文档 |
| #346 | 生图意图前缀不泄漏到 UI 展示 | 修复 修复 |
| #347 | 周报团队 Prompt 设置权限收紧 | 安全 安全 |
| #348 | 周报团队可见性权限收紧 | 安全 安全 |
| #349 | CDS compose 全局变量拆分 | 架构 架构 |
| #351 | 37 篇设计文档补全 + 6 篇新文档 + 废弃清理 | 文档 文档 |
| #352 | 产品评审员 Agent 全功能上线 | 新功能 新功能 |
| #353 | CDS 色标切换冲击波动画 | 界面 UI/UX |
| #354 | CDS 资源优化 + 部署模式切换 |  性能 |
| #355 | 转录工作台 Agent + 豆包 ASR 中继 | 新功能 新功能 |
| #356 | CDS badge 弹窗自适应 + 日志复制 | 修复 修复 |
| #357 | AppCallerCode 清理 15 个未使用注册项 | 架构 架构 |
| #358 | CDS 标签编辑 + AI 边框 + 环境变量回退 | 新功能 新功能 |
| #359 | 流式 ASR 完整改造：ffmpeg 全格式 + SSE 逐帧推送 | 新功能 新功能 |
