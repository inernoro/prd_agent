...
- **解决什么问题**：当前图生图仅支持单张参考图（`/v1/images/edits`），用户期望多张图片同时被模型"看到"进行融合创作
- **方案概述**：利用 nanobanana Vision API（`/v1/chat/completions`）传递 1-6 张图片，按图片数量自动路由到文生图/单图生图/多图 Vision API 三种模式
- **业务价值**：突破单图限制，支持多图融合生成，显著提升视觉创作 Agent 的创作能力
- **影响范围**：后端（OpenAIImageClient、ImageGenRunWorker、Vision 请求模型）
- **预计风险**：中 — Vision API 稳定性待验证，已设计配置开关可回退单图模式
