...
| 状态 | partial（核心已落地；Claude 流式 tool_use 与池路径能力兜底已补齐，以下保留未完成边界） |
...
「函数调用（tools/tool_calls）穿协议不丢」已落地核心路径（OpenAI 透传 + Claude 原生互转 + Claude 流式 tool_use 增量、Open Platform 代理回吐 + 池路径能力软门），但仍留下若干**已知边界**，下一波补齐。
