# 视频场景代码生成（Scene Codegen） · 设计

> **版本**：v1.0 | **日期**：2026-03-08 | **状态**：已实现

## 一、管理摘要

- **解决什么问题**：视频 Agent 仅有 8 种硬编码场景模板，视觉效果千篇一律
- **方案概述**：通过 LLM 为每个分镜生成定制化 Remotion React 组件代码，生成失败时无缝回退到硬编码模板
- **业务价值**：实现"千镜千面"的视觉效果，大幅提升视频内容差异化和质量
- **影响范围**：prd-api VideoGenRunWorker、prd-video Remotion 场景渲染层
- **预计风险**：中 — LLM 生成代码存在编译失败风险，通过回退机制兜底
