...
访客在托管页面上问问题的入口，从「一个按钮 + 一个抽屉」变成一件会形变的东西（胶囊 → 中下玻璃长条 → 右侧对话栏 → 竖条），开场问题不再要上传者手填。**对话半边已在预览域名验通**；开场问题自动生成还差网关的意图池放一个模型。
...
| 访客真问一句看到答案 | 答案流式长出来且只依据本页正文 | **通过**（2026-08-28）：答出正文原话，模型行 `gpt-3.5-turbo · OpenAI`，额度 16→15 |
| **开场问题自动生成** | **真写出 5 条** | **未验** —— 网关的意图池成员数为 0，端点稳定返回 ModelUnavailable |
...
| 网关对话默认池里的 `chat-latest` 不是可调用模型 | 健康状态复位后，第一次提问会先撞一次 400 | P1 | 网关配置负责人 | 从对话默认池移除该成员 |
| 网关意图池成员数为 0 | 开场问题自动生成用不了 | P1 | 网关配置负责人 | 放一个便宜模型进去；放完可立刻复验 |
