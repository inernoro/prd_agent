...
当前生产已经完成的是 **执行层 full-http**：MAP 运行时通过 `llmgw-serve` 发送模型请求，GW 日志能看到 `transport=http`。这不等于目标架构全部完成。目标态还要完成 **配置权威迁移**：appCallerCode、模型池、平台、exchange、key、路由策略、GW 请求日志、shadow 证据、操作审计都归 LLM Gateway 管理。
...
截至 2026-07-09，本计划已有以下代码切片落地，仍未代表目标态全部完成：
...
| appCaller registry | 请求进入 GW 后写入 `llm_gateway.llmgw_app_callers`；首次 discovered 会用请求里的 modelPolicy、modelPoolId、parameterPolicy 初始化治理建议，后续请求只更新 `LastObservedModelPolicy`、`LastObservedModelPoolId`、`LastObservedParameterPolicy`，不覆盖管理员配置；控制台提供 `/app-callers` 只读和状态/模型池/策略写入，并可按 route/parameter/any drift 筛出“配置值与最近请求值不一致”的调用方；概览页可将 active 调用方按 requestType 绑定到同类型 GW 默认池；appCaller 页面可维护 `owner`、`monthlyBudgetUsd`、`RateLimitPerMinute` 治理元数据并写操作审计；控制台可按当前筛选批量设置 owner、USD 月预算、RPM、状态、modelPolicy、parameterPolicy，并写 `app_caller.bulk_governance` 操作审计；控制台新增 `/audits` 审计页，可按 action、targetType、actor、success、搜索和时间窗口筛选 `llmgw_operation_audits`；serving 真实发送入口已按 `RateLimitPerMinute` 写 `llmgw_app_caller_rate_windows` 并返回 429；月预算 gate 已按现有 `TotalCostUsd` / `CostUsd` / `EstimatedCostUsd` 日志证据累计本月花费，超额返回 429；GW 日志已写入模型池价格快照和 `EstimatedCost*` 字段，控制台列表/详情/summary 可展示成本证据；日志详情已派生 `routerTrace`，控制台详情抽屉可展示 sourceSystem、ingressProtocol、modelPolicy、modelPoolId、route mode、requested/actual model、pool、platform、provider、transport、fallback、参数策略与 dropped 参数；日志列表、summary、timeseries、sessions 已支持按 `sourceSystem`、`ingressProtocol`、`modelPolicy` 过滤，控制台筛选条可直接定位某类入口协议或路由策略；日志写入已新增 `ProviderAttempts`，当前记录最终发送尝试和 fallback 前原池候选，并在完成/错误路径补齐 HTTP status、duration、error、endedAt，Exchange async raw 路径会逐条记录 submit、poll 和 poll-timeout attempts；非流式 auto 模式会在 402/408/409/425/429/5xx 时按已解析候选切换下一个 provider；流式 auto 模式会在输出前的可重试失败切换下一个 provider；raw JSON/multipart submit 阶段会在可重试失败时切换下一个 provider；这些路径都会把每次发送写入 attempts 并在成功完成时把日志根字段更新为最终 provider/model；详情抽屉可展示 attempts；模型池成员价格币种支持 CNY/USD，USD 成本可进入月预算统计；控制台可按模型类型批量校准 GW 权威池成员 PriceCurrency，默认仅补已有价格字段且币种为空的历史成员，并写操作审计 | appCaller registry 当前缺口已收敛到生产迁移执行与 MAP 读取退场前复核；流式 retry 仅覆盖输出前失败，输出后断流不跨 provider 续接；raw retry 仅覆盖 submit 阶段，Exchange async poll、二进制下载和 WebSocket ASR 不跨 provider 重放；provider retry 仍需要生产灰度证据 |
| registry 优先路由 | `ModelResolver` 已在 pinned 之后优先读取 `Status=active + ModelPoolId` 的 GW registry 绑定；新增 `LlmGateway:DisableMapConfigFallbackForActiveAppCallers` 退场门，开启后 active appCaller 未命中有效 GW 池会 fail closed，不再隐式回退 MAP | 生产默认仍不开启；需先完成 MAP-only 认领和 active 绑池复核 |
| GW-owned model pool | 控制台可直接新建 `llm_gateway.llmgw_model_pools`，也可批量认领 MAP `model_groups`；resolver 命中 active appCaller 时优先读取 GW 池，找不到再回退 MAP 池；退场门开启后 active appCaller 只接受 GW-owned 池，且跳过 expectedModel 的 MAP 全量池与 LLMModels 直连兜底；GW 权威池已有属性编辑、成员添加、批量导入、删除和优先级更新入口；模型页可按平台/启用态批量维护 GW-owned 模型通用能力矩阵，并可套用 OpenAI/Claude/Gemini/OpenRouter 字段级参数模板；成员保存或批量导入时记录模型能力快照，模型池页可按当前池类型或 vision/image/tool/parallel-tools/parameters/thinking/structured-output/logprobs/prompt-cache 过滤候选；池成员可用轻量输入维护 `parameter:<name>` 字段级能力，参数名提示来自 `/gw/parameter-capabilities/meta`；运行时池解析已读取 function_calling/parallel_tool_calls/vision/image_generation/thinking/structured_output/logprobs 能力和 `parameter:<name>` 字段级能力，明确不支持时在 Gateway 发送前熔断；概览页已有配置权威迁移报告、MAP-only 批量认领入口、active appCaller 自动绑默认 GW 池入口和 active fallback readiness 字段 | 生产执行与开关启用未完成 |
| GW-owned platforms/models/exchanges | 控制台 API 可将 MAP `llmplatforms`、`llmmodels`、`model_exchanges` 认领到 `llm_gateway.llmgw_platforms`、`llmgw_models`、`llmgw_model_exchanges`；resolver 解析普通池、pinned、exchange 和可用池展示时先读 GW 副本，缺失再回退 MAP；退场门开启后 active appCaller 的 GW 池条目不再借 MAP platform/exchange；控制台已有平台、模型、Exchange 观测、认领、基础密钥轮换、批量密钥轮换、密钥删除、key 解密健康自检、MAP-only 缺口报告和统一批量认领入口 | 平台/模型/Exchange 深度编辑和生产退场复核未完成 |
