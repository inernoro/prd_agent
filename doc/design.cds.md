...
> **版本**：v4.0 | **日期**：2026-07-15 | **状态**：已落地

**一句话**：把代码仓库变成可持续部署、预览、验收与恢复的运行环境：项目是权限边界，分支是预览环境。
**谁该读**：第一次了解这套系统架构的人。
**读完能做什么**：说清项目、分支、部署记录、版本四者的关系。
...
非 systemd 环境下的后台启动接入进程 supervisor 保活：CDS 进程退出或完成自更新后由 supervisor 自动重新拉起，不依赖宿主机的 systemd 单元。前端与该行为配套区分两类断线：**明确重启**（自更新/主动重启触发，界面不长时间全屏遮罩，快速重试后即恢复）与**未知断线**（网络波动等），避免把预期内的短暂重启体验成故障。
...
CDS 系统设置新增「外部接入」统一入口，把与 MAP（PrdAgent）等外部系统的互联收敛到一处：

- 展示 MAP 系统互联与缺陷转发的当前状态，页面内可发起授权、验证连通性、撤销/清除连接——不提供手工填写地址或 Token 的入口；
- 缺陷转发凭据存入 CDS 系统级密钥存储（不落项目变量），避免与项目环境变量的迁移分类混淆；**加密落库以配置了系统级加密密钥为前提**——未配置时该存储层不做静态加密，退化为明文兼容模式，是已知安全边界（见「已知边界」）；
- 接入方式从手工搬运地址/Token/配对码，改为跳转 MAP 完成一次授权、由服务端自动交换长期系统连接与缺陷转发凭据；MAP 侧为 CDS 自动签发最小权限的永久缺陷 Key，删除该基础设施连接时同步撤销；
- 授权、导入、更新与 GitHub 推送等原本散落的浮层统一收进信息中心，消除底部常驻浮层的重叠问题。
...
- 系统级密钥存储未配置加密密钥时以明文兼容模式落库，需要显式配置才有静态加密。
