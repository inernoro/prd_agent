...
> 类型：debt（工程债务台账） | 模块：OpenRouter 对外开放网关 | 状态：active
> 最后更新：2026-06-03

## 背景

把开放平台做成贴近真实 OpenRouter 的对外 LLM 网关。外部调用方用标准 OpenAI/OpenRouter
...
- **伞形 appCallerCode 必须静态注册**：`open-router.proxy::chat` / `open-router.proxy::generation`
  ...
  守卫测试：`tests/PrdAgent.Tests/OpenRouterRegistrationTests.cs`。
...
- CI 守卫：`OpenRouterRegistrationTests` + `AppCallerCodeRegistryGuardTests`（kebab-case）。
