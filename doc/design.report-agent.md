...
周报（富文本正文、日报、评论）三处独立的图片上传表单收敛为共用实现，评论输入器升级为支持粘贴截图、选图上传、待发缩略图删除的多行 composer。评论创建接口新增 `attachmentIds`（上限 9 张、按归属校验、仅图片不夹带文字），列表接口批量解析并返回附件详情。图片大图预览统一走 `createPortal` 挂载到 `document.body` 并提升层级，避免被右侧栏或卡片遮挡（呼应 `frontend-modal.md` 的浮层三硬约束）。
