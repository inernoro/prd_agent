...
- [AI 大模型网关从 MAP 剥离 · 债务台账](debt.llm-gateway-isolation) `debt.llm-gateway-isolation`
  > 波1(观测增强/独立观测控制台 prd-llmgw/生图统一)+波2(serving 网关 llmgw-serve HOST 既有实现+/gw/v1/*；MAP 侧 HttpLlmGatewayClient+LlmGateway__Mode flag 默认 inproc) 均已编译验证(5 镜像 CI 绿)；待用户 1 次 CDS 拓扑导入审批(pending a204d9ea7c2b)开预览 runtime；http 模式默认 OFF、影子比对/前端上预览为波3
