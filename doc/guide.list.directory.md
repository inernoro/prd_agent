...
- [应用注册中心协议规范](spec.platform.app-registry) `spec.platform.app-registry`
  > 应用注册、路由分发的协议规范
...
- [私人执行助理（PA Agent）产品方案](spec.product-agent.pa) `spec.product-agent.pa`
  > PA Agent 产品定位、Phase 1 能力与路线图，可独立下载评审

- [毒舌秘书：PA Agent 最小可行升级方案](spec.product-agent.pa.savage-upgrade) `spec.product-agent.pa.savage-upgrade`
  > PA Agent 升级为毒舌秘书的最小可行方案，1-2 天内可上线，保留架构只改产品灵魂

- [毒舌秘书 v2 迭代升级方案](spec.product-agent.pa.savage-iter-v2) `spec.product-agent.pa.savage-iter-v2`
...
- [MAP 知识库传输协议（MAP-KBTP v1）规格](spec.knowledge-base.transfer-protocol) `spec.knowledge-base.transfer-protocol`
...
- [统一短链系统规格](spec.platform.short-links) `spec.platform.short-links`
...
- [手机端整体重构调研](design.frontend.mobile-refactor) `design.frontend.mobile-refactor`
...
- [知识库智能体架构设计](design.knowledge-base.agent-architecture) `design.knowledge-base.agent-architecture`
  > 三层模型(脑/技能/沙箱) + map-agent·cds-agent 命名 + 借运行时不重建 + 唯一接缝(KB 当工作目录进、改动经 diff 闸出)；md→ppt 避坑
- [MAP MCP 连接器设计](design.platform.map-mcp-connector) `design.platform.map-mcp-connector`
  > 把 AgentOpenEndpoint 登记表翻译成 MCP 工具，远程 /mcp 端点（Streamable HTTP）复用 sk-ak + scope，可被 Claude/Codex 当连接器接入
- [服务器权威性设计](design.platform.server-authority) `design.platform.server-authority`
  > 客户端断开不取消服务器任务的架构设计
...
- [工作流引擎设计方案](design.workflow-agent.engine) `design.workflow-agent.engine`
  > 可视化工作流引擎的整体架构设计

- [工作流自动配置设计](design.workflow-agent.auto-config) `design.workflow-agent.auto-config`
...
- [工作流引擎 v2 流程控制舱与 SSE 实时推送设计](design.workflow-agent.control-flow-sse) `design.workflow-agent.control-flow-sse`
  > 工作流引擎 v2 的控制流与 SSE 实时推送

- [PRD Admin 设计系统规范](design.admin) `design.admin`
  > 管理后台的设计系统、组件规范与视觉标准

- [账户数据共享设计文档](design.platform.account-data-sharing) `design.platform.account-data-sharing`
  > 账户数据跨应用共享的技术方案

- [桌面端更新与分布式登录/会话审计说明](design.platform.ops-auth) `design.platform.ops-auth`
  > 桌面端自动更新机制与分布式会话管理

- [多通道适配器设计](design.channel-agent.adapter) `design.channel-agent.adapter`
  > 多消息通道接入的适配器架构设计

- [苹果快捷指令集成设计方案](design.shortcuts-agent.apple) `design.shortcuts-agent.apple`
...
- [缺陷管理标签体系设计](design.defect-agent.labels) `design.defect-agent.labels`
  > 缺陷协作标签的枚举、权限、展示和桌面端同步设计

- [缺陷截图 VLM 预解析设计文档](design.defect-agent.image-analysis) `design.defect-agent.image-analysis`
  > 使用视觉语言模型自动解析缺陷截图的设计
...
- [多文档知识库与文档类型系统设计文档](design.knowledge-base.multi-doc) `design.knowledge-base.multi-doc`
  > 知识库多文档管理与文档类型系统的设计

- [多文档上下文与引用系统重设计](design.knowledge-base.multi-doc-citations) `design.knowledge-base.multi-doc-citations`
  > 多文档引用与上下文管理的重构方案

- [多图组合生成设计](design.visual-agent.multi-image-compose) `design.visual-agent.multi-image-compose`
  > 多张图片组合生成的技术方案

- [多图生成设计文档（Vision API 方案）](design.visual-agent.multi-image-vision-api) `design.visual-agent.multi-image-vision-api`
  > 基于 Vision API 的多图生成技术方案

- [内联图片聊天分析功能改进设计](design.visual-agent.inline-image-chat) `design.visual-agent.inline-image-chat`
  > 聊天中内联图片分析能力的改进方案

- [图片引用日志与消息持久化架构设计](design.platform.image-ref-and-persistence) `design.platform.image-ref-and-persistence`
  > 图片引用追踪与消息服务器权威持久化的架构

- [视频场景代码生成架构设计](design.video-agent.scene-codegen) `design.video-agent.scene-codegen`
  > LLM 驱动的 Remotion 视频场景代码生成架构

- [Remotion 视频质量差距分析报告](design.video-agent.remotion-gap) `design.video-agent.remotion-gap`
  > Remotion 视频生成质量问题的分析与改进方案

- [网页托管与分享设计文档](design.web-hosting) `design.web-hosting`
  > COS 静态站点托管与分享链接的设计方案

- [AI 竞技场设计方案](design.arena) `design.arena`
  > 多模型对抗评测的竞技场功能设计

- [AI 百宝箱设计方案](design.ai-toolbox) `design.ai-toolbox`
  > AI 工具集合的整体架构与功能设计

- [可复用组件产品方案](design.frontend.reusable-patterns) `design.frontend.reusable-patterns`
  > 跨模块可复用 UI 组件与模式的产品方案

- [左右布局重新设计总结](design.frontend.left-right-layout) `design.frontend.left-right-layout`
  > 页面左右分栏布局的重构总结

- [Toast 通知系统实现](design.frontend.toast) `design.frontend.toast`
  > 全局 Toast 通知组件的实现方案
...
- [CDS Agent 工作区基础设施模型](design.knowledge-base.agent-architecture) `design.knowledge-base.agent-architecture`
...
- [AI 周报系统市场调研报告](design.report-agent.ai-systems) `design.report-agent.ai-systems`
  > AI 周报领域 20+ 竞品分析与战略方向研究

- [CDS 部署架构与冲突分析](design.platform.exec-bt-deployment) `design.platform.exec-bt-deployment`
  > 部署脚本的架构设计与冲突处理策略

- [模型池故障转移与自动探活设计](design.platform.model-pool-failover) `design.platform.model-pool-failover`
  > 模型池健康检查与故障自动恢复的设计方案

- [生成快照设计](design.platform.generation-snapshot) `design.platform.generation-snapshot`
...
- [开放接口（OpenAI 兼容对外网关）技术设计](design.open-platform.open-api) `design.open-platform.open-api`
  > AgentApiKey 长效 M2M 鉴权 + scope 白名单 + findmapskills 官方技能同步到海鲜市场 + P3 Agent 开放接口铺路

- [RBAC 权限系统设计](design.platform.rbac-permission) `design.platform.rbac-permission`
...
- [系统涌现：从基础组件到协同智能](design.emergence.system) `design.emergence.system`
  > MAP 平台从基础能力到协同智能的涌现机制说明

- [Visual Agent (视觉创作) 架构设计](design.visual-agent) `design.visual-agent`
  > 视觉创作 Agent 的三栏工作区架构与生图流水线设计

- [技能系统统一设计](design.skill.unified-skill-system) `design.skill.unified-skill-system`
  > Claude Code Skill 的统一管理架构与注册机制设计

- [文档空间设计](design.knowledge-base.store) `design.knowledge-base.store`
  > 文档空间多文档上传、内容预览与订阅源定期同步设计

- [知识库跨环境同步设计](design.knowledge-base.store-sync) `design.knowledge-base.store-sync`
...
- [外部授权中心设计](design.platform.external-authorization) `design.platform.external-authorization`
...
- [GitHub 基础设施层设计](design.platform.github-infrastructure) `design.platform.github-infrastructure`
  > Infrastructure.GitHub 层的架构与 webhook 集成设计

- [模型中继虚拟平台设计](design.platform.exchange-virtual-platform) `design.platform.exchange-virtual-platform`
  > Exchange as Virtual Platform 的模型中继与虚拟平台设计

- [Claude SDK 执行器设计](design.cds.agent.sdk-executor) `design.cds.agent.sdk-executor`
  > CLI Agent 执行器 claude-sdk 类型的架构与 Python sidecar 设计

- [CLI Agent 工作空间设计](design.platform.workspace) `design.platform.workspace`
  > CLI Agent 工作空间上下文共享与隔离架构设计

- [跨存储迁移与资源分离设计](design.platform.storage-migration) `design.platform.storage-migration`
...
- [缺陷自动化自治体系](design.defect-agent.automation-autonomy) `design.defect-agent.automation-autonomy`
  > 把缺陷自动化从「会办事的脚本」收敛为有边界、能担责的自治体系：自主三档边界 + 五层自治回路（认知/规划/执行/记忆/监督），每层接真实代码

- [系统级跨节点互传（Peer Sync）设计](design.platform.peer-sync) `design.platform.peer-sync`
  > 多节点间消息互传的协议设计、数据模型与同步机制

- [文档再加工 · 智能体调用路由设计](design.prd-agent.reprocess-chat-routing) `design.prd-agent.reprocess-chat-routing`
...
- [PRD Agent 开发文档](guide.platform.development-guide) `guide.platform.development-guide`
  > 开发环境搭建、构建命令、调试技巧
...
- [MAP MCP 连接器接入指南](guide.platform.mcp-connector) `guide.platform.mcp-connector`
...
- [周报功能完整操作指南](guide.report-agent.weekly) `guide.report-agent.weekly`
  > 周报功能端到端操作指南

- [多平台博主订阅 → 首页海报弹窗](guide.submission-gallery.poster-feed-card) `guide.submission-gallery.poster-feed-card`
...
- [疑难杂症排查手册](guide.platform.troubleshooting) `guide.platform.troubleshooting`
  > 常见问题的诊断与解决方案（ReactFlow 路由冲突等）

- [AI 百宝箱运维指南](guide.ai-toolbox.ops) `guide.ai-toolbox.ops`
  > AI 百宝箱功能的运维操作手册

- [MongoDB 索引手册](guide.platform.mongodb-indexes) `guide.platform.mongodb-indexes`
  > MongoDB 索引的创建规范与手动维护操作指南

- [多图组合功能测试备忘录](guide.visual-agent.multi-image-compose-test) `guide.visual-agent.multi-image-compose-test`
  > 多图组合生成功能的测试用例与验证记录

- [豆包 ASR 模型中继接入指南](guide.speech-agent.doubao-asr-relay) `guide.speech-agent.doubao-asr-relay`
- [开放接口（OpenAI 兼容对外网关）接入指南](guide.open-platform.open-api) `guide.open-platform.open-api`
...
- [Claude SDK 三步接入指南](guide.cds.agent.sdk-quickstart) `guide.cds.agent.sdk-quickstart`
  > 三步把 Claude Agent SDK 接进本系统，零代码改动、零专业知识

- [Claude SDK + CDS MAP 配对 MVP 指南](guide.cds.agent.sdk-cds-map-mvp) `guide.cds.agent.sdk-cds-map-mvp`
  > CDS 调度外部 Anthropic Agent SDK sidecar，MAP 通过 CDS-MAP 配对发现并路由到该 sidecar

- [Playwright E2E 测试指南](guide.platform.e2e-tests) `guide.platform.e2e-tests`
  > 端到端测试的编写、运行与调试操作指南
...
- [冒烟测试指南](guide.platform.smoke-tests) `guide.platform.smoke-tests`
...
- [应用身份定义规则](rule.platform.app-identity) `rule.platform.app-identity`
  > appKey、Feature、appCallerCode 的命名与使用规范

- [Agent 权限分类规则](rule.platform.agent-permissions) `rule.platform.agent-permissions`
  > Agent 权限的分类体系与注册规范

- [Agent 开发交付流程规范](rule.platform.agent-development) `rule.platform.agent-development`
  > Agent 类功能从设计到交付的标准流程

- [数据字典](rule.platform.data-dictionary) `rule.platform.data-dictionary`
  > 数据库集合、缓存 Key 与持久化结构的完整清单

- [默认可编辑原则](rule.frontend.default-editable) `rule.frontend.default-editable`
  > 表单字段默认可编辑的设计原则与例外条件

- [前端组件复用规则](rule.frontend.frontend-component-reuse) `rule.frontend.frontend-component-reuse`
  > 跨页面共享组件的提取与复用规范
...
- [PRD Agent 全面代码审计报告](rule.frontend.audit-prd-desktop-codebase) `rule.frontend.audit-prd-desktop-codebase`
  > 桌面端代码库的全面审计报告与改进建议
...
- [流式文本动效统一规范](rule.frontend.streaming-text) `rule.frontend.streaming-text`
...
- [前端模态框布局硬约束规则](rule.frontend.frontend-modal) `rule.frontend.frontend-modal`
...
- [PRD Agent 多文档系统设计方案](plan.knowledge-base.multi-document) `plan.knowledge-base.multi-document`
  > 多文档系统的整体设计与实施方案

- [多图 AI 交互方案](plan.visual-agent.multi-image-ai-interaction) `plan.visual-agent.multi-image-ai-interaction`
  > 多图场景下 AI 交互的实施方案

- [视频 TTS 语音接入与场景视觉升级实施方案](plan.video-agent.tts-and-scene-upgrade) `plan.video-agent.tts-and-scene-upgrade`
  > 视频生成 TTS 语音与场景视觉效果的升级方案

- [网页托管与分享实现计划](plan.web-hosting) `plan.web-hosting`
  > 网页托管功能的分阶段实现计划

- [海报工坊设计器重构交接](plan.report-agent.weekly-poster-handoff) `plan.report-agent.weekly-poster-handoff`
...
- [AI 文本辅助通用 Domain 设计](plan.ai-toolbox.text-assist) `plan.ai-toolbox.text-assist`
...
- [配图标记手动干预](plan.visual-agent.manual-image-marking) `plan.visual-agent.manual-image-marking`
  > 配图标记从"自动黑盒"升级为"提示词+位置策略+段落级操作"的分阶段计划（Phase 1/2/3）

- [动画优化计划](plan.frontend.animation-optimization) `plan.frontend.animation-optimization`
...
- [合并计划书：三入口守门员统一](plan.visual-agent.merge-image-ref-resolver) `plan.visual-agent.merge-image-ref-resolver`
  > 图片引用解析三入口的统一合并方案
...
- [Design 文档优化项目进度](plan.doc.design-doc-optimization) `plan.doc.design-doc-optimization`
...
- [prd-admin 样式统一迁移看板](plan.frontend.prd-admin-surface-style-migration) `plan.frontend.prd-admin-surface-style-migration`
...
- [更新中心（终身存储 + SSE 推送）债务台账](debt.platform.changelog-center) `debt.platform.changelog-center`
  > 3 条 open：推送中枢进程内单例（多实例需 Redis/change stream）/ 刷新周期不分视图冷热 / GitHub 日志前端 35s 轮询与 SSE 并存
- [项目管理智能体 · 债务台账](debt.product-agent.pm) `debt.product-agent.pm`
...
- [知识库划词评论（批注栏/内联 + 就地输入 + 回读闭环）· 债务台账](debt.knowledge-base.inline-comment) `debt.knowledge-base.inline-comment`
  > 6 条边界：图片批注未做、inline 卡片定位 MVP、批注栏按时间非锚点排序、批注栏/TOC 互斥、回读为轮询、偏好走 localStorage
- [知识库划词 AI 局部编辑（AI 改写 + 选区配图）· 债务台账](debt.knowledge-base.selection-ai) `debt.knowledge-base.selection-ai`
...
- [视觉分镜台 · 债务台账](debt.visual-agent.storyboard) `debt.visual-agent.storyboard`
...
- [知识库跨环境同步 · 债务台账](debt.knowledge-base.store-sync) `debt.knowledge-base.store-sync`
  > 库↔库/跨环境同步首版已知边界：不传播删除、只搬文本、双向冲突本地优先、库级变更检测、令牌永久、需网络互通

- [分享链接安全债务台账](debt.platform.share-link-security) `debt.platform.share-link-security`
  > 分享链接系统的已知安全边界与待补项

- [网页托管真实客户端 IP 债务台账](debt.web-hosting.client-ip) `debt.web-hosting.client-ip`
  > XFF 解析的私网链伪造边界与彻底方案（需部署侧可信代理配置）

- [团队（网页托管 + 知识库）债务台账](debt.team-activity.team-feature) `debt.team-activity.team-feature`
  > 团队 wave 1 已知边界：回收站未做 / KB 次要端点仍 owner-only / 公共团队仅留字段 / 邀请链接路由 / 旧条目头像兜底
- [验收技能（create-visual-test-to-kb）债务台账](debt.visual-agent.acceptance-skill) `debt.visual-agent.acceptance-skill`
...
- [殿堂阅读器（克莱风暖色）保留未融合 DocBrowser 的债务台账](debt.knowledge-base.library-doc-reader) `debt.knowledge-base.library-doc-reader`
  > LibraryDocReader 720 行刻意做了米黄+圆体字+暖色调差异化品牌，融合需先做 DocBrowser 皮肤系统；记录重启评估的 4 个触发条件，避免下次重复调研
- [周报详情页（异构数据）保留独立实现的债务台账](debt.report-agent.detail) `debt.report-agent.detail`
...
- [开放接口对外网关债务台账](debt.open-platform.open-api) `debt.open-platform.open-api`
  > Phase 1 落地网关 + 按 Key 绑定 + 默认回落 + 导航修复；伞形 appCallerCode 静态注册不变量（守卫测试）；限流按 Key 桶 / 降级预警 / 配额 / embeddings 分到 Phase 2

- [日报技能债务台账](debt.report-agent.daily) `debt.report-agent.daily`
  > daily-report-summary 技能与 reference/publish.py 的已知边界与待补项

- [系统级跨节点互传（Peer Sync）债务台账](debt.platform.peer-sync) `debt.platform.peer-sync`
  > prd-api PeerSync + prd-admin 系统互联的已知边界与 Phase 2 待补功能

- [个人任务树 Agent · 债务台账](debt.workflow-agent.task-tree) `debt.workflow-agent.task-tree`
...
| 2026-05-07 | 调整 | `plan.emergence.tiktok-douyin-poster` | Phase 3 已交付，新增 §3 多平台 / media-rehost / feed-card / ASR 字幕 + §3.7 关键文件 + §3.8 已知边界 |
...
| 2026-05-06 | 调整 | `plan.emergence.tiktok-douyin-poster` | Phase 2 任务 A/B/C 已交付，新增 §2 完整交付总览 + §2.6 已知边界 |
...
| 2026-03-31 | 新增 | `design.platform.rbac-permission` | RBAC 权限系统设计 |
| 2026-03-31 | 新增 | `design.report-agent` | 周报管理 Agent 架构设计 |
| 2026-03-31 | 新增 | `design.review-agent` | 产品评审员技术设计文档 |
| 2026-03-31 | 新增 | `design.emergence.system` | 系统涌现：从基础组件到协同智能 |
| 2026-03-31 | 新增 | `design.visual-agent` | Visual Agent 架构设计 |
| 2026-03-31 | 新增 | `guide.platform.mongodb-indexes` | MongoDB 索引手册 |
| 2026-03-31 | 新增 | `guide.speech-agent.doubao-asr-relay` | 豆包 ASR 模型中继接入指南 |
...
| 2026-03-23 | 新增 | `design.platform.generation-snapshot` | 生成快照设计 |
...
| 2026-03-15 | 新增 | `guide.platform.troubleshooting` | 疑难杂症排查手册 |
