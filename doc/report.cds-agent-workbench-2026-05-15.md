...
> **版本**：v1.1 | **日期**：2026-05-17 | **状态**：MVP 已落地，生产级限制已补充说明

## 一句话结论

CDS Agent 工作台已经从“连接探活页面”推进到“可由 MAP 统一配置、启动远程 sandbox、对话、审批工具、回看日志和产物、操作远程浏览器、接入工作流与智能体，并最终让远程 Agent 巡检本仓库提交 PR”的 MVP 闭环。

这里的 MVP 闭环不等于生产级完整闭环：当前页面仍靠轮询刷新事件，发送消息会同步等待后端处理，停止不保证取消已经派发的 sidecar run，事件详情默认最多 500 条，长命令仍有 180 秒上限。

命名校准：报告中的 `claude-sdk` 是历史 runtime 名。当前实现使用官方 `anthropic` Python SDK 调 Claude Messages API，但 agent loop、sidecar HTTP/SSE 协议、工具审批、repo/PR/Bridge 工具和 MAP/CDS 事件转译都是本仓库自研。它不是完整官方 Claude Code SDK / Claude Agent SDK 接入。
...
| 远程会话生命周期 | 新建、启动、发送、停止、刷新恢复 | A10 会话完成后回写 `stopped`，但停止取消 sidecar run 仍待补 |
| 事件时间线 | 页面展示状态、文本、工具、日志、错误、完成事件 | A10 页面显示 500 条事件；长会话分页待补 |
...
3. 再从 fake runtime 切到真实 Claude sidecar runtime。
...
| 发送消息同步等待 | 用户发起长任务时页面可能像卡住 | 当前靠轮询刷新事件缓解 | 后台 run + 真 SSE |
| 停止不取消 sidecar run | 资源可能继续消耗 | 当前停止 CDS session | 持久化 runId 并调用 sidecar cancel |
| 事件 500 条截断 | 长审查证据可能不完整 | 当前 A10 刚好以 500 条展示 | afterSeq 分页和证据包 |
| 180 秒命令限制 | 大型测试容易超时 | 当前适合最小测试 | 长命令后台化 |
| `claude-sdk` 命名误导 | 误判官方 SDK 接入程度 | 本报告 v1.1 已校准 | UI/配置文案继续收敛 |
...
| 多 runtime adapter | medium | 当前主路径是 Claude sidecar runtime | 后续补官方 Claude Code SDK/Claude Agent SDK、Codex/OpenAI Agents SDK 或自定义容器 adapter |
