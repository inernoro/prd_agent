...
| 10 | 停止但未删除的 PR 分支不走分流页 | PR closed 后若分支**未被自动删除**（仓库关了 auto-delete merged branch），`BranchEntry` 仍在、状态 `stopped`，`proxy.routeToBranch` 命中现存 entry → 服务泛化「分支已停止」状态页，**走不到** `serveBranchGonePage`，合并/放弃页不显示（Codex P2，2026-06-24）。**本仓库 `inernoro/prd_agent` 自动删除合并分支**（delete webhook 必发），所以合并/放弃页在此生效，仅「保留 PR 分支」的仓库受影响。修复需改 `routeToBranch` 核心路由（与 auto-wake/中断恢复/冷却交织）或新增 proxy→gone 页回调，回归风险 > 收益，暂记债务不动核心路由 | 低（本仓库不触发） |
