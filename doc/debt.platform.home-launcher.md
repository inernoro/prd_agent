...
| # | 边界 | 说明 | 补法 |
|---|------|------|------|
| 1 | 「继续上次」埋点覆盖 6 类（2026-07-05 二期补齐） | 已覆盖：视觉工作区、文学工作区、工作流、缺陷、周报、产品评审（打开详情端点即打点）。仍未覆盖及原因：知识库（`/document-store` 页面不支持 `?store=` 深链，需先加查询参数路由）、涌现树 / 视频创作 / PR 审查（无 `:id` 详情路由，工作现场是页内状态）、网页托管（管理动作非工作现场）、对话式 Agent（会话另有历史入口） | 补新来源三步：详情端点加 `RecentOpenTracker.TouchAsync` + 聚合端点补标题富化与路由 + 前端 `RECENT_AGENT_META` 补图标标签；无深链的先给页面加 `?id=` 路由支持 |
