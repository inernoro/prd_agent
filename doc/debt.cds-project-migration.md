...
| 6 | 迁移为系统级 admin-only | PR #909 review（Codex P1 / Bugbot）指出：迁移会跨节点并可能回退本机 bootstrap `AI_ACCESS_KEY` 鉴权远端，若放开给项目级 Agent Key，持有者可新增攻击者控制 baseUrl 的 peer 诱导服务端外泄 bootstrap key，或看/删别项目的全局迁移目标。已把整个 `project-migration` 路由 `guard()` 改为拒绝 `req.cdsProjectKey`（仅 admin/cookie/bootstrap 可用） | 若将来要支持项目级 Agent 自助迁移，需把 CdsPeer 下沉为项目级资源 + 禁用 bootstrap-key 回退（强制每 peer 显式 key） |
