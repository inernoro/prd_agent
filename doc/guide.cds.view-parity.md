...
| # | 动作 | 触发位置 | 底层函数 | 状态 |
|---|---|---|---|---|
| L1 | 部署 / 重新部署 | 卡片左侧 split-button 主体 | `deployBranch(id)` | 是 |
| L2 | 重新部署单个服务 | deploy dropdown → 选择服务 | `deploySingleService(id, profileId)` | 是 |
| L3 | 切换部署模式 | deploy dropdown → 部署模式 | `switchModeAndDeploy(id, profileId, modeId)` | 是 |
| L4 | 指定集群派发节点 | deploy dropdown → 派发到 | `deployToTarget(id, executorId)` | 是 |
| L5 | 停止所有服务 | deploy dropdown → 停止 | `stopBranch(id)` | 是 |
| L6 | 查看部署日志 | deploy dropdown → 部署日志 / 卡片内 inline | `viewBranchLogs(id)` / `openFullDeployLog(id)` | 是 |
| L7 | 容器配置(继承/覆盖) | deploy dropdown → 容器配置 | `openOverrideModal(id)` | 是 |
| L8 | 删除分支 | deploy dropdown → 删除分支 (danger) | `removeBranch(id)` | 是 |
| L9 | 预览 | 右上角 preview 按钮(running 时) | `previewBranch(id)` | 是 |
| L10 | 错误重置 | stopped/error 时右侧按钮 | `resetBranch(id)` | 是 |
| L11 | 查看单服务容器日志 | 端口 chip 点击 | `viewContainerLogs(id, profileId)` | 是 |
| L12 | 添加标签 | 分支标签行 "+ 标签" | `addTagToBranch(id)` | 是 |
| L13 | 删除标签 | 标签 `×` 按钮 | `removeTagFromBranch(id, tag)` | 是 |
| L14 | 编辑标签 | 标签行  图标 | `editBranchTags(id)` | 是 |
| L15 | 按标签筛选 | 标签本体点击 | `filterByTag(tag)` | 是 |
| L16 | 查看提交日志 | commit area 点击 | `toggleCommitLog(id)` | 是 |
...
| LG2 | 刷新远端分支列表 | 搜索框右侧 更新 | `refreshAll()` |
| LG3 | 构建配置管理 | 工作流 菜单 → 构建配置 | `openProfileModal()` |
| LG4 | 路由规则管理 | 工作流 菜单 → 路由规则 | `openRoutingModal()` |
| LG5 | 基础设施管理 | 工作流 菜单 → 基础设施 | `openInfraModal()` |
| LG6 | 项目导出 | 工作流 菜单 → 导出 | `showExportDialog()` |
...
| T2 | 左侧 sub-nav | 工作流 项目设置 | 跳 `settings.html` |
...
| L2 | deploySingleService(单服务) | — | 否 拓扑不支持多服务内部选一个(可 future) |
| L3 | switchModeAndDeploy | Settings tab 部署模式展示 | 警告 只展示 · 不切换 |
| L4 | deployToTarget | Settings tab 集群派发展示 | 警告 只展示 · 不指定 |
...
| L10 | resetBranch | — | 否 错误状态重置没入口(可 future) |
| L11 | viewContainerLogs(单 profile) | 部署日志 tab | 一致 |
| L12-L15 | 标签 CRUD + 筛选 | 备注 tab(只读) | 警告 只读 · 不能 inline 改 |
| L16 | toggleCommitLog | — | 否 拓扑没有 commit 历史入口 |
| LG1 | addBranch | 分支 combobox + Enter(UF-07) | 一致 |
| LG2 | refreshAll | — | 否 拓扑没刷新按钮(轮询自动刷新) |
...
| **GAP-16** | 拓扑缺手动刷新按钮 | P3 | 顶栏加 更新 图标调 `refreshAll` |
