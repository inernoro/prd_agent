...
| 2 | open(2026-09-03) | 自更新的 restart 步骤两次未真正重启进程：`self update` 报 `restarted: true`、事件走到 `drain done`，但 `/api/self-status` 的 pidStartedAt 不变、接口仍返回旧代码；`self restart` 能正常重启 | 后端改动自更新后看似成功实则没上线，验收会对着旧进程取证（本次靠对比接口字段发现）；零停机前端路径不受影响 | 排查 drain 之后的重启分支为何未触发（systemd restart 未下发或被排空闸门吞掉），自更新完成后应比对 pidStartedAt 与更新时间并在不一致时报错，而不是只报 restarted:true |
