...
> **主干落地 160 次提交 | 全分支研发活动 593 次提交 | 490 个文件变更 | +69,908 行 / -6,939 行 | 20 个 PR 收口项（详见附录）**
>
> **贡献者（主干可达）**：Claude (84)、inernoro / InerNoro (74)、hy736797616-beep (1)、Yu Ruipeng (1)
> **贡献者（全分支研发活动 593）**：inernoro (410)、Claude (134)、Cursor Agent (27)、InerNoro (20)、hy736797616-beep (1)、Yu Ruipeng (1)
>
> **统计口径**：头部"主干落地"数字仅统计 `origin/main` 主干分支（weekly 技能纪律 #2：禁用 `--all`），按提交日期文本（`%cd --date=short`）过滤 `2026-05-18 ~ 2026-05-24`；PR 边界以 GitHub `mergedAt` 落地主干判断；文件 / 行变更口径为 `git diff --shortstat FIRST^..LAST`（包含跨 PR 合并副作用）。
> **为什么主干 160 ≪ 全分支 593**：① squash 合并把单个 PR 分支的几十个 commit 压成主干上的 1 个；② 大量提交仍在未合并的特性分支上。"全分支 593"才是本周真实敲键次数（按 `git log --all` 同口径日期过滤），"主干 160"是落地主干的去重视角，两者回答不同问题，均无错。`first-parent` 主干落地事件为 107 次。
