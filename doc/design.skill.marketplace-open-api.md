...
## 十一、技能分享 UX 增强（2026-06-14）

> 对应 changelog：`2026-06-14_marketplace-skill-preview-and-share.md`

### 11.1 技能分享弹窗（SkillShareDialog）

用户可对自己发布的技能生成有效期链接，支持选择有效期（永久 / 7 / 30 / 90 天）。

核心设计：
- 全局单例 `SkillShareDialog`，由卡片与详情弹窗共用，避免多实例竞态
- 生成链接期间 busy 闸在点击瞬间同步置位（不走 effect），杜绝竞态窗口
- 详情弹窗 Esc 让位给上层分享弹窗，不连关两层

分享链接格式：`/s/skill/{token}`，聚合进「我的分享」管理页（`marketplace_skill` 资源类型）。

### 11.2 公开页免登录下载

技能公开分享页顶部横条新增「下载技能压缩包」按钮：
- 免登录直接触发 zip 下载（复用已有公开 zip 端点）
- 访客无需注册即可获取技能包

### 11.3 「我的分享」聚合扩展

`GET /api/my-shares` 聚合接口扩展至 5 类资源，新增 `marketplace_skill` 类型。前端「我的分享」页新增「技能」分类标签与筛选。

### 11.4 官方技能排序调整

取消「官方推荐」独立置顶区，官方与社区技能按当前排序（热门 / 最新）混排。官方身份仍由卡片「官方」徽章标识，不再做列表层面的区域置顶。
