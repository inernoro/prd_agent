...
- **方案概述**：参考 Cursor 网络诊断实现，新增 6 项并发诊断（DNS/SSL/API/Ping/Chat/Agent），后端 + Rust + 前端三层协作
- **业务价值**：帮助用户快速定位网络连接问题，减少运维排查成本
- **影响范围**：后端（DiagnosticsController）、Rust Tauri 命令层、桌面端前端（NetworkDiagnosticsModal）
- **预计风险**：低 — 独立功能模块，不影响核心业务流程
