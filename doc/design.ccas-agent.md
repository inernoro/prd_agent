...
> **版本**：v1.0 | **日期**：2026-06-15 | **状态**：已落地

**一句话**：面向产线赋码业务的三件套：需求文档生成、设备素材库、流程示意图。
**谁该读**：该智能体的产品与工程师。
**读完能做什么**：说清它覆盖哪三块能力与各自边界。

---

> **appKey**：`ccas-agent` | **路由**：`/ccas-agent`
...
| POST | `/api/ccas-agent/equipment/generate` | AI 生图（非 SSE，返回图片 URL） |
...
| 方法 | 路径 | 用途 |
|------|------|------|
| GET | `/api/ccas-agent/flow/diagrams` | 历史列表（分页） |
| POST | `/api/ccas-agent/flow/parse-stream` | AI 生成（SSE） |
| POST | `/api/ccas-agent/flow/diagrams` | 保存流程图 |
| GET | `/api/ccas-agent/flow/diagrams/{id}` | 获取单条 |
