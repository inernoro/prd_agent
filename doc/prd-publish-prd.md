...
- **手动重试**：发布失败后显示 [重试] 按钮，一键重新执行
- **自动重试**（可配置）：
  - `PUBLISH_AUTO_RETRY=true` 启用
  - `PUBLISH_RETRY_COUNT=3` 最大重试次数
  - `PUBLISH_RETRY_DELAY=5000` 重试间隔（毫秒）
- **重试策略**：
  - 仅对超时、网络错误自动重试
  - 脚本逻辑错误不自动重试
- **重试状态展示**：
...
| 信息 | 说明 | 示例 |
|------|------|------|
| 当前 Commit | 短 hash + 消息 | `a1b2c3d` feat: Add feature |
| 部署时间 | 上次成功发布时间 | `2小时前` |
| 部署状态 | 成功/失败/进行中 | ✓ 成功 |
| 运行时长 | 当前版本运行时间 | `运行 3天 2小时` |

#### 3.4.2 版本对比

- 当前版本在列表中高亮标识（绿色圆点 ●）
- 其他版本显示空心圆点（○）
- 如果当前版本不在可见列表中，显示 "跳转到当前版本"

#### 3.4.3 版本不一致警告

- 检测 Git HEAD 与记录的部署版本是否一致
- 不一致时显示警告：⚠️ "检测到版本不一致，可能有人手动修改"
...
| `GET /api/status` | GET | 当前状态（含当前版本） | 是 |
| `POST /api/deploy` | POST | 执行发布 | 是 |
| `GET /api/deploy/stream` | GET | 发布日志 SSE | 是 |
| `POST /api/deploy/cancel` | POST | 取消发布 | 是 |
| `POST /api/deploy/retry` | POST | 重试上次失败的发布 | 是 |
| `GET /api/history` | GET | 发布历史 | 是 |
| `POST /api/history/:id/retry` | POST | 从历史记录重试 | 是 |
...
# 重试配置
PUBLISH_AUTO_RETRY=false
PUBLISH_RETRY_COUNT=3
PUBLISH_RETRY_DELAY=5000
```
