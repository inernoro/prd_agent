...
- 沉淀：`.claude/rules/compute-then-send.md` 规则上线 + `.claude/skills/llm-call-trace/` 技能上线 + [doc/design.platform.llm-gateway.refactor.md](./design.platform.llm-gateway.refactor.md) 状态文档。
...
- 沉淀：[doc/design.skill.marketplace-open-api.md](./design.skill.marketplace-open-api.md) + [doc/plan.skill.marketplace-open-api-next.md](./plan.skill.marketplace-open-api-next.md) + `agent_api_keys` / `agent_open_endpoints` / `external_authorizations` 三个新集合。
...
- 文档：[doc/design.daily-tips.md](./design.daily-tips.md)（原理）+ [doc/plan.daily-tips.scenarios-and-staleness.md](./plan.daily-tips.scenarios-and-staleness.md)（当前剩余工作交接）。
...
- PR #484：项目隔离 8 处修复 + [doc/rule.cds.project-isolation-audit.md](./rule.cds.project-isolation-audit.md) 反思 MECE 矩阵盲区。
...
- PR #457：GitHub Webhook 噪声事件过滤 + 断开 500 重试循环 + 部署去重 + [doc/guide.cds.github-webhook-events.md](./guide.cds.github-webhook-events.md)。
...
- 文档：[doc/design.platform.external-authorization.md](./design.platform.external-authorization.md)。
...
- 文档：[doc/plan.visual-agent.manual-image-marking.md](./plan.visual-agent.manual-image-marking.md)（计划） + [doc/design.literary-agent.md](./design.literary-agent.md)（原理归档）。
- 相关 PR：#464（4 commits）。
...
- 新增 [doc/debt.video-agent.md](./debt.video-agent.md) 录入视频 Agent 4 条债务 + 修订 [doc/rule.doc.naming.md](./rule.doc.naming.md) 引入 debt 前缀 + 同步 `doc/index.yml` 与 [doc/guide.list.directory.md](./guide.list.directory.md)。
...
| P1  | Daily Tips 真实运营回归 + 接力交接落地          | 当前交接统一收敛到 [doc/plan.daily-tips.scenarios-and-staleness.md](./plan.daily-tips.scenarios-and-staleness.md)。下周需要把"用户实际使用率 / 完成率 / 跳过率 / 7 天后再次推送"等指标接入。 |
