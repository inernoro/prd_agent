...
> **版本**：v0.3 | **日期**：2026-09-25 | **状态**：已确认，实施中
...
## 六、已拍板的两件事（2026-09-24）

**1. 隔离：方案 A。** 每个服务实例同一时间只跑一个任务，跑完清空工作目录与引擎数据目录；实例忙时拒绝新任务并告诉 MAP 多久后再试，MAP 按排队处理并在界面显示排队位置。要并发就多开实例。没有选的方案 B（服务为每个任务再起隔离容器）等真到并发不够再评估。

**2. 出网：** 服务内部把引擎的模型地址固定为本机转发口，由转发口带上 MAP 下发的本次任务票据转到 MAP 出口，引擎本身拿不到票据；联网搜索保持关闭。网络层白名单交给生产部署配置；分支预览接受「能出网但不带任何密钥」的状态，记入台账。

| 做法 | 隔离强度 | 代价 |
|---|---|---|
| A（已选）：每实例一次一个任务，跑完清空 | 任务之间不共存，彼此读不到文件与票据 | 并发数等于实例数，排队时用户看到排队位置 |
| B（未选）：服务为每个任务再起隔离容器 | 最强 | 服务要操作 docker，部署与安全审查都重 |

## 七、迁移步骤

| 阶段 | 内容 | 验收 | 状态（2026-09-25） |
|---|---|---|---|
| 1 | 新建 OpenDesign 服务：把 CDS 里的设计执行逻辑（任务书、提示词、自查与修复、预览、结果打包）原样搬进去，实现第四节的协议；分支镜像与部署清单各加一条 | 分支预览里起得来，查能力返回健康 | 代码已合入；远端构建与分支预览结论见台账 |
| 2 | MAP 新增「按协议调用执行服务」的适配器；新增合格判断接口，执行服务的修复回路改问 MAP | 在分支预览里真人走通一次生成与一次截图修改，并看到实时预览与风格效果 | 前半已落地：适配器直连执行服务（查能力、提交、读事件断线续读、忙时排队重试、取消），事件翻译与经 CDS 路径共用一份，面板显示网关实际使用的模型。后半「合格判断接口」未做，记入台账。真人走通未验 |
| 3 | MAP 默认切到新适配器，旧的经 CDS 会话路径保留一个版本作回退 | 连续数次生成成功，两道判据不再出现分歧 | 已落地开关：地址与密钥都配好的环境默认走新适配器，缺任一项照旧走 CDS 会话；运维可用一项配置显式退回旧路径。新路径出错时如实失败，不会悄悄改走旧路径。「连续数次生成成功」未验 |
| 4 | 从 CDS 删除设计执行代码，CDS 只剩通用部署能力 | CDS 中搜不到任务书、提示词、设计系统相关代码 | 未开始 |

回退开关只有一处判定：部署配置里的传输方式一项（`service` 直连执行服务 / `cds-session` 经 CDS 会话），不填时按执行服务的地址与密钥是否齐全自动选择。显式要求直连但地址或密钥缺失时，OpenDesign 在能力列表里显示为不可用并说明缺什么，而不是退回旧路径——两条路径的失败长得不一样，静默切换会让排查找错地方。
