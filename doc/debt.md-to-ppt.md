...
工作上下文隔离已部署，但真实请求捕获又证明导入尚未完成时快速发送可以提交空引用；等待导入完成后同一请求来源正确。新修复让发送按钮与快捷键共用首帧导入门禁，失败可重试并保留输入；实际组件、反向验证、全量和独立复审通过，仍待部署与真实生成闭环。不能用验收脚本额外等待掩盖产品问题。
