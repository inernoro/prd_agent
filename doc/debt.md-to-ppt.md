...
| 编号 | 问题及影响 | 当前处理 | 关闭条件 |
|------|------------|----------|----------|
| A35 | 刷新后导航提前报告就绪，用户可点击的首个翻页动作丢失 | 已取得就绪、点击、模板监听安装的准确时序；统一当前预览文档的就绪状态，不靠脚本等待 | 首次允许点击有效，旧文档消息不能污染新稿，编辑与圈选不退化 |
| A36 | 大纲跨进程调用缺少显式任务身份，审计只能按时间推断关联 | 两个大纲入口显式传递同一请求与任务身份，保持既有自动模型路由 | 真实大纲调用可按本次 Run 精确查询；历史缺失归属不伪造回填 |
| A38 | 四页真实演示稿中三页两轮校验失败后使用本地兜底，不代表四页模型结果通过 | 已排除传输失败；事实校验正确性、文案完整性和版式质量仍需独立核对 | 合法内容不被误拒，实际产物通过视觉与来源核验，再验证发布闭环 |
