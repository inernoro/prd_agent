...
| A36 | 大纲跨进程调用缺少显式任务身份，审计只能按时间推断关联 | 两个大纲入口显式传递同一请求与任务身份，保持既有自动模型路由 | 真实大纲调用可按本次 Run 精确查询；历史缺失归属不伪造回填 |
| A38 | 四页真实演示稿中三页两轮校验失败后使用本地兜底，不代表四页模型结果通过 | 完整响应独立核验发现正确页码、忠实缩写被拒，逐字要求与自行精炼冲突，重试输入相同；统一生成合同和安全拒绝反馈，保留原质量闸 | 生成内容满足统一合同，实际产物通过视觉与来源核验，再验证发布闭环 |
| A39 | 本分支额外收紧大纲和页面生成输出上限，与用户仅使用 Key 门禁的决定不一致 | 移除两条生成路径的任务侧固定 token 值；保留既有协议适配、模型能力、截图识别及工具执行安全边界 | 网关请求不再额外注入固定输出值，模型路由和显式任务审计保持，真实调用复验通过 |
