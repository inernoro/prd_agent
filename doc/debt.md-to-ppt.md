...
> **版本**：v1.1 | **日期**：2026-09-04 | **状态**：开发中

**一句话**：把长文转成网页幻灯片这条链路的已知边界与必须在后续重构里偿还的债。
**谁该读**：接手这个功能的工程师。
**读完能做什么**：判断预览与生成的当前限制，并挑出必还项。
...
## 新知识启动与旧会话串稿（A34，P1，来源绑定已复验）

从知识库明确选择另一篇资料启动 HTML PPT 时，旧会话恢复可能带回已生成网页，使发送动作误入旧稿修改。新知识显示为已带入，并不能证明它被本次生成使用。

修复限定为工作上下文隔离：新知识启动独立任务，普通菜单继续恢复最近上下文，刷新恢复当前任务，保留旧历史与可恢复草稿。旧请求迟到的响应不得覆盖新上下文；知识内容仍由现有服务解析和校验，不改模型配置或扩大执行器范围。

工作上下文隔离已部署，但真实请求捕获又证明导入尚未完成时快速发送可以提交空引用；新修复让发送按钮与快捷键共用首帧导入门禁，失败可重试并保留输入。25cc4c457 已真实观察到首帧以及有字输入期间均禁止发送，导入后大纲和生成请求携带正确来源身份与哈希，确认大纲和刷新恢复保持绑定；组件、反向验证与独立复审证据保留。发布与生成质量尚未通过，不能混用这项来源绑定结论。

验收必须覆盖新知识生成、导入中快速发送、导入失败恢复、刷新恢复、旧请求迟到和历史访问，并核对知识来源与生成任务绑定。组件回归通过后仍需 CDS 真实路径复验。

## 本轮真实验收仍未通过的项目（2026-09-09）

| 编号 | 问题及影响 | 当前处理 | 关闭条件 |
|------|------------|----------|----------|
| A35（closed） | 刷新后导航提前报告就绪，用户可点击的首个翻页动作丢失 | ff2143066 真实刷新从禁用到就绪，唯一首击进入第二页；连续可见性、视口命中及独立读图印证，旧文档消息与编辑合同回归保留 | 本项关闭不代表原有降级作品质量或完整发布验收通过 |
| A36（closed） | 大纲跨进程调用缺少显式任务身份，审计只能按时间推断关联 | 7b536232e 真实大纲精确任务查询返回唯一物理调用，请求、会话、用户、来源和版本均绑定；既有自动路由保持 | 新运行强关联已证明；历史缺失归属不伪造回填 |
| A38 | 生成流程完成不代表完整设计成功 | 5346672e2 同来源默认主题真实四页仍有三页兜底，独立评分 53 分未通过。来源已完整到达；实际模型原文包含免费、借阅未开等合法事实，完整原句门禁与短标签冲突后，程序兜底确定性丢失这些规则。换行数字边界与全主题真实表格已本地修复 | 生成、唯一重试与兜底须共享来源事实，受控标签与完整事实分开验证，不能开放任意子串或否定反转；新真实产物保留表格及参与规则且无占位、标题残片，本地测试不替代真实质量验收 |
| A39（closed） | 本分支额外收紧大纲和页面生成输出上限，与用户仅使用 Key 门禁的决定不一致 | 7b536232e 大纲一条与页级五条实际网关请求均未注入输出 token 字段，原路由及审计保持 | 任务侧限制已移除；模型能力、协议适配和 Key 门禁保留，不宣称上游无限输出 |

新样本四页桌面导航均经真实可见性检查；九张截图已全部回读。首张知识入口图仍处于正文加载阶段，只能证明入口，不能证明正文已渲染。主题探针未检测到背景变化，移动端未执行，因此这些证据不构成完整双主题或移动端验收。可见正文按完整行核对仅匹配部分来源是启发式信号，不能把所有未匹配行直接判为事实遗漏；明确可见的占位、裁切和缺表仍阻断整体质量通过。

新增表格的本地四、五、六列样例在桌面与手机视口共六种组合中未检测到单元格溢出或页眉页脚重叠，但这只是模板几何检查，不是模型产物验收。固定幻灯片在 390 像素视口缩放后文字仅约 6.9 像素；已为柔和编辑主题补阅读模式，本地正文放大至约 17 像素，主树真实触控确认等距平移不缩放、捏合后按钮首击生效及嵌套表格原生滚动。其他主题及实际生成发布后的手机阅读未通过，不能靠缩小字号消除溢出来关闭问题。
...
「对话 + artifact 实时预览」重构(见 [doc/plan.md-to-ppt.chat-redesign.md](./plan.md-to-ppt.chat-redesign.md))替换。在即将重写的
...
## 2026-06-23 补：降级统计修正 + 刷新恢复告警（PR #902 review 反哺）
...
## 零配置网关兜底与输出长度（2026-09-04）

真实 CDS 验收发现，新账号没有个人运行配置时，HTML PPT 会在已有 MAP 模型网关可用的情况下仍要求配置 CDS 连接和模型密钥。现已增加只依赖平台 AppCaller 的“MAP 默认模型”作为零配置兜底；只有用户明确选择 Codex、自定义 runtime 或 CDS Agent 兼容配置时，才进入动态容器与 CDS 授权链路。

当前默认池仍可能回落到最大输出 4096 token 的模型。并行逐页生成已把每页控制在该范围内，短 deck 的整篇兼容路径也能运行；超长单页或大体量自定义模板仍可能截断。后续应为 HTML PPT 配置专用长输出模型并做质量对照，不应重新把超限参数直接发给短输出模型。
