...
> **版本**：v1.3 | **日期**：2026-09-09 | **状态**：来源绑定及手机修复本地回归通过，真实质量验收未通过
...
| A38 | 生成流程完成不代表完整设计成功 | 5346672e2 同来源默认主题真实四页仍有三页兜底，独立评分 53 分未通过。58fc02ff0 已部署换行数字与真实表格修复；新来源绑定将完整冻结正文原位填入模型布局，初试、既有重试与兜底共用计划。最新合流后端 5067 项、前端 2490 项通过，尚未部署新来源绑定 | 新真实产物保留完整表格及参与规则，无占位、标题残片或不可读布局；受控 HTML 与本地测试不替代真实质量评分 |
...
知识大纲确认后，每个完整结构块归属于确认页面，生成任务保留自己的不可变副本；标题编辑不等于修改知识事实。以下限制明确保留，不能因初次生成覆盖通过而宣称完整编辑生命周期已验收。

| 边界 | 当前行为与影响 | 后续关闭条件 |
|------|----------------|--------------|
| 历史知识大纲 | 没有来源计划时要求重新生成；原作品保留可查看，不自动迁移历史证据 | 如需兼容，应明确来源重确认与历史身份，不能伪造原始快照 |
| 自定义整篇模板与目标页数 | 新知识路径暂只支持官方逐页主题和原目标页数；不满足时给恢复提示 | 模板能承载完整来源，页数调整有明确重新分配流程并经真实验收 |
| 后续人工编辑与单页修改 | 本批保证限于大纲到初次生成，既有编辑不自动继承不可删源承诺 | 单独明确用户修改事实、删源与审计的产品合同 |
| 任意 CSS 遮挡和长内容 | 静态门检查显式隐藏及最终内容覆盖，不能证明所有层叠、遮挡、滚动和手机可读性 | 实际模型产物在桌面与手机逐页视觉通过，不靠缩小字号消除溢出 |
| 降级稿的设计质量 | 完整来源兜底不截断事实，但仍如实标记降级 | 独立评分达到质量门，不将内容完整等同 AI 设计优秀 |

独立受控视觉复核发现默认主题手机只能停留第一页，按 P1 处理；表格日期与主题逐字挤压，按 P2 处理。新增明确翻页操作及原字号自然宽度横向阅读后，最终合体独立检查 64 项通过：四页触摸往返、首尾禁用、长页底部、表格横滚不翻页、完整尾注、剩余名额以及桌面键盘均成立，14 张截图逐张复核无阻断。尚未部署及完成 CDS 真实模型产物验收，不能把受控槽位稿当作 AI 作品通过。
