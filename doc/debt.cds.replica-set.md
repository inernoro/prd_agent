...
| 22 | open | 两页签重构定案（用户 2026-07-24 拍板）：容器级页签 = 一屏展示全部容器自上而下调用关系（废除服务下拉框），每容器独立加副本；项目级页签 = 整组添加（影子容器不隐藏，特殊标记「新创建容器·已做好负载·复制集成员」）；数据隔离时两级统一战线——隔离决策升到分支级，全部副本/影子统一切同一专用隔离实例，禁止一半连主库一半连隔离库 | 当前下拉框切换单服务的设计已确认不符合用户心智 | 波 6 与影子分支(#21)合并实现；执行计划引擎/专用隔离实例通道复用 |
