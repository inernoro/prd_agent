...
- **解决什么问题**：用户无法直观对比不同 AI 模型的回答质量，难以做出模型选型决策
- **方案概述**：构建盲评对战竞技场，用户提问后多模型匿名并行回答，用户先阅读再揭晓模型身份，消除品牌偏见
- **业务价值**：帮助用户基于内容质量而非品牌选择模型，同时为平台积累模型评测数据
- **影响范围**：新增用户对战页（/arena）、管理配置页（/lab?tab=arena），涉及 arena_groups、arena_slots、arena_battles 三个集合
- **预计风险**：中 — 多模型并行请求对 LLM Gateway 并发能力有要求
...
- **不做 ELO 排行** — 没有投票就没有排行
- **不做多轮对话** — v1 仅单轮（详见 12.1）
- **不做自定义 system prompt** — 纯净对比（同样为了消除上下文变量）
- **不做加密隔离** — 盲评是 UI 层匿名化，前端内存中有 modelId（发请求必需）

### 12.1 为什么 v1 不做多轮对话（FAQ）

**用户反馈**："Arena 没有上下文，AI 没有记忆。"

**这是 v1 设计取舍，不是 bug**。理由：

1. **盲评公平性**：多轮对话会让"谁的上下文窗口大、谁的 KV Cache 命中好"成为变量，而 Arena 想比较的是 **模型本身在同一道题上的输出质量**。带上历史，比较结果就掺了"记性好坏"。
2. **System Prompt 同理**：v1 也不让用户自定义系统提示词，所有模型在完全裸的状态下作答，输入条件对齐。
3. **附件不算"上下文"**：当前问题携带的图片/文件会一并传给所有模型（`ArenaRunWorker.cs:211-262` 已实现），这是同一道题的一部分，不破坏公平性。
4. **想多轮追问怎么办**：用揭晓功能找出心仪模型，去对应 Agent 对话页面继续——那才是多轮场景。

**v2 计划**：在 13.1 验收完成后，加一个"会话模式"开关——开启后揭晓即生效、按选定模型进入多轮聊天。盲评模式仍然作为默认。
