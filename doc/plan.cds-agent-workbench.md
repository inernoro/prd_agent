...
|------|----------|--------------|--------------|------|
| P0 计划文档与任务看板 | [x] | [x] | [ ] | 本文档落地，文档索引冒烟通过；真实页面未搜到新文档，视觉测试失败待修复 |
| P1 会话 API 骨架 | [ ] | [ ] | [ ] | 能创建、查询、停止会话 |
...
|----|----------|--------------|--------------|------|
| P0.1 创建计划文档 | [x] | [x] | [ ] | 新增 `doc/plan.cds-agent-workbench.md`，文件存在校验通过 |
| P0.2 同步文档索引 | [x] | [x] | [ ] | 更新 `doc/index.yml` 与 `doc/guide.list.directory.md`，`rg` 校验通过 |
| P0.3 文档命名校验 | [x] | [x] | [ ] | 文件名、头部、索引映射已校验 |
| P0.4 当前范围声明 | [x] | [x] | [ ] | 已明确连接页通过不等于 Agent 工作台通过 |
| P0.5 文档空间视觉可见 | [ ] | [ ] | [ ] | 从首页进知识库打开 MAP 空间后，搜索新文档无匹配，待补同步 |
...
- 打开 MAP 文档页或知识库入口，确认计划文档可被索引发现。
- 若文档系统暂不支持直接预览计划文档，则记录为“视觉测试阻塞”，不得勾选。

视觉测试记录：

| 日期 | 路径 | 结果 | 后续 |
|------|------|------|------|
| 2026-05-14 | 首页 → 知识库 → MAP 空间 → 搜索 `CDS Agent 工作台` / `plan.cds-agent` | 失败：页面显示“无匹配文件”，MAP 空间仍为 200 个文件 | 需要补齐文档空间从仓库 `doc/` 到 MAP 空间的同步动作后重测 |
...
| 项 | 开发完成 | 冒烟测试完成 | 视觉测试完成 | 说明 |
|----|----------|--------------|--------------|------|
| P1.1 新增 `InfraAgentSession` Model | [ ] | [ ] | [ ] | 按现有 Model 规范写 Id，不加错误 Bson 标注 |
| P1.2 新增 `InfraAgentMessage` Model | [ ] | [ ] | [ ] | 支持会话消息持久化 |
| P1.3 新增 `InfraAgentEvent` Model | [ ] | [ ] | [ ] | 支持 SSE 断线恢复 |
| P1.4 新增 repository/service | [ ] | [ ] | [ ] | 会话创建、查询、状态更新 |
| P1.5 新增 Controller | [ ] | [ ] | [ ] | `GET/POST/STOP` 基础 API |
| P1.6 权限检查 | [ ] | [ ] | [ ] | 只能操作自己的连接和会话 |
| P1.7 错误语义 | [ ] | [ ] | [ ] | 连接不存在、已撤销、CDS 不可达、会话不存在 |
...
- 在 MAP 页面新增“Agent 测试台”区域。
- 页面能看到“新建会话”按钮。
- 新建后列表出现一条会话卡片，状态文案准确。
