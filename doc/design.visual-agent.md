...
### 场景身份路由（2026-09-21 补充）

同一个生图模型在文生图、图生图、多图组合三种场景下对应不同的运行时场景身份，而白名单是按场景身份配置的。旧路径按"提交时携带的参考图数量"临时推断场景，一旦推断结果与用户实际选择的场景对不上，就会出现"模型在选择器里可见、提交后却被白名单拒绝"的体验：模型是允许的，只是被路由到了错误的场景身份下检查。修复后同步生图路径改为按参考图数量与选择场景一致的规则显式确定场景身份，不再临时猜测。

配套的深度自检新增两项：对所有已开放生图模型做场景路由预检（同一模型在三种场景下的身份映射是否与白名单一致），以及对生图真实调用结果与样本量做持续监控，让"模型被拒绝"或"上游失败"这类只有真实调用才会暴露的问题也能触发 CDS 告警，而不必等用户反馈。

### 生图请求拒绝与服务故障的区分（2026-09-22）

上游对生图请求给出安全拒绝（如复述/版权类拒绝 `IMAGE_RECITATION`，或响应体里带拒绝原因但没有图片）时，旧路径把"上游有响应、只是不含图片"和"上游真的不可用"混为一谈，统一报成服务故障，用户看到的是"生成失败，请重试"——重试对安全拒绝毫无意义，只会原样再被拒一次。修复后区分两类结果：安全拒绝归类为"请求需调整"，把可执行的调整提示（换一张参考图、改写描述）返回给用户，原始拒绝原因继续写入网关日志供排查；只有上游真正不可用（超时、无响应体、非预期错误）才归类为服务故障。这条边界同时延伸到标准生图响应的归一化路径与 MAP 生图消费者，避免文学创作等复用同一生图引擎的场景（见八、关联设计文档）在归一化时把拒绝语义弄丢、回退成误导性的"服务故障"提示。

生图限流恢复指引也一并修正：供应商限流（QPS/并发触顶）此前被误判为"单次提交图片数量过多"，用户按提示减少数量后仍然被限流；现按限流场景给出正确的恢复动作（稍后重试），不再指向一个不相关的参数。

首个失败图片的稳定错误码与脱敏后的错误文案会汇总进 run 的终态和 `runDone` SSE 事件，避免轮询接口和 SSE 事件对同一次失败给出不一致的结论。

### 模型目录与尺寸只信网关快照，不再本地推断（2026-09-22）

视觉创作过去按上游型号名的字面特征（如名字里含某个关键词）本地推断这个模型属于哪个目录、支持哪些尺寸；上游一旦调整命名或新增一个特征不匹配既有推断规则的型号，尺寸档位就会推断错误或直接推不出来，用户在模型设置里看到的可选尺寸和实际支持的对不上。修复后视觉创作只消费 LLM Gateway 对外发布的模型与图片能力快照——目录归属、支持哪些尺寸档位，全部以网关快照声明的为准，不再按上游型号名做本地猜测；网关没有声明能力的模型，视觉创作也不会替它猜一个。模型设置页的尺寸选项相应改为按网关目录动态展示，保存状态统一走同一个进度组件，避免多处各自维护一份不一致的保存态。

### 生图任务取消的即时性（2026-09-22）

用户点击取消后，正在执行中的生图任务此前只是把状态标记为「已取消」，但已经发给上游的请求仍在继续跑，占着任务槽位直到上游自然返回，期间工作区清理会被这个「名义上取消、实际仍在占用」的任务卡住。修复后取消信号会传递到正在执行的上游请求本身，真正中断调用而不是仅仅改状态位。排队中尚未开始执行的任务同样存在类似问题：取消后 Worker 的过滤条件没有覆盖到这类任务，导致它们既不会被继续执行、也不会进入终态，永久卡在排队状态。修复后排队中的任务取消后立即进入终态。
