...
图层面板（内部称「组装台」）不是一次性导出结果的展示区，而是一个可交互的合成工作台：显隐、不透明度、叠放次序的任何一次调整，合成预览立刻重算，导出内容与预览所见严格一致。这个设计目的是让用户在导出 PSD 之前就能确认最终效果，而不是导出后才在设计软件里发现次序不对。

分层结果落到画布上表现为一个「Frame」（编组），默认叠回原图所在的位置、与原图观感一致，每个部件仍可单独选中、拖动；平铺展开成为一种可选视图而非默认强制布局。图层归属（分组、序号、显隐、不透明度、次序）持久化到画布数据里，刷新页面或重新打开不会丢失已调整好的状态。
...
分层结果支持四种下载出口：分层 PSD（每层只写「有内容的最小矩形」而非铺满整张画布）、合成 PNG、全部图层打包 ZIP、单层透明 PNG。四种导出复用同一份图层面板状态，确保导出内容与用户当前看到的合成效果一致。
...
网关侧的上游故障与恢复会产生站内信通知，按平台和模型去重（避免同一个故障刷屏），并带日志深链方便定位；只有具备管理员权限的用户能看到和处理这类通知。
