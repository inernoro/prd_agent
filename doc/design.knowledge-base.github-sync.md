...
所以列目录之前先把分支解析成一个**不可变的提交号**，再按它去列：在提交号上拿到「查无此目录」，
就证明那一刻它确实不存在，而解析这一步本身已经证明仓库与分支都够得着。解析不出来**整轮就地中止**，
既不列也不删，如实告诉用户这一轮没同步、稍后重试——绝不拿一次权限变动去换一次数据清空。
这里不留「退回按分支名列」的退路：那条退路会让上面那句结论在它自己身上不成立，而且不成立得很安静。
按分支名列再事后补一次确认同样不够：两次请求之间分支还会变（删掉目录的提交之后又来一个恢复它的），
