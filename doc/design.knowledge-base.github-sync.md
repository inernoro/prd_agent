...
- **远端目录被删光**时旧子文档不再残留。此前列目录拿到 0 篇 Markdown 就直接返回，跳过了整个删除
  环节——"目录被清空"和"文件被逐个删光"对用户是同一件事，却走了两条路。
  这里有个容易漏掉的形状：Git 里没有空目录，**删光最后一个文件，目录本身就不存在了**，
  于是拿到的是"查无此目录"而不是"空清单"——这反而是最常见的一种删除。两种都要调和。
  但"查无此目录"同时也是无权访问、仓库改名、分支被删时的回答，所以动手删之前先确认仓库与分支
  这一层还够得着；确认不了就按失败处理，绝不拿一次权限变动去换一次数据清空。
  同理，上游一次最多回一千条目录项、超了就截断且不明说：清单可能不完整时整个删除环节都让路，
  只做新增与更新。判断完整性看**过滤前**的原始条数——过滤后的"零篇"既可能是真没有 Markdown，
  也可能是 Markdown 全被挤出了窗口，这两件事只有原始条数分得开。
  一句话概括这一节：**新增与更新见到什么做什么，删除必须有完整清单撑腰。**
  跳过删除的那一轮不会装作没事：条目会红着并说清「这个目录太大、本轮没处理删除、把目录拆细」，
  否则「远端删了而这边没动」会被一个绿色的成功状态盖住，用户无从知道差在哪。
...
- **连接状态**不再只看库里有没有记录。用户在 GitHub 那边撤销授权后记录还在，界面照样显示"已连接"、
  ...
  这类措辞（标题栏怎么称呼这条连接、换账号时说什么、失效时给什么提示）统一从同一处判据出，
  不散落在各个组件里——散开的代价当场兑现过：第一次只堵了连接步骤那一句，标题栏照样在说
  "已连接"，与同屏的"授权已被撤销"当面打架。
