...
所以列目录之前先把订阅地址里那个 ref 解析成一个**不可变的提交号**，再按它去列。解析不出来
**整轮就地中止**，既不列也不删，如实告诉用户这一轮没同步、稍后重试。这里不留「退回按名字列」
的退路：那条退路会让下面那句「共用同一份快照」在它自己身上不成立，而且不成立得很安静。
（ref 不一定是分支——订阅地址里那一段允许写标签或提交号，所以解析要用对三者一视同仁的接口；
只认分支会让这类订阅从能用变成永远失败。）

**但要认清提交号定住了什么**：它定住的是**内容的版本**，定不住**授权**。列目录之前那次解析
只能证明那一刻读得到；等到列目录的请求落地，中间可能已经被收回授权、被移出组织、仓库被删——
回来的仍是一模一样的 404。所以在提交号上拿到「查无此目录」**还不能**直接当成「远端删光了」。

补一次「此刻还读不读得到这个仓库」也不够：它证明的是仓库，而结论说的是这个路径，两者不是同一件事——
授权在两次请求之间抖一下，确认照样通过，而那条 404 其实是权限造成的。所以要取的是一份**说的是同一件事
的正面证据**：从目标逐级上溯，找第一个在同一个提交上列得出来的祖先，看它的清单里有没有下一级。
祖先列得出来，就说明此刻确实读得到这个仓库的内容、读的又是同一个提交，于是「清单里没有它」说的
就是「它不存在」，而不是「我们看不见」。撞见**是个文件**（目录被同名文件顶替掉，是真实会发生的一种删除）
同样是正面证据：文件底下不可能挂着东西。订阅的那个路径自己变成文件时更直接——上游回的是
200 加一个对象而不是 404，那份 200 本身就证明了「这个目录在这个提交上不存在了」。
两处共用同一条「目录回数组、文件回对象」的判别，少判一次的后果不是判错，是直接抛异常、
整轮同步失败、旧文档永远留着。一路上溯到仓库根都列不出来，
那只能说明读不到——仓库根不会被删掉。祖先的清单本身可能被截断时（见下文那条上限）一律判为没有结论：没看见不等于不存在，
而这条结论下一步就是删东西。绝不拿一次权限变动去换一次数据清空。

反过来，按名字列再事后补一次确认仍然不够：两次请求之间分支还会变（删掉目录的提交之后又来一个恢复它的），
于是「确认分支好好的」会被误读成「目录真没了」，把刚恢复的文档连历史版本一起删掉。

钉住提交号之后，**本轮所有读取都用同一个它**——清单、正文、最近修改时间——否则会出现「清单来自
一个提交、正文来自另一个」，把后者的内容按前者的版本号存起来。唯一的例外是条目上存给用户点的那个
链接：它刻意指向分支而非提交，好让链接随文档更新而更新。

还有一条连带账要认：**认亲要认路径，不认地址里那段会变的 ref**。早期条目没单独存路径、只存了地址，
而地址里嵌着分支名；一旦改成按提交号列，上游给回的地址也换了形状，逐字比对就认不出同一个文件——
它会被当成新文件建一遍，原件被当成「远端已不存在」删掉。所以存量条目先从地址里抠出路径来认，
抠不出来才退回用原地址当键。
