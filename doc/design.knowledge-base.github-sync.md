...
- 申请的授权范围是 `repo`。GitHub 对它的定义包含私有仓代码的**读写**，比"读文档目录"需要的大得多，
  而传统 OAuth App 无法再细分（`public_repo` 读不了私有仓）。要真正做到最小权限只能改用 GitHub App
  按仓库授权、只勾 Contents: Read —— 那是独立工程（见 debt 的 K-19）。
- 令牌不做刷新。传统 OAuth App 发的令牌不过期，这条不成立；若线上配的是开启了"过期用户令牌"的
  GitHub App，令牌 8 小时失效后需要用户重连（K-17）。失效本身不会静默错乱：同步会失败并说明原因，
  连接状态也会标成已失效。
- 仓库搜索是在当前这一页里过滤，名下仓库很多时想找的那个可能要翻几页（K-18）。

### 6.1 2026-09-15 对抗审查后补掉的三处

同一份代码被按"完整功能上线"重审了一轮，补掉三处此前没有判据管着的边界：

- **远端目录被清空**时旧子文档不再残留。此前列目录拿到 0 篇 Markdown 就直接返回，跳过了整个删除
  环节——"目录被清空"和"文件被逐个删光"对用户是同一件事，却走了两条路。判据收敛进
  `SelectStaleChildren`，调用方不再有权跳过。
- **断开连接**现在先去 GitHub 撤销授权再删本地令牌。顺序不能反：先删本地就再也拿不到那把令牌。
  撤销失败不阻断本地删除，但必须如实告诉用户 GitHub 那边没撤掉、去哪儿手动移除。
- **连接状态**不再只看库里有没有记录。用户在 GitHub 那边撤销授权后记录还在，界面照样显示"已连接"、
  一点仓库就是 401；现在连着的时候会真打一次 GitHub，把结果放进 `usable` 三态
  （`unknown` 表示没问出结论，一律放行，不把网络抖动当成失效）。
