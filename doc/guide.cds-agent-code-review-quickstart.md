...
脚本会自动推断当前远程 preview host；不要为了远程 preview 手动填 `SMOKE_TEST_HOST`。最新一次证据目录 `/tmp/cds-agent-cycle-20260518032750` 的结果是 `blocked_r1`、`commercialComplete=false`，总耗时 53s，最慢的是 N6 non-code compatibility 16s、alias stability 10s、doctor 10s；这类时间线就是后续判断“时间花在哪里”的主记录。
