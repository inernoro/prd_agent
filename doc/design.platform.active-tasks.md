...
标题栏那句结论（「你认领了 3 条，另有 5 条还没人管」）是**整块看板**的事实，
不跟着「只看我的」或模块筛选走。这条不是洁癖：计数一旦跟着筛选走，开「只看我的」
时「没人管」会被算成 0，于是说出「其余都有人管了」—— 而实际上还有一百多条没人管。
一句会随视图变脸的结论，等于没有结论。

同一个道理的第二个受害者是那块区域自己：总数跟着筛选归零时，「一条都没有就整块不渲染」
这条守卫会连同切回「看全部」的开关一起隐藏掉，用户走进去就出不来。所以筛完没东西
要给一行空态，不是让整块消失。

### 「已转成 N 条活」的根是那条活本身

债务上记着 `ConvertedTaskIds`，但只数个数是不够的 —— 那条任务可能已经被删了。
id 还挂着、状态还停在 converted，界面上点过去是空气，而且这条债务永远回不到
open / claimed。所以读的时候对一遍活人名单，把悬空的 id 摘掉并让状态落回真值。
...
**整个功能默认是关的**：公开地址直接 404，不泄露「这里本来有个面板」。这一屏对着的是不需要
登录的任何人，端出去的是同事真名与此刻在做什么 —— 这种东西的安全默认只能是关，由管理员在
「大家在做什么 → 看板设置」里显式打开，并在那里选给到哪一档。

打开之后默认**脱敏**：访客看得到谁在忙、谁卡住、谁没活、今天投入多久，看不到任务标题正文、
备注、在等谁 —— 任务标题常带项目代号和缺陷细节。三档可逐级放开（脱敏 / 全文 / 仅头条结论）。

这里栽过两次，形状不同但根都在「默认」这两个字上：

- **默认开过一版。** 部署那一刻起，只要有人知道地址就能看到全公司谁在忙谁卡住。更糟的是当时
  前端连关掉它的入口都还没接上 —— 后端端点在、前端封装也写了，但全仓没有一处调用它，
  于是「可以整个关掉」这句承诺只兑现了后端那一半。这种缺陷不报错、页面照常渲染、测试照常绿，
  只有真去找这个开关的人才会发现它不存在（`predicate-and-wiring-discipline` 形状 2）。
- **脱敏留过开头一两个字**「好有点形状」，那也是错的：任务标题开头最常见的恰恰是客户名、
  项目代号、缺陷编号，留两个字就等于把「看不到标题正文」这句承诺撕掉一角。现在一个原文字符
  都不留，只用点的个数给出长度感（空标题另给一句人话，不返回空串 —— 那会让前端渲染成空行）。
