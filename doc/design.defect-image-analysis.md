...
> **版本**：v1.0 | **日期**：2026-02-27 | **状态**：已实现

---

## 一、管理摘要

- **解决什么问题**：缺陷提交时 AI 润色只能处理纯文本，用户粘贴的截图中的关键信息（红框标记、错误弹窗）被完全忽略
- **方案概述**：将"看图"与"润色"解耦——上传时 VLM 异步预分析截图提取文字描述，润色时将描述作为上下文传给低成本文本模型
- **业务价值**：一次 VLM 调用即可让截图信息融入缺陷报告，降低成本的同时提升润色质量和透明度
- **影响范围**：缺陷 Agent 后端（新增 analyze-image 端点）、缺陷提交对话框前端（附件状态模型重构）
- **预计风险**：低 — VLM 分析失败不阻塞提交和润色流程，完全降级兼容
