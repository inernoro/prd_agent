...
截至 2026-05-18，远程 preview 的前端执行面板已在 `4f2fb589` 验证，脚本-only 断言提交 `ad3879a` 只刷新 GitHub commit metadata，没有触发 preview 重新构建。最新无 provider one-cycle 证据是 `/tmp/cds-agent-cycle-heartbeat-fixed`：`status=blocked_r1`、`commercialComplete=false`、`R0=pass`、`A0=pass`、`V1=pass`、`N6=pass`、`R1/S1/S2/S3=pending`，总耗时 75s，最慢步骤是 V1 视觉 29s、R0 sidecar alias 11s、doctor 10s。V1 在 15s 时输出了 heartbeat，证明长视觉步骤可观察；其他快步骤没有被 heartbeat 人为拖慢。目标审计报告是 `/tmp/cds-agent-goal-audit-freshness-current.json`，结论仍是 `goalStatus=not_complete`，本地 guardrail 耗时 3s，`cycleFreshness=fresh`；把 `CDS_AGENT_GOAL_CYCLE_MAX_AGE_SECONDS=1` 设成负向检查时会得到 `cycleFreshness=stale` 和 `CYCLE_FRESHNESS` 未证明项。当前默认 profile 是 `OpenRouter DeepSeek V4 Pro / openai-compatible / deepseek/deepseek-v4-pro`，有 key 但不兼容 `claude-agent-sdk`；下一条有效命令是：
...
目标审计还会检查 one-cycle 摘要的新鲜度。默认 `CDS_AGENT_GOAL_CYCLE_MAX_AGE_SECONDS=86400`；
超过该时间的 `cycle-summary.json` 会被标为 `cycleFreshness=stale`，并加入 `missingOrUnproved`。
stale 证据可以用来排障，但不能用来宣称目标完成。新生成的 one-cycle summary 会写入本地 git
branch/commit，方便确认这份证据对应哪次脚本和代码状态。
