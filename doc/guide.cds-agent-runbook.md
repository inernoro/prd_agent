...
截至 2026-05-18，最新 runtime pool 恢复证据是 `/tmp/cds-agent-runtime-pool-evidence-current`：`BRANCH_LOCAL_SIDECAR_CLEAN=contaminated:4`、`REMOTE_HOST_AVAILABLE=missing`、`SHARED_POOL_RUNNING=missing`，总耗时约 15s。结构性调查报告在 `doc/report.cds-agent-runtime-pool-contamination-2026-05-18.md`，当前进度面板在 `doc/status.cds-agent-current-progress.md`。当前正确顺序是先清理 `prd-agent` branch-local sidecar residual，再登记 enabled remote host 并部署 shared official SDK runtime，最后重跑 MAP R0/S1/S2/S3/one-cycle。此前 `/tmp/cds-agent-cycle-20260518131650` 的 `blocked_r1` 证据只代表当时 preview sidecar alias 绕行可用；它不能覆盖当前 shared runtime pool 为空和 branch-local sidecar 污染的事实。
