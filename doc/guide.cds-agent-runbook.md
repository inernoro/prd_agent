...
当前进度看 `doc/status.cds-agent-current-progress.md`。它记录最新 evidence 目录、当前阻塞、耗时和下一步，不需要从聊天或多个 `/tmp` 路径里拼状态。
...
方便页面或 CI 不解析终端日志也能复原时间线。汇总里的 `Deploy/build advice` 是是否需要重新部署的判定；branch-local sidecar contamination、remote host missing、shared pool not running 不靠普通 preview redeploy 解决；`blocked_r1`、`ready_for_provider_smokes`、`blocked_provider_smokes`、`provider_smokes_incomplete` 都不应该靠重复部署解决，先补 provider key 或跑 provider smoke。
...
截至 2026-05-18，最新 runtime pool 恢复证据是 `/tmp/cds-agent-runtime-pool-evidence-20260518152052`：`BRANCH_LOCAL_SIDECAR_CLEAN=contaminated:7`、`REMOTE_HOST_AVAILABLE=missing`、`SHARED_POOL_RUNNING=missing`，总耗时约 14s。结构性调查报告在 `doc/report.cds-agent-runtime-pool-contamination-2026-05-18.md`，当前进度面板在 `doc/status.cds-agent-current-progress.md`。当前正确顺序是先清理 `prd-agent` branch-local sidecar residual，再登记 enabled remote host 并部署 shared official SDK runtime，最后重跑 MAP R0/S1/S2/S3/one-cycle。此前 `/tmp/cds-agent-cycle-20260518131650` 的 `blocked_r1` 证据只代表当时 preview sidecar alias 绕行可用；它不能覆盖当前 shared runtime pool 为空和 branch-local sidecar 污染的事实。
