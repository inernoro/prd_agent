...
**为什么不顺手补**：R1 要给单服务部署补与完整部署同款的执行器分发；R2 要给部署链加「同镜像、同提交、只刷新环境变量」的重建模式；R3 需要「项目之间允许引用谁」的策略（项目级 ACL），CDS 目前没有这个概念。三者都是新契约，不该由一个界面 PR 顺手定下。

**做完算数的判据**：远端执行器分支上切换引用后，目标容器在该执行器上被重建；钉住的分支切换引用后仍停在原提交；项目级凭据在环境变量里写下指向不可见项目的引用时，生效配置与部署期都拒绝解析。
