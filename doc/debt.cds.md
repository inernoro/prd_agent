...
**现状**：结论条那三个日计数由前端过滤运行记录得出——取当天 `queuedAt >= 今日零点` 的条数。
而这份数据有两道截断：`StateService` 每个任务只保留 120 条运行记录，页面又只请求最近 400 条
（全局）。一个每 5 分钟跑一次的任务一天产生 288 条，120 条只覆盖大约 10 小时，于是过了上午
它的早班记录就被挤掉了，而页面照旧把剩下的当成「今日执行」的全量。

**影响**：高频任务多的项目里，日计数会**少报**，且越到晚上少得越多。低频任务（每天一两次）
不受影响——这也是它此前没被发现的原因：演示数据和大多数真实项目都是低频。

**为什么这次不修**：正确的修法是让日计数有一个不被保留策略截断的来源——要么服务端持久化
一份按天的聚合，要么给运行记录查询加一个覆盖整个本地日的时间窗并相应放宽保留。两条都是**新的
后端契约**，超出本次「把任务调度页改成可值班」的目标（CLAUDE.md §5.5 的 B 类：有价值但扩范围）。
在同一个 PR 里顺手加聚合表，会把一个界面改版变成一次存储改动。
...
**现状**：结论条统计段的「接下来 6H」把每个任务的 `nextRuns` 里落在未来 6 小时内的时刻数
加总。而 `nextRuns` 是**按条数**投影的：页面请求 `?nextRuns=24`，路由还把它夹在 64 以内。
一个每 5 分钟触发的任务 6 小时内该有 72 次，只拿得到 24 次；每分钟的任务该有 360 次，
即使把请求提到上限 64 也还差得远。

**影响**：与日计数那条同形状——高频任务越多，这个数字越偏小，且**只会少不会多**，
所以它不会造成「以为没事其实要出事」的误判，但会让人低估接下来的负载。低频任务不受影响。

**为什么这次不修**：真正的修法是把投影从「按条数」换成**按时间边界**（给
`GET /api/scheduled-jobs` 加一个「投影到某个时刻为止」的参数，服务端按时间窗展开并自带
上限），这是一个新的查询契约。本 PR 的目标是「把任务调度页改成可值班」，且这已是
Codex 第三轮在同一处统计口径上提意见——按 CLAUDE.md §5.5 的熔断条款，停手记账，
由用户决定是否单开一次扩范围。用户 2026-09-01 明确选择「修 P50 与已删除任务两条，
这条记账」。

**做完算数的判据**：一个 5 分钟间隔的任务在 6 小时窗口里贡献 72 次，而不是请求到的条数。
关键取舍是投影要从「按条数」改成「按时间边界」，同时保留一个上限——否则每秒级的调度
会把响应撑爆。投影已有唯一入口，不必新起一套到期逻辑。
...
**为什么不顺手补**：能用的机制其实已经有了——项目级 Agent Key 天生带作用域，跨项目访问会被拦。
缺的是「子实例启动时拿到一把只对自己有效的钥匙」这一段，它属于预览实例的身份设计，
不是某个界面 PR 顺手能定的契约。
