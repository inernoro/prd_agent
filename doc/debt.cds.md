...
| E16 | **高** | 2026-08-16 | redis / memcached / kafka / nats 四个预设**从创建起就没有认证**（catalog 无 `secretKeys`，连接串是裸的 `redis://redis:6379`） | 用这些预设建服务；叠加端口公网暴露即裸奔 | open | 不是「轮换」问题——没有口令可换。补法：catalog 条目加 secretKeys + 容器侧开认证（redis requirepass / kafka SASL / nats token）+ 连接串带凭据，三处必须同批改，否则一加认证所有消费方立刻连不上。在补上之前唯一有效的动作是关掉公网暴露 |
