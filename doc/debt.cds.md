...
PR「服务关系与跨项目引用」评审中按规则 5.5 判为 B 类（有价值但扩范围）的三项，本 PR 只做拒绝或说明，不展开。

| ID | 严重度 | 创建日期 | 描述 | 触发条件 | 状态 | 备注 |
|---|---|---|---|---|---|---|
| R1 | 中 | 2026-09-02 | 单服务部署接口没有执行器分发，只会在 master 本机操作；引用切换后的重建复用它，远端执行器分支上会落错宿主 | 分支归远端执行器时切换引用 | open | 引用切换接口对这类分支直接拒绝，提示走完整重新部署。正解是给单服务部署补与完整部署同款的执行器分发 |
| R2 | 中 | 2026-09-02 | 引用切换后的重建走单服务部署，会拉最新代码并清掉钉住的提交；没有「保留当前版本只重建容器」的模式 | 分支钉在某个提交时切换引用 | open | 接口对钉住的分支拒绝。正解是给部署链加「同版本重建」模式（同镜像、同 worktree 提交，只刷新环境变量） |
| R3 | 中 | 2026-09-02 | 跨项目引用只在专用的引用读写接口上按凭据可见性拦截；项目环境变量、compose 导入等摄入路径与部署期解析没有请求上下文，项目级凭据仍可写入指向其他项目的引用并在生效配置里看到目标地址 | 项目级凭据在环境变量里手写指向别的项目的引用 | open | 需要「项目之间允许引用谁」的策略（项目级 ACL），CDS 目前没有这个概念，属于新契约；落地前引用的可见性以专用接口为准，其余路径按现状 |
