...
**另一条已知边界：分支库名会撞车**。CDS 派生的分支库名把分支 slug 截断到 28 字符，两个前 28 字符相同的分支会算出同一个库名（实测长前缀命名下真的会撞）。这意味着「复算出来的名字必然唯一」不成立。当前处理是**撞名即拒绝 reset**（fail-closed，返回 409 并列出撞车的分支），不改命名规则——改了存量分支库就对不上、reset 会拒绝它本该修的库。彻底解决要给破坏性操作用抗碰撞的分支身份（分支 ID 或哈希）而不是显示名 slug，同样是独立一次改动。
