...
## 跨项目首屏不再交代自己统计的是哪两环（2026-09-16，已还）

来源：设计文档与代码的对照审查。首页统计的是「起预览」与「跑验收」两环，开分支与合并主干
只作背景。早先有一条五步流程条把这件事讲在屏幕上，在放大态改成与紧凑态同一套视觉语言那一次
被一并删掉了，现在屏幕上没有任何一句交代范围。

这是对着改版目标的回归：当初重做首页，要解决的正是「进来的人不知道这个页面在做什么」。
不当场补是因为它要重新设计一块屏幕位置，且与下一条（判断句该不该进紧凑态）是同一个取舍，
应当一起定。

**做完算数的判据**：不选项目进首页，不点任何东西，屏幕上能读到「这一屏统计的是部署与验收
两环」这个意思。

**2026-09-16 已还**：范围条恢复，五步路径写在标题下、两态共用，并标出总览条按起预览与跑验收
分段。措辞从「整页只统计这两环」收紧为「分段用这两环」——结论三档与合并的漏点也在这一屏上。
守卫钉住「渲染在两态之前」与「标记恰好是那两环」，四轮篡改逐一验过会红。

## 跨项目紧凑态没有那句判断，与「先给结论」的初衷有落差（2026-09-16，已还）

来源：同上。选了项目时结论头条就是第一屏；不选项目时紧凑态只渲染总览条与走向折线，
判断句要点「放大」才出现。

紧凑态少字多图是明确定下来的取舍，所以这不是缺陷，是两条要求打架：一边要第一屏给结论，
一边要第一屏少字。自动审查提过三次，每次都被判为「产品决定不是缺陷」。要结掉它得由人在
两者之间定一个折中（例如只放判断句本身、不放支撑点与下一步）。

**做完算数的判据**：由人明确选定一种，并在本文件销账；在那之前不接受自动审查反复重提。

**2026-09-16 已还**：选定的折中是「紧凑态只给句子本身，支撑点与下一步留到放大态」。
实现上不必新写组件——Headline 的紧凑变体早就写好了（连「只给一句」的注释都在），
缺的只是紧凑那一支从没引用它，属建了一半的接线。守卫改成两态都要有，并反向钉住
紧凑那一支不许传铺开开关。

## 三条口径是约定不是闸门：换算边界、对齐判据、标题解析（2026-09-16）

来源：同上。设计文档一度把这三条写成「有守卫盯着」，对照下来并没有：

1. **生效结论的换算只在边界发生**——换算函数现有两个调用点（聚合边界、回写 GitHub 的路由，
   后者刻意不走聚合）。没有任何东西扫描「还有没有第三个消费方自己判了一遍阻断缺陷」，
   而那正是这个判据前三轮补一个漏一个的成因。
2. **对齐判据只有一份实现，前端不得另写**——目前确实只有一份，但没有守卫拦住第二份。
3. **标题解析是服务端唯一事实来源**——同上。

三条的正解是同一种形状：扫全仓，断言该判断的定义只许出现在指定文件里，缓迁的显式进白名单。
不在本轮展开是因为它要先把「什么算一次换算 / 一次解析」定义成可扫描的形状，属新引入机制。

**做完算数的判据**：在任意第二个文件里手写一次阻断缺陷判断，或在前端实现一份标题解析，
对应的守卫变红。
