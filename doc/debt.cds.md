...
| E4 | 中 | 2026-08-16 | infra 没有周期备份，只能手工触发 | 需要回滚或恢复时 | **已建，首轮跳过** | 周期自动备份（mongo / redis）已上线，带磁盘闸与保留策略（每服务 7 份 / 14 天，**最新一份永不删**）。首轮实跑撞出真问题：原先写死的落盘目录在宿主上不存在，已改成逐个候选试写（`CDS_BACKUP_DIR` > 历史路径 > repoRoot 旁），与手工备份共用同一份候选。仍缺：离机副本、恢复演练 |
