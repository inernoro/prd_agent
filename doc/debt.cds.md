...
| E36 | **高** | 2026-08-18 | 暴露面自检失败时只 `console.warn`，不落任何事件——面板上看起来「和上次一样」，实际上已经好几轮没跑成 | 自检里任何一步抛异常（读路由、读防火墙规则、列容器） | 未修 | 2026-08-18 实测：09:17 之后 CDS 重启三次，每次本该在启动两分钟后自检一次，却一条事件都没有。于是最近一条 warn 1 被当成「当前状态」，其实是几小时前的快照。一个不会报错的自检比没有自检更糟——它让人以为查过了。改法：catch 里也 record 一条 error 事件（带失败原因），并在状态里带上「上次成功自检时间」，超过一个周期没成功就自己报警 |
