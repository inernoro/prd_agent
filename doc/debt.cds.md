...
| E35 | **高** | 2026-08-18 | Redis 恢复是「往运行中的容器写 RDB，然后 `docker restart`」——关闭时 redis 会按 save 点把当前数据存一次，**正好覆盖掉刚上传的快照** | 恢复一个配了 save 点的 redis（默认配置就是） | 未修 | 覆盖之后重启加载到的是覆盖后的内容，而接口回「已恢复」。恢复场景里这种谎话代价最大：用户以为数据回来了，其实没有，而原始快照可能已经被当成「用过了」。顺带第二个口子：`appendonly yes` 的实例启动时读 AOF 不读 RDB，同样「恢复成功」但一条没变。正解是把顺序改成先停容器、再 `docker cp` 覆盖文件、最后启动（停的时候那次 save 发生在覆盖之前，无害），并在开着 AOF 时明确拒绝而不是假装成功。2026-08-18 收窄绑定时读代码发现，当时那台 redis 恰好是空库，没有踩到 |
