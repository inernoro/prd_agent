...
| E34 | 中 | 2026-08-18 | redis 备份的凭据兜底「扫进程命令行找 `--requirepass`」，对默认配置的 redis 是一条走不通的路 | 某个 redis 开了认证、而口令不在容器 env 里 | 未修 | 拿真 redis 量过：默认 `set-proc-title yes` 时它启动后会把自己的 argv 整个改写成 `redis-server *:6379`，口令连同其它参数一起消失，扫不到；显式关掉 proc-title 才扫得到。当前线上这批 redis 都没有口令，所以不影响；一旦按 E17 给它们开上认证，这条兜底就顶不住了。正解是让 CDS 用自己存的那份密码（infraServices 的 secrets）经 stdin 喂进容器——不能走 `docker exec -e`，那会把口令摆进宿主命令行。在此之前，这种情形如实报「找不到能通过认证的凭据」并失败，不静默拷走旧快照 |
