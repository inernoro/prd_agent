...
| E45 | 中 | 2026-08-18 | `cloudbridge-db` 的周期备份长期失败：`mysqldump: Got error: 1045: Access denied for user 'root'@'localhost'` | 每一轮周期备份 | **未修（缺凭据）** | 容器 env 里的 `MYSQL_ROOT_PASSWORD` 与该库实际 root 口令不符，与 E34（redis env 存过期口令）同一族。代码侧改不了——需要正确口令，属于用户/运维侧。在拿到之前这个库**没有任何可用备份**，而其它 12 个至少还有导出成功、只是离机没上去 |
