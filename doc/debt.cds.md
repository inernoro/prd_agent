...
| E34 | **高** | 2026-08-17 | IPv6 侧的对外端口巡检从落地那天起**一次都没真跑过**：两个环境的 IPv6 巡检目标 Secret（`CDS_EDGE_AUDIT_IPV6` / `PRODUCTION_EDGE_AUDIT_IPV6`）从未配置，job 每次都停在「目标缺失」那一步 | 源站有全局 IPv6，而防火墙只按 IPv4 配 | **待外部输入** | 扫描与校验两侧的代码都是对的——校验器用「必需端口一个都没扫到就判失败」挡住了「什么都没扫到也算绿」这种假绿，所以这是**缺配置**不是缺实现。别把它改成「没配就跳过」：留空与「忘了配」在系统里无法区分，那等于把一条永远不会红的巡检挂在看板上（比没有巡检更糟，因为它看着像在保护）。需要的外部输入只有两条值：两个环境**源站**的全局 IPv6 地址，在各自宿主上用 `ip -6 addr show scope global` 取，`fe80::` 开头的链路本地地址不对外、填了扫不到；也不能填经 CDN 代理的域名，那样扫到的是边缘节点不是主机。若确认某台主机压根没有全局 IPv6，正解是显式登记「该环境无 IPv6」并让这条巡检按登记值判绿，而不是删检查或留空 |
