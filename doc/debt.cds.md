...
| E4 | 中 | 2026-08-16 | infra 没有周期备份，只能手工触发 | 需要回滚或恢复时 | **已建，未上线** | 已实现周期自动备份（mongo / redis），落盘与手工备份同目录，因此备份历史接口不用改就能看到。带磁盘闸（可用空间不足或读不到就跳过本轮）与保留策略（每服务最近 7 份、不超过 14 天，**最新一份永不删**）。仍缺：离机副本、恢复演练 |
