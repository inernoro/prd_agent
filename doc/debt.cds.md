...
| E28 | **高** | 2026-08-17 | redis 预设的 `sh -c` 顶掉了官方镜像 entrypoint 的 `redis-server` 分支，导致 redis 以 **root** 运行、`/data` 变成 root 属主 | 本 PR 引入口令后新建的每个 redis | **已修** | entrypoint 只在第一个参数是 `redis-server` 时才 chown + 降权，看到 `$1 = sh` 就走兜底 `exec "$@"`。改法：`sh -c 'exec docker-entrypoint.sh redis-server --requirepass "$REDIS_PASSWORD"'`，展开口令之后把控制权交回 entrypoint。**判据同步升级**：原来那几条只扫命令字符串（两种写法长得几乎一样，扫不出差别），新增 `redis-preset-privilege.docker.test.ts` 起真容器断言「PID 1 是 redis-server、属主是 redis、/data 属主是 redis、不带口令连不上」；无 docker 环境跳过并打印原因，CI 上真跑 |
