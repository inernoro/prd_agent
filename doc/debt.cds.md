...
cds 的 emoji 棘轮判定从 U+2600 起步，注释写明「刻意不含方向箭头（→ ← ↑ ↓ ↔ ⇒，
U+2190-21FF）……这些在本项目里大量用于日志/注释画调用链，不是 emoji」。这个判断对绝大多数
成员是对的：`←`(U+2190) `→`(U+2192) 在 Unicode 的 emoji 数据表里标着 `Emoji=No`，连表情
变体选择子都挂不上，仓库自身（含全体 Agent 规则总纲 131 处）共用了 12337 次。

漏的是同一区段里另外几个：`↔`(U+2194) 到 `↙`(U+2199)、`↩`(U+21A9) `↪`(U+21AA) 标着
...
（即只放行 `Emoji=No` 的那些），并补一条守卫断言 `↔`、`↩` 会被判红而 `→`、`←` 不会。

**做完算数的判据**：在 cds 的源码目录里任写一个 `↔️`，棘轮变红；写一个 `→`，仍然绿。
