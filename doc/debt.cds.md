...
## 移动端布局冒烟没接进任何 CI（2026-09-03）

**现状**：整页移动端布局冒烟覆盖三档窄屏 × 六个页面，判据也不弱（横向溢出、文字被压成竖排、
可点目标被遮挡），但它只是包管理清单里的一条命令——在工作流目录里搜它的名字结果为空，
**没有任何流水线调用它**。

**后果**：这条防线从落地起就没有自动跑过，等于形状 7（守卫自己没接上线）的极端情形。
每个 PR 实际跑的窄屏判据只有本次新增的那一条（只覆盖上手向导这一屏）。

**为什么没顺手接上**：它要连真实 CDS 实例、要有项目数据、要能打开分支详情抽屉，PR CI 上
没有这些前置。硬接会变成一条经常因环境而红、于是很快被人关掉的假守卫。

**要做时怎么做**：两条路——把它拆成「不依赖实例的那部分」照本次的探针模式改造进 PR CI；
或者接进部署后的巡检（CDS 分支就绪后跑一次，失败回写到 PR）。后者顺带能覆盖登录态之后的页面。

### 关联文件

- `cds/scripts/mobile-layout-smoke.mjs` — 没被调用的那条整页冒烟
