...
来源：用户对监控中心第一屏提的第三条——「允许公开一些面板，让大家看得见」。
设计稿已经画了（对外只出业务名、红黄绿、7 天条带、更新时间；地址、判据、产物图、
日志、分支名一律不出去；业务名可改成对外叫法；链接可加口令、可撤销），代码一行没写。

**为什么这一轮不做**：第一屏本身（多环境 + 主动/被动）已经跨了后端模型、探测、
摘要、前端四层，再叠一个匿名只读出口会让这个改动失控（协作规则 §5.5 的范围熔断）。

**做完算数的判据**：拿一个没有登录态的浏览器打开公开链接，看得到业务名与红绿条带，
且页面源码里搜不到任何环境地址、判据表达式、产物 URL、分支名与提交号；撤销之后
同一个链接立刻 404。
