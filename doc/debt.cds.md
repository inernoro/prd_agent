...
**要做时怎么做**：优先给 `GET /api/scheduled-jobs/runs` 加 `since` 参数并让保留策略至少覆盖
一个自然日（按任务保留 max(120, 一天的预期次数)）；若量级不合适再上按天聚合。改完把结论条的
日计数切到新来源，并补一条守卫：构造一个 300 条/天的高频任务，断言日计数不随保留策略变化。
...
**要做时怎么做**：`projectUpcomingRuns` 已经是唯一的投影入口（锚在已存的 `nextRunAt` 上），
给它加一个 `until` 参数，循环条件从「够 N 个」换成「超过 until 就停」，并保留一个硬上限防
每秒级 cron 打爆响应。前端把 `?nextRuns=24` 换成按 6 小时请求。守卫：构造一个 5 分钟间隔的
任务，断言 6 小时窗口拿到 72 个时刻而不是请求条数。
