...
| 项目 | 当前风险 | 状态 | 完成条件 |
|---|---|---|---|
| 资源响应泄漏基础设施环境值 | 实际运行控制面缺少本分支已有的安全投影，分支状态响应曾将数据库凭据带入工具输出；该对话输出无法从仓库清除 | P0 未完成；后续诊断只输出白名单摘要，不记录凭据值 | 在保留当前控制面其他变更的候选中恢复既有脱敏；真实分支及资源响应均验证不含敏感值，内部数据库与备份操作继续使用原对象 |
| 受影响凭据轮换 | MongoDB、Redis 凭据仍需处置；备份代码存在不代表已经有可用恢复副本，账号重置不等于完整轮换 | P1 未完成；尚未轮换或撤销旧凭据 | 先盘点所有消费者并验证恢复副本，再验证新凭据、切换消费者、撤销旧凭据及复验；任何环节失败均保留恢复路径 |

共享控制面与本业务分支已分叉，不允许用整个业务分支覆盖线上控制面来恢复小修复。产品入口可用、单元测试通过或本地存在脱敏代码，都不能关闭上述风险。该处只记录有效风险与完成条件，临时运行日志及具体凭据不进入本台账。
...
| E86 | 低 | 2026-08-29 | **只以「盘上还留着文件」形式存在的目标，在备份面板上没有行——文件却照样进页脚的份数**：清单的并集只有两个来源（上一轮记录、此刻的基础设施台账），没有第三个来源「盘上的文件名」。两种服务因此看不见：(a) 运维故意停掉的（在台账里，但被 `status === 'running'` 滤掉）；(b) 已经删掉、之后又跑过一轮的（记录被重写，台账里也没有）。两种都留着旧备份，页脚数得出来，用户对不上是谁的 | 有服务长期停着、或删掉后盘上仍留着备份的项目 | **故意没做** | 来自 Codex 第二、三、四轮**互相拉扯**的同一条边界：第二轮要「停着的也要看得见」，第三轮要「别为故意停掉的报警」，第四轮要「删掉但文件还在的也要看得见」。三条都对，但同时满足需要把「哪些目标该出现在这一屏」整体重想——至少再分两档（在跑却没备到 vs 停着所以没备 vs 服务已不在、只剩存档），外加一条从文件名反推 id 的解析路径。按 CLAUDE.md §5.5 这是 B 类，且同一诉求已经在四轮里反复出现、我两次中间的修改还朝相反方向拉，正是「该整体重做而不是打第四块补丁」的信号。当前取「不报警」那一侧：宁可少说一句，不要一盏没人看的灯。**现状已被一条用例钉住**（`backup-panel.test.ts` 的「只以盘上文件形式存在的目标，此刻没有行」）——真去做时它会红，逼人回来一起改，而不是让这条债务只活在文档里。真要做时与 E83 / E84 同批 |
| E87 | 中 | 2026-08-29 | **面板只读「最新的那个」备份目录，另一个候选里留存的备份从此看不见**：只读档现在按结果文件的时间取最新的候选目录（第三轮 P2 修的），但文件清单仍然只 `ls` 那一个目录。写入端从一个变只读的高优先级目录切到后备目录之后，前者里存着的旧备份不再进 `fileCount` / `lastSuccessAt` / 备份历史——某个目标这一轮失败时，面板会说「盘上没有更早的副本」，而那份副本其实还在 | 备份目录发生过切换、且旧目录里还留着文件的部署 | **故意没做** | Codex review 第五轮 P2，事实成立。**方向上它是「少报」不是「假绿」**：面板会把处境说得比实际更糟，而这一整批改动治的是反过来那种病，所以不与之同批。真要做还得先答三个问题：同名文件在两个目录里出现算一份还是两份；聚合出来的「上次 X 前」指向的是写入端已经不再维护的目录，保留策略不会去清理它，面板却拿它当有效副本；备份历史与恢复走的是同一份目录解析，只改列表会让「列得出来、恢复时找不到」。这三条要一次想清楚，属 CLAUDE.md §5.5 的 B 类 |
| E88 | 低 | 2026-08-29 | **判「这台服务有没有数据要备」时，现算那条路拿不到 env 与启动命令**：面板给台账里那些「上一轮压根没出现」的服务现算覆盖结论时，手上只有镜像名与 id/容器名。`classifyBackupCoverage` 对 nats 的判据要读 env / command 才知道有没有开 JetStream，读不到就落到「没有持久状态」那一档——于是一台开着 JetStream、上一轮之后才建的 nats 会被静静归进「不必惊动人」 | 上一轮之后才建、且把 JetStream 开在 env/command 里的 nats | **故意没做** | 与 E68 / E72 / E77 同族：判据只看了 N 个来源里的一部分。**触发窗口很窄**——它只在「这台服务还没进过任何一轮备份」的那几个小时里成立，进过一轮之后走的是按构造那条路（落盘缺口已经被 `blocksHealthy` 筛过）。改法是把 env / command / entrypoint 一起传进面板的 infra 事实，与 E68/E77 那两处收窄的入参一次对齐 |
| E89 | 低 | 2026-08-29 | **项目设置的「交付模式」tab 在命令面板里搜不出来**：它有侧栏入口，但 `PROJECT_SETTINGS_INDEX` 里没有对应条目，于是搜「交付」「delivery」都到不了这一页 | 想用搜索直达交付模式设置的人 | **故意没做** | 本轮给「周期备份」补登记时，新加的覆盖守卫当场把它一起查了出来（守卫按 `PROJECT_TAB_LABELS` 判，已补上它的中文名，命令面板显示路径不再是裸 tab 值）。**没顺手补索引条目**：那要为别人的功能编一组搜索关键词——「交付模式」下有哪些概念、用户会用什么词找它，我没有把握，编一组不准的关键词比没有更糟（搜出来落错页）。留给这个功能的负责人补，形状与 `proj:backup:health` 那条一样 |
| E90 | 中 | 2026-08-29 | **cdscli 部署触发失败（如项目暂停 423）曾只报裸 `http_423`**：服务端响应体里其实带着结构化原因，但调用方从没读它，只把状态码转成了一个不带语义的字符串 | 项目处于暂停等非常规状态时触发部署 | **已修（#1433）** | 改成透出服务端 body 里的结构化原因，排障不必再去翻状态码对照表猜含义 |
| E91 | 高 | 2026-09-07 | **container-exec 命令曾以未加引号方式拼进容器 sh，宿主 shell 会先展开命令里的 `$VAR`/`$(...)`**，构成宿主环境变量泄露与命令注入面；脱敏器也不认 PEM 私钥整块，printenv/cat 输出的未闭合私钥体会原样落进日志 | 通过 container-exec 执行含 `$`/反引号的命令；或输出含 PEM 私钥体的内容 | **已修（#1448）** | 命令改单引号传入容器 sh，堵住宿主侧展开；脱敏器新增 PEM 整块识别（BEGIN 到 END，未闭合时到文末） |
...
来源 PR #1513（项目级「Agent 只允许极速版部署」门禁）Codex 第三、四轮 review。门禁已覆盖整分支与单服务部署、分支模式覆盖的增删改、项目默认模式与对齐、构建配置的通用修改与批量改模式、远端执行器派发共十余条入口，判定收在一处。按 CLAUDE.md §5.5 的 review 范围熔断（连续三轮出现新语义类别），以下三条记台账，不在该 PR 展开：

| ID | 严重度 | 创建日期 | 描述 | 触发条件 | 状态 | 备注 |
|---|---|---|---|---|---|---|
| G1 | 中 | 2026-09-08 | 「导入 compose 并立即初始化」这条入口接受调用方自带的构建配置并直接起服务，不经过分支部署的门禁 | 开了门禁的项目下，Agent 带源码模式的配置走这条入口 | open | 该入口起服务前应与分支部署同一判据、同一拒绝方式，且不许回退源码编译 |
| G2 | 中 | 2026-09-08 | 「从另一条分支拷贝配置并重部署」会把来源分支（可能是真人的 dev 分支）的模式覆盖原样拷入，再由系统内部自行重部署；内部重部署不带调用方身份，门禁不认 | Agent 从源码模式分支拷配置并要求顺带重部署 | open | 拷贝落盘前应按门禁判拷贝结果；或让内部重部署继承原调用方身份 |
| G3 | 低 | 2026-09-08 | 新建构建配置、给分支加额外服务这两条入口允许 Agent 把一份普通镜像标成「预构建」 | Agent 自建「假极速版」配置后部署 | open | 已核实标成预构建的配置不挂载源码目录，容器里没有仓库源码可编译、也不占构建槽，不违反门禁不变量（宿主不跑源码编译）；若要限制，等于拦掉面向 Agent 的分支额外服务功能，属新策略，需用户拍板 |
| G4 | 中 | 2026-09-08 | 「给分支加额外服务并重部署」保存源码模式的额外服务后由系统内部自行重部署，与 G2 同一形状 | Agent 加额外服务时要求顺带重部署，且没标预构建 | open | 修法与 G2 同 |

**为什么不顺手补**：G1、G2、G4 各是一条独立的部署入口，与该 PR 已接的十余条同属「入口接线」，但已触发熔断；G3 经核实不构成宿主源码编译，限制它会牺牲既有功能。

**做完算数的判据**：开了门禁的项目下，Agent 走 G1 / G2 / G4 三条路径部署源码模式服务均被门禁拒绝，拒绝理由与分支部署一致；真人操作与系统内部派发行为不变；分支路由的门禁测试组各加一条「同一请求，真人放行、Agent 拒绝」的差分用例。

**绕行**：门禁的定位是防「忘了」不是防「故意」；审计日志按操作者可追溯谁用了这几条路径。
...
**现状（2026-08-30 更新）**：分支库「推倒重建」这个动作**已从 CDS 撤下**。此前它只覆盖 CDS 按命名规则派生的分支库，项目自建命名的分支库（典型是 mdimp 的 `imp_b_<16 位 token>`，由项目 bootstrap 指定而非 CDS 派生）本就不在范围内；撤下后一律不可用。

**为什么撤下**：这个动作会以 root 凭据删库，所以必须先回答「这个库是不是本分支的」。这个问题在 review 里被连续打穿**四次**，每次都是「所有权的证据不成立」——每修一次就多一道拒绝，下一轮又出新进路：

| 尝试的证据 | 为什么不成立 |
|---|---|
| 分支 scope 的 `MYSQL_DATABASE` | 可写配置。改成兄弟分支的库名即可越权删别人的库 |
| 已完成建库任务的台账记录 | 建空库这个动作接受调用方指定库名，且建库时「库已存在就跳过」——指向一个**已存在的兄弟库**照样写出完成记录。台账证明的是「这条任务跑过」，不是「这个库是本分支建的」 |
| 按命名规则复算出的库名 | 分支名超长时后缀被截断，两个分支会算出同一个名字（见下一段） |
| 复算的名字 + 撞名拒绝 | 兄弟分支可以先用建空库的动作，把自己绑到本分支会算出的那个名字上。撞名检查只比对「重新算出来的名字」，从不看别的分支实际绑定在哪个库——于是本分支一次删库就把兄弟分支正在用的库删了 |

四条进路的共同点：这个动作的安全性建立在一个**并不存在的库所有权模型**上。没有那个模型，护栏就补不完——补第五道也只是等下一条进路被找出来。所以撤下，等模型建好再重开。

**表里第三条展开说**：CDS 派生的分支库名把分支 slug 截断到 28 字符，两个前 28 字符相同的分支会算出同一个库名（实测长前缀命名下真的会撞）。撤下之前曾用「撞名即拒绝」挡过这一条，但那道闸随功能一起移除了——**它不是当前行为**。这条命名缺陷本身仍在（别处派生分支库名时照样会撞），是重开时必须先解决的两件事之一。

**重开需要什么**：两件事一起做，缺一不可。其一，CDS 建库时必须真正区分「这个库是我新建的」与「它本来就在」，并把这个事实持久化，删库只认带此标记的记录。其二，破坏性操作的目标必须用抗碰撞的分支身份，而不是会被截断的显示名。两件都要动「分支库身份」这个概念，属于同一次结构性改动。

**在此之前的绕行**：分支库出现脏迁移状态时，只能由有权限的人直连该数据库手工删库后重新部署。CDS 侧不提供这个动作。

**当前守卫盯的是什么**：撤下之后守卫也跟着翻面了，现在锁的是三件事——请求这个动作会被显式拒绝且拒绝文案给得出去处；建库路径里不存在任何删库语句；撤下前那套目标计算与三道闸没有留下死代码。换句话说，守卫防的不再是「用错」，而是「悄悄回来」。

---

## 分支数据库账号：身份已抗碰撞，库名仍会撞（2026-09-01）

**事故**：数据工作台报 `ERROR 1045 (28000): Access denied for user 'cds_mdimp_claude_open_api_ch'@'localhost'`，同一台库的部分分支正常。那个账号名是旧规则「`cds_` + 分支 id 截断到 24 字符」的产物——同项目下所有 `claude/open-api-ch…` 开头的分支算出的是**同一个账号**。派发流程每次都 `ALTER USER … IDENTIFIED BY <新随机口令>`，于是最后派发的分支能连、先派发的兄弟分支全部 1045；兄弟分支删库时的 `DROP USER` 还会把共用账号整个删掉（账号不存在时 MySQL 报的也是 1045，与口令错一模一样）。

**已修**：账号身份收敛到一处（分支库身份模块，唯一派发入口）——`cds_` + 可读前缀（可截断、允许撞，只给人看）+ `_` + 分支 id 的 sha256 前 8 位（唯一性由它保证），MySQL 的 32 字符上限内成立，postgres / mongo 复用同一个名字。删库时的 `DROP USER` / `DROP ROLE` 也加了所有权判据：只有存量账号名等于按新规则复算的名字才删，否则保留并在活动日志里说明——这正是本文件上一节「破坏性操作要用抗碰撞身份」要求的那一半。

**存量分支怎么回到可用**：新规则只影响**新派发**。已经把旧共用账号写进分支环境变量的分支，仍然连不上，需要在资源面板点一次「重置连接凭据」重新派发（会二次确认，重置后应用容器要重新部署才拿到新口令）。工作台的 1045 现在会直接把这句下一步写在错误里，不再只甩一行 mysql 原文。

## 分支凭据只有一份，而一个分支可以挂多台同类型库（2026-09-01）

**这才是「CDS 自己建的库，CDS 自己连不上」的直接原因**。分支作用域的环境变量是一张平表：
`MYSQL_USER` / `MYSQL_PASSWORD` / `MYSQL_DATABASE` / `DATABASE_URL` 各只有一份。但一个分支
完全可以挂两台 MySQL（现场就是 `cloudbridge-db` 与 `mysql-mdimp` 并存）。后派发的那台把前
一台的账号、口令、库名整个盖掉，工作台再拿这一份去连另一台——账号在那台库里根本不存在，
报 1045。同一处漏判在删库路径上更狠：删 B 资源，掉的是 A 的库。

**已修**：派发时写进分支 env 的 `*_HOST`（值就是服务 id）本来就是归属标记，现在据此判断
「这套凭据是不是给这台服务的」。不是本服务的一律不用（回落到该服务容器自己的账号与库名），
删库路径直接拒绝并说明。判断不出归属（老数据没有 HOST、也没有可解析的连接串）时维持既有
行为，不让存量分支因为多一条判据而连不上。

**仍欠的两件**：
1. **一个分支仍然只能有一套「分支库」凭据**。判归属只是不再把 A 的凭据错用到 B 上；如果两台
   库都要给同一个分支派专属库和账号，第二次派发照样会盖掉第一次的 env（应用也只有一个
   `DATABASE_URL` 可用）。要真正支持，得把分支凭据按资源分槽（例如带资源前缀的键名），
   属于契约变更，影响应用侧读法，没有夹在这次修 bug 里做。
2. **写操作要 admin**。数据工作台的 DDL/DML 走 `data-write`，要求 admin；浏览器登录的人是
   admin（cookie 鉴权）没问题，但项目 Key / AI 会话解析成 developer，只能读不能写。Agent
   要在面板之外改数据，目前得走初始化 SQL 或人来点。

## 分支数据库账号：身份已抗碰撞，库名仍会撞（续）

**仍欠的三件**：
1. **库名仍会撞**——`branchDatabaseName` 把分支 slug 截到 28 字符，前 28 字符相同的分支算出同一个库名（上一节表格第三条）。这次没动它：改命名会让存量分支指向另一个（空的）库，属于要迁移的结构性改动，不能夹在修 bug 里。
2. **旧共用账号成孤儿**——重置凭据后旧账号仍留在库里、仍带着各分支库的授权。不敢自动删：可能还有兄弟分支在用它。要清理得先能证明「没有任何分支还绑着它」。
3. **重置只修 CDS 侧**——应用容器里的口令要等下一次部署才更新，这中间应用连不上库。目前靠文案说清，没有自动重部署。


## MySQL 连接上限审计只看启动命令，不查运行时真值（2026-08-30）

**现状**：复用既有 MySQL 容器时，CDS 会检查它的启动命令声明的连接上限够不够，不够就发一条告警并指出重建路径。判断依据是容器的启动命令，**不是数据库里的真实变量**。

**已知漏报**：容器用自定义 entrypoint（例如把启动包成一层 shell）时，启动命令里的参数可能被那层 shell 吃掉、根本没传给数据库进程。此时命令上看声明了足够的上限，实际仍是出厂默认，审计不会告警。

**为什么先不做**：查真实变量要建数据库连接、要凭据、还要处理「数据库刚起来还没就绪」的时序，代价与收益不匹配——这条路径唯一的动作是发一条告警，漏报的后果是少一条提示，不会做出任何错误动作。

**要做时怎么做**：连上去读服务器当前生效的连接上限，与目标值比较；或者退一步，把「参数会不会真的传到数据库进程」这个判断也纳入（识别包了 shell 的启动方式并按不确定处理）。

**绕行**：怀疑某台库欠配时，直接连上去看当前连接上限，与并发分支数对照。


## 表面分层：同色贴同色的剩余点位与探测器盲区（2026-09-01）

**背景**：用户反馈白天主题「有点光光的」。量下来根因不是白天的层次比深色弱（实测反而更强，
相邻面对比 1.622 : 1.187），而是**卡片填充和它所在的面用了同一个 token**——对比度 1.000，
只剩 1px 发丝线撑着；深色靠近黑底还能靠强调色顶出形状（琥珀对填充 9.84），白天没这个红利
（同一对降到 2.92），于是整片纯白。技能库浮层已按「内容区下沉一层」修好，两个主题都回到 ~1.21，
并有一条契约测试盯着这个不变量（用例名「技能卡浮在下沉的内容区上」，红绿都验过）。
以下是这一轮**明确没有一起修**的部分。

**剩余点位**：登录后带数据的 20 个页面里，白天 0 处同色贴同色，深色还有 2 处。没跟着一起修，
是因为深色下这两处有强调色和近黑底提供的形状线索，肉眼不成立为缺陷；把它们一起翻层会动到与
本次改动无关的组件，超出这次的范围。要做时按同一条不变量处理：卡片填充不许和它所在的面同层。

**探测器的盲区**：这次用来扫点位的静态检查只认 className 里字面写出的表面 token。三类看不见：
className 由变量拼出来的、跨文件组件的根表面、以及**列表项这种在运行时才决定填充的结构**——
验收报告列表就落在最后一类里，扫不出来不等于那里没有。所以它给的是「值得去看一眼的位置」，
不是确诊清单；判断某一屏干不干净仍要以真实截图为准。

**顺带发现的一处空转**：新栈样式表里的 `--shadow-card` 是一份写得不错的双主题五层阴影
配方，全仓组件引用次数为 0——没人用。要么接上（卡片阴影统一走它），
要么删掉；现状是既维护着又不生效，属于 `predicate-and-wiring-discipline` 形状 2 的静态版本。

## 预览实例演示数据只覆盖到分支/任务/报告三页（2026-09-01）

**现状**：预览实例（CDS 部署自己）首启会 seed 一个演示项目 + 五条不同状态的分支 + 活动日志，
后续补播了三个定时任务与三份验收报告，覆盖分支列表、拓扑、项目设置、任务调度、验收报告几页。
seed 是**分节幂等**的：首播（项目 + 构建配置 + 活动日志）只在空库跑，补播（分支 / 定时任务 /
验收报告）各判各的，分支还按 id 逐条比对。分节这一层曾经只做到「这一类有没有」，结果分支清单
从 3 条扩到 5 条之后，跑着的预览实例始终停在 3 条——「构建中 / 冷分支」两种卡片在实例里从来
没出现过，是上线后看健康检查里的 `branches=3` 才发现的。已改为逐条补，并有一条红绿都验过的
用例钉状态覆盖面（原来那条用例断言「分支还是原来那一条」，反而把这个 bug 锁死了）。

**没覆盖到的**：发布中心与发布控制台仍是纯空状态。发布目标牵扯真实主机、凭据与发布链路语义，
造一份「看着像真的」的演示数据，风险大于它带来的验收价值——空状态本身也是这两页的真实形态之一。

**要做时怎么做**：补一个只读、明确标注「演示数据」的发布目标，且必须确保它在任何路径上都不可能
被真的触发发布（`no-rootless-tree`：虚构数据必须显式标注，且不许冒充可执行对象）。

**绕行**：验收这两页时用真实项目的发布配置，或直接接受空状态作为验收对象。


## 演示定时任务仍可被手动执行（2026-09-01）

**现状**：预览实例的三条演示定时任务已全部停用，调度器不会执行它们（停用之前实机验到
「每 30 分钟同步」真的被跑过，`lastRunAt` 从 seed 写的值变成了当天的真实执行时间）。但
停用只挡住调度，任务列表上每条仍有「立即执行」按钮，手动触发会真的执行动作——HTTP 那条
打不到 `example.invalid`、必然失败，命令那条会真的跑一次。后果是把 seed 摆出来的成功/失败
示例状态覆盖掉，并往运行历史里加一条噪音。

**为什么先不做**：要挡住手动执行，得给定时任务新增一类「演示/只读」语义，并贯穿调度器、
执行接口与前端按钮态。这是一个新的语义类别，属于产品决策而不是修一个缺陷；在只为修技能库
出口的那个 PR 里展开它，正是范围熔断要拦的情况。

**已做的兜底**：三条任务的描述文案改成如实的——写明「已停用，不会被调度器执行；手动执行
会真的发起请求」，不再声称「不会真的发出请求」。用户点之前看得到会发生什么。

**要做时怎么做**：给任务加一个来源标记（seed 播下的 vs 用户建的），执行接口对前者直接拒绝
并给出可读原因，前端把按钮置灰并说明理由。同一个标记也能用来防止演示数据被误编辑。

**绕行**：状态被手动执行改乱之后，删掉这三条任务并重启预览实例容器即可重新播一份。


## 「断网也装得上」这个判据的精度上限（2026-09-01）

**现状**：技能来源面板会报「这台 CDS 上有 N 个现成的」，N = 本机技能目录命中数 + 缓存包命中数。
缓存那一档已经按下载路径的同一套校验判（非空 + zip 魔数，且只异步读前 4 字节，不搬整包）。

**已知不准**：本机目录那一档只判「目录存在」。真正打包时会扫目录找常规文件，一个空目录或只有
符号链接的目录打不出包来，但面板已经把它算进了「现成的」。

**为什么先不修**：要判准就得对每个技能递归走目录找常规文件——而这条路挂在匿名的技能清单端点
上、每个技能一次。同一个判据在三轮 review 里被要求收紧了三次（缓存没算进去 → 算进去的方式把
事件循环按住 → 本机目录档不够准），每收紧一次都在这个端点上加开销，且下一个更细的不准确处仍在。
追精度和省开销在这条判据上直接冲突。

**建议的收法不是把判据修得更准，而是降低这句话的强度**：面板别再声称「N 个断网也装得上」这种
对安装结果的预测，改成陈述查到的东西——「这台 CDS 上有 N 个的本机副本或缓存包」，告警相应改成
「可能装不上」。判据说得准什么就只说什么，是这类问题的收敛方向。

**绕行**：真装不上时错误信息本身是准的（`技能来源当前不可达，且本地没有可用缓存`），面板这个数
只影响事前预期。


## 技能库浮层没有完整的焦点约束（2026-09-01）

**现状**：技能库浮层开着时，被它盖住的那一屏（向导步骤内容）已经整块 `inert`，浮层本身有
`role="dialog"` + `aria-modal`，打开时焦点会收进搜索框。这挡住了「Tab 摸到被盖住的主按钮并
把向导推进一步」这个具体缺陷。

**没做到的**：浮层嵌在外层的技能下载弹窗里，外层的 tab 按钮和关闭按钮仍可聚焦，而
`aria-modal` 本身不实现焦点循环。键盘用户从浮层最后一个操作继续 Tab，仍可能走到外层去，
比如切到技能市场——浮层看着还开着。关闭浮层后也没有把焦点还回「打开技能库」。

**为什么先不做**：完整的焦点陷阱（Tab / Shift+Tab 环绕 + 焦点还原 + 外层兄弟节点全部 inert）
是一套通用能力，应该做成可复用的浮层基建，而不是在这一个组件里手写一份——手写的那份会成为
下一个浮层抄错的模板。这属于新增能力，不在只为修技能库出口的那个 PR 里展开。

**要做时怎么做**：抽一个焦点陷阱 hook（或直接用 shadcn Dialog 嵌套），让浮层开启时外层所有
兄弟节点 inert、关闭后焦点还原到触发元素；本组件改为消费它。

**绕行**：鼠标与触屏用户不受影响；键盘用户误切到外层时，回到上手助手 tab 向导状态仍在（切走
只藏不卸），不会丢配置。


## cds 侧没有 DOM 测试环境，交互只能靠源码守卫近似（2026-09-01）

**现状**：cds 的前端测试全部走 `renderToStaticMarkup` 的服务端渲染，加上对源码文本的守卫。
仓库里没有 jsdom / happy-dom，也没有 testing-library——**没有任何一条测试能真的点一下**。

**后果**：凡是「点开浮层 → 按某个键 → 期望某件事发生」这类不变量，只能用「源码里有没有这段
接线」来近似。近似得再小心，也仍是形状 4a 说的那类判据：钉的是实现痕迹，不是行为。本轮已经
把最脆的一条（逐字要求某段 JSX 存在）改掉，留下的否定断言与渲染断言双向验过——事故写法回来
即红、保行为的重构不红——但这只是把近似做得更稳，不是消除近似。

**要做时怎么做**：给 cds 侧加 DOM 测试环境（vitest environment + testing-library），
把这几条改写成真的渲染向导、点开浮层、按键、断言结果。这会引入新依赖，属于测试基建，
应当单独一次做完并把现有源码守卫成批改写，而不是在功能 PR 里零敲碎打。

**绕行**：真交互回归目前靠 `/验收` 的 Playwright 无头浏览器路径兜底，但那是人工触发的，
不在每个 PR 的 CI 上跑。

**2026-09-03 更新**：窄屏可达性这一面已经不再靠人工触发。新的探针自己起一个 vite 开发服务、
挂一个只渲染这个弹窗的测试入口（不进生产产物），在四档窄屏下量上手向导每一步主操作的位置，
接在每个 PR 的 CDS 巡检上。它不需要真实实例、不需要登录态，这也是它能进 PR CI 而整页移动端
冒烟不能的原因。覆盖面仅限这一屏的「主操作够不够得着」，上面说的「点开浮层 → 按键 →
期望某件事发生」那类不变量仍然只有源码守卫。

## 窄屏适配靠一串按位置数出来的选择器，换 DOM 结构会静默失效（2026-09-03）

**现状**：上手助手的窄屏全屏化整段写在样式表的 640px 断点里，靠「第一个子元素」「最后一个
子元素」这类**按位置数**的选择器认出「哪一层是头部、哪一层是内容」，再用百分比高度一层层
往下传。

**后果**：tab 区多插一层包裹、或调一下子元素顺序，某一环就静默不匹配——页面照常渲染、
测试照常绿，只是面板长成内容自然高、主操作被外层裁在屏幕外。2026-09-03 用户在手机上撞到的
「步骤 03 没有下一步、也滑不动」正是这个形态：断链点是接入弹窗里那个为「切走只藏不卸」
加的无样式包裹层，它自己没有高度声明，子元素的百分比高度就无从解析。

**这次做了什么**：断掉的那一环改成认一个显式标记而不是认位置；窄屏那层的纵向溢出从裁死
放宽成可滚当兜底，让高度链再断时至少还能滚到主操作；并补了真浏览器判据（见上一条更新）
挡住回归。

**没做的**：其余几条按位置数的选择器（藏头部、藏 tab 那几条）原样保留——改它们要动窄屏
整段布局，超出这次修复的范围，且真出问题时新判据会红。要做时一起改成认标记或改成组件里的
响应式类，别再让样式表去数 DOM。

**绕行**：动接入弹窗的 tab 区结构前，先跑一次 `pnpm run smoke:agent-starter-mobile`。

### 关联文件

- `cds/web/src/index.css` — 窄屏断点那一整段
- `cds/web/src/components/SkillDownloadDialog.tsx` — 断链点所在的包裹层
- `cds/scripts/agent-starter-mobile-probe.mjs` — 挡回归的真浏览器判据

## 整页窄屏冒烟的判据覆盖面，比它看起来窄得多（2026-09-03）

**背景**：这条冒烟此前从未被任何流水线调用过（在工作流目录里搜它的名字零命中），
原因是它要连真实 CDS 实例、要有项目数据、要能打开分支详情抽屉，PR CI 上凑不齐。
同日给它加了离线模式（自起前端 + 合成数据），已接进每个 PR。**接线问题解决了，
但接上之后量出来的覆盖面必须记下来，否则下一个人会以为窄屏布局有人管了。**

**实测出的三个盲区**（都是照着判据自己的条件注入缺陷、看它红不红量出来的）：

| 判据 | 实际抓的 | 抓不到的 |
|---|---|---|
| 横向溢出 | 文档级横向滚动 | 内部滚动容器里的溢出——外壳把内容裁住，文档级永远不超 |
| 文字压成竖排 | 特定标签上 2–12 字的短文本 | 卡片被压成 20px 宽（它是 article 元素、六十多字，两个条件都不满足） |
| 可点目标被遮挡 | 被别的元素盖住 | 被祖先滚动容器裁到视口外的——判据主动把这种情况排除了 |

第三条尤其要注意：同日那个「上手向导在窄屏没有下一步」的缺陷，形态正是「按钮被滚动
祖先裁在视口外」，**这条冒烟即使当时在跑也抓不到它**。两条守卫因此是互补而不是包含关系。

**红绿闭环**：给导航里的短文字按钮加 20px 宽、60px 高，390 那一档立刻报
「text squeezed into vertical layout」并列出九个被压的导航项；撤掉即绿。所以判据链路
是通的，窄只是窄在覆盖面。

**要做时怎么做**：把「可点目标被裁到视口外」补成一条正式判据（照上手向导探针那条硬
判据的写法，量元素矩形而不是命中测试），并让横向溢出改看每个滚动容器自身的宽度而不只是
文档级。这会让存量页面报出一批真问题，属于单独一轮的工作量。

**离线模式测不到什么**：合成数据的长度与条数是编的，真实数据把布局撑成什么样它不知道。
那部分仍然要靠真实实例模式（给 baseUrl 跑同一个脚本）或视觉验收。

**抽屉的「配置」tab 在离线模式下整个是坏的，而冒烟根本没点进去**：实测点开配置 tab
会看到「读取失败：后端响应格式异常」，因为 `effective-env` 的合成数据形状与真实端点对不上；
profile 卡因此永远渲染不出来。冒烟只在抽屉的默认屏做判定，没有切 tab，所以这一块从来
就不在它的覆盖范围内——把 profile 的 fixture 键名改对（`profiles` 而非 `overrides`）只是
让契约不再误导下一个人，并不等于补上了覆盖。要真覆盖得做两件事：把 `effective-env` 的
合成数据对齐真实返回，并让冒烟在抽屉里逐个 tab 走一遍。后者会让每档窄屏的耗时明显上涨，
属于单独一轮。

**fixture 匹配仍按路径形状，不按真实路由表**：匹配器认的是「几段路径」，不是「这个端点
真实存在且返回什么」。所以带保留字的单段路由（例如 infra 家族里那种动作型端点）会被当成
一个 ID 落进兜底 fixture，拿到一个形状对但语义不对的空响应，而不是被判为未登记。今天没有
被覆盖页面请求到这类端点，冒烟跑下来零未登记路径；真要根治得让匹配器读真实路由表或区分
HTTP 方法，那是另一件事的工作量。**判断要不要继续收紧的判据是「有没有被覆盖页面真的请求
到」，不是「能不能构造出一个绕过它的字符串」**——后者永远构造得出来，按那个走会没完。

### 关联文件

- `cds/scripts/mobile-layout-smoke.mjs` — 判据本体与两种模式
- `cds/scripts/fixtures/mobile-layout-fixtures.mjs` — 离线模式的合成数据

## 技能库浮层里按 Escape 会关掉整个弹窗（2026-09-01）

**现状**：技能库浮层是普通 `div`，没有自己的 Escape 处理。按 Esc 时事件走到外层 Radix
`DialogContent`，于是整个技能下载弹窗被关掉，向导配置一起没了——用户的预期显然是「关掉浮层，
回到推荐页」。

**不是本次引入的**：浮层之前，步骤 02 按 Esc 同样关掉整个弹窗、同样丢状态。这是这个弹窗
一直以来的行为，浮层只是让它更违反直觉（眼前明明有一层浮层，Esc 却把底下那层也带走了）。

**为什么先不改**：修法本身不长（在浮层上拦 keydown、阻止原生事件继续冒到 document、改走
`onDone`），但它依赖「React 合成事件能不能拦住 Radix 挂在 document 上的监听」这一层细节，
而 cds 侧没有 DOM 测试环境（见上一条），推上去没有任何自动化手段能证明它真的生效。
推一个验不了的修法，比留着一个已知问题更糟。

**要做时怎么做**：和上一条一起做——先有 DOM 测试环境，再补 Escape 处理并用真按键断言：
浮层关闭、外层弹窗仍开、选择保留。

**绕行**：用浮层自己的三个出口（关闭 / 放弃这次改动 / 完成选择），别按 Esc。

## 任务调度「今日执行/成功/失败」的分母来自被截断的运行记录（2026-09-01）

**现状**：结论条那三个日计数是从运行记录里数当天条数得出的，而运行记录本身有两道截断：
每个任务只保留最近一百多条，页面又只取全局最近几百条。一个每 5 分钟跑一次的任务一天产生
将近三百次，保留额度大约只覆盖十小时，于是过了上午它的早班记录就被挤掉了，
而页面照旧把剩下的当成「今日执行」的全量。

**影响**：高频任务多的项目里，日计数会**少报**，且越到晚上少得越多。低频任务（每天一两次）
不受影响——这也是它此前没被发现的原因：演示数据和大多数真实项目都是低频。

**为什么这次不修**：要让这个数字可信，得给它一个不受保留策略影响的来源——无论走哪条路，
都是一次**新的后端契约**，超出本次「把任务调度页改成可值班」的目标（CLAUDE.md §5.5 的 B 类：
有价值但扩范围）。在同一个界面改版里顺手动存储，是把两件事捆在一起。

**做完算数的判据**：日计数不再随运行记录的保留策略变化——高频任务跑满一天之后，
页面上的「今日执行」与当天真实触发次数一致。走按天聚合还是放宽查询窗口是实现选择，
取舍在于「多一份要维护的聚合」与「多留一批明细记录」哪个代价小。

**绕行**：日计数只在低频任务场景可信；要看高频任务的当天全量，走运行流的筛选逐条看，
或直接查后端记录。

## 任务调度「接下来 6H」在高频任务上少报（2026-09-01）

**现状**：「接下来 6H」把每个任务未来 6 小时内的待触发时刻加总，而这份待触发序列是
**按条数**投影的，条数本身还有上限。一个每 5 分钟触发的任务 6 小时内该有 72 次，
只拿得到二十来次；每分钟的任务该有 360 次，即便顶到上限也还差得远。

**影响**：与日计数那条同形状——高频任务越多，这个数字越偏小，且**只会少不会多**，
所以它不会造成「以为没事其实要出事」的误判，但会让人低估接下来的负载。低频任务不受影响。

**为什么这次不修**：真正的修法是把投影的口径从「给我 N 条」换成「给我到某个时刻为止」，
这是一个新的查询契约。本 PR 的目标是「把任务调度页改成可值班」，且这已是
Codex 第三轮在同一处统计口径上提意见——按 CLAUDE.md §5.5 的熔断条款，停手记账，
由用户决定是否单开一次扩范围。用户 2026-09-01 明确选择「修 P50 与已删除任务两条，
这条记账」。

**做完算数的判据**：一个 5 分钟间隔的任务在 6 小时窗口里贡献 72 次，而不是取回来的条数。
取舍在于换成时间口径之后仍要有一个上限——否则秒级调度会把响应撑爆；投影逻辑本身已经只有
一处，不必为此另起一套到期判断。

**绕行**：把「接下来 6H」读成下界（「至少这么多次」），要准确排期看调度轴与单任务的待触发列表。

## Agent 没有免登录自测子实例预览的通道（2026-09-02）

**现状**：CDS 托管 CDS 的子实例预览带门禁（用户名口令），Agent 要验证自己刚部署的界面，
只能拿到那对口令——而它是项目级的、同项目每条分支预览共用一份，交给未合并代码不合适。
本 PR 一度加过一条「子实例专用 AI 静态钥匙」的环境变量通道，评审指出并核实它同样不成立：
项目级 env 本来就是整个项目共享的（隔离通道 9），而进程级静态钥匙在鉴权里**不带项目作用域、
等价管理员**——任一条未合并分支就能以管理员身份打到兄弟分支的预览。该通道已在合并前撤除。

**影响**：Agent 交付带 UI 的改动时，做不到规则要求的「以真人路径在预览域名打开那一屏」。
退而求其次只能验证「线上跑的确实是这次的构建」（比对资产指纹）并在本地渲染同一份产物，
这不是完整验收，交付时必须如实说明卡在哪。

**为什么不顺手补**：项目级的 Agent 凭据天生带作用域，看着像现成答案，其实不是——它是拿
收方实例自己库里的哈希比对的，而子实例独立存储、首启 seed，父实例签的那把它不认。
缺的是「子实例在自己库里建一把只对自己有效的钥匙」这一段，它属于预览实例的身份设计，
不是某个界面 PR 顺手能定的契约。

**做完算数的判据**：一条分支预览拿到的凭据，拿去打同项目另一条分支预览时被拒；
Agent 用它能完成自己那条预览的登录后取证。

**绕行**：由人把口令交给 Agent，或人工代为验收那一屏。

## 基础设施凭据轮换的剩余运维边界（2026-09-06）

**现状**：MongoDB 与 Redis 的轮换、手工备份恢复、数据工作台、资源复制、迁移和副本隔离路径
已经统一使用 stdin、受限配置或 0600 临时环境文件，公开状态和日志只保留脱敏信息。轮换期间的
长作业也进入持久排空门禁。仍有三类非阻断边界：维护作业没有 owner lease 与心跳；宿主被
SIGKILL 或掉电时可能遗留 0600 临时认证文件；MySQL、PostgreSQL 与 ClickHouse 的部分旧命令
仍使用环境参数或命令参数传递凭据，本轮没有迁移。

**风险**：CDS 在维护作业执行中崩溃会留下永久 active 记录并安全地阻断下一次轮换，需要人工
确认后处置。宿主异常断电可能留下只有 CDS 用户可读的临时认证文件。其他数据库的旧路径仍可能
被同机进程诊断观察到，但不参与本轮 MongoDB 与 Redis 轮换。

**做完算数的判据**：维护作业具有 lease、owner 与启动重整；CDS 启动时按受管前缀清理已确认
无存活进程引用的孤儿认证文件；MySQL、PostgreSQL 与 ClickHouse 的操作也通过统一安全命令层，
并以 canary 证明 argv、日志、状态和响应均无原始或编码后的秘密。

**绕行**：维护作业遗留时先核对真实子进程和备份产物，再由管理员解除 fail-closed 记录；其他
数据库在专轨完成前限制高敏运维入口，仅在受控窗口使用。

**恢复副本清理补充（2026-09-08）**：TTL 清理与人工接管已按副本串行协调，尚未开始删除时
人工接管优先；删除已开始则接管返回副本不存在，不能虚报保留成功。该协调限于同一 CDS
进程，重启后的保留判断依靠持久人工标记。多个 CDS 进程不得共同写同一恢复目录；若将来
需要共享目录，须先补跨进程互斥与竞争回归，不能复用单进程通过结论。

## 密封存储初始化的剩余运维边界（2026-09-06）

**现状**：CDS 已提供仅限全局人工管理员触发的密封存储初始化通道。主密钥由服务端生成，
通过跨进程互斥、原子替换、落盘同步、0600 权限和读回校验持久化；请求体、查询、调用方请求
标识与网络头不会进入响应、HTTP 日志、活动流、控制台或持久审计。独立安全复审结论为
P0=0、P1=0。仍有三项非阻断边界：进程被强杀或主机掉电可能留下失败关闭的写锁；初始化只
保护后续写入，不会自动迁移历史明文字段；Basic 人工会话采用有期 HMAC，没有逐会话服务端
吊销表，已复制的令牌在有效期内只能通过更换密码整体失效。

**风险**：遗留锁会阻止新的密封写入，不会造成并发覆盖，但需要管理员先确认没有存活写进程
再清理。历史明文若不单独盘点，不能因为主密钥已经存在就视为已加密。Basic 会话令牌泄漏后
无法只吊销单个会话。

**做完算数的判据**：锁文件携带可验证的进程代际并有安全的启动恢复流程；提供只读盘点与
显式迁移任务，证明历史敏感字段全部变为密封格式；人工会话进入服务端会话表并支持单会话
吊销、全部吊销与审计查询。

**绕行**：遇到遗留锁先停写并核对进程，再由管理员清理；初始化后立即运行历史明文盘点；
Basic 会话疑似泄漏时更换管理员密码并重启 CDS，使既有 HMAC 令牌整体失效。

## OpenDesign Workspace Session 的剩余扩展边界（2026-09-06）

**现状**：OpenDesign 已通过会话级容器、独立命名卷、资源硬限制、限域出口代理和原子产物清单
接入 CDS。MAP 冻结本次运行的 CDS 连接和知识快照，外部执行器不持有对象存储或真实模型凭据。
仍有三项非阻断边界：运行中的会话状态主要属于单个 CDS 进程，控制面重启会中断在途任务；首版
工作区物化的是服务端解析后的知识文本和受控附件描述，不等同于把 PDF、图片等全部原始二进制
文件交给执行器；Provider 定义与适配器仍随代码发布注册，不支持在线安装任意第三方插件。

**风险**：CDS 重启期间的任务需要按失败任务重新发起，不能承诺从容器内部断点续跑。依赖原始
版式、图片像素或嵌入附件的设计任务可能丢失视觉上下文。新增 ClosedDesign 等 Provider 仍需要
发布 CDS 或 MAP 适配器，不能只上传一个配置包立即启用。

**做完算数的判据**：会话租约、阶段和回收状态进入共享事实源，重启后可以判定接管或安全终止；
工作区合同支持带哈希、大小和媒体类型的原始附件清单，并有病毒扫描和权限快照；Provider 清单、
能力探针、许可声明与适配器包具有签名、版本、灰度和回滚合同。

**绕行**：当前发布期间停止接收新的 OpenDesign 长任务；涉及原始附件时先由 MAP 转换为受控文本
或图片摘要并提示来源形态；新增 Provider 继续走代码审查、固定版本镜像和 CDS 灰度部署。

## 项目暂停后恢复，被保留的容器仍可能被一次预览访问拉起（2026-09-02）

**现象**：项目暂停 → 恢复（不重新部署）。分支停在 idle、`lastStopSource='system'`，
而项目的 `paused` 已回到 false。此时任何人访问预览域名，自动唤醒判据看到
「来源是 system（在白名单里）+ 项目未暂停」，就把保留下来的容器 docker restart 起来了。

**为什么算债**：恢复路由的契约写的是「解冻，但不自动重新部署，需用户手动 deploy」。
一次被动的预览访问把容器拉起来，和那句话是拧着的。

**为什么这次没修**：判据本身分不出「这条 system 停机是项目暂停造成的」还是
「auto-lifecycle 造成的」——停机理由是**按 trigger 推导**的（`stopAttributionFromRequest`），
暂停路由在 body 里带的那句「项目已暂停，自动停止容器」根本没被存下来，
落库的是统一的「系统触发停止」。要分开得先让这个理由能传下去（或新增一档停机来源），
那是跨三个文件的语义扩展，超出当次 PR（首页交接与画布落位）的目标。

**修的时候怎么做**：给暂停停机一个能落库的标记（照 `BRANCH_DELETE_CLEANUP_MARKER` 的样子，
定义在 `branch-wake-eligibility.ts` 这一处），暂停路由发它、停止路由存它、唤醒判据排除它。
守卫：同一个分支，标记在就不许唤醒、标记不在照常唤醒——两条并排断言，
否则把整个函数改成 return false 也会绿。

**边界**：这条只影响「暂停过又恢复、且没重新部署」的分支。没暂停过的分支不受影响。

## 服务关系与跨项目引用（2026-09-02）

**现状**：PR「服务关系与跨项目引用」评审中按规则 5.5 判为 B 类（有价值但扩范围）的三项，本 PR 只做拒绝式守卫或说明，不展开。

- R1 单服务部署接口没有执行器分发，只会在 master 本机操作；引用切换后的重建复用它，远端执行器分支上会落错宿主。本 PR 对这类分支直接拒绝切换并提示走完整重新部署。
- R2 引用切换后的重建走单服务部署，会拉最新代码并清掉钉住的提交；没有「保留当前版本只重建容器」的模式。本 PR 对钉住的分支拒绝切换。
- R3 跨项目引用只在专用的引用读写接口上按凭据可见性拦截；项目环境变量、compose 导入等摄入路径与部署期解析没有请求上下文，项目级凭据仍可写入指向其他项目的引用并在生效配置里看到目标地址。
- R4 端口模式预览（HTTP 与 WebSocket 两条路径）仍用自己的一套路由判定，没接到转发器与 master 兜底共用的那份；同一路径在端口模式下可能落到不同服务，且忽略分支级 profile 覆盖。

**影响**：远端执行器分支与钉住提交的分支不能在引用分区切换目标，只能改配置后走完整重新部署；跨项目引用的可见性保证只覆盖专用接口。

**为什么不顺手补**：R1 要给单服务部署补与完整部署同款的执行器分发；R2 要给部署链加「同镜像、同提交、只刷新环境变量」的重建模式；R3 需要「项目之间允许引用谁」的策略（项目级 ACL），CDS 目前没有这个概念；R4 是本 PR 未触及的既有接口的接线改造。四者都不该由一个界面 PR 顺手定下。

**做完算数的判据**：远端执行器分支上切换引用后，目标容器在该执行器上被重建；钉住的分支切换引用后仍停在原提交；项目级凭据在环境变量里写下指向不可见项目的引用时，生效配置与部署期都拒绝解析；同一条带前缀的路径在端口模式预览、转发器、master 兜底三处落到同一个服务。

**绕行**：改 compose 或项目环境变量后走完整重新部署；跨项目引用的授权以专用引用接口为准。

## 复制集面板的画布常量不随界面尺度走（2026-09-08）

**现象**：整站按根字号 85% 呈现、三档「界面尺度」可切后，`ReplicaSetPanel` 的画布仍按 px 常量（`GROUP_W` / `PROJ_W` / 行高等）与量出来的容器宽度排布，切档时框的大小不变、只有容器变窄。守卫 `rem-scale-guard` 把它登记在 `CANVAS_FILES`，整文件按画布 px 单位保留，避免「框是 px、字是 rem」的双重缩小。

**为什么不顺手补**：画布常量散在 1600 多行的布局函数里，改成随根字号走要把 `dataGeo` 的输入乘上 `根字号 / 16`，并逐处核对 SVG 连线与绝对定位框仍对齐；这是一次独立的视觉验收，不该混在整站单位迁移里。

**做完算数的判据**：切到紧凑 / 宽松档后，复制集面板的框与字按同一比例变化，SVG 连线端点仍落在框边；守卫的 `CANVAS_FILES` 里删掉该文件仍绿。

**绕行**：面板按容器宽度自适应，切档只影响外围留白，不影响功能。

## 分支库供应已由 CDS 原生提供，mdimp 的反向驱动脚本尚未下线（2026-09-08）

来源 #1436（2026-08-28，2026-09-08 吸收进本台账后关闭）。issue 提出的「分支库应由 CDS 供应、
以普通 env 注入、应用只消费，而不是让应用容器持 RW 密钥反向驱动平台」这一诉求，CDS 侧已经由
PR #1490（2026-09-04，数据库隔离收敛 0-5）落地：`dbScope=per-branch` 自动给库名 env 加分支后缀、
部署前时间点克隆、台账与删分支默认保留、分支账号派发抗碰撞；看板 [plan.cds.database-isolation.md](./plan.cds.database-isolation.md)
收敛 0-5 全部已验收。与 #1423（共享 infra 的账号口令由 CDS 自己发）是同一方向的两层，都已完成。

**仍欠的不在本仓库**：mdimp 自建命名的 `imp_b_<token>` 库不在 CDS 派生范围内（见上文
「分支库推倒重建只覆盖 CDS 派生命名」），它的 bootstrap 脚本仍持 RW 密钥反向建库。收尾动作是
...
## 停机原因的施动者只到「CDS」这一层，基础设施类操作还没记下是谁点的（2026-09-09）

来源 PR #1520（停机原因按「先说外因再说内因」重写）第四轮 review。容器停止原因现在会讲清
「谁做了什么、要不要紧」，但它只能讲上游记下来的东西：分支服务的停止 / 部署替换那几条路径
早就带着 `actor` / `trigger` / `requestId`，读起来是「由 AI Agent 手动触发部署」；基础设施
服务的停止、删除、重启、重同步这一类只记了容器名、意图种类和一句原因，于是同一句话在这类
操作上只能显示「CDS（未记录触发者）」。

那句话本身没说谎——上游确实没记，规则也要求追不到就明说——但它把「谁停的这台数据库」这个
问题留给了人去翻服务器事件日志。

**为什么不顺手补**：停基础设施容器的那个服务方法要接受一整套生命周期上下文，四个路由
（删除 / 停止 / 重启 / 重同步）各自从请求里派生施动者与入口再传下来。这是采集层的新数据流，
不是本 PR 那句文案的缺陷；PR #1520 已经走到第四轮 review，按协作规则里的 Review 范围熔断
条款属于「有价值但扩范围」，留到这里。

**做完算数的判据**：在项目设置里手动停一个基础设施服务，它的停机原因第一句能读出是谁在哪个
入口停的（例如「由 <用户> 在项目设置手动停止」），而不是「CDS（未记录触发者）」。

**绕行**：服务器事件日志里「请求停止基础设施」那条带着请求上下文，能人工对上。

## cds-stop 一个种类承接三类语义（2026-09-09）

同一来源。`cds-stop` 这一个意图种类同时承接手动停止、调度降温和失败收尾——副本集成员就绪
检查失败后停容器走的就是它。PR #1520 的做法是把上游记录的原因原文摆进那句话，让人自己看出
`replica-member-not-ready` 是一次失败；更彻底的做法是按失败与否分出独立的意图种类，让结论
本身就能分开写。

**做完算数的判据**：失败收尾的停止有自己的结论句（不再是「原因见下一句」这种把判断交还给人
的写法），且新增的意图种类必须在结论表里补齐，漏了编译不过。

## 公开面板的口令保护还没做（2026-09-11）

公开状态页本身已落地（`/s/<token>`，匿名只读，对外只出业务名 / 红黄绿 / 7 天条带 /
更新时间，随时可撤销）。设计稿里还有一档没做：**给链接再加一道口令**，用于发给客户
这种「链接可能被转发」的场景。

**现状的安全模型**：token 是 16 字节随机串，不可枚举；拿到链接的人就能看。
对「给团队其他人看」够用，对「发给外部客户且不希望被继续转发」不够。

**做完算数的判据**：开启口令后，不带口令打开同一个链接拿不到任何业务名与状态；
输入口令后可见；改口令之后旧口令立刻失效。

## 存量监控的环境判定依赖运行时反查，没有一次性回填（2026-09-10）

`previewBranchId`（地址落在哪条分支预览上）是写入时盖的戳。这个字段出现之前登记的
监控身上没有戳，靠 `getSummary` 每次按地址反查一次兜底——判定是对的，代价是每次摘要
对这些监控多查一遍项目地址台账（按项目缓存，一次摘要一遍）。

**为什么不顺手回填**：回填要么写一次性迁移脚本，要么在服务启动时扫一遍全量监控改写
台账——前者是一次性产物、后者会在启动路径上加一次全量写。当前实例的自定义监控是个位数，
反查成本可以忽略。

**做完算数的判据**：自定义监控数量上到三位数后，`GET /api/uptime/summary` 的耗时不因为
...
仍是 building、没有终态。启动时的中断收敛只认了一部分，剩下的靠 45 分钟硬超时才会
被判卡住。自检端点的「部署卡住」现在按心跳（停超 10 分钟）报，所以面板与通知会先响；
但响了之后没有任何自动动作，人得自己去把这些记录标成 interrupted。

**判据**：启动收敛必须把「心跳早于本次启动时刻的在途记录」全部收成 interrupted，
且自检的「部署卡住」在重启后 15 分钟内应回到 0。

**已处理（同日）**：启动时先跑一遍「心跳早于本进程启动时刻的在途 run 一律收敛为失败
（可重试）」，再跑原来的周期收割。剩下的欠账只有上面那条：自更新的重启没换进程。

### 追加：自发现监控的告警一律按「业务」分类（2026-09-16，Codex #1543 第四轮）

告警分类现在只看探测来源：`custom`（自定义 / 自发现）算业务，分支容器与发布目标算基础设施。
CDS 监控自己那 13 条也是自发现来的，于是磁盘、Docker、探测器停摆这类宿主级故障会以
「业务挂了」的名义发出去：只订了 `infra-down` 的通道收不到，订了业务事件的通道收到的
标签是错的。人还是会被通知到（默认订阅是业务事件），错的是分类，不是有没有铃。

**为什么没有在这个 PR 里修**：正解是让自检端点在 `cds:monitor` 里自报一档告警分类
（业务 / 基础设施），发现器带过来、路由按它分——这是给协议加一个新语义字段，跨自检、
发现器、对账、路由四处，不是本 PR「判据由被监控方自报、CDS 只消费」这个目标内的直接缺陷。
按名字猜（componentId 带 host / docker 就算基础设施）是第二个判定源，不做。

**判据**：CDS 自身的 `host.disk-usage` 与 `host.docker-ping` 翻红时，只订 `infra-down`
...
分支只记 `lastDeployAt`（最后一次部署的时刻），没有部署历史。按天分桶它得到的是
「最后一次部署时间的分布」，不是每日部署量——两者今天可能看着差不多，明天就静默错位。

所以 `buildPipelineSeries` 刻意不产出部署序列（`deployNote: 'no-deploy-history'`），
页面放大态照实写明。代价是「改动 → 部署 → 验收」这条流转链在走向图里缺中间一环：
读者看得到改动开出来、看得到报告归档，看不到中间有多少起了预览。

补法：后端记一张部署事件表（每次部署一行，含 branchId / 时刻 / 结果），
`buildPipelineSeries` 再按它分桶。守卫 `tests/services/pipeline-series.test.ts`
里有一条源码扫描，防止有人图省事把 `lastDeployAt` 按天分桶冒充历史。

### 2. 夜间主题的语义三色越出暗底明度带

拿 dataviz 的 `validate_palette.js` 跑仓库实际 token：

| 主题 | 取值（ok / warn / bad） | 结果 |
|---|---|---|
| 白天 | `#059467 / #db7706 / #ba1c1c` | 六项全过，但色觉分辨 ΔE 7.7 落在 6–8 地板区，**必须配第二编码** |
| 夜间 | `#36d399 / #fbbd23 / #f87272` | **明度带 FAIL**：三色明度 0.773 / 0.833 / 0.712，越出暗底该有的上界 |

白天那条地板区的义务已经履行：`TrendCharts` 给三档配了互不相同的线型（实线 / 长虚 / 点虚）
与图例同款线型，守卫钉死「三档线型必须互不相同」。

夜间那条明度 FAIL 没修：`--ok/--warn/--bad` 是全 CDS 共用的 token，改它会波及每一个
状态标、每一张徽章、每一处结论色，远超本次范围。对细线的危害小于对大色块（线只占几个像素宽，
过亮主要是刺眼不是读不出），所以判为可接受的已知边界。
...
## 流水线缺两样持久事实：改动开启日 与 部署证据（2026-09-15）

来源 PR #1532 第四轮 Codex review（一条 P1、一条 P2）。两条不同的表征，同一个根因：
**分支被删之后，CDS 手上只剩一块墓碑，而墓碑记的是「什么时候撤下的」，
不记「什么时候建的」，也不记「有没有真的起过预览」。**

### 1. 「新开改动」的柱子会随分支回收而变矮（P1）

`buildPipelineSeries` 按现存 `BranchEntry.createdAt` 给改动分桶。仓库开了
「合并后自动删分支」的话，那条改动当天就从 90 天柱状里消失，于是走向图上会出现
「有报告归档、柱子却是零」——图自己和自己矛盾。

本轮的处置是**照实写在图上**（放大态页脚新增一段），没有编数补齐。
做完算数的判据：后端记一张改动开启事件表（每条改动一行，含分支 / 项目 / 开启时刻），
`buildPipelineSeries` 改按它分桶；然后删掉一条分支，柱子不许因此变矮。

### 2. 墓碑写死 `deployed: true`（P2）

`pull_request.closed` 的分发器在**没有对应 BranchEntry** 时也会写墓碑
（PR 从未被 CDS 跟踪、或 delete 事件先到）。流水线把每块墓碑一律当成「起过预览」，
于是这些 PR 会把漏斗的「已部署」那一段撑大一点。

本轮先做了收窄：放弃（abandoned）的墓碑不再进 changes，所以受影响的只剩合并过的 PR，
而 CDS 对已接管的项目是 push 即部署，合并过的分支绝大多数确实起过预览——
量级小，但它仍然是个假设而不是证据。

**为什么不在那个 PR 里顺手做**：要给 `BranchTombstone` 加一个持久字段、改 webhook
写入面，还要给**存量墓碑**定一个「没有这个字段时算什么」的口径——存量一律算未部署
会让历史漏斗整体塌一截，一律算部署等于没改。这是新的数据契约，按协作规则 §5.5
的「有价值但扩范围」留在这里。

**判据**：造一条「PR 合并但从未有过 BranchEntry」的墓碑，漏斗的「已部署」不许 +1。

两条补法可以合并成同一张事件表：一行记 `{改动, 项目, 开启时刻, 首次部署时刻}`，
第 1 条用开启时刻分桶，第 2 条用首次部署时刻判真伪。

## 报告认领仍然只认分支名（无 PR 号 / 无 commit 的历史报告）（2026-09-15）

来源 PR #1532 第六轮 Codex review（P2）。`matchesChange` 的三把钥匙里，PR 号那一把
已经改成「两边都记了就以它为准，不等即否决」。但**两边都没记 PR 号也没记 commit** 的
历史报告仍然只能靠分支名认领——分支名会被复用，于是一份旧报告可能被挂到同名的新改动上，
新改动因此继承旧结论、并从「部署了没验收」里消失。

**判据**：造一条报告，分支名与当前活分支同名、无 PR 号无 commit、`createdAt` 早于
该分支这一代的 `branchLiveSince`。它不许被认领。

**为什么不在那个 PR 里继续修**，理由按协作规则 §5.5：这是同一个判据在同一个 PR 里
**第三次**被要求扩张（第一次墓碑作用域、第二次 PR 号否决、这是第三次），而这一次要往
`ChangeKeys` 里塞进「这一代从何时开始」这个新概念，并想清楚合并覆盖那一侧
（`buildMergeCoverage` 拿的是墓碑，墓碑没有 incarnation 可言）该用什么。规则原话是
「同一个判据第二次被要求扩张」就该停下来跑 `/scope-check`，不该在 review 轮次里
边修边长。

**补法**：与本文件上一条「改动开启事件表」同一件事——改动单元一旦有了自己的开启时刻，
「报告早于这一代」就是一次直接比较，不必再往自由文本判据上加第四把钥匙。
在那张表落地之前，这里维持现状。

**影响面**：只在「分支名被复用 + 旧报告没记 PR 号与 commit」同时成立时发生。
本仓库的 Agent 分支带随机后缀（`claude/xxx-t57jzo`），复用率低；手写分支名
（`feat/login`、`fix/ci`）才是高危区。

## 报告版本折叠只按标题元组，不看它验的是哪条改动（2026-09-15）

来源 PR #1532 第七轮 Codex review（P2）。`foldReportVersions` 的身份键是
`projectId | kind | target | targetDate`——同一个项目、同一类验收、同一个对象名、同一天的
两份报告会被判成「同一目标的两个版本」，只留新的那份。如果这两份验的其实是**不同的**
PR / commit / 分支，另一条改动就凭空变成「没验过」，它的结论也一并消失。

**判据**：造两份报告，标题元组完全相同、`prNumber` 不同，跑一次 `buildPipelineOverview`，
两条改动都要能认领到自己的那一份。

**为什么不在那个 PR 里修**，理由与本文件上一条（分支名认领）同源，且更重：

1. 折叠是**所有聚合的上游**——总览、走向序列、流水线漏斗都吃它的输出。改这个键会让
   首页每一个数字同时变化，而本 PR 剩下的评估面已经不够覆盖这种全局位移。
2. 这是同一族判据（报告 ↔ 改动的身份）在本 PR 里第四次被要求生长。协作规则 §5.5 的
   熔断条件已经命中（连续三轮 review 都在产生新语义类别），规则要求此时回到原始目标，
   而不是继续往判据上加钥匙。

**补法**：与本文件已记的「改动开启事件表」是同一件事。改动单元一旦有稳定身份，
折叠键就该是「同一个改动的第几版」，而不是「同一个标题的第几版」——那时这三条
（分支名认领、版本折叠、部署证据）会一起消掉，而不是各打各的补丁。

## 报告旧版本对象不再即时删除，需要一次回收（2026-09-15）

来源 PR #1532 第八轮 Codex review（P1）。改格式重写正文时，对象键的扩展名会变，
旧对象就没人认领了。原先的写法是当场 `remove` 掉——**顺序错了**：`save()` 是写后即返回的
（Mongo 后端更是排队写），删完之后进程要是退了、或者那次写盘失败，持久的元数据仍然指着
刚被删掉的旧键，报告重启即不可读，而新对象反倒成了孤儿。

现在的口径：**不删**。一个删不掉的孤儿只占空间，一份读不出来的报告是数据丢失，
两者不对等，所以宁可留。

**欠的是回收那一半**：桶里会慢慢积下「曾经是某份报告某个版本」的对象。做完算数的判据是
一次可重跑的清理——列出报告前缀下的全部对象，减去 `acceptanceReports` 里所有在用的
`objectKey`，剩下的按年龄删（给一个足够长的宽限期，避免删掉正在写的那一个）。

**为什么不在那个 PR 里顺手做**：那是一条新的运维链路（列桶、对账、定时、宽限期、
可观测），不是那次顺序修正的缺陷。按协作规则 §5.5 属「有价值但扩范围」。

**影响面**：只在「改格式 + 同时重写正文」这一条路径上产生孤儿，不是常见操作；
且每份报告最多留下它历史格式数那么多个对象。

## 「正文只在本地盘」这件事没有告诉用户（2026-09-15）

来源 PR #1532 第九轮 Codex review（P2）。四个 R2 变量全空时，归档照常成功、接口回 201，
元数据上如实记了 `storage: 'local'`——但**前端拿不到也不显示它**：`AcceptanceReport`
类型里没有这个字段，新建结果、台账、查看器都没有任何提示。于是用户拿到的是完全正常的
成功态，而那份正文下一次容器重建就没了，他只会在若干天后点开时撞见 404。

标记本身是这个 PR 做的（那一步是对的），欠的是**把它端到用户眼前**。

**做完算数的判据**：在未配对象存储的实例上归档一份报告，新建结果那一屏就能看到
「这份正文只在本机，重建即丢」，且台账里该行有可见标记——不需要点开才知道。

**为什么不在那个 PR 里做**：要动前端类型、创建结果面板、台账列与查看器四处，
是产品呈现而不是这次口径修正的缺陷，按协作规则 §5.5 属「有价值但扩范围」。
另一个可选口径（「除非显式选了开发模式，否则拒绝只用本地」）更是产品决定，
需要仓库所有者拍板。

**当前缓解**：`/raw` 与 `/download` 取不到正文时已经分四种成因给出可执行原因，
其中就包括「未配对象存储」。所以用户撞见时至少知道为什么，只是知道得太晚。

## 结论三档的计数与漏斗「已验完」那一段对不上（2026-09-15）

来源 PR #1532 第十轮 Codex review（P2）。后端给 `funnel.pass/conditional/fail` 计数时，
只看「这条改动有没有匹配到报告」，不看它有没有部署过；而前端那条分流图的「已验完」
一段走 `splitFunnel`，会把 accepted 逐级夹进 deployed。报告挂在一条从没部署过的分支上时，
同一屏会出现「已验完 0 条」配着「通过 1 · 未通过 2」的结论条——两个数来自同一次请求，
却互相矛盾。

**判据**：造一条从没部署过的分支 + 一份挂在它上面的通过报告，跑 `buildPipelineOverview`，
断言结论三档之和不超过 `splitFunnel(total).accepted`。

**为什么不在那个 PR 里修**：两种修法都要先定语义，而这是产品决定不是缺陷修正——
夹掉多出来的结论（那要决定丢哪一份）、还是把这类报告单列成第四种状态（那是新语义类别）。
按协作规则 §5.5 属「有价值但扩范围」，且本 PR 的 review 修复提交已越过熔断线。

**它和本文件里另外两条同根**：`deployed` 这个信号本身就弱——墓碑写死 `deployed: true`、
分支只记最后一次部署时刻。三条一起看就清楚了：**改动到底有没有起过预览，系统现在答不准**。
那张改动事件表落地之后，这条会连着一起消掉。

## 对象存储的失败原文直接回给了用户（2026-09-15）

来源 PR #1532 第十一轮 Codex review（P2），对照 Codex 侧那条「错误要给人看得懂的下一步」的规则。
本地缓存未命中时会回源对象存储；非 404 的失败（网络、5xx、校验和不符）沿着异常消息
一路冒到正文读取与下载两个接口的响应里。用户看到的是上游 HTTP 诊断，既不稳定，
也不告诉他下一步该做什么。

**判据**：造一次上游 5xx，断言接口回的是**稳定错误码 + 一句可执行的中文**，
且原始诊断只出现在服务端日志里，不出现在响应体中。

**为什么不在那个 PR 里做**：要定一张错误码表（哪些算「暂时性、可重试」、哪些算
「配置问题、去改凭据」、哪些算「对象真没了」），那是新的接口契约面。本 PR 的 review
修复提交已远越 §5.5 熔断线，按规则该停。

**注意别和已修的那条混了**：「取不到正文时分四种成因给出可执行原因」已经做了
（未配对象存储 / 归档于本次改动之前 / 对象不在 / 登记了却取不回来）。欠的是
**取回过程本身抛出的异常**那一条路——它绕过了那四档，直接把原文透出去。

## 删报告时元数据还没真正落盘就开始删对象（2026-09-15）

来源 PR #1532 第十二轮 Codex review（P1），是同一条线索的更深一层。

本 PR 第九轮已经把顺序改对了——先落元数据再删对象。但 `save()` 只是把写入排进队列
（Mongo 后端是写后即返回），删对象紧接着就发出去了。如果删赢了这场竞速、而那次写盘
随后失败或进程退出，持久的元数据还在、还指着一个被删掉的键——这条报告重启即永远打不开。

**做完算数的判据**：删一份报告，在 `save()` 与远端删除之间注入一次写盘失败，重启后
这条报告要么完整可读、要么根本不在列表里；**不许出现「列表里有、点开 404」**。

**为什么不在那个 PR 里做**：要让删除路径变成异步并等一次真正的 flush，而
`deleteAcceptanceReport` 现在是同步的、有同步调用方，改签名会往外扩一圈。
按协作规则 §5.5 属「有价值但扩范围」，且本 PR 的 review 修复提交已远越熔断线。

**它比看起来轻一些**：现状下留下的是「元数据在、对象没了」，而对象删除本身是
fire-and-forget 且失败只记日志——真正撞上要求「删对象成功 + 写盘失败 + 进程退出」
三件事同时发生。但判据仍然是那条：宁可留孤儿，不可留幽灵。

## 版本折叠先于时间窗切分，上一窗的分母会被削掉（2026-09-15）

来源 PR #1532 第十二轮 Codex review（P2）。`foldReportVersions` 对**整段历史**先折叠一次，
再按窗口切分。于是一份归档于上一窗、却在本窗被新版本取代的报告，会在折叠时就被丢掉——
上一窗的分母因此偏小甚至为空，环比那一栏说的不是当时真实的状态。带 `to` 的历史查询同理，
会被切点之后才产生的版本影响。

**判据**：造一份上窗归档、本窗被取代的报告，断言上窗的分母把它算进去。

**为什么不在那个 PR 里做**：折叠是所有聚合的上游（总览、走向序列、流水线漏斗都吃它），
改成「按窗口各折一次」会让每一屏的数字同时位移，而本 PR 剩下的评估面覆盖不住这种全局变化。
与本文件里「版本折叠只按标题元组」那条是同一个函数的两个方向，应当一起改。

## 报告正文换版没有走「先落新键、再切元数据」（2026-09-15）

来源 PR #1532 第十三轮 Codex review（P1）。PATCH 只换正文、不换格式时，对象键由报告 id
与格式推出来，两版正文因此落在同一个键上：新正文一写进去，唯一那份持久的旧正文当场没了。
紧接着的元数据保存是写后即返回的，它要是失败、或者进程在落盘前退出，持久元数据仍然停在
旧标题、旧结论、旧哈希、旧字节数上，而读正文拿回来的已经是新的一版——账和货对不上。

窗口很窄（两步之间），影响是一份报告的元数据与正文错配，不是数据丢失；但它确实存在，
判据也清楚：**对象键要带版本、不可变，先把新对象落定，元数据持久化成功之后再退役旧键**。
退役同样不当场删，按上面那条孤儿回收的口径走。

**为什么不在那个 PR 里做**：带版本的键会牵出键的命名契约、读路径的回退顺序、退役时机与
回收对账，是一条新的存储语义，不是这次「正文改存对象存储」的缺陷。按协作规则 §5.5 属
「有价值但扩范围」。做完算数的判据是：在元数据保存必然失败的注入下改一次正文，重启后
读到的元数据与正文仍然是同一版。

## 走向图末格是半天，这件事只在放大态说了（2026-09-15）

来源 PR #1532 第十四轮 Codex review（P2）。序列的最后一格是「今天到此刻为止」，白天任何时候
打开都是如此；而这件事只写在放大态的脚注里。紧凑态是默认视图，最右一点看起来和前面每一天
一样完整，于是一天正常的半程量会被读成突然掉头向下。

不当场做的原因是它要的不是加一句话。第一屏的指示是「少字或无字」，再塞一行脚注正好撞上；
正解是给末格一个**视觉**上的记号（末段虚线、末点空心、或末格底色浅一档），那是新的一档图元
约定，要同时管三张图、双主题、以及与既有线型不冲突，属协作规则 §5.5 的「有价值但扩范围」。

**做完算数的判据**：紧凑态下 `lastDayPartial` 为真时，末格在 DOM 上带得出机读标记，
且双主题截图里肉眼能看出它与前一格不同；为假时该标记不出现。

## 同一份报告的并发改写没有串行化（2026-09-15）

来源 PR #1532 第十六轮 Codex review（P1）。异步更新走的是「在 state 里那个活对象上直接改，
IO 段失败就用进入时拍的快照整组回滚」。这条回滚是为了修另一个缺陷加的，它在单请求下成立；
两个请求同时改同一份报告时不成立：后到的那个写成功了，先到的那个失败回滚，会把后者的结果
按前者进入时的快照覆盖回去。随后延迟落盘把这份被退回的元数据写进库，而对象存储与本地缓存
里躺着的是后者的正文——账和货各说各的。

真正要的是把改写串起来（按报告 id 排队），或者改在一份脱离 state 的候选对象上、成功之后
再整体换进去。两条都是新的并发控制机制，不是这次回滚写法的笔误；本 PR 的 review 修复提交
早已越过协作规则 §5.5 的熔断线，故记账不展开。

**做完算数的判据**：对同一份报告并发发两个 PATCH，让先到的那个在 IO 段必然失败，
重启后读到的元数据与正文必须是后到那一版，而不是任何一版的一半。

## 发布闸认的是全历史最近一份发布验收，不认当前要发的那一版（2026-09-15）

来源 PR #1532 第十七轮 Codex review（P1）。发布闸只看「最近一份发布验收的结论」：
A 版通过之后，B 版的提交与分支照常推进，闸门仍然显示「可以发布」，而 B 版从未被验收过。
一份全历史的最近结论授权不了当前这一件产物。

修它需要一个这个聚合现在拿不到的输入：**当前要发的是哪一版**（候选提交或版本号）。
`buildReportsOverview` 收到的只有报告与墓碑，没有候选标识；把闸门绑到候选上，等于给这个
函数加一条新的输入契约，并在调用方补上「谁是候选」的判定。按协作规则 §5.5 属「有价值但
扩范围」。也不宜退而求其次判成 unknown——只要此后归档过任何一份报告就转 unknown 的话，
这个闸门在真实数据上几乎永远是 unknown，那是用噪音换正确。

眼下的缓解只是句子本身带着日期（「最近一次发布验收 2026-09-01 通过」），读者看得出它有多旧，
但顶上那个「可以发布」的标签仍然可能是假的，这一点如实记在这里。

**做完算数的判据**：给定一份 A 版通过的发布验收，再造出晚于它的 B 版候选，闸门必须不是 open。

## 读正文的回填与改正文的写入会互相覆盖（2026-09-15）

来源 PR #1532 第十七轮 Codex review（P1）。与上面那条「并发改写同一份报告」同源，另一半在读路径：
本地缓存未命中时会回源对象存储再回填，如果这次读取拿到的是旧对象，而在它落盘之前有一次
PATCH 写入了新正文与新缓存，回填的重命名就会把旧字节盖回新缓存上。此后的读一律本地优先，
这份报告就会一直端出旧正文，下载与下游导入跟着一起错。

要的同样是串行化（按报告 id 排队），或者在回填前后比对一个内容代次、不一致就丢弃这次回填。
两条都属于新引入并发控制机制，与上一条合并处理，不在本 PR 展开。

**做完算数的判据**：让一次回源读取在返回后、落盘前被一次 PATCH 抢先完成，读方的回填必须
不生效；随后读到的正文是 PATCH 写入的那一版。

## 切时间窗会把流水线请求发两遍（2026-09-15）

来源 PR #1532 第十八轮 Codex review（P2）。`loadPipeline` 的身份随 `overviewDays` 变化，
于是切 7 / 14 / 30 天时两条 effect 同时触发：一条因为窗口变了，另一条因为依赖里那两个
加载器换了身份。竞态守卫会丢掉先到的那份响应，所以数据是对的，浪费的是一次跨项目聚合
与一次网络往返。

不当场改的原因是它要动加载触发的接线：把加载器做成不随窗口变身份（窗口值改从 ref 读），
或者把「状态变了」与「窗口变了」拆成两条互不重叠的 effect。这一块的接线在本 PR 里已经
因为竞态与空状态各改过一轮并各引入过一次回归，为省一次请求再动它，风险高于收益。
按协作规则 §5.5 记账。

**做完算数的判据**：在 7 / 14 / 30 天之间切一次，`GET /api/reports/pipeline` 只发一次。

## 一份报告会被两条改动同时认领（2026-09-15）

来源 PR #1532 第二十四轮 Codex review（P2）。`buildPipelineOverview` 对每条改动各跑一次
`projectRefs.filter((r) => matchesChange(r, c))`，认领是**独立判定**的：同一个项目里两条分支
指向同一个 commit 时（兄弟分支共用基线，很常见），一份记了那个 commit 的报告会同时被两条
改动认领。后果是它在 `accepted` 与结论三档里各记一次，总数对不上；另一条分支本该留在
「部署了没验收」里，却因为「有报告」而消失。

正解是把认领从「每条改动各自挑」改成一次全局指派：按身份强度排序（PR 号 > commit + 分支名
> commit > 分支名），一份报告只归最强的那一条。这是新引入一套指派机制，而本 PR 的对齐判据
`matchesChange` 已经因为 review 改过三轮（加 PR 号否决、剔墓碑、放弃分支不入漏斗），
按协作规则 §5.5 停止在当前 PR 继续扩张它。

**做完算数的判据**：同一项目下两条分支指向同一 commit、只有一份报告记了该 commit 时，
`funnel.accepted` 为 1，另一条分支仍出现在「部署了没验收」里。

## 「之后没有复测」这句话没有查过后来有没有复测（2026-09-15）

来源 PR #1532 第二十四轮 Codex review（P2）。结论头条的 `newFail` 支撑句直接写
「之后没有复测」，但它的依据只是那个簇本身：`buildClusters` 不把 `缺陷复测` 类报告并进
未通过簇，所以同一个验收对象在本窗内后来真的复测通过了，这句话照样会出现，把读者指回
一件已经做完的事。

正解是在出这句话之前，回头看同项目同对象在该报告之后有没有更晚的复测记录，有就改口径
（说「已复测通过」或干脆不出这条支撑句）。它要引入「跨簇按对象回查后续报告」这条新关系，
而本 PR 的簇与支撑句已经改过多轮，按 §5.5 记账不在当前 PR 展开。

**做完算数的判据**：对象 B 先有一份未通过、之后同窗有一份通过的 `缺陷复测` 报告时，
头条不再出现「之后没有复测」。

## 换对象存储桶之后，换之前的报告取不回来（2026-09-15）

来源 PR #1532 第二十五轮 Codex review（P1）。报告元数据里只存 `objectKey`，不存它当时
落在哪个 endpoint / bucket。所以一旦通过离机备份面板把 R2 指到另一个桶，换之前写的报告
会被拿新桶去找，本地读缓存一旦失效或容器重建就全变 404——正文其实好端端躺在旧桶里。

这不是热重解析引入的：换桶之后重启一次，结果一模一样。热重解析只是把它从「重启后才暴露」
变成「当场暴露」。正解是元数据记下它当时的落点（endpoint / bucket / 前缀），读的时候按
记下的落点去取；换桶时要么迁移对象，要么两边都认。这是给元数据加新字段并配存量迁移，
...
## emoji 棘轮排除了整个 U+2190-21FF，其中有几个真的是 emoji（2026-09-16，B 类）

cds 的 emoji 棘轮判定从 U+2600 起步，注释写明「刻意不含方向箭头（U+2190-21FF）……这些在本项目里大量用于日志/注释画调用链，不是 emoji」。这个判断对绝大多数
成员是对的：`←`(U+2190) `→`(U+2192) 在 Unicode 的 emoji 数据表里标着 `Emoji=No`，连表情
变体选择子都挂不上，仓库自身（含全体 Agent 规则总纲 131 处）共用了 12337 次。

漏的是同一区段里另外几个：U+2194 到 U+2199、U+21A9 与 U+21AA 标着
`Emoji=Yes`，在 Unicode 的表情变体序列表里有 `FE0F` 变体，加上变体选择子就是彩色表情。
排除口径按**区段**写，于是把这几个一并放行了——判据比它该管的范围宽（形状 1）。

**为什么不在本 PR 修**：那条棘轮属于 cds 的守卫体系，本 PR 一个字都没动它；收窄排除口径
是给它引入新的字符分类语义，按协作规则 §5.5 属于 B 类。

**要修的话**：把「排除 U+2190-21FF」改成「排除 U+2190-2193、U+219B-21A8、U+21AB-21FF」
（即只放行 `Emoji=No` 的那些），并补一条守卫断言 U+2194、U+21A9 会被判红，而 `→`、`←` 不会。

**做完算数的判据**：在 cds 的源码目录里任写一个 `U+2194 U+FE0F`，棘轮变红；写一个 `→`，仍然绿。

### 实现来源（本节）

| 位置 | 文件 |
|------|------|
| emoji 棘轮判定与基线 | `cds/tests/services/emoji-hardcode-ratchet.test.ts`、`cds/tests/services/emojiHardcodeBaseline.json` |
| 规则出处 | `AGENTS.md` §0 |

## 含单引号的 env 值，shell 写得对、Node 读不回来（2026-09-16，B 类）

CDS 的系统环境文件有两个读者：`source` 它的 shell，和 Node 启动最早期的自加载器。
写入端只有一个（安装脚本），它按 POSIX 规矩把值用单引号包住、把值里的 `'` 写成 `'\''`——
这是为了让含换行、`$`、反引号的多行 PEM 私钥被 `source` 时原样保留，判断是对的。

但 Node 那一侧的行解析正则只认 `'([^']*)'`，遇到拼接形式 `'p@ss'\''word'` 单引号分支
不命中，落到无引号兜底分支，解出的是**字面量** `'p@ss'\''word'`。实测确认：

```
原始行  : export CDS_PASSWORD='p@ss'\''word'
命中分支: 无引号兜底
解出的值: 'p@ss'\''word'
```

后果：管理员在初始化交互里输了一个带撇号的密码，`source` 这个文件起的进程用的是真密码，
而靠 Node 自加载器的直起方式（pnpm 的 serve / start 脚本）用的是带引号的另一个串——
同一份文件，两个读者读出两个值，面板登不进去且看不出为什么
（形状 3：同一判据两份实现各自漂移）。

**为什么不在本 PR 修**：这套转义与这条正则在主干上一字未改，本 PR 只是把写入端的函数体
搬进了带锁版本（为了多键一次落地），转义逐字节相同。不是本 PR 引入的回归，
按协作规则 §5.5 归 B 类。

**要修的话改读者那一侧**：让 Node 的解析认 `'...'\''...'` 拼接形式。写入端不能改用双引号，
那会把 PEM 里的 `$` 与反引号交给 shell 展开，正是当初选单引号的原因。

**做完算数的判据**：把登录密码设成 `p@ss'word`，shell 与 Node 自加载器读出同一个值；
含 `$`、反引号、换行的多行 PEM 仍然原样往返。

### 实现来源（本节）

| 位置 | 文件 |
|------|------|
| 写入端转义 | `cds/exec_cds.sh`（`env_upsert_locked` / `env_upsert_many_locked`） |
| Node 行解析 | `cds/src/services/env-file.ts`（`parseEnvFile`）、`cds/src/load-env.ts` |
## 预览实例上镜像来的分支没有门禁掉写操作（2026-09-20）

来源 PR #1553 Codex review（P2）。子预览实例把父实例的项目 / 分支 / 构建档 / 部署记录
镜像进自己的 JSON 库之后，这些分支在界面上和本地分支长得一样：删除、重部署、改环境变量
这几条写接口都能打到它们头上。写下去不会伤到父实例（子实例没有父实例的任何凭据，写只落
在自己的库里），但会让下一次镜像覆盖时把用户刚做的改动静默抹掉，用户以为「删掉了」的分支
过几分钟又回来了。

正解是给镜像分支盖一个「来源 = 镜像」的标记，写接口读到标记就拒绝并说明「这条分支只是
父实例的快照，请去父实例操作」，前端按同一个标记把动作按钮换成说明。这引入了一类新的
分支来源语义与对应的界面态，按协作规则 §5.5 记入台账，不在当前 PR 展开。
...
§5.5 熔断：不再往谓词里加输入源。现实配置（自托管的 compose 模板）把开关写在构建档 env 里，
...
## 通知历史的回溯边界与接收确认（2026-09-20）

**状态**：明确保留的边界；负责人：CDS 维护者。

通知逐次记录启用前，仅有故障时间线和通道最近一次结果，不能还原手机实际收到多少条。旧截图中的数字不得回填成发送成功记录。上线后记录发送开始、推送服务接受、失败及合并原因；重启中断的发送显示结果未确认。推送服务接受不等于手机实际展示。

查询支持最近 1 小时、24 小时和 7 天，受服务器事件存储保留期与 1000 条事件上限约束；触顶明确标记部分统计。需要更久历史或全量导出时，应先设计分页和保留预算，不可把部分记录冒充全部。

自更新补发只处理端点明确拒绝的新请求，最多 3 次、30 分钟。旧失败不根据错误文案自动补发；发送超时或重启中断导致接收结果不明时，保留待处理异常，避免重复部署。完成条件：以后需要补偿此类记录时，以部署运行编号确认去重后再扩大自动恢复范围。

2026-09-22 复发取证：9 月 21 日的一次跨分支非快进发布移除了通知修复，导致记录中断及 P95 零样本误报复发。通知修复需同步主线；普通更新保留当前提交，验证其他功能不得通过跨分支覆盖移除线上修复。被覆盖期间缺少投递事实，故障/恢复事件数不能作为手机送达数。所有自定义业务监控共用 30 分钟冷却和持续健康 10 分钟恢复规则，首次正常不推送；历史版本已发出的故障若没有成功通道记录，不补发恢复。


## 2026-09-24：监控首页改版的处置边界

| 事项 | 当前边界 | 完成条件 |
|---|---|---|
| Webhook 未解决派发 | 首页展示并解释真实异常，立即检查只复核，不会清除积压；具体受影响分支仍需进入项目部署记录核对。2026-09-24 经管理员页面确认，剩余 1 项来自 BDE（互动营销）master 的历史派发失败；项目使用组合有礼演示启动配置，main 等普通业务分支缺少演示启动脚本。当前仍保留真实告警，待确认该项目应只部署演示分支还是承载业务分支。 | 明确部署范围后修复对应构建配置或路由规则；用真实部署结果与可追溯处置记录闭环，禁止直接删除失败记录或改阈值制造恢复。 |
| 处理记录 | 展示现有故障发生与恢复账本，检查次数不等于新故障数；暂不把页面点击作为人工处置工单。 | 若需要负责人、处理备注与处置结果，应与真实部署运行记录关联，单独设计审计契约。 |
| 旧通知落点 | 新发通知携带具体检查 ID；已发到外部通知应用的旧消息无法追改，仍进入业务总览。 | 旧通知自然留存，新通知通过详情深链验证。 |
| 手机登录验证 | 本次通过已有 CDS 登录态验证桌面及窄屏路径，独立触控浏览器使用 Agent 授权验证数据显示；未取得独立 CDS 测试账号，不据此声称已验证手机密码登录全流程。 | 使用授权 CDS 测试账号在独立触控环境完成登录、导航、列表和详情检查，生成新的验收证据。 |
| 验收报告存储 | 2026-09-24 监控首页取证的 10 张截图已上传，报告正文两次归档均被服务端拒绝，原因是“离机对象大小或 checksum 与本地产物不一致”；页面发布与用户路径检查已完成，完整在线验收归档未完成。 | CDS 存储维护者核对离机对象大小及校验和，恢复报告正文入库后重新归档，并验证正文、图片和内部链接可打开。 |
