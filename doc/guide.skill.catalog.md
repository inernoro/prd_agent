...
**一句话**：核心技能索引：每个技能说清输入什么、输出什么、什么时候用。
**谁该读**：想知道该用哪个技能的人和 AI。
**读完能做什么**：按当前任务挑出对的技能；要全量清单去开发纪律总纲查。
...
> 本表为核心常用技能摘录，**不是全量清单**——完整且权威的技能列表以根 `CLAUDE.md` 的「质量保障技能链」为准（本文件可能滞后）。
...
### 文档写作技能精简（2026-04-04 合并 → 2026-05-30 再裁剪）

| 原技能 | 操作 | 结果 |
|--------|------|------|
| **doc-writer** | 保留 | `doc/` 目录命名和格式守护（元数据治理），文档写作统一入口 |
| **documentation-writer** | 已删除 | 曾合并入 technical-documentation |
| **technical-writing** | 已删除 | 曾合并入 technical-documentation |
| **user-guide-writing** | 已删除 | 曾合并入 technical-documentation |
| **technical-documentation** | 已删除（2026-05-30） | 与 doc-writer 重叠，本仓库文档统一走 `doc/` 前缀体系；通用文档写作改用 doc-writer |
...
> 2026-05-30 裁剪：`fix-unused-imports`（仅处理 TS6133，`eslint --fix` 即可替代）、`cn-brief-summary`（已并入 `dev-completion-report` Part 1）、`daily-entropy-plan`（`entropy-cleanup` + `doc-sync` 的无人值守编排壳）、`bridge`（Bridge HTTP 轮询默认关闭，验收改走 `/验收` Playwright 直连）一并下线。
