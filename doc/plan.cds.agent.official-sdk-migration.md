...
- 2026-05-18 最新 runtime pool 只读复核目录为 `/tmp/cds-agent-runtime-pool-evidence-latest`，总耗时 12s：`prd-agent` 远程 state 已无 branch-local `claude-agent-sdk-runtime-v2-prd-agent`，`shared-sidecar-pool-mp4anabh` 当前 `sharedRunning=0`，remote host `0/0`。阻塞项为 `BRANCH_LOCAL_SIDECAR_CLEAN=pass`、`REMOTE_HOST_AVAILABLE=missing`、`SHARED_POOL_RUNNING=missing`。因此当前有效顺序是：登记 remote host 并部署 shared official SDK runtime，最后重跑 MAP R0/S1/S2/S3/one-cycle；旧 `/tmp/cds-agent-cycle-*` 只能作为历史 preview alias 绕行证据，不能证明当前 shared runtime pool 已恢复。
