...
**最后更新**：2026-09-03 | **轨道**：第一期实现 | **距离可发布**：代码全部落地并在预览环境构建通过，只差真人验收那一步 —— 没跑完端到端就不算完成。
...
| P4.1 C# 单测 | 0 | 未跑 | 分支 CI 只构建镜像（`dotnet publish` 单个项目），`ci.yml` 的 `dotnet test` 只在 PR 或 main 上跑 | 开 PR 时自然会跑；在那之前不能说这批守卫测试是绿的 | 待补 |
