...
> **版本**：v0.1（草稿，待确认） | **日期**：2026-09-03 | **状态**：方案征求意见
...
**最后更新**：2026-09-03 | **轨道**：方案阶段 | **距离可发布**：五块能力里只有海鲜市场与知识库的只读部分已通，写入与三块创作类能力尚未开放，前端入口为零，不可发布。

| 阶段 | 进度% | 状态 | 当前 blocker | 下一步 | 验收证据 |
|---|---|---|---|---|---|
| P0 方案定稿 | 60 | 进行中 | 第八节四个开放问题待用户拍板 | 拿到拍板结论后冻结工具清单 | 本文档 |
| P1 授权收口 | 0 | 未开始 | 无 | scope 白名单扩容 + 用户权限交集校验 + 按 Key 配额 | 待补 |
| P2 能力开放层 | 0 | 未开始 | 依赖 P1 | 视觉创作 / 文学创作 / 网页托管 / 知识库写 四个 `/api/open/*` 薄控制器 | 待补 |
| P3 左下角入口 | 0 | 未开始 | 依赖 P1 | 侧栏账号菜单新增接入入口 + 三步向导弹窗 | 待补 |
| P4 真视觉验收 | 0 | 未开始 | 依赖 P2/P3 | 真人路径双主题取证 + 真实客户端连通 | 待补 |
| P5 OAuth 授权 | 0 | 未开始 | 无（可延后） | 标准授权流替代手工贴密钥 | 待补 |
