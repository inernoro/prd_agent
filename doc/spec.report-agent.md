...
- `full_team`：可见团队完整汇总（leader/deputy/view.all，或团队可见性为 `all_members` 的成员）
- `self_only`：仅可见本人已提交周报汇总（团队可见性 `leaders_only` 下的普通成员）

可由负责人手动触发 AI 生成团队汇总。汇总页与“团队周报列表”分离，支持显式返回列表主视图：
