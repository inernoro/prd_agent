...
- 设置团队负责人
- 查看团队成员列表（默认折叠，按需展开）
- 选择团队后显示“团队成员”按钮，点击右侧抽屉查看/管理成员
- 区分“我管理的团队”和“我加入的团队”
- 我加入的团队支持主动退出（Leader 需先移交再退出）
...
|------|-----------|-----------|-----------|
| 创建系统模板 | 是 | 否 | 否 |
| 创建团队模板 | 是 | 是（本部门） | 是（本团队） |
| 编辑模板 | 是 | 自己创建的 | 自己创建的 |
| 删除模板 | 是 | 自己创建的 | 自己创建的 |
| 绑定模板到团队 | 是 | 是（本部门） | 是（本团队） |
...
- 自动保存草稿（每 30 秒），避免丢失
- 作者在周报进入 `Reviewed` 前可继续修改和删除（含 `Submitted` 状态）
- RichText 板块支持直接粘贴图片；若图片超过 5MB，前端会自动压缩到 5MB 内再上传并插入 Markdown 图片链接
...
| `Submitted` | 已提交 | 领导查阅、退回（需填写退回原因）；作者可继续编辑、删除（审阅前） |
| `Reviewed` | 领导已查阅 | 领导可再次退回（需填写退回原因） |
...
```
┌──────────────────────────────────────────────────────────────┐
│  我管理的团队 / 我加入的团队  [团队选择] [周期] [团队成员]      │
├──────────────────────────────────────────────────────────────┤
│  ── 团队周报列表（主视图） ───────────────────────────────      │
│                                                              │
│  团队总人数 12  |  已提交 8  |  待提交 4                         │
│                                                              │
│  张三  已提交  周五 15:30   [查看]                              │
│  李四  已审阅  周五 16:02   [查看]                              │
│  王五  已查看  周六 09:12   [查看]                              │
│                                                              │
│  [进入团队周报AI分析]（独立入口，支持返回周报列表）               │
│  （成员区通过右侧抽屉查看，支持按权限查看或管理）                 │
│                                                              │
└──────────────────────────────────────────────────────────────┘
```
...
- `full_team`：可见团队完整分析（leader/deputy/view.all，或团队可见性为 `all_members` 的成员）
- `self_only`：仅可见本人已提交周报分析（团队可见性 `leaders_only` 下的普通成员）

可由负责人手动触发团队周报AI分析。分析页与“团队周报列表”分离，支持显式返回列表主视图：
...
| 我的周报页 | 员工编辑器 |
| 团队工作台 | 区分“我管理/我加入”，主视图展示团队周报列表，AI 汇总通过独立入口打开 |
