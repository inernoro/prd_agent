...
1. **三级调度/三级链路默认启用**：所有 LLM 请求必须通过统一调度算法选择模型，调度顺序为  
   **专属模型池 → 默认模型池 → 直连单模型**。  
...
### 4.23 周报管理模块（Report Agent）

> **v3.0 新增**

#### 4.23.1 REPORT-001 团队与组织模型
...
**功能详述**：
1. 模板由有序板块（Section）列表构成，每个板块定义标题、输入类型、是否必填等
2. 支持 4 种输入类型：BulletList / RichText / KeyValue / ProgressTable
3. 模板可绑定团队和岗位，按优先级匹配：团队+岗位 → 团队通用 → 系统默认
4. 创建周报时深拷贝模板结构作为快照，模板后续修改不影响已创建周报

**数据模型**：`ReportTemplate`、`ReportTemplateSection`（集合：`report_templates`）

**API 端点**：
- `GET/POST /api/report-agent/templates` — 模板列表/创建
- `GET/PUT/DELETE /api/report-agent/templates/{id}` — 模板详情/更新/删除

#### 4.23.3 REPORT-003 周报 CRUD 与状态机
