...
| v3.1 | 2026-03-04 | 新增 PRD Agent 自动技能沉淀接口：latest suggestion + confirm suggestion（会话完成后建议入库） | - |
...
#### 5.2.2.1 API-002A 自动技能沉淀建议（PRD Agent）

> 场景：用户完成一轮对话后，系统对“可复用能力”做轻量识别，并返回可确认入库的技能草稿。

```http
GET /api/prd-agent/skills/suggestions/latest?sessionId={sessionId}&assistantMessageId={messageId?}
...
Response: 200 OK
{
  ...
  "data": {
    "suggestion": {
      "suggestionId": "sugg-xxxx",
      "sessionId": "xxx",
      "sourceUserMessageId": "msg-user-xxx",
      "sourceAssistantMessageId": "msg-ai-xxx",
      "title": "PRD拆解为任务清单",
      "description": "由对话自动沉淀：将需求拆解为研发任务",
      "reason": "检测到重复/模板化表达；本轮回复已形成稳定输出结构",
      "confidence": 0.82,
      "tags": ["prd", "task", "structured"],
      "draft": {
        "title": "PRD拆解为任务清单",
        "description": "由对话自动沉淀：将需求拆解为研发任务",
        "category": "analysis",
        "input": { "contextScope": "prd", "acceptsUserInput": true, "parameters": [] },
...
```http
POST /api/prd-agent/skills/suggestions/confirm
Content-Type: application/json
Authorization: Bearer <JWT>
...
Request:
{
  "sessionId": "xxx",
  "suggestionId": "sugg-xxxx",
  "assistantMessageId": "msg-ai-xxx",
  "titleOverride": "可选-用户修订标题",
  "descriptionOverride": "可选-用户修订描述"
}
...
补充约束：
- 仅会话可访问者（个人会话 owner / 群成员）可读取与确认建议；
- 若建议变化（suggestionId 不匹配）返回冲突，前端需刷新建议；
- 若用户已有同标题个人技能，返回 `alreadyExists=true`，避免重复沉淀。
