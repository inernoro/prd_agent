...
6. 33 章每章都有操作图片；全部操作证据点都有对应的圈选、编号和中文说明。图片数量由功能点决定，不设低上限；当前仓库源包含 132 张不同图片、222 个功能证据映射，并在 253 个编号步骤及补充说明下内联 292 次图片，截图逐张读回，双主题和移动端均有证据。
...
| 工作包 | 当前事实 | 完成门 |
|---|---|---|
| 教程密度 | 原版 33 章共 243 个编号步骤，但只有 24 处图片引用、19 张不同图片，20 章没有图片 | 已完成 132 张不同图片、222 个功能证据映射；修订后的 253 个编号步骤与补充说明共 292 次内联图片 |
...
| 集中验收 | 旧报告 19 张图不能覆盖整本书的功能点 | 权威教程终验已按 104 张截图集中取证；最终交付复核又形成 113 张连续截图，仓库正文现有 222 个功能证据映射和 292 次内联，覆盖桌面主流程、移动端、双主题、权限边界、关联预览、系统运维和异常解释；Exchange 同一故事另有读回、追加前、追加后三张连续证据 |
...
| 合计 | 222 个功能证据映射 | 132 张不同图片、292 次内联；同一权威画面可服务相邻操作，但每个编号步骤下方都直接有图 |
