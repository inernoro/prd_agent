...
| 产品修复 | 进行中 | T0、T1、T2 已合并；T3 已实测发现并修复提示词字符口径含混、请求日志多记字符数、操作审计多记字符数/上限/开关、费用页术语难懂和 appCaller 心智缺失五个问题 | 以独立 T3 PR 等待 CI、CDS、公网页面和隔离数据验收 |
| 连续实测 | 进行中 | 四协议同钥、无 key 401、unknown cost 已通过；PromptPolicy 已完成 v1、v2、回滚生成 v3、当前运行态保存 v5；页面和 MongoDB 双向确认 v5 审计只含目标 id、version、policyHash；chat/vision、日志和四种费用状态定向测试 9/9 通过 | 提交独立 T3 PR，等待 CI、CDS 与公网预览验收 |
| 低理解力复验 | 基线完成 | 无背景智能体无法从公开入口完成首请求 | 教程发布后重新复验 |
| MAP 发布 | 在途总览已发布 | “模型网关权威教程”已建立唯一在途文档“LLM Gateway 权威教程：实时验收总览”，展示固定权重、PR、证据、问题和未完成项 | 持续更新同一文档；T4 后再写入 33 章正文和图片 |
...
总进度按固定权重计算，禁止凭当前动作主观报数：T0 8%、T1 14%、T2 20%、T3 20%、T4 23%、T5 15%。当前 T0 至 T2 完成，T3 阶段内完成 85%，因此总进度为 `8 + 14 + 20 + 20 × 85% = 59%`。每次更新同时记录阶段内进度、累计贡献、完成门、PR/commit、实测证据和阻塞。MAP 在途总览当前已发布快照仍为 49%；生产 MAP 改为 SSO 后，本轮自动更新被扫码登录门禁阻断，仓库计划文档先作为 59% 最新账本，SSO 会话恢复后再把同一数值同步到固定总览。

最终 CDS L2 报告与在途总览职责不同：在途总览用于实时纠偏；最终报告必须在至少 12 张有效标注截图、双主题、移动端、负向路径和低理解力复验全部完成后归档，并提供可打开的 CDS 深链与匿名分享链。
