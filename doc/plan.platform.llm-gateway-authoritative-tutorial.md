...
> **版本**：v1.2 | **日期**：2026-07-16 | **状态**：验收中（视觉增密已完成，等待集中发布终验）
...
6. 33 章每章都有操作图片；全部操作证据点都有对应的圈选、编号和中文说明。图片数量由功能点决定，不设低上限；本轮已形成 104 张不同画面、212 个功能证据映射，并在 250 个编号步骤下逐步内联 284 次图片，截图逐张读回，双主题和移动端均有证据。
...
| 工作包 | 当前事实 | 完成门 |
|---|---|---|
| 教程密度 | 原版 33 章共 243 个编号步骤，但只有 24 处图片引用、19 张不同图片，20 章没有图片 | 已完成 104 张不同截图、212 个功能证据映射；修订后的 250 个编号步骤均在步骤下方直接跟图，共 284 次内联图片 |
...
| 集中验收 | 旧报告 19 张图不能覆盖整本书的功能点 | 已按 104 张截图、212 个功能证据映射和 284 次逐步内联批量取证，覆盖桌面主流程、移动端、双主题、权限边界和异常解释 |
...
| 合计 | 212 个功能证据映射 | 104 张不同画面、284 次逐步内联；同一权威画面可服务相邻操作，但每个编号步骤下方都直接有图 |
