...
- CDS main 在本轮检查前已经通过正常网页链路创建 appCaller 和 tenant-scoped service key，并成功完成一次 OpenAI-compatible chat；随后 `/gw/overview`、`/gw/logs`、`/gw/logs/summary`、`/gw/logs/timeseries`、`/gw/pools` 与 `/gw/app-callers` 均返回非空响应。该记录保留用于测试环境刷新与页面验收，本轮不再调用付费上游。
