...
3. 概览只保留实际/期望模型、Provider、协议、传输和状态；请求区只保留 `G-${appCallerCode}`、团队、接入密钥快照、requestId、generationId、结束原因和流式状态。
4. Provider Responses 独立成节，按尝试顺序展示 Provider、模型、HTTP 状态、耗时和错误；路由 trace、费用凭证、完整身份时间、模型策略、参数策略与提示词策略进入高级折叠区。
5. 请求、响应和原始数据继续保留，但改为中文标签和 13–14px 基线；请求记录表同步使用 14px 表头与正文、最小 46px 行高，并把 App 主标识统一为 `G-${appCallerCode}`。
