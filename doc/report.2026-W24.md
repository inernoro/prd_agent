...
- **设计文档 + 债务台账**：[doc/design.knowledge-base.mention-network.md](./design.knowledge-base.mention-network.md) + [doc/debt.knowledge-base.md](./debt.knowledge-base.md)（v2 候选：跨实体引用 / AI 自动补链 / 时间轴回放）
...
|------------------------------|--------------|
| P0 CDS Agent R1 vs Lite 路线**强制决策** | 警告 部分回避。CDS 本周聚焦资源工作台/数据库工作台/SSH 发布作战台/Agent 请求观测台/forwarder 签名链/跨项目密钥事故，没有动 Agent R1 vs Lite 路线。已第四次提醒。 |
| P0 6/5-6/7 滞留上游 PR 修复 | 是 隐式落地。W23 末提到的 #722-#725 滞留 PR 本周已陆续进 main（#745 起编号连续），且 W24 56 个 PR 全部经过 merge-base ancestor 校验。 |
| P0 4 个新智能体（除 PM Agent 外）真人验收 | 否 未做。CCAS / Project Route / 个人任务树仍未跑 `create-visual-test-to-kb`。已第五次提醒，建议下周排专人 0.5 天集中跑。 |
| P1 智能体宇宙能力契约真实接入 ≥ 2 个 Agent | 否 未做。W23 #719 落地的契约本周未推广。下周建议先挑 visual-agent / literary-agent 一个接入验证。 |
| P1 PM Agent Phase 1 真人 UAT + 移动端适配 | 警告 部分。本周 PM Agent 一口气推进到 Phase 2（P0~P3），但**真人 UAT 没跑**，移动端适配未做。Phase 1 没 UAT 就跳 Phase 2 是隐性技术债。 |
| P1 知识库列表反复修改的根因复盘 | 否 未做。W23 #704→#713 一周内反向改造未做流程复盘。但本周知识库重心转向"双链 + 宇宙图"完全不同的方向，原列表布局问题暂时被掩盖。 |
| P2 CDS 日志 Mongo 后端容量与清理策略 | 否 未做。W21 起第四次提醒。 |
| P2 智能体宇宙能力契约推广到再加工 / 工作流 | 否 未做。 |
