...
python3 scripts/llmgw-readiness-audit.py \
  ...
  --run-smoke \
  --smoke-route-matrix \
  --smoke-route-pool-id <gw-pool-id> \
  --smoke-route-pinned-platform-id <platform-id> \
  --smoke-route-pinned-model-id <model-id> \
  --run-serving-probe \
  --run-cds-runtime \
  --run-shadow-coverage \
...
`/client-stream` 与 canary 必败；传 `--smoke-route-matrix` 时还会用 `/gw/v1/resolve` 验证 `auto`、`pool`、
`pinned` 三种路由语义，且 `pool` 和 `pinned` 必须给出真实 GW 池、平台与模型，否则不得计入发布证据；
...
`http-full` 的 release gate 还必须带 `--require-config-authority`，并通过 `LLMGW_CONSOLE_BASE`
加 `LLMGW_CONSOLE_TOKEN`，或 `LLMGW_CONSOLE_USER` / `LLMGW_CONSOLE_PASSWORD` 登录控制台读取
`/gw/config-authority/report`。该门要求 `status=ready`、`mapFallbackObjectsRemaining=0`、
