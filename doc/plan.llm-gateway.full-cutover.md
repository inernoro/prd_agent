...
| 阶段 | 目标 | 风险 | 关键改动 | 验证 |
|---|---|---|---|---|
| **S0.5** GW 数据域隔离 | GW 账号/审计/网关日志落 `llm_gateway` | 中 | `prd-llmgw` 账号库切 `llm_gateway`；登录审计落 `llmgw_login_audits`；serving 日志 writer 支持 GW DB；env 口令仅 bootstrap/破玻璃 | `admin/admin` 首登改密后重启不被 env 覆盖；MAP 日志仍留 MAP |
...
- GW 独立数据域部分落地：控制台账号与登录审计已切 `llm_gateway`；后续还需把 GW serving 请求日志与操作审计切入该库。
