...
> 2026-07-12 状态校正：生产执行迁移已完成，`Mode=http`、active MAP fallback 已关闭、直连棘轮 baseline 为 0、配置权威 ready，六类生产验收已通过。本文不能标为“已落地”，因为 S6 中删除 `LlmGateway.cs`、`ShadowLlmGateway.cs` 和 legacy resolver 的物理清理尚未执行；当前旧代码只作为显式回滚保险，不承载正常生产流量。
...
| 阶段 | 目标 | 风险 | 关键改动 | 验证 |
|---|---|---|---|---|
| **S0.5** GW 数据域隔离 | GW 账号/审计/网关日志落 `llm_gateway` | 中 | `llmgw/console-api` 账号库切 `llm_gateway`；登录审计落 `llmgw_login_audits`；serving 日志 writer 支持 GW DB；env 口令仅 bootstrap/破玻璃 | `admin/admin` 首登改密后重启不被 env 覆盖；MAP 日志仍留 MAP；serving 请求日志与 shadow 证据写 `llm_gateway` |
...
| scripts/llmgw-config-authority-apply.py | 配置权威退场工具 | yes | dry-run + execute 证据 | 默认只读；显式 `--execute` 才 bulk-claim/bind；`--require-ready` 失败即阻断 |
| scripts/llmgw-release-gate.py | 发布证据门 | yes | healthz + shadow 汇总；`http-full` 额外检查配置权威 | healthz 200、critical=0、httpFail=0、样本数达标；全量阶段 `mapFallbackObjectsRemaining=0` 且 `activeAppCallerMapFallbackReady=true` |
...
发布前证据门命令（S5/S6 必跑，以下命令只使用当前 main 已支持的参数；协议路由矩阵和配置权威 gate 在对应实现 PR 合入后再追加）：
...
配置权威实现 PR 合入后，`http-full` 的 release gate 还必须读取 `/gw/config-authority/report`，可通过
`LLMGW_CONSOLE_BASE` 加 `LLMGW_CONSOLE_TOKEN`，或 `LLMGW_CONSOLE_USER` / `LLMGW_CONSOLE_PASSWORD`
登录控制台。该门要求 `status=ready`、`mapFallbackObjectsRemaining=0`、
`activeAppCallerMapFallbackReady=true` 且 `activeBoundPoolWithoutUsableMember=0`；rollout ledger 会拒绝缺少该配置权威证据的 `http-full` 成功记录。
如果该门不 ready，先运行 `scripts/llmgw-config-authority-apply.py`。默认 dry-run 只读取 report 并写证据；
确认后才传 `--execute` 执行 `bulk-claim` 与 `bind-active-app-callers`，再传 `--require-ready` 固化最终报告。
该脚本是配置退场操作工具，不能替代 release gate；release gate 仍是全量 `http-full` 放行的最后门禁。
...
`open-platform-agent.proxy::chat`；`--include-open-api-chat` / `--include-open-api-image` 会分别走 OpenAI-compatible
`/api/v1/chat/completions` 与 `/api/v1/images/generations`，用于证明 `open-api.proxy::chat` 与
...
`--include-transcript-asr` / `--include-document-store-subtitle-asr` / `--include-video-to-doc-asr` /
`--include-video-to-text-asr-workflow`。视频路径通过 `/api/video-agent/videogen-direct`
...
视频 ASR 路径必须额外提供 `--asr-video-url` 或 `LLMGW_SHADOW_ASR_VIDEO_URL`：`--include-video-to-doc-asr`
通过 `/api/video-agent/v2d/runs` 等待后台 VideoToDoc worker 完成，覆盖
`video-agent.v2d.transcribe::asr:raw`；`--include-video-to-text-asr-workflow` 会创建临时 workflow，
通过 `video-to-text` capsule 的 ASR 模式等待 WorkflowRunWorker 完成，覆盖
...
LLMGW_STAGE_SHADOW_SEED_FLAGS="--include-image-worker-text2img --include-image-worker-img2img --include-image-worker-vision --include-video-direct --include-visual-video-direct --include-transcript-asr --include-document-store-subtitle-asr --include-video-to-doc-asr --include-video-to-text-asr-workflow --asr-video-url <reachable-mp4-url> --summary-poll-seconds 240" \
...
调用 `scripts/llmgw-rollout-ledger.py audit --require-target-success` 生成 `stage-audit.json/md`。`config-authority`
阶段排在 `rollback-rehearsal` 之后、所有 canary 之前；它不执行 `fast.sh` / `exec_dep.sh`，会先运行
`scripts/llmgw-config-authority-backup.sh` 在生产机本地备份 `llm_gateway` 全库与 MAP 模型配置关键集合，
再调用 `scripts/llmgw-config-authority-apply.py --execute --require-ready`。台账会强制读取
`*.config-authority-backup.json/md` 与 `*.config-authority.json/md`，其中备份证据必须是非 dry-run、包含
`backupDir`、`archiveCount>0` 与 `sha256Sums`；workflow 需配置 `vars.LLMGW_PROD_CONSOLE_BASE` 或从 `map_base`
推导 `/gw`，并提供 `secrets.LLMGW_PROD_CONSOLE_TOKEN` 或 `LLMGW_PROD_CONSOLE_USER/PASSWORD`。这样每个
shadow/canary/config-authority/http/rollback 阶段都留下可下载、可复核、无密钥的 release-gate、serving-probe、
gw-smoke、config-authority、config-authority-backup、stage report 与 rollout ledger 证据包，避免证据只存在某台生产机器本地，也避免 checkout 清理未跟踪目录后丢失前序台账。
...
支持阶段：`shadow-start`、`rollback-rehearsal`、`config-authority`、`canary-intent-text`、`canary-chat`、`canary-streaming`、`canary-vision`、
...
`*.upstream-readiness.*`（video/ASR/full 阶段的 resolve 配置门）、`*.config-authority-backup.*`（配置权威写库前备份证据）、
`*.config-authority.*`（配置权威退场证据）、
...
stage/serving-probe/gw-smoke 证据；`config-authority` 阶段必须先读到 verdict=pass、非 dry-run、`archiveCount>0`
且带 `sha256Sums` 的 `externalBackupJson`，再读到 verdict=pass 且 `execute=true` 的
`configAuthorityJson`，其最终 report 必须 `status=ready`、`mapFallbackObjectsRemaining=0`、
`activeAppCallerMapFallbackReady=true` 且 `activeBoundPoolWithoutUsableMember=0`；canary/http 阶段还必须读到 verdict=pass 的 release-gate 证据。把
`config-authority` 放在 canary 前面，是为了让后续灰度验证全部基于 GW 权威模型池，而不是继续借 MAP 配置兜底。
`config-authority` 是配置数据迁移阶段，不切用户流量，因此不消耗也不制造 24 小时观察窗口；后续 canary 的
`--min-observation-hours` 仍按上一个真实流量阶段计算。
...
做 `mongodump` 备份，再把现有豆包 ASR exchange 绑定成指定 ASR pool，并覆盖四个 ASR
caller（文档字幕、转录工作台、视频转文档、视频转文字）。默认 `LLMGW_ASR_BOOTSTRAP_MODE=bigmodel` 使用
`asr_doubao_bigmodel_pool` / `doubao-asr-bigmodel` / `doubao-asr`；若生产 BigModel 凭据被上游拒绝，可先用
`LLMGW_ASR_BOOTSTRAP_MODE=stream` dry-run 候选 `asr_doubao_stream_pool` / `doubao-asr-stream` /
`doubao-asr-stream`，再用 provider audit 的 `--asr-pool-id`、`--asr-model-id`、`--asr-transformer`
参数只读审计。真实写库前仍必须有外置或本地备份和磁盘空间门通过；写库后必须跑 ASR raw seed 证明上游可用。
为避免候选池准备阶段误切生产 ASR caller，可先用
`LLMGW_ASR_BOOTSTRAP_MODE=stream LLMGW_ASR_BOOTSTRAP_BIND_CALLERS=0 LLMGW_ASR_BOOTSTRAP_DEFAULT_FOR_TYPE=0`
只创建 stream pool；只有真实 seed/审计证明可用后，才允许把 `BIND_CALLERS=1` 作为切换动作。
它只能修复“无 ASR 模型池 / caller 未绑定 / 选择错误 ASR exchange”的配置缺口；
如果豆包 key 或 resourceId 不可用，真实 `llmgw-map-shadow-seed.py --include-*-asr` 仍必须失败并阻断发布。
`scripts/llmgw-prod-video-caller-bootstrap.sh` 是生产视觉视频 caller 绑定脚本：默认 dry-run；执行模式会先备份
`prdagent`，再把 `visual-agent.videogen::video-gen` 绑定到 `video-agent.videogen::video-gen` 已使用的 video-gen
模型池。它只能修复“视觉视频入口未绑定 video-gen 池”的配置缺口，不能把不可用视频上游变成可用；执行后仍必须跑
provider config audit、upstream readiness 和真实视频 raw seed，证明 video-gen 模型健康且 `httpFail=0`。
...
`scripts/llmgw-prod-external-backup.sh` 是外置流式备份脚本，应在清理生产 `/root/backups` 或任何写库动作前从可信工作站运行：
默认通过 SSH 从生产 `mongodump` 流式拉取 `prdagent` 与 `llm_gateway`，在本地压缩、`gzip -t` 校验并写
`SHA256SUMS`，不会在生产根盘创建大 archive。默认只保存 redacted env；确需完整 `.env` 时必须显式设置
`LLMGW_EXTERNAL_BACKUP_INCLUDE_SECRETS=1`，并确保本地目录权限和密钥保管到位。远端旧备份删除不属于脚本自动动作；
必须先确认外置备份 SHA 与恢复责任人后再人工清理。
全量模式 `LLMGW_EXTERNAL_BACKUP_MODE=full` 会备份完整 `prdagent`，包含体量很大的 `apirequestlogs`，适合异步归档；
发布解阻优先用 `LLMGW_EXTERNAL_BACKUP_MODE=critical`，只备份模型池、平台、appCaller、MAP LLM 日志与完整
`llm_gateway`，用于保护网关迁移相关状态并避免历史 API 日志拖住根盘清理。
2026-07-07 生产取证确认：补池后四个 ASR caller 已可 resolve，但真实 `transcript-agent` 与 `document-store`
ASR seed 均被上游拒绝，错误为 `Invalid X-Api-Key`；因此 `canary-video-asr` 仍不得执行。生产库同时存在
...
http/canary 发布默认还会在部署后强制运行 `scripts/gw-smoke.py`，默认只真打低成本 chat 的
healthz/pools/invoke/send-compat/stream/client-stream/canary；
...
intent/vision/image/asr 等非 chat 路径必须由阶段 gate、shadow 样本或显式 `GW_SMOKE_MODEL_TYPES` /
`GW_SMOKE_APP_CALLERS` 打开，避免默认发布校验造成过量真实模型请求；
...
rollout ledger 会把 `smokeRouteMatrixRequired=true` 写进 stage report，并在后续 audit 中强制校验
`gw-smoke.json` 里 `route-auto`、`route-pool`、`route-pinned` 三行均为 pass 且不能是 skipped。
这组证据只证明 router 解析语义，不写 LLM 请求日志，不计入 `appcaller_runtime_coverage`、
`current_commit_http_transport` 或参数丢弃运行态 gate；这些 gate 必须由真实 send/stream/raw 日志或 shadow comparison 证明。
同时默认在部署后强制运行 `scripts/llmgw-serving-probe.py`，连续检查 serving healthz commit 稳定性与无 key 访问 401；
stage runner 默认设置 `LLMGW_SERVING_PROBE_REQUIRE_ROUTE_SELF_TEST=1`，要求部署后同一 serving 再用 gateway key
执行 `/gw/v1/route-self-test`，并把 `routeSelfTest` 写进 serving-probe 证据；rollout ledger 会拒绝缺少或失败的
部署后 `routeSelfTest`，防止只凭发布前 preflight 证明协议入口未漂移；
...
`open-api.proxy::chat`、`open-api.proxy::generation`、
