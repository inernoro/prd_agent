...
该脚本默认只跑文档 session chat 与 preview-ask 两条低成本文本路径；图片、视频、ASR/raw 证据仍必须通过对应真实业务
入口或后续显式 raw seed 开关产生，不能用文本样本替代 raw gate。脚本执行后会读取 `/gw/v1/shadow-comparisons`
输出 global/send/stream 覆盖摘要，便于证据期连续采样。
