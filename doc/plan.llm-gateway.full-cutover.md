...
`activeAppCallerMapFallbackReady=true`；rollout ledger 会拒绝缺少该配置权威证据的 `http-full` 成功记录。
...
`activeAppCallerMapFallbackReady=true`；canary/http 阶段还必须读到 verdict=pass 的 release-gate 证据。把
