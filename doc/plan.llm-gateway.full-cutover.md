...
python3 scripts/llmgw-release-gate.py --min-total 30 \
  ...
  --json-out /tmp/llmgw-release-gate.json \
  --report-md /tmp/llmgw-release-gate.md
```
...
`LLMGW_MODE=shadow|inproc` 不挡发布，便于先以 shadow 积累证据。全量 `http` 时 `exec_dep.sh` 默认
用 `LLMGW_GATE_HEALTH_SAMPLES=3` 和 `LLMGW_GATE_HEALTH_INTERVAL_SECONDS=5` 连续采样 healthz，任一
