...
外部团队最容易卡死的不是技术栈，是这个环：**没有密钥调不了接口，没有项目又签不出密钥。** 这条链路是专门为解开它设计的，三个断点各有各的解法，缺一个环就重新闭合。

### 7.1 一条链走到底

| # | 谁在动 | 做什么 | 此刻手里有什么 |
|---|---|---|---|
| 1 | 用户 | 在 CDS 页面点「接入 Agent」，选接入目标（新建项目 / 已有项目），复制口令给 Agent | 一段不含密钥的文字 |
| 2 | Agent | 按口令匿名下载 CDS 技能包，装进项目级技能目录 | 有了 CLI，仍然没有凭据 |
| 3 | Agent | 发起一次「新项目接入申请」，然后轮询等待 | 一个申请号 + 一张只有它自己有的轮询票据 |
| 4 | **人** | 在 CDS 右下角看到申请，点批准 | 信任在这里产生：批的是人，不是密钥 |
| 5 | Agent | 轮询取回一次性授权（明文只交付一次） | 一把只能建项目、不能碰任何现存项目的钥匙 |
| 6 | Agent | 用它创建项目 | 项目有了 |
| 7 | CDS | 建成的同时签发该项目的长期项目级凭据，并立刻吊销那把一次性钥匙 | 长期凭据落到项目本地、且被 Git 忽略 |
| 8 | Agent | 用项目级凭据部署、取真实预览地址 | 环解开了 |

### 7.2 三个断点与各自的守卫

| 断点 | 不处理会怎样 | 解法 | 守卫 |
|---|---|---|---|
| 装 CLI 需要凭据 | 客户还没有任何凭据，下载技能包就 401，卡在第零步 | 技能包端点匿名（要登录的那个导出端点只作已登录用户的备选） | 技能安装契约测试 + 上游代理测试；口令生成用例断言口令写的是匿名端点 |
| 调接口需要密钥 | 申请授权本身也要密钥，死循环 | 「发起申请」「轮询结果」两条免密，信任锚点换成人在页面点一下 | 服务端把这两条显式登记为公开路由；审批端点只认登录的人，机器凭据一律拒绝 |
| 建完项目用不了它 | 一次性钥匙作用域为空，建完还是碰不到自己刚建的项目 | 建项目时当场签发该项目凭据，并吊销一次性钥匙 | 建项目路由的一次性钥匙回收逻辑 + 全局密钥测试 |

### 7.3 2026-08-25 复发点：链建好了，入口没接上

后端与 CLI 三个断点都解开之后，仍然有用户在「上手助手」上撞回原地。原因不在链路，在**入口**：

- 弹窗从项目列表打开时，接入目标默认落到「系统任务」这一档；
- 系统任务分支的口令明确写着不创建项目、不申请项目授权——于是零基础用户复制到的口令，恰好缺了解环的那一步；
- 目标选择的按钮此前只在「自动接入」页签出现，上手助手上既看不见也改不了。

这是典型的「能力建好了但没人调用」：删掉这段接线，所有测试照样绿。**修法**是把目标解析收敛成一个函数（上手助手不允许落到系统任务档），并让目标选择在该页签同样可见；口令里的技能包地址同时改指匿名端点。**判据**：上手助手拿到的目标必须能生成带「新项目授权」的口令，用例直接断言这件事，而不是扫源码。

### 7.4 换钥这一步必须是代码级的，不能交给模型接力

链路里最脆的一步不是申请，是**换钥**：后端一旦签发项目级钥匙就立刻吊销那把一次性钥匙，而明文只发一次，错过不可逆。所以它不能是「提示词告诉模型：建完项目记得把返回的钥匙存下来」——模型少跑一步、跑错参数、或在两条命令之间改主意，项目就会建出来而没人有钥匙。

现在的约定：

- **建项目只有一个实现**，换钥、用新钥匙回读自证、断言本地不再持有一次性钥匙，全在那一处完成；任何新的建项目入口都必须走它。此前 `onboard` 自己实现了一遍并把返回的钥匙丢掉，走那条路的 Agent 项目建好、钥匙当场作废，下一步直接 401——同一件事有两份实现就会这样。
- **接入与建项目压成一条命令**：批准之后的「拿一次性钥匙 → 建项目 → 换成项目级钥匙 → 回读自证」在同一个进程里跑完，那把一次性钥匙不在模型手里停留。口令里给的就是这一条命令。
- **拿不到新钥匙必须显式失败**，并把已经建出来的项目 id 告诉人，让人去项目卡补签一把；不许静默继续跑出一串 401。

### 7.4 以后怎么防它再闭合

新增任何接入入口（新页签、新向导、新落地页）时，逐条自问：

1. 这个入口生成的指引，**在用户一把密钥都没有时**能不能走通第一步？
2. 它给出的接入目标是不是真实可执行的那一档，而不是「系统任务」这类占位？
3. 需要人授权的那一步，是不是明确告诉用户「去哪点、点什么」，而不是让 Agent 干等？
4. 一次性权限用完之后，有没有换成长期凭据并立即回收？
5. 上面四条各自有没有一条会红的用例——删掉接线之后测试仍然全绿，就说明还没有。
...
| 位置 | 文件 | 作用 |
|------|------|------|
| 十、关联 | `cds/web/src/components/SkillDownloadDialog.tsx` | 接入智能体弹窗 |
