...
> **版本**：v1.2 | **日期**：2026-05-18 | **状态**：已落地

**一句话**：Agent 工作台该怎么测才不算假通过：分层测试矩阵、最小自动化套件与手工视觉标准。
**谁该读**：接手这块测试的下一位工程师或智能体。
**读完能做什么**：按矩阵铺测试，并判断一次通过是真通过还是假通过。
...
第 6 个问题也必须回答：文档和页面有没有夸大官方 SDK 接入程度。当前 `claude-sdk` 是历史 runtime 名；新的默认代码审查路径必须看 `runtimeAdapter=claude-agent-sdk`、`loopOwner=claude-agent-sdk` 和 A0 boundary smoke。旧 A10 复盘里的 legacy sidecar 证据不能替代当前官方 SDK adapter 的 R1/S1/S2/S3 商业级证据。
...
官方 SDK adapter 迁移只应该改变代码型远程 runtime，不应该把所有智能体都塞进 CDS Agent。

| Agent | 当前 runtime | 是否应走官方 Claude Agent SDK adapter | 兼容性风险 | 最小回归 |
| --- | --- | --- | --- | --- |
| `cds-agent` | MAP/CDS session + sidecar runtime | 是，目标主路径 | sidecar pool 发现、官方 SDK 包、MAP approval、cancel | 创建句柄、事件流、审批、停止、PR |
| `prd-agent` | Toolbox adapter + 现有 LLM/业务服务 | 否，除非未来需要仓库工具 | 不应被 sidecar pool 不可用阻断 | PRD 分析最小输入返回 artifact |
| `defect-agent` | Toolbox adapter + 现有 LLM/业务服务 | 否 | 不应依赖 CDS runtime profile | 缺陷提取/分类最小输入 |
| `literary-agent` | Toolbox adapter + 现有 LLM/业务服务 | 否 | 不应继承代码工具审批策略 | 文案生成最小输入 |
| `visual-agent` | Toolbox adapter + 图像/视觉服务 | 否 | 不应被代码 runtime/cancel 语义影响 | 视觉 prompt 生成或图片任务最小路径 |
| `report-agent` | 独立报表/文档链路 | 否 | 不应被 CDS Agent migration 改路由 | 报表任务列表/最小生成入口 |

当前远程 preview 的真实阻塞仍是 `cds-agent` 专属，但最新 blocker 已回到 R0 runtime pool 结构层：`prd-agent` 业务项目仍有 branch-local `claude-agent-sdk-runtime-v2-prd-agent` 残留，`shared-sidecar-pool-mp4anabh` 没有 running instance，CDS remote host 列表为空。A0 official SDK boundary 本地证据仍有效；R1 默认 runtime profile 仍需要后续 Anthropic/Claude-compatible profile 证据，但只有 R0 shared runtime pool 恢复后，R1/S1/S2/S3 才有意义。非代码智能体的兼容性 smoke 仍要跑，但它们不应该因为 CDS Agent 的 R0/R1/provider gate 暂未关闭而失败。若它们失败，应按各自服务链路排查，而不是把问题归因到官方 SDK adapter。

自动化护栏：`CdsAgentRuntimeCompatibilityTests` 会扫描 Toolbox adapter 源码，要求
`IInfraAgentRuntimeAdapter` / `IClaudeSidecarRouter` / `InfraAgentRuntimes` 只出现在
`CdsAgentAdapter`。如果 PRD/缺陷/文学/视觉 adapter 误接入 CDS sidecar runtime，
该测试必须失败；只有确有代码仓库执行需求时，才应先更新本表和设计文档，再调整测试白名单。
