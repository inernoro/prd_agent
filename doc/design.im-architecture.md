...
> **版本**：v1.0 | **日期**：2026-02-06 | **状态**：已废弃

> ⚠️ 本方案已被 Run/Worker + afterSeq 方案替代，保留供架构参考。

---

## 管理摘要

- **解决什么问题**：当前 IM 架构采用 N 条短生命周期 SSE 连接 + 单槽位状态 + 每端独立实现，导致连接慢、切换卡顿、无法水平扩展
- **方案概述**：改为 1 条长生命周期 WebSocket 连接 + 多槽位状态 + 桌面/Web 共享协议层的新架构
- **业务价值**：群组切换零延迟、断线自动重连、桌面/Web 代码复用、支持多实例水平扩展
- **影响范围**：prd-desktop（Rust SSE 代理重写）、prd-admin（SSE 改 WebSocket）、prd-api（新增 RealtimeHub）
- **预计风险**：高 — 涉及三端通信层重构，本方案已被 Run/Worker + afterSeq 方案替代
