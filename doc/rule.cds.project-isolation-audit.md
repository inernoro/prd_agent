...
| 审计维度 | 旧矩阵 | 应有 |
|---------|--------|------|
| **文件读取向量** | 否 未覆盖 | 每个读文件操作的根目录是否隔离到项目 repo root |
| **入口点完整性** | 否 未覆盖 | 所有写数据的入口点（不只是 /import-config）是否都被审计 |
| **共享资源污染** | 否 未覆盖 | globalEnv、infra、proxy.ts 等跨项目共享的对象是否有越界写 |
| **紧急修复后的数据完整性** | 否 未覆盖 | 手工修复 API 后是否验证了 env/dependsOn 的完整性 |
