...
### 证据图与知识库查找（2026-09-07 日报事故留尾）

背景：2026-09-07 日报的「视察评审」附加章节把证据图占位放在了正文样例容器之外，四张截图按
2880x1800 的原始像素平铺，撑破了页面（用户反馈「截图尺寸过大」）。已做的修复：两份刊物模板的
证据图尺寸规则改为无作用域；发布脚本新增「占位不许塞进标签属性」与「模板必须有无作用域尺寸规则」
两道闸；新增一条 CI 守卫跑真展开函数校验。原则已写进刊系设计规则 §1.6 与判据纪律的形状 9。
以下为主动收窄未做的边界：

| ID | 说明 | 优先级 | 触发条件 | 状态 |
|---|---|---|---|---|
| EVF-1 | 截图体积只告警不拒发：发布脚本对单张超过 1.5MB 的图告警，但取证脚本忘了改成 1x 采集时，一篇日报仍可能嵌进 6MB 图。根治是上传前按版心宽度（1440px）降采样，需引入图像库并确认 CI 与发布环境都装得上 | P2 | 分享链首屏加载明显变慢 / 知识库存储告警 | open |
| EVF-2 | 图说只能来自截图清单的 caption，正文里无法就地改图说；想写更长的图注只能改清单再重发。可考虑让占位携带图说、由正文覆盖 | P3 | 有人第二次抱怨「改个图说要重跑取证」 | open |
| KBS-1 | 「我的空间」下存在两个同名「日报知识库」（一个在写入，另一个停更于 08-16 且被置顶）。水位线读取与发布都按名字取列表第一个匹配，今天落对纯属返回顺序凑巧；顺序一变就会读错水位线、发错库，且不报错。根治两步：脚本侧改为「同名多库时按最近日报条目选并打印告警」；产品侧知识库创建加同名校验（见该日日报改进方向第一波） | P1 | 下一次日报发进了停更库 / 水位线回退重报 | open |

#### 实现来源

- 发布脚本与两道闸：`.claude/skills/daily-report-summary/reference/publish.py`
- 水位线读取（同样按名找库）：`.claude/skills/daily-report-summary/reference/coverage_window.py`
- CI 守卫：`scripts/tests/test_report_evidence_figure.py`
- 原则：`.claude/rules/report-design-system.md` §1.6、`.claude/rules/predicate-and-wiring-discipline.md` 形状 9
