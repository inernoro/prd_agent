...
> **版本**：v1.0 | **日期**：2026-03-19 | **状态**：已实现
>
> 关联文档：`design.platform.model-pool.md`（三级调度基础架构）

## 一、管理摘要

- **解决什么问题**：模型被标记为不可用后无法自动恢复（死锁），全池耗尽时用户无感知，配置降级链操作繁琐
- **方案概述**：后台探活服务周期性检测不健康端点并自动恢复，全池耗尽时主动通知管理员和用户，新增快捷配置 API 一步创建带降级的模型池
- **业务价值**：消除人工干预恢复模型的运维负担，故障时用户及时获知状态，降低模型池配置门槛
- **影响范围**：prd-api ModelPool 模块、AdminNotification 通知系统、prd-admin 日志过滤
- **预计风险**：中 — 探活请求消耗 API 额度，需合理控制冷却期和并发数
