...
| **Web管理后台** | React 18 + TypeScript 5 + Vite + Radix UI + Tailwind CSS + Zustand |
...
| `get_config` | - | `Config` | 获取本地配置 |
| `save_config` | `config` | void | 保存本地配置 |
...
| `run-event` | `{ runId, seq, event }` | Run/Worker 事件（支持 afterSeq 续传） |
...
**Admin API 路径**：`/api/{module}`（禁止使用 `/v1/` 版本号）

**常用接口**：
- `GET /api/users` - 用户列表（分页、筛选）
- `POST /api/users` - 创建用户/生成邀请码
- `GET /api/mds/platforms` - 获取 LLM 平台列表
- `GET /api/mds/models` - 获取模型配置列表（API Key 脱敏显示）
- `POST /api/mds/models` - 创建模型配置
- `GET /api/dashboard/stats` - 仪表盘统计
- `GET /api/llm-logs` - LLM 请求日志（列表/筛选）
- `GET /api/llm-logs/{id}` - LLM 请求日志（详情）
- `GET /api/authz/permissions` - 权限列表
...
| API路径 | kebab-case | `/api/content-gaps`、`/sessions` |
