...
|------|------|
| 1.why.md | 项目背景与设计思路 |
| doc/2.srs.md | 软件需求规格说明书（需求基线，按 ISO/IEC/IEEE 29148:2018 组织） |
| 3.prd.md | 产品需求文档（功能需求） |

补充说明：本开发文档用于描述架构、技术选型、项目结构、构建部署与开发实践；不替代 SRS。SRS 仅作为需求基线与验收依据。
...
4. **克隆项目** → `cd PrdAgent.Server && dotnet restore`
5. **配置环境变量**（见附录11.1）
...
3. **安装依赖** → `cd prd-agent-desktop && pnpm install`
4. **配置后端地址** → 环境变量 `VITE_API_BASE_URL`
5. **启动开发** → `pnpm tauri dev`

### 3.3 Web管理后台环境

1. **安装依赖** → `cd prd-agent-admin && pnpm install`
2. **配置环境** → 复制 `.env.example` 为 `.env.local`，设置 API 地址
3. **启动开发** → `pnpm dev`
4. 服务地址：`http://localhost:5173`
...
| 大模型配置 | `/llm-config` | 服务商切换、API Key配置（加密存储、界面脱敏）、模型参数、速率限制 |
| 请求日志 | `/logs` | LLM请求日志查看、筛选、导出（脱敏） |
...
- `GET /admin/llm-config` - 获取大模型配置（API Key脱敏显示）
- `PUT /admin/llm-config` - 更新大模型配置（API Key加密存储）
