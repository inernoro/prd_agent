...
| **Phase 1.7** | 自适应大/小标题识别：前端扫全文取 `Math.min(headings)` 当大标题、比它更深的当小标题；LLM prompt 同步改为"以本文实际用法为准"。原理已归档至 `design.literary-agent.md` | ✅ 已完成（2026-04-20） | 同上文件 + `design.literary-agent.md` 新增章节 |
