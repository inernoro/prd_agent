...
#### 5. 智能体知识库
- [ ] 向导第 2 步"测试调优"右侧能看到"知识库"上传区（非"即将上线"占位）
- [ ] 能上传文档，列表显示文件名 + 大小，可移除
- [ ] 上传中"创建智能体"按钮禁用并显示"上传中…"
- [ ] 测试对话中提问能引用所传文档内容（保存前即可验证）
- [ ] 完整模式编辑器"能力增强"Tab 能看到已绑定文档并可增删
- [ ] 切换模板/空白重建后，先前上传的文件被清空（不串到新智能体）
...
让智能体基于你上传的私有文档回答（文档全文会注入对话上下文）。两个入口都可用：

**入口 A — 快速创建向导**
1. 进入"创建智能体"，选模板或空白
2. 第 1 步填名称/提示词，第 2 步进入"测试调优"
3. 右侧"知识库"区块点击上传区，选择文档（支持 PDF、Word、Excel、PPT、TXT 等）
4. 文件出现在列表（上传中显示转圈，完成显示文件名 + 大小；可点 X 移除）
5. 上传未完成时"创建智能体"按钮显示"上传中…"并禁用，等完成后再保存
6. 保存前可直接在"测试对话"里提问验证：智能体应能引用文档内容作答

**入口 B — 完整模式编辑器**
1. 在向导点"完整模式"，或在工具列表对自定义智能体点"编辑"
2. 切到"能力增强"Tab，找到"知识库"区块
...
| 2026-05-22 | v0.2 | 新增"为智能体配置知识库"操作说明 + 验收清单项（PR #652 文档级知识库落地） |
