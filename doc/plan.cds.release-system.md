...
## 十三、字距与配色回滚，控制台状态修正（2026-08-15）

十二节的视觉基线换代之后，当天又做了几轮修正与一次部分回滚：

- **字距修复**：正文英文收紧字距后，等宽 advance 让中文串里夹的数字两侧空隙翻倍，随后修复。
- **可读性守卫**：白天主色文字统一走 `--primary-ink`，此前浅底配亮绿字实测对比度仅 1.22:1、读不出来；新增调色板守卫测试，禁止再用 `--primary` 直接当文字色或硬编码调色板状态色，四档语义状态色双主题必须齐。
- **项目卡派生标签**：发布控制台项目栏的项目卡新增标签（推送即部署 / 共享服务 / 已暂停 / 克隆失败 / N 个在跑），全部从既有字段推导，不增加新的维护成本。
- **控制台状态修正**：发布控制台区分「历史记录」与「本次操作」，从发布中心跳入时不再把上一次发布渲染成刚刚发生；版本下拉标出已在线上的那一版，选中线上版本时明说再发一次不会改变线上内容。
- **配色与字体部分回滚**：全局配色回滚到十二节改动前的暖橙配色（语义 token 层保留、值改回旧观感）；正文字体回归比例字（自托管 Inter），等宽字体收窄为只用于标识符（分支名、commit sha 等），修复正文使用等宽字体导致页面横向撑大约 10%、以及负字距把中文一起收紧的观感问题；白天主色文字在此基础上进一步压深两档，解决部分按钮看不清的问题。
