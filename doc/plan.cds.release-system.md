...
**不做多环境晋级链。** `ReleaseTarget.environment`目前是死数据，UI 写死 `'production'`。在阶段四的不可变产物落地之前，"同一产物晋级到下一环境"根本无法保证是同一产物，做出来是假的。等阶段四之后再评估。
...
- **自动发布（定时任务）**：基于 scheduled-job 的发布规则支持增删改查、启停、立即试跑；支持"需要人工确认"（到点只跑发布前检查、生成待确认站内信，绝不自动发布）、连续失败 2 次自动停用并告警、失败自动回滚、目标版本未变或目标忙时按并发策略跳过。**并发闸不变量**：HTTP 发布路由与定时任务调度器必须共用同一个 `ReleaseService` 实例，绝不能各自 `new` 一个——`inFlight`（在途发布）表是实例私有的，两个实例各持一张互不可见的表，settling 判定会当场失明，两条并发闸都会误判「空闲」而同时放行同一目标（此前路由侧确实自建过第二个实例，现已改为共用并由专门的单实例守卫测试钉住）。
...
- **新建服务器就地化**：加环境向导内嵌新建服务器表单（密钥对 / 粘贴私钥 / 用户名密码三种接法），不再把用户支到系统设置再走回来。两条安全不变量：密码认证主机对外暴露的公开标识是与口令无关的随机串（不是口令派生的哈希，避免公开接口返回的指纹被离线撞库还原口令）；SSH 执行失败摘要与发布日志共用同一个脱敏器，统一盖住 Authorization 头 / JSON 口令字段 / URL userinfo 三类格式，避免凭据经由失败信息展示在发布中心。
