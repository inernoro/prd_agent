...
- **自动发布（定时任务）**：基于 scheduled-job 的发布规则支持增删改查、启停、立即试跑；支持"需要人工确认"（到点只跑发布前检查、生成待确认站内信，绝不自动发布）、连续失败 2 次自动停用并告警、失败自动回滚、目标版本未变或目标忙时按并发策略跳过。**并发闸不变量**：HTTP 发布路由与定时任务调度器必须共用同一个 `ReleaseService` 实例，绝不能各自 `new` 一个——`inFlight`（在途发布）表是实例私有的，两个实例各持一张互不可见的表，settling 判定会当场失明，两条并发闸都会误判「空闲」而同时放行同一目标（此前路由侧确实自建过第二个实例，现已改为共用并由专门的单实例守卫测试钉住）。**版本钳制不变量**："需要人工确认"的站内信必须钉死那次过检的具体 commit（不是"当前最新"）：分支在等待人确认期间可能继续推进，通知深链带着过检时的 commit 一路透传到发布确认弹窗的 `expectedCommitSha`，人几小时后点进来发布的仍是被检查过的那一版，而不是分支当下的最新版。**编辑不变量**：`PATCH /api/scheduled-jobs/:id` 把 `actions` 当权威全量覆盖，而一个任务可能同时挂着 HTTP 回调、命令、发往其他环境的 release 等兄弟动作；编辑自动发布规则必须只替换本目标匹配的那条 release 动作、原样保留其余动作，不能直接提交单元素数组把兄弟动作静默删光。**人工确认失败计数不变量**："需要人工确认"的规则若预检没过，必须记成 `failed`、不能记成 `skipped`——跳过会顺带清零连续失败计数，一条每次都过不了预检的规则就永远够不到"连续失败 2 次自动停用"的阈值，坏着坏着就没人管；`skipped` 只留给"候选确实已就绪、只差人点头"这一种情况。
