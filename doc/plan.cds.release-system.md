...
- **自动发布（定时任务）**：基于 scheduled-job 的发布规则支持增删改查、启停、立即试跑；支持"需要人工确认"（到点只跑发布前检查、生成待确认站内信，绝不自动发布）、连续失败 2 次自动停用并告警、失败自动回滚、目标版本未变或目标忙时按并发策略跳过。**并发闸不变量**：HTTP 发布路由与定时任务调度器必须共用同一个 `ReleaseService` 实例，绝不能各自 `new` 一个——`inFlight`（在途发布）表是实例私有的，两个实例各持一张互不可见的表，settling 判定会当场失明，两条并发闸都会误判「空闲」而同时放行同一目标（此前路由侧确实自建过第二个实例，现已改为共用并由专门的单实例守卫测试钉住）。**版本钳制不变量**："需要人工确认"的站内信必须钉死那次过检的具体 commit（不是"当前最新"）：分支在等待人确认期间可能继续推进，通知深链带着过检时的 commit 一路透传到发布确认弹窗的 `expectedCommitSha`，人几小时后点进来发布的仍是被检查过的那一版，而不是分支当下的最新版。**编辑不变量**：`PATCH /api/scheduled-jobs/:id` 把 `actions` 当权威全量覆盖，而一个任务可能同时挂着 HTTP 回调、命令、发往其他环境的 release 等兄弟动作；编辑自动发布规则必须只替换本目标匹配的那条 release 动作、原样保留其余动作，不能直接提交单元素数组把兄弟动作静默删光。**人工确认失败计数不变量**：`skipped` 在规则禁用、并发中、目标版本未变、目标忙等场景都是合法结果，但"需要人工确认"分支是唯一例外——预检没过时必须记成 `failed`、不能顺着别的分支记成 `skipped`，否则会清零连续失败计数，一条每次都过不了预检的规则永远够不到"连续失败 2 次自动停用"的阈值，坏着坏着就没人管；`skipped` 只在该规则"候选确实已就绪、只差人点头"时使用。
...
## 十一、发布控制台落地 + 事件驱动自动发布规则（2026-08-12 ~ 2026-08-14）

在 v2 界面重做之后，发布中心又做了一轮更大的结构调整：从"先选目标再看它的六个页签"（控制台视角）换成两个各司其职的落地页。

- **发布控制台**（`/release-console`，左项目环境 / 中状态与实时输出 / 右历史与失败与只读发布配置）取代原「Releases」的入口地位，成为日常发布时的默认落地页；「试跑」按钮删除，发布前检查并入「开始发布」的第一步，有阻断项就摊开原因拦在发布之前，不再让人误以为不点试跑就不检查。
- **发布中心**退居低频管理面（建站向导 / 配置 / 自动发布规则 / 健康监测 / 证据归档），并按设计稿重构成"全环境矩阵 + 常驻监控条"：首屏是一句挂着数字的判断句（治理视角），而不是要人逐项去读的六个页签；矩阵横向对比所有环境的健康、可用率、落后主干提交数、DORA 指标，算不出的判断句宁可写「未监测 / 样本不足」也不编造。两页之间的下钻带着环境上下文互跳，但「查配置」不离开发布中心本页。
- **事件驱动自动发布规则**：在原有的定时（scheduled-job）自动发布之外，新增「分支被 push / 开 PR 时自动发布」，支持分支 glob、路径过滤与手动批准；定时驱动的判据收敛成单一来源函数，替换掉此前散在四处的 `type !== 'manual'` 判断。事件规则的来源分支由触发事件本身决定（而不是要求预先填一个固定分支），修复了此前"来源分支必填"拒绝建规则、以及 `release/*` 类分支只能发到固定一个目标的问题。定时规则与事件规则在入口 UI 上做了明确区分（各自的标题与「新建规则」按钮不再重叠、混淆），复用既有的并发闸、审批、回滚、运行记录能力，不新增独立的执行链路。

已知边界与安全取舍同样归入 [doc/debt.cds.release-system.md](./debt.cds.release-system.md)。
