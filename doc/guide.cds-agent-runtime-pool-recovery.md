...
预期 `diagnostics.instanceCount > 0`，并且实例级 `/readyz` 显示 `agentAdapter=claude-agent-sdk`、`loopOwner=claude-agent-sdk`。如果使用环境变量旁路，`AutoConfigureFromEnv=true` 时 `CLAUDE_SIDECAR_BASE_URL` 或 `CLAUDE_SIDECAR_TOKEN` 会自动注入静态 sidecar 并启用 `ClaudeSdkExecutor`；如果手写 appsettings，则仍需确认 `ClaudeSdkExecutor:Enabled=true`，否则静态 `Sidecars` 不会被 MAP 路由和健康检查纳入。
