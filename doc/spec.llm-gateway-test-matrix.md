# spec.llm-gateway-test-matrix

> 类型: spec（What — 测什么）| 状态: active | 更新: 2026-06-29
> 关联: `doc/design.llm-gateway-physical-isolation.md`、`doc/debt.llm-gateway-isolation.md`、
> `prd-api/tests/PrdAgent.Api.Tests/Gateway/`、`scripts/gw-smoke.py`
...
| 层 | 跑在哪 | 依赖 | 现状 |
|----|--------|------|------|
| A 解析/调度 | CI 单元(golden) | 反射，无 Mongo；集成那条需 Mongo | 已存在，复用 `AppCallerRegistryGoldenSnapshotTests` + `LlmResolutionGoldenIntegrationTests` |
| B 协议保真 | CI 单元 | 无（纯函数喂 canned payload） | 本轮新增 `GatewayProtocolFidelityTests` |
| C 跨进程传输 | CI 单元 | 无（真 Kestrel loopback + stub gateway） | 本轮扩展 `CrossProcessServingSelfTest` |
| D 真机 | CDS 起来后脚本 | 真网关 + 真/桩上游 | 本轮出脚本 `scripts/gw-smoke.py`，待 CDS 跑 |
