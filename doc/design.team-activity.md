...
**一句话**：把成员在平台上的关键动作汇成一条团队时间线：谁发布了文档、谁修了缺陷、谁交了周报。
**谁该读**：想看团队动态的管理者；要给新动作加留痕的工程师。
**读完能做什么**：说清留痕从哪采集、怎么加一个新动作而不改埋点代码。
...
每个要留痕的动作在注册表里声明四件事：**属于哪个模块**（带中文名，直接进时间线）、
**动作怎么念**（如「发布了文档」）、**从哪儿取操作对象的标识**（通常是路由参数名）、
**标题从请求里的哪个字段取**。

这样加一个新动作就是加一行声明，**不用改任何埋点代码**——避免每个功能各写各的留痕逻辑。
...
索引（DBA 手工执行，见 [doc/guide.platform.mongodb-indexes.md](./guide.platform.mongodb-indexes.md)）：`{CreatedAt:-1}`、`{ActorId:1,CreatedAt:-1}`、`{Module:1,CreatedAt:-1}`。
...
- [doc/guide.platform.mongodb-indexes.md](./guide.platform.mongodb-indexes.md) — activity_logs 手工索引
