# debt.knowledge-base.store-sync

| 字段 | 内容 |
|---|---|
| 模块 | 知识库跨环境 / 本地库↔库 同步（`DocumentStoreSyncController` + `SyncManagerPanel`） |
| 状态 | open（首版已交付，2026-06-04；以下为已知边界） |
| 关联 | `prd-api/.../Controllers/Api/DocumentStoreSyncController.cs`、`prd-admin/src/pages/document-store/SyncManagerPanel.tsx`、集合 `document_store_sync_links`、`DocumentStore.SyncToken`、设计文档 `design.knowledge-base.store-sync.md` |
