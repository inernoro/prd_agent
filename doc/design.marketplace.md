...
**一句话**：把自己调好的配置发布出去，别人复制一份到自己名下用，原件不受影响、敏感字段不跟着走。
**谁该读**：要把新配置类型接进市场的工程师；关心复制边界的产品。
**读完能做什么**：说清发布与复制的规则，并按白名单原则判断哪些字段可以被带走。
...
每个可复制的配置必须**显式声明哪些字段允许被复制**（白名单，不是排除法）。
白名单之外的一律不带走——归属人、密钥、统计数据、私有备注都留在原件那边。
**用白名单而不是黑名单**：新增字段时默认不复制是安全的，默认复制则会静默泄露。
