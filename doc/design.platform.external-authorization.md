...
- 否 不是统一的 API 请求抽象层（不去包装 TAPD/Yuque/GitHub 的业务 API）
- 否 不是团队级凭证池（先做个人级）
- 否 不替换现有集成（GitHub OAuth Device Flow、CDS GitHub App 都不动）
