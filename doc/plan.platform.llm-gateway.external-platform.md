...
> **版本**：v1.34 | **日期**：2026-07-20 | **状态**：开发中

**一句话**：把内部可运维的网关推进成外部团队能安全自助接入的治理平台，只补五类有限能力。
**谁该读**：推进网关外部化的产品与工程师。
**读完能做什么**：知道要补哪五类能力，以及明确不做什么。
...
### 2026-07-20 架构校正：逻辑模型优先，多上游由 Gateway 内聚

早期设计把 LLM Gateway 放在 MAP 内部，导致视觉创作的模型目录、appCaller、模型池和上游组合被混成一条链。用户已明确校正产品边界：视觉创作只维护租户授权的逻辑模型列表并按逻辑模型调用 Gateway；Gateway 独立维护 Provider、Endpoint、Credential、Offering、协议适配、限流、健康和负载均衡；模型池只在请求没有显式选择模型时承担默认与兼容兜底。本节是对本文件早期“不重做模型池”边界的明确后续变更，历史 PR 事实保留，但不能再用旧边界阻止本次独立有限 PR。

本次目标固定为：

1. 新增租户级逻辑模型与 Offering 权威；同一逻辑模型可连接多个普通模型或 Exchange。
2. 显式模型选择先解析逻辑模型，未知、跨租户或无 appCaller 权限时 fail-closed；未显式选择才进入旧模型池链路。
3. 图片请求保留协议无关 canonical request，OpenAI、OpenRouter、Google 与 Exchange 之间故障切换时逐候选重建 wire request。
4. Offering 独立声明优先级、权重、并发、每分钟速率与健康；限制和健康回写由 Gateway 数据面跨实例执行。
...
| ADV-06 | 创建租户或成员中断留下半成品 | tenant、team、membership、user 分多次写入，不能依赖异常 catch 覆盖进程硬退出 | PR-6 + 2026-07-18 加固 | 对每个可捕获写失败执行补偿；对硬退出使用带精确对象 ID、租约和 generation 的恢复操作，由启动与周期修复器回滚半成品。按 slug 的完整重放会完成原操作并关闭 pending，避免稍后误删已恢复租户 |
...
PR-6 的 30 秒 owner 进程租约已在 2026-07-18 加固中移除。active owner membership ID 与 fencing generation 现在位于租户权威文档；并发移除通过单文档原子条件保证至少保留一名 owner，membership 跨文档收口交给可接管的恢复操作。
...
- 发布稳定性实现：PR #1174 承载 gateway 常驻、静态原子切换、精确公网门禁、不可覆盖证据和独立巡检；生产实证以该 PR 合并后的不可变提交为准。
...
- 独立 CDS 容器：main 精确部署 `dr_b81c9be1b631b2cfd64457bd` 的 API、Admin、Console API、Serving、Web 五个服务全部运行；三个 LLM Gateway profile 均为 `express`，镜像精确对应 `b9058316`。随后复核发现共享预览 `/llmgw/` 会被 Admin 的 `/` 兜底规则接管，不能作为独立网关证据；PR #1174 改以 CDS 服务端生成的 `llmgw-web` 命名子域验收。该独立域返回 Gateway HTML、实际 JS/CSS，Console 与 Serving 双健康均精确返回功能提交 `87b44caaee926d5844064a4e9d4a90d297680c0d`，GW Native、OpenAI、Claude、Gemini 四入口无 key 全部为 401。至此才证明 Gateway 能随 prd-agent 更新并以三个独立容器持续存在，无需删除重建 profile。
...
- `https://main-prd-agent-llmgw-web.miduo.org/` 公网 HTML、实际 JS/CSS、Console health 与 Serving health 均为 200，五个 main 服务运行同一提交。所谓“全是空的”的直接原因是未登录访问被守卫送到 `/login`，旧页面只有居中登录卡和大面积留白，不是容器、静态目录或 API 配置为空。
...
用户在 CDS main 实际接入时发现：Quickstart 的网页测试和生成 cURL 都强制携带 `X-Gateway-Dry-Run: quickstart`，页面没有真实请求入口；同时当前测试租户没有 GW 权威模型池，appCaller 即使完成 key 鉴权也无法解析真实上游。这是两个叠加问题，不是用户配置错误。

本批从最新 main 建立独立分支 `codex/llmgw-real-request-ux`，不重做模型迁移、模型池算法或发布 Gate，只复用已有能力完成以下闭环：

1. Quickstart 明确拆成“安全连通”和“真实模型”两种模式。安全连通继续在模型解析和预算预占前结束；真实模型只有服务端路由预览成功且候选不是明显开发桩时才允许点击，并把 requestId、Provider、实际模型和日志入口放在同一屏。
2. 新签发的 Quickstart key 只增加只读 `route:read` scope，不增加通配 scope。路由预览调用受 scoped key 保护的 `/gw/v1/resolve`，TenantId 仍只由服务端 key 上下文写入，前端不发送 tenantId。
3. 内部租户缺真实路由时，同页提供显式“一键准备现有真实上游”：调用已有非覆盖配置认领与“有则增加、无则不变”的程序池类型初始化，再把当前 appCaller 绑定到同 requestType 的可用默认池。该动作不覆盖、不删除、不裁剪历史池；外部租户仍需配置自己的 Provider、模型与密钥。
4. appCaller 首屏从逐行表单改为只读摘要表，只显示身份、状态、调用方式、路由、治理和最近请求；筛选、批量治理和单项配置采用渐进展开，所有原有治理字段继续保留。

OpenRouter 登录态实测形成以下微观比例基线，仅借鉴信息层级和密度，不复制品牌与视觉资产：

| 元素 | OpenRouter 实测 | GW 本批目标 |
|---|---:|---:|
| 页面标题 | 20px / 600 / 36px 行高 | 20px / 600 / 36px 行高 |
| 正文与表格 | 14px / 约 450 | 14px / 400-500 |
| 表头 | 14px / 500 / 约 47px | 14px / 500 / 46px |
| 表格行 | 14px / 约 46px | 14px / 最小 46px |
| 左侧导航 | 14px / 500 / 约 31px | 14px / 500 / 36px，移动端 44px |
| 首要搜索 | 单一 16px 搜索，筛选后置 | 单一 14px 搜索，筛选与批量治理后置 |

交付门：TypeScript、生产构建、Gateway 数据域守卫和四协议安全测试全绿；CDS 分支测试库具备可读的真实 Provider、模型、默认池和 appCaller；只允许一次真实 chat 验收，其余协议使用既有 dry-run 或假上游；桌面、移动、深浅主题均无横向页面溢出；PR 独立等待 CI 与 CDS 后合并，再复验 main。

### 9.5 请求详情阅读顺序补强（2026-07-17）

OpenRouter 登录态的 Generation details 以“模型与 Provider → 六项核心指标 → 概览 → 请求身份 → Provider Responses → Prompt/Completion/Raw JSON”为固定阅读顺序。GW 原抽屉虽然字段更多，但把价格、路由、身份、时间和策略约四十项连续平铺，且主要使用 10–12px 字号，用户难以先判断请求是否真的执行。

本批保留全部审计能力，同时调整为以下渐进结构：

1. 标题区先显示实际模型与 Provider；首屏六格只展示上游耗时、首字节、生成速度、费用可信状态、输入输出 Token 和是否回退。
2. “费用”优先显示 Provider 实际费用；未对账时才显示 Gateway 估算并明确标记，缺 token 或价格快照保持未知。actual 与 estimated 不互相覆盖，跨币种仍不直接计算差额。
3. 详情主体不再是一条需要反复滚动的长表，固定拆为“概览、请求与响应、路由、审计”四个页签。概览先回答是否成功、用了谁、花了多久和多少钱；请求与响应直接展示 `G-${appCallerCode}`、密钥快照、requestId、generationId 与正文，不再把正文压到页面最底部。
4. Provider Responses 保持概览中的独立分节，按尝试顺序展示 Provider、模型、HTTP 状态、耗时和错误；路由 trace、模型策略、参数策略与提示词策略归入路由页签，费用凭证和完整身份时间归入审计页签。
5. 请求、响应和原始数据使用中文标签和 13–14px 基线；请求记录表同步使用 14px 表头与正文、最小 46px 行高，并把 App 主标识统一为 `G-${appCallerCode}`。详情接口失败时显示明确原因与返回入口，不再永久停留在加载状态。
6. 首页最近请求、全局搜索和复制的 `requestId` 深链在唯一命中后直接打开生成详情；用户主动关闭后保持关闭，不反复弹出。移动端不得要求再次点击横向表格。

分支首次公网验收还暴露出一处服务端身份漂移：`/gw/v1/resolve` 的鉴权预读和端点实现历史上强制把来源当作 `map`，因此 external scoped key 即使正确发送 `X-Gateway-Source: external` 也会被拒绝。修复后，路由预览显式在请求体 Context 中发送 `external`，服务端继续只从已校验 key 获取 TenantId 和 TeamId，但允许 SourceSystem 与 header、body 保持一致；原有未声明来源的 MAP 内部调用仍默认 `map`。这项修复不放宽 key 的 tenant、team、appCaller、协议或 scope 边界。

同一轮精确 SHA 公网复测确认 `/gw/v1/resolve` 已返回真实 OpenRouter Provider 和非桩模型，但 Gateway wire contract 为兼容现有调用方保留 PascalCase，Web 起初只识别 camelCase，因而把成功响应误报为“无法识别”。Web 现对该只读结果同时归一化 PascalCase 与 camelCase；Gateway 协议本身不改名，真实请求开关仍要求 `Success=true`、存在实际模型且身份不含 stub/mock/fake。

390×844 深浅主题验收进一步发现：桌面表格虽已消除逐字挤压，手机仍只能在内部横向滚动查看后半列。最终版在 680px 以下改为纵向摘要卡片，单卡直接显示调用方式、当前路由、治理、最近请求和提示词策略入口；配置展开后仍可编辑模型池、策略、负责人、预算和 RPM。桌面继续保留高密度摘要表，两种布局共用同一份服务端数据和保存动作。

### 9.6 三步客户端接入、并发边界与多上游模型池设计（2026-07-18）

本节只补充当前 SSOT，不替代 `plan.platform.llm-gateway-protocol-router` 与 `design.platform.model-pool`，也不授权多上游架构编码。三步客户端接入与并发安全已经按独立 PR 完成；多上游部分先完成产品和数据设计，待用户另行批准后再拆有限 PR。

#### 9.6.1 当前交付状态

| 目标 | 状态 | 证据 | 完成门 |
|---|---|---|---|
| Cherry Studio 三步接入 | 已合并并完成 main 复验 | PR [#1179](https://github.com/inernoro/prd_agent/pull/1179) 已 squash 合并为 `01678dd7eba7db132421f2f670785eaf2852744c`；专用预设覆盖 Provider、Base URL、密钥和模型配置 | 已完成；主分支 Quickstart 公网返回 200，部署产物包含 Cherry Studio 三步入口 |
| OpenClaw 三步接入 | 已合并并完成 main 复验 | PR #1179 已提供专用预设；官方 CLI 配置校验 0 warning | 已完成；主分支部署产物包含 OpenClaw 三步入口，无密钥调用返回 401 |
| 基础/高级密钥分层 | 已合并并完成视觉验收 | 基础区只保留名称、用途和生成；协议、scope、CIDR、限流、过期、team 等收进高级区；[视觉验收报告](https://cds.miduo.org/reports?project=prd-agent&folder=5cc486d71ffd4badab2fce1a98188a89&report=847c88f3f7eb4f46aeff813fa5276be0) 已归档 | 已完成；桌面与移动端均可在三步内完成基础接入 |
| 并发与跨租户限流 | 已合并并完成 main 复验 | PR [#1183](https://github.com/inernoro/prd_agent/pull/1183) 已 squash 合并为 `0ace8e486f93a67470d542cf27e9125784e72d77`；修复首分钟 appCaller 并发 upsert 竞态，并新增 40 路跨租户并发断言 | 已完成；main CI 全绿，CDS 版本 `dv_d64b1adc016acc45a9af69b4` 的 5 个服务就绪且 `smokeOk=true` |
| 多上游与聚合平台 | 本节完成设计，不编码 | OpenRouter 官方 Provider Routing、BYOK、Models 和 Providers API 对照 | 用户批准实施范围后再拆独立 PR |

PR #1179 的真实上游验收只发送了一次付费协议请求。请求已到达 OpenRouter 并返回真实额度错误 `LLM_QUOTA_EXCEEDED`，证明不是桩链路；依照“每类真实协议最多一次”的成本约束不重试。费用保持 unknown，不伪装为 0，也不跨币种相加。

PR #1183 的并发验收全部使用 MongoDB 与假上游，不产生付费调用。appCaller 场景对两个租户各发 20 路并发、每租户限额 4，结果为 8 次成功、32 次 429，假上游每租户恰好命中 4 次；Service Key 场景每租户限额 5，结果为 10 次放行、30 次 429，30 条拒绝审计按 TenantId 各 15 条。主分支 GitHub run `29633065223` 全绿，CDS 运行 `dr_f8956c32e4e1906a06566665` 精确部署提交 `0ace8e486f93a67470d542cf27e9125784e72d77`。公网 `/gw/healthz` 与 `/gw/v1/healthz` 返回 200 并回显相同提交，无密钥调用 `/v1/chat/completions` 返回 `GATEWAY_KEY_REQUIRED`。

#### 9.6.2 三步接入合同

所有客户端统一为三步，不要求用户先理解模型池、协议适配器或治理字段：

1. 选择客户端预设并一键生成租户专属 `gwk_*` 密钥。
2. 复制 Gateway 地址和密钥到 Cherry Studio、OpenClaw 或其他 Agent；兼容客户端只需选择 OpenAI-compatible。
3. 点击“测试连接”，成功后直接查看同一 requestId 的请求、路由和费用详情。

基础区只显示完成上述三步所需字段；高级区才显示 scope、Team、AppCallerCodes、CIDR、RPM、有效期和入口协议。密钥明文只出现一次；没有密钥不得调用，也不得生成无法归属到租户和 key 的审计记录。

#### 9.6.3 当前模型的结构性问题

当前 Provider 创建模型把 `platformType`、`apiUrl`、通讯密钥和最大并发放在同一个对象中。模型又可以通过 pool item 覆盖 `Protocol`，运行时也已经优先按解析后的 wire protocol 选择 adapter。这一实现能兼容单端点 OpenAI/Claude，但下面场景会变得含混：

- OpenRouter 是聚合供应方，但对外使用 OpenAI-compatible 线协议；它不能因此被建模成 OpenAI。
- 同一供应方可以有多个账号、区域、Base URL、部署和密钥，各自健康度、价格、并发和数据政策不同。
- 同一逻辑模型可以由多个供应方提供；同一供应方也可能有多个 endpoint 提供同一模型。
- adapter 解决“怎样发 HTTP 请求”，Provider 解决“跟谁采购”，两者不是同一概念。
- 只在平台或模型上记并发，无法准确表达账号级、端点级和凭据级配额。

因此后续设计必须拆开供应商身份、可路由端点、通讯凭据、线协议适配器和逻辑模型，不能继续向 `platformType` 追加特例。

#### 9.6.4 目标架构

```mermaid
flowchart LR
    K["租户 Service Key"] --> A["appCaller 与请求类型"]
    A --> L["逻辑模型或能力"]
    L --> P["模型池"]
    P --> R["路由策略"]
    R --> O1["供应方案 A"]
    R --> O2["供应方案 B"]
    O1 --> E1["Provider Endpoint"]
    O2 --> E2["Provider Endpoint"]
    E1 --> C1["Credential Set"]
    E2 --> C2["Credential Set"]
    E1 --> W1["Wire Adapter"]
    E2 --> W2["Wire Adapter"]
    W1 --> U1["直接供应方或 OpenRouter"]
    W2 --> U2["直接供应方或 OpenRouter"]
```

| 对象 | 回答的问题 | 必须包含 | 不应包含 |
|---|---|---|---|
| Provider | 向谁采购或由谁聚合 | 名称、类型、合规和账单身份 | 线协议实现、业务 appCaller |
| Provider Endpoint | 请求具体发到哪里 | ProviderId、Base URL、区域、部署、健康、并发、数据政策 | 密钥明文、租户自报身份 |
| Credential Set | 用哪个账号鉴权 | EndpointId、优先级、加密密钥、状态、配额元数据 | 响应回显的密钥明文 |
| Wire Adapter | 怎样编码请求和解释响应 | openai-compatible、claude-compatible、gemini-compatible、gw-native 等适配能力 | 供应商商业身份、价格策略 |
| Logical Model | 用户选择什么能力 | 稳定 ID、能力、上下文、输入输出模态 | 单一供应商地址和密钥 |
| Offering | 哪个端点能提供哪个逻辑模型 | LogicalModelId、EndpointId、UpstreamModelId、价格、能力覆盖 | appCaller 业务规则 |
| Model Pool | 一组业务可用能力 | 预定义池类型、用途、Offering 集合 | 上游密钥、HTTP 实现 |
| Routing Policy | 为什么选这条路 | 顺序、权重、排序、fallback、价格、延迟、吞吐、数据边界 | 原始请求 tenantId |

OpenRouter 的正确表达是一个 `Provider` 加一个或多个 `Provider Endpoint`，endpoint 的 wire adapter 可以是 `openai-compatible`。OpenRouter 内部再次选择哪一家上游，是聚合平台的路由；LLM Gateway 仍需记录自己选择了 OpenRouter endpoint、发送的逻辑模型和返回的上游归因信息。直接 OpenAI、Claude、Gemini 与 OpenRouter 可以同时成为同一逻辑模型的 Offering，而不相互冒充。

#### 9.6.5 多上游路由示例

例如“默认聊天池”预定义包含逻辑模型 `general-chat`，可追加以下供应方案：

| 优先级 | Offering | Endpoint | Adapter | 适用条件 |
|---:|---|---|---|---|
| 1 | `general-chat -> provider-a/model-x` | 国内区域账号 A | openai-compatible | 数据必须境内、健康且未超并发 |
| 2 | `general-chat -> openrouter/model-x` | OpenRouter 主账号 | openai-compatible | 允许聚合路由且费用上限满足 |
| 3 | `general-chat -> provider-b/model-y` | 供应方 B 账号 | claude-compatible | 前两项限流、5xx 或熔断时 fallback |

路由先应用硬条件：TenantId、池类型、能力、数据政策、禁用状态、预算和密钥可用性；再按策略对候选排序。排序可以是固定顺序、权重、价格、吞吐或延迟。每次请求都生成可预览的候选链，说明排除原因、最终命中和 fallback 过程，避免用户必须跳到其他页面猜测。

#### 9.6.6 “有则增加，无则不变”合同

默认程序池类型仍是唯一注册表。新增模型或 Offering 时：

1. 命中已有预定义池类型和能力规则，只向对应默认池 append-only 追加，不覆盖人工调整。
2. 未命中任何规则时，不创建臆测池、不移动现有成员、不改变默认路由。
3. 特殊业务通过专用池或专用路由策略特殊化，不污染默认类型。
4. 删除 Provider、Endpoint、Credential 或 Offering 前必须显示受影响的池、appCaller 和最近请求，禁止静默断链。

#### 9.6.7 控制台呈现方案

首屏不再铺满输入框，采用“摘要卡片、列表、就地预览、渐进展开”：

- Provider 列表先显示供应方名称、endpoint 数、健康容量、凭据状态和可用模型数。
- 点击 Provider 打开 endpoint 抽屉，查看 Base URL、区域、adapter、延迟、并发和健康；密钥只显示“已配置/需轮换”。
- 模型页先显示 Logical Model，再展开各 Offering；用户不需要把同一模型误建多次。
- 模型池顶部先解释“这个池服务什么请求”，并显示默认池类型、成员数、健康候选数和当前路由摘要。
- appCaller 和 Quickstart 中的 Provider、模型池、策略均可就地预览候选链及影响，不要求跳页验证。
- 新增 endpoint 的基础流程只要供应方、地址/协议预设和密钥；区域、数据政策、价格覆盖、并发细分收进高级区。
...
| 波次 | 范围 | 兼容方式 | 验收 |
|---|---|---|---|
| M1 | 引入 Endpoint、Credential Set、Logical Model、Offering 的数据合同和只读投影 | 每个旧 platform 投影为一个 endpoint 与一个 credential；不迁移、不删除旧集合 | 旧请求和旧池结果完全不变 |
| M2 | Provider、Endpoint、Credential 渐进式控制台 | 旧 Provider 编辑继续可用，新对象走抽屉与基础、高级分层 | OpenRouter 和同供应方双账号可配置 |
| M3 | Routing Policy 与候选链预览 | 默认仍沿用既有 priority；只有显式启用新策略才生效 | 固定顺序、权重、价格、延迟、吞吐和 fallback 可解释 |
| M4 | endpoint、credential 级健康、并发、预算与审计 | 平台级指标保留为聚合视图 | 跨租户隔离、并发不超卖、费用不伪造 |
| M5 | 受控切换与回滚 | 双读比对、显式发布、可回滚；不重做 full-http gate | 四协议、OpenRouter、直接供应方和多账号端到端通过 |

任何实施 PR 都必须保持 TenantId 服务端解析，所有新集合、唯一索引、日志、预算和审计包含 TenantId。不会重做模型迁移、模型池和发布 gate，也不会把本设计混入客户端接入或并发修复 PR。

#### 9.6.9 官方依据

- [OpenRouter Provider Routing](https://openrouter.ai/docs/guides/routing/provider-selection)：提供 order、sort、only、allow_fallbacks，以及价格、吞吐和延迟排序。
- [OpenRouter BYOK](https://openrouter.ai/docs/guides/overview/auth/byok)：同一 Provider 可配置多个 key、优先级、模型或用户过滤与 fallback。
- [OpenRouter Models](https://openrouter.ai/docs/guides/overview/models)：模型身份、能力、模态和价格与供应端点分开展示。
- [OpenRouter OpenAPI](https://openrouter.ai/openapi.json)：`GET /api/v1/providers` 把 Provider 元数据作为独立资源返回，可与请求协议和 endpoint 分开建模。
