# CDS Agent 工作台完成复盘（grandfather 保留） · 报告
...
命名校准：本报告记录的是 2026-05-15 A10 MVP 闭环，当时 `claude-sdk` 仍主要代表历史 sidecar 路径。当前 2026-05-18 目标路径已校准为官方 `claude-agent-sdk` adapter：agent turn loop、上下文和 Claude Code 工具默认归官方 SDK，MAP/CDS 保留 session、profile、审批、事件、日志、workspace、产物和运行时控制面。旧 A10 证明“工作台 MVP 能远程干活”，但不能替代当前官方 SDK adapter 的 R1/S1/S2/S3 商业级证据。
...
4. doc/plan.cds.agent.official-sdk-migration.md
...
- 当前官方 SDK adapter 迁移的真实阻塞是 R1 默认 Anthropic/Claude-compatible runtime profile；不要再按旧实例发现空结果方向排障
- 后续优先做 R1 provider closure、S1/S2/S3 真实 provider smoke、长命令内网 callback、自动验收回放、PR ready 策略、会话清理和用量面板
