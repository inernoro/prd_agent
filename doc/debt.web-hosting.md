...
| 64（网关配置，需人工处理） | `意图识别默认池` 成员数为 0 | 开场问题自动生成走 intent 类型，池子是空的，所以 `regenerate` 端点稳定返回 `ModelUnavailable`。这不是代码问题——同一套代码在 chat 侧已经验通 | 往意图池里放至少一个模型（便宜的即可，如 `gpt-4.1-mini`）。放完我可以立刻复验「开场问题真生成 5 条」 |
