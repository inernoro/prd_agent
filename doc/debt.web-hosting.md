...
| 63（网关配置，需人工处理） | `对话默认池` 里有个叫 `chat-latest` 的成员，它不是可调用模型 | 来路查清了：OpenAI 的 `/v1/models` **确实**返回一条 id 为 `chat-latest` 的条目（124 条上游模型里就有它），批量导入把它当普通模型收了进来，还给了 priority 10——于是它排在 262 个成员的最前面，每次都先被选中，发给 `/v1/chat/completions` 必 400。现在它已被健康机制降级为 Unavailable，调度自动跳到下一个（`gpt-3.5-turbo`），所以对话能用了 | **它仍在池里**：健康状态一旦复位，第一次提问还会先撞它一次。应在网关控制台把这个成员从对话默认池移除。AI 侧被权限拦下，需人工执行 |
| 64（网关判据死锁，2026-08-28 closed） | `意图识别默认池` 成员数为 0，而且**放不进去** | 一开始以为是「没人往里放」。实际是判据死锁：intent 成员资格只认模型文档上的 `IsIntent` / `IsMain` 两个布尔位，而这两位只在**建模型那一刻**由能力勾选写入；从上游批量导入的 319 个模型拿到的恒为 false，控制台唯一能维护的能力面 `Capabilities` 数组判据又不读。三条路全堵：`PUT` 池成员报 `INCOMPATIBLE_MODEL_TYPE`（实测）、`bulk-import` 筛不出候选、删掉重建被 `MODEL_IN_USE` + `DUPLICATE_MODEL` 挡住。判据读的不是运维真正能改的那个值 | 已修（`deda2ead`）：`IsIntentCapable` 收为唯一判据，三个值都认（`IsIntent` / `IsMain` / `Capabilities` 里显式声明的 `intent`）；`Program.cs` 里那份只认布尔位的拷贝改为委托 + 正则守卫；能力维护新增 `modelIds` 过滤，不必按平台整片刷。刻意**不**认 chat 能力——默认池建模型时会自动追加所有兼容模型，认了 chat 等于把几百个对话模型无差别灌进意图池。随后按用户指定把 `gpt-5.6-sol`（OpenAI 平台）放进意图池，实测 4 个站点的开场问题真生成，池成员连续成功 1 |
| 65（调度质量） | 对话默认池 262 个成员，选中的是 `gpt-3.5-turbo` | 池子是把上游能列出来的模型全导进去的，里面还混着 `gpt-3.5-turbo-instruct` 这类根本不支持 chat 的补全模型。优先级排序决定了「谁先被选」，现在轮到的是一个很旧的模型 | 对话默认池该按用途挑几个模型并排优先级，而不是全量导入。属网关配置治理，不在本次范围 |
