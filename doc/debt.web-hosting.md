...
| 65（池治理，2026-08-28 已逐个实测，待用户裁决） | `对话默认池` 是一次 262 个成员的批量导入，没人知道里面哪些真能用 | 发一把临时 scoped key（用完即撤），对 262 个成员逐个 pin 到具体平台+模型发一次真实调用。判据是**上游认不认这个模型名**，不是名字看着像不像——`chat-latest` 正是名字正常、上游 `/v1/models` 也返回、真发过去必 400 的那种。结果：通过 173、稳定失败 87、限流偶发 2。失败分五类：批处理专用 `:batch` 48、不是对话模型（instruct/realtime/pro→responses）20、上游已下线 13、OpenRouter 无可用路由 3、稳定 400/404 2 | 扫描中**顺带发现并已修**一个网关缺陷（`b39acbba`）：18 个 gpt-5.0–5.5 成员全部报 `max_tokens is not supported`，因为字段改名的判据只认 gpt-5.6，而「向我提问」固定发 `max_tokens=2048`——这 18 个对它是地雷。修完复测 16 个转通过、0 回归，另 2 个 gpt-5.1 是探针只给 8 token 预算所致，按真实 2048 预算复测也通过。**剩下 87 条停不停由用户裁决**，未擅自处置；托管默认池删不掉成员，处置手段是停用模型（调度侧已在 `0a8cc8b5` 支持） |
