...
| 92（自测口径，2026-08-29 closed） | CI 红：改的是前端文件，却打红了一条后端守卫，而我按「纯前端改动」只跑了前端套件 | `UploadProgressWiringTests` 钉着 `reuploadSite` 的**完整位置参数列表**，#91 给它加了个 `signal` 参数就当场判红——它要防的那件事（uploadId 一路传到解包层）一个字没变。两层问题：① 钉签名字面量是形状 4a，改坏了不一定红、改对了反而红；② 这条后端守卫读的是**前端源码**，所以「本次只改了前端」推不出「不用跑后端套件」 | 已修：断言改成钉不变量（收得到 `uploadId?: string` 且真的 `fd.append` 进表单），不再钉整串签名；红绿闭环用「把 uploadId 参数整个拿掉」验过仍会红。**自测口径同时收紧**：后端一律跑 CI 的原命令 `dotnet test PrdAgent.sln -c Release --filter "Category!=Integration&Category!=Manual"`（跨两个项目、Release、按 Category 过滤），不再用自己拼的 `FullyQualifiedName!~IntegrationTests`——口径不一致就会漏掉 CI 会跑到的用例 |
