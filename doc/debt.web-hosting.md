...
| 108（结论句的真机边界未验，open） | #105 / #106 / #107 只验到了单元断言层，**没有在预览域名上构造边界数据看真实文案** | 三种边界（访客超 200 条、窗口内日志超 5000 条、过期链接已撤销或超 7 天）都要造数据才能触发，而预览环境与同项目其它分支共用一个 Mongo（`cross-project-isolation` 通道 4），造 5000 条访问日志会污染共享库、干扰其它分支的统计。接线层已单独核过（新字段确实出现在 `shares/analytics` 响应里） | 验收标准：在隔离数据环境（本地 mongod 或独立库）造上述三种边界数据，逐条截图抽屉第一行文案，确认与单元断言一致；或后端补集成测试直接断言三个新字段的取值 |
