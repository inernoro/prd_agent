...
| 17 | `EmbeddingService` 在 `ResolveModelAsync` 之前没有打开 `LlmRequestContext` 作用域，构造函数也没注入 `ILLMRequestContextAccessor`。inproc 模式下 `ModelResolver` 读环境上下文取租户，取不到就退到内部租户——租户维度的调用方可能解析到**别的租户的模型池**，而这一步发生在第 8 轮补的 `Context` 生效之前 | Review 第十轮（PR #1351）提出，判断成立。目前 `EmbedAsync` 尚无调用方，属未接线的潜在问题，不是在跑的 bug | 让 `EmbedAsync` 接收调用方上下文，把 resolve 与 send **一起**包进 `BeginScope`，与 `llm-gateway` 规则要求一致 |
