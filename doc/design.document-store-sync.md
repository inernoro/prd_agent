...
> **关联设计**：`design.document-store.md`（知识库主设计）、`design.acceptance-kb.md` §5.C（前身：export/import + kb_sync.py CLI，本设计将其泛化为通用 UI 能力）、`debt.document-store-sync.md`（已知边界与还款台账）
...
- `design.document-store.md` —— 知识库（文档空间）主设计，本特性是其子能力。
...
- `debt.document-store-sync.md` —— 本特性的已知边界与还款台账（删除传播 / 附件 / 异步化 / 条目级合并 / 教程补步）。
...
详见 `debt.document-store-sync.md`，要点：
