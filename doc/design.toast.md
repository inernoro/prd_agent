...
- **解决什么问题**：模态对话框（需点击"知道了"）打断用户操作流程，体验差
- **方案概述**：实现非阻塞 Toast 通知系统，支持 4 种类型（success/error/info/warning），自动消失 + 毛玻璃效果
- **业务价值**：提升操作反馈的流畅度，减少用户点击成本
- **影响范围**：prd-admin 全局通知组件，替代原有模态对话框
- **预计风险**：低 — 纯前端 UI 组件，不涉及后端变更
