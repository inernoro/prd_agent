...
- **解决什么问题**：LLM 应用调用者管理页面采用树形下拉布局，用户需逐层展开才能看到子功能，配置效率低
- **方案概述**：改为左右分栏布局，左侧扁平应用列表，右侧垂直展示所有功能及其模型配置
- **业务价值**：一次选中即可纵览全部功能与模型配置，大幅提升运维效率
- **影响范围**：prd-admin 管理后台 LLM 应用调用者页面
- **预计风险**：低 — 纯前端布局重构，不涉及后端变更
