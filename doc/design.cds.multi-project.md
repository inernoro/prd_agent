...
> - [doc/rule.cds.mongo-migration.md](./rule.cds.mongo-migration.md) — 迁移与回滚规范
> **基线**：[doc/design.cds.md](./design.cds.md) v3.2（现有单项目架构）
...
详细字段定义见 [doc/spec.cds.project-model.md](./spec.cds.project-model.md)。这里列集合总表：
...
详见 [doc/rule.cds.mongo-migration.md](./rule.cds.mongo-migration.md)。简要三阶段：
...
详见 [doc/plan.cds.multi-project-phases.md](./plan.cds.multi-project-phases.md)。

> **2026-05-03 实施现状**:P1-P4 全部上线 + 部分 P3/P5 提前实现 + mongo-split 默认 + onboarding UAT 收尾。详见 [doc/plan.cds.status.md](./plan.cds.status.md) §二「已完成里程碑」。
...
- [doc/spec.cds.project-model.md](./spec.cds.project-model.md) — 数据字典（User / Workspace / Project / Environment 全字段表 + 索引）
- [doc/plan.cds.multi-project-phases.md](./plan.cds.multi-project-phases.md) — 7 期详细交付计划 + 每期验收标准
- [doc/rule.cds.mongo-migration.md](./rule.cds.mongo-migration.md) — state.json → MongoDB 迁移与回滚操作规范
- [doc/design.cds.md](./design.cds.md) — 现有 v3.2 单项目架构（本文档的基线，仍然有效直到 P4）
- [doc/design.cds.resilience.md](./design.cds.resilience.md) — 温池 / 集群调度（P4 后需与 project 层集成）
- [doc/design.cds.onboarding.md](./design.cds.onboarding.md) — 一键导入配置（P4 中需扩展支持多项目）
- [doc/design.cds.data-migration.md](./design.cds.data-migration.md) — CDS 数据迁移（P3 时需同步思路）
...
### 仓库共享：一个仓库绑定多个项目（2026-09-02）

- 一个 Git 仓库允许绑定第二个项目，但 link 与建项目都需要一次显式确认，不带确认仍拦住——默认仍是「一仓一项目」，共享是显式选择而非静默放行
- 新增同仓关系判据：算出兄弟项目、未声明构建范围的项目、以及真正撞在一起的共享数据库与缓存（呼应跨项目隔离规则的隔离通道清单）
- 新增「构建范围」声明：按各服务启动命令里的工作目录切换推断本项目在仓库中的目录，横幅给出建议 + 一键采纳；范围对话框改为勾选仓库内真实一级目录，不再是空文本框
- 项目只声明了部分服务的范围时退回全通配，不收窄判据——避免漏声明的服务被误判为"已隔离"而静默不部署
- 同仓多个项目共享的 webhook 事件（删分支 / 关 PR / 镜像完成）改为逐项目分发，一个项目出错不影响其余项目
- 同仓关系与兄弟项目信息只对机器凭据（AgentApiKey）暴露数量，不暴露 id 与名字，避免跨项目信息泄漏
