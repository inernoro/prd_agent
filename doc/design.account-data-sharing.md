...
- **解决什么问题**：用户之间无法流转工作空间、提示词、参考图等数据资产，导致人员交接困难和新人上手慢
- **方案概述**：通过点对点深拷贝机制，发送方选择数据项并指定接收方，接收方确认后系统执行独立副本拷贝
- **业务价值**：消除人员变动时的数据资产丢失和重复劳动，加速新成员上手
- **影响范围**：新增 AccountDataTransferController、通知系统（3 种新通知）、prd-admin 新增 DataSharingPage
- **预计风险**：中 — 工作空间深拷贝涉及多集合数据（workspace + assets + messages + canvas），需确保拷贝完整性
