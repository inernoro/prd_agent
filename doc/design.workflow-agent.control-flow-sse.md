# 工作流引擎 v2：流程控制舱 + SSE 实时推送 · 设计

> **版本**：v1.0 | **日期**：2026-02-15 | **状态**：已实现
>
> **基于**：`design.workflow-agent.engine.md` v1.0 | **appKey**：`workflow-agent`

## 一、管理摘要

- **解决什么问题**：v1.0 工作流引擎仅支持无条件串行管道，无法条件分支；前端轮询状态延迟高、体验差
- **方案概述**：引入流程控制类舱（延时、条件分支）使引擎支持 DAG + 条件决策；用 SSE 实时推送替代轮询，节点状态变化 < 400ms 到达前端
- **业务价值**：工作流从线性管道进化为可编排的自动化决策引擎，用户可实时观察执行进度
- **影响范围**：prd-api 工作流引擎执行层、prd-admin 工作流画布和执行面板
- **预计风险**：中 — 条件分支引入 DAG 跳过逻辑，需确保分支合流时状态正确传播
...
本文档是 `design.workflow-agent.engine.md` v1.0 的**增量扩展**，不替换原有设计。
