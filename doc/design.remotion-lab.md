...
- [x] **重构 AI 生成模式布局**
  - [x] 左侧面板：输入区 + 示例 prompts（紧凑）
  - [x] 右侧面板：代码/预览 Tab 切换
  - [x] 参考 Remotion Studio 布局风格
...
- [x] **修复 AI 生成非 Remotion 代码问题**
  - [x] 强化 System Prompt 约束
  - [x] 添加代码格式校验（必须包含 useCurrentFrame）
  - [ ] 生成失败时提供修复建议

- [x] **代码校验层**
  - [x] 编译前检查：是否为 React 函数组件
  - [x] 运行前检查：是否使用了 Remotion API
  - [x] 错误时显示警告提示
...
| 2026-02-03 | v2.0 | 完成布局重构：左输入/右代码预览 Tab 切换，代码校验层，流式显示优化 |
