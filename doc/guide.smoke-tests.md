...
| `SMOKE_SKIP` | _(空)_ | 逗号/空格分隔要跳过的 key（`health`/`cds-agent-runtime`/`cds-agent-templates`/`cds-agent-preflight`/`cds-agent-readiness`/`cds-agent-s1`/`cds-agent-controls`/`prd-agent`/`defect`/`report`） |
...
`r1-report.json`、`s1-report.json`、`cycle-summary.json` 和可选视觉截图。终端汇总里的 `Passed`
