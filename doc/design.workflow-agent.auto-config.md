...
> **版本**：v1.1 | **日期**：2026-06-14 | **状态**：已落地 | **appKey**：`workflow-agent` | **主文档**：`design.workflow-agent.engine.md`
>
...
> 对应 changelog：`2026-06-14_workflow-auto-config.md`

Phase 1-2 已全量落地，与本文设计方案基本一致，以下为落地时的关键修订与修复：

| 项目 | 说明 |
|------|------|
| SSE 事件修复 | from-chat 流误用 `event: message` 包裹导致前端分发失效，改为 workflow_created/generated/delta 直接传递 |
| 校验容错补强 | 重复 nodeId / null 节点 config / 重复变量 key 均作容错，不再崩 SSE |
| 自动接线改为补缺连线 | 仅补漏接的处理节点，不全量重连 |
| 按插槽粒度接线 | data-merger 等多输入节点的每个必填插槽单独补，补不上的暴露为结构问题 |
| 条件必填扫描 | TAPD 认证方式影响 cookie/authToken 的必填判定，缺项扫描新增条件分支 |
