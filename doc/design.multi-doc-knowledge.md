...
> **版本**：v3.0 | **日期**：2026-03-23 | **状态**：已实现（含渐进式披露 Phase 2 + 文件上传与三阶段格式检测）

---

## 一、管理摘要

- **解决什么问题**：单文档对话无法满足实际工作中 AI 需要交叉参考多份文档才能给出高质量回答的需求
- **方案概述**：以主文档为锚点、辅助文档为上下文，在 Session 层绑定文档类型与角色，支持多文档知识库 + 文件上传 + 三阶段格式检测
- **业务价值**：显著提升 AI 对话的回答质量和上下文理解能力，支撑 PRD 评审、技术方案评审等多文档场景
- **影响范围**：后端（Session/Document 模型、ChatService）、桌面端（文档选择 UI）、管理后台（文档类型管理）
- **预计风险**：低 — 向后兼容旧单文档 Session，无破坏性变更
