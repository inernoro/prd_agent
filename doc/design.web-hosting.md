...
提问会消耗创建者账号下的模型调用额度，因此匿名访客的提问必须被限流，否则任何人都能通过反复提问烧光站点主人的额度。配额闸分两层：

- **访客维度**：同一访客（按登录身份或匿名会话识别）在一段时间窗口内的提问次数上限。
- **站点维度**：单个站点每天能被提问的总次数上限，防止某个爆款分享页被集中刷爆额度。

两层配额独立计数、独立拒绝；某一层触发时，另一层已经预先扣掉的额度会退回，避免「站点配额满了，但访客自己的额度也跟着白白扣掉」这种连坐。
