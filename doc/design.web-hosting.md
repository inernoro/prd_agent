...
系统在提问前会对站点内容取一份「正文快照」：直读对象存储里的站点文件，覆盖单文件、多文件站点、PDF 包装站三种形状。正文体量超过文件数或大小上限时，如实标记为「已截断」，不会让提示词假装自己看到了整个页面。
...
回答失败时只向访客展示稳定的中文文案与错误码，网关或上游的原始错误只进入服务端日志，不透传给匿名访客。
