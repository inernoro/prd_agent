...
- **访客维度**：登录访客按用户 ID 计数（每小时上限更宽松）；匿名访客按客户端 IP 的哈希值计数（每小时上限更紧，因为身份不可靠）。IP 哈希是粗粒度兜底——同一 NAT 出口的多个访客会共享同一个计数桶，且它不随浏览器会话刷新重置，这是已知的精度取舍，见 [debt.web-hosting.md](./debt.web-hosting.md)。
...
回答失败时只向访客展示稳定的中文文案与错误码；网关或上游的原始错误会落进服务端日志与该站点的对话消息落库记录，但同样不会透传给匿名访客本人。这条消息记录目前只写不读——创建者尚无法通过界面查看，属于已知边界，见 [debt.web-hosting.md](./debt.web-hosting.md)。

已知边界（配额键的部署作用域、正文快照的诚实性判据、追问历史的字符预算等）见 [debt.web-hosting.md](./debt.web-hosting.md)。
