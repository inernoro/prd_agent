...
- 站点缩略图不再因外部字体资源加载缓慢而永久停在占位图标；大预览弹窗不再把「还在加载」误判成「加载失败或超时」并盖住已经渲染好的页面。
- 分享页获取网页原文改走服务端同源代理，修复此前跨域被拦截、导致降级方案从未真正生效的问题；打包型 SPA 入口改为直接走原始链接预览，避免不透明来源下外链脚本被拦成白屏。
- 取原文的加载遮罩改为限时让位：超过等待阈值后让位给已渲染好的直链页面，且让位后不会再用迟到的原文替换用户正在看的页面，避免当面重载丢失滚动位置与输入内容。
- 页面自带的相对路径基准按站点真实地址重新解析，修复内嵌预览下资源被解析到宿主应用自己身上的问题。
- 新增分享 token 域的站点正文读取端点，复用既有分享门禁；分享过期判定统一到访问链接的豁免逻辑，修复历史分享链接「页面打得开、正文却报已过期」的不一致。
- 匿名正文代理加了边读边限流上限与读取截止时间，防止拿分享 token 发起大额资源占用。
