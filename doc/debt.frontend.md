...
| 边界 | 影响 | 状态 |
|------|------|------|
| 总览里「已下线」判定改用服务端全量菜单目录（user-layouts 附带），清理接口另在服务端拒绝仍有效的菜单 key | 权限不全的管理员不会再误标、更不会误删他人合法菜单；代价是无权限的项会多画一两枚 | 已还（2026-09-05，Codex P1） |
| 总览每行复演侧栏的「新菜单自动追加」（`lib/navEffectiveOrder`），自动补上的项用虚线灰框标出 | 所见即侧栏 | 已还（2026-09-05 用户反馈「只覆盖了一部分」后补） |
| 新增的两个接口没有 xUnit 用例，只做了 CDS 部署后的接口冒烟 | 回归靠人工 | 待办：补 `DefaultNavConfigController` 的集成测试（列表排序 + 单人重置不波及他人） |
| 总览的「最近改动时间」取自新字段 NavLayoutUpdatedAt，本次上线前已有的自定义记录没有这个值 | 存量自定义用户显示「时间未知」并排在有时间的之后，直到他们再动一次导航 | 已知即可，不做回填（回填只能猜） |
| 候选池条目拖进导航条时没有让位动画，只有插入标记 | 体验弱于导航内部排序 | 可选：把候选池也纳入 SortableContext 做跨容器排序 |
| 授权健康页「模型平台凭据 / LLMGW 服务身份」的处理入口现指向请求日志页，不是模型网关 | 凭据问题的真正修复地在 LLM Gateway 控制台，点「立即恢复」落到日志页只能看不能修 | 待办（Codex 第三轮 P2，判为扩范围）：给健康项加 actionKind=llm-gateway，健康页复用通知的 SSO 深链；下线前该入口指向的墓碑页也只是一个「打开网关」按钮 |
| 总览每行用全量目录复演侧栏，不按每个用户自己的权限过滤 | 无某权限的用户会被多画一两枚其侧栏没有的菜单（如 users.read 缺失仍画「团队」） | 待办（Codex 第三轮 P2，判为扩范围）：服务端按人算有效权限后返回每人的可见 id 集，全量目录只留给「已下线」判据 |
| 统一目录把 group 为空的菜单项（prompts / skills 等「其他菜单」）当成可用条目，但侧栏 getSidebarMenuItems 只渲染有 group 的项 | 编辑器候选池里能拖进去、侧栏却不画；remove-tokens 也按目录全集把它们判为有效而拒清 | 待办（Codex 第三轮 P2，PR 之前就存在的目录口径不一致）：统一「可进导航」的判据为 group 非空，或给这些项补路由与分组 |

---

## 公共藏书阁（/bookshelf）

2026-09-09 落地。书目与考题是**公共**内容（`src/lib/bookshelf/catalog.ts`、`exams.ts` 全站同一份，
有 18 条守卫护着，见 `src/lib/bookshelf/__tests__/`）；个人进度是本地的。
...
| 后端还有 35 处裸写 `new { success = true, ... }`，同样缺 error 键 | 这些端点的响应一律过不了 `apiClient.isApiResponseLike`（要 success/data/error 三键齐全），前端只会拿到 `INVALID_FORMAT` 的失败壳；消费方若没兜底就会重演藏书阁这次的整页崩。已修的只有藏书阁那 3 处，其余未逐个核对前端是否受影响 | 待办：加一条守卫扫 `Controllers/**/*.cs`，`return Ok(new { success = ...` 一律判红，逼回 `ApiResponse<T>.Ok(...)`（全仓已有 1834 处走的是这个）。守卫落地前，新写端点一律用包装类 |
...
| ~~卷六「驭 AI」偏模型原理，缺「怎么审 AI 写的代码」~~ | | 已还（2026-09-11）：补 5 本实操书（共同智能 / Prompt Engineering for LLMs / AI Engineering / 代码阅读方法与实践 / 你的代码就是犯罪现场）+ 3 道考题。新守卫「每卷至少三本 level ≤ 2 的可上手书」先红在卷六再转绿，验收脚本加「卷六有怎么审 AI 的书」双主题断言 |
| 书目是策展静态数据，新增一本要改代码发版 | 非工程角色无法补充书目 | 已知即可。这是有意取舍：书单的价值在于**精选与编排**，开放 UGC 会稀释它。真要开放，走「推荐入口 + 人工收编」而不是直接可写 |
| 手机档角色 chip 只有 3 个（全部/开发者/产品经理），终稿画了 8 个 | 终稿的 chip 行含「测试 / 设计 / 运维 / 技术负责人 / 新人」，但书目的 `track` 只有 `dev` / `pm` / `both` 三种取值，那五个点下去必然是空列表。实现时只放有数据的三个——宁可少一个入口，也不给一条走进空屋子的路 | 待办：要补齐得先扩 `Track` 联合类型并给 51 本书逐本重打标（六层涟漪见 `enum-ripple-audit`），不是加两个 chip 就行。守卫 `bookshelfMobile.test.ts`「角色 chip 不给没有书的角色留入口」盯着，补了数据才允许加回来 |
| 手机档考试进度不进 URL，刷新即重来 | 考到第 5 题刷新页面会回到卷页重新开始，已答的选项不保留 | 已知即可，是有意取舍：考到一半的卷子不该被分享出去，`?vol=` 只表达「在看哪一卷」。真要保留就得把 picked 落 sessionStorage，但那会让「再考一次」的语义变含糊 |
| 终稿只画了 3 屏，另外 2 屏（答题、团队看板）由我按同一套档位表推导 | 推导的两屏没有设计稿可比对，还原度只能靠「档位来自同一份 AS_TOKENS + 与 1d 同构」自证，不能像 2a/1c/1d 那样做并排图核对 | 已知即可。推导依据写在各自文件头注释里；若设计师后续补画这两屏，按 `design-replication` 重跑 Step 5 并排核对 |
| 结业考题库固定 28 题、不打乱顺序 | 刷第二遍时题序与答案位置不变，可背 | 待办：交卷后随机化选项顺序，或扩到每卷 8 题随机抽 5 |
| 51 本书里 1 本有意留空 `relatedRules` | 2026-09-15 已逐本填完 50 本，共引用 41 条规则，且每本至少挂着一条带真实事故的规则。唯一留空的是《深度学习》——它讲反向传播与优化器那一层的数学，60 条规则里没有一条对得上，硬挂一条「AI 相关」的上去，第三段就会顺着那条去凑 | 已知即可。留空之后第三段会如实写「这本书还没有对上我们自己的事故记录」，这比编一条强。哪天真在模型训练这一层栽过并写成规则，再回来填。守卫钉住的三条：规则必须真实存在、同一本书不重复、（人工核对）没有两本书挂一模一样的规则组合 |
| 桌面档还没有精读稿入口 | 桌面点一本书仍是原来的就地展开，读不到稿子 —— 同一个功能在两个档位上不一致 | 待办：桌面书卡加入口。手机是第三级页面，桌面单页滚动的骨架里更适合用 Dialog，复用同一套 service |
| 精读稿那条 SSE「必须传 CancellationToken.None」没有守卫 | 2026-09-15 修掉了一次真实事故：模型流与写库都透传了 `HttpContext.RequestAborted`，读者读到一半退出页面，三千多字的稿子连根掐掉、库里什么都没落下。现在改成 `None` 了，但改回 `ct` 不会有任何东西变红（`server-authority` 规则 1 全靠人记得） | 待办：在 `prd-api/tests/` 加一条源码守卫，扫 Controller 里 `_gateway.StreamAsync(` 的第二个实参必须是 `CancellationToken.None`。本轮没加是因为沙箱里没有 dotnet，新写的 C# 测试要等一轮 CI 才知道能不能编译，而当时人在等截图 |
| 两个人同时第一次点同一本书会各生成一篇 | 后写的那篇覆盖前一篇，等于多烧一次生成。51 本书、内部工具，撞上的概率低 | 已知即可。真要治得在生成前用 FindOneAndUpdate 抢一个 Generating 占位，抢不到的那个轮询等结果 |
| 「稿子生成完但没存下」只有日志里看得见 | 2026-09-15 踩实一次：`ReplaceOne` 拿新对象去替换已存在文档，Mongo 以 code 66 拒绝（`_id` 不可变），而异常发生在 SSE 已开始输出之后，`ExceptionMiddleware` 想改 header 又炸一次，前端只看到流无声断掉、库里还是旧的那篇——连着三次「重新生成」全部石沉大海。根因已修（沿用库里那份 _id），写库也兜了 try/catch 改推 `error` 事件 | 待办：这类「SSE 开流之后才抛的异常」在整个仓库都是同一个形状（前端看不到、中间件又二次抛）。要治得在 SSE 端点统一一层：开流后所有异常一律转成 `error` 事件，而不是每个端点各写一遍 try |
| 提示词升版会让存量稿子全部重写一遍 | 2026-09-15 起 `PromptVersion` 真正生效：服务端版本对不上就不复用、前端拿到 `stale` 也不吃缓存，于是升一次版所有读过的书都会在下一次点开时重烧一篇 | 已知即可。51 本书、内部工具，重烧的代价小于「留着旧版写法的稿子还得靠人记得哪些该重生成」。真要省，得给书页加一行「这篇是旧版写法」+ 按钮让人自己决定，但那等于把判断推给读者 |
| 精读稿用哪个模型是环境配置，不在代码里 | 首篇稿子跑在 `gpt-3.5-turbo-1106` 上——「对话默认池」是按 OpenAI 模型清单自动导入的 262 个模型、按名字字母序排优先级，头部正好是 `gpt-3.5-*`。代码里没有任何地方能改这件事（`GatewayRequest` 只有 `ExpectedModel` 提示和 `PinnedModelId` 硬锁，写死模型名违反 `llm-gateway` 规则 1） | 已配：网关控制台建了「藏书阁精读稿池」（gpt-5.6-sol / gpt-5.4 / gpt-4.1 三档），并把 `prd-agent.bookshelf.digest::chat` 绑上去、关掉跨池兜底。**这是部署侧数据，不随代码走**——换一个部署得重配一次。根子上的问题是那个 262 个模型的默认池：按字母序定优先级等于随机选模型，任何没有专属池的调用方都会中招 |
| 51 本书的封面不做 | 书目行只有文字排版，视觉上比卷面那一层单薄 | 已知即可，是有意取舍：实体书封有版权，让模型编一张既不像也不合适。真要有封面只能走「人工上传真实书影」那条路，那是 `homepageAssetSlots` 那一摊的事，不归系统配图 |
| 七卷卷面图靠文生图，跨卷的一致性只由拍法前缀保证 | 同一档拍法下七张图仍可能在构图与色温上各走各的，凑在七卷行里看会不齐；本环境没有配 vision 模型池，图生图一律 `MODEL_POOL_EMPTY`，锁不住参考图 | 已知即可。管理员可逐张改画面描述往回收；模型池补上 vision 之后，可给系统配图加「以这一张为参考」的图生图路径（与首页那对雾天/晴天图是同一笔账） |
| 卷面图与处境卡共用同一张，没有按位置裁切 | 处境卡是 308×120 的横条，卷页是 168 高的通栏，同一张图两处都按 `cover` 居中裁——主体偏上或偏下的那几张会在其中一处被切掉重点 | 已知即可。真要治得给图位加 `focalPoint`（或让生图返回主体框）再按位置换 `background-position`；先看真实出图效果再决定值不值 |
