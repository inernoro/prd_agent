...
有 17 条守卫护着，见 `src/lib/bookshelf/__tests__/`）；个人进度是本地的。

| 欠什么 | 影响 | 处置 |
|---|---|---|
| 已读标记与结业考成绩只落 localStorage（`bookshelf-progress`） | 换设备、换浏览器、清缓存即丢；团队内互相看不到进度 | 待办：后端建进度实体（对照 `DefectReport.cs` 写法），补 `GET/PUT /api/bookshelf/progress`，前端 store 改为服务端优先、本地兜底 |
| 没有团队看板，看不到「谁通关了哪几卷」 | 规则 #9 想治的「什么都不跟我说」只解决了一半——书有了，但管理者仍不知道谁读到哪 | 待办：进度上后端之后加团队维度聚合，按卷统计通关率 |
| 书目是策展静态数据，新增一本要改代码发版 | 非工程角色无法补充书目 | 已知即可。这是有意取舍：书单的价值在于**精选与编排**，开放 UGC 会稀释它。真要开放，走「推荐入口 + 人工收编」而不是直接可写 |
| 结业考题库固定 25 题、不打乱顺序 | 刷第二遍时题序与答案位置不变，可背 | 待办：交卷后随机化选项顺序，或扩到每卷 8 题随机抽 5 |
