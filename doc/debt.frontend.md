...
| 服务端 hydration 期间用户的操作会被整体覆盖 | 进页面先拉 `/api/bookshelf/progress`，回来之后是整份替换。如果用户在这段窗口里标了已读、改了笔记或交了卷，那次改动会被服务端快照顶掉；更糟的是若响应赶在 400ms 防抖之前到达，随后的 flush 会把被覆盖的旧快照再 PUT 回去，用户的动作无声消失 | 待办（Codex review P2，2026-09-16）。正解是记下 hydration 窗口内发生的变更，响应到达后做合并或重放，而不是整份替换。不在当前 PR 展开是因为它要引入「本地变更集」这个新语义（哪些字段算冲突、以谁为准），按 5.5 属于 B 类 |
