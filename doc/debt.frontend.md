...
| 并发保存会丢掉另一台设备刚存进去的成绩 | `PUT /api/bookshelf/progress` 是读-改-写：两台设备同时提交，都读到同一份旧的 `ExamResults`，各自合并自己那次，然后整份 `Set` 回去——后写的那一发会抹掉先写的那条，或者用更差的分数盖掉更好的。**这打的是本功能自己许下的承诺**：「跨设备取更好的那次」这句话写在代码注释和[design.prd-agent.bookshelf.exam.md](./design.prd-agent.bookshelf.exam.md) 里，而它在并发下不成立 | 待办（Codex review P1，2026-09-16）。正解是 CAS 重试或按卷的原子更新（`$max` 一类）。**不在当前 PR 展开的理由**：乐观并发是新机械、新语义类别，而当时已到 5.5 熔断阈值（修复提交 7/8），用户明确要求收口；且本地无 dotnet 与 Mongo，并发行为验证不了，靠推演上线风险更高。触发窗口是「同一个人两台设备几乎同时交卷」，且服务端至少留下其中一份，不是数据损坏 |
