...
**同一条路径上还欠着两笔**（2026-08-29 补，来自首页重构 PR 的第九轮评审）：

1. **供应商的临时图片地址被原样收下**（P1，本条里最该先修的一笔）。生图完成后，认领接口
   把上游返回的那个地址直接写进首页槽位。那类地址通常是带签名、有有效期的——它指向供应商
   的对象存储，不是我们的托管资源。平台侧本来有一条「把产出复制进受管存储」的路径，但它只对
   带工作区归属的 run 生效，而首页这条 run 没有工作区，于是复制那一步整个跳过了。**后果是
   时间炸弹**：生成当天首页一切正常，等地址过期，未登录用户打开首页看到的就是一排裂图，
   而管理端不会有任何报错。修法与上面那条同源——认领落位搬到服务端时，顺手把「先落进受管
   存储再写槽位」一起做掉，不要只搬提交时机。
2. **列表拉不回来时，改动按钮仍然可点**（P2）。这一屏进来先拉一次现有配图与提示词；这次拉取
...
**第三笔同源的**：**发起那一下的回应丢了，也会重复计费**。发起生成是一次普通请求，
...
「收到了但回执丢了」。修法与上面三笔一起做：对账路径建起来之后，发起失败先查一次
再决定要不要换模型。

| 状态 | 项 | 优先级 |
|---|---|---|
| open | 认领的图先落进受管存储再写槽位，杜绝临时地址过期后首页裂图 | P1 |
