...
| 进度整包 PUT，没有并发合并 | 同一人在两个标签页同时操作，后写的那次覆盖前一次的已读列表（成绩同样会被覆盖：「取更好的那次」是拿各自读到的那份旧值比的，两端都判自己更好时照样后写的赢） | 已知即可。单人多标签同时改书单是极低频场景，为它上乐观锁不划算；真要治就给文档加 version 字段做 CAS |
...
| 精读稿生成直接跑在 SSE 请求里，没有心跳也不能断点续传 | 多分钟的模型调用占着一条 HTTP 连接：模型解析或隐式思考期间连接空闲久了会被代理掐断，客户端也没有 `afterSeq` 之类的续传手段。断了之后重开这本书就是再烧一次生成，而上一次仍在后台跑到写库——重复计费，中间产物也拿不回来 | 待办（Codex review P2，2026-09-16）。正解是 `server-authority` 规则里那套：落一条持久化 run，交给 Worker 跑，SSE 只做投影并带 keepalive 心跳与 `afterSeq` 续传。不在当前 PR 展开是因为它是重构级改动（新实体 + 新 Worker + 前端重连），按 CLAUDE.md 5.5 属于 B 类「有价值但扩范围」 |
| 服务端 hydration 期间用户的操作会被整体覆盖 | 进页面先拉 `/api/bookshelf/progress`，回来之后是整份替换。如果用户在这段窗口里标了已读、改了笔记或交了卷，那次改动会被服务端快照顶掉；更糟的是若响应赶在 400ms 防抖之前到达，随后的 flush 会把被覆盖的旧快照再 PUT 回去，用户的动作无声消失 | 2026-09-16 已修掉其中最伤的一支：书页的笔记输入框。它的初值在 hydration 之前取，之后不跟随，于是服务端已有的笔记显示成空框——用户一个字没打，只要聚焦再失焦，onBlur 的比较就成立，把那条笔记用空串覆盖掉并同步出去（**不是少显示一条，是删数据**）。现在未编辑时跟随服务端、编辑过之后以用户手上那份为准，守卫见 stores 的登出与拉取作废用例。剩下的「已读勾选 / 交卷」落在同一窗口里仍会被整份替换顶掉——待办。正解是记下 hydration 窗口内发生的变更，响应到达后做合并或重放，而不是整份替换。不在当前 PR 展开是因为它要引入「本地变更集」这个新语义（哪些字段算冲突、以谁为准），按 5.5 属于 B 类 |
...
| ~~`relatedRules` 改了，稿子不会自动失效~~ | | 已还（2026-09-16）：新鲜度判据从一维（提示词版本）改成两维——再加一份「这本书那份材料的内容指纹」，`BookDigest.MaterialFingerprint` 存下生成当时那个值，任一维对不上即不复用。判据从「人记得改版本号」变成「材料变了自动对不上」，取稿与生成两处走同一个 `IsFresh`。守卫 8 条，把指纹从判据里拿掉会精准变红。存量文档没有这个字段，一律判过期、下次点开各重生成一篇 |
| 结业考及格线有两份 | 服务端 `BookshelfExamScoring.PassRate` 是落库权威，前端考试常量里那份只管交卷那一屏的即时反馈。两处必须同值，改一处要改两处——这是明知故犯的判据分裂（`predicate-and-wiring-discipline` 形状 3），接受它是因为另一条路（把及格线塞进接口、让前端告诉服务端「我及格了」）代价更大 | 待办：要收敛成一份得由服务端下发及格线给前端渲染。理由与取舍见 [doc/design.prd-agent.bookshelf.exam.md](./design.prd-agent.bookshelf.exam.md) |
| 存量成绩的 `Passed` 不回溯重算 | 2026-09-16 起及格与否由服务端算，但库里已有记录保持原值不迁移。这些值都是同一条前端判据算出来的、本身正确，所以没做迁移；但若日后调整及格线，存量与新记录会是两套口径 | 待办：改及格线时必须同时写一次性迁移，把存量记录按新线重算 |
| 保存成功后不采纳服务端回传的合并快照 | PUT 的响应里带着服务端合并后的那一份（考试取更好的那次，可能来自另一台设备），但客户端只看 `success` 就把本地标成已同步。于是这台设备显示的分数比服务端和团队看板低，直到下次整页重载才自己纠正 | 待办（Codex review P2，2026-09-16）。**不在当前 PR 展开的理由**：要采纳回传快照，就得先回答「这一发出去之后用户又改了什么、哪些字段以谁为准」——那正是上一行 hydration 那条推迟掉的同一套「本地变更集」语义，两条该一起做。按 5.5 属于 B 类；现状是显示偏低而不是数据错，服务端与看板始终是对的 |
| 并发 / 离线保存会丢掉另一台设备的已读与心得（成绩只是其中最轻的一项） | `PUT /api/bookshelf/progress` 是读-改-写，而且**整份替换**：`ReadBookIds` 是整条列表 `Set`、`BookNotes` 是整张表 `Set`。两台设备从同一份基线各改各的，或者离线设备联网后补推，后写的那一发会**静默删掉只存在于另一份快照里的已读标记与心得**——那些数据没有第二个副本。成绩**并不比它们轻**——这一点我先前写错了两次：「取更好的那次」是各自拿**自己读到的那份旧值**比出来的，两端都判自己比旧值更好时，`$set` 整张表照样后写的赢，差的那次能盖掉好的那次（Codex 第 23 轮指出）。所以三类数据是同一种丢法，没有哪一类有额外保护。**这打的是本功能自己许下的承诺**：「跨设备取更好的那次」写在代码注释和 [design.prd-agent.bookshelf.exam.md](./design.prd-agent.bookshelf.exam.md) 里，而它在并发下不成立 | 待办（Codex review P1，2026-09-16）。正解是按修订号的 CAS + 冲突调和，或按书的原子写 + 显式删除墓碑；两者都要先定义「删除」这个语义——现在整份替换里，「没出现在列表里」同时表示「我删了它」和「我不知道它」。**不在当前 PR 展开的理由**：这是新机械、新语义类别（乐观并发 + 墓碑），且本地无 dotnet 与 Mongo，并发行为验证不了，靠推演上线风险高于缺口本身。**两次更正**：本条早先写着「服务端至少留下其中一份，不是数据损坏」——那句话对已读与心得是错的；改完之后仍留着「成绩最轻」的说法，那句对成绩也是错的。两次都是同一个毛病：**把一个没验证过的乐观结论写进台账**，而台账正是别人用来判断要不要现在修的依据。已改 |
| 精读稿的 SSE 还不能断线续传 | 2026-09-16 已补 10 秒 keepalive 心跳与写入串行化，且落库前要求见过终止块（Error 块与「没有终止块就结束」两条路径都挡住了），但**断了之后接不回来**：没有 Run/Worker 承载、没有 afterSeq 续传，重进页面靠的是「库里已经有了就直接读」这条兜底；客户端侧无声 EOF 时页面还会停在「生成中」 | 待办：走 Run/Worker + afterSeq（`server-authority` 规则 3/4 的完整形态）。**更正**：本条早先写着「要挡住无声中断得要求见过 Done 块，而我无法确认 http 模式下 Done 一定被转发」——那句话是错的，读一眼 `/gw/v1/stream` 就能确认它是逐块原样转发、不做过滤。我把一个查得到的事实当成了不确定性，据此记账不做；2026-09-16 第十七轮被指出后已补上 |
| 并发首次生成时，输的那一方读到的是一篇没有落库的稿子 | 两个人同时点开同一本还没有稿子的书，两边各自流式生成了**不同的**一篇。赢的那篇落库，输的那一方屏幕上是自己那篇（内容有效、材料相同，但与库里那篇不是同一份），刷新之后会变成赢的那篇。用户看到文章「换了一篇」而没有任何解释 | 待办（Codex review P2，2026-09-16）。正解是流式开始前先协调（抢占一条生成锁），或在撞键之后把赢家那篇读回来替换客户端缓冲。**不在当前 PR 展开的理由**：前者是新机械（分布式锁），后者会让读者眼前的正文突然整段替换——都要先想清楚交互，不是一行判断。当前已经不会写出两篇（唯一索引 + 撞键当赢家处理），坏的只是这一次的显示一致性 |
| 桌面档没有精读稿入口 | `getBookDigest` / `streamBookDigest` 目前只有手机档的书页在调，桌面宽度下书卡仍只有「已读」勾选与写一句心得，`?book=` 深链在桌面被忽略——本 PR 的主功能在桌面档**完全不可达** | 待办（Codex review P1，2026-09-16）。这是开工时就声明的范围边界（手机档先行），不是接线漏了；桌面形态建议复用同一套 service 走 Dialog，而不是再做一级路由。在它落地之前，桌面用户看到的藏书阁与改版前一样 |
| 结业考结果页的推荐书行点不动 | 交卷后那几行明写「建议从这两本开始」并画了向右的箭头，却没有 onClick——GroupRow 退化成普通 div，点下去什么都不发生。结果页的主要下一步是个死的 affordance | 待办（Codex review P2，2026-09-16）。要么接上「退出考试并打开这本书」的回调，要么把箭头去掉别做出可点的样子。接回调涉及考试屏与书页的导航衔接，不是一行；本轮已到收口阶段，先记账 |
| 手机档笔记只在失焦时落盘 | `onBlur` 是这个草稿唯一的持久化触发点。用户打完字后用浏览器手势返回、刷新、或直接关标签页时，输入框还聚焦着，组件卸载不保证触发 blur，那句心得就静默没了——用户以为写完了 | 待办（Codex review P2，2026-09-16）。修法是 onChange 防抖落盘，或在卸载/导航时把动过的草稿冲一次。不在当前 PR 展开：前者要和已有的 400ms 同步防抖协调（两层防抖叠在一起容易写出竞态），后者要接 React Router 的导航拦截；本轮已到收口阶段。当前影响限于「打了字但没失焦就离开」这一条路径 |
| 桌面团队看板不随进度刷新 | `useTeamBoard` 的 effect 依赖数组是空的，整屏只拉一次快照。之后标已读、写心得、交卷都成功写进了服务端，但成员行、每卷通关人数、「最薄弱的一卷」那句结论一直是旧的，要整页重载才更新——用户刚做完的事在看板上看不见 | 待办（Codex review P2，2026-09-16）。修法是进度保存成功后让看板失效或重拉。手机档看板是独立一屏、进去才拉，没有这个问题 |
| 精读稿流无声 EOF 时页面永远停在「生成中」 | 服务端或代理在发出 `done` / `error` 之前干净地关掉响应时，`connectSse` 会以 `success: true` 返回，而这一层没有记录「终止事件到底来没来」。于是书页的 phase 一直是 generating：半截正文和秒表留在屏上一直转，没有重试入口，用户只能自己刷新 | 待办（Codex review P2，2026-09-16）。修法是在包装层记一笔「见过终止事件没有」，EOF 时没见过就按失败返回。与后端那条「无声中断仍挡不住」是同一件事的两端，该一起做 |
| 配图生成中途离开设置页，产出成孤儿 | 卸载时的清理会 abort 所有生图流，但服务端的 run 继续跑完；而认领图片（把产出落到图位）只发生在被丢弃的那条流的 imageDone 回调里，下次进页面既不恢复 run id 也不恢复图位映射。**钱花了、图生出来了、却没人认领**，图位仍显示缺图，诱使管理员再花一次 | 待办（Codex review P2，2026-09-16）。正解是把认领挪到服务端完成路径，或持久化待认领的 run 下次进页面续上。这是 `server-authority` 规则 3（长任务与 HTTP 连接解耦）在这条链路上没做完，不是一行能补的 |
| 生图 run 创建结果不明时直接换模型重来，可能双倍计费 | `runOnce` 的 `idempotencyKey` 每次调用现生成，而 `if (!created.success) return targets` 之后外层循环会带着**新的**幂等键换下一个模型再发一次。创建请求其实已经到达服务端、只是回包丢了或超时的那种情况，前一条 run 仍在跑并照常计费，于是同一批图被生成两次、算两次钱 | 待办（Codex review P2，2026-09-16）。正解要么拿原幂等键重试（得先能把网络失败和服务端拒绝分开），要么先按幂等键回查那条 run 的下落再决定要不要换模型——两条都要新的服务端查询能力，不在本 PR 的范围内。与上一行同族：都是「钱花了，但账对不上」 |
| 系统配图的写入没有部署作用域，一条预览改图会改掉所有分支看到的图 | `homepage_assets` 一个 slot 一行、对象 key 也按 slot 确定性算（`icon/homepage/bookshelf/vol/boot.png`），而同一个 CDS 项目下所有分支共用同一个 Mongo 与同一个桶（`cross-project-isolation` 通道 4）。在任一条预览上生成一张藏书阁卷面图，**当场改掉所有兄弟分支页面上的那张图**——后果不是数据坏了，是把别人的验收变成谎话：他以为看到的是自己代码的产出（通道 8、通道 10 同一个形状） | 待办（Codex review P1，2026-09-16）。**这不是本 PR 引入的**——首页那七张从一开始就是这个机制，本 PR 只是往里加了藏书阁的槽位，把撞车面扩大了。正解照通道 10 的做法：记录与对象 key 都盖 `DeploymentScope.CurrentDurable`，读「自己的优先、权威的兜底」；或者干脆拒绝非权威部署写入。要动的是既有的 `homepage_assets` 契约与全部存量槽位，不是本 PR 能顺手带的 |
| 交卷成绩服务端无从核对，登录用户可以直接伪造 | `PUT /api/bookshelf/progress` 收的是聚合数字（correct / total / readAtExam），服务端只做自洽性校验，不验卷号、不验题数、不验作答、也不验真实已读进度。任何登录用户绕开界面直接发一份 `{correct:1,total:1,readAtExam:1}` 就能拿到通关，并写进全队共读的看板 | 待办（Codex review P2，2026-09-16）。要真验就得把考题搬到服务端（或改成提交作答由服务端判分），而「书目与考题的 SSOT 留在前端策展常量」是本 PR 开工时声明的不变量，改它是另一件事。现实的威胁模型也要说清：这是一块自报的学习记录，不是凭据——伪造它只骗得到自己和看板上的一行数字 |
| 「重写一篇」对所有登录用户开放 | 精读稿生成端点的 `force=true` 会跳过缓存、用新生成的那篇替换本部署的**公共**稿子，而手机端书页把这个操作以「重写一篇」直接给了每一个人。任一普通用户都能反复触发付费生成，并替换全队正在读的内容 | 待办（Codex review P1，2026-09-16）。正解是给强制重写加管理员或内容管理权限，普通的缓存未命中生成不受影响。**不在当前 PR 展开的理由**：这要引入一条新的权限判定与相应的 UI 分支（谁看得见那个按钮），属于新语义类别；且本轮已到收口阶段。当前缓解是它只在已登录范围内，且稿子可再次重写 |
| 手机档卷页不带角色筛选 | 落地页选了「开发者」或「产品经理」之后，计数按角色筛过了，但点进卷页仍然列出 `volume.books` 的全部条目——手机壳层从没把选中的角色传下去（桌面档有）。第一次导航就把用户的筛选悄悄丢了，显示另一条线的书 | 待办（Codex review P2，2026-09-16）。修法是把角色透下去并在卷页套同一个 matchRole 判据 |
| 藏书阁注册为正式 infra 入口而非 wip | AGENTS.md §9 要求新功能默认进百宝箱并带 `wip: true`，通过规则 #8 验收后才转正。本 PR 把它直接注册成了 `infra` 段的正式入口，而桌面档主功能仍完全不可达 | 待办 / 待定（Codex review P1，2026-09-16）。改成 wip 只需一个字段，但它改变功能在导航里的呈现位置，属于产品决定而非缺陷修复，留给维护者定 |
| 精读稿不校验流是否有终止块（Done） | 现在挡住的是网关明确发 Error 块的那种中断（`LlmGateway` 中途断流走的正是这条）。若上游连接无声中断、既不抛异常也不发 Error 块，`await foreach` 会正常结束，半篇稿子照样会被当成写完了落库 | 待办：落库前要求见过一个终止 Done 块。**本轮没做的理由是不能验证**——生产走的是 http 模式，`HttpLlmGatewayClient` 转发的是 serving 序列化过的块，我无法在本地确认 Done 在这条路径上一定被转发；假设错了的后果是每一篇都存不下，比现在这个残留缺口严重。要做得先在预览上记一轮真实块序列 |
| 精读稿第一、二段没有书本正文作依据 | 喂给模型的材料只有书名、作者、编者推荐理由与一句收获，正文一个字都没有，而提示词要求给出书中的具体做法、术语与例子。冷门书上模型只能靠记忆，记忆不可靠时就会编，而编出来的东西会作为公共学习材料落库。2026-09-16 已给第一段补上与第三段对称的逃生口（没把握就明说、只按编者视角谈），但那是**约束模型自觉**，不是给它依据 | 待办：给书目补一份可核对的内容梗概（编者手写或检索得来）作为第一段的材料。这会引入「书本内容」这一层新材料与它的维护成本，按 5.5 属于 B 类，不在当前 PR 展开 |
