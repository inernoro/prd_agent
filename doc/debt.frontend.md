...
审计工具本身在 PR #1374 被自动评审连着修了六轮。下面只留**尚未解决、会影响结论可信度**
的边界；过程、方法与计数不进本文档（AGENTS.md §10：评测产出归验收知识库）。

| 未解边界 | 对结论的影响 |
|---|---|
| 去重早于采样 | 重复卡片只有第一个进采样；若后续实例压在渐变另一段且第一个恰好达标，后者测不到 |
| 众数底而非最差底 | 字跨非匀渐变时，大片安全区赢下直方图会掩盖小段低对比 |
| ~~「按主题分支跳过」是文件级~~ | **已修，且我此前那句「实测敞口为 0」是错的**：文件级豁免连深色前景一起放走，而深色前景只可能是浅色档的值。实测藏了 report-agent 三档真缺陷（沟通 4.09 / 文档 4.00 / Todo 4.36）与海鲜市场接入 AI 按钮 hover（4.38）。现按前景亮度收窄：亮度 < 0.30 的仍判（浅色档 700 在 0.14~0.16，暗色档 300 在 0.52~0.58，分得很开） |
| 具名 style 对象里的暗底 | 「给暗岛补 surface-tone-dark」的自动扫描按「同一开标签」判定，够不着这种写法，只能手工 grep |
| SVG 多色图标只判根节点 | 已修：根 fill 是 UA 默认黑或 none 时改为遍历上色子形状。副作用两类已处理——paint server（`url(#grad)`）标 unresolved 而不是硬算成假的 1:1，低透明度描边（进度环底槽这类纯装饰）不按 3:1 判 |
| 样式表里 452 处存量硬编码 | 双皮肤棘轮此前只扫 `.ts/.tsx`，19 个 `.css` 从来不在覆盖内（surface.css 里就查出过真缺陷）。已把 `.css` 纳入并录基线：新增会红，存量 452 处按「走到哪清到哪」偿还 |
...
**结论怎么用**：同一把尺子前后对比是可靠的；用它宣称「全站零缺陷」不可靠 ——
「文字压非匀渐变」这一类仍可能少报。
