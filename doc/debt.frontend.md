...
| PUT 失败只记 console 不提示用户 | 网络断了时用户以为标记成功了，实际只在本地 | 待办：失败后在页面角上给一条「本次未同步」的轻提示，下次操作自动重试（快照已是完整的，重试天然幂等） |
| 团队看板拉全表，没有分页 | 成员规模到几百人时首屏会变慢 | 待办：`GET /api/bookshelf/team` 加 limit/cursor，或只返回聚合 + Top N |
| 卷六「驭 AI」偏模型原理，缺「怎么审 AI 写的代码」 | 用户原话是「没怎么看 AI 写的代码，AI 都在乱写」——那是代码审查问题，这一卷现在只解释了 AI 为什么必然乱写，没教怎么设卡 | 待办：补两三本工程向的（AI 辅助开发的评审实践、生成代码的测试策略） |
