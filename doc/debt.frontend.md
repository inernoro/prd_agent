...
| `relatedRules` 改了，稿子不会自动失效 | staleness 只看 `PromptVersion`——提示词没改，它就认为库里那篇还新鲜。可是第三段的内容全部来自 `relatedRules` 指到的规则：给一本书改挂、增删规则之后，库里那篇仍是用旧材料写的，读者看到的第三段与当前挂的规则对不上，而且没有任何东西提示它过期了。2026-09-16 给《动手学深度学习》补第二条规则时发现这个缺口——当时以为它库里有旧稿、会照旧显示单条材料写的那篇，核对后发现这本书从没被人点开过，库里本来就是空的，所以这一次并没有真的踩到。缺口本身成立（判据里确实没有材料这一维），但**目前还没有真实事故**：51 本里已生成的只有 5 本，改挂规则撞上已有稿子是迟早的事 | 待办：staleness 判据改成「提示词版本 + 这本书那份材料的内容指纹」，两者任一变化即不复用。指纹可以在生成 `bookshelf-context.json` 时按书算一个 hash 一并写进去，`BookDigest` 存下当时那个值——判据从「人记得改版本号」变成「材料变了自动对不上」。在它落地之前，改完 `relatedRules` 必须记得手动重跑受影响的那几本 |
