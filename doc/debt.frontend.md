...
| 并发 / 离线保存会丢掉另一台设备的已读与心得（成绩只是其中最轻的一项） | `PUT /api/bookshelf/progress` 是读-改-写，而且**整份替换**：`ReadBookIds` 是整条列表 `Set`、`BookNotes` 是整张表 `Set`。两台设备从同一份基线各改各的，或者离线设备联网后补推，后写的那一发会**静默删掉只存在于另一份快照里的已读标记与心得**——那些数据没有第二个副本。成绩那一项反而是最轻的：它至少走「取更好的那次」合并，服务端会留下其中一份；已读与心得连这层保护都没有。**这打的是本功能自己许下的承诺**：「跨设备取更好的那次」写在代码注释和 [design.prd-agent.bookshelf.exam.md](./design.prd-agent.bookshelf.exam.md) 里，而它在并发下不成立 | 待办（Codex review P1，2026-09-16）。正解是按修订号的 CAS + 冲突调和，或按书的原子写 + 显式删除墓碑；两者都要先定义「删除」这个语义——现在整份替换里，「没出现在列表里」同时表示「我删了它」和「我不知道它」。**不在当前 PR 展开的理由**：这是新机械、新语义类别（乐观并发 + 墓碑），且本地无 dotnet 与 Mongo，并发行为验证不了，靠推演上线风险高于缺口本身。**更正**：本条早先写着「服务端至少留下其中一份，不是数据损坏」——那句话只对成绩成立，对已读与心得是错的，会让读这份台账的人低估它。已改 |
