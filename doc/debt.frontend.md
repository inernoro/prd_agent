...
| 精读稿的 SSE 还不能断线续传 | 2026-09-16 已补 10 秒 keepalive 心跳（长静默不再被 nginx/CDN 按空闲超时掐断）与写入串行化，但**断了之后接不回来**：没有 Run/Worker 承载、没有 afterSeq 续传，重进页面靠的是「库里已经有了就直接读」这条兜底 | 待办：走 Run/Worker + afterSeq（`server-authority` 规则 3/4 的完整形态）。这是一次架构改造，不在本 PR 范围。当前的兜底能覆盖绝大多数情况——稿子是公共内容，写完就落库，断线的那个人重进就能读到 |
