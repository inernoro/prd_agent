...
- 评论、每日输出和验收报告必须引用缺陷原始内容，不能只写缺陷编号、修复标题或截图。
...
- 标记缺陷为已解决。
- 更新 run item 为 `fixed`。
- 生成缺陷完成评论，评论必须包含缺陷编号、标题、用户原始问题摘录、修复目标、PR/commit 证据、验收地址和发布状态。
...
| 验收事项清单 | 从缺陷内容和 PR/commit 总结出的 3-8 条验收事项，每条包含预期结果和证据位置 |
...
- 无缺陷、无待验收通知项时的正常结束规则。
- 每日输出字段：runId、缺陷引用（defectNo、标题、用户原始问题摘录）、处理结果、commit/PR 可点击证据、预览地址、验收事项清单、验收报告、通知结果、阻塞项。
