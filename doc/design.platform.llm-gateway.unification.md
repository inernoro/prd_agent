...
> **关联债务**：[debt.platform.llm-gateway.md](./debt.platform.llm-gateway.md)
> **一句话**：把分散的模型调用统一到一个网关：三级模型池调度、平台适配、健康管理与统一日志。
> **谁该读**：接手网关的工程师。
> **读完能做什么**：说清模型是怎么被三级调度选出来的。
...
- **G1-G5 函数调用穿协议** —— 已落地核心路径（2026-06-26）。取证：Open Platform 的 `/api/v1/chat/completions`（`OpenApiController.cs`）是 **OpenAI 兼容代理**，`RequestBody = body` 全量透传客户端请求体——函数调用客户端**会**传 `tools`/`tool_choice`。修复分层：
  ...
  - **已知边界**（见 [doc/debt.platform.llm-gateway.protocol-fidelity.md](./debt.platform.llm-gateway.protocol-fidelity.md)）：Claude 流式 tool_use 增量未映射；能力软门池路径为 null（best-effort 放行）；Extensions 容器已建未消费。
...
§10 与 [debt.platform.llm-gateway.md](./debt.platform.llm-gateway.md) 挖出的每个意外，落一条**命名回归测试**（不只债务条），撞过的坑永不复发：
...
- [debt.platform.llm-gateway.md](./debt.platform.llm-gateway.md)——本次迁移的已知边界台账
