...
test.describe('录音连续性发布门禁', () => {
  ...
  test('长原文不遮挡控制区，结束后直达可播放结果，刷新不发生页面循环', async ({ page }, testInfo) => {
    ...
    const nowPlaying = page.getByTestId('recording-karaoke-now-playing');
    const activeCue = page.getByTestId('recording-karaoke-active-cue');
    const nextCue = page.getByTestId('recording-karaoke-next-cue');
    await expect(nowPlaying).toBeVisible();
    await expect(activeCue).toContainText('验收注入第 1 段');
    await expect(nextCue).toContainText('验收注入第 2 段');
