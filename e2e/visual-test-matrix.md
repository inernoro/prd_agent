...
对应提交: `e55ea33d fix: clarify branch deploy runtime state`
当前追加验证提交: 待提交
...
| V-02 | 验证发布版仍有明确标识 | 登录 CDS，打开分支列表，扫描发布/混合运行模式文案 | 发布版显示“发布版”badge；混合模式显示“混合”badge | 已覆盖但环境依赖：当前测试环境未稳定命中发布/混合卡片时自动 skip；需保留作发布分支存在时的回归项 |
...
| V-05 | 验证详情抽屉部署卡片耗时可见 | 从分支列表点击卡片打开右侧详情抽屉，进入“部署”tab | 右上角有“已用/耗时 xx”胶囊，不再只是弱灰小字 | 部分通过：截图可见“耗时 1m 18s”；后续由 V-06 一起复测 |
| V-06 | 验证详情抽屉挂载真实容器日志 | 从分支列表点击卡片打开右侧详情抽屉，进入“部署”tab，查看阶段树下的“容器日志” | 存在可展开“容器日志”区域；running 时默认展开，success/error 时可展开查看 | 失败后已修：实测抽屉没有 `summary 容器日志`；已补诊断 tail log 兜底，待发布后复测 |
